...
description: >-
  [NeurIPS 2025][因果推理][多模态因果发现] 提出 MLLM-CD 框架，首次实现从多模态非结构化数据（文本+图像）中进行因果发现，通过对比因子发现识别因果变量、统计方法推断因果结构、迭代多模态反事实推理消除结构歧义。 因果发现旨在从数据中推断因果结构，是科学进步的基础。传统因果发现方法依赖预定义的结构化变量…
tags:
  - "NeurIPS 2025"
  - "因果推理"
  - "多模态因果发现"
