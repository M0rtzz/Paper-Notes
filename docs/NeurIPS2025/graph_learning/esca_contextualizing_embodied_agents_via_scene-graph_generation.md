...
1. **细粒度视觉-语义关联不足**：MLLM 难以可靠地建立低层视觉特征与高层文本语义之间的联系，导致空间和时间上的视觉定位能力薄弱
2. **感知错误是主要失败原因**：实证分析表明，高达 69% 的智能体失败源于感知错误（如物体幻觉、实体误识别、空间关系错误等）
3. **现有视觉增强模块的局限**：Grounding DINO、YOLO 等物体检测模型主要关注物体识别，忽略了语义属性、物体间关系和时间一致性
