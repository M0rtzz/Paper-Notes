...
1. **计算成本高**: LiRA 需要训练大量阴影模型来估计成员与非成员的损失分布，计算开销巨大
2. **图数据场景缺乏理论基础**: 针对图神经网络（GNN）的节点级 MIA 缺乏最优查询策略的理论分析
3. **理论与实践脱节**: 现有贝叶斯决策理论框架（Sablayrolles et al.）仅适用于 i.i.d. 数据，未扩展到图结构数据
