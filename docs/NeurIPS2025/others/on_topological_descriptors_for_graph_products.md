...
## 研究背景与动机
1. **领域现状**：拓扑数据分析（TDA）中的持续同调（Persistent Homology, PH）和欧拉特征（Euler Characteristic, EC）已广泛用于捕获关系数据的多尺度结构信息，尤其在图神经网络表达能力受限时提供互补视角。
2. **现有痛点**：对单图施加滤过计算 PH/EC 已有研究，但如何通过**图积**（graph product）增强拓扑描述子的表达能力缺乏理论分析。
3. **核心矛盾**：EC 计算高效但表达力弱，PH 表达力强但计算开销大。图积是否能同时提升两者的判别力？
4. **切入角度**：从理论角度完整刻画 EC 在通用色基滤过下的表达力上界，并证明 PH 通过图积获得严格更强的信息。
5. **核心idea一句话**：利用图的 box 积构造丰富的滤过空间，使 PH 描述子获得严格更强于单图上计算的判别能力。
