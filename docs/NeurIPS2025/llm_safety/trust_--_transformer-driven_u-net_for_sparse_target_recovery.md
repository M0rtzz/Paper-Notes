...
1. **感知矩阵未知**: 多数传统方法假设 $\mathbf{A}$ 已知，但实际应用中往往未知
2. **训练数据有限**: 只有少量观测-目标对 $\{(\mathbf{x}, \mathbf{y})\}$ 可用
3. **幻觉伪影**: 深度学习方法容易产生与真实信号不一致的幻觉
4. **局部-全局特征**: 纯 U-Net 缺乏全局感受野，纯 Transformer 缺乏多尺度局部特征
