---
title: "[论文解读] SECA: Semantically Equivalent and Coherent Attacks for Eliciting LLM Hallucinations"
description: "[NeurIPS 2025][AI安全] 提出SECA框架，通过语义等价且连贯的对抗性提示变换诱发LLM幻觉"
tags: [NeurIPS 2025, AI安全, LLM幻觉, 对抗攻击, prompt robustness]
---
...
## 研究背景与动机
1. **领域现状**：LLM 在高风险领域的部署日益增多，但幻觉（hallucination）问题严重威胁可靠性。
2. **现有痛点**：已有对抗攻击方法依赖不现实的提示（插入无意义 token 或改变原始语义意图），无法揭示现实场景中幻觉的产生机制。
3. **核心矛盾**：CV 中对抗攻击通常涉及现实的输入修改，但 NLP 中缺乏对应的现实主义对抗提示研究。
4. **切入角度**：将寻找现实对抗提示形式化为带语义等价和连贯性约束的优化问题。
