...
1. **计算和内存开销高**: DP-SGD 需要对每个样本单独计算梯度（per-sample gradient），然后裁剪和加噪
2. **零阶方法的潜力**: 零阶（zeroth-order）方法通过函数值评估近似梯度，天然容易私有化，但效用较低
3. **公共数据未被充分利用**: 实际中常有公共数据（如预训练数据），但现有零阶方法未利用这一信息
