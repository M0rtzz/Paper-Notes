...
description: >-
  [NeurIPS 2025][持续学习][模型反演] 提出 PMI（Per-layer Model Inversion）方法，通过逐层优化加速模型反演，结合类别级特征的高斯建模和对比学习实现合成-真实特征对齐，在无数据持续学习中取得强效果。
tags:
  - NeurIPS 2025
  - AI安全
  - 持续学习
...
1. **领域现状**：持续学习（Continual Learning, CL）旨在让模型在学习新任务时保持对旧任务的性能。传统方法依赖存储和重放旧数据，但在实际场景中常因隐私法规（如 GDPR）或安全约束无法实现。
2. **无数据持续学习**：Data-free CL 在不存储任何旧数据的前提下进行增量学习。核心思路：通过模型反演（Model Inversion）从已训练模型中合成伪数据用于回放。
3. **现有痛点**：
...
4. **核心挑战**：如何在保持合成数据质量的同时提升模型反演的效率？
5. **切入角度**：受单层优化收敛更快的启发，提出逐层反演策略作为全模型反演的初始化；同时用概率模型约束中间特征分布。
