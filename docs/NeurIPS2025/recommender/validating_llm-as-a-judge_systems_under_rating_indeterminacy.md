...
1. **评分不确定性 (Rating Indeterminacy)**: 对于很多评估条目，评分标准允许多种合理解释，人类评分者和 LLM 可能对同一条目给出不同但都"正确"的评分
2. **强制选择偏差**: 现有方法要求评分者只选一个评分（forced-choice），掩盖了评分的内在不确定性
3. **验证失真**: 人类和 LLM 在处理不确定性时的方式不同，导致基于强制选择评分的验证严重偏倚
