...
## 研究背景与动机
1. **领域现状**：部分可观测马尔可夫决策过程（POMDP）中的最优决策要求智能体平衡减少不确定性（探索）与追求即时目标（利用）。
2. **现有痛点**：现有连续 POMDP 策略优化方法要么使用次优近似（如信念点方法），要么依赖手工启发式奖励塑造来鼓励探索。
3. **核心矛盾**：POMDP 的信念空间是无限维的，直接优化计算不可行；但简化近似会丢失信息收集的价值。
4. **切入角度**：将策略学习转化为概率推断问题，通过 Feynman-Kac 模型自然编码信息价值。
