...
## 研究背景与动机
1. **领域现状**：激励设计（Incentive Design）旨在为多智能体系统设计激励机制以诱导理想的 Nash 均衡，广泛应用于拍卖、定价和交通管控。
2. **现有痛点**：当智能体数量$N$很大时，直接求解$N$-玩家博弈的计算复杂度极高；现有方法难以扩展到大规模场景。
3. **核心矛盾**：有限$N$博弈的精确优化不可行，但简单的平均场近似缺乏严格的近似保证。
4. **切入角度**：在可交换性（exchangeability）假设下，将 ID 问题重塑为参数化平均场博弈，利用无限种群极限降低复杂度。
5. **核心 idea 一句话**：平均场博弈 + 伴随方法高效梯度计算 = 大规模激励设计的可扩展解法。
