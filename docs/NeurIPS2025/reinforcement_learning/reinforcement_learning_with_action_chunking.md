...
关键动机：
1. **探索困难**: 长 horizon 稀疏奖励任务中,随机探索几乎不可能到达目标
2. **离线数据利用不足**: 现有方法在利用离线数据获得良好探索策略方面不够有效
3. **动作分块的 RL 潜力未被开发**: 在模仿学习中,ACT等方法已证明动作分块的价值,但 TD 学习中尚未系统研究
4. **时序一致性**: 逐步预测动作导致行为的时序不连续,不利于机器人操作等任务
