...
关键问题：
1. **SFT 的局限性**: 现有个性化方法主要依赖监督微调(SFT),但大规模高质量个性化描述数据**获取成本高昂**
2. **多概念困难**: 当图像中包含多个需要个性化识别的概念时,SFT方法频繁失败
3. **视觉识别不足**: 即使在高质量数据上训练,模型在真实场景中仍经常无法正确识别特定实体
4. **SFT 的数据瓶颈**: SFT 的性能天花板受限于训练数据质量,而 RL 可以超越这一限制
