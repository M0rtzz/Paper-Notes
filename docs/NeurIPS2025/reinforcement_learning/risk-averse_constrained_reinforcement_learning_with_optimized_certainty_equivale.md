...
description: >-
  [NeurIPS 2025][强化学习][约束强化学习] 提出一种基于奖励层面(reward-based)的风险感知约束RL框架，使用优化确定性等价(OCE)风险度量同时覆盖目标和约束，建立了参数化强对偶性，并给出模块化算法——可包装标准RL求解器（如PPO）作为黑盒使用。 约束RL是处理多冲突目标的常见框架（如迷宫导航中既…
tags:
  - "NeurIPS 2025"
  - "强化学习"
  - "约束强化学习"
