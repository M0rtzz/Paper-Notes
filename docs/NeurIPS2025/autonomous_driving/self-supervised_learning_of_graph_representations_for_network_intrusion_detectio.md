...
## 研究背景与动机
1. **领域现状**：网络入侵检测在有限标注和不断演变的攻击模式下极具挑战，图神经网络（GNN）近年被引入该领域。
2. **现有痛点**：现有 GNN 方法将表示学习与异常检测解耦，导致学到的嵌入并非为识别攻击而优化。
3. **核心矛盾**：需要无监督/自监督方法（因为攻击类型不断变化，标注数据不足），但表示学习目标与检测目标不一致。
4. **切入角度**：将图表示学习与异常检测统一到端到端框架中，使嵌入直接优化于下游检测任务。
