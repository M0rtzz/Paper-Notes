...
1. **3D 构象不可靠性**: 3D 结构通常通过 RDKit 等工具从 2D 结构生成，存在噪声和不确定性。不同的构象生成方法可能产生差异很大的 3D 结构，导致下游性能不稳定。

2. **模态坍缩（Modality Collapse）**: 天真的融合方法（如简单拼接或相加）往往导致模型过度依赖某一模态而忽略其他模态的信息，使得多模态融合退化为单模态学习。
