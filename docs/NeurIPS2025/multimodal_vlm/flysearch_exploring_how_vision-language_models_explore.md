...
1. **环境限制**：多数 ObjectNav 基准（Habitat、AI2-THOR）聚焦室内场景
2. **系统评测 vs 模型评测**：已有工作多将 VLM 作为组件构建复杂系统，而非评测 VLM 本身的探索能力
3. **缺乏零样本开放性**：多数方法依赖特定目标检测器，不适合开放世界搜索
4. **视觉真实感不足**：部分基准使用简化的视觉渲染
