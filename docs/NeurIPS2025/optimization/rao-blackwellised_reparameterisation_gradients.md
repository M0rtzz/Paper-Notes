...
核心挑战：
1. **方差仍有改进空间**: 重参数化梯度虽优于 REINFORCE，但在复杂模型中方差仍然较高
2. **Rao-Blackwell 化机会**: 利用条件期望降低方差是经典统计思想，但如何系统应用于重参数化梯度尚不清楚
3. **局部重参数化的理论理解不足**: 局部重参数化（Kingma et al.）在 Bayesian MLP 中表现出色，但其与 Rao-Blackwell 化的联系未被揭示
