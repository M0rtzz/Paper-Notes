...
## 研究背景与动机
1. **领域现状**：高维非凸优化问题（如深度学习训练）的最小值点通常形成光滑流形，源于模型的过参数化或对称性。
2. **现有痛点**：标准非凸优化分析只关注一般性收敛保证，忽视了解结构带来的加速潜力。
3. **核心矛盾**：实践中利用结构信息的算法经验上更快收敛，但缺乏统一的理论解释。
4. **切入角度**：若最优解集的几何结构（至少局部）已知，可通过 reduction mapping 重参数化去除冗余方向。
