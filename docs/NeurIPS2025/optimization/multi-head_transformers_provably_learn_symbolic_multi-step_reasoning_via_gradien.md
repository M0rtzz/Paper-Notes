...
description: >-
  [NeurIPS 2025][理论分析][Transformer推理] 从梯度下降动力学角度严格证明单层多头 Transformer 可学会符号化多步推理任务（树路径查找），揭示不同注意力头的自主分工和协调机制。
tags:
  - NeurIPS 2025
  - 优化理论
  - Transformer
...
1. **领域现状**：Transformer 在多步推理任务中展示了出色能力，Chain-of-Thought（CoT）提示进一步释放了这种能力。但我们对 Transformer "如何通过训练习得推理能力"的理解仍非常有限。
2. **理论缺口**：
...
4. **切入角度**：选择树路径查找（Tree Path-Finding）作为符号化多步推理的抽象任务模型——结构清晰、可分析、同时捕获推理的关键要素。
