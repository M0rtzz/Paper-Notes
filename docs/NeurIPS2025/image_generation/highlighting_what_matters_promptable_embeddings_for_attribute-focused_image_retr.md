...
1. **CLIP类检索器的局限**：由于效率和零样本能力被广泛采用，但其图像嵌入侧重于全局语义和主体对象，在处理属性聚焦查询时表现不佳且不均衡
2. **MLLM类检索器的不足**：即使是基于多模态大语言模型(MLLM)的更强检索器，尽管输出维度更大，在属性聚焦查询上仍然力不从心
3. **根本问题**：使用通用图像嵌入进行属性聚焦检索本身就是次优的——一个固定的嵌入难以同时对所有可能的属性查询最优
