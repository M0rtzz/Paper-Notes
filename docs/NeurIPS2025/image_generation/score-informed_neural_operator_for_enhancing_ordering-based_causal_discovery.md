---
title: "[论文解读] SciNO: Score-informed Neural Operator for Causal Discovery"
description: "[NeurIPS 2025][因果发现] 提出Score-informed Neural Operator在光滑函数空间中稳定逼近Hessian对角，提升排序式因果发现性能"
tags: [NeurIPS 2025, 因果发现, 神经算子, score matching, 因果推理]
---
...
## 研究背景与动机
1. **领域现状**：排序式因果发现通过识别因果图的拓扑序来锚定因果结构，是组合搜索方法的可扩展替代方案。
2. **现有痛点**：在加性噪声模型（ANM）假设下，因果排序方法需要精确估计 log-密度的 Hessian 对角。Stein 梯度估计计算昂贵、内存密集；扩散模型方法因二阶导数不稳定。
3. **核心矛盾**：需要 Hessian 对角的精确估计，但现有方法要么昂贵要么不稳定。
4. **切入角度**：在光滑函数空间中设计概率模型，保留结构信息的同时稳定近似 Hessian。
