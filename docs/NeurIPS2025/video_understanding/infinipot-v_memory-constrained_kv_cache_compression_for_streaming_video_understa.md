...
description: >-
  [NeurIPS 2025][视频理解][KV Cache压缩] 提出首个无需训练、查询无关的流式视频理解框架InfiniPot-V，通过时序冗余度(TaR)和值范数(VaN)两个度量在视频编码过程中实时压缩KV缓存，强制维持固定内存上限。在4个开源MLLM和4个长视频基准上，GPU内存降低最高94%同时保持或超越全缓存精度。
tags:
  - NeurIPS 2025
  - 视频理解
  - KV Cache压缩
...
这一问题在以下场景尤为突出：
1. **移动端/边缘设备**：手机、AR眼镜、机器人等设备的显存固定且有限
2. **流式视频**：视频长度未知且持续增长，无法预先分配内存
3. **多轮对话**：用户可能在视频播放过程中多次提问，缓存需要长期维护
