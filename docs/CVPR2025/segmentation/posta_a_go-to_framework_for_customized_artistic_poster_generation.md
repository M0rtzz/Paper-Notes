...
- 海报设计是视觉传达的关键媒介，在广告、教育和艺术领域有广泛需求
- 现有自动海报生成方法面临四大挑战：文字生成不准确（拼写错误、字符扭曲）、设计元素控制不灵活（"一键生成"范式缺乏细粒度调整）、美学质量不足、缺乏系统化工作流
- 艺术海报（电影、展览、演唱会等）对内容准确性和视觉冲击力有双重要求
- 已有端到端方法如 COLE 虽然涵盖完整流程，但过程复杂、需要大量中间检查
- 现有文字生成模型在处理长文字序列时仍容易产生乱码和不可读字符
- 需要一个模块化、可控、高美感的系统化解决方案
