...
1. **数据规模庞大**：动作分割数据集通常包含长时间的连续视频。例如Breakfast数据集包含约28GB的特征数据，50Salads数据集也有约4.5GB
2. **标注成本极高**：需要对视频中每一帧进行动作类别标注，这比图像分类的标注要昂贵得多
3. **隐私与分发问题**：在某些领域（如医疗、工业），原始视频数据可能涉及敏感信息，无法自由分发
4. **现有蒸馏方法的局限**：图像级别的数据集蒸馏方法（如Dataset Distillation）难以直接应用于时序数据，因为动作分割涉及长序列的时间结构和动作转换模式
5. **时序一致性**：蒸馏后的数据需要保持动作的时序结构，简单的帧级采样会破坏动作的连续性和转换模式
