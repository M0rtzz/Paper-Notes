...
1. **固定分辨率的局限**：ViT 通常将输入图像 resize 为固定尺寸（如 224×224），这会破坏图像的原始构图和长宽比，而这些恰恰是美学评估的关键因素
2. **高分辨率的计算代价**：直接处理高分辨率图像会导致 token 数量急剧增加，注意力计算复杂度为 $O(n^2)$，实际部署困难
3. **构图信息丢失**：传统的 resize 和 center crop 会改变主体位置关系和前景/背景比例，导致模型无法学习到真正的构图美学
4. **多尺度信息缺失**：美学评估需要同时关注全局布局（构图、色彩分布）和局部细节（纹理、锐度），单一尺度的 ViT 难以兼顾
