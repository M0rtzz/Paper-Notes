...
description: >-
  [CVPR 2025][模型压缩][传输矩阵] 本文从频域角度分析了多层传输矩阵逼近的理论基础，揭示了显微镜中的"缺失锥"问题在波前整形场景下反而成为优势，证明少量 SLM 层即可在有限视场内实现有效散射校正。 领域现状：波前整形（wavefront shaping）是深层生物组织成像的核心技术。通过在光路中放置空间光调制器…
tags:
  - "CVPR 2025"
  - "模型压缩"
  - "传输矩阵"
