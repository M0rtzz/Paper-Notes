...
1. **领域现状**：预训练模型+参数高效微调（adapter/prompt）已成为类增量学习(CIL)的主流范式。EASE为每个任务训练独立adapter并用各adapter提取原型做cosine分类，精度SOTA但推理时需逐adapter前向，复杂度O(T)。

2. **现有痛点**：现有方法存在精度-速度权衡——SimpleCIL/APER推理O(1)但精度有限（仅第一任务adapter），EASE精度SOTA但40个任务时推理慢39倍。隐私场景下不能存储旧任务样本（exemplar-free），进一步增加难度。

3. **核心矛盾**：adapter合并（权重平均）后旧任务原型在新子空间中位置发生偏移，原型不再准确。但没有旧任务数据无法重新计算原型。

4. **核心idea**：利用当前任务数据（唯一可用数据）在新旧子空间中的原型偏移作为线索，用仿射变换近似将旧原型映射到新子空间。
...
2. **Centroid Prototype Mapping**：
   - 做什么：在无旧任务数据的情况下，将旧原型映射到当前merged adapter子空间
