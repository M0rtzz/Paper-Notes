...
- 传统3D重建方法（SfM、SLAM、NeRF等）从头开始处理每个场景（tabula rasa），难以应对稀疏观测、退化运动等情况
- DUSt3R等学习方法仅能处理图像对，扩展到多视图需要耗时的全局对齐，且无法在线更新
- Spann3R虽支持连续重建，但其空间记忆仅作为缓存，不能推断未观测区域
- 人类是在线视觉学习者：持续处理视觉流、累积观测、利用先验知识推断遮挡区域
- 需要一个统一框架：(1)从少量观测重建3D，(2)随新观测持续细化，(3)推断未观测区域
