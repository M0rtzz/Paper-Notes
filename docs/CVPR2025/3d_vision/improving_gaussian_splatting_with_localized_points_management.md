...
description: >-
  [CVPR 2025][3D视觉][3D高斯溅射] 本文提出局部化点管理（LPM）策略，通过多视角几何约束定位导致渲染误差的 3D 区域，在这些区域内执行针对性的点加密和不透明度重置，作为即插即用模块可提升多种 3DGS 模型的重建质量，同时保持实时渲染速度。 领域现状：3D 高斯溅射（3DGS）通过 SfM 初始化 3D…
tags:
  - "CVPR 2025"
  - "3D视觉"
  - "3D高斯溅射"
