...
- 点云补全是 3D 视觉中的关键任务，但单视图部分点云常缺失超过一半的形状信息，解空间巨大
- 现有方法从部分点云提取特征直接预测缺失区域，在严重遮挡情况下（如缺失顶部灯罩的台灯）会产生随机猜测
- 利用图像作为额外引导虽能改善效果，但获取配对的图像-部分点云数据在实践中困难
- 大规模多视图扩散模型生成的视图一致图像编码了全局和局部形状线索，特别有利于形状补全（如捕获对称结构）
- 但扩散生成的多视图图像存在固有不一致性，直接融合会引入噪声和不可靠点
