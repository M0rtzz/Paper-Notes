...
- 迭代式方法（如RAFT-Stereo、IGEV-Stereo）已成为立体匹配主流，但严重依赖标注数据
- 高质量立体标注获取成本高昂，现有标注数据多为合成数据，在真实场景泛化性差
- 已有自训练方法仅适用于代价体方法，无法直接应用于无完整代价体的迭代式方法
- 已有伪标签过滤策略采用硬阈值二值选择，既丢弃有价值的困难样本，又无法区分阈值内伪标签的可靠性差异
- 核心观察：**误差较大的区域在模型预测中表现出更明显的振荡特性**
