...
description: >-
  [CVPR 2025][3D视觉][相对位姿估计] 本文提出三个新的相对位姿求解器，通过显式建模单目深度预测的仿射（尺度+偏移）模糊性来利用深度先验，并设计混合估计框架将深度感知求解器与经典点求解器结合，在标定和非标定设置下均大幅提升位姿估计精度。 领域现状：单目深度估计（MDE）模型近年取得巨大进步，从 Marigold…
tags:
  - "CVPR 2025"
  - "3D视觉"
  - "相对位姿估计"
