...
description: >-
  [CVPR 2025][3D视觉][3D编辑] 本文提出 PrEditor3D，一种免训练的 3D 编辑方法，通过同步多视图扩散编辑+前馈 3D 重建的管线，结合基于颜色编码的 3D 分割和体素特征融合，实现了快速（数分钟内）、精确（仅修改目标区域）的高质量 3D 形状编辑。 - 3D 编辑的实际需求：3D 内容编辑是动画、…
tags:
  - "CVPR 2025"
  - "3D视觉"
  - "3D编辑"
