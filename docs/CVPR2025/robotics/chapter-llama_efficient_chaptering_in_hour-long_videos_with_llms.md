...
1. **长视频处理的计算瓶颈**：小时级视频包含数十万帧，即使以低帧率采样（如 1fps），仍有数千帧需要处理。现有 Video LLM 的上下文窗口和计算资源难以支撑如此大规模的视觉输入
2. **均匀采样的低效性**：传统方法等间隔采样固定数量的帧（如100帧），但章节边界处的信息密度远高于章节内部，均匀采样大量浪费计算在冗余帧上
3. **视觉与语音信息的互补性未被充分利用**：长视频通常包含丰富的语音信息（如讲解、对话），这些语音信号天然地标记了内容的语义转换点，但现有方法主要依赖视觉特征
4. **现有方法的局限**：
