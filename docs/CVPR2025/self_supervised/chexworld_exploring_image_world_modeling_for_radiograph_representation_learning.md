...
1. **解剖结构的固定性**：与自然图像不同，胸片中的器官位置相对固定（心脏在左下、肺野在两侧），模型需要理解这种固定的空间布局。
2. **病灶的局部性**：大多数病变（结节、浸润等）仅占图像的很小区域，全局特征学习可能忽略这些关键局部。
3. **域差异的多样性**：不同设备、不同拍摄参数下的同一患者胸片可能差异极大，模型需要对这些域变化保持不变性。
