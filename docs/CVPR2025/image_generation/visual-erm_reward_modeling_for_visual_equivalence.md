...
description: >-
  [CVPR2025][图像生成][奖励模型] 提出 Visual-ERM，一个多模态生成式奖励模型，在视觉空间中直接评估 vision-to-code 任务的渲染质量，提供细粒度、可解释、任务无关的奖励信号，用于 RL 训练和测试时缩放。 Vision-to-code 的重要性：将结构化视觉输入（图表、表格、SVG）转为可执…
tags:
  - "CVPR2025"
  - "图像生成"
  - "奖励模型"
