...
1. **时序一致性**：逐帧着色容易产生颜色闪烁（flickering），相邻帧之间的颜色分配可能不一致
2. **稀疏参考**：实际生产中，画师通常只提供关键帧（key frames）的彩色参考，中间帧需要自动推断
3. **线稿质量差异**：手绘线稿（sketch）与数字线稿在清晰度、线条粗细、封闭性等方面差异巨大
4. **背景干扰**：训练数据中的复杂背景会干扰模型对前景角色的着色学习
