...
description: >-
  [CVPR 2025][图像生成][连续归一化流] ResFlow 提出将图像退化过程建模为确定性连续归一化流（而非随机扩散过程），通过辅助变量消解退化的不可逆性实现可逆建模，采用熵守恒调度策略，仅需 4 步采样即可完成高质量图像复原，在去雪/去雨/去雾/去噪/去压缩伪影等任务上达到 SOTA。 领域现状：图像复原旨在从退化…
tags:
  - "CVPR 2025"
  - "图像生成"
  - "连续归一化流"
