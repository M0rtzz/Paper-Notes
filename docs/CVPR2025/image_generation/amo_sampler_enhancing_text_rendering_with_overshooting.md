...
## 研究背景与动机
1. **领域现状**：基于rectified flow的扩散模型（如Stable Diffusion 3、Flux）在图像生成质量上取得了突破性进展，但在图像中准确渲染文本（text rendering）方面仍然存在显著缺陷。即使是最先进的SD3模型，文本渲染正确率也仅约32.5%。
2. **现有痛点**：(1) 文本渲染要求像素级精确——每个字母的形状、大小、间距都必须正确，这对生成模型的采样精度提出了极高要求；(2) 现有的采样器（如Euler、DPM-Solver）在有限步数下的离散化误差会累积，导致文本区域的细节丢失或扭曲；(3) 训练专门的文本渲染模型（如TextDiffuser）需要大量文本-图像对和额外训练成本。
3. **核心矛盾**：更多的采样步数可以减小离散化误差但增加推理成本，而少步数采样在文本这种需要高精度的区域容易出错。如何在不增加训练成本的前提下，在有限步数内提升采样精度？
4. **本文解决什么？** 设计一种无需训练的采样器增强方法，专门改善rectified flow模型在文本渲染区域的采样精度。
5. **切入角度**：借鉴Langevin动力学中的"噪声-去噪"校正机制——在ODE采样轨迹上故意"过冲"到噪声更大的位置，然后精确补偿噪声回到目标位置，通过这种"绕路"实现局部采样精度的提升。
6. **核心idea一句话**：通过过冲+噪声补偿构造等效的Langevin校正步，并用注意力图自适应地只在文本区域施加校正，避免对非文本区域的干扰。
...
1. **ODE过冲（Overshooting）**:
   - 做什么：在标准Euler步完成后，沿速度场方向继续推进到更远的时间步
...
3. **注意力调制（Attention Modulation）**:
   - 做什么：利用文本-图像交叉注意力分数自适应控制每个空间位置的过冲强度
