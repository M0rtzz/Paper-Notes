...
description: >-
  [CVPR 2025][图像生成][统一图像生成] 首个通用图像生成基础模型，仅由 VAE 和 Transformer 组成，通过统一多模态输入格式实现文生图、图像编辑、可控生成等多任务端到端处理。 现有痛点 现有痛点：领域现状：大语言模型 (LLM) 已统一语言生成任务并革新人机交互，但图像生成领域仍缺乏能在单一框架内处理…
tags:
  - "CVPR 2025"
  - "图像生成"
  - "统一图像生成"
