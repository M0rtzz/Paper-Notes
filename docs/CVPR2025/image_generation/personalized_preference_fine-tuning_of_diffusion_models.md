...
description: >-
  [CVPR 2025][图像生成][个性化偏好对齐] PPD 提出个性化偏好扩散模型微调框架：利用 VLM 从少量（4 对）偏好示例中提取用户嵌入，通过解耦交叉注意力层注入扩散模型，结合 DPO 目标同时优化多用户的个性化偏好，仅需 4 个偏好对即可为新用户生成匹配其偏好的图像（76% 胜率）。 领域现状：Diffusion…
tags:
  - "CVPR 2025"
  - "图像生成"
  - "个性化偏好对齐"
