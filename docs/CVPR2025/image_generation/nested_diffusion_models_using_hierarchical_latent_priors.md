...
description: >-
  [CVPR 2025][图像生成][嵌套扩散] 本文提出嵌套扩散模型，用一系列从粗到细的扩散模型逐级生成不同语义层级的潜变量，每级以上级输出为条件，在 ImageNet 256×256 上仅增加 25% 计算量便将无条件 FID 从 45.19 降至 11.05，有条件 FID 降至 3.97。 1. 领域现状：扩散模型（D…
tags:
  - "CVPR 2025"
  - "图像生成"
  - "嵌套扩散"
