...
1. **领域现状**：Dynamic Integration of Task-Specific Adapters  方向近年取得了显著进展，但仍存在关键挑战。
...
5. **切入角度**：每个任务对应一个 LoRA/Adapter，推理时通过任务识别模块确定输入属于哪个任务，动态加载相应适配器。

6. **核心idea一句话**：通过动态集成任务特定适配器实现类增量学习。
