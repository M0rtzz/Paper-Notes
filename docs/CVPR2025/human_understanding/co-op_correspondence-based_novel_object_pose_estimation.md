...
1. **CAD模型依赖**：大多数高精度的位姿估计方法需要目标物体的精确3D CAD模型。在实际应用中，新物体的CAD模型往往不可获得
2. **物体特定训练**：现有方法通常需要针对每个目标物体进行专门训练，面对新物体时需要重新收集数据和训练，效率极低
3. **泛化能力不足**：在训练集中未出现的物体上，现有方法的性能通常急剧下降
4. **参考视图利用不充分**：一些无CAD方法虽然使用参考视图替代CAD模型，但对参考视图中几何信息的利用不够充分
5. **对称性与遮挡**：物体的对称性和场景中的遮挡使得位姿估计更加困难
