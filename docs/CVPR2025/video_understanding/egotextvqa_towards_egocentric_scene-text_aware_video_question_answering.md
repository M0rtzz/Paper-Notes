...
现有场景文字 VQA 数据集（如 TextVQA、ST-VQA）假设用户能拍到清晰聚焦的图片且问题直接指向文字区域，这在实际应用中不太现实。考虑视障人士等场景，用户很难拍出清晰图片或指向文字区域。而已有的视频文字问答数据集（如 RoadTextVQA）仍然假设用户知道场景文字的位置，问题设计过于简单，只需要 OCR 抽取即可回答。

本文的动机是构建一个更贴近真实需求的第一人称场景文字视频问答基准：
- 问题反映**真实用户需求**，而非直接指向场景文字
- 支持**实时流式问答**（每个问题有时间戳，模型只能访问时间戳前的内容）
- 覆盖**室内外多种场景**（户外驾驶 + 室内家务）
...
- [Do You See What I Am Pointing At? Gesture-Based Egocentric Video Question Answering](../../CVPR2026/video_understanding/do_you_see_what_i_am_pointing_at_gesture-based_egocentric_video_question_answeri.md)
