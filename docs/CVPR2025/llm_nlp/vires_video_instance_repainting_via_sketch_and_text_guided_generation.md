...
1. **领域现状**：视频编辑需求日益增长，但现有方法在精确控制特定实例的外观变化时面临挑战。文本引导的视频编辑（如Tune-A-Video）缺乏空间精度；图像修复方法（如ProPainter）缺乏语义控制。
...
3. **核心思路**：结合草图（空间控制）和文本（语义控制）双重引导，配合实例分割掩码限制编辑区域，在扩散模型框架中融入时序注意力确保时间一致性。
