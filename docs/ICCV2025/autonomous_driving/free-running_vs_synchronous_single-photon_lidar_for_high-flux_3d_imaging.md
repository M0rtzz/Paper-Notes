...
description: >-
  [ICCV 2025][自动驾驶][单光子激光雷达] 本文系统比较了单光子激光雷达（SPL）的自由运行模式和同步模式在高通量条件下的深度成像性能，提出了高效的联合最大似然估计器和基于分数模型的深度正则化算法 SSDR，证明自由运行模式在各种光通量和信背比条件下均优于同步模式。 领域现状：单光子激光雷达（SPL）利用皮秒级激光…
tags:
  - "ICCV 2025"
  - "自动驾驶"
  - "单光子激光雷达"
