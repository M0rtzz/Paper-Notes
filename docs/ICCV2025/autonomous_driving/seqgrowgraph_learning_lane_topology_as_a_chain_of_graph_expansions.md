...
description: >-
  [自动驾驶] 模拟人类绘图过程，将车道拓扑建模为逐步图扩展的链式序列，通过自回归变换器增量构建有向车道图，克服 DAG 方法无法表达环路和双向车道的局限。 精确的车道拓扑对自动驾驶至关重要，现有方法存在三类局限： 检测式方法的碎片化：像素级方法（HDMapNet）无法推断全局拓扑；片段级方法（TopoNet）分段检测中心线导…
tags:
  - "自动驾驶"
---
