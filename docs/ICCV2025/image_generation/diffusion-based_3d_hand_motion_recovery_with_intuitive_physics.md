...
description: >-
  [ICCV 2025][图像生成][扩散模型] 提出一种物理增强的条件扩散模型，通过迭代去噪过程将逐帧 3D 手部重建结果细化为时序一致的运动序列，并结合直觉物理约束（运动学约束和稳定性约束）大幅提升重建精度和物理合理性。 从单目 RGB 视频恢复 3D 手部运动是 VR/AR、机器人灵巧操作等领域的关键问题…
tags:
  - "ICCV 2025"
  - "图像生成"
  - "扩散模型"
