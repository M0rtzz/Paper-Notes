...
GameplayQA 要解决的是：现有视频基准多是慢节奏被动观察、只给全局分、还只看单视角，没法考验智能体在快节奏3D环境里的感知。它把多人3D游戏当作"认知沙箱"，搭了一条端到端流水线——先从9款多人3D游戏收集同步多视角视频；再按6种实体类型（SA/SS/OA/OS/WO/WE）做密集多轨时间线标注，密度达 1.22 标签/秒；接着为每个标注配上带负面标签的干扰项用来诱导幻觉；然后用组合式模板算法从标注里生成 QA，初始产出 40 万候选对、降采样到 4K、再质量过滤到最终 2,365 对；最后即可在这套题上评估模型并做细粒度幻觉分析。质量过滤本身分两阶段：先做 blind filtering（语言先验过滤）剔掉不看视觉就能答的题，再对 120 道均匀采样题人工评估，约 8% 被标为有缺陷而移除。
