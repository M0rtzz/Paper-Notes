...
### 整体框架
Qayyem 采用前后端分离架构：Assignment Manager（Next.js 全栈应用）负责 Web 交互与作业管理，Scoring Server（FastAPI）负责模型推理与 API 服务。完整工作流包含四步：(1) 题目与评分标准设计 → (2) 作业配置（选择评分维度与模型）→ (3) 批量评分 → (4) 结果审核与人工修正。

### 关键设计
1. **跨题目多维度评分架构**:

    - 功能：支持对 7 个写作维度（相关性、组织、词汇、风格、展开、拼写标点、语法）分别选用不同模型进行评分
    - 核心思路：每个维度独立配置模型，模型均在 LAILA 数据集的跨题目划分上训练
    - 设计动机：不同写作维度的评估难度和最优模型可能不同，分维度选模型允许灵活组合

2. **模块化评分服务器**:

    - 功能：通过配置文件管理所有模型的加载、启用和元数据，对外暴露统一 REST + SSE API
    - 核心思路：所有模型遵循共享接口，通过 API Key 鉴权 + 速率限制保护访问，支持实时流式进度更新
    - 设计动机：允许系统管理员在不修改核心代码的情况下添加或禁用模型，支持热更新

3. **多层次模型部署（特征模型 + SOTA 模型）**:

    - 功能：同时部署轻量特征模型（RF、NN、XGB）和 SOTA 模型（TRATES、MOOSE），提供效果-效率梯度选择
    - 核心思路：特征模型基于 816 个手工特征，TRATES 利用 LLM 生成 trait-specific 特征 + 语言特征，MOOSE 结合 MoE + 成对排序 + 相关性建模
    - 设计动机：教育场景中实时性和准确性需求因用途而异，提供多档模型满足不同需求
