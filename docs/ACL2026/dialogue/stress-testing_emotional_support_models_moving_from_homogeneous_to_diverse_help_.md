...
1. **九维求助者画像**:
	...
	- 功能：把真实求助行为拆成可组合、可控制、可验证的特征向量。
	- 核心思路：作者从 11,066 条 Reddit 情感支持对话中抽取心理与语言变量；心理特征用 LLM tagger 标注，语言特征用规则提取，例如 profanity 由 profanity-check 检测，verbosity 由求助者话语 token 数离散化，turn level 由对话轮数确定。
	- 设计动机：传统 persona 往往只描述身份或处境，控制不了“是否抵触建议”“是否愿意披露”“是否话很多”。九维画像直接对准支持模型最容易失效的互动维度，让后续评测可以按人群切片。

2. **行为路由的 LoRA-MoE 模拟器**:

	- 功能：在同一个 seeker 模型中学习多个行为子空间，并根据画像动态组合 experts。
	- 核心思路：先训练并冻结 SFT backbone，再在每个线性层加入多个低秩专家；路由网络把结构化 seeker feature vector 映射为共享的 dialogue-level routing vector $\alpha$，线性层输出可写作原始变换加上 $\sum_i \alpha_i \Delta_i(x)$。专家语义不由人工指定，而是在语言建模损失和 routing 共同优化中自然分化。
	- 设计动机：长对话历史和 system prompt 会稀释画像信号，单纯 SFT 容易回到“礼貌、合作、平均化”的语言模式。MoE 把画像控制从文本提示中解耦出来，用参数子空间承接不同求助风格。

3. **从模拟器验证到 supporter 压力测试的闭环**:

	- 功能：先验证 seeker 是否真的像目标画像，再用它评估 supporter 在不同人群上的表现。
	- 核心思路：作者先用 profile adherence、expert fidelity、diversity 检查模拟器，再用 300 个 held-out profiles 生成 seeker-supporter 对话，并用十项情感支持与通用对话指标评价 supporter。模拟器还学习 `<|end_of_dialogue|>` 结束符，使对话长度受 total turns level 控制。
	- 设计动机：一个模拟器如果只“看起来多样”但不遵守画像，就会把评测噪声传给 supporter 排名。论文先验证模拟器本身，再做模型压力测试，逻辑上更扎实。
...
- [\[ICML 2026\] MoshiRAG: Asynchronous Knowledge Retrieval for Full-Duplex Speech Language Models](../../ICML2026/dialogue/moshirag_asynchronous_knowledge_retrieval_for_full-duplex_speech_language_models.md)
