...
1. **双时间尺度 EMA + Intervention Coefficient $\lambda^{(t)}$**:

    - 功能：实时识别"训练已进入僵局"的具体时刻，避免持续不必要的干预破坏正常学习。
    - 核心思路：维护两条 baseline $b_p^{fast,t} = \alpha_{fast} b_p^{fast,t-1} + (1-\alpha_{fast}) R_p(\tau)$ 与 $b_p^{slow,t}$（$\alpha_{fast} = 0.5 < \alpha_{slow} = 0.95$）。理论上 $\mathbb{E}[|b_p^{fast} - b_p^{slow}|] \propto |d\mu/dt|$（论文 Prop A.1 证明），所以散度反映"训练速度"。Stagnation coefficient $\sigma^{(t)} = 1 - \tanh(|b_p^{fast} - b_p^{slow}|)$，combined with match entropy $\lambda^{(t)} = \sigma^{(t)} \cdot \sqrt{1 - H_{match}^{(t)}}$。
    - 设计动机：单 baseline 在"稳定最优"和"卡在次优"两种状态下都会平稳，无法区分。双 baseline 的散度可以区分"baseline 稳定但仍在快速进步（divergence 大）"与"baseline 稳定且不再变化（divergence 小）"，再叠加 entropy 信息就能精确锁定僵局。

2. **Asymmetric Advantage Reshaping + 全局历史界限**:

    - 功能：当僵局触发（$\lambda^{(t)} \to 1$）时，对 dominant 与 rare outcome 施加非对称压力——压制 over-represented 行为，放大 under-represented 行为。
    - 核心思路：维护历史值界限 $V_{max}^{(t)} = \max_{i \leq t} b_p^{fast,i}$ 和 $V_{min}^{(t)} = \min_{i \leq t} b_p^{fast,i}$（用 fast baseline 以保留极值敏感性）。然后定义非对称 target value $V_{asym}(\tau, b_p^{fast}) = \mathbb{I}(o_\tau = o_{dom}) \cdot V_{max}^{(t)} + \mathbb{I}(o_\tau \neq o_{dom}) \cdot V_{min}^{(t)}$。对 dominant 轨迹，advantage $R - V_{max}$ 总是非正，强抑制；对 rare 轨迹，advantage $R - V_{min}$ 最大化，强放大。
    - 设计动机：标准方法中 baseline = batch 平均 dominant return，dominant 样本的 advantage 趋零，且 rare 样本数量太少，总梯度消失。Asymmetric reshaping 用 push-pull 机制注入合成方差 $\nu_{syn} \propto (V_{max} - V_{min})^2$，确保 $\|\nabla J\| > 0$（论文 Thm A.3 证明）。

3. **Adaptive Baseline Fusion 平滑过渡**:

    - 功能：避免干预与非干预阶段的"硬切换"破坏训练稳定性。
    - 核心思路：最终 baseline $\tilde{b}_p(\tau) = (1 - \lambda^{(t)}) \cdot b_p^{slow,(t)} + \lambda^{(t)} \cdot V_{asym}(\tau, b_p^{fast})$。$\lambda^{(t)}$ 小时退化为标准 slow baseline；$\lambda^{(t)} \to 1$ 时完全由 asymmetric value 主导。
    - 设计动机：把 intervention 当作连续标量而非二值开关，让 policy update 在僵局边缘平滑增强干预强度，不会在 $\sigma$ 微小波动时出现抖动。
