...
### 整体框架
输入是一批待评估论文，论文表示主要来自标题与摘要。框架先通过 pair sampling 选择需要比较的论文对，再让经过训练的 LLM 对每个论文对输出偏好标签。训练阶段，采样出的论文对还会经过分数差和出现次数约束过滤，形成可靠监督；模型先用冷启动 SFT 学会比较式推理，再用基于真实均分偏好的 RLVR 强化。推理阶段，系统控制采样比例，只比较一小部分论文对，随后用 Bradley-Terry 模型估计每篇论文的潜在质量分数，并按排序阈值做接收/拒稿决策。

### 关键设计
1. **GBR-BR 语义图采样**:

	- 功能：在所有可能论文对中优先挑出“既可比又有信息量”的 pair，减少无意义比较。
	- 核心思路：每篇论文先用 embedding 检索候选邻居，再用 reranker 得到双向排名。若论文 $p_i$ 和 $p_j$ 在任一方向进入前列，就在图中连边，并用 $2k_r-r_{ij}-r_{ji}$ 作为边权。图必须保持连通，若出现孤立节点就放宽检索和重排阈值，最后按边权排序得到优先比较列表。
	- 设计动机：只比较随机论文会产生大量跨领域、难以判断或信息密度低的 pair；只比较同领域又可能损失跨领域泛化。语义图让相近论文形成细粒度监督，同时保留全局连通性，避免某些论文完全没有比较信号。

2. **比较式 SFT + RLVR 训练**:

	- 功能：让 7B 模型真正学会“哪篇论文更强”，而不是把比较任务交给未训练的通用 LLM。
	- 核心思路：训练 pair 需满足两个约束：平均审稿分差至少为 $d_{min}=1.5$，且每篇论文最多出现 $c_{max}=1$ 次，以减少难分样本噪声并提升多样性。模型先用 instruct LLM 生成比较推理链做冷启动 SFT，再用改进 GRPO 做强化学习；奖励来自真实平均分的偏好标签 $y_{ij}=\mathbb{I}(s_i>s_j)$，模型预测正确时给 $R_l=\gamma\cdot\mathbb{I}(y_{ij}=\hat y_{ij}^{(l)})$，其中实验设 $\gamma=5$。
	- 设计动机：直接 RL 容易不稳定，单纯 SFT 又未必能优化最终偏好正确性。SFT 提供比较推理格式和冷启动能力，RLVR 用可验证标签继续强化相对质量判断。

3. **偏好聚合与排序决策**:

	- 功能：把局部成对偏好转成一批论文的全局质量排名。
	- 核心思路：推理时仅采样理论 $O(n^2)$ 论文对中的一部分，比例设为 $\alpha=0.05$。每个 pair 由训练后的 LLM 产生偏好标签，再用 Bradley-Terry 模型估计潜在质量 $\theta_i$，其中 $p_{ij}=e^{\theta_i}/(e^{\theta_i}+e^{\theta_j})$ 表示 $i$ 优于 $j$ 的概率。最大化所有偏好标签的似然后，按 $\theta_i$ 排序并用预设录用率确定接收论文。
	- 设计动机：pairwise judgment 本身不能直接给出会议层面的录用集合，Bradley-Terry 提供了可解释、经典且实验中效果最好的聚合方式。
