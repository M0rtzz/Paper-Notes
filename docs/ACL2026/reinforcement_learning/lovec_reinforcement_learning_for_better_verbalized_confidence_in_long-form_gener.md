...
flowchart TD
    A["查询 q"] --> B["SFT 学输出格式<br/>只在 winning 样本上学打置信度标签"]
    B -->|"有 fact-checker oracle（在线）"| C["GRPO + log-base 校准 reward<br/>群体相对优势，BCE 近似 proper scoring"]
    B -->|"无 oracle，仅离线偏好对"| D["DPO + 合成偏好对<br/>句子相同、只偏移置信度数字"]
    C --> E["策略 πθ：单次 decoding<br/>同时产出 (句子, 0–10 置信度) 对"]
    D --> E
    E --> F["Free-form / Iterative Tagging 双协议<br/>解耦内容质量与打分质量"]
```

### 关键设计

**1. GRPO + log-base 校准 reward：把"置信度该不该信"写成 proper scoring 的群体相对优势**

要让 confidence 数字真正反映事实性，奖励函数本身必须是个 proper scoring rule，否则模型只学到排序、学不到校准。LoVeC 先把 $c_i,f_i$ 都归一到 $[0,1]$，定义置信度奖励

$$r_{\mathrm{conf}} = \lambda\cdot \frac{1}{n}\mathbf{1}^\top\left(1+\frac{f\odot\log c + (1-f)\odot\log(1-c)}{R_{\max}}\right)$$

本质就是 BCE 的负值，再叠加 informativeness 与 format 两个子奖励。它对"高置信但事实错"的惩罚趋于 $-\infty$，远比 linear / quadratic loss 更狠，从而逼模型 calibrate 而不是只学相对排序。优化用 GRPO：对每个 query 采样 $G$ 条轨迹 $\{y_j\}$，算 group-mean 归一的优势 $\hat A_j=\frac{r_j-\mathrm{mean}(r)}{\mathrm{std}(r)}$（省掉单独 critic、省显存），目标为
...
reward 还做了 $\gamma=1.5$ 的拉伸 $r\leftarrow\mathrm{sign}(r)|r|^\gamma$ 进一步放大好坏样本的差异。

**2. DPO + 算法 1 的合成偏好对：在没有 online fact-checker 时改用离线偏好学习**

GRPO 训练时要反复调 fact-checker，成本极高，所以离线场景需要一条把 oracle 调用集中到数据构造阶段的路。对每个 query $(q,E)$，先用 base 模型生成纯文本 $y_{\mathrm{base}}=\{s_1,\dots,s_n\}$，用 GPT-4o + 检索证据算 fact label $f_j$，再构造 winning 集合 $y_w=\{(s_j,f_j)\}$（直接拿真实事实分当 confidence）和 losing 集合 $y_l=\{(s_j,c'_j)\}$——其中 $c'_j$ 从 $\{0,\dots,10\}\setminus\{f_j\}$ 均匀采样。关键巧思是 winning/losing 句子完全相同、只让置信度数字偏离真实事实分，这样模型被迫聚焦于学"怎么打分"而非"怎么写句子"，不污染语言能力。随后用标准 DPO 损失优化：
...
**3. Free-form vs. Iterative Tagging 双协议：把"内容写得好"和"分打得准"分开评**

现有 verbalized 方法各自生成不同内容，BS/ECE 没法横向对比，于是要拆掉这层混淆。Free-form 让模型自由生成 $y_t=\arg\max_{y_t}\pi_\theta(y_t|y_{<t},q)$，同时输出答案和 `<confidence>`，保留真实使用场景；Iterative 则固定 base 模型已经写好的 $\{s_1,\dots,s_n\}$，模型只逐句条件预测 $c_i=\arg\max_c\pi_\theta(\{q,(s_1,c_1),\dots,(s_{i-1},c_{i-1}),s_i\},c)$，剥离内容变异让不同模型在完全相同的文本上比谁打分更准。两个协议一起用，就能干净地分辨"是写得更好还是打分更准"。
