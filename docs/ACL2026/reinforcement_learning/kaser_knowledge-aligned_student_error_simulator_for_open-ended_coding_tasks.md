...
1. **Student Knowledge Estimator**:
	...
	- 功能：把学生历史提交压缩成可解释的知识掌握向量。
	- 核心思路：知识追踪模型产生学生状态 $h_t$，线性层加sigmoid得到 $m_t \in [0,1]^k$，其中每一维对应一个KC掌握度。对于下一题，只取该题涉及的KCs并用补偿模型平均得到整体正确率估计，用BCE损失训练。
	- 设计动机：直接使用学生ID或历史代码很难解释“为什么会犯这个错”。KC mastery让生成错误能和具体知识点缺陷对应起来。

2. **Knowledge-guided code predictor**:

	- 功能：让LLM在预测代码时显式看到学生的知识画像。
	- 核心思路：prompt包含题目描述和每个相关KC的掌握度，例如“Conditional logic mastery = 0.21”。Qwen2.5-Coder 7B Instruct根据题目和掌握度逐token生成预测代码，SFT阶段用标准交叉熵学习真实学生提交。
	- 设计动机：SFT提供基本任务适配，但仅靠SFT会倾向生成平均化、正确化的代码；知识条件让后续RL有可对齐的控制变量。

3. **GRPO hybrid reward**:

	- 功能：同时优化代码相似、错误匹配和生成多样性，避免只追求一个指标。
	- 核心思路：每个输入采样 $G=5$ 个候选代码，用 $R=R_{Sim}+R_{Error}+R_{Div}$ 打分。$R_{Sim}$ 是CodeBLEU；$R_{Error}$ 是预测错误集合和真实错误集合的IoU，若两者都正确则为1；$R_{Div}=1-\max CodeBLEU(\hat{c},\hat{c_i})$，鼓励同组候选之间不要太相似。GRPO用组内z-score归一化奖励形成优势，并加KL惩罚约束策略偏移。
	- 设计动机：代码相似度保证基本语法和解法接近真实提交，错误IoU让模型关注学生实际错误，多样性奖励防止SFT式mode collapse。
