...
### 整体框架
输入是同一基座模型在相同数学训练数据上得到的两个后训练版本：SFT 模型和 RL 模型。作者在 Qwen3-4B-Base、Qwen2.5-7B 上做主实验，并在 Llama3.1-8B-Instruct 上验证趋势。

流程上，作者先训练两个 pairwise Sparse Crosscoder：Base-SFT 和 Base-RL，用来比较每种后训练方式相对基座引入了多少特异特征。随后训练一个三模型 Sparse Crosscoder，把 Base、SFT、RL 三者放进同一稀疏特征空间，并用 MAS 衡量每个特征归属于哪个模型。最后，作者聚焦“基座答错、RL 答对”的 generalization-critical samples，找出 RL 相比基座显著激活的特征，再通过置零和放大实验验证这些特征的因果作用。

### 关键设计
1. **两模型 Sparse Crosscoder 与 NRN 特征特异性度量**:

    - 功能：把基座模型和一个后训练模型的同层残差流激活编码到共享稀疏特征空间，衡量每个特征更偏向基座还是后训练模型。
    - 核心思路：Crosscoder 同时重建两个模型的激活，稀疏向量中的每个维度可看作一个共享或特异功能特征。作者用 NRN 度量特征归属，直观形式是 $NRN=||W^{T}_{dec,k}||_1/(||W^{O}_{dec,k}||_1+||W^{T}_{dec,k}||_1)$；接近 1 表示后训练模型特异，接近 0 表示基座特异，接近 0.5 表示共享。
    - 设计动机：直接比较隐藏状态容易混在一起看不清，而 NRN 可以揭示 SFT 和 RL 对特征空间的改写强度。实验表明 SFT 有明显右尾，说明产生大量强特异特征；RL 的极端特征少得多，说明它主要保留并轻微重排基座表示。

2. **三模型 Crosscoder 与 MAS 统一归因**:

    - 功能：解决两个 pairwise Crosscoder 无法直接比较特征索引的问题，在同一个稀疏基底下同时对齐 Base、SFT 和 RL。
    - 核心思路：三模型 Crosscoder 共同编码三种模型的激活，并分别用三套 decoder 重建。MAS 把每个特征的三个 decoder norm 归一化为 $MAS_O$、$MAS_S$、$MAS_R$，三者相加为 1，最大者表示该特征最强归属于哪个训练范式。
    - 设计动机：如果只训练 Base-SFT 和 Base-RL 两个 Crosscoder，两个空间里的“第 2026 个特征”并不对应。三模型对齐让作者能直接判断某个特征是 SFT 独有、RL 独有、还是三者共享，从而更公平地比较训练范式。

3. **泛化关键样本上的特征定位与因果干预**:

    - 功能：从描述性特征差异进一步走到因果机制，找出真正支撑 RL 跨任务泛化的特征。
    - 核心思路：对每个非数学评测任务，选取“基座模型失败但 RL 模型成功”的样本集合。对这些样本，比较 RL 与基座在 Crosscoder 特征上的平均激活差，特征得分可理解为 $Score_k=E[f_k^{RL}(x)-f_k^{Base}(x)]$。每个任务保留超过阈值的特征，再取跨任务交集，得到任务无关的 generalization-controlling features。
    - 设计动机：泛化特征不应依赖“Wait”“However”等表面词触发，而应在模型真正从错变对的样本中体现功能差异。后续置零这些特征会让 RL 回答变差，放大这些特征会让基座模型变好，因此它们不是相关性噪声，而是有因果作用的内部控制点。
