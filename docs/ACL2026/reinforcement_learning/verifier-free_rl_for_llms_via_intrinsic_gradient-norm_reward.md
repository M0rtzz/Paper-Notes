...
### 关键设计
1. **梯度范数作为 verifier-free intrinsic reward**:

	- 功能：在没有 gold answer、执行器或外部 judge 的情况下，为每个 completion 生成可比较的 reward。
	- 核心思路：把 completion 当作 teacher-forced 序列，计算 token 平均 NLL 对当前参数的梯度范数。低梯度范数表示该输出对当前模型来说更平滑、更少引发剧烈更新，因此在同组候选中被认为更优。
	- 设计动机：相比 entropy/self-certainty 直接来自词表概率分布，梯度范数汇总了高维参数空间的变化，理论上更不容易被简单的 token-level 表面模式操纵。

2. **$\sqrt{T}$ 长度校正**:

	- 功能：防止模型通过生成更长 completion 来人为降低平均梯度范数。
	- 核心思路：论文实证发现，原始梯度范数在约 250/500/750/1000 token 长度 bin 中从约 180 降到约 90，但乘以 $\sqrt{T}$ 后都稳定在约 $2.85\times10^3$ 到 $2.91\times10^3$。因此 reward 使用 $S_{GN}=-\sqrt{T}\|g\|_2$。
	- 设计动机：没有长度校正时，reward 会把“长”误判成“好”，尤其在 3B 模型上导致长度膨胀和准确率崩溃。长度校正把梯度信号从长度因素中解耦出来。

3. **组内 rank-based reward shaping**:

	- 功能：消除不同 prompt 间梯度范数尺度差异，让 reward 更适合 GRPO 的组相对优化。
	- 核心思路：对同一 prompt 的 $G$ 个 $S_{GN}$ 排序，设 rank 从 0 到 $G-1$，reward 为 $R_{GN}(x,y_i)=2\frac{rank_i}{G-1}-1$。这样最差为 -1、最好为 +1，只保留相对顺序，不保留原始幅值。
	- 设计动机：不同问题的梯度范数可能尺度差异很大，直接使用原始值会让少数极端 prompt 主导更新。rank shaping 让每个 prompt 内部贡献均衡，也降低 reward outlier 的影响。

### 损失函数 / 训练策略
VIGOR 直接接入 GRPO。每个 prompt 采样 $G=8$ 个 completions，计算长度校正的梯度范数 reward，做 rank normalization，再做组内 mean-std 标准化得到 advantage $\hat{A}_i$。策略目标与 GRPO 类似：最大化 clipped probability ratio 与 advantage 的乘积，并加 KL 正则约束当前 policy 与 reference policy。关键实现细节是 stop-gradient：虽然 reward 由当前参数的梯度范数计算得到，但训练时把它 detach 成常数，避免二阶梯度开销和不稳定。
