...
1. **Parallel Trajectory-level Success Reward（并行成功的"OR" 语义）**:

    - 功能：把传统 ReAct 的 0/1 task success reward 扩展到并行设定下。
    - 核心思路：$R_{traj}(\tau)=1$ 当 $S_T\cap\mathcal{G}\neq\emptyset$（任意一个并行环境到达目标状态即整体成功），否则 0。同 GRPO 一样用 group-relative advantage：从 $N$ 条 group trajectory 算均值和标准差，$\Phi_{traj}(\tau_i)=(R(\tau_i)-\text{mean})/\text{std}$。
    - 设计动机："任一环境成功即成功"的 OR 语义鼓励 agent 把多环境视为冗余备份 + 信息源——一手探索一手押注，与并行计算的本质一致；group-relative 又消除了 critic model 依赖。

2. **Diverse Action Reward（深度 × 宽度的双向去重）**:

    - 功能：在 step 内部直接惩罚动作重复，鼓励 agent 在不同环境间发不同的牌。
    - 核心思路：定义 $R_{action}(A_t)=\frac{1}{|\mathcal{E}'_t|}\sum_{E_i\in\mathcal{E}'_t}\alpha^{C_{depth}(E_i, a_t)}+\omega^{C_{width}(A_t)}$。其中 $C_{depth}$ 是动作 $a_t$ 在环境 $E_i$ 历史中的重复次数（深度方向 = 同一环境内是否反复尝试同一动作），$C_{width}$ 是 $A_t$ 当前步内重复动作数（宽度方向 = 多个环境是否都派了同样的动作）；$\alpha, \omega\in(0,1]$ 为折扣因子。重复次数越多，奖励越接近 0。
    - 设计动机：把"多样性"从模糊概念变成可微分的密集奖励信号；深度方向避免"原地打转"，宽度方向避免"复制粘贴"，让 $K$ 个并行环境真正去探索互补的状态空间。

3. **Diverse State Transition Reward + 分层 Advantage 组合**:

    - 功能：进一步惩罚"看到相同状态后又做相同动作"的重复 transition，并把步级奖励嵌入到轨迹级 advantage 中。
    - 核心思路：定义 transition $p_t = s_t\to a_t$，$R_{transition}=\frac{1}{|\mathcal{E}'_t|}\sum_i\gamma^{M_{depth}(E_i, p_t)} + \frac{1}{|\mathcal{E}'_t|}\sum_i\beta^{M_{width}(E_i, p_t)}$，其中 $M_{depth}$ 数 transition 在 $E_i$ 内重复次数，$M_{width}$ 数其他选中环境中相同 transition 的出现次数。两个 step-level 奖励平均：$R_{step}=(R_{action}+R_{transition})/2$。组合 advantage：若 $\Phi_{traj}(\tau_i)>0$ 则 $\Phi_{step}=R_{step}$，否则 $\Phi_{step}=2-R_{step}$；最终 $\Phi(A_{i,t})=\Phi_{step}\cdot\Phi_{traj}$。
    - 设计动机：仅看 action 不够——同一动作在不同状态语义不同；transition 才是 agent 实际"经验"的最小单元。条件翻转 $2-R_{step}$ 妙在：成功 trajectory 中多样性是加分，但失败 trajectory 中如果 step 多样性高，意味着 agent 在乱试，应该反向降权，让模型既"敢探索"又"知收敛"。
