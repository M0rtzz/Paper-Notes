...
### 关键设计
1. **扩展动作空间的自主 rollout loop**:

	- 功能：把复杂时序推理拆成可观测、可优化的动作序列。
	- 核心思路：每次响应必须从 <plan> 开始，模型可交替使用 <search> 获取事实、<filter> 处理时间/语义约束、<rank> 对候选事实按时间排序，并在 <answer> 中输出最终答案。
	- 设计动机：单一 <think> 标签让所有推理都隐式发生，复杂 temporal constraints 容易混乱；显式动作让 RL 能分别奖励工具调用、过滤和排序策略。

2. **SFT 冷启动 + masked loss**:

	- 功能：让基础模型先学会合法标签格式和基本工具链，再进入 RL 探索。
	- 核心思路：作者用 GPT-4o 构造约 1000 条高质量 $(q,\tau_{gold})$ 轨迹，并过滤结构正确性和答案语义正确性。SFT 时只对 agent 生成 token 计算交叉熵，系统 prompt、用户问题和检索观察都被 mask。
	- 设计动机：如果从 base model 直接 RL，模型早期容易输出非法标签、无意义搜索或混乱推理；冷启动提供一个稳定初始策略，避免 KL 爆炸和训练崩溃。

3. **GRPO 与反向课程学习**:

	- 功能：用可验证答案奖励发现更灵活的时序推理策略，并避免简单问题造成 shortcut。
	- 核心思路：对每个问题采样一组轨迹，用答案是否等于 gold answer 得到二值终端奖励，再用组内均值和标准差归一化 advantage，优化带 KL 正则的 GRPO 目标。训练数据调度采用 hard-first：前期只给复杂 multi-hop / multiple-constraint 问题，超过阈值后再加入简单问题。
	- 设计动机：普通 easy-first curriculum 会让模型早早学到 <search>→<answer> 的捷径，复杂问题时不会主动组合 <filter>/<rank> 等动作；先训练难题能迫使 agent 掌握高级工具链。
