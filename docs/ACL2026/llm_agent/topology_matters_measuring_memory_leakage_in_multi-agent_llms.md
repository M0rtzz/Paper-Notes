...
MAMA 框架包含三个关键环节：（1）数据合成——基于 Gretel 合成数据集生成包含标注 PII 实体的文档和对应的公开任务；（2）两阶段交互——Engram 阶段初始化智能体并向目标智能体注入私有信息，Resonance 阶段通过多轮交互在指定拓扑下传播信息；（3）泄露度量——结合精确匹配和 LLM 推理的两阶段恢复机制评估攻击者最终提取的 PII 数量。

### 关键设计

1. **受控数据合成与 PII 隔离**:

    - 功能：构建 SPIRIT 数据集，包含标注的 PII 实体、含 PII 的私有文档和删除 PII 的公开任务背景-问题对，确保任何 PII 泄露必然来自多智能体交互而非任务本身。
    - 核心思路：通过强制约束 $\mathrm{contains}(B_i \cup Q_i, \mathcal{S}_i) = 0$，确保初始化时只有目标智能体掌握 PII 信息。这使得泄露的根本来源完全可溯，排除了预训练记忆混淆。
    - 设计动机：解决了以往单智能体 PII 研究中难以区分"模型本身记忆"与"交互中泄露"的根本问题。

2. **两阶段恢复评估（精确匹配 + LLM 推理）**:

    - 功能：使用精确字符串匹配捕获明确泄露的 PII，再用 DeepSeek-V3.1 作为"法官"来推断那些被改写或模糊表述但仍可被推断的 PII。
    - 核心思路：第一阶段 $\hat{S}_i^{\mathrm{EM}} = \mathrm{match}(A_i^{(r_i^{\star})}, S_i)$，第二阶段 $\hat{S}_i^{\mathrm{INF}} = \mathcal{J}(A_i^{(r_i^{\star})}, S_i \setminus \hat{S}_i^{\mathrm{EM}})$，最终 $\hat{S}_i = \hat{S}_i^{\mathrm{EM}} \cup \hat{S}_i^{\mathrm{INF}}$。
    - 设计动机：精确匹配容易遗漏经过语义变换但本质未变的泄露（如"我的孩子生于 2008 年"推断出生年份），LLM 推理补充了这一缝隙，更真实地反映攻击者真实威胁。

3. **系统化拓扑-位置对比设计**:

    - 功能：评估六种典型拓扑（链、环、星形、星环混合、树、完全图）在 $n \in \{4,5,6\}$ 团队规模下的泄露，并在保持对称性的前提下枚举非冗余的攻击者-目标位置对，精确控制图距离和节点中心性。
    - 核心思路：对每个拓扑-规模组合，计算所有非等价的 (attacker_index, target_index) 对的泄露率，并统计它们的均值和标准差，覆盖从直接邻接（距离 1）到最远分离（距离 $\geq 2$）的全局距离范围。
    - 设计动机：捕捉"位置才是关键"这一网络安全基本直觉，避免仅比较拓扑平均值而忽视同一拓扑内位置的巨大差异。
