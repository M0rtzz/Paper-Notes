...
flowchart TD
    A["输入：论文 PDF C + 查询 Q"] --> B["minerU 解析<br/>PDF → 带标题的 Markdown C′"]
    B --> C["层级保留<br/>扁平标题 → section tree → 精简标题集 H"]
    C --> D["推理式章节排序<br/>据领域常识排出重排序列 C_R"]
    D --> E["迭代阅读<br/>沿 C_R 取下一节、抽细节 D_i"]
    E --> F{"充分性检查<br/>已抽证据够回答 Q 吗"}
    F -->|NO：读下一节| E
    F -->|YES：停止| G["合成答案 A = LLM(D₁…Dₘ, Q)"]
```
