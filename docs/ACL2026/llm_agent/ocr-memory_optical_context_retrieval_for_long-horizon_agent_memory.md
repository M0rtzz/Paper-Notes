...
### 关键设计
1. **Locate-and-Transcribe 视觉索引检索**:

    - 功能：把历史检索从“生成相关文本”改成“选择相关编号”，降低幻觉和长序列解码成本。
    - 核心思路：每个记忆项可写成 $m_i=(I_i,\{s_{i,k}\}_{k=1}^{K_i},\pi_i)$，其中 $I_i$ 是带编号的轨迹图像，$s_{i,k}$ 是第 $k$ 个编号对应的原始文本。检索模型输出每个片段是否相关的二值向量，得到索引集合 $\hat{S}(q)=\{(i,k)\mid \hat{y}_{i,k}=1\}$，再通过 $Fetch$ 取回 $s_{i,k}$。
    - 设计动机：视觉模型直接转写低分辨率图像容易出错，也可能凭语义补全不存在的信息。让模型只输出 0/1 或编号，相当于把任务约束成可验证的指针选择，最终文本由数据库确定性返回。

2. **召回优先的打分与选择规则**:

    - 功能：减少相关证据被漏掉的风险，让记忆检索更适合 Agent 决策场景。
    - 核心思路：模型虽然可以生成二值标签，但推理时作者不只依赖 greedy 输出，而是读取 “1” 与 “0” 两个标签 token 的 logits，计算 $p_{i,k}=\exp z_{i,k}(1)/(\exp z_{i,k}(1)+\exp z_{i,k}(0))$。选择时使用低阈值 $\tau=0.4$，并在没有高置信片段时用 Top-K 兜底；实现中 Top-K 为 5，总取回片段上限为 20。
    - 设计动机：长期 Agent 任务中，漏掉关键历史往往比多给一些无关片段更致命。这个规则显式偏向 recall，使 retriever 在不确定时仍能给主 Agent 保留最低限度的历史线索。

3. **多分辨率记忆老化与主动回忆**:

    - 功能：用低分辨率图像保存较旧历史，降低视觉 token 成本；当旧记忆被命中时，再恢复为高分辨率以便后续访问。
    - 核心思路：最近 5 个交互步骤使用 1024×1024 高分辨率，较早历史降到 512×512；对应 DeepSeek-OCR 的视觉 token 预算约为 256 和 64。若低分辨率记忆被检索命中，系统从原始日志重新渲染高分辨率版本，并在当前 episode 剩余时间里保持高保真状态。
    - 设计动机：多数旧历史只需要保留语义轮廓，少数被重新激活的历史才需要细节。这个设计把“记忆衰退”变成一种自适应压缩策略，在容量和可读性之间取得折中。
