...
1. **Safe Stackelberg Power Game**:
	...
	- 功能：把“防御 agent”形式化为安全约束下的工具集合选择问题。
	- 核心思路：论文把状态分为 safe、critical、unsafe。critical 表示当前还没失败，但存在某些动作会转入 unsafe。SafeMCP 的目标是在状态 $s_t$ 下选择安全工具集合 $\Phi_t^* = \{a \in \mathcal{A} \mid P(s' \in \mathcal{S}_{unsafe} \mid s_t,a)=0\}$，agent 则在 $\Phi_t^*$ 内选择效用最大的动作。
	- 设计动机：这比 post-hoc guardrail 更细，因为它不是直接拒绝 agent，而是重塑 agent 的搜索空间，让任务继续朝安全路径推进。

2. **两层推理时防御机制**:

	- 功能：同时处理“未来风险”和“当前危险调用”。
	- 核心思路：第一层是 proactive power regulation：在 agent 请求工具列表时，提前移除会导致危险状态转移的工具。第二层是 immediate fail-safe：当 agent 已请求某个工具调用时，SafeMCP 预测下一状态，如果它属于 unsafe，就阻断执行并返回错误。
	- 设计动机：主动过滤能减少 workflow 中断，但世界模型可能漏判；即时拦截作为兜底，降低危险调用穿透的概率。

3. **三阶段训练与双重奖励**:

	- 功能：让 SafeMCP 同时学会环境动态、状态安全判断和工具过滤。
	- 核心思路：Stage 1 是 Environmental Dynamics Grounding，用 next-state prediction 的 NLL 损失 $\mathcal{L}_{next}$ 学 $P(s_{i+1}\mid h_i,a_i)$，并用 unsafe steps prediction 的 $\mathcal{L}_{unsafe}$ 预测未来危险动作/状态集合。Stage 2 用 2,000 条 oracle-augmented 高质量 reasoning 响应做冷启动 SFT，并保持 safe/critical/unsafe 为 1:1:1。Stage 3 用 RLVR 强化双阶段推理，奖励由 safety binary reward、STCH scalarized tool-filtering reward 和 format reward 组成。
	- 设计动机：单纯二值 reward 会造成 gradient starvation，漏一个危险工具和全错的惩罚近似相同。STCH 将 false negative 与 false positive 的集合错误变成连续信号，兼顾安全和效用。
