...
1. **跨编辑共享二值 mask**:
	...
	- 功能：检验不同事实编辑是否依赖同一组功能权重位置。
	- 核心思路：mask 值为 1 表示保留编辑权重，0 表示移除其贡献。训练时同一个 $K$ 同时作用于多种 facts 的编辑矩阵，只允许 $K$ 更新。
	- 设计动机：如果编辑是事实特异的，一个共享 mask 不应泛化；如果共享 mask 能反转未见编辑，就说明 ROME / MEMIT 使用了共同机制。

2. **三重约束训练目标**:

	- 功能：让 mask 既能恢复旧事实，又不会粗暴破坏模型。
	- 核心思路：restoration loss 要求 pruned model 对原 object 的概率高于 edited object；sparsity loss 限制剪掉的权重比例；KL preservation loss 让 pruned model 的输出分布接近原模型。综合目标包含 $\mathcal{L}_{KL}$、$\max(0,\mathcal{L}_{sparsity}-S_{max})$ 和 $\max(0,\mathcal{L}_{restoration}+\delta)$。
	- 设计动机：只追求反转编辑可能通过破坏模型实现，加入稀疏和 KL 约束后，mask 更可能定位真正维持编辑的关键通路。

3. **残差流分解与编辑阻断实验**:

	- 功能：解释 mask 反转编辑时到底消除了什么机制，并验证该机制是否必要。
	- 核心思路：作者用 Logit Lens 分解 residual stream 中 MLP 与 attention 对目标 token logit 的贡献，比较 original、edited、pruned 三种模型。随后在编辑过程中预先注入 mask，看 ROME 是否还能绕过该子空间完成新编辑。
	- 设计动机：高 RSR 只能说明 mask 有效，不能说明为什么有效；残差分解和 blocking experiment 共同证明 overattention 既足以解释反转，也对编辑成功是必要的。
