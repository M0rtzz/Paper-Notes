...
description: >-
  [ACL 2026][LLM/NLP][表格问答] 通过分解问题中的语义单元并构建证据树进行透明化表格修剪，EnoTab 框架在处理复杂问题和超大表格时实现显著性能提升，通过双重去噪机制有效缓解噪声数据对推理的负面影响。
tags:
  - "ACL 2026"
  - "LLM/NLP"
  - "表格问答"
...
1. **证据生成与评估**：LLM 将复杂问题分解为最小语义单元（证据），每个证据 $e=(area, condition, action)$ 代表问题中的一个属性约束。例如"Tel Aviv 的城市"对应 $(District\ \text{列}, Tel\ Aviv, \text{字符匹配})$。通过多轮一致性检验和可用性检验，筛出可靠证据集。
...
1. **证据一致性与可用性双重评估**:
    ...
    - 功能：判断分解出的每个语义单元是否真正与答案推理相关。
    - 核心思路：而非直接依赖 LLM 评分，采用两个客观标准。一致性通过 5 轮多次生成检查——若某个证据在多轮生成中稳定出现（一致性分数 $S \geq 0.8$），则认为其重要；可用性通过工具箱 $\mathcal{P}$ 检查证据是否能在表中找到匹配数据。只有同时通过两项检验的证据才被保留。
    - 设计动机：解决问题分解方法"虚假相关性难辨"的痛点。一致性指标模仿人类推理的稳定性；可用性指标保证证据有表中数据支撑，避免幻觉。

2. **证据树的后序回滚机制**:

    - 功能：在表格修剪过程中检测和修复异常状态（如空表），防止答案关键数据被误删。
    - 核心思路：执行证据树后序遍历时，若 AND 节点产生空表（多条件交集为空），执行 And2Or 操作：将 AND 改为 OR，进而得到更宽松的超集。若仍为空或验证器 $M_i$ 判定信息不完整，则回滚到前一节点的结果并重试（最多 2 次）。
    - 设计动机：黑盒 SQL 方法遇到错误需全部重做，而证据树每步都可观测可验证。And2Or 的关键洞察是：对 TableQA 而言，召回比精度更重要（遗漏证据无法恢复，但多余行可被后续过滤）。

3. **两阶段检索优化证据生成效率**:

    - 功能：从超大表格中高效选出 k=10 个代表行，作为 evidence generation 的上下文。
    - 核心思路：第一阶段用 LSH 进行关键词粗过滤（速度快但精度低）；第二阶段计算每个候选行与问题的语义相似度 $Score(r,Q) = 0.7 \cdot S_{sem}(r,Q) + 0.3 \cdot S_{lex}(r,Q)$，其中 $S_{sem}$ 用预训练 embedding 模型测算，$S_{lex}$ 用编辑距离计算。
    - 设计动机：直接对全表进行证据生成计算量巨大且易出错，取代表性样本既保持质量又大幅降低成本。
...
- [\[ACL 2026\] Iterative Formalization and Planning in Partially Observable Environments](iterative_formalization_and_planning_in_partially_observable_environments.md)
