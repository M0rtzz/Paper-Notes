...
### 关键设计
1. **从用户生成内容构造 Persona Bank**:

	- 功能：获得比手写人设更接近日常社交行为的 persona 数据。
	- 核心思路：作者从超过 19k 用户、681k 博客帖子出发，过滤并匿名化私人信息，再用 LLM 推断 narrative description 和结构化事实，包括 demographic、occupation、personality traits、values、interests、experiences。最终 persona corpus 包含 1,000 个独特 persona。
	- 设计动机：普通人的 persona 不只是姓名和职业，而是由经历、价值观、兴趣和情绪模式共同构成。真实用户内容比虚构名人角色更适合评估日常社交真实性。

2. **动态社交沙盒与 Environment Agent**:

	- 功能：让 persona 在多轮互动中自然表达，而不是通过静态问答被动暴露。
	- 核心思路：protagonist 采用 BDI 风格的目标条件推理，维护 Self-Belief 和 Env-Belief；NPC 保持固定 self-belief，只根据主角行为更新环境理解。Environment Agent 负责 interaction analysis、adaptive turn control、character state update 和 environment update，并监控 Background、Personality、Values、Interests、Experiences 五个 checkpoint。
	- 设计动机：角色扮演质量取决于连续行为是否因果一致、情绪是否随场景变化、persona 维度是否充分表达。环境控制器可以平衡覆盖度和效率，避免轨迹过短或冗长。

3. **多评委辩论与轨迹后训练**:

	- 功能：降低单个 LLM judge 的标尺偏差，并把评估轨迹转成训练信号。
	- 核心思路：PersonaArena 使用多个 LLM judge 对 8 个指标打分，均值聚合；若 judge 分歧较大，每个 judge 提交评分、理由和证据片段，再由 referee 生成统一 rationale 和 reconciled score。高分轨迹可拆成 SFT 样本，轨迹对可构造 DPO preference pairs。
	- 设计动机：单一 judge 可能过宽或过严，而多评委辩论能平滑标尺偏差。评估框架产出的轨迹本身也能成为提升 role-playing 的高质量数据。
