...
1. **双代理 OOD 评分**:
    ...
    - 功能：在目标域不可见的情况下，估计源域样本对目标分布的亲和度。
    - 核心思路：源域代理通过对源域标注数据的指令微调得到，目标域代理保留预训练 LLM 的先验知识。对同一样本 x，源代理倾向高困惑度（因为已过度适配源域特征），目标代理保持更"通用"的视角。若样本 x 在源代理下困惑度高、在目标代理下困惑度低，说明该样本可能是"源域异常值"，更可能在未知目标域中表现良好。OOD 得分定义为 $S(x) = \log PPL_{target}^{proxy}(x) - \log PPL_{source}^{proxy}(x)$，得分越低表示样本越适配目标。
    - 设计动机：相比于直接使用统一分布作为目标代理（会导致更松的误差界），利用预训练 LLM 本身作为目标代理既充分利用了其通用知识，又避免了过度强假设。理论上通过 Theorem 1 的有界代理误差保证，该近似的偏离度被限制在 $(\epsilon_t/m_t + \epsilon_s/m_s)$ 的范围内。

2. **马哈拉诺比斯距离多样性约束**:

    - 功能：在已选演示集合中引入全局多样性度量，避免相似度排序导致的冗余和短文本偏好。
    - 核心思路：初始化时根据与测试样本的相似度选择前 C 个候选。随后计算候选集中所有样本对的平均马哈拉诺比斯距离 $Div = \frac{2}{|\mathcal{D}_{demo}|(|\mathcal{D}_{demo}|-1)}\sum_{i<j}\sqrt{\mathbf{D}_{ij}^T \Sigma^{-1} \mathbf{D}_{ij}}$，其中 $\Sigma$ 是样本表示的经验协方差矩阵。后续候选样本只有在加入后不降低整体多样性时才被保留。
    - 设计动机：单纯基于困惑度的 OOD 评分存在隐患：困惑度主要衡量 token 级别的流畅度，对短文本和高频语言模式有偏好，导致检索结果倾向于选择相似的短样本。加入多样性约束后，能强制算法在保证相似度的前提下，展开演示的表示空间。

3. **无目标域演示检索算法**:

    - 功能：整合 OOD 评分和多样性约束，执行演示选择的完整流程。
    - 核心思路：首先单遍遍历源域数据，用 OOD 得分筛选出规模为 k 的候选集 $\hat{\mathcal{D}}_S$。然后对候选集按与测试样本的相似度排序，初始化演示集为相似度最高的 N×|Y| 个样本。最后从剩余候选中逐一考察，只要加入不会减少多样性就将其纳入演示集，直到达到目标规模。
    - 设计动机：两层筛选机制分工明确：OOD 评分处理"分布偏移"问题（哪些样本来自目标邻域），多样性约束处理"表示覆盖"问题（演示间是否互补）。
