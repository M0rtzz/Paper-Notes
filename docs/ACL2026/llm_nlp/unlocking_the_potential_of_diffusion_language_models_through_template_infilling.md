...
完整流程可以概括为三步。第一步，根据任务构造一个静态结构模板，论文为了验证普适性，避免了复杂 prompt engineering。第二步，把模板和 mask 放入 DLM 的并行采样过程，模型在所有空段中同时去噪。第三步，若某个片段的置信度持续偏低，则用 DSA 扩展该片段的 mask 长度，让模型获得更多表达空间。

### 关键设计
1. **全局模板锚点**:

	- 功能：把原本只在开头出现的软提示，变成贯穿输出空间的结构边界条件。
	- 核心思路：模板由多个固定锚点组成，每个锚点承担不同的推理角色，例如计划、步骤、检查、答案。DLM 在填充任意片段时都可以同时注意到所有锚点，因此生成目标从“接着前缀写”变成“在既定骨架内完成整段响应”。
	- 设计动机：DLM 的优势是双向和任意位置条件化，prefix prompting 没有利用这一点。全局锚点把高自由度采样变成有边界的搜索，减少逻辑漂移，同时保留并行生成能力。

2. **动态片段分配 DSA**:

	- 功能：解决静态模板 mask 长度过短导致推理被截断的问题。
	- 核心思路：在每个扩散步骤中监控片段内 token 的预测置信度。如果某个片段最不确定 token 的概率低于阈值 $\tau$，则把该片段长度从 $|M_i|$ 扩展为 $|M_i|+\delta$。论文实验中最多允许每步扩展 8 个 token，最多扩展 10 次。
	- 设计动机：不同问题需要的推理空间不同。固定长度的模板会把复杂题压缩到过短片段里，DSA 则让模型在结构不变的前提下自己申请更多“纸面空间”。

3. **结构先验对采样轨迹的重排**:

	- 功能：把无序的并行去噪轨迹引导成更接近全局计划的生成顺序。
	- 核心思路：论文观察到 Dream-Base 这类从自回归模型适配来的 DLM，在无条件生成时会出现混乱填充顺序；加入模板后，模型往往先稳定结构锚点附近的信息，再同步补齐空缺内容。安全实验中的“Draft-Critique-Refine”模板也体现了同一机制。
	- 设计动机：TI 不只是 prompt 文字变多，而是在采样空间里注入一种结构先验。它让模型先满足高层约束，再填局部细节，因此在快速采样和长输出下更稳。
