...
### 关键设计
1. **截断 SHA-256 哈希作为可发布文本替身**:

    - 功能：把受版权保护的 token 序列转换成不可读的技术标识，使语料创建者可以公开标注与哈希，而不直接发布原文。
    - 核心思路：每个 token 先用 SHA-256 得到哈希，再把哈希截短到较少字符。完整 SHA-256 几乎不会碰撞，但太容易被词表预计算表攻击；截断后，不同 token 会共享同一个短哈希，攻击者即使知道语言词表，也只能得到一组候选词，无法确认真实 token。
    - 设计动机：这里的“碰撞”不是错误，而是安全机制的一部分。论文需要在两个目标之间折中：哈希太长，安全性弱；哈希太短，对齐错误增加。实验最后选择长度 2 作为主要设置，因为它能制造足够多的候选碰撞，同时保持可接受的对齐质量。

2. **基于哈希序列的鲁棒对齐**:

    - 功能：把创建者的标注映射到用户自己的文本版本上，允许用户版本和创建者版本存在小范围差异。
    - 核心思路：论文使用 Python 标准库 difflib 中的增强 gestalt pattern matching 算法，对 $f(X)$ 和 $f(X')$ 做序列对齐。两个位置只有在截断哈希相等时才可能被直接匹配，匹配后系统假设它们对应同一个 token，并转移标注。为了降低长篇小说上的二次复杂度开销，如果两端章节数一致，作者按章节对齐；章节结构不一致时才对整本书直接对齐。
    - 设计动机：用户版本可能有额外 token、缺失 token 或局部替换。编辑距离式全局对齐能利用上下文顺序，让短哈希碰撞在多数情况下被邻近 token 消解；按章节处理则利用文学文本天然结构，避免全书级对齐的运行时间爆炸。

3. **面向未对齐位置的补救策略组合**:

    - 功能：在初始哈希对齐之后，继续修复由版本差异、tokenization、大小写和 OCR/拼写错误造成的缺口。
    - 核心思路：propagate 策略把已经对齐过的相同哈希位置当作投票来源，用多数 token 推断尚未对齐位置；retokenize 策略枚举用户 token 的切分或相邻 token 的合并，检查重新哈希后是否匹配创建者哈希；case 策略尝试不同大小写变体；mlm 策略在用户侧上下文中插入 `[MASK]`，用 ModernBERT-base 预测候选词，再验证其哈希是否等于创建者哈希。pipe 元策略按 retokenize、mlm、case、propagate 的顺序串联这些方法。
    - 设计动机：不同差异来源需要不同修复机制。retokenize 对 tokenizer 不一致特别有效，case 处理大小写差异，mlm 利用用户本地上下文补缺词，propagate 利用长文本中重复词的冗余。作者把更高精度的策略放在前面，是为了先修复最可信的错误，减少后续传播错误。
