...
1. **步骤前激活提取与线性可分性度量**:
    ...
    - 功能：在每个 `Step k:` 标记前一刻提取所有层的隐状态 $\mathbf{h}_{t(\text{Step }k)-1}^{(\ell)}$，再用 one-vs-rest 二分类 probe 测各步是否占据独立线性子空间。
    - 核心思路：对每个 step $k$ 训一个线性分类器 $\hat y = \mathrm{sign}(w^\top h)$，positive 是"恰好完成第 $k$ 步"的激活，negative 是其它步；同时跑跨步骤、跨模型变体迁移以排除"只是表面格式"假设。
    - 设计动机：在标记前取激活是为了拿到"已积累的推理状态"而非格式信息；线性可分性是 representation learning 中"信息可被下游直接读出"的标准判据。

2. **轨迹距离差与中段正确性预测**:

    - 功能：对每条轨迹的相邻步骤计算距离 $d(\mathbf{h}_{t(a)-1}^{(L)}, \mathbf{h}_{t(b)-1}^{(L)})$（Euclidean 衡量幅度、cosine 衡量方向），用 $\Delta(\text{Incorrect}-\text{Correct})$ 找正确/错误轨迹开始分叉的步骤。
    - 核心思路：发现早步距离统计上不可分（95% CI 重合），后段（约第 4 步之后）开始系统性分叉；以此为基础在 ROC-AUC 框架下用线性分类器从后段激活 + 距离特征预测最终对错，平均 AUC 0.83、layer 29 峰值 0.87。
    - 设计动机：把"对错信号"从最终 token 推到中段，可以在答案输出前就触发干预，节省 test-time compute 并避免错误传播。

3. **轨迹 steering：基于理想轨迹的自适应低秩修正**:

    - 功能：运行时实时追踪当前轨迹与"正确轨迹质心"的偏离度；超过容忍阈值就在当前激活上加一个低秩 steering 更新 $\mathbf{h}' = \mathbf{h} + \alpha U V^\top \mathbf{h}$，把模型推回理想方向。
    - 核心思路：理想轨迹 = 正确样本激活的平均路径；steering 矩阵从正确-错误差向量奇异分解取低秩近似；同样地，把激活向 termination 子空间 push/pull 可控推理长度（短/长 CoT）。
    - 设计动机：相比无条件 test-time scaling 注入 "Wait" / "Let me think again" 等 token，门控 + 轨迹偏离量给出了"何时干预 + 干预多少"的客观判据，扰动只发生在错误轨迹上，对正确轨迹近乎透明。
