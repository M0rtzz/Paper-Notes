...
description: >-
  [ACL 2026][LLM推理][测试时计算] 这篇论文把 self-consistency、self-refinement、multi-agent debate 和 Mixture-of-Agents 放到统一计算预算下比较，发现多智能体推理尤其是 MoA 在 Pareto 前沿上更高效…
tags:
  - "ACL 2026"
  - "LLM推理"
  - "测试时计算"
...
1. **统一的测试时计算扩展坐标系**:
    ...
    - 功能：把四类结构差异很大的推理方法放到同一张 cost-accuracy 图上比较
    - 核心思路：把扩展方式拆成并行扩展和顺序扩展。self-consistency 主要增加并行 CoT 样本数；self-refinement 主要增加顺序修正步数；debate 同时有智能体数和讨论轮数；MoA 同时有 proposer 数和聚合层数。每个配置都被映射成一个“准确率 + 估计时间成本”的点
    - 设计动机：过去常见比较容易混淆“方法更好”和“计算更多”。统一坐标系让 debate 与 MoA 这类多智能体方法必须在同样预算下和 self-consistency 竞争，结论更接近部署场景

2. **面向部署的计算成本估计**:

    - 功能：用比 FLOPs 更贴近实际推理延迟的方式衡量测试时预算
    - 核心思路：对 prefill 和 decode 分别估计两类时间：计算时间由 $2 \cdot P \cdot T$ 形式的 FLOPs 决定，内存传输时间由模型参数量、量化精度、batch size、GPU 数和显存带宽决定；每个阶段取 $\max(\text{FLOP time}, \text{memory time})$，再求和得到总时间
    - 设计动机：小模型多次调用在 FLOPs 上可能看起来便宜，但真实推理还要反复搬运权重。把内存传输计入成本后，才能更公平地判断“小模型多算几轮”和“大模型少算几轮”的取舍

3. **用 Pareto-front 提炼多智能体设计规则**:

    - 功能：从大量配置中找出真正高效的参数组合，而不是只看单点最高分
    - 核心思路：作者对 34 个配置、100 多次评估计算 Pareto 前沿，然后观察前沿上 debate 与 MoA 的参数规律。debate 中，高效点主要来自增加 agent 数而不是增加 round；MoA 中，高效点多出现在 proposer 数比 layer 数多 1 的配置，例如 3 models/2 layers、4 models/3 layers、5 models/4 layers
    - 设计动机：多智能体系统参数空间大，盲目加 agent 或加 round 容易浪费预算甚至降分。Pareto-front 能把“值得扩展的方向”从实验数据里抽出来，形成可执行的配置建议
