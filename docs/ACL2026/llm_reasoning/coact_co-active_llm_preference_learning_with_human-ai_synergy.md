...
1. **基于自一致性的偏好对构造与划分**:
    ...
    - 功能：用一份"一致性分数"同时承担两件事 —— 给样本打偏好对、给样本贴"可信度标签"。
    - 核心思路：对每条 $x$ 抽 $k$ 次 response，按答案抽取后算 $C(y)=\frac{1}{k}\sum_m \mathbf{1}\{\text{ans}(y_m)=\text{ans}(y)\}$，以最 consistent 为 chosen、最不 consistent 为 rejected。再用阈值 $\tau$ 把 batch 切成两堆：$C(y^+)\geq\tau$ 是高一致（默认 self-label 候选）、$C(y^+)<\tau$ 是低一致（直接送 Oracle）。
    - 设计动机：避免 self-rewarding 类方法常用的 LLM-as-Judge 评分（成本高且偏置大），用纯统计指标做最便宜的 proxy；同时把"用 AI 自标"和"找 oracle 该标谁"这两个看似矛盾的目标用同一个信号统一起来。

2. **k-NN 距离驱动的高一致样本隐患检出**:

    - 功能：在高一致的"看似可信"样本里，把"自洽但其实错"的样本挖出来送给 Oracle，堵住 self-bias 放大的口子。
    - 核心思路：把上一轮 oracle 验证过的正确偏好样本的 penultimate hidden state $\phi(x)/\|\phi(x)\|_2$ 当作 in-distribution（ID）集 $\mathcal{Z}_{ID}^{(t)}$；对当前高一致样本计算 $r_k(z_i)=\min_{z\in\mathcal{Z}_{ID}^{(t)}}\|z_i-z\|_2$，距离最大的 Top-$M_{high}$ 视为 OOD，送 Oracle。这是经典 OOD detection 思路 (Sun et al. 2022) 在 preference learning 上的迁移。
    - 设计动机：单看 self-consistency 容易漏掉"模型对错误命题异常自信"的情况；用 k-NN 给高一致样本再加一层"是否落在已知正确分布内"的校验，把最可能反噬训练的样本优先交给人。实验里这个策略让 GSM8K/MATH/WebInstruct 上 oracle 命中的错误率分别提到 82.56% / 87.32% / 84.17%，远高于随机选与最低 consistency 选（≈70%）。

3. **Oracle 引导的指令增广 + 修正 DPO 目标**:

    - 功能：让 oracle 反馈不仅产生标签，还充当"难度过滤器"，引导模型生成处在自己可解范围内的新指令；同时用 NLL 正则稳住 chosen 序列的 likelihood。
    - 核心思路：取高一致里被 oracle 标为正确的样本 $\mathcal{D}_{correct}^{(t)}$ 作为 ICL demo，让模型生成 $N_{new}$ 条新 instruction $\mathcal{D}_{new}^{(t)}$，再跑一遍自一致流程过滤。优化目标在 DPO 之外加了 NLL 项：$\mathcal{L}(\theta_t)=-\mathbb{E}[\log\sigma(\beta\log\frac{\theta_t(y^+|x)}{\theta_0(y^+|x)}-\beta\log\frac{\theta_t(y^-|x)}{\theta_0(y^-|x)})-\alpha|y^+|\log\theta_t(y^+|x)]$，其中 $\beta=0.5,\alpha=1$。
    - 设计动机：MetaMath 类合成数据常常超出基础模型可解范围，导致 self-label 信号噪声爆炸；用 oracle-verified 正确样本当 anchor 生成新指令，等于把"难度上限"锁在模型可解的范围内，配合 DPO+NLL 防止 chosen log-prob 在迭代中崩塌。
