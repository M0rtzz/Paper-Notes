...
### 关键设计
1. **多视角 persuasive prompt 生成**:

	- 功能：从不同心理和话语策略出发，探索能让 LRM 缩短推理的提示空间。
	- 核心思路：Whisper 使用 emotional appeal、threat、evidence-based persuasion、role-playing 以及 detailed instruction 等视角。比如 evidence 视角会引用“短解释同样有效”的研究式论证，role-play 视角会让模型扮演需要极简表达的专家。
	- 设计动机：单一 “Be concise” 指令太弱，多视角生成可以覆盖模型对权威证据、角色约束、情绪语境和结构化要求的不同敏感性。

2. **准确率约束下的候选筛选**:

	- 功能：避免把模型压缩成短但错的回答。
	- 核心思路：候选 $P_{adv}^j$ 在开发集上计算平均长度 $L_{avg}^j$ 和准确率 $ACC_{avg}^j$；若准确率下降超过容忍阈值 $\tau$，候选直接丢弃；其余候选按平均长度排序。
	- 设计动机：推理压缩的真正目标是 efficiency-performance trade-off，而不是单纯少输出 token。NoThinking 很短，但准确率大幅下降，因此不能算成功。

3. **迭代 refinement**:

	- 功能：让 prompt generator 从上一轮有效后缀中学习，逐步靠近更优的压缩策略。
	- 核心思路：每轮选出的 top-k 后缀作为下一轮 exemplars，GPT-4o 基于这些例子继续合成候选。论文实验发现压缩收益从第一轮到第三轮持续提升，之后收益趋于饱和。
	- 设计动机：手写 prompt 很难一次到位；把有效候选反馈给生成器，相当于在黑盒空间里做轻量级 prompt evolution。
