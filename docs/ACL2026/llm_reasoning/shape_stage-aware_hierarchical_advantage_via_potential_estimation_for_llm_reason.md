...
1. **熵驱动的轨迹切分与潜势估计**:
    ...
    - 功能：把长推理链切成更接近语义决策点的段，并为每个段边界估计状态质量。
    - 核心思路：作者借鉴 SPO 的 adaptive cutpoint partition，但不用低概率 token，而是用 token-level entropy 寻找高不确定性位置。熵超过阈值的位置作为候选切分点，再下采样到固定段数 $K$。在每个边界 $s_k$，模型执行若干短 rollout，以正确率均值估计 $\Phi(s_k)$；为控制成本，使用 vLLM prefix caching 和较短的 rollout 长度。
    - 设计动机：数学推理的关键转折往往发生在模型不确定的位置。用换行等刚性分隔符可能切到格式边界而非逻辑边界，导致潜势估计噪声更大。

2. **长度感知的 Stage-aware Advantage**:

    - 功能：在段级别奖励有效进步，并惩罚高潜势状态下的冗长推理。
    - 核心思路：SHAPE 使用 $A_k=R_{outcome}+\alpha(\gamma_k(L_k)\Phi(s_{k+1})-\Phi(s_k))$。其中 $\gamma_k(L_k)$ 随段长 $L_k$ 线性下降，最低为 $\gamma_{min}$。把 shaping 项近似展开后，可以看成潜势增益 $\Delta_k$ 减去一个 reasoning tax：$(1-\gamma_k)\Phi(s_k)$。段越长，tax 越大；当前潜势越高，继续啰嗦的成本越高。
    - 设计动机：低潜势状态中的突破更难、更有价值，应被鼓励；高潜势状态中的冗余细化往往是 overthinking，应被压制。这个公式用一个动态折扣同时编码这两点。

3. **段内 token 级信用重分配**:

    - 功能：避免把同一段里的所有 token 视为同等重要。
    - 核心思路：在每个段内对 token 熵做 z-score 标准化，得到局部重要性权重 $w_t=clip(1+\beta\tilde{H}(x_t),\delta_{min},\delta_{max})$，最终 token advantage 为 $A_t=A_k\cdot w_t$。平均熵 token 近似保留原段级优势，高熵 token 获得放大。
    - 设计动机：推理链里真正改变方向的 token 往往出现在高不确定性位置；用段级 reward 做稳定锚点，再局部重分配，可以比直接对全局 outcome reward 做 token shaping 更低方差。
