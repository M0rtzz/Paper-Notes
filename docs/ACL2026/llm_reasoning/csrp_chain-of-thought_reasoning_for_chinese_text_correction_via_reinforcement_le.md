...
1. **Balanced Continued Pre-training**:
	...
	- 功能：把中文语言规范和学习者错误分布写入模型参数。
	- 核心思路：从 wiki-zh-25、wiki-zh-23、cci2、lang8+HSK 等来源构造原始数据，经 MinHash 去重和启发式过滤后，从 7,287,295 条压缩到 5,901,700 条高质量样本；训练时采用约 4.72M 通用样本和 1.18M 纠错样本的 8:2 混合。
	- 设计动机：直接 SFT 很难弥补通用模型对中文非规范错误的知识稀疏问题，CPT 先提供底层语言约束。

2. **Rationale-Augmented SFT**:

	- 功能：让模型在执行纠错前学习定位、分类和解释错误。
	- 核心思路：teacher 只生成 source 与 gold target 之间的中间 rationale，而不是自由给出最终纠错；rationale 需符合 `<think>...</think>` 格式，并经过过滤。1,000 条随机 rationale 的双盲人工评估显示 95.2% 被判定为 linguistically faithful，Cohen's $\kappa=0.81$。
	- 设计动机：普通 SFT 把纠错当黑盒翻译，模型容易过度改写；诊断式 CoT 能把“哪里错、错在哪类、为什么改”显式化。

3. **Efficiency-Aware Policy Alignment**:

	- 功能：用强化学习校准“edit-or-not”边界，减少过纠正。
	- 核心思路：定义 Relative Improvement $RI=\frac{d(S,G)-d(P,G)}{d(S,G)+\epsilon}$ 和 Edit Efficiency Ratio $\eta=\frac{d(S,G)-d(P,G)}{d(S,P)+\epsilon}$，其中 $d$ 是 Levenshtein 距离。奖励函数对接近 gold 的高效编辑给高分，对无效编辑和空输出惩罚；当原句本来正确时，保持不变得 +2.0，任何修改得 -2.0。
	- 设计动机：GEC 的主指标 $F_{0.5}$ 更重 precision。奖励必须告诉模型“不要为了流畅而改”，而不是只奖励更像参考答案。
