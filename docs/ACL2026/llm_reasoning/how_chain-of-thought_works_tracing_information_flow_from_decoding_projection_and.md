...
1. **解码层的结构遵循分析**:
	...
	- 功能：判断 CoT 是否通过模板化结构约束输出，而不是完全依赖自由生成的逻辑。
	- 核心思路：作者定义四类 test points：time、action、loc&peo、number，统计生成文本中这些关键词来自 prompt 还是来自问题；进一步定义 CoT reasoning structure，即 input entity 经过 operation 得到 generated entity，再接 final answer statement，并用 Imitation Count 衡量结构遵循度。
	- 设计动机：如果 CoT 的收益来自结构模板，那么结构遵循度应该和任务准确率强相关；这比只看最终准确率更能解释 prompt 格式为何重要。

2. **概率投影层的置信度分析**:

	- 功能：观察 CoT 是否让模型在答案生成阶段更确定。
	- 核心思路：作者跟踪常见决策短语“answer is ...”的 token probability sequence，并用 KDE 比较 CoT 与 standard prompt 的概率分布；对 AQuA、Sports、Coin Flip 这类闭集答案空间，取答案候选 token 的 top-k 概率并计算 entropy。
	- 设计动机：如果 CoT 真在收窄解码空间，那么生成答案 token 时概率应该更集中、熵更低，而不是只在文本上多输出几步解释。

3. **FFN 激活层的任务依赖调制**:

	- 功能：分析 CoT 在内部计算层是否改变神经元参与方式。
	- 核心思路：作者把 FFN 激活函数输出大于 0 的单元视为 activated neuron，统计生成过程中各层平均激活数量，并比较 CoT 与 standard prompt 的差值。总体激活看全局效率，layer-wise difference 看 CoT 主要影响哪些层。
	- 设计动机：如果 CoT 只是输出格式变化，内部激活不一定有系统性差异；但论文发现差异集中在后 1/3 层，并且开放域任务和闭集任务方向相反，说明 CoT 对内部处理方式有任务依赖影响。
