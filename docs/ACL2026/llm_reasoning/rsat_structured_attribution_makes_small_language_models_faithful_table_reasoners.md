...
1. **结构化单元格级推理输出**:
    ...
    - 功能：把答案、推理步骤和证据坐标统一放进可解析 JSON，使每个 claim 都能被人工或程序追溯到表格中的具体单元格。
    - 核心思路：表格序列化时显式保留行号和列位置；输出中每个 reasoning step 都必须给出若干 `[row, col]` 坐标。这样 citation 不是段落级或 passage 级，而是表格结构中最小可审计单元。
    - 设计动机：表格中的错误通常来自选择错行、错列或把多个单元格关系串错。单元格级引用能暴露这些错误，也让“答案正确但证据不对”的情况可见。

2. **SFT 只负责教格式，GRPO 负责教质量**:

    - 功能：把“会输出”与“输出可信”拆开优化，避免把所有能力寄托在少量人工或教师 trace 上。
    - 核心思路：SFT trace 只需要通过 JSON、坐标边界和步数检查，因此能稳定教会格式；GRPO 阶段不需要 trace，只用 gold answer 和奖励函数评价模型自己生成的候选。
    - 设计动机：论文发现 SFT 后格式成功率和引用合法性接近 99%，但忠实性只有 0.224，说明监督 trace 的表面模仿不足以保证真实 grounding。

3. **以 NLI 忠实性为核心的复合奖励**:

    - 功能：同时优化答案、引用合法性、引用忠实性、引用简洁性和格式约束。
    - 核心思路：奖励为 $R=R_{ans}+0.3R_{cite}+0.5R_{faith}+0.2R_{pars}+R_{fmt}$。其中 $R_{faith}$ 把每一步引用单元格的值拼成 evidence string，再用 DeBERTa-v3-base NLI 判断它是否蕴含该步文本；$R_{pars}$ 惩罚一次引用过多单元格；JSON 解析失败会触发硬格式惩罚。
    - 设计动机：只奖励答案会让模型忽略引用，只奖励合法坐标会让模型随机引用真实单元格。NLI 忠实性把“引用是否支撑 claim”变成可优化信号，是整篇论文最关键的设计。
...
- [\[NeurIPS 2025\] Mapping Faithful Reasoning in Language Models](../../NeurIPS2025/llm_reasoning/mapping_faithful_reasoning_in_language_models.md)
