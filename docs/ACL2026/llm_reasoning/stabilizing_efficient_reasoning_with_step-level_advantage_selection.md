...
1. **短上下文压缩效应的解耦实验**:
	...
	- 功能：证明 4K 短上下文后训练本身已经是强压缩信号。
	- 核心思路：作者从 DeepScaleR-1.5B-Preview 出发，只用普通 GRPO 和任务正确性奖励，在 4K 最大上下文下训练，不加入任何长度惩罚。结果显示输出长度会在训练早期快速下降，并接近甚至短于 LAPO、ThinkPrune 等高效推理方法。
	- 设计动机：如果不先解耦上下文长度，就无法判断已有方法的收益到底来自长度奖励还是训练上下文变化。这个观察也解释了为什么简单短上下文训练会“看起来有效”，但后期准确率波动和熵坍塌严重。

2. **正确 rollout 中的低置信步骤屏蔽**:

	- 功能：避免模型把正确答案里的自我怀疑、重复检查、无关绕路也当作优质推理来强化。
	- 核心思路：对 reward=1 的 rollout，将推理轨迹切成步骤，计算每一步平均 log probability 作为置信度，选择比例 $r$ 的低置信步骤，把这些步骤中所有 token 的 advantage 设为 0，其余步骤保留正 advantage。
	- 设计动机：正确答案不等于每一步都值得学习。长 CoT 中常有冗余验证和模式化重复，rollout 级正反馈会把这些冗余行为固化，进一步造成过度推理。

3. **失败 rollout 中的高置信步骤保护**:

	- 功能：缓解短上下文截断导致的误惩罚。
	- 核心思路：对 reward=0 的 rollout，同样计算步骤置信度，但选择高置信步骤并把它们的负 advantage 改成 0。低置信或明显错误步骤仍保留负 advantage。
	- 设计动机：作者发现把 8K 正确 rollout 截成 4K 后，约 29% 会因为缺少最终答案或收尾步骤而被 verifier 判为失败。这些样本中很多中间推理是正确的，直接给全轨迹负信号会破坏训练稳定性。
