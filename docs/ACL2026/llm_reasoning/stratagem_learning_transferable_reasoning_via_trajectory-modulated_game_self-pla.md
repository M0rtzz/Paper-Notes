...
flowchart TD
    A["三个文本零和游戏自博弈<br/>Tic-Tac-Toe / Kuhn Poker / Simple Negotiation"] --> B["共享 LLM policy<br/>角色条件区分当前行动方"]
    B --> C["游戏轨迹<br/>多轮状态 / 回复 / 推理 / 动作"]
    C --> D["SPIRAL 计算游戏优势 A_game<br/>终局胜负 + 角色 baseline"]
    C --> EVAL["GPT-4 评估器<br/>给整条轨迹的推理打分"]
    EVAL --> E["推理可迁移系数 φ<br/>抽象推理保留优势, 规则套路压低"]
    EVAL --> F["推理演化奖励 ψ<br/>推理随上下文深化得正奖励"]
    D --> G["轨迹优势调制训练<br/>A_mod = A_game·φ + β·ψ"]
    E --> G
    F --> G
    G --> H["self-play 策略梯度更新"]
    H -->|不混入下游数据| I["迁移到数学 / 通用推理 / 代码"]
```
