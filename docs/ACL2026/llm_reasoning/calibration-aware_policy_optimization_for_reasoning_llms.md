...
**3. gradient analysis：从梯度形状解释为什么 CAPO 能同时拉高 AUC 和 acc。**

这一点不是新模块，而是把前两个设计的效果讲透。因为 $\phi'(t)=-\sigma(-t)$ 在 $t<0$ 区间数值最大，CAPO 的 advantage 恰好在"correct 且高 PPL"或"incorrect 且低 PPL"这类 PPL-misranked 样本上给最大梯度，而这些正是决策边界上信息量最丰富的样本。对照之下，GRPO 给所有 correct 一律 $+|\bar R|$、所有 incorrect 一律 $-|\bar R|$，完全不区分 uncertainty。所以 CAPO 并不是简单"外挂一个 calibration regularizer"，而是从根上改变了 sample-level 的权重分布——把训练从"扁平 reward"升级成"难样本聚焦 + 自动校准"。
