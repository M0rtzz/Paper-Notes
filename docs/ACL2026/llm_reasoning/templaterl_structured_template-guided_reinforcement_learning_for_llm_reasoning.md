...
1. **基于 MCTS 的模板构造与复杂度感知**:

    - 功能：从小规模高质量示例中自动抽象出高层推理策略
    - 核心思路：对每个种子问题，用 MCTS 构建多条解答树，每条树的边代表一个"动作"（如"提出子问题""推导下一步"）。对树中的每条路径，用评分函数 $\text{Score}(\mathbf{s}_i, \mathbf{t}_{i,j}) = b \cdot R(\mathbf{t}_{i,j}|\mathbf{s}_i) - (1-b) \cdot C(\mathbf{t}_{i,j})$ 平衡正确性和复杂度，筛出最优路径。所有路径的动作序列再按"问题条件复杂度"（PCC，即问题中前置条件的数量）聚类，构成模板库 $\mathcal{L} = \{\hat{T}_1, \ldots, \hat{T}_{|\mathcal{L}|}\}$，每个模板记录其动作序列和对应问题的平均 PCC。
    - 设计动机：PCC 是问题难度的代理指标。通过按 PCC 分组，后续检索时可快速找到与新问题难度匹配的模板，避免用简单问题的模板去引导复杂问题，或反之。

2. **自适应模板检索与多群组 RL 优化**:

    - 功能：在训练时根据问题复杂度动态选择合适的模板，并分离不同模板的优化目标
    - 核心思路：对训练问题 $\mathbf{q}$，计算其 PCC，定义与模板 $\hat{T}_j$ 的距离为 $d(\mathbf{q}, \hat{T}_j) = |{\rm PCC}(\mathbf{q}) - {\rm PCC}_{T_j}|$，选出距离最近的 top-$k$ 模板。对每个模板 $T_i$，采样 $G_i$ 条轨迹，沿模板动作序列逐步生成。将多群组的轨迹合并进 GRPO 损失：$\tilde{\mathcal{J}}_{\text{GRPO}}(\pi_\theta) = \frac{1}{\sum_i G_i} \sum_i \sum_j \sum_t \min[\rho_{i,j,t} A_{i,t}, \hat{\rho}_{i,j,t} A_{i,t}]$，其中 $\rho$ 是概率比，$A$ 是优势估计。这等价于对每个模板定义子目标 $\mathcal{J}_i(\pi_\theta)$，整体目标变成加权平均。
    - 设计动机：(i) 分离模板使模型在不同策略模式上学习，避免单一奖励信号淹没多样性；(ii) 论文证明了两个理论性质：Prop 3.1 说多群组分组能增加至少获得一条正向轨迹的概率，Prop 3.2 说相似问题的模板迁移能提升成功概率。

3. **动态模板库扩展**:

    - 功能：让模板库在训练和推理时持续演进，不局限于初始的静态库
    - 核心思路：训练过程中，对正确的生成轨迹，用关键词抽取或轻量级模型解析其动作序列 $T' = (a_1', \ldots, a_d')$，加入库中。推理时，对每个测试样本，用 5 个模板生成多条路径，多数投票选答案，再从投票结果中提取新模板加入库，继续下一个样本。这样形成持续学习循环。
    - 设计动机：模板库不是一成不变的。通过从成功示例中吸收新策略，模型可以在特定领域逐渐积累推理技巧，特别有利于需要迭代更新的领域（如医学推理）。
