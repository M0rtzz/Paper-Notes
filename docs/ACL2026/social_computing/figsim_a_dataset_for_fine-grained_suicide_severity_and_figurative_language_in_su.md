...
### 关键设计
1. **临床量表与在线传播指南结合的标注体系**:

	- 功能：把 meme 中的心理健康风险拆成可操作标签，而不是停留在“是否自杀相关”的二分类。
	- 核心思路：严重度标签借鉴 C-SSRS，并额外加入 suicide death；自杀相关内容标签借鉴 #chatsafe 和 Mindframe，区分方法/地点描绘、保护性因素、有害因素和第三人称描述。
	- 设计动机：自杀相关内容的影响取决于表达方式、严重度和上下文。细粒度标签能帮助研究哪些内容更可能需要不同等级的审核或干预。

2. **心理健康专家参与的多轮标注流程**:

	- 功能：提高敏感任务的标注一致性，并把主观判断限制在明确 guideline 下。
	- 核心思路：先进行两轮 pilot annotation，每轮 50 张，由注册心理学家和研究者参与；再通过 calibration batch 检查一致性；大规模标注时两名标注者独立标注，第三名标注者 adjudicate 分歧。
	- 设计动机：figurative expression 和 suicide severity 都高度主观，不能直接交给众包无校准标注。专家参与能提升标签的概念清晰度和伦理稳健性。

3. **多类型模型基线与偏差分析**:

	- 功能：系统判断哪些模型在 suicide meme 理解上更可靠，并揭示错误类型。
	- 核心思路：评测 BERT/MentalBERT/RoBERTa、ResNet/ViT/DINOv2、CLIP/BLIP-2、多种 MLLM、Yadav et al. 和 M3H 等任务邻近模型；随后分析 per-label F1、严重度 over/underprediction、context-required 和 modality 相关错误。
	- 设计动机：只报告平均 F1 会掩盖最关键的安全风险。对于内容审核，高严重度低估比低严重度误报更值得关注。
