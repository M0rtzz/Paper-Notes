...
1. **上下文条件的 AMD 定义**:
	...
	- 功能：区分真正的情感理解分歧与单纯上下文使用差异。
	- 核心思路：边缘 AMD 比较 `Mbar_1(.|x)` 与 `Mbar_2(.|x)`，条件 AMD 比较同一上下文下的 `M_1(.|x,c)` 与 `M_2(.|x,c)`，上下文差异则比较 `P_1(.|x)` 与 `P_2(.|x)`。论文证明边缘 AMD 可被上下文差异和条件 AMD 上界分解。
	- 设计动机：两个人可能都把 “fine” 理解成同样情绪，只是一个人在收尾语境用、另一个人在抱怨语境用；如果不控制上下文，会把用法差异误判成意义分歧。

2. **修复博弈与分岔理论**:

	- 功能：解释为什么连续增加的 AMD 会导致突然的对话坍塌。
	- 核心思路：修复优势为 `Delta U(q)=Bq-c0`，AMD 以 `kappa=c0+lambda D` 的形式增加负载；熵正则 best response 为 `q_{t+1}=sigma(beta(alpha q_t-kappa))`。当 `beta alpha <= 4` 时固定点唯一且连续；当 `beta alpha > 4` 时存在两个稳定吸引子和一个不稳定中间点，跨过 `kappa_+` 会从高修复状态跳到低修复状态。
	- 设计动机：这给“隐藏漂移后突然破裂”一个可检验机制，并自然产生 hysteresis：关系崩掉后，不是把负载降回触发点就能恢复，而要降到更低的 `kappa_-` 以下。

3. **构念对齐的经验估计**:

	- 功能：把理论中的 `P_i(s|x,c)` 近似成真实对话里的可计算量。
	- 核心思路：用 RoBERTa-base GoEmotions 分类器输出 27 类情绪加 neutral 的分布；用 regex token 提取出现至少 3 次的锚词；上下文由前一轮 dialog act 与 TF-IDF topic cluster 组成；同一说话者含有锚词和上下文的 utterance 情绪分布被平均为 `M_i`。
	- 设计动机：作者没有简单用情感词典平均，而是用上下文情绪分类器来接近 formal construct，同时明确承认这仍是 proxy，不是潜在情感意义的直接观测。
