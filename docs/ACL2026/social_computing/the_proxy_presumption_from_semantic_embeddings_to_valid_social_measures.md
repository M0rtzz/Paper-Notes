...
1. **不可识别性论证**:
	...
	- 功能：解释为什么“无监督 embedding 自动分离社会构念”这个假设不成立。
	- 核心思路：作者令潜变量 $h=[c;z]$ 服从各向同性高斯先验。对任意正交旋转矩阵 $R$，旋转后的潜空间 $h'=Rh$ 与原潜空间会给出相同的观测分布，因此无监督似然目标无法区分“真实构念坐标”和“构念与 nuisance 的线性混合”。
	- 设计动机：这把 proxy presumption 从经验批评提升为识别问题。即使真实世界中 $c$ 与 $z$ 独立，模型学到的 embedding 也可能把二者混在一起，不能靠更大的语料或更深的 encoder 自动解决。

2. **Counterfactual Neutralization**:

	- 功能：在得分函数层面抵消主题、风格、实体等 nuisance 对 proxy 的贡献。
	- 核心思路：不是直接报告 $f(e_{obs})$，而是构造一个反事实中性文本，例如移除 stance、novelty claim 或情绪表达但尽量保留主题内容，计算 $\hat{C}=f(e_{obs})-f(e_{base})$。这个差分分数希望留下由目标构念变化引起的部分。
	- 设计动机：很多场景拿不到完整 nuisance label，也不方便重新训练 embedding。反事实重写提供了一个 text-native intervention，可以利用 LLM 的生成能力，在输入保持可读的同时显式操控构念强度。

3. **Construct Validity Protocol**:

	- 功能：给 embedding-based social measures 提供一套可报告、可复现、可审计的效度流程。
	- 核心思路：Phase 1 要产出 construct map、facet blueprint 和三档 exemplar set；Phase 2 要说明输入、表征和 scoring function 如何控制 nuisance；Phase 3 要报告 Validity Card，包括可靠性/稳定性、收敛效度、区分与增量效度、known-groups validity 和 criterion-related evidence。
	- 设计动机：很多论文只证明 proxy “有用”或“相关”，但没有证明它不是另一个 nuisance 的替身。CVP 把“测得准不准”拆成多个互补证据，尤其强调区分效度和增量效度，因为它们最能暴露 topic/style surrogacy。
