...
1. **MoRE 道德驱动的实体化认知架构**:
    ...
    - 功能：让 agent 不是固定策略表，而是具有道德价值、记忆、判断、规划和反思的认知体。
    - 核心思路：agent 的记忆不是简单事件日志，而是围绕实体组织：对每个其他 agent 维护互动历史、关系判断和计划；对猎物维护狩猎历史、协作计划和分配记录；对家庭成员维护状态和照护计划；对繁衍维护前提和时机判断。
    - 设计动机：道德演化很多机制依赖“我如何看待他人”而不是单步收益。实体化记忆能让 agent 形成声誉、信任、怀疑和报复计划，比 event log 更接近人类社会推理。

2. **Social-Evol 狩猎采集生态环境**:

    - 功能：提供资源约束、合作机会、竞争风险和代际选择压力。
    - 核心思路：agent 有 HP、年龄、体能和寿命约束。采集植物稳定但收益低；狩猎收益高但可能失败并损失 HP，合作能提高成功率和分摊风险；繁衍需要年龄和 HP 门槛，且会消耗父母 HP；分配和沟通允许合作形成，抢夺和战斗允许竞争策略显性化。
    - 设计动机：如果环境只有抽象收益矩阵，就无法观察到“沟通成本导致错过合作”“家庭照护拖垮父母”“自利者互相攻击”等动态。Social-Evol 把这些机制作为可观测过程暴露出来。

3. **可控条件下的长期演化与 mini-game 双模式**:

    - 功能：既观察长期群体动态，也隔离具体因果机制。
    - 核心思路：Evolutionary game 运行完整生命周期直到 80 步或全部死亡，用不同资源丰度、沟通轮数和道德类型可见性作为变量。Mini-game 则固定具体场景，例如亲子 HP 分配，观察不同道德类型如何做单点决策。
    - 设计动机：长期仿真能看 emergent outcome，但难以解释每个 outcome 的局部原因；mini-game 能把某个机制放大，例如亲缘利他、资源分配或组队协商，从而连接微观行为和宏观演化结果。
