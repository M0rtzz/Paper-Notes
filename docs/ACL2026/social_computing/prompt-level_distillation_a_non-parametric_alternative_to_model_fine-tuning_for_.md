...
1. **监督式指令抽取**:
	...
	- 功能：把教师模型对单个训练样本的推理过程压缩成可迁移的自然语言规则。
	- 核心思路：提示教师同时完成两件事：先基于 ground-truth label 做 CoT 式分析，再把这个分析抽象成一条去实体化的 instruction。这样既利用了标签监督，又避免把样本内容原样塞进 prompt。
	- 设计动机：如果只让教师生成答案，学生学不到判别边界；如果直接保存 CoT，又会过长且含样本细节。抽象 instruction 是两者之间的折中。

2. **DBSCAN 语义聚类与逻辑合成**:

	- 功能：把大量重复、细碎、局部的微规则合并成少量可读规则。
	- 核心思路：用 Gemini Embedding 得到 768 维向量，DBSCAN 采用 cosine distance、$\epsilon=0.4$、`min_samples=6`。DBSCAN 不强迫所有点进入簇，噪声点会被丢弃；每个簇再交给 Gemini 3 Pro 合成为统一 heuristic。
	- 设计动机：规则数量如果过多，system prompt 会膨胀；如果合并过粗，又会丢掉少数类和边缘规则。密度聚类可以让规则数量从数据中自然出现。

3. **闭环冲突消解**:

	- 功能：修复合并规则之间的矛盾，以及初始抽取没有覆盖的细微边界。
	- 核心思路：把当前 instruction set 放入学生模型，在训练数据上推理，挑出模型遵循规则但仍预测错误的案例；再把失败案例和成功案例一起交给冲突消解模型，生成更新后的规则。循环到验证误差收敛。
	- 设计动机：复杂任务中的规则往往存在优先级和例外条件。单次聚类合成容易把少数类规则平均掉，闭环错误分析能把这些长尾约束补回来。

### 损失函数 / 训练策略
PLD 本身不更新学生模型参数，因此没有传统训练损失。它的“优化目标”体现在 prompt 搜索和错误闭环中：教师抽取阶段最大化每条规则对标签的解释性，聚类阶段在 prompt 长度和规则覆盖之间取平衡，冲突消解阶段以学生模型在训练/验证样本上的错误率收敛作为停止信号。

实验中教师抽取使用 Gemini 3 Flash thinking mode，聚类合成和冲突消解使用 Gemini 3 Pro thinking mode。学生模型包括 Gemma-3 4B、Mistral Small 3.1 24B 和 Gemini 2 Flash。对比方法包括 zero-shot、5-shot、TextGrad，以及对 Gemma/Mistral 做 LoRA fine-tuning 的参数化蒸馏基线。
