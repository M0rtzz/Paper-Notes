...
1. **高质量短视频谣言数据构造**:
    ...
    - 功能：提供一个小而精、带证据链的中文短视频谣言评测集。
    - 核心思路：先从专业辟谣渠道收集 100 个虚假案例，再找回原始谣言视频；真实集则从中国互联网联合辟谣平台抽取已验证真实声明，并匹配相近主题和视觉风格的视频。虚假样本被标成实验错误、逻辑谬误、虚构声明三类。
    - 设计动机：自动爬取或弱标签数据很难判断“为什么错”。作者牺牲规模换标注质量，目的是让模型评测能深入到推理路径。

2. **多模态输入与 Belief Score 指标**:

    - 功能：拆解视频中的视觉、字幕、语音和人工声明对模型判断的贡献。
    - 核心思路：五种设置分别是 Claim、Textual、Aural、Visual、Multimodal。Claim 使用人工提炼的核心声明，Textual 使用 OCR，Aural 使用 ASR，Visual 使用 0.5 FPS 抽帧且最多 32 帧，Multimodal 使用帧和 transcript。模型输出 1-7 Likert 可信度，Belief Score 只奖励“对假内容怀疑”或“对真内容相信”的方向正确评分。
    - 设计动机：短视频中字幕、画面实验和口播往往互相冗余又互相干扰；分模态测试能看出模型依赖哪类信息。

3. **从众效应与权威偏差分析**:

    - 功能：评估模型是否会被非内容线索影响真实性判断。
    - 核心思路：从众效应实验把播放、点赞、评论、分享等指标按数量级缩放后加入 prompt；权威偏差实验揭示频道 ID 和认证状态，包括未认证、黄 V 个人、蓝 V 企业、红 V 组织。
    - 设计动机：现实平台上的谣言传播不只是内容问题，模型如果像人一样被“很多人看过”或“账号看起来权威”影响，就会在实际应用中放大平台偏差。
