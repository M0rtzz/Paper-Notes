...
### 关键设计
1. **短 prompt + 明确定义的 LLM 分类器**:

	- 功能：把类别定义直接放在输入前，让 instruction-tuned LLM 在相近毒性类别之间有更清楚的判别边界。
	- 核心思路：prompt 用短格式列出 `0=Non-toxic` 到 `5=Extremism` 的简短定义，然后接 `Message: [input text]`。作者认为短 prompt 能保留类别语义，又避免过长说明挤占 384 token 的最大长度。
	- 设计动机：游戏聊天句子本来很短，冗长说明会让训练目标变得啰嗦；但完全不给类别解释，又会让模型把 Insults、Other Offensive、Hate/Harassment 这几个边界模糊的类混淆。

2. **少数类 paraphrase 合成数据**:

	- 功能：提升 Class 2/3/4/5 这些稀有或易混类别的训练信号。
	- 核心思路：作者让 LLM 对真实少数类消息做语义保持的改写，而不是凭空生成毒性句子。最终合成池有 10,464 条过滤后的改写，其中 Class 2 有 8,348 条，Class 3 有 1,633 条，Class 4 有 343 条，Class 5 有 140 条；最终只采样 1,921 条加入训练，占训练数据 4.998%。
	- 设计动机：初步直接生成会产生过于通用、不像游戏聊天的毒性样本；paraphrase 保留真实语境和短 slang 风格，更适合补充稀有类别。

3. **验证陷阱驱动的比例选择**:

	- 功能：防止模型在验证集上看起来更好、但在测试集上更差。
	- 核心思路：作者扫描 0%、2%、3%、5%、7%、10%、15% 的合成比例，并比较验证 F1、测试 F1 和测试预测分布。5% 合成数据让 Non-toxic 预测从 79.6% 降到 79.0%，Class 2 从 4.9% 升到 5.7%，更接近测试集需要的少数类敏感度。
	- 设计动机：这个任务里验证集并不是最终泛化的可靠代理。小比例合成能调整决策边界，大比例合成则让模型过拟合合成风格或过度预测少数类。
