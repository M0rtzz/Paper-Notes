...
1. **对偶式数据增强**：
    ...
    - 功能：在不引入外部数据的前提下，让模型学到"语义不变性"——同一段文本经过表面扰动后仍应得到相同极化标签。
    - 核心思路：对每条原始训练样本 $x$ 都生成一份增强副本 $T(x)$，把 $(x, y)$ 和 $(T(x), y)$ 都喂给模型，再用全局去重去掉与原文相同的副本。四种 $T$ 分别是：(a) **匿名化** —— 用 `[EMAIL]/[USER]/[PHONE]` 替换识别出的个人信息，削弱对特定用户名的过拟合；(b) **小写化**；(c) **大写化**；(d) **同形字化** —— 把部分 ASCII 字符替换成视觉相同但属于不同 Unicode 块的字符（如希腊小写 a 替拉丁 a）。subtask 1 中四种增强各加 5%，合计扩大训练集 20%。
    - 设计动机：同形字攻击在作者上一篇机器生成文本检测工作里是干扰最强的对抗策略（它会让 BPE/SentencePiece tokenizer 把单词切成完全不同的子词序列），因此把它当训练时正则、而不是测试时防御，可以一举两得地提升鲁棒性与表面稳健性。

2. **22 语言合并 + QLoRA 单一模型**：

    - 功能：让一个 32B 模型一次性处理 22 种语言而不是为每种语言训一个分类头，最大化低资源语言的知识共享。
    - 核心思路：把所有语言的训练数据按行混合，用 4-bit QLoRA 微调 base LLM，learning rate $2 \times 10^{-5}$ 常数、warmup ratio 0.03、paged AdamW、batch size = 1、每 500 步在均衡采样的 4400 条 dev 集上验证，单 epoch 训完。subtask 2/3 因为是多标签，把 LM head 换成 sigmoid 多输出头并配 BCE 损失。
    - 设计动机：低资源语言（如 Hausa、Khmer）单语样本不足以微调 32B 模型，把所有语言堆在一起既让模型暴露在更多极化样本里，又强迫它学习"任务而非语言"的表示；选择 Qwen3-32B / Gemma-3-27B 是因为它们都原生支持 100+ 语言，省去了 cross-lingual transfer 的额外手段。

3. **Appraisal 标注作为可解释替代路径（探索性）**：

    - 功能：在主系统外探索一条用「情感的认知 appraisal 维度」（愉悦度、可预测性、可控性、对自己/他人后果、价值观对齐等）作为特征的轻量级路径。
    - 核心思路：用 XLM-Roberta 编码文本，多任务回归头同时预测 5 个二元 appraisal 维度 + 4 个事件描述维度，损失是 MSE + BCE 的加权和（架构改自 AppraisePLM）。然后用 LogisticRegression（默认超参，random state 42，80/20 切分）按 appraisal 特征做极化分类，每种语言独立训一个。
    - 设计动机：appraisal 维度是可解释的认知信号，能解释"为什么这段话是极化的"。虽然单独看 macro-F1 接近随机，但每个 label 的 AUC（threshold-independent）显示出足够的判别力，提示未来可以把 appraisal 信号融进主模型作为额外特征。
