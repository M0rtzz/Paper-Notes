...
1. **真实沟通式任务设计**:
	...
	- 功能：把新词评测从“知道定义”推进到“能否在社交消息中自然使用”。
	- 核心思路：每个新词都放在一个真实感较强的社交媒体帖子中，非母语者先学习，再写给假想朋友 Jo 的消息，同时还要判断其他样本的语境合适性。产出任务衡量能不能用，理解任务衡量能不能判断别人用得对不对。
	- 设计动机：网络新词的难点在语用和社群语境，单纯选择题无法测出“母语者读起来是否自然”。这个设计把评测对象从模型答案转向真实沟通结果。

2. **五种支持条件的可控比较**:

	- 功能：区分不同 AI 使用方式到底提供了什么帮助。
	- 核心思路：实验设置 Control、AI Definition、AI Rewrite、AI Explanation 和 Non-AI Dictionary 五组。AI Definition 给词典式定义，AI Rewrite 把含新词帖子改写成简单英语，AI Explanation 用 3-5 句解释含义、语气、使用场景和受众，Non-AI Dictionary 则提供 Merriam-Webster 的完整页面。
	- 设计动机：用户实际用 AI 时并不只问“这个词是什么意思”，还会要求解释、改写或举例。分开比较这些交互模式，可以看出真正有效的是低密度定义、简化上下文，还是带语境的用法说明。

3. **多视角交际能力测量**:

	- 功能：同时观察外部交际效果、学习者理解能力和主观感受。
	- 核心思路：母语者评分覆盖 well-formedness、contextual appropriateness、understandability；非母语者理解能力用其语境合适性评分与母语者基准的距离衡量；自评包括 confidence、helpfulness、reliance、future trust、mental burden 和 task difficulty。
	- 设计动机：AI 工具常让用户“感觉更懂”，但这个感觉未必等于真实交际能力。多视角测量能显式暴露“自信提升但母语者不买账”的错位。
