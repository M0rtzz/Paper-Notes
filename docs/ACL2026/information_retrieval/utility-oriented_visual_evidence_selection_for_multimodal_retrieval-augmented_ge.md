...
1. **从相关性到信息增益效用**:
	...
	- 功能：为视觉证据选择建立一个面向生成结果的目标，而不是沿用检索相似度。
	- 核心思路：高效用证据应该显著改变模型对答案的后验信念，论文用 $IG(Y;C=c|q)$ 表达这种变化。它关注证据是否提供判别信息，而不是图像和查询在表面语义上是否接近。
	- 设计动机：多模态 RAG 的失败常来自“看起来相关但答题无用”的图像。把目标改成信息增益，可以自然解释为什么某些低相似度但包含关键线索的图像更值得选。

2. **潜在 helpfulness 变量**:

	- 功能：把不可计算的答案空间效用转化为可估计的二值判断。
	- 核心思路：定义 $Z=1$ 表示候选图像对当前 query 有帮助。论文通过理论推导说明，在合理假设下，按 $P(Z=1|C=c,q)$ 排序能保留答案空间效用排序的最优性。实际系统只需要让模型回答证据是否 helpful，然后用 True logit 排序。
	- 设计动机：答案级不确定性容易混入措辞差异、采样噪声和幻觉；helpfulness 判断更像一个判别任务，输入短、输出短，适合用小模型批量执行。

3. **Surrogate-accelerated 执行**:

	- 功能：把候选池的 $O(N)$ 次打分从大主模型转移到小代理模型，降低 FLOPs 和延迟。
	- 核心思路：作者提出 utility transferability hypothesis：如果某张图对小模型显然无助或矛盾，那么它很可能对大模型也无助。于是用 Qwen3-VL-2B、Ovis2.5-2B 等轻量模型做候选排序，主模型只进行一次最终生成。
	- 设计动机：真实 RAG 候选池可能很大，让 8B 到 12B 主模型逐个判断或生成答案代价过高。surrogate 排序保留效用导向，同时让系统可部署。
