...
### 整体框架
论文首先从 cninfo、SEC 等来源收集中国 A 股和美国上市公司的年报、ESG 报告、公告等文档，共 589 份文档、超过 80,000 页。每个问题平均关联 14.8 份 PDF、149.7 页文档，文档集合规模明显大于多数现有多文档问答基准。

每份文档都有显式元数据，包括股票代码、财政年度和文档类型。这些字段不是答案本身，却是系统筛选和规划任务的关键入口。比如“2022–2023 年收入增长率最高的三家公司”必须先知道哪些文档属于哪个年份、哪家公司、哪类报告，然后才谈得上抽取数值和计算增长率。

问题构造采用 distant supervision 与专家审核结合。作者先从权威金融数据库中整理收入、成本、股利、社会责任指标、高管信息等结构化数据，再由金融专家设计自然语言问题模板，覆盖趋势分析、同行比较、异常值识别、跨年变化等任务。对于每个问题，作者不仅标注最终答案，还把回答所需的中间事实转写成自然语言集合，用来诊断模型是否真的抽到了必要信息。

评测时，系统输入为问题 $Q_j$、文档集合 $D_j$、元数据集合 $M_j$，输出为最终答案。数据集形式可概括为 $X = \{(Q_j, D_j, M_j, S_j)\}$，其中 $S_j$ 是该问题所需的中间事实集合。这个设计让 MuDABench 不只检查最终字符串是否正确，也能观察抽取链条在哪一步断掉。

作者提出的参考系统是 metadata-aware multi-agent workflow。它不把所有文档切块后直接丢给同一个 RAG 查询，而是把复杂分析拆成四段：规划 Agent 读问题和元数据 schema 生成逐文档子查询；文档级抽取器在每份相关文档上执行 RAG 问答；规范化 Agent 把自然语言抽取结果转换成扁平 JSON；代码 Agent 根据 JSON schema 写程序完成排序、过滤、比较或统计计算，最后再生成答案。

### 关键设计
1. **带元数据的基准构造**:

    - 功能：把大规模金融披露材料组织成半结构化文档集合，使多文档问答更接近真实分析场景。
    - 核心思路：每份文档绑定股票代码、财政年度、文档类型三类元数据；每个问题绑定一个由 5 到 38 份 PDF 组成的文档集合；问题既有简单的单年度统计，也有需要跨年份、条件筛选和计算的复杂查询。相比把文档简单当作文本块，MuDABench 把“文档属于哪个实体、哪个年份、哪种披露类型”显式放进评测环境。
    - 设计动机：真实文档库往往天然有元数据，忽略这些结构会让 RAG 在巨大语料池里盲目召回。把元数据纳入基准后，系统必须学会先缩小分析空间，再做文档内抽取和跨文档聚合。

2. **中间事实驱动的评测协议**:

    - 功能：除了最终答案准确率，还衡量系统是否抽取到了支撑答案的必要事实。
    - 核心思路：对标准 RAG，作者用 LLM-as-judge 判断模型输出或召回内容覆盖了多少 gold supporting facts，并用 double-check judge 估计错误或遗漏比例，得到更保守的覆盖分数；对文档对齐的工作流，则把结构化来源表的 metric cell 作为评测单元，计算正确抽取 cell 的比例。最终报告 process accuracy、final-answer accuracy 和 full accuracy，后者要求过程完全正确且最终答案正确。
    - 设计动机：多文档分析中，最终答案可能因为偶然的数值或宽松 judge 而看似正确，但抽取链条其实不可靠。中间事实标注让研究者能区分“检索没找到”“抽取错了”“聚合算错了”等不同失败原因。

3. **规划-抽取-规范化-代码聚合的 Agent 管线**:

    - 功能：用模块化流程替代单次 RAG 生成，让系统能处理超过上下文窗口的大规模文档集合。
    - 核心思路：Plan Agent 先根据问题和 metadata schema 生成子查询模板，并可附带年份、文档类型等限制；Document-Level Extractor 把模板实例化到具体文档，用文件搜索和单文档问答抽取局部事实；Norm Agent 先从少量样例定义扁平 JSON schema，再批量把抽取对话转成统一记录；Code Agent 只看到 schema、样例和 JSON 路径，写可执行程序在完整结构化数据上做分析。
    - 设计动机：复杂问题经常需要对几十甚至上千份文档做同类抽取，再执行表格式聚合。让 LLM 一次性读完所有内容既不现实，也容易混淆实体和年份；而 JSON 加代码把“抽取”和“计算”分开，可以让数值分析更可控、可扩展。
