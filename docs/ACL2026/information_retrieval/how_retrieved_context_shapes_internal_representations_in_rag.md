...
**1. 受控文档分类体系：把“检索混合体”拆成三种可隔离的輔助变量**
...
模型已有参数化知识时和缺乏知识时，对检索文档的处理机制可能完全不同，混在一起统计会抹掉这种差异。本文对每个查询先用纯查询（无检索）测一遍：能答对的标为 easy，否则为 hard。这样就能分开考察检索文档对“已知知识”和“未知知识”的不同影响——后面会发现随机文档息息能把 easy 查询送进拒答，这个区分是看出现象的关键。
