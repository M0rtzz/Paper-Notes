...
该评估协议针对的是"低精度打分产生大量虚假并列、导致排序随机抖动"这个根因，从打分和指标两个环节同时下手。输入是低精度检索系统返回的 top-K 候选及其分数，输出是稳定可复现的评估结果。协议的两个组件相互独立又互补：HPS 在打分环节用极小代价把并列消除掉，让排序变确定；TRM 在指标环节直面残余并列，把"如果还有并列、指标会落在什么范围"明确报告出来，而不是假装并列不存在。

### 关键设计

**1. 高精度打分（HPS）：只在最后一步加精度，把虚假并列从根上抹掉**

虚假并列的本质是数值粒度太粗——二值嵌入的汉明距离只有有限离散取值，很多本不相同的文档会算出完全一样的分数，于是排序被任意的平局打破规则（如文档 ID 顺序）支配。HPS 的做法是让检索全程仍用低精度完成以保住效率，只对最终返回的 top-K 候选，把它们的嵌入上转到高精度（如 FP32）重算相似度。由于评估指标对靠前位置最敏感，而 top-K 文档数量有限，这一步只带来 <1% 的额外计算，却能几乎完全消除 top-K 内的并列——是一种投入极小、收益极大的最小化干预。

**2. 并列感知检索指标（TRM）：不消除并列，而是诚实量化它带来的不确定性**

即便用了 HPS，极端低精度场景仍可能残留并列，此时再用单一确定值报告指标就是自欺欺人。TRM 对每个并列文档组考虑其所有可能排序排列，报告期望值 $E[M]$、最优值 $M_{max}$、最差值 $M_{min}$，以及相对默认排序的偏差 $M_{bias} = E[M] - M_{default}$。实现上通过解析公式高效求解，无需真正枚举所有排列。这套指标把评估从"一个可能带系统性偏差的点估计"变成"一段诚实的区间估计"，让读者清楚看到结论对并列打破策略有多敏感。

**3. 三种打分函数的统一处理：覆盖主流量化方案，保证协议通用**

不同的低精度量化方案对应不同的相似度计算，并列的产生机制和频率也随之不同。论文针对内积（dot product）、余弦相似度（cosine similarity）、汉明距离（Hamming distance）三种常见打分函数分别分析了并列的成因与发生率，并逐一验证 HPS 和 TRM 在每种打分函数下都成立。这种统一处理保证了协议不绑定某一种量化方案，能被低精度检索社区广泛复用。
