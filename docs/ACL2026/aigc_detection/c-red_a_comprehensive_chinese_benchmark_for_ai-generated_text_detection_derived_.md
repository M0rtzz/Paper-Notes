...
**1. 真实 prompt 驱动的数据构造：让 AI 文本接近用户真实调用 LLM 的产物，而不是续写或模板 QA**

如果检测器只在简化 prompt 或固定 QA 模板上训练，它学到的往往是某种浅层“机器腔”痕迹，一旦遇到现实中带详细指令、特定风格约束或专业场景引导的 AI 文本就失灵。C-ReD 先分析每个领域人类语料的题材、长度和风格，再设计贴合该领域使用习惯的 prompt：学术写作区分“根据标题关键词写摘要”和“根据摘要扩写引言”两种模式，新闻按类别套写作模板，作文直接采用真实高考题描述。prompt 越贴近真实调用方式，生成出来的 AI 文本就越像部署时真正要拦的那种，检测难度也更可信。

**2. 多领域、多生成器黑盒生成：把现实里最常见的 domain shift 和 generator shift 一次摆上台面**

真实检测场景既不知道文本出自哪个生成器，也拿不到模型内部 logits。C-ReD 的人类文本横跨新闻、Q&A、影评、作文和学术写作五类简体场景（外加一类繁体新闻），AI 文本则交给九个模型生成——gpt-3.5-turbo、gpt-4o、Gemini-2.5-Flash、Claude-3.5-Haiku、Deepseek-V3、Deepseek-R1、Qwen2.5、Qwen3、Doubao-1.5-Pro，特意把 Deepseek、Qwen、Doubao 这些中文生态常用模型纳入。所有模型一律按黑盒 API 调用，不依赖内部 logits 或采样细节，这样测出来的才是检测器到底有没有学到可迁移的信号，而不是记住某个旧模型的固定风格。

**3. 多协议评测泛化能力：把“在本数据集上分类准”拆成跨生成器、跨领域、跨 prompt、跨简繁、跨外部数据等更细的问题**

AI 文本检测极易过拟合 benchmark，一个平均 AUROC 看着漂亮，换个领域或新模型就崩。C-ReD 不止报告内部 AUROC：它把 Claude-3.5-Haiku 和 Gemini-2.5-Flash 留作 held-out OOD 生成器，训练单域检测器画跨域迁移热图，对比原始与简化的学术 prompt，再在繁体新闻和外部 M4 中文 QA 子集上做外部验证。七类协议层层施压，就是要逼出检测器究竟在学“AI 文体”这种可迁移特征，还是只记住了某个领域或某个模型的表面痕迹。
