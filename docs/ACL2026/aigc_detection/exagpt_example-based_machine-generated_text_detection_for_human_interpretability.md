...
### 整体框架
ExaGPT 的 pipeline 分两阶段：(1) **Span Scoring**——把目标文本 $x$ 切成所有可能的 $n$-gram span（$n \in [1, 20]$），用 BERT 第二层 hidden state 的均值作为 span embedding，对每个 span 在 datastore 上做 top-$k$（$k=10$）k-NN 检索（FAISS），记录每个邻居的 (相似度 $c_j$, 原始标签 $l_j \in \{\text{Human}, \text{LLM}\}$）。(2) **Span Selection**——用动态规划在所有 $n$-gram 候选切分里选一组无重叠覆盖整段文本的 span 序列 $T=[t_1,\dots,t_H]$，使得"span 长度 + 检索相似度"加权平均最大。最终用所选 span 的 prediction score 均值判定整段文本是 Human 还是 LLM，并把每个 span 对应的 top-$k$ 检索结果作为证据展示。

### 关键设计

1. **三元 span 评分（length / reliability / prediction）**:

    - 功能：把"span 是否值得作为证据"拆成可独立优化的三个标量信号。
    - 核心思路：对每个目标 span $x_{i:i+n}$，定义长度分 $L = n$、可靠度分 $R = \frac{1}{k}\sum_j c_j$（k-NN 邻居平均相似度，衡量"datastore 里有没有真正像它的 span"）、预测分 $P = \frac{1}{k}\sum_j \mathbb{1}(l_j = \text{LLM})$（k-NN 中 LLM 标签比例，即"这个 span 看起来更像 LLM 还是人"）。
    - 设计动机：人在判断 AI 文本时其实在做两个判断——"这种说法在 AI 文本里见过没"（reliability）和"是哪一侧见得多"（prediction）；分开成两个 score 后才能在第二阶段做精细的 span 切分。

2. **动态规划 span 切分（balance length vs. reliability）**:

    - 功能：从指数级别的 $n$-gram 切分组合里找出"既够长又够像 datastore 中真实片段"的最优切分。
    - 核心思路：定义切分总分 $S(T) = \frac{1}{H}\sum_h [\alpha L^{\text{std}}(t_h) + (1-\alpha) R^{\text{std}}(t_h)]$，其中 $L^{\text{std}}, R^{\text{std}}$ 是 validation 上归一化后的标准分。DP 状态 $\text{dp}[i]$ 记录前缀 $x_{0:i}$ 的最佳累积分和回溯指针；转移时枚举上一个切点 $j \in [i-N, i)$ 并取使均值最大的 $j$；最后回溯得到切分序列 $T$。复杂度 $O(m \cdot N)$，$N=20$ 是最大 $n$-gram。
    - 设计动机：朴素切分（如固定 $n=5$）会丢"非常 verbatim 的长重叠"和"非常稀有的高判别力短 span"两端信息；DP 让长 span 和高相似 span 同时被偏好，对应到 UI 上就是用户能看到"长又像"的连续证据，可读性最高。

3. **k-NN 检索给出 evidence 而非分数**:

    - 功能：让最终决策与展示给用户的证据是**同一**机制产出的，不存在"模型决策"和"事后解释"的脱节。
    - 核心思路：整段文本判定 $P_{\text{overall}} = \frac{1}{H}\sum_h P(t_h) > \epsilon$ 即判 LLM；同时把 $E = \{(t_h, [s_h^1, \dots, s_h^k])\}_{h=1}^H$ 以 tooltip 形式按颜色（红/绿/蓝代表 Human/中性/LLM）展示，每个 span 鼠标悬停就看到 top-$k$ 相似邻居和它们的标签分布。
    - 设计动机：传统 SHAP/LIME 解释只能说"哪几个 token 对预测重要"，但不能告诉用户"为什么"；ExaGPT 让用户直接看"datastore 里和这段最像的 10 个真实片段"，自己就能判断证据靠不靠谱——这把"决策"和"证据"绑死，避免了事后归因的不一致。

### 损失函数 / 训练策略
ExaGPT 本身**完全 training-free**：检测阶段只用 BERT-large-uncased 做 embedding（取第二层 mean pooling 因为该层在 lexical 和 semantic 之间最平衡），datastore 用 M4 数据集的 train split（每个 domain×generator 组合 2000 对样本）建 FAISS 索引。唯一需要调的"超参"是切分系数 $\alpha \in \{0, 0.125, 0.25, \dots, 1.0\}$，在 validation 上选 detection 性能最好的值（实验显示偏向 reliability 即 $\alpha$ 小一点更好），以及 detection 阈值 $\epsilon$ 按"validation 上 FPR=1%"反推。
