...
### 关键设计
1. **二分类 framing operationalization**:

	- 功能：把复杂的 framing 理论转成可评测的摘要属性，即摘要是否存在解释性框架。
	- 核心思路：当摘要通过选择性强调、遗漏、评价措辞、因果或责任归因让某种解读更突出时，标为 Framed；若只陈述核心事件且不引入明显解释视角，则标为 Not Framed。
	- 设计动机：细粒度 frame taxonomy 更适合传播学分析，但系统评测首先需要知道模型多大概率会引入 framing，因此二分类更稳、更可扩展。

2. **LLM jury + 专家金标校准**:

	- 功能：在大规模覆盖和专家可靠性之间折中。
	- 核心思路：三种 LLM 独立标注后多数投票，得到 15,499 条银标；再用 320 条专家标注估计 jury 的偏差。jury 标为 Framed 时，专家也标为 Framed 的概率为 77.8%；jury 标为 Not Framed 时，专家仍标为 Framed 的概率为 16.3%。
	- 设计动机：只靠专家标注成本太高，只靠 LLM 标注又可能继承模型偏见；校准权重把这两者连接起来，使模型级统计更接近专家判断。

3. **专家校准 framing rate**:

	- 功能：把每个模型或主题的 framing 频率从原始二值银标转换为专家校准估计。
	- 核心思路：对摘要集合 $S$，计算 $FR(S)=\frac{1}{|S|}\sum_{s\in S}w_s$，其中 jury 标为 Framed 时 $w_s=0.778$，jury 标为 Not Framed 时 $w_s=0.163$。
	- 设计动机：这个估计承认 jury 有误差，不把 LLM 标签当绝对真值；同时保留大规模统计能力，便于比较模型容量、微调方式和新闻主题的影响。
