...
## 方法详解
DetectRL-X 是一个 benchmark 与评测框架。它的“方法”主要体现在数据构建、任务定义、攻击生成和评价模块设计上。整体设计围绕三个文本类别：HWT 是人写文本，LGT 是 LLM 生成文本，HLT 是人写文本经 LLM 辅助修订后的混合文本。Binary 任务判断 $\{HWT,LGT\}$，Ternary 任务判断 $\{HWT,HLT,LGT\}$。

### 整体框架
数据构建先收集 8 种语言的真实人写文本，语言包括 English、German、Spanish、French、Portuguese、Russian、Arabic 和 Chinese，并按语言复杂度分为 high、medium、low 三组。文本来源覆盖 Academic、News、Novel、SEO、Wiki 和 WebText 六个领域，且只使用 2022 年之前公开发布的文本，以降低 LGT 污染风险。

随后，作者使用 DeepSeek-V3、Gemini-2.5-flash、GPT-4o 和 Qwen-Max 生成 LGT，并用 Qwen-Max 对 HWT/LGT 做 polishing、expanding、condensing 等修订，构造 HLT 和后处理后的 LGT。最后，框架加入多语言 paraphrase attacks 和 perturbation attacks，并按 64、128、256、512 tokens 生成不同长度粒度的子样本。最终数据规模为 3,456,000 个样本，train/test 按 2:1 划分。

### 关键设计
1. **从二分类扩展到三分类**:

    - 功能：让评测覆盖真实的人机协作写作，而不只区分纯人写和纯机写。
    - 核心思路：传统检测函数是 $f_{Binary}:T\to\{HWT,LGT\}$，本文增加 $f_{Ternary}:T\to\{HWT,HLT,LGT\}$。HLT 来自人写文本经 LLM 润色、扩写或压缩，反映真实办公和内容生产中常见的辅助写作。
    - 设计动机：如果检测器只能判断“有没有机器味”，就很难处理人类原稿被 LLM 局部改写的灰区。三分类能更准确暴露检测器在混合作者身份上的边界问题。

2. **多语言、多域、多生成器数据构造**:

    - 功能：避免检测器只在英语或单一写作风格上表现好。
    - 核心思路：数据覆盖 8 种语言和 6 个领域；生成器选择 DeepSeek-V3、Gemini-2.5-flash、GPT-4o 和 Qwen-Max；语言复杂度按形态丰富度和与英语的类型距离划分，高复杂度包括 Arabic、Russian、Chinese，中等复杂度包括 German、French、Spanish、Portuguese，低复杂度为 English。
    - 设计动机：大多数 LLM 和检测器的训练分布偏英语，非英语尤其是文字系统和形态结构差异大的语言会带来 tokenization 与表示难题。

3. **攻击、长度和修订维度的鲁棒性评测**:

    - 功能：模拟真实用户会对文本做的改写和噪声操作。
    - 核心思路：Paraphrase Attacks 包括 Encoder Paraphrasing、Seq2seq Paraphrasing、Decoder Paraphrasing 和 Back-Translation；Perturbation Attacks 包括 Character Insertion、Substitution、Deletion 和 Zero-width Insertion；同时构造 64/128/256/512 tokens 子样本，用来评估长度敏感性。
    - 设计动机：真实文本检测面对的是“被修改过的 LLM 文本”，不是原始模型输出。攻击维度能测出检测器是否依赖脆弱的表面统计特征。
