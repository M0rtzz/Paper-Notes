...
1. **结构 tag + 全 trace SFT**:

    - 功能：教单个 LLM 模仿多智能体辩论的结构性话语（多 agent 发言 + 多轮迭代 + 共识）。
    - 核心思路：与 DebateGPT 等只在"最终共识"上 SFT 的工作不同，本文在带结构 tag 的整条 trace 上做 cross-entropy 训练。结构 tag 给后续 RL 提供了可解析的奖励钩子，也让 agent 之间在 latent 上更容易分离（消融显示去掉 tag 会显著降低 agent 子空间的可分性）。
    - 设计动机：SFT 后的模型在每步生成时能"看到"前面所有 agent 的发言（真实 MAD 中 agent 是并行的、只在 round 末互看），反而比真 MAD 更不容易出现协调失败，这也是 SFT 单阶段就已强于显式辩论的原因。

2. **动态双奖励 RL（格式权重衰减 + 长度退火）**:

    - 功能：把"显式吐辩论"逐步压成"在 latent 里推理"，同时保证答案正确。
    - 核心思路：奖励函数 $r(x,y)=w_{fmt}R^{fmt}+w_{clip}R(y;l)$。$R^{fmt}$ 是结构 tag 匹配的简单奖励（鼓励早期保留辩论格式），其权重 $w_{fmt}$ 从 1.0 线性衰减到 0.05；$R(y;l)$ 是 length-clipping 正确性奖励：$R(y;l)=1$ 当且仅当正确答案 $y^*$ 出现在 $y$ 的前 $l$ 个 token 里，否则为 0。长度上限 $l$ 从 2000 退火到 500，使得"既要写完整辩论又要把答案放进前 500 token"变得不可能，唯一可行策略就是把多视角推理搬进隐空间。
    - 设计动机：直接给一个紧的 $l$ 会让模型一开始就没有探索推理空间的余地；让 $l$ 渐进收紧 + 让格式奖励渐进消失，两个动态信号合力把模型从"显式辩论员"推到"隐式辩论员"。这种设计借鉴了 Hou et al. (2025) 把长 CoT 内化的思路，但首次用于多智能体场景。

3. **基于差分均值的智能体 steering vector**:

    - 功能：把"模型内化后每个 agent 长什么样"显式提取出来，作为隐空间中的可加向量，用于探针解释和行为控制。
    - 核心思路：用 Contrastive Activation Addition（CAA），对每个 agent $i$ 构造正样本（同样的辩论历史后接 agent $i$ 真实回复）和负样本（同样历史后接其他两个 agent 回复的平均激活），在第 $\ell$ 层取差分均值 $\mathbf{v}_i = \frac{1}{|\mathcal{D}|}\sum_{p,c\in\mathcal{D}}(\mathbf{h}_\ell(p,c_i) - \mathbf{h}_\ell(p,c_{\neg i}))$。推理时把 $\alpha\cdot \mathbf{v}_i$ 加到隐藏状态上，正 $\alpha$ 放大该 agent 特性，负 $\alpha$ 压制。Steering vector 从 SFT checkpoint 提取，避免 RL 阶段优化引入的伪影。
    - 设计动机：要回答"内化后多 agent 结构是真存在还是被压成无差别推理"，最直接的办法就是看能否在隐空间里找到 agent 特定方向；可分离的方向也直接给"行为控制"提供了抓手——把恶意 agent 的方向找出来负向加，就能选择性压制坏行为而不毁掉通用能力。
