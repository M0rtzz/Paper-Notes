...
description: >-
  [ACL 2026][多智能体 / LLM 训练 / MCTS / 数据合成][多智能体系统] 提出 DITS，把 "训练数据影响分数 (influence score)" 而非传统 Q-value 作为 MCTS 树搜索和偏好数据选择的指挥棒，并为不可微指标推导出一个 "前向推理就能算" 的影响分数估计公式…
tags:
  - "ACL 2026"
  - "多智能体 / LLM 训练 / MCTS / 数据合成"
  - "多智能体系统"
  - "影响函数"
  - "蒙特卡洛树搜索"
  - "DPO"
...
1. **影响导向的 MCTS 节点扩展**：
    ...
    - 功能：在 selection 阶段不再只看 Q-value，而是优先扩展那些更可能产出 "高训练增益" 偏好对的节点。
    - 核心思路：候选节点集 $N_{\text{cand}}$ 先用 edit-distance 相似度阈值 $S_{i,j}\geq 0.25$ 过滤掉与已展开节点高度重复的分支，再按 $n\sim\text{Softmax}(\{Q(n)\}_{n\in N_{\text{cand}}})$ 采样；最终偏好对由当前状态下 Q-value 最高 / 最低的两个动作 $(a_i^h, a_i^l)$ 构成。
    - 设计动机：MCTS 原版 selection 主要服务于推理阶段 "找正确答案"，所以倾向于反复深入高 Q-value 子树，导致多样性塌缩；引入影响分数后选择更分散、面向 "教得动模型" 而非 "答得对题目"。

2. **不可微指标上的影响分数估计**：

    - 功能：用前向推理代替 Hessian / 高阶梯度，把影响分数算到 8B 级 LLM 上仍然可承受。
    - 核心思路：定义 $\mathcal{I}_{\mathcal{F}_{\text{val}}}(z_i,\mathcal{D}_{\text{val}}):=\frac{\mathcal{F}_{\text{val}}(z_i,\theta_{\epsilon,z_i}^{*})-\mathcal{F}_{\text{val}}(z_i,\theta^{*})}{\epsilon}$，再把扰动后的最优参数近似为一步梯度下降 $\theta_{\epsilon,z_i}^{*}\approx\theta^{*}-\eta\epsilon\nabla_{\theta}\mathcal{L}_{\text{tr}}(z_i,\theta^{*})$，于是 $\mathcal{I}\approx\frac{1}{\epsilon}[\mathcal{F}_{\text{val}}(z_i,\theta^{*}-\eta\epsilon\nabla_{\theta}L_{\text{tr}}(z_i,\theta^{*}))-\mathcal{F}_{\text{val}}(z_i,\theta^{*})]$，整个量只需对每条候选数据做一次 LoRA 一步更新 + 一次验证集前向，无需算二阶梯度。
    - 设计动机：DPO loss 与下游指标相关性 < 0.2，所以传统基于 loss 的 influence function 几乎没用；改成直接看 F1 / EM 的扰动响应，绕开了 "loss 不代表真本事" 的痛点，同时把 "求 Hessian-vector product" 这步换成廉价的前向，使大模型 + 大批量数据可行。

3. **影响-Q 联合评分 + 迭代自训练**：

    - 功能：把 Q-value 当成 "合理性先验"，把影响分数当成 "训练增益的真信号"，二者加权后挑出 Top-α 数据；多轮迭代不断刷新模型与合成数据。
    - 核心思路：综合分数 $H(z_i)=\mathcal{I}_{\mathcal{F}_{\text{val}}}(z_i,\mathcal{D}_{\text{val}},\theta)+\gamma\cdot Q(s,a_i^h)$，其中 $\gamma=0$ 表示纯影响分数、$\gamma=1$ 表示与 Q-value 等权融合；第 $t$ 轮用 $\theta_{t-1}$ 跑 MCTS 合成 $\mathcal{D}_{\text{tr}}^{t}$，从初始 SFT 模型重新训出 $\theta_t$。
    - 设计动机：单看影响分数会被噪声指标 (如 F1) 干扰，单看 Q-value 又训练增益不可靠，两者互补；迭代刷新让 "更强模型 → 更高质量合成数据 → 再次更强模型" 形成正反馈。
