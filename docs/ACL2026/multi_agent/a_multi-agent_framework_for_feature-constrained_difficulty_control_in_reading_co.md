...
### 关键设计
1. **特征级 Evaluator 与错误报告**:

	- 功能：把生成题目拆成多个可测量特征，判断每个特征是否满足目标约束，并输出具体错误报告。
	- 核心思路：表层特征使用规则或现成 NLP 工具，例如 NLTK 统计篇章长度和句长；词汇等级使用外部 CEFR 词表，以题目中最高词汇等级作为约束检查依据；推理复杂度、事实性和中立性这类语义特征则用 LLM judge 加 CoT 与 self-consistency 评估。附录中报告，Evidence Scope 和 Transformation Level 两个推理复杂度子维度的 Macro F1 分别为 69.0 和 70.8。
	- 设计动机：难度控制失败往往不是因为模型不会写题，而是因为它不知道自己哪里没满足约束。把“评价”从生成模型内部拿出来，能让系统获得可操作的诊断信号，也让后续修订更像有靶子的编辑过程。

2. **Planner + Reworder/Editor 的分工修订**:

	- 功能：Planner 负责根据错误报告制定修订策略，并选择具体执行者；Reworder 专门处理词汇等级；Editor 处理除词汇外的其他约束。
	- 核心思路：Planner 的输入包括当前题目状态、错误报告和历史修订记忆，它会避免重复低效的修改。当某个约束连续若干轮无法满足时，Creativity Enhancement Prompting 会要求 Planner 从小幅改动转向更激进的策略，例如删除问题片段后重新生成。Reworder 采用 RAG 流程：先提出上下文合适的替代表达，再用词表检索候选词等级，最后选择符合目标等级的替换项；Editor 则根据 Planner 指令修改篇章或选项，但不向 Planner 反馈自我评估，以免把不准确的自评噪声带入下一轮。
	- 设计动机：词汇约束依赖外部标准，不能只靠 LLM 语感；推理复杂度、中立性等约束又需要语义编辑。把不同错误交给不同角色，可以减少“一个模型同时做所有事”的混乱，也让修订路径更稳定。

3. **难度校准的特征约束序列**:

	- 功能：把多个特征组合成 8 个难度等级，使相邻等级理论上和经验上都呈现单调上升。
	- 核心思路：作者先根据教育测量理论构造 16 个候选约束集，再用 MAFIG 为每个候选集生成题目；随后让 LLM judge 对相邻候选等级的题目做成对难度比较，并计算 Difficulty Alignment Score，记为 $DAS(Q_i,Q_j)=\frac{\sum_{n=1}^{N}x_f^{(n)}+\sum_{n=1}^{N}(-x_r^{(n)})}{2N}$。其中正向和反向比较都采样 $N=4$ 次，以缓解位置偏差；只有 DAS 超过阈值 $\rho=0.4$ 的约束对会被保留，最终得到 8 级单调难度序列。
	- 设计动机：即使题目满足某些特征，也不保证学生或专家会感到难度上升。因此论文把“理论上更难”再经过经验验证过滤，避免生成一个形式上递增但实际难度不可区分的等级体系。
