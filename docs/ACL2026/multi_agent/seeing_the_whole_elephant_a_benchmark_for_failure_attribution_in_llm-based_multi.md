...
description: >-
  [ACL2026][LLM评测 / 多智能体调试][失败归因] TraceElephant 主张多智能体失败归因应在开发者真实可见的完整执行轨迹下评测，并提供 220 条失败 trace、责任 agent 与关键失败 step 标注…
tags:
  - "ACL2026"
  - "LLM评测 / 多智能体调试"
  - "失败归因"
...
1. **完整执行轨迹 schema**:
    ...
    - 功能：记录开发者调试时真正需要的信息，而不只记录 agent 输出。
    - 核心思路：trace-level metadata 包含 task_id、task_instruction、system_name、agent_configuration 和 system_architecture。每个 step 记录输入字段与输出字段：输入包括 step_id、agent_id、agent_name、input_context；输出包括 output_content 和 tool_logs。tool_logs 保存工具名、输入参数、输出和执行状态。
    - 设计动机：很多错误的根因不是某一步输出本身，而是该 agent 接收到的信息不完整、prompt 构造错误或工具返回异常。完整 schema 让归因方法能区分“上游传错信息”和“当前组件推理错”。

2. **多轮专家失败归因标注**:

    - 功能：为每条失败 trace 提供责任组件与关键失败步骤标签。
    - 核心思路：专家首先独立标注 agent-level 和 step-level 标签，再对不确定案例联合讨论达成共识。第一轮标注的 Krippendorff's alpha 为 0.72（agent-level）和 0.64（step-level），说明 agent 归因一致性较高，step 归因难度更大。
    - 设计动机：失败归因不是简单分类，尤其 step-level 需要判断“最早不可逆失败点”。多轮标注提高可靠性，也反映任务本身的细粒度难度。

3. **静态与动态归因评测**:

    - 功能：比较不同可观测性和归因技术在真实调试场景中的效果。
    - 核心思路：静态配置只使用完整 trace；动态配置额外提供可 replay 环境，允许从候选失败点重新运行并做 counterfactual checks。静态方法包括 All-at-Once、Binary Search、Step-by-Step 和 Static Agentic；Dynamic Agentic 先从静态 agentic 方法提出候选归因，再通过重运行验证。
    - 设计动机：开发者调试不一定停留在读日志，常常会重跑系统、修改输入、验证假设。benchmark 把这种主动调试能力纳入评测，推动从被动日志分类走向交互式诊断。
