...
description: >-
  [ACL2026][Agent / 多智能体系统][多智能体系统] OxyGent 把 agent、工具、LLM 与推理流程统一封装成可插拔的 Oxy 原子组件，并用权限驱动的动态规划与 OxyBank 数据回流机制，让工业级多智能体系统更容易搭建、监控和持续演化。
tags:
  - "ACL2026"
  - "Agent / 多智能体系统"
  - "多智能体系统"
...
2. **权限驱动动态规划与标准执行生命周期**:
    ...
    - 核心思路：开发者不需要手写完整 DAG，而是为 Oxy 节点定义可调用关系和权限边界。执行时，编排器只在这些被允许的边中选择下一步，从而得到动态但受约束的路径。每个节点执行时会经过 `_pre_process`、`_pre_save_data`、`_execute`、`_post_process`、`_format_output` 等阶段，系统可以在这些阶段统一插入日志、监控、安全检查和格式转换。
