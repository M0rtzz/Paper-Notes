...
1. **梯度空间的方向先验（而非权重空间插值）**：

    - 功能：把 expert→VL 的参数差当作"该往哪走"的指南针，但只用它"拨"梯度方向，不直接改权重。
    - 核心思路：原始 BR2V 做 $\phi_{\text{VL}\oplus\text{reason}} = \phi_{\text{base}} + \beta(\phi_{\text{VL}}-\phi_{\text{base}}) + (1-\beta)(\phi_{\text{reason}}-\phi_{\text{base}})$，对 $\beta$ 极度敏感；DRIFT 改成在 SFT 每一步对每个被选模块的梯度做 $\tilde{g} = g + \alpha \cdot \text{scale}(g, \Delta)$，让权重从 $\phi_{\text{VL}}$ 出发被多模态 loss 主导、被 $\Delta$ 温柔拉偏。
    - 设计动机：参数级合并的破坏性来自"一步跨过去"，而梯度级注入是"每步偏一点"，配合 multimodal CoT 数据天然耦合感知+推理，避免破坏视觉对齐。

2. **三种 scale 变体（决定 $\Delta$ 怎么和 $g$ 交互）**：

    - 功能：控制方向先验的强度，使其与当前梯度尺度匹配。
    - 核心思路：作者对比三种公式——(i) **Absolute** $\tilde{g} = g + \alpha \Delta$，直接把 $\Delta$ 加到梯度上；(ii) **Grad-Norm** $\tilde{g} = g + \alpha \|g\| \frac{\Delta}{\|\Delta\|}$，只取 $\Delta$ 方向、保留 $g$ 的模长；(iii) **Grad-Norm w/ Adaptive $\alpha$** $\tilde{g} = g + \alpha' \|g\| \frac{\Delta}{\|\Delta\|}$ 且 $\alpha' = \alpha \cdot \frac{1 + \cos(g, \Delta)}{2}$，当 $g$ 与 $\Delta$ 同向时多推、反向时少推。
    - 设计动机：Absolute 把权重硬拉向 reasoning expert（实测 MathVista 掉 3、LogicVista 掉 19.7），说明"绝对幅度"会破坏多模态对齐；Grad-Norm 把强度跟随当前梯度尺度，是稳的；Adaptive $\alpha$ 进一步根据方向一致性自适应，是最稳的——这三档对比直接验证了"只借方向、不借幅度"才是关键。

3. **模块选择：只在注意力投影上注入最稳**：

    - 功能：决定 $\Delta$ 注入到 transformer 的哪些子模块。
    - 核心思路：在 ATTN(Q/K/V/O)、MLP、Norm、LM Head 上分别消融。结果是只挑 {ATTN} 时 LogicVista +3.8、MathVerse +2.4 最稳；加 MLP 反而拉低；加 Norm 引噪；扩展到 LM Head 收益不稳。
    - 设计动机：注意力投影是 token 之间"决定看谁"的核心，承载了推理链路所需的长程依赖路由；MLP 更像"局部知识检索"，跨域差异大、噪声多；Norm 参数尺度敏感、最容易把训练带跑偏。
