...
### 关键设计
1. **真实照片中的受控 gaze/head 分离**:

	- 功能：在保留真实光照、阴影和人脸细节的同时，控制眼睛方向和头部方向是否一致。
	- 核心思路：Natural condition 让被拍摄者自然转头并看向目标；Congruent condition 要求头部和眼睛都朝向同一物体；Incongruent condition 固定头部朝向，再只移动眼睛去看其他物体。这样从 congruent 切到 incongruent 时，场景中真正改变的是眼睛外观。
	- 设计动机：合成图可能不具备真实照片的统计规律，而互联网图又难以控制变量。受控真实拍摄能同时满足现实性和因果解释。

2. **从整体差距到机制分类的测试链**:

	- 功能：把“模型准确率低”进一步解释成具体偏差类型。
	- 核心思路：Test 1 比较 VLM 与人类总体差距；Head-Bias Test 2 包含四个子测试，检查模型是否更常选 HeadTarget、是否在只改变眼睛时保持同一选择、是否在 head/gaze 一致时更好、错误距离是否随 HeadTarget-GazeTarget 距离增大；Test 3 检查模型是否对眼睛目标变化有响应；Test 4 区分模型是在 gaze/head 之间随机摇摆，还是只能粗略选择中间区域。
	- 设计动机：单个统计结果容易被其他因素解释。多个测试共同指向头部朝向偏差时，结论更可信，也能指导后续改进。

3. **严格的响应解析与 GLMM 分析**:

	- 功能：确保结果不是因为选项匹配、prompt 模板或随机刺激造成的。
	- 核心思路：模型可以直接输出选项字母，也可以自由回答；系统先用规则匹配，再用 Llama-3.1-70B 做语义匹配，仍不能解析时人工复核。统计上使用广义线性混合效应模型，把 StimulusID、PromptID、Objects 等作为随机效应，把 View、Proximity、物体数量和 Condition 作为固定效应或交互项。
	- 设计动机：这类行为评估很容易被 prompt、选项顺序和个别图片影响。混合模型能把这些因素纳入分析，避免只凭平均准确率下结论。
