...
1. **Distributed Instruction**:
    ...
    - 功能：把原始危险意图拆分到多张 typographic images 中，降低任意单张图的显式风险。
    - 核心思路：每张图只承载指令中的局部词或片段，模型只有在跨图聚合后才能恢复完整语义。论文用它和 single-image instruction 对比，检验“分散呈现”是否比“单图呈现”更容易绕过安全机制。
    - 设计动机：许多安全过滤器更擅长识别单张图或单段文本里的显式危险意图，对跨图组合的全局语义检测不足。

2. **Multimodal Evidence**:

    - 功能：为请求构造更丰富的视觉和文本背景，使目标模型更容易生成细节化回答。
    - 核心思路：论文把图像生成包装成现实案例材料的构造过程，先由辅助 LLM 生成场景化证据，再转成适合 T2I 模型的图像描述，并在生成失败时进行提示改写重试。
    - 设计动机：直接要求 T2I 模型生成危险相关图像容易被拒绝；用案例化、间接化的证据生成流程可以提高图像生成成功率，也让多图输入包含更多上下文。

3. **Number Chain Task**:

    - 功能：加入一个额外的跨图推理任务，分散模型注意力并增加输入处理复杂度。
    - 核心思路：每个数字链 frame 包含一个数字和指向下一帧的索引，模型被要求按指定顺序恢复数字链。论文比较 blank frame indexing、cat/dog frame indexing 和 number chain，发现任务每帧信息量越高，ASR 越高。
    - 设计动机：复杂辅助任务可能占用模型跨图注意力，使其在完成推理任务时弱化对安全语义的整体审查。
