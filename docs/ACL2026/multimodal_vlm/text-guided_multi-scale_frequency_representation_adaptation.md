...
1. **频域适配而非空间域适配**:
	...
	- 功能：减少冗余表示中的噪声，让少量参数集中调整更有信息密度的频率分量。
	- 核心思路：DCT 是正交变换，可以把嵌入从空间/通道形式转成频率系数。论文经验发现低频分量保留了大量语义信息，因此在频域做适配等价于对不同频带做更可控的修正，而不必对全部 patch 表示做高冗余更新。
	- 设计动机：空间域 adapter 在有限训练步数下容易过拟合局部噪声。频域将低频结构、高频细节和部分噪声更好地区分开，能让参数更新更平滑，训练一轮内就收敛。

2. **Multi-Scale Adaptation Strategy**:

	- 功能：让适配模块同时看到局部细节和粗粒度全局结构。
	- 核心思路：对视觉频域网格按尺度 $n$ 做 downsampling，得到 $X_{v,n}$；每个尺度都有自己的 MGFA 和 MCFA，输出 $\tilde{X}_{v,n}=G_n+wC_n$；再用 interleave-repeat 恢复到原尺寸，并对 $N$ 个尺度平均。论文在附录中发现 $N=3$ 效果最好，过大的感受野会损失局部信息。
	- 设计动机：图文匹配既需要识别局部物体，也需要理解整体场景。单一尺度要么只改细节，要么过度平滑；多尺度能按 caption 中的语义重点选择不同 receptive field。

3. **MGFA + MCFA 的互补频率调制**:

	- 功能：MGFA 做视觉频率的全局稳定校准，MCFA 做文本条件下的细粒度跨模态对齐。
	- 核心思路：MGFA 是轻量 bottleneck，两层投影加 ReLU，对每个尺度的视觉频率特征做全局变换 $G_n=f(X_{v,n})$。MCFA 则从文本频率表示 $X_t$ 预测调制参数 $\gamma,\beta$，对视觉特征做 $C_n=\gamma\odot X_{v,n}+\beta$。最终用权重 $w$ 控制跨模态注入强度。
	- 设计动机：只做视觉全局校准会缺少文本条件，只做文本调制又可能过度干扰视觉表征。两个模块相加能兼顾稳定性和语义对齐。
