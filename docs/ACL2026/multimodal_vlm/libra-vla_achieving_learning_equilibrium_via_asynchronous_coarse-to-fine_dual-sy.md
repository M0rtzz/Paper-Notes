...
description: >-
  [ACL 2026][多模态VLM][视觉语言动作模型] Libra-VLA 把机器人动作分解为"离散宏方向（macro-intent）+ 连续微姿态（micro-pose）"的混合动作空间…
tags:
  - "ACL 2026"
  - "多模态VLM"
  - "视觉语言动作模型"
...
### 整体框架
- **输入**：指令 $L$ + 观察 $\mathbf{o}_t$
- **概率分解**：$P(\mathbf{a}_t \mid \mathbf{o}_t, L) \approx \underbrace{P(\mathbf{a}_t^f \mid \mathbf{a}_t^c, \mathbf{o}_t)}_{\text{Action Refiner}} \cdot \underbrace{P(\mathbf{a}_t^c \mid \mathbf{o}_t, L)}_{\text{Semantic Planner}}$
- **System 2 (Semantic Planner)**：InternVL2.5-2B 主干 + Parallel Coarse-Action Head（12 层 transformer，hidden=1024），低频输出 $L_{\text{macro}} = M \times H_{\text{chunk}}$ 个粗 token（每步 $D$ 维 action，每维 $N$ 个 bin）
- **System 1 (Action Refiner)**：diffusion transformer + 独立 SigLIP 视觉编码器 $\mathcal{E}_{\text{vis}}$，高频从 intent buffer 取 slice 当条件，去噪出连续动作
- **Bridge**：FIFO Intent Buffer $\mathcal{Q}$ + 可学习 codebook $\mathbf{E} \in \mathbb{R}^{N \times D}$ 把离散 bin 映射成 embedding
- **输出**：连续动作 $\mathbf{a}_t \in [-1, 1]^D$

### 关键设计

1. **Hybrid Action Space + Coarse-Grained Directional Discretization**:

    - 功能：把动作每维量化成 $N$ 个 uniform bin，$y_{t,i}^{gt} = \mathrm{clip}(\lfloor (a_{t,i}+1)/2 \times N \rfloor, 0, N-1)$，但**故意用很小的 $N \ll 256$**。
    - 核心思路：之前的离散 VLA（OpenVLA）追求 $N=256$ 来逼近连续控制，这反而让 token 空间太大，VLM 学不动；本文把 $N$ 砍到很小（消融发现 $N$ 适中时性能最佳，呈倒 U 曲线），让 token 真正表示"粗方向 macro-intent"——VLM 的语义抽象力被发挥到 sweet spot，量化损失则交给后面的 continuous refiner 来补。
    - 设计动机：作者识别出"**learning complexity equipartition**"原则——性能随分解粒度呈**倒 U**曲线，**当两个子系统学习难度均衡时达到峰值**。这是本文最关键的实证发现，把"超参 $N$"提升为方法论原则。

2. **Parallel Coarse-Action Head + Adaptive Intent Injection（curriculum 解决 teacher forcing 偏置）**:

    - 功能：用 $K$ 个 learnable query 与 VLM 输出 $\mathbf{H}_t$ 做 self-attention 并行预测整个 chunk 的所有粗 token：$\mathbf{Z}_{\text{act}} = \mathrm{SelfAttn}([\mathbf{Q}_{\text{act}}; \mathbf{H}_t])_{0:K}$，再 Linear+Softmax 输出 $P(\mathbf{a}_t^c)$。
    - 核心思路：训练时 System 1 需要 $\mathbf{e}_{\text{intent}}$ 当条件——早期 System 2 还没学好，直接用预测会引入噪声破坏 System 1 训练；标准 teacher forcing 又让 System 1 永远只见过 ground truth，inference 时崩。本文用**dynamic curriculum**：System 2 预测准确率低于阈值 $\tau$ 时用 GT 取 codebook embedding，超过 $\tau$ 后切到从 $P(\mathbf{a}_t^c)$ 采样——逐步让 refiner 适应 planner 噪声。
    - 设计动机：并行 head 是为推理速度（不做 AR）；curriculum 是为训-推 gap，同时让 refiner 内化"对 planner 偏差的容错能力"，这正是 dual-system VLA 的鲁棒性根源。

3. **Asynchronous Execution with Intent Buffer + Horizon Expansion**:

    - 功能：System 2 一次预测未来 $L_{\text{macro}} = M \cdot H_{\text{chunk}}$ 步的 macro token 推进 FIFO buffer $\mathcal{Q}$，System 1 每个控制 step 从 buffer pop $H_{\text{chunk}}$ 个 token 作 condition；接下来的 $M-1$ 个 chunk 内 System 2 完全休眠。
    - 核心思路：传统 dual-system（GR00T-N1）用 static latent 当 bridge，时间一久就和环境状态脱钩；本文用 **predictive intent buffer** 让 System 2 提前规划好整个 horizon 的离散方向序列，System 1 每步取的 slice 是**时间同步**的，避免 lagging。同时离散 token 本身就具有物理可解释性（"+x 大、+y 小"），比黑盒 latent 透明。
    - 设计动机：把 VLM 这个昂贵开销从"每个控制 step 一次"摊到"每 M 个 chunk 一次"，把控制频率和规划频率解耦——这是真正可用于实时机器人的关键工程突破。
