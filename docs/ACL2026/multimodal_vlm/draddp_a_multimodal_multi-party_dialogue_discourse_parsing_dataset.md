...
### 整体框架
整体 pipeline 分为四步。第一步是数据准备：从 Friends 第一季 24 集中抽取对话片段，把官方字幕行作为基本话语单元，并利用字幕时间戳对齐视频帧和音频片段。第二步是人工标注：基于 SDRT 关系体系，为每个 utterance 标注其父节点和关系类型。第三步是质量控制：用预标注模型辅助但不决定最终标签，通过多人标注、讨论和第三方仲裁控制一致性。第四步是 benchmark：在 DraDDP 和 MODDP 上评测 RLTST、BERTLine、MODDP、LLaMIPa 以及 Qwen 系列文本/音频/视频/全模态模型。

### 关键设计
1. **面向多方互动的多模态数据构建**:

    - 功能：把英文电视剧中的多人对话转换为同步的文本、视频、音频话语解析样本。
    - 核心思路：作者把官方字幕行作为 EDU，因为字幕长度适中、通常对应说话轮次和语义边界，并且自带时间戳，方便把每个话语单元与视频帧、音频片段对齐。最终数据集包含 495 个 dialogue segments、6,374 个 utterances 和 9.1 小时平行视频内容。
    - 设计动机：论坛或游戏文本数据缺少面部表情、视线、语气等互动线索；双人多模态数据又无法覆盖多方话题分叉。电视剧对话虽然是脚本化的，但能提供稳定、密集、可对齐的多人互动样本。

2. **四阶段人工标注与预标注辅助**:

    - 功能：在复杂多方场景中得到可靠的话语依存边和关系类型标注。
    - 核心思路：作者先用在 STAC 上微调的 LLaMA3 对 DraDDP 做文本预标注，随后由 2 名博士生和 4 名硕士生观看视频并修正。标注流程包含协同标注 1/6 数据以统一标准、双人独立标注 1/3 数据并讨论分歧、剩余数据由两人初标后由第三标注者仲裁。
    - 设计动机：纯人工从零标注成本高，而完全依赖模型会引入偏差。预标注在依存结构上有 72.69% F1、关系类型上有 41.31% F1，足以减轻短距离依存的工作量，但最终决策仍由人类基于文本和音视频共同判断。

3. **按模态和说话人数拆解的 benchmark**:

    - 功能：评估多模态信息在依存边识别和关系类型识别中的真实收益。
    - 核心思路：论文使用 micro F1，Link-F1 只要求边正确，Link&Rel-F1 要求边和关系都正确。除传统模型外，作者还把 LLaMIPa 框架替换成 Qwen2.5、Qwen2.5-VL、Qwen2-Audio、Qwen2.5-Omni，分别测试文本、文本+视频、文本+音频、文本+视频+音频。
    - 设计动机：多模态不是一概有利。视频可能捕捉视线和动作，也可能引入场景噪声；音频可能对语气和情绪更敏感。把结果按说话人数和关系混淆类型展开，才能看出模态作用的条件性。
