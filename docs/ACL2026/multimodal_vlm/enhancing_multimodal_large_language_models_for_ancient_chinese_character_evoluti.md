...
**1. 古汉字演变基准：把零散的甲骨文研究升级成覆盖完整演变链的 11 任务评测。**

现有古文字基准大多只盯着甲骨文这一个阶段、或只做单一任务，根本量不出 MLLM 在"演变分析"上到底缺哪块能力。本文按字形发展把汉字切成甲骨文、金文、篆书、隶书、楷书五个阶段，收集 7740 个有完整演变记录的汉字、近 3 万张摹本图像，再在其上设计三大类共 11 个子任务：T1 基础识别（字体风格识别、时代判断）、T2 字形理解（图像级字符识别、结构分析）、T3 演变分析（跨时代对比、演变路径推理）。

所有任务统一成图文混合输入、文本输出的 QA 形式，从单图识别一路爬到跨时代推理。这样的多维度切分之所以重要，是因为它能把"模型到底栽在哪"暴露得很细——后面实验正是靠它定位出字符级识别（T2.1）是几乎所有模型的共同盲区。

**2. 字形驱动对比微调 GEVO：用正负字形对 + 课程学习，逼模型盯住笔画级的微妙差异。**

古文字演变里的字形变化往往极其细微——笔画简化、结构微调，如果直接在识别任务上做 SFT，模型很容易学到的只是表面纹理特征，而非真正的演变规律。GEVO 的破局点是把任务改造成对比学习：正对取同一字符在不同时代的变体，让模型抓住"万变不离其宗"的演变一致性；负对取不同字符的字形，逼模型学会区分字间差异。训练目标是字形识别与对比判断两类任务的混合。

为了不让模型一上来就被最难的样本淹没，GEVO 叠了一层课程学习：按字形差异的视觉显著度排序，从视觉差异大、好区分的简单对起步，逐步过渡到差异微妙的困难对。对比迫使注意力落在细节上、课程保证学习曲线平滑，两者合起来才让一个 2B 小模型在全部 11 个任务上都拿到提升。

**3. 多维评估协议：用 11 个粒度 + 专家校验，给 19 个 MLLM 画出能力地图。**

单一准确率说不清一个模型到底是"看不清字"还是"推不出演变"，所以本文把评估摊到 11 个子任务上，每个任务考的能力不同——有的吃视觉理解、有的吃知识推理、有的吃跨时代关联，再辅以领域专家验证保证标注可靠。评测覆盖从 1B 到 72B 的 19 个 MLLM，包括 GPT-4o-mini、GPT-5-mini 等闭源模型。

> ⚠️ GPT-5-mini、Gemini-3-Flash 等模型名以原文为准。

正因为评估按维度铺开，才能得出"开源 7B 反超闭源大模型""字符级识别普遍接近 0"这类细颗粒结论，精确定位每个模型的强项与短板，而不是只给一个含糊的总分。
