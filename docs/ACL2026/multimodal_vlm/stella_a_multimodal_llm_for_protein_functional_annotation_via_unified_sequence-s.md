...
1. **ESM3 统一序列-结构编码器**:
	...
	- 功能：用单个蛋白基础模型同时承载序列和三维结构信息。
	- 核心思路：ESM3 将多种蛋白模态组织到统一 embedding space 中，并在早期 transformer block 中引入几何注意力，有利于捕捉结构拓扑和局部空间关系。STELLA 直接使用 esm3_sm_open_v1 作为蛋白 encoder。
	- 设计动机：相比分别使用序列 encoder 和结构 encoder，再设计复杂融合模块，统一 encoder 能减少架构碎片化，也让后续 connector 只需学习蛋白表示到文本空间的映射。

2. **轻量 connector 与 LLM 生成头**:

	- 功能：把蛋白表示转换成 Llama 可消费的语言模型输入表示。
	- 核心思路：作者使用单层线性投影作为 modality connector。LLM 采用 Llama-3.1-8B-Instruct，负责解析任务指令并输出功能叙述、选择题答案或酶反应名称。
	- 设计动机：线性 connector 结构简单、训练稳定、计算成本低。把复杂推理和语言表达交给 LLM，可以弥补传统 pLM 难以生成自然语言功能解释的问题。

3. **OPI-Struc 多模态指令基准**:

	- 功能：为蛋白结构到文本任务提供统一训练和评测资源。
	- 核心思路：OPI-Struc 包含 Function 和 Enzyme 两个域。Function 域包括 free-text 功能描述任务和 multiple-choice 功能识别任务；Enzyme 域把酶催化功能表示为标准名称预测。评测包含 FP_ft_eval、时间外 FP_ft_eval_v2401、FP_mc_eval_1x、FP_mc_eval_4x 和 EP_eval。
	- 设计动机：只有自由文本指标会受到措辞差异影响，因此作者加入 MCQA 和酶功能准确率，让 benchmark 同时覆盖生成质量与客观判别能力。
