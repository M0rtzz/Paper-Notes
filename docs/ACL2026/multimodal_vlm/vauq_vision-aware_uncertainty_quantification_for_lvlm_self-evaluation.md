...
1. **Image-Information Score**:
	...
	- 功能：衡量图像输入对当前回答的预测不确定性贡献。
	- 核心思路：比较有图像和无图像时同一回答的条件熵。如果 $H(y|empty,t)$ 明显高于 $H(y|v,t)$，说明图像确实帮助模型确认了回答；如果差距很小，模型的信心可能主要来自语言先验。
	- 设计动机：纯文本不确定性只能说明模型是否自信，不能说明自信是否有视觉依据。IS 把视觉 evidence 的使用显式纳入评分。

2. **无监督核心区域遮蔽**:

	- 功能：避免整图移除或背景扰动带来的噪声，让 IS 更聚焦任务相关视觉证据。
	- 核心思路：聚合中后层 transformer 中生成 token 对视觉 token 的注意力，得到每个图像 patch 的重要性分数，再选取 top-K% patch 作为核心区域。实验中作者发现早期层难以定位证据，中后层更能捕捉语义区域。
	- 设计动机：直接 blank 掉整张图会混入背景和无关区域，随机遮蔽又会扰乱输入。核心区域遮蔽更接近“移除模型真正依赖的证据”，因此更能测试回答是否 grounded。

3. **熵与视觉信息的线性组合**:

	- 功能：同时捕捉语言层面的预测不确定性和视觉 grounding 强度。
	- 核心思路：$s_{VAUQ}=H(y|v,t)-\alpha IS_{core}$。预测熵高会提高风险分数，说明模型本身不确定；核心视觉信息分数高会降低风险分数，说明模型信心来自视觉证据。超参数 $\alpha$ 控制两者权重。
	- 设计动机：仅靠熵会被语言先验骗过，仅靠视觉信息可能忽视模型本身的生成不确定性。两者互补，尤其适合 factual / counterfactual 混合的真实分布。
