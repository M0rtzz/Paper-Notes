...
### 关键设计
1. **真实 A/B test grounded 数据构建**:

    - 功能：确保每个 winner 不是主观偏好，而是由真实用户行为验证。
    - 核心思路：从可信 A/B test 平台聚合 UI 变体与结果，清洗视觉标注后保留干净 UI 图像，并记录哪个版本带来更多目标用户行动。
    - 设计动机：UI/UX 的最终目标是行为结果；没有真实行为验证的 benchmark 很容易退化为视觉审美或规范检查。

2. **专家解释与 UX 维度标注**:

    - 功能：为 interpretation task 提供语义评价基准。
    - 核心思路：三位 UI/UX 专家在知道 winner 的情况下独立标注关键 UI 修改及其行为影响，只保留至少两位专家有实质重合的解释。每条解释还被映射到 12 条 UX laws，并归入 Norman 的 perception、memory、action 三个认知维度。
    - 设计动机：模型不仅要猜中哪张图好，还要说出符合专家认知的原因。专家解释让评测从二选一扩展到可诊断的行为推理。

3. **双任务评测协议**:

    - 功能：分别测模型的预测能力和解释能力。
    - 核心思路：selection task 用 FA、SA、AA 和 CA 衡量，特别是 CA 要求模型在图片顺序互换后仍选中同一个正确 UI，用来剥离位置偏差；interpretation task 用 GPT-4o evaluator 判断模型自由文本是否覆盖每条专家解释，报告 Interpretation Recall 和 Instance-level Recall。
    - 设计动机：pairwise 任务容易出现“总选第一张/第二张”的偏差，CA 能更真实反映内容理解；解释任务则揭示模型是否只是猜中结果，还是理解行为机制。
