...
**2. 双模式评估协议（ETC + EPD）：从两个难度层级测模型的自省能力。**

光让模型在"已知有错"的前提下分类，测不出它会不会主动发现错误。MMErroR 因此设了两档：ETC（错误类型分类）直接告诉模型这条链必然有错、只要它分出是哪一类，考的是已知有错时的诊断精度；EPD（错误存在检测）则要求模型先自己判断"到底有没有错"再分类，是更难的受控压力测试。由于当前数据集只含错误样本，EPD 看似可以靠"一律报错"蒙混，但评分规则要求同时正确分类错误类型才给分，这条捷径被堵死，逼模型真去定位错误。两档结合，既量化诊断精度、又量化主动发现错误的能力。

**3. 单错误受控设计 + 多阶段质量过滤：保证每条链可归因、且高质量。**

真实推理失败常是多个错误级联，但那样会让归因彻底混乱、标签说不清。MMErroR 宁可牺牲一点真实性，规定每条链只注入一个根因错误、其余步骤保持局部连贯和逻辑有效，换来诊断标签的明确性。质量则靠多阶段过滤把关：三轮专家审核（一致通过才保留）叠加四维度质量评分（连贯性 / 步骤清晰度 / 错误可定位性 / 语义一致性，均分阈值 0.5）。最终标注一致性 Cohen's Kappa $\kappa = 0.796$，第三轮淘汰率仅 2.81%（观察一致率 97.19%），整条流程从初始 10000 样本筛到 1997。
