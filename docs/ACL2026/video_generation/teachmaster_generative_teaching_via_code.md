...
1. **代码作为教育视频的中间语义媒介**:
	...
	- 功能：把不可编辑的视频像素生成转化为可执行、可检查、可修改的动画程序生成。
	- 核心思路：TeachMaster 主要通过 Python / Manim 渲染教育动画。视觉对象、几何关系、颜色、运动轨迹、等待时间和文本元素都落在代码里。模型不直接生成最终像素，而是生成可以渲染为视频片段的程序，再把每页片段与旁白音频合成完整视频。
	- 设计动机：教育内容强调准确性和可维护性。代码中间表示让教师和系统都能定位问题，例如改一个公式位置、调整动画节奏、替换图像资产，而不需要重新生成整段黑盒视频。

2. **内容规划到呈现生成的多智能体分工**:

	- 功能：把课程生产拆成脚本、分页、视觉、旁白和音频等可控子任务。
	- 核心思路：composition agent 负责 semantic skeletonization、content expansion 和 length refinement，先把教学意图变成长度合适的讲稿。pagination agent 负责页面粒度分割。routing agent 选择 Standard 或 ImageEnhanced 模式，coding agent 生成视觉代码，narration agent 结合页面蓝图、上一段讲稿和视觉代码生成连贯旁白，TTS agent 生成音频和语速信息。
	- 设计动机：教育视频要求叙事、画面和声音协同。单流生成容易在画面漂亮、脚本完整和时长可控之间失衡；多智能体分工让每个模块围绕一个明确质量目标优化。

3. **渲染闭环中的调试、同步与布局验证**:

	- 功能：把 LLM 生成代码的不稳定性和多模态错位问题压到可接受范围。
	- 核心思路：debugging agent 通过执行渲染拿到错误栈，然后修复代码，超过重试阈值时用标准模板兜底；synchronization agent 将 debug 后代码中的事件锚点和旁白语义单元对齐，根据语速插入 timing 控制；layout agent 检测对象重叠，按启发式扫描寻找更优坐标并回写代码。
	- 设计动机：教学视频失败常常不是单纯“内容错”，而是字幕挡住图、动画比旁白快、代码渲染失败。把这些问题变成可执行验证和代码修复，比事后人工审查每帧更可扩展。
