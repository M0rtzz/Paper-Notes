...
1. **RMR-75K 段级映射数据集**:
	...
	- 功能：把 reviewer 的具体问题和 rebuttal 中对应回应对齐，为 actionability 提供可训练标签。
	- 核心思路：先用结构化线索或 GPT-5 将 weakness/question 切成单一关注点；再通过显式锚点和语义匹配把 review segment 对齐到 rebuttal span；最后按置信度贪心选择一对一匹配。
	- 设计动机：整篇 review 与整篇 rebuttal 的粒度太粗，无法判断哪条评论带来了哪种回应。段级映射让“作者采纳了哪一条建议”变成可观测信号。

2. **Perspective 与 impact 双标签**:

	- 功能：同时控制反馈主题和衡量反馈可执行性。
	- 核心思路：review segment 被分成 Experiments、Evaluation、Reproducibility、Novelty、Theory、Writing、Presentation 七类；rebuttal impact 被分成 CRP、SRP、VCR、DWC、DRF 五类，分别对应已完成修改、具体修改计划、模糊承诺、防守不改、转移问题。
	- 设计动机：同一条论文可能在实验、理论、写作等方面有不同问题；只有在同一 paper 和同一 perspective 内比较，偏好排序才不会混入主题差异。

3. **基于 rebuttal impact 的 DPO 偏好优化**:

	- 功能：让生成模型优先产生更可能引发实际修改的评审反馈。
	- 核心思路：在同一论文、同一 perspective 内构造 pair，按 `CRP > SRP > VCR > DWC > DRF` 排序，把高 impact 的 review segment 作为 chosen，低 impact 的 segment 作为 rejected。DPO 目标提升 chosen 相对 rejected 的概率，并混入少量 SFT loss 防止长上下文下 perspective 控制漂移。
	- 设计动机：rebuttal 不是显式人工打分，但它反映了作者实际反应。把它转为 pairwise preference，比直接预测一个粗糙 actionability 分数更稳健。
