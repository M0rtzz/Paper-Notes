...
1. **层选择的 LoRA 风格适配**:
	...
	- 功能：只在高层 transformer block 的 attention/FFN 投影上训练低秩 adapter，把教授风格写入很小的参数集合。
	- 核心思路：对选中的权重矩阵使用 LoRA 增量，底座权重冻结；论文的典型设置是 top-4 LoRA。这样可训练参数规模约为千万级，而不是 20-30 亿全量参数。
	- 设计动机：风格、语气、语篇格式等属性更像高层表达控制，而不是底层语法或代码知识。只更新高层能减少灾难性漂移，也便于为多个教师保存不同个性化模块。

2. **教授偏好奖励模型**:

	- 功能：把“更像教授、更有教学价值、更正确”的偏好转成可优化的标量奖励。
	- 核心思路：奖励模型学习 Bradley-Terry 形式的偏好概率，核心是让 `r_phi(x, y_w) - r_phi(x, y_l)` 越大越好，损失可写为 `-log sigma(r_phi(x, y_w) - r_phi(x, y_l))`。
	- 设计动机：SFT 只能模仿平均示范，容易学到格式但抓不住教师对措辞强弱、鼓励方式、边界用例提醒的细微偏好；奖励模型给 PPO 提供了更直接的风格选择信号。

3. **KL 约束下的 PPO 策略优化**:

	- 功能：在奖励模型偏好的方向上继续优化反馈生成，同时避免模型离开 SFT 后的可靠区域。
	- 核心思路：目标是最大化 `r_phi(x, y) - beta KL(pi_theta(.|x) || pi_ref(.|x))`，其中 `pi_ref` 是冻结的 SFT 策略；PPO 使用 clipped ratio 稳定 token-level 更新。
	- 设计动机：教育反馈不能只追求“像教授”，还要避免为了风格奖励而输出错误诊断。KL 项把策略拴在已经具备基础教学格式和正确性的 SFT 模型附近。
