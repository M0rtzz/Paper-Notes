...
1. **三源奖励的职责划分**:
	...
	- 功能：让不同 reward 处理不同类型的业务要求，避免单一奖励源既要理解人类风格又要承担硬规则检查。
	- 核心思路：RM 由 6632 条高质量偏好样本训练，覆盖线上 case、语言专家标注、任务专家标注和 SOP 路径样本，负责学习人类 BD 的自然表达和整体偏好；RJ 是 LLM 评审器，使用可编辑 rubric 扣分，覆盖安抚、继续追价、越界承诺、重复确认、无响应误判等复杂行为；RF 是规则函数，快速捕捉内部术语、格式破坏、CoT 泄露、混合语言、过长输出等确定性问题。
	- 设计动机：酒店价格谈判里，软能力和硬约束混在一起。把所有问题都交给 RM 会受数据覆盖限制，把所有问题都交给规则又无法学习说服技巧。三源分工让系统既能学到“像优秀 BD 一样说话”，又能对上线事故做快速补丁。

2. **有界的 Reward Enhancement**:

	- 功能：让 RJ/RF 修正 RM，而不是直接和 RM 线性相加导致 reward 尺度失控。
	- 核心思路：REPO 先把 judge 分数和规则分数聚合为 $E_{judge}+E_{func}$，再裁剪到 $[-n,n]$，得到 $E_{enh}$；总奖励写成 $R_{total}=R_{model}(1 \pm E_{enh}/n)$，实验中 $n=100$。当 RM 为正且辅助信号也正时奖励被放大；当 RM 为正但辅助信号负时奖励被削弱；当 RM 为负且辅助信号负时惩罚被放大；当 RM 为负但辅助信号正时惩罚被缓和。
	- 设计动机：RM 是主方向，RJ/RF 是稳定性补充。这样设计避免了多源 reward 简单相加时某个规则分数压过偏好模型，也让业务补丁以“调制”方式影响策略，减少 RL 训练震荡和 reward hacking。

3. **部署驱动的快速迭代闭环**:

	- 功能：让线上观察到的问题能迅速进入训练信号，而不必重新收集大规模偏好数据。
	- 核心思路：如果线上发现模型在多工单对话里只处理一个房型，或泄露“work-order”等内部词，团队可以更新 RJ prompt 中的扣分规则，或新增 RF 正则，然后继续 RL 训练。RM 仍提供基础人类偏好，RJ/RF 负责针对新错误类型打补丁。
	- 设计动机：工业对话的约束会随着业务流程变化而变化，标注数据通常跟不上。REPO 的迭代机制把 reward engineering 变成可审计、可快速修改的工程接口。
