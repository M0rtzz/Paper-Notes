...
1. **多维度迭代精修**:
	...
	- 功能：把文学翻译的质量优化拆成表达流畅和文学效果两个子目标，让不同 LLM 模块分别处理。
	- 核心思路：Expression Optimizer 负责让中文译文更自然、凝练、符合搭配；Literary Effect Preserver 负责保留比喻、修辞、语气和文学张力；Aggregator 再把二者合并为一个平衡版本。评估器不仅打分，还提供下一轮修订的具体反馈。
	- 设计动机：随机采样 + judge 只能在候选之间选择，普通自我改写又容易被单一偏好牵引。多维度拆解让模型在“好读”和“有文学性”之间显式探索，迭代轨迹也自然形成了质量层级。

2. **从迭代历史构造偏好数据**:

	- 功能：把每轮生成的不同质量译文转化为可复用的偏好对，而不是只保留最终答案。
	- 核心思路：对同一源句下所有被评估过的译文，如果 $score(T_w)>score(T_l)$，就构造偏好对 $(T_w,T_l)$。这样每个样本不只贡献一个 SFT target，还贡献多个细粒度比较信号。
	- 设计动机：文学翻译的细微质量差异很重要，单个参考译文只能告诉模型“该输出什么”，偏好对则告诉模型“哪些细节更好”。这些偏好对可用于 DPO、CPO、SimPO，也可用于训练单独的 reward model。

3. **显式奖励模型 + 复合 GRPO 奖励**:

	- 功能：用稳定、低成本的本地奖励替代训练时反复调用 LLM-as-a-judge。
	- 核心思路：作者把 LLM 的语言建模头替换为线性标量头，用 Bradley-Terry loss 训练奖励模型，并加入 $\lambda(r_w+r_l)^2$ 让 reward 居中。GRPO 阶段对每个源句采样 $G=16$ 个译文，使用 $r(x,y)=r_{RM}+0.05\cdot r_{BLEU}+r_{fmt}$：reward model 负责整体质量，BLEU 提供词汇稳定信号，格式奖励惩罚不符合 JSON 输出格式的结果。
	- 设计动机：论文发现 DPO 系列在该任务中会让性能下降，说明离线偏好对直接推策略并不稳定。显式 RM 加 GRPO 的在线探索更能利用偏好数据，同时避免每步调用强 LLM 的高成本。
