...
1. **Constrained Uniform Top-K Sampling**:
	...
	- 功能：在不破坏局部语义合理性的前提下，为推理 rollout 注入多样性。
	- 核心思路：给定当前位置分布 $P_\theta(v|q,x_{<t})$，先取 Top-K 候选集合，再保留概率大于阈值 $\delta$ 的 token，得到 $\mathcal{S}_t$；若集合为空则退回 Top-K，若只有一个候选则退化为确定性选择。最终在 $\mathcal{S}_t$ 上均匀采样，即每个候选概率都是 $1/|\mathcal{S}_t|$。
	- 设计动机：推理任务需要探索，但探索必须是“结构保持”的。CUTS 只在模型自己认为高置信的局部邻域里拉平概率，因此比全局熵正则或高温采样更不容易产生不连贯推理。

2. **Mixed-CUTS 双流 rollout**:

	- 功能：把稳定利用和受控探索放进同一个 GRPO 组内，恢复相对优势信号。
	- 核心思路：每个 prompt 的组 $\mathcal{G}$ 被拆成标准采样子组 $\mathcal{G}_{std}$ 和 CUTS 子组 $\mathcal{G}_{CUTS}$。优势仍在合并组上计算。论文用总方差分解说明混合组方差包含组内方差和组间均值差两部分：只要 CUTS 子组的平均奖励或内部方差不同于标准子组，$\sigma^2_{mixed}$ 就不会为 0。
	- 设计动机：只用 CUTS 会让行为策略偏离当前模型，训练可能不稳定；只用标准采样又会在饱和数据上塌缩。混合设计让标准轨迹提供 anchor，CUTS 轨迹提供 contrast。

3. **前缀保护与受控 off-policy 偏差**:

	- 功能：避免探索在推理链开头做出不可挽回的错误，同时控制 CUTS 行为策略与旧策略的偏离。
	- 核心思路：CUTS 在前 $T_{warm}$ 个 token 使用标准采样，等问题理解和推理格式稳定后再启用局部均匀采样。训练仍使用标准 GRPO clipped objective，偏差由三层限制共同约束：Top-K 保证候选来自模型高概率区，阈值 $\delta$ 去掉尾部，PPO/GRPO clipping 限制单步更新幅度。
	- 设计动机：数学推理早期决策很关键，过早随机探索可能毁掉整个解题过程。前缀保护把探索推迟到更适合分叉的位置，让多样性更像“换一条合理解法”，而不是“从第一步开始乱走”。
