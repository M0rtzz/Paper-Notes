...
SiPeR 把情景对话推荐拆成"Where"和"What"两个决策串起来。给定视觉场景和对话历史，系统先用场景转换估计（STE）判断要不要换场景、换到哪个场景，把用户引导到正确的"货架"前；再用贝叶斯逆推理（BI-INF）从对话里反推用户真正想要的物品，在当前场景的候选里排序。两者中间靠对话状态追踪把自然语言对话压成结构化意图元组，给概率推理一个干净的输入。

### 关键设计

**1. 场景转换估计（STE）：决定何时换场景、换到哪个场景。**
...
**2. 贝叶斯逆推理（BI-INF）：把用户话语当成行动，反推隐式偏好。**
...
**3. 对话状态追踪：把对话压成结构化意图，给概率推理一个干净输入。**

在自然语言空间里直接做贝叶斯推理太不可控，需要先收敛成结构化表示。这一步直接指示一个强 LLM 从对话历史中抽取 ⟨intent, slot, value⟩ 元组，人工验证准确率达 98.8%。有了这组结构化意图，BI-INF 的似然计算就不必在开放文本上展开，推理空间更可控。
