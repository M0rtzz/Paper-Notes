...
1. **基于最近距离的洗牌激活对齐**:
    ...
    - 功能：把两条经过不同随机排列的激活向量对齐到同一排列，使同一隐藏维度的元素重新对应起来。
    - 核心思路：如果原始向量 $x_a$ 和 $x_b$ 足够接近，那么洗牌后的 $x_a'$ 和 $x_b'$ 虽然坐标顺序不同，但对应元素的数值距离仍然最小。作者把对齐写成 $\min_M \|x_a' - x_b'M\|_2$，其中 $M$ 是置换矩阵；再把它转为标准 assignment problem，构造代价矩阵 $D[i,j]=(x_a'[i]-x_b'[j])^2$，用 Hungarian algorithm 找最小总代价匹配。
    - 设计动机：已有洗牌防御假设攻击者必须猜 $h!$ 种排列之一，但这里的攻击根本不猜全局排列，而是利用数值接近性逐元素匹配。只要扰动小到不打乱邻近关系，指数级排列空间就退化成一个可高效求解的二分匹配问题。

2. **利用安全截断误差构造近邻激活**:

    - 功能：在 Transformer 输入是离散 token、不能任意微调输入向量的限制下，仍然制造一批彼此很近但不完全相同的中间激活。
    - 核心思路：Transformer 的 token 输入先经过 embedding，攻击者不能像连续输入任务那样直接加 $\epsilon$ 扰动。作者转而利用安全推理中的定点数截断：模型浮点值通常编码为 $x=\lfloor x_f\cdot 2^p\rceil$，乘法后为了保持精度要截断，而 ABY3、CrypTFlow2、SecretFlow-SPU 等协议可能产生随机 1-bit 误差。以默认 18-bit 精度为例，这相当于约 $2^{-18}$ 量级的微扰；攻击者反复提交同一 prompt，就能得到可对齐的近邻激活集合。
    - 设计动机：这一步是论文最关键的“落地”部分。单纯说神经网络是 Lipschitz 连续还不够，因为 GPT 输入离散；截断误差把协议实现细节变成攻击所需的随机微扰源，而且仍然符合半诚实威胁模型。

3. **在未知排列下抽取功能等价权重**:

    - 功能：用对齐后的激活输入输出对恢复每个线性层的权重，并解释为什么不知道真实排列也不妨碍攻击价值。
    - 核心思路：对第 $l$ 层，原始关系是 $X^{(l+1)}=X^{(l)}W^{(l)}$。攻击者实际解到的是 $W'^{(l)} = \pi^{(l)^{-1}}W^{(l)}\pi^{(l+1)}$，即原权重经过输入维行置换和输出维列置换。由于 GELU、softmax、LayerNorm 等非线性在对应维度置换下可以保持等价，连续层使用相互兼容的置换后，整个前向传播仍能输出被同样坐标变换过的中间表示，并最终保持功能一致。
    - 设计动机：这直接回应了一个可能的反驳：抽到的不是原矩阵，是否还有用。论文指出神经网络权重存在排列对称性，行列置换版权重仍可作为代理模型、分析模型信息，或作为后续黑盒攻击的初始化。
