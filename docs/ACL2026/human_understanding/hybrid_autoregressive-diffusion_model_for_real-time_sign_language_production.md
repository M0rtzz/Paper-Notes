...
1. **混合自回归-扩散生成框架**:
	...
	- 功能：同时获得自回归模型的低首帧延迟和扩散模型的高质量姿态细化。
	- 核心思路：在 diffusion denoiser 的 self-attention 中加入 causal mask，让位置 $i$ 只能关注 $j <= i$ 的当前或历史 token；同时使用 flow-based diffusion 学习从噪声到目标姿态的连续变换，以减少传统 DDPM 多步采样成本。
	- 设计动机：纯扩散模式质量高但首帧慢，纯自回归模式快但质量下降。混合模式让系统先快速响应，再在每帧生成中保留细化能力。

2. **三专家多尺度姿态表示**:

	- 功能：分别建模面部、身体和双手的局部动态，再融合成全身姿态。
	- 核心思路：每个关键点包含 $(x,y,c)$，先经过 MLP 和时间位置编码；再按 face/body/hands 分组进入专门 expert；每个 expert 输出经过 joint average pooling 后，通过 attention-based fusion 得到当前帧融合表示。
	- 设计动机：手语不是全身均匀运动，面部非手势信号、身体姿态和双手轨迹具有不同尺度。特别是双手经常有强耦合，把左右手拆开反而可能破坏相对几何和同步关系。

3. **Confidence-Aware Causal Attention**:

	- 功能：在 noisy 2D pose 输入下，更信任高置信度关键点和帧，提升自回归生成鲁棒性。
	- 核心思路：在 causal attention 的 logits 上加入平均关键点置信度偏置，形式上相当于把 attention score 加上 $beta * c_bar(s)$，其中 $c_bar(s)$ 是第 $s$ 帧关键点置信度均值，$beta$ 是可学习标量。
	- 设计动机：2D 姿态估计不可避免有遮挡和低置信度点。让注意力显式感知可靠性，可以减少错误关键点在后续帧中的传播。
