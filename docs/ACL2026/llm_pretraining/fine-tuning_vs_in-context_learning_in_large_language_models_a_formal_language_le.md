...
1. **三大 desiderata (D1/D2/D3) 框架**:

    - 功能：把"公平比较 FT 与 ICL"这个模糊问题拆成三条可验证的标准——D1 任务规约 (纯语法+零 prompt)；D2 资源平等 (同样的训练串、各自最优超参)；D3 指标可比。
    - 核心思路：作者明确指出 Mosbach 2023 只满足 D1+D2 但违反 D3，这正是结论矛盾的根源。每一条都对应自然语言基准的一个具体缺陷 (任务模糊 / 资源不公 / loss 不可比)，因此三条同时满足才能得出可信结论。
    - 设计动机：让"比 FT 和 ICL"这件事第一次有了可证伪的实验协议，而不是各做各的。

2. **HPCFG 形式语言作为受控测试床**:

    - 功能：替代自然语言基准，提供精确边界 ($P_L(s)>0$ 为 in-language)、可控采样、零污染。
    - 核心思路：HPCFG 是带概率的层级 CFG (如图 2)，从开始符号 $S$ 递归展开产生式，直至 alphabet $\mathbf{T}$；一个串的生成概率等于所用产生式概率之积 (如示例 $P(s) = 0.5^{23}$)。alphabet $\mathbf{T}\subset\mathbf{V}$ 是 LLM 词表的子集，可以是数字 / 拉丁 / 罕见 token。Out-of-distribution 语言通过修改语法规则得到，距离 $\mathtt{dist}(L_1, L_1^{(\ell)})$ 随被改规则数 $\ell$ 单调增。
    - 设计动机：自然语言里 in-distribution 和 out-of-distribution 没有形式化定义；HPCFG 让"距离"可计算、"边界"可证明，且 alphabet 替换可以隔离"语法"与"token"两个变量。

3. **判别式 AUC 测试 (Discriminative Test)**:

    - 功能：取代生成式 loss/perplexity，作为跨 LLM、跨范式可比的语言熟练度指标。
    - 核心思路：生成 loss 不可比的原因有二——绝对 loss 受预训练先验影响，且 FT 与 ICL 的输入 prompt 不同。判别式测试改为问"模型给 in-language 串的 loss 是否系统低于 out-of-language 串"，转成二分类 AUC：$\mathtt{auc}_M(L, \mathsf{T}(L)) \in [0,1]$。同一 LLM 在同种 prompt 格式下评分，模型先验和 prompt 偏差被消掉。Out-of-language 串通过"edit by k 次插/删/替"或"按字母表完全随机"两种方式构造，edit 距离越小越严苛。
    - 设计动机：作者类比"两个非母语者生成质量可能相同，但犯的错可以暴露语言先验"——只看模型生成什么不够，还要看它能否拒绝什么。
