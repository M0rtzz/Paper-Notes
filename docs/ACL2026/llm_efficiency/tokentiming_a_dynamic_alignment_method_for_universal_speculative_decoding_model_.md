...
### 关键设计
1. **Dynamic Token Warping 对齐**:

	- 功能：在 draft token 序列和 target proxy token 序列之间构造完整的多对多映射，解决异构 tokenizer 下 token 粒度和位置不一致的问题。
	- 核心思路：算法构造累计代价矩阵 $C$，局部距离主要使用 Levenshtein 编辑距离；每个位置选择来自上、左、左上的最小累计代价，最后从终点回溯得到最优路径 $\pi^*$。这条路径不是简单的词表交集匹配，而是按字符串相似性和序列顺序共同决定的动态对齐。
	- 设计动机：异构 tokenizer 的错位通常是局部的、单调的，而不是完全无序的。DTW 能在保持顺序的前提下容纳局部拉伸或压缩，因此比“只看 token 是否相同”的 TLI 更适合处理 BPE、WordPiece 等分词粒度差异。

2. **带窗口约束的概率转移**:

	- 功能：把 draft 侧候选 token 的概率转换成 target proxy token 的候选概率，使标准投机采样公式仍可使用。
	- 核心思路：对齐路径给出 draft token block 与 target token block 的对应关系，TokenTiming 沿路径分配概率；target 端验证时仍用 $\min(1, q(t_j) / p(t_j))$ 作为接受概率，其中 $q$ 来自 target model，$p$ 来自映射后的 draft 分布。为了减少 DTW 计算，作者加入 Sakoe-Chiba Band，只在 $|i-j| \leq w$ 的带状区域搜索，把复杂度从 $O(km)$ 降到 $O(w\max(k,m))$。
	- 设计动机：无损投机解码的关键不是让 token 字符串看起来一致，而是要让接受-拒绝校正有一个合理的 draft 概率。窗口约束则利用分词错位通常不会无限漂移的事实，用很小的对齐开销换取更稳定的概率映射。

3. **即插即用的异构 draft model 使用方式**:

	- 功能：让任意现成小模型都可以作为 draft model，而不需要重新训练 draft head 或对齐词表。
	- 核心思路：TokenTiming 把 tokenizer 差异都放在每轮 Draft-Verify 之间的轻量对齐步骤中处理；target model、draft model 本身都不需要改权重。实验中 draft model 从 68M 的 Vicuna 到 350M OPT、0.5B/0.6B Qwen 都可接入，目标模型覆盖 dense、distilled 和 MoE 架构。
	- 设计动机：投机解码真正难落地的地方不是算法形式，而是“为每个 target 找一个又小又同词表的 draft”。如果异构词表不再是硬约束，系统可以优先按速度、能力和部署条件选择 draft model。
