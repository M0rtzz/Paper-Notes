...
1. **四类思维线索的结构化生成**:
	...
	- 功能：把大模型冗长的 chain-of-thought 压缩成小模型更容易利用的高层指导。
	- 核心思路：作者借鉴认知模块化和 LLM agent 的问答流程，把推理拆成 Goal、Planning、Retrieval、Action。Goal 让小模型明确要解什么，Planning 给出子问题分解和路线，Retrieval 补充必要知识，Action 则提供关键运算或逻辑步骤。每个 effort level 都覆盖这四类信息，只是深度和 token budget 逐级增加。
	- 设计动机：直接把大模型完整思维链喂给小模型并不经济，也可能超过小模型理解能力。结构化 insight 保留“为什么这样解”的骨架，同时去掉大量试错和解释性冗余，让小模型获得比普通提示更强、比完整思维链更短的指导。

2. **基于 PPL / entropy 的 sufficiency classifier**:

	- 功能：判断当前 insight 是否足够小模型完成问题，决定是否继续调用大模型。
	- 核心思路：SLM 处理 $Q \oplus I^t$ 时会产生每个 token 的预测分布。论文计算每个 token 的 PPL 和 entropy，再取均值、标准差、中位数、最大值、最小值、25/75 分位数，以及末 20 个 token 与前 20 个 token 的趋势差。直觉是：如果线索能帮助小模型形成稳定预测，entropy 会更低，分布统计也更像“可解”样本。
	- 设计动机：固定低/中/高 thinking budget 会浪费简单题，也可能不够解决难题。用小模型自身的不确定性作为信号，可以把资源分配和模型能力绑定起来：同一条 insight 对强 SLM 可能足够，对弱 SLM 可能还不够。

3. **阶段化早停与失败回退策略**:

	- 功能：在性能和成本之间做细粒度折中，而不是一次性选择 SLM 或 LLM。
	- 核心思路：LLM 先生成 low effort insight，classifier 判够则立即停；不够再生成 medium，再不够生成 high。若全部阶段都不达阈值，则使用 sufficiency score 最高的阶段，而不是机械采用最长线索。
	- 设计动机：许多题只需要目标澄清或简单规划，继续生成 action 细节会增加成本；另一些题虽然需要更长指导，但最终答案仍可由小模型生成。阶段化设计让 Tandem 介于 LLM cascade 和完整大模型推理之间，既不是单次路由，也不是全程大模型接管。
