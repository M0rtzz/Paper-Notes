...
## 方法详解
STReasoner 的方法由三层组成：数据层用 Network SDE 生成可控时空系统，模型层用时间序列 encoder 把数值信号接入 LLM，训练层用 Align、SFT 和 S-GRPO 逐步建立模态对齐、推理冷启动和空间感知推理行为。

### 整体框架
给定图 $G=(V,E)$、每个节点的时间序列 $T_i$ 和自然语言 query $Q$，模型需要输出中间推理过程 $R$ 和最终答案 $A$。数据合成阶段，Scenario Generation Agent 先生成交通、能源、公共健康等场景描述；Scenario Parsing Agent 把描述解析成节点、边、时间模式和空间依赖；Judge Agent 检查逻辑；SDE Parameters Agent 和 Time-Varying Adjacency Agent 分别生成节点 drift/diffusion、耦合强度、时间变化边权和传播时滞；仿真模块积分网络 SDE 得到时空序列。模型训练阶段，STReasoner 把节点时间序列 patchify 后送入 5 层 MLP encoder，再按节点顺序把时间序列 embedding 插入文本 token 流，和图结构、问题一起送入 Qwen3-8B。

### 关键设计
1. **Network SDE 多智能体数据合成**:

	- 功能：生成同时具有可控时间动态、空间依赖和文本语义的训练/评测数据。
	- 核心思路：每个节点的连续 latent process 由 drift、diffusion 和邻居耦合项共同决定。作者区分 demand source nodes 和 propagation nodes：前者可以有正弦或均值回复等外生时间模式，后者主要受邻居影响；边权是 time-varying adjacency，可以模拟早晚高峰方向变化；每条边有 propagation lag，模拟交通、污染或疾病传播的延迟。多智能体流水线负责生成场景、解析结构、设定 SDE 参数、验证对齐并仿真。
	- 设计动机：如果数据只有时间序列没有文本，LLM 学不到语言语义；如果只有文本没有可控图动力学，又无法验证空间推理是否正确。Network SDE 给了可解释、可控、可自动问答生成的底层世界模型。

2. **STReasoner 时间序列-语言融合架构**:

	- 功能：把节点级时间序列、图结构和自然语言 query 合成 LLM 可处理的上下文。
	- 核心思路：模型使用轻量 5 层 MLP 作为 time series encoder。输入时间序列先 patchify，编码后的 patch embedding 作为特殊标记插入文本流，例如 `[Node1:<TS1>, Node2:<TS2>, ..., Graph, Question]`。图结构以文本方式提供，时间序列 embedding 按节点顺序嵌入。作者还采用类似 ChatTS 的 value-preserving normalization，尽量保持原始数值信息而不是只保留形状。
	- 设计动机：纯文本表示时间序列 token 成本高，纯图像表示可能丢失精确数值。单独的 TS encoder 在全局形状和数值精度之间折中，也让 STReasoner 的 token 成本远低于把时间序列写成长文本。

3. **三阶段训练与 Spatial-Aware GRPO**:

	- 功能：先对齐模态，再注入推理，再强制模型利用空间结构。
	- 核心思路：Stage 1 用 ST-Align 做大规模对齐预训练，问题覆盖时间属性、空间属性和时空属性。Stage 2 用 ST-CoT 做 SFT，CoT 由 Claude-4.5-Sonnet 采样 5 个候选并通过 rejection sampling 保留答案正确的推理轨迹。Stage 3 使用 S-GRPO：同一问题生成两组回复，一组带显式空间结构 $o^{sp}$，一组不带空间结构 $o^{ns}$。如果带空间结构的奖励 $r^{sp}$ 明显优于无空间结构奖励，即 $r^{sp}>\beta r^{ns}$，则给额外空间奖励 $\alpha$；否则只用原始 $r^{sp}$。最终组内归一化 advantage 并用 GRPO 目标更新。
	- 设计动机：普通 GRPO 只看最终对错，模型可能从时间趋势中猜答案。S-GRPO 的对比设计要求“有图确实更好”才加奖，直接把空间结构使用变成优化目标。
