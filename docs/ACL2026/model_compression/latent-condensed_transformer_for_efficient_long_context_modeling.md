...
description: >-
  [ACL2026][模型压缩][长上下文建模] LCA 提出在 MLA 的潜空间中直接进行上下文压缩——对语义潜向量用查询感知加权池化聚合、对位置键用锚点选择保持位置精度——在 128K 上下文中实现 2.5 倍预填充加速和 90% KV 缓存压缩，同时保持竞争性性能。 领域现状：LLM 长上下文处理面临两大瓶颈：KV 缓存…
tags:
  - "ACL2026"
  - "模型压缩"
  - "长上下文建模"
