...
早选省计算但掉精度，晚选保精度但少省内存，这个 trade-off 没有放之四海皆准的最优点。ASL 把它暴露成单一阈值 $\theta$：方差一降到 $\theta$ 以下就触发选择，$\theta$ 越高越早选（更快、可能损精度），$\theta$ 越低越晚选（更精确、更慢）。比起让工程师对每个任务手动去试选择层，一个连续可调的旋钮在不同精度-速度需求的场景间切换要实用得多——实验里阈值扫描也确实给出平滑过渡而非突变。
