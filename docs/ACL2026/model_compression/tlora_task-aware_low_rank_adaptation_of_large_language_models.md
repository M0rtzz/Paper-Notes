...
flowchart TD
    A["输入：预训练权重 W₀ + 下游任务训练样本"]
    A --> B["用少量样本估计<br/>输入激活协方差 C + 模块敏感性分数 S(Wᵢ)"]
    B --> C["任务感知 A 初始化<br/>对 W₀C 做 SVD 取 top-r 右奇异向量得 A（冻结），令 B=0"]
    C --> D["自适应 rank 分配<br/>rᵢ ∝ S(Wᵢ)，关键模块分到更高秩"]
    D --> E["rank 耦合的 scaling 分配<br/>按重要性给每个模块单独的 αᵢ"]
    E --> F["训练：冻结 W₀ 和 A，只更新 B"]
    F --> G["输出：约半参数、合并友好的低秩适配"]
```
