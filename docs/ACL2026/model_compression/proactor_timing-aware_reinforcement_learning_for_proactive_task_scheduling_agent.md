...
### 关键设计
1. **domain-agnostic reference action 自动标注**:

	- 功能：为 RL 提供大规模 proactive timing supervision，同时避免人工标注每个 turn 的高成本。
	- 核心思路：oracle annotator 在完整对话上下文和统一 action catalog 下，为每个 turn 生成 reference action candidates；对于有历史 action observation 的数据，还可以用实际 triggered actions 过滤标注质量。
	- 设计动机：主动行为本来就有多个合理时机，所以标注不能被理解成唯一答案。把 annotation 视为 reference range，才能支持 RL 探索比 SFT 更灵活的 timing policy。

2. **proactiveness metrics 与 PRI**:

	- 功能：把“动作是否对”和“是否主动但不过度”拆成可量化指标。
	- 核心思路：AC 衡量预测动作与 reference action 的平均参数/动作对齐，Max AC 衡量最佳动作对齐，Difference 衡量预测稳定性；PT 奖励不晚于 reference-ready window 的 ready action；FTR 惩罚 reference coverage 外的误触发；RAR 衡量 ready action 比例。综合排名用 PRI，即 consistency index 和 timing index 的 harmonic mean。
	- 设计动机：单看 AC 会偏向保守模型，单看 PT/RAR 又可能鼓励乱触发。PRI 让模型必须同时兼顾一致性和主动性。

3. **turn-level GRPO 与 stage-aware rewards**:

	- 功能：在 dialogue turn 粒度给密集 reward，降低长对话 trajectory reward 的 credit assignment 难度。
	- 核心思路：每个 turn 采样 $K$ 个 action candidates，用 turn-level reward 计算 advantage，再用 GRPO/PPO-style clipping 更新。reward 包括 RAC/Max RAC、General/Custom RULER、Weighted Metric、Adaptive Metric 和 Adaptive RULER。Adaptive RULER 使用 $R_{adR}(u)=(1-\lambda_u)R_{metric}(u)+\lambda_u R_{RULER}(u)$，其中 $\lambda_u$ 逐步升到 $\lambda_{max}=0.3$。
	- 设计动机：早期训练需要探索 timing，后期需要控制误触发和一致性。固定单目标 reward 难以覆盖整个学习过程，stage-aware composite reward 更适合主动调度。
