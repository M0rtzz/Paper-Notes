...
训练数据刻意用 OOD 的 AIME 2022/2023 和 MATH 500，与评估集 AIME 2024/2025 不重叠，再配过采样平衡正负样本。这样训出来的模型在 OOD 上达到 AUROC=0.7072，明显超过两个基线，能在轨迹只跑了一半时就预判它会不会和别人撞答案。
...
这种在线贪心的好处是冗余轨迹在半途就被掐断、而不是白白生成到底，省下的 token 远多于事后剪枝；同时"每组留一条代表"保证了答案多样性不被破坏——少数不同答案各自成组，正确答案所在的稀有分支不会被误剪。贪心虽不保证全局最优，但在实践中很好地平衡了效率与多样性。

**3. OOD 泛化训练策略：判断模型要能用在没见过的推理模型上。**

真实部署里不可能为每个新上线的推理模型重训一个判断模型，所以判断模型必须跨模型泛化。本文把训练集（AIME 2022/2023、MATH 500）和评估集（AIME 2024/2025）严格分开，且训练轨迹来自多个不同推理模型生成的并行对，逼判断模型去学"推理过程是否同质"这种与具体生成模型无关的信号，而非记住某个模型的表面文风。最终它能泛化到训练时未见过的推理模型，是整套方案具备实用价值的关键。
