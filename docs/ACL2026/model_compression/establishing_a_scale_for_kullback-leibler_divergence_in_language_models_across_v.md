...
description: >-
  [ACL 2026 Findings][模型压缩][KL散度] 本文利用对数似然向量将不同架构的语言模型嵌入统一空间，系统测量了预训练、模型规模、随机种子、量化、微调和层间等多种设置下的 KL 散度特征尺度，并发现预训练轨迹在对数似然空间中呈亚扩散行为——尽管权重空间持续漂移，模型输出分布早期即趋于稳定。
tags:
  - "ACL 2026 Findings"
  - "模型压缩"
  - "KL散度"
...
**1. 跨设置 KL 散度尺度建立：给"0.1 bits/byte 到底算大还是算小"一个可对照的标尺**

KL 散度本身是个抽象数字，孤立看根本不知道它意味着多大的行为差异——0.1 bits/byte 在两个相邻检查点之间可能已是显著变化，放到跨模型类型比较里却是微不足道的扰动，没有参照系就无法正确解读。本文用 Pile 语料库中 10,000 条文本作为统一文本集，把 KL 散度按平均文本长度归一化到 bits/byte，再系统测出六类设置各自的特征尺度：预训练后期连续检查点约 0.01–0.05、早期训练约 0.05–0.1、不同随机种子约 0.1、不同模型规模约 0.15–1.7、8-bit/4-bit 量化约 0.44/0.49、微调约 0.40 bits/byte。有了这把横跨两个数量级的标尺，任何一次测得的 KL 散度都能被放回光谱里判断"相当于哪一类变化"。

**2. 预训练轨迹扩散分析：发现权重一直在漂、但模型行为很早就定型了**

人们常默认权重还在变就代表模型还在学，但这未必成立。本文对 Pythia 系列（410M–6.9B，7 个随机种子）分别在权重空间和对数似然空间测扩散指数：权重空间呈标准布朗运动（扩散指数 $c_w \approx 1$），对数似然空间却呈强烈的亚扩散（$c_q \approx 0.2$）。两个指数的反差直接说明，模型的输出分布在训练早期就趋于稳定，此后尽管权重持续漂移，行为层面几乎不再移动。这把"权重收敛"和"行为收敛"明确剥离开——判断模型是否真正学完，该看输出分布而非权重变化。

**3. Hölder 正则性与几何折叠：从理论上解释权重的大变化为何只换来行为的小变化**

亚扩散现象需要一个机制层面的解释。本文把从权重到对数似然的映射 $f: W \mapsto q(W)$ 的有效 Hölder 指数算成 $\alpha = c_q/c_w \approx 0.2$，远低于 Lipschitz 连续性的 $\alpha = 1$，意味着这个映射有强烈的"折叠"效应：由于隐藏单元的置换对称性，大量不同的权重配置会被压到相同或相近的输出分布上。配套的有效分形维度也佐证了这点——权重侧 $D_w \approx 2$，对数似然侧却膨胀到 $D_q \approx 10$。正是这种几何折叠，让训练后期权重仍在游走、模型行为却基本钉死。
