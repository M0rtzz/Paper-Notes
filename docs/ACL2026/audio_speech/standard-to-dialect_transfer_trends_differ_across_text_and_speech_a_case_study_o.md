...
1. **三路径迁移对照**:
	...
	- 功能：把文本、语音、ASR级联放在同一个标准语到方言迁移框架下比较。
	- 核心思路：所有系统都只用标准德语训练或调参，在测试时分别面对标准德语与方言输入。这样可以把“输入模态差异”和“标准语到方言差异”拆开观察。
	- 设计动机：如果只比较文本模型，结论会被非标准拼写主导；如果只比较 ASR，结论又会混入转写错误。三路径设计能看到不同模态如何改变迁移趋势。

2. **平行或可比的方言评测数据**:

	- 功能：尽量保证性能差异来自语言变体与模态，而不是标签集或内容分布不同。
	- 核心思路：意图分类中，作者将 MASSIVE 标签映射到 xSID 使用的 10 个意图标签，得到 2.5k/459/361 的训练、开发、测试实例，并使用 412 条 xSID 测试实例。主题分类中，SwissDial 被整理为 10 个主题，划分为 1.5k/194/396。
	- 设计动机：方言资源通常碎片化，任务和标签不统一会让跨模态结论很难解释。作者通过标签对齐和并行测试集降低了这种混杂。

3. **模型族和 ASR 行为的细粒度比较**:

	- 功能：观察结论是否只由某个模型规模或某个 ASR 系统驱动。
	- 核心思路：文本模型包括 mBERT、mDeBERTa、XLM-R base/large；语音模型包括 mHuBERT、XLS-R、MMS、Whisper 各尺寸；级联系统还测试额外 ASR 模型。作者同时记录 ASR 错误率与分类性能差异的相关性。
	- 设计动机：ASR 对方言的一个重要现象是“规范化”：转写可能偏向标准德语。这个行为有时会帮助文本分类，有时会制造无意义输出，因此需要和分类结果一起分析。
