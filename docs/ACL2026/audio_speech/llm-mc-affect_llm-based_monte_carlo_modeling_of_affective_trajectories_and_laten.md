...
得到教师和学生两条标准化轨迹后，方法计算不同对话滞后 $L$ 下的归一化互相关 $R_{TS}(L)$，并选取 $L^*=\arg\max_L |R_{TS}(L)|$ 作为主导时滞。如果 $L^*>0$，表示教师情感领先学生；如果 $L^*<0$，表示学生情感领先教师。同时，方法对每条轨迹做线性回归，用斜率 $eta_T$ 和 $eta_S$ 概括长期情感趋势。

### 关键设计
1. **随机解码作为情感分布采样**:

	- 功能：把一句话的情感判断从单次确定性输出扩展为一组样本。
	- 核心思路：在非零 temperature 下对同一对话上下文重复推理 $K=20$ 次，得到 $\{\hat{s}_{t,k}\}_{k=1}^K$，用样本均值表示中心倾向，用样本方差表示潜在歧义。
	- 设计动机：传统单点情感分数会把歧义平均掉；这里保留方差，能区分“模型很确定地判断为中性”和“多个合理情感解释相互竞争”。

2. **符号稳定的情感评分与极性映射**:

	- 功能：降低 LLM 在正负情感评分时的符号混乱，并把结果变成标准情感坐标。
	- 核心思路：先要求模型在 $0$ 到 $5$ 上打分，$2.5$ 为中性，越接近 $0$ 越积极，越接近 $5$ 越消极；随后用 $\tilde{s}_t=1-2(s_t/5)$ 映射到 $[-1,1]$，方差按 $(2/5)^2$ 缩放。
	- 设计动机：非负分数更容易被 LLM 稳定执行，而标准化后的正负极性又方便后续与情感计算文献对齐。

3. **互相关与斜率联合解释互动模式**:

	- 功能：把底层情感轨迹转成“有效支架”“共同疲劳”“动态补偿”等互动解释。
	- 核心思路：互相关给出双方情感变化的相位关系，斜率给出长期改善或恶化方向；论文用 $L^*$、$R_{TS}(L^*)$ 的符号以及 $eta_T,eta_S$ 的正负组合定义师生互动类型。
	- 设计动机：只看一个人的情绪高低无法说明互动关系，联合时滞和趋势才能解释谁在引导情绪，以及这种引导是正向还是负向。
