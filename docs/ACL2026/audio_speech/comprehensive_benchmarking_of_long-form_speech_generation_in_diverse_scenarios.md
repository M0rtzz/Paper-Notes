...
短句 TTS 测试集只能反映“一句话清不清楚”，根本暴露不了分钟级生成里逐渐累积的音色漂移、声场不稳和表达塌缩。SwanBench-Speech 因此把长文本语音质量拆成三条挑战轴——Acoustics（音色、混响、保真）、Semantics（内容准确、韵律自然）、Expressiveness（情绪丰富、表达层次）——再把它们落到 17 个真实下游场景上：声学相关的客服、播客、闲聊、辩论、有声书、访谈；语义密集的课程、科普、演示、研讨会、新闻；表达性强的戏剧、脱口秀、主持、演讲、直播、体育解说。按场景拆开后，“模型在哪类应用掉点”就从一个模糊的总分变成可定位的诊断结果，分钟级生成里的累积错误也终于有了落点。
