...
1. **键盘布局相邻 + 10 指左右手约束的四类 typo 操作**：

    - 功能：把"换皮版"的四种字符扰动改造成与真实打字物理对应的操作。
    - 核心思路：Replacement 只允许把字符替换为同一语言键盘上相邻的键；Insertion 在正确字符后插入一个相邻键，模拟"同时按两键"；Deletion 随机删掉一个字符；Transposition 只允许在"分属左右手"的两个相邻字符间换位——这一约束直接来自"10 指打字法 5TGB 归左手、6YHN 归右手"的经验观察 (Logan et al., 2016)。
    - 设计动机：人类 typo 的本质来自手指物理动作，单纯按字符相似度替换会生成"看起来像但人不会犯"的错误。键盘约束让 MulTypo 在 6/7 种语言上的"自然度"显著优于 naive baseline（Table 2，多语言 p<0.001）。

2. **长度感知的词采样 + 位置感知的字符采样**：

    - 功能：决定"哪些词更容易被打错、词内哪个位置更容易被打错"。
    - 核心思路：每个词的采样概率正比于 $\frac{\sqrt{|w|}}{\sum_w \sqrt{|w|}}$，长词被采到的概率高，但用 sqrt 而非线性避免对单个长词的过度集中；词内位置则按 Lisbach & Meyer (2013) 的经验分布——errors 更易出现在词的中后段，赋予非均匀概率。
    - 设计动机：心理语言学早就发现 typo 在长词中更密集 (Peterson 1986, Kukich 1992)，把这一先验直接编码到采样器里，等价于免费拿到一份"人类 typo 分布先验"，比均匀随机更真实。

3. **数字与特殊符号忽略集 (ignoring string sets)**：

    - 功能：屏蔽掉一切会污染评测语义的 token。
    - 核心思路：为每种语言维护一个集合，包含数字（"1", "2", "3"）、数字词形（"three", "hundred", "million"）以及标点 / Enter / 修饰键，凡是匹配或包含这些子串的词在 typo 阶段直接跳过。
    - 设计动机：在数学推理任务（如 MGSM）中，把 "500" 打成 "5O0" 实际是修改了答案而非添加噪声，会让评测变成"看模型能不能猜出原数字"。这一设计让评测的是 robustness 本身，而非数字识别。
