...
1. **成对差值监督**:
	...
	- 功能：把 MT 质量估计直接建模成两个候选之间的相对质量差。
	- 核心思路：对源句 $s$ 和候选 $mt_a, mt_b$，PEAR 预测 $\hat{\Delta}_{ab}=f_\theta(s,mt_a,mt_b)$；监督目标由人工段级分数构造为 $\Delta^*_{ab}=s_h(s,mt_a)-s_h(s,mt_b)$。
	- 设计动机：系统比较、候选排序和 MBR utility 本来都是相对任务。直接学习相对差值可以保留平局附近的细粒度强弱，而不是压成二分类偏好。

2. **源感知成对编码与共享候选头**:

	- 功能：在同一上下文中比较两个译文，并避免两个候选使用不同参数导致标尺不一致。
	- 核心思路：输入序列为 $[BOS]\ s\ [SEP]\ mt_a\ [SEP]\ mt_b\ [EOS]$。模型对 source、candidate A、candidate B 分别做 masked mean pooling，得到 $h_{src},h_a,h_b$；每个候选构造 $[h_k;h_k\odot h_{src};|h_k-h_{src}|]$，经过共享投影和 FFN 得到 $u_a,u_b$，再用 $z=u_a-u_b$ 形成比较 logit。
	- 设计动机：共享候选头让两个候选处在同一内部标尺上，显式相减则把输出限制在“比较”而不是“绝对质量分”。

3. **反对称正则与多种推理模式**:

	- 功能：让候选顺序翻转时预测符号也翻转，并降低成对推理的计算成本。
	- 核心思路：训练损失为 Huber 差值回归加顺序翻转项 $\mathcal{L}=\mathcal{L}_{diff}+\lambda_{flip}\mathcal{L}_{flip}$，其中 $\mathcal{L}_{flip}=(\hat{\Delta}_{ab}+\hat{\Delta}_{ba})^2$。推理时可以只跑单顺序，也可以用 PEARboth 计算 $\frac{1}{2}(\hat{\Delta}_{ab}-\hat{\Delta}_{ba})$；有参考译文时还可用 PEARref 把参考作为固定 anchor。
	- 设计动机：比较函数理想上应满足 $f(s,a,b)=-f(s,b,a)$。反对称性还能支持 MBR 中只计算 $N\times N$ utility 矩阵的一半，再用符号翻转补全。
