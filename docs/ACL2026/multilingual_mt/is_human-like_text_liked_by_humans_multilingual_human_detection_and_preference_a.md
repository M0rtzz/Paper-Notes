...
1. **四种检测协议（I/II/III/IV）的分层设计**:

    - 功能：把"信息量"和"任务难度"显式拆开，控制"是否有配对参考"这一变量对人评准确率的影响。
    - 核心思路：设定 I. Single-Binary（输入 1 段，判 Y/N 是否人写）、II. Pair-Binary（输入一对 hwt+mgt，二选一）、III. Triplet-Three-Class（hwt + 2 个不同 LLM 的 mgt，三选一）、IV. Pair-Four-Class（一对文本可能各自是 hwt 或 mgt，输出 A/B/none/both）。理论上难度排序应为 I > IV > III > II，因为 I 没有可比对的参考，II 二选一信息量最大。
    - 设计动机：以往工作各家用各家协议，导致"60% 准确率"和"90% 准确率"无法直接比较；通过同一批标注者在同一数据集跨协议对比（如阿拉伯 Tweet 在 I 下 50.1% vs. 在 II 下 92.7%），可干净地分离出"配对参考能带来约 +20%"这一系统性效应。

2. **改写 prompt 的五维差异化模板**:

    - 功能：把"哪些维度上机器和人不一样"翻译成一组可执行的生成指令，作为对照实验来探测 prompting 的极限。
    - 核心思路：标注者先在第一轮总结出 5 类系统性差异——具体性（concreteness）、文化/宗教 nuance、长度/结构/情感多样性、Markdown formatting、混语；然后在 prompt 里显式要求"加入具体数字/人名/地名/URL"、"避免 bullet 与 Markdown"、"长度风格风格情感随机化"、"使用本族语母语写作避免英文混入"，对同一条 hwt 重新生成 mgt。
    - 设计动机：直接测 LLM 是否"知道自己哪里不像人"。如果 prompting 能填上 gap，说明缺陷是"对齐表层"问题；如果填不上（特别是文化 nuance 和多样性），说明缺陷已经深入到训练数据和优化目标，需要更根本的解法。

3. **检测能力 × 偏好的交叉分析（human-like vs. liked-by-human）**:

    - 功能：检验"让模型更像人"是否等价于"让人更喜欢模型输出"这一隐含前提。
    - 核心思路：在同一组样本上同时收集"哪个是人写"（检测）和"你更喜欢哪个"（偏好）两个标签，并按检测准确率分层（高/中/低）汇总偏好分布。形式化看，记某条样本上标注者的检测正确率为 $p$、偏好人文本的概率为 $q$，作者发现 $q$ 与 $p$ 正相关但远非 1：在 $p \approx 1$ 时 $q \approx 0.6$，在 $p \approx 0.5$ 时 $q$ 反而 $< 0.5$，说明"看不出谁是人写时人会倾向机器"。
    - 设计动机：把"是否像人"和"是否被喜欢"两条独立轴放在同一实验里量化，直接挑战 RLHF/对齐文献中"模仿人类偏好 = 模仿人类写作"的常见混淆，得到一个"liked-by-human"是和"human-like"分离的优化目标的实证证据。
...
- [\[ACL 2025\] Context Augmented Token-Level Post-Editing for Human Interpreting](../../ACL2025/multilingual_mt/context_augmented_token-level_post-editing_for_human_interpreting.md)
