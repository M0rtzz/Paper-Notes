...
**1. 五大动机分类体系：按"为什么用音译"而非方法或任务来组织文献**

以往综述大多按方法或下游任务切分，结果是同一种音译技术散落在各处，看不出它在领域里角色的演变。本文改以动机为轴，归出五类：命名实体与 OOV 处理（最早期的应用，把外来人名地名转写过来）、代码混合文本处理（同一句里多脚本混用时先统一书写系统）、跨脚本语言相似性利用（印地语/乌尔都语这种亲缘语言却用不同脚本书写时，转写后能对齐）、英语中心迁移（借英语为中心的预训练模型做跳板）、统一预处理（把多语言统一成一种脚本以压缩词表）。这五类大致也是一条时间线，串起来正好揭示了音译从"哪里不通就补哪里"的修补手段，逐渐变成预处理流水线里的系统设计选择。

**2. 六种整合方式的比较：从"怎么把音译塞进流水线"给出可查表的选型**

光知道为什么用还不够，实践者更关心怎么接进现有系统。本文把整合方式排成从激进到保守的六档：完全替换为音译语料、音译增强（保留原文再追加音译版本）、词汇表增强、嵌入层拼接/融合、提示策略（在 ICL 上下文里加音译）、以及多编码器/多集成架构。它们在实现复杂度、效果上限和适用条件上差异极大——直接整体替换往往最简单也可能最有效，但同时风险最高（表意文字一替换就丢语义），而音译增强这类"原文+音译"双语料的做法更稳，是更安全的默认选项。

**3. LLM 时代的重新审视：反驳"大模型已经让音译多余"的直觉**

一个自然的怀疑是：大规模预训练之后，音译是不是已经没必要了？本文给出三条反例说明瓶颈仍在。其一，tokenizer 对低资源脚本的覆盖依旧很差，一个词常被切成 10+ 个 token，碎片化严重；其二，把低资源语言罗马化能把 token 数压下 2–5 倍，直接改善推理效率和成本；其三，对那些训练数据极少的脚本，音译至今仍是最实用的接入方案。结论是别陷入"LLM 解决一切"的误解——tokenization 瓶颈在多语言 LLM 里并没有消失，音译反而获得了新的、面向效率的存在理由。
