...
description: >-
  [ACL2026][多语言/翻译][光泽无关手语翻译] 这篇论文发现手语翻译中随机 in-batch 负样本经常是不可靠或语义冲突的监督信号，因此用参考模型的相似度轨迹筛选更有信息量的负样本，并通过从易到难的课程式对比学习提升 gloss-free 手语翻译质量。
tags:
  - "ACL2026"
  - "多语言/翻译"
  - "光泽无关手语翻译"
...
1. **基于轨迹的负样本打分**:
    ...
    - 功能：把负样本从“随机碰到什么就学什么”改成“根据训练动态主动挑选”。
    - 核心思路：对视频 $V_i$ 和非匹配文本 $T_j$，用参考模型早期和末期检查点的相似度差 $\delta_{i,j}=\hat{s}^{K}(V_i,T_j)-\hat{s}^{0}(V_i,T_j)$ 表示该负对的轨迹变化。一个 batch 的分数是其中所有非对角视频-文本负对变化量之和。直觉上，H→L 类负样本说明模型能把它们推开，L→H 类负样本说明它们越来越难区分，H→H 则可能是语义近邻或伪负样本。
    - 设计动机：标准对比学习只看当前 batch 中的瞬时相似度，无法分辨“真的难”和“其实不该推远”。轨迹信号把训练历史引入样本选择，能减少被噪声负样本主导的更新。

2. **课程式 Pair Selection**:

    - 功能：在每个 epoch 构造符合当前训练阶段的 mini-batch。
    - 核心思路：算法先随机选一个正例作为 batch 种子，然后对候选样本计算加入当前 batch 后带来的增量分数 $\Delta(s_u;\mathcal{C})$，将候选按分数排序，并按课程比例 $\alpha=e/E$ 选取对应分位的样本。训练早期偏向更容易分开的负样本，训练后期逐渐转向相似度更高或更难区分的负样本。
    - 设计动机：如果一开始就只喂最难负样本，模型可能被伪负样本和语义近邻扰乱；如果一直只喂容易样本，对齐能力又会停在表层。课程调度让模型先学稳定边界，再处理细粒度差异。

3. **选择性对比训练与翻译微调解耦**:

    - 功能：先学习跨模态对齐，再专注翻译生成。
    - 核心思路：选择性对比阶段计算视频到文本和文本到视频两个方向的 CLIP 式损失，并保留权重为 1.0 的翻译损失以避免表示只服务于检索。随后在 SLT Fine-tuning 阶段移除辅助文本编码器和对齐模块，解码器只根据视频表示生成目标句子。
    - 设计动机：论文的附录显示，直接把普通对比目标并入端到端翻译训练可能严重掉点；将对比学习作为预训练式对齐阶段，再进行翻译微调，更符合视频-文本表示学习和句子生成的不同目标。
