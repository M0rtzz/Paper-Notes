...
1. **把多语迁移与文化评测放入同一证据链**:
	...
	- 功能：连接过去相对分离的两类文献：一类研究为什么某些语言迁移效果好，另一类研究模型是否理解文化语境。
	- 核心思路：作者把资源覆盖、预训练分布、词汇重叠、script、tokenizer 行为等技术因素视为“语言能力”的解释变量，同时把文化敏感子集、原生作者题目、价值观 probe、多模态文化线索和交互式任务作为“文化能力”的证据来源。
	- 设计动机：这样可以避免一个常见误判：模型在多语 benchmark 上提高了，并不代表它在本地文化场景中可用。

2. **分层评估协议**:

	- 功能：把文化能力评估拆成多个可报告、可审计的层，而不是压成单一 culture score。
	- 核心思路：协议包含 representation audit、elicitation diversity、ecological validity、community validation 和 adaptation reporting。也就是说，论文应报告谁写了数据、是否翻译、涵盖哪些语言变体和群体；用选择题、开放生成、pairwise judgment 和错误分析混合 elicitation；在对话、web agent、图像、视频、区域任务等切片上评估；让 native speaker 或社区参与验证；并公开文化调优数据的来源和目标人群。
	- 设计动机：文化错误常出现在 benchmark 没覆盖的交互方式、模态或社区细分里。分层协议能让研究声明更窄但更可信。

3. **从一次性对齐转向持续本地化**:

	- 功能：把文化对齐理解为长期维护的本地化基础设施，而不是训练完一次就结束的模型属性。
	- 核心思路：论文建议文化资源要版本化、刷新、重标注，并保留上下文元数据。文化知识和社会规范会随公共事件、社区实践和可见性变化而改变，因此 benchmark 也应被视为 living resources。
	- 设计动机：如果文化 benchmark 固定在一次收集时刻，它可能把某个历史切片、主流群体或国家级平均值固化成默认标准，反而压制少数群体和内部变化。
