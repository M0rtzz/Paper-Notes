...
### 关键设计
1. **12 类 prompt 变体作为控制变量**:

	- 功能：系统分析 prompt 中定义、推理指令和样例对分类结果的影响。
	- 核心思路：Prompts 1-2 几乎不提供上下文，Prompts 3-4 给出短任务定义，Prompts 5-6 明确定义 polarized / non-polarized，Prompts 7-8 要求逐步分析后再给答案，Prompts 9-12 加入 in-context examples。
	- 设计动机：极化判断高度依赖边界定义，prompt 的术语清晰度会直接改变模型何时预测正类。

2. **统一英文 prompt 跨语言推理**:

	- 功能：用同一套 prompt 覆盖 22 种语言，测试 multilingual LLM 的跨语言泛化能力。
	- 核心思路：prompt 用英文书写，不为每种语言定制模板；模型依赖自身多语种能力理解输入文本并输出二分类或多标签集合。
	- 设计动机：语言特定 prompt 成本较高，且 SemEval 系统需要快速覆盖大量语言。统一 prompt 便于比较语言差异，也能暴露 prompt-only 方法的跨语言弱点。

3. **保守决策式最终 prompt**:

	- 功能：降低粗粒度二分类中的 false positives。
	- 核心思路：最终 prompt 倾向于只有在文本有明确、无歧义证据时才预测极化；这在 Subtask 1 中提升宏 F1，但在 Subtask 2/3 中抑制隐含目标和微妙表现的召回。
	- 设计动机：官方宏 F1 对误报和漏报都敏感，粗粒度任务中保守边界较稳；但该策略也构成后续性能退化的主要原因。
