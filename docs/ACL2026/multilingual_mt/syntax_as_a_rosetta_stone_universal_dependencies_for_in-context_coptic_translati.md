...
这篇论文要回答的问题很聚焦：在纯 ICL（不训练）的设定下，UD 句法信息能不能给低资源的科普特语→英语翻译带来词典之外的互补增益。围绕这个问题，作者设计了四个可以塞进提示的信息组件——LEX（双语词典映射）、DEP（UD 依赖关系的自然语言动词化）、CON（基于错误分析的 26 条构式规则）、CoNLLU（原始 UD 解析输出），然后系统比较单组件和组合设置，以 BERTScore F1 为主指标，看"词典 + 句法"能否突破词典单独使用的天花板。
