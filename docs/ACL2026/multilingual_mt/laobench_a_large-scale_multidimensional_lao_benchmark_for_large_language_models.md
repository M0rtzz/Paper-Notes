...
1. **开源 + 黑盒双子集对抗数据污染**：
    ...
    - 功能：Lao-7k 开源 MCQ 供复现研究、Lao-10k 闭源 MCQ 由受控服务接收提交、只返回 aggregate 分数。
    - 核心思路：作者明确把 Lao-10k 永远不放出（test items 不公开），评测者要么提交 item ID → answer 字典、要么提供 inference API endpoint 由服务方按 standardized prompt 跑，最终只回 overall + subdomain 准确率，配上 submission rate limit 阻击 leaderboard overfit。开源子集还做了 web overlap 检索 + n-gram overlap 检查（6.2% 候选样本有疑似重叠，多数为常识性陈述，非直接泄露）。
    - 设计动机：低资源语言一旦放出 benchmark，几乎一定会被新模型的预训练语料"吃掉"；黑盒服务是低资源 benchmark 长寿命的唯一现实路径。

2. **专家 + Agent 双 loop 构建管线**：

    - 功能：兼顾"母语专家保真"与"规模化产能"，让 17k+ 样本既高质又能在合理周期内完成。
    - 核心思路：55 位贡献者按角色分工（25 位领域专家写题、11 位翻译专家做双语对齐、10 位资深 reviewer 终审、9 位 NLP 数据 curator）；每题至少 2 位独立专家审，分歧由 senior reviewer adjudicate；Agent 端跑 duplicate detection（字符 n-gram + 嵌入检索）、semantic consistency（验证唯一正确答案）、context independence（删依赖外部信息的题）、sensitivity screening（先 Agent 筛再专家复核）；500 题抽样 inter-annotator agreement 测出 Fleiss $\kappa{=}0.87$ 大致达到 substantial agreement。
    - 设计动机：纯人工成本爆炸、纯 Agent 翻车率高；按 Hendrycks-style "hybrid pipeline"思路，把可机械化的步骤外包给 Agent，把价值判断和文化对错留给人。

3. **Lao-aware 翻译评测 + Arena 双 judge 开放式评测**：

    - 功能：解决"老挝文无词边界 → BLEU 失真"与"MCQ 看不到生成质量"两个具体痛点。
    - 核心思路：(i) 翻译评测用 SacreBLEU + Lao-aware LaoNLP v0.7 词切分 + 额外报 chrF++（字符 n-gram，对分词不敏感）；(ii) Lao-500 走 Arena pairwise：以 GPT-5-High 为固定 baseline $B$，给每个 prompt $x_i$ 让候选 $M$ 与 $B$ 各产一个回答 $y_i^M, y_i^B$，judge model（Gemini-2.5-Pro + Qwen3-Max 双 judge）按 correctness / completeness / reasoning / clarity / Lao fluency 判 winner，输出严格 JSON 防 leak；为消位置偏置，每对评两次（A/B 互换）取平均；ties 给 0.5；用 bootstrap 重采样 prompt 给 95% CI；最终 $S(M)=\frac{1}{|\mathcal{J}|}\sum_{J}\frac{1}{N}\sum_i w_i^J(M)$ 双 judge 平均。
    - 设计动机：Lao 翻译的 BLEU 单跑出来根本不可解释；Arena 模式则把"生成质量"以人 / LLM 都能判的方式标准化，bootstrap CI 让不同模型之间的差异是否显著一目了然。
