...
RST 传统做法在 EDU 级别把每句切成数个最小篇章单元，长文档下计算开销大、语义太碎。本文把解析上升到句子级：先做**粒度适配**——把现有 EDU 数据集做两步转换，句子内的 EDU 合并成统一句子单元，句间关系则用原 EDU 树的最低公共祖先（LCA）推断。再做**语言适配**——用 GPT-4o 把 RST-DT 训练集句子级翻译到中文，与原始英文语料合并训练出一个统一解析器 $f_{discourse}$，从而无需任何中文 RST 标注就能跨语言迁移，对低资源语言友好。
