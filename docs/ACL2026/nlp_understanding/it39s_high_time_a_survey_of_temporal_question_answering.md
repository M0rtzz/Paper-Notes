...
很多 TQA 方法换个语料就失灵，根源在于没区分语料的时间结构。综述把语料分成两类：共时语料（如某个维基百科快照）里事件的时间关系得从文档内部结构去推断；历时语料（如多年新闻档案）的时间线则直接来自文档集合的时间分布。关键差异在相对时间表达——"今天""下周"这类词必须锚定到文档的发布日期才能正确理解，而锚点在两类语料里来源完全不同。这个区分解释了为什么同一种方法在一类语料上有效、换到另一类就崩。

**2. 数据集与基准分类：给散乱的 TQA 数据集一个统一坐标系**

此前不同 TQA 数据集缺乏可比性，没法系统对照。综述按知识来源（新闻/维基百科/Freebase）、创建方式（众包/自动生成）、答案类型（抽取式/自由形式）、时间范围以及是否支持多跳推理等维度，把现有数据集统一归类，并标出 ComplexTempQA（1 亿+ 问题）、ArchivalQA（53.2 万条、跨 20 年新闻）等代表性资源在各维度上的位置。有了这套坐标，读者才能看清现有基准覆盖了哪些时间能力、又留下了哪些空白（如几乎没有面向未来的时间 QA）。

**3. LLM 时代的方法谱系：强大但仍漏时间**

LLM 被广泛用于 TQA，但它的时间能力边界没被讲清。综述把这一时期的进展梳理成三条线：时间语言建模——在带时间戳的文本上预训练，把时间感知注入模型（如 TempLM、TEMPLAMA）；时间感知 RAG——在检索阶段引入时间过滤和重排序；持续时间适应——靠持续预训练跟上知识更新。同时点明 LLM 的两大软肋：知识衰退（对训练截止日期之后的事件了解有限）和时间推理能力不足，正是这两点让 RAG 成为当前补时间知识的主力，却也仍不成熟。
