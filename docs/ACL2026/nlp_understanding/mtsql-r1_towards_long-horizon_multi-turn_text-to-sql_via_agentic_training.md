...
1. **长程 MDP 与双环境反馈**:
	...
	- 功能：把多轮 SQL 生成拆成可执行、可验证、可修正的一系列动作，而不是一次性输出 SQL。
	- 核心思路：环境由数据库和长期记忆两部分组成。数据库负责返回执行结果、空结果或错误；长期记忆保存历史问句、历史 SQL、解析出的约束和实体，用于检查当前 SQL 是否遗漏或违背前文条件。动作转移遵循固定顺序：初始 PROPOSE，随后 EXECUTE，再 E-VERIFY；若执行通过则 M-VERIFY，若任一验证失败则 SELF-CORRECT，全部通过后 FINALIZE。
	- 设计动机：数据库能暴露语法和执行层面的错误，记忆能暴露多轮一致性错误。两者结合后，模型不仅知道“错了”，还知道错误属于执行问题还是上下文一致性问题，从而更容易做针对性修正。

2. **自教式 Warm-Start SFT 与难度感知轨迹筛选**:

	- 功能：在 RL 前先让模型学会稳定地遵循 agent 行为格式和工具调用流程。
	- 核心思路：每轮对每个训练样本采样 20 条轨迹，保留最终 SQL 同时通过 EX 和 EM 的轨迹。简单样本或 20 次全对的样本只保留少量短轨迹，避免把简单问题人为拉长；困难样本优先保留包含更多交互的长轨迹，并用 Qwen3-Embedding 聚类后选代表轨迹，保证轨迹多样性。训练后把已获得高质量轨迹的样本从下一轮待探索集合中移除，继续迭代扩大覆盖面。
	- 设计动机：直接用 gold SQL 合成轨迹会显得太“完美”，缺少真实的执行失败和修正过程；只用 base model 采样又会覆盖不足。自教式迭代在两者之间折中，让模型通过自己产生的成功经验逐步学会长程推理。

3. **Outcome + Process 的多层级奖励**:

	- 功能：缓解只看最终 SQL 时奖励过稀疏的问题，让模型在中间动作上也能得到学习信号。
	- 核心思路：最终奖励包含 EX 和 EM。过程奖励则按动作类型设计：PROPOSE 和 SELF-CORRECT 用 SELECT、WHERE、JOIN、GROUP、ORDER 等 SQL clause 的平均 F1 衡量候选 SQL 与 gold SQL 的接近程度；E-VERIFY 根据执行结果和模型给出的 pass/fail 判断是否匹配来给分；M-VERIFY 根据候选 SQL 与 gold SQL 的 clause F1 以及记忆验证结论给分。整体奖励是 outcome reward 与 process reward 的加权和，权重在小验证集上网格搜索。
	- 设计动机：难题和深轮次问题经常需要多次试错，只有最终 0/1 奖励很难告诉模型哪一步变好了。过程奖励把“提出更接近的 SQL”“正确识别执行错误”“正确发现记忆冲突”都变成可优化目标。
