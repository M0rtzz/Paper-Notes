...
flowchart TD
    A["量化现象<br/>CQO vs QOC 平均差 14.72%"] --> B["提出三个竞争假说"]
    B --> C["排除假说一——训练数据偏差<br/>base/instruct 对比 + few-shot"]
    B --> E["排除假说二——选项回忆失败<br/>逐字回忆精确匹配"]
    B --> G["验证假说三——因果注意力机制<br/>架构对比 + 上下文移除 + 归因"]
    C -->|"差距不变, 5-shot 仅 +3.1%"| D["排除"]
    E -->|"QOC 回忆率不低反高"| F["排除"]
    G -->|"双向注意力差距消失, I=0"| H["成立"]
    D --> H
    F --> H
    H --> I["调控因素与干预实验<br/>剪枝 / 激活补丁 / QOCO / CoT"]
```
