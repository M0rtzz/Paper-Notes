...
1. **Top-1 gap token score**:
	...
	- 功能：把“模型是否自信地偏离真实 token”转成 token 级检测信号。
	- 核心思路：对每个位置 $t$，计算 $g_t=(\log p(x_t|x_{<t})-\max_{v\in V}\log p(v|x_{<t}))/\sigma_t$。这个值恒不大于 0；越接近 0，真实 token 越接近 top-1；越负，说明模型越自信地偏向其他 token。
	- 设计动机：Min-K%++ 用真实 token 相对均值的偏离衡量异常，但两个分布的均值偏离可能相同，一个是“整体平坦不确定”，另一个是“top-1 非常尖锐但真实 token 落后”。Gap-K% 用 top-1 直接刻画后一种 confident error。

2. **Sequential smoothing**:

	- 功能：把孤立 token 的噪声变成连续片段的 membership 证据。
	- 核心思路：用窗口 $w$ 对 gap 序列做平均，$\bar g_t^{(w)}=\frac{1}{w}\sum_{i=0}^{w-1}g_{t+i}$。论文对 LLaMA 系列设置 $w=6$，其他模型设置 $w=3$。
	- 设计动机：LLM 的记忆通常不是单个 token 级别，而是连续短语或句段。若相邻 token 都存在大 gap，更像非训练文本；若只是单个 token 异常，可能只是自然语言的偶然波动。

3. **Bottom-k% aggregation**:

	- 功能：聚焦最能反驳 membership 的局部片段，而不是平均掉强信号。
	- 核心思路：设 $\tilde{\mathcal{I}}_k(\mathbf{x})$ 是平滑 gap 中最低的 $k\%$ 位置，最终分数为 $\text{Gap-K}(\mathbf{x})=\frac{1}{|\tilde{\mathcal{I}}_k(\mathbf{x})|}\sum_{t\in\tilde{\mathcal{I}}_k(\mathbf{x})}\bar g_t^{(w)}$。实验默认与 Min-K%++ 公平对齐，使用 $k=20\%$，并分析 $5\%-50\%$ 范围。
	- 设计动机：训练数据检测依赖的是少数最异常片段。直接平均全序列会被大量普通 token 稀释；bottom-k 聚合保留最有判别力的连续低 gap 区域。
