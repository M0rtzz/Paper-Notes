...
### 整体框架
SLIM 包含两个阶段。水印阶段选择极少量目标序列，将其分成前缀和后续内容，然后构造若干语义接近但续写方向分歧的变体，并把这些变体混入训练数据。验证阶段只使用黑盒生成访问，对目标前缀和局部参考前缀分别采样多次续写，再比较两组续写开头之间的语义相似度分布，若目标前缀附近出现显著更强的不稳定性，就认为存在水印痕迹。
...
1. **低覆盖水印目标**:
    ...
    - 功能：把数据水印从“拥有整批训练集的人”扩展到“只拥有几条数据的个人贡献者”。
    - 核心思路：水印信号不依赖大面积重复注入，而是集中作用在极少数目标序列附近的局部表示区域。论文默认每个水印实例只修改单个目标序列，并在 500K arXiv abstract 语料中模拟信号稀释。
    - 设计动机：现实训练语料来自海量个体，单个所有者无法控制大比例数据；如果方法必须高覆盖，实际维权或授权验证价值会很低。

2. **Latent-Space Confusion Zone**:

    - 功能：让被训练模型在目标前缀附近产生可检测的续写不稳定性。
    - 核心思路：语义相近的前缀会落在潜在空间相邻区域，若训练时把这些相近前缀关联到多个差异较大的合理续写，模型上层生成分布会在这个局部区域形成“混淆区”。推理时同一前缀多次采样，续写开头之间的相似度会异常降低或波动。
    - 设计动机：相比随机字符或虚构知识，局部潜在空间行为不依赖显眼表面模式，更适合绕开常规去重、压缩异常和 embedding 密度清洗。

3. **黑盒统计验证**:

    - 功能：在无法访问训练 loss、模型权重或内部表示的情况下进行归属判断。
    - 核心思路：验证者对目标前缀和局部参考前缀分别收集多次生成，比较两组生成开头的 pairwise semantic similarity 分布，并用统计检验得到验证分数。若有公开或可获得的基座模型，可做 reference model-based 比较；若没有，则用非水印样本构造 reference model-free 的零假设分布。
    - 设计动机：商业模型通常只开放 API，严格黑盒验证比依赖 loss/perplexity 或内部 logits 更现实。

### 损失函数 / 训练策略
SLIM 本身不是新的模型训练目标，而是一种数据侧水印构造与验证框架。实验主要在 Gemma-3-4B 的微调阶段和 Pythia-1.4B 的预训练阶段评估，训练语料来自 gfissore/arxiv-abstracts-2021 的前 500,000 条序列，约 100M tokens。模型训练持续两个 epoch，以降低过拟合影响；生成验证使用随机解码来观察局部不稳定性。
