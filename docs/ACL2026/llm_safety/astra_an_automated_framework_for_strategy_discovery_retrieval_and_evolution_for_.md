...
### 整体框架
ASTRA 由三大模块组成：**Attack Designer**（生成 prompt，有 Strategy-Agnostic / Strategy-Guided 两模式）、**Strategy Extractor**（让 LLM 反思一次交互、按打分蒸馏出结构化策略 JSON）、**Strategy Storage & Retrieval**（用 text embedding 对原始有害 query 编码作索引，按 cosine 相似度 top-$k$ 检索）。攻击预算 $N=10$、retrieval $k=9$（每类 top-3）、Attacker / Extractor 用 GLM-4.5，Judge 用 GPT-4o。

### 关键设计

1. **闭环 "attack–evaluate–distill–reuse"**:

    - 功能：把每次 (prompt, response, score) 都转成可检索的结构化策略，形成"打怪→升级→换装备"循环。
    - 核心思路：目标函数从 $\sigma^* = \arg\max_\sigma J_\theta(q, T_\theta(A_\theta(q, S(\sigma))))$ 出发，Judge $J_\theta$ 输出 $s \in [1,10]$ 而非 0/1。整个生命周期：retrieve 策略 → 生成 prompt → 目标 LLM 回答 → Judge 打分 → Extractor 蒸馏策略 → 写入对应库；当 $s_i=10$ 直接 break。
    - 设计动机：连续打分把「near-miss」（如 $s=8$ 的「方向对但被最后一步过滤」）变成有价值的学习信号，这是 PAIR/TAP 的二元 reward 无法捕捉的。

2. **三层动态策略库 + 分类蒸馏（Effective / Promising / Ineffective）**:

    - 功能：让攻击同时享受「模仿成功 + 探索潜在 + 避开陷阱」三种知识。
    - 核心思路：按 Judge 打分路由——$s=10$ 进 **Effective**（直接复用），$5<s<10$ 进 **Promising**（含改进建议），$s\le 5$ 进 **Ineffective**（写成 avoidance guideline）。每条策略都是 JSON：core description + usage guidelines + illustrative example。下一次 retrieval 时 prompt 把三类策略一起塞给 Attacker，让它「学好的、改差的、避坏的」。
    - 设计动机：相比单一「成功池」，三类库分别解决 exploit / explore / pruning，从向量空间到策略空间都对应不同语义；消融显示去掉 Ineffective 库 ASR 也从 71.9% 降到 65.4%，证明「失败也是知识」。

3. **基于原始有害 query 的语义检索**:

    - 功能：把策略以 query 为索引存储，跨不同的有害问题做策略迁移。
    - 核心思路：用 text-embedding-3-small 把原始 $q$ 编码成 $\mathbf{v}_q$ 作为索引，存 $(\mathbf{v}_q, \sigma)$ 对；新问题 $q_\text{new}$ 编码后 cosine 相似度 top-$k$ 命中。retrieval 个数 $k$ 越大 ASR 越高但增益递减，论文取 $k=9$。
    - 设计动机：以「问题语义」（不是 prompt 表面文本）作 key，使得攻击「教师 / 学生 / 化学武器」等不同问题之间能共享有效策略，验证表明 mature library 转到 AdvBench-50 几乎不掉点。
