...
1. **客户端侧安全过滤器**:
    ...
    - 功能：在本地 LoRA 训练前删除被判定为 unsafe 的训练样本，降低有害响应进入联邦聚合的概率。
    - 核心思路：作者采用 Llama Guard 3 作为 `(query, response)` 安全分类器，并在 S-LG20K 上微调它。原始 LG3 在该任务上几乎把样本都判为安全，recall 只有 0.5%，因此必须先适配到 SQuARe 风格数据。
    - 设计动机：FedLLM 的服务器不能集中查看客户端数据，数据清洗只能发生在客户端；安全过滤器不需要客户端训练，只需本地推理，因此比较适合联邦环境。

2. **服务器端轻量 Constitutional AI**:

    - 功能：在全局模型聚合后，用宪法式规则约束模型不要生成 red responses，修复已经进入模型行为层面的安全问题。
    - 核心思路：CAI 让模型根据“不要生成有害回答”等原则自我批判、自我修订，再用修订后的数据继续训练模型。论文没有在每个客户端或每轮进行完整 CAI，而是只在全局模型上执行少量迭代。
    - 设计动机：完整 CAI 每轮一整个 epoch 的代价过高，论文报告 4 张 A100 上一轮约 80 分钟；把训练缩短到 50 iterations 后，每轮约 3.2 分钟，计算时间减少 96%，但仍保留了主要安全收益。

3. **FedLLM 安全评估闭环**:

    - 功能：同时衡量安全性和有用性，避免只让模型变得“更拒答”而忽视回答质量。
    - 核心思路：安全评估使用 AdvBench 和 HHH，helpfulness 使用 MT-Bench；联邦算法同时测试 FedAvg 和 SCAFFOLD。训练集 SQuARe20K 被构造成 6K red + 14K acceptable 的混合，每个客户端约 30% 有害内容。
    - 设计动机：如果只看安全过滤本身，无法判断过滤后的全局模型是否仍有用；如果只看单个联邦算法，也无法说明该方案是不是依赖某个聚合器。
