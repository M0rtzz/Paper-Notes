...
1. **功能保持与抽取预防的联合目标**:
    ...
    - 功能：让 proxy prompt 对正常用户仍然有用，同时降低被抽取内容的语义价值。
    - 核心思路：第一项最小化原 prompt 和 proxy prompt 在代表性查询上的回答差异；第二项让 proxy 在抽取式请求下远离原 prompt 语义、靠近防御者指定的无关目标语义。两项合在一个 embedding-space 优化问题中。
    - 设计动机：单纯 soft prompt 只优化任务效用，可能仍泄露任务意图；加入抽取预防项后，proxy 不只是功能近似，还要在被抽取时语义偏离。

2. **连续空间 proxy 与离散解码损失**:

    - 功能：利用连续表示与离散 token 之间的 gap 增强保护。
    - 核心思路：proxy prompt 是连续 embedding，不一定落在自然语言 token manifold 上。当模型试图把它“说出来”时，需要经过有损的连续到离散映射，抽取文本通常无法保留原功能。
    - 设计动机：传统 prompt 是可读文本，一旦复述就可迁移。连续 prompt 即使产生某些近邻 token，也可能丢失大量任务结构。

3. **语义级泄露指标**:

    - 功能：检测 word-level overlap 看不到的语义重述泄露。
    - 核心思路：除了 Exact-Match 和 Approx-Match，论文提出 Semantic-Match 和 Most-Similar，在句子粒度比较原 prompt 与抽取内容的语义等价程度。Semantic-Match 更关注是否存在语义可替代句，Most-Similar 衡量最接近片段的相似度。
    - 设计动机：安全评估不能只看字符串重叠。一个没有共享 n-gram 的转述也可能泄露真正的任务规则。
