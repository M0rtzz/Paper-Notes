...
1. **安全漂移分解**:
    ...
    - 功能：解释为什么单独约束权重或激活都不够
    - 核心思路：对某层输出 $y=f(Wh)$，微调后漂移可近似为 $\Delta y\approx f'(W_0h_0)\odot(W_0\Delta h+\Delta W h_0)$。其中 $W_0\Delta h$ 是激活变化造成的漂移，$\Delta W h_0$ 是权重变化造成的漂移。若只约束 $\Delta W$，激活仍可偏移；若只约束 $\Delta h$，权重仍可在未见输入上造成漂移
    - 设计动机：这个分解把“安全漂移”从经验现象变成可分析的两项误差上界，为后续双约束设计提供理论依据

2. **基于 SAE 的激活安全约束**:

    - 功能：在微调过程中保留与拒答相关的内部安全特征
    - 核心思路：作者训练 TopK Sparse Autoencoder，将残差流激活分解为稀疏可解释特征。SAE 训练语料约 1 亿 token，其中 30% 来自 OpenWebText2，70% 来自基模型正确拒答的有害提示。微调前记录基模型在有害提示上的安全关键特征 $z^{baseline}$，微调时加入 $L_{safety}=\frac{1}{B}\sum_b\|z_b^{current}-z_b^{baseline}\|^2$，只约束 top safety-critical latents
    - 设计动机：直接约束整条激活向量会妨碍下游任务适配；SAE 让方法只锁住拒答相关的稀疏维度，把安全保持和任务学习尽量解耦

3. **权重安全子空间投影**:

    - 功能：限制权重更新不要改变模型在有害提示上的安全输出
    - 核心思路：对每个 FFN output projection 层收集基模型处理有害提示时的输入矩阵 $X_l$，希望更新满足 $\Delta W_lX_l\approx0$。直接处理 $X_l$ 代价高，因此计算协方差 $C_l=X_lX_l^T$，做 SVD 后保留小特征值对应方向构成 $\hat U_l$，定义投影矩阵 $\Pi_l=\hat U_l\hat U_l^T$。微调时将梯度更新投影为 $\Delta W_l\leftarrow\Delta W_l\Pi_l$
    - 设计动机：小特征值方向对有害提示安全输出影响最小，把更新限制在这些方向上可以保留下游任务学习空间，同时减少对拒答行为的破坏
