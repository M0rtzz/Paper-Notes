...
### 关键设计
1. **三阶段推理分解与 Self-Jailbreak 分类**:

    - 功能：把安全失败定位到推理轨迹中的具体阶段，区分“没识别风险”和“识别后又自我推翻”。
    - 核心思路：每条轨迹被拆成 $d(x)$、$a(x)$、$p(x)$ 三部分，再用 judge 判断风险意识是否存在、最终回答是否安全，以及中间推理是否把早期判断改写成可回答理由。作者进一步把 Self-Jailbreak 归为 Benign Reframing、Warning、Logical Fallacies 三类。
    - 设计动机：如果模型已经能识别风险，那么继续强化风险识别并不是最有效的修复方向。分类让后续干预可以针对“重新包装意图”“以警告代替拒绝”“逻辑绕行”等不同失败机制，而不是一刀切地压制所有长推理。

2. **Safety Recomposition 与 Safety Backtrack 双路径修复**:

    - 功能：为不同训练需求生成安全导向 CoT，同时尽量保留模型原本的推理能力。
    - 核心思路：SafR 直接把危险的 $a(x)$ 和 $p(x)$ 重写为安全版本 $\hat{a}(x)$ 与 $\hat{p}(x)$，适合构造干净、连贯的监督样本；SafB 不删原链，而是追加一个 self-check segment，让模型在看到原推理后进行纠偏，再输出安全响应。
    - 设计动机：SafR 更像“重写错误步骤”，训练信号清晰；SafB 更像“保留现场并学习回溯”，能让模型处理真实生成中已经出现的偏航轨迹。两者互补，使 CoG 不依赖单一修复范式。

3. **选择性 Loss Masking**:

    - 功能：防止模型在 SafB 训练中把原始危险推理也当成应该模仿的目标。
    - 核心思路：SafR 对重组后的安全轨迹做普通 SFT；SafB 则只对 self-check 片段和最终安全回答计算损失，原始危险推理片段只作为上下文。目标可写成 $\mathcal{L}_{SafB}=-\sum_{t\in\mathcal{T}_{sub}}\log \pi_\theta(y_t|x,y_{<t})$，其中 $\mathcal{T}_{sub}$ 只包含自检和最终响应 token。
    - 设计动机：如果对整条序列都监督，模型会同时学习“危险转向”和“后续纠正”，信号冲突很大。masking 把 flawed reasoning 变成反例上下文，而不是模仿目标，这是 SafB 能保持安全性的关键。
