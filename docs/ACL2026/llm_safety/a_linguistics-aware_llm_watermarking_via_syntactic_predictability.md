...
1. **Linguistic Indeterminacy $\lambda(c_t)$——POS n-gram 条件熵作 model-free 调制信号**:

    - 功能：用一个 0–1 的标量量化「在 POS 上下文 $c_t$ 之后，下一个 POS tag 的语法自由度」，作为水印强度的 multiplier。
    - 核心思路：定义条件 Shannon 熵 $H(P(\pi_t \mid c_t)) = -\sum_{\pi'} P(\pi' \mid c_t) \log P(\pi' \mid c_t)$，再用上下文 $c_t$ 后实际出现的不同 tag 数 $K_{c_t}$ 做归一化 $\lambda(c_t) = H(P(\pi_t \mid c_t)) / \log K_{c_t}$。$\lambda \to 1$ 表示「几乎任何 POS 都可能」，$\lambda \to 0$ 表示「下一个 POS 几乎被语法定死」。$k$ 是上下文窗口超参，英文用 $k=2$、中文/韩文用 $k=4$（基于语言类型学）。
    - 设计动机：句法是语言本身的属性，与模型无关；用 POS 条件熵替代 token entropy，把「调制信号」从模型空间搬到语言空间，彻底打破「自适应 → 需 logits」的耦合。归一化保证不同语言/上下文可比。

2. **Adaptive Insertion：$\delta'_t = \delta \cdot \lambda(c_t)$**:

    - 功能：在生成端按语法自由度动态调整 green-list 偏置强度，语法约束位置几乎不加偏置（保质量），自由位置加强偏置（提检测力）。
    - 核心思路：把 KGW 的固定偏置改为线性缩放 $l'_{t, i} = l_{t, i} + \delta'_t \cdot \mathbb{I}[i \in \mathcal{V}_G]$。语法约束高的位置（如韩语主格助词必须出现的位置）$\lambda \approx 0$，水印几乎不干扰；语法自由位置 $\lambda \approx 1$，水印按全强度嵌入。
    - 设计动机：让水印「随波逐流」——在语言本身允许选择的地方做手脚，在不允许的地方让步——这与人写文本的统计性质一致，因而 perplexity 几乎不变。

3. **Adaptive Detection：weighted z-score $z'$**:

    - 功能：把同样的 $\lambda$ 权重应用到检测端，让高 indeterminacy 位置的 green token 贡献更大检测信号，低 indeterminacy 位置弱化贡献。
    - 核心思路：对每个 token 定义权重 $w_t = \lambda(c_t)$，加权统计 $W_G = \sum_t w_t \cdot \mathbb{I}(x_t \in \mathcal{V}_{G, t})$，在零假设 $H_0$ 下 $\mathbb{E}[W_G] = \gamma \sum_t w_t$，$\text{Var}(W_G) = \gamma(1-\gamma) \sum_t w_t^2$，z-score $z' = \frac{W_G - \gamma \sum_t w_t}{\sqrt{\gamma(1-\gamma) \sum_t w_t^2}}$。
    - 设计动机：让检测端与生成端的「按位置调制」对齐，可信赖度高的位置贡献多、可信赖度低的位置贡献少；同时全程不读 LLM logits，仅需 POS 分析，保留 KGW 的公开验证属性。
