...
description: >-
  [ACL 2026][LLM安全][数据级去毒] 本文提出 HSPD（层次化语义保留去毒）流水线，通过 SoCD（软对比解码）引导 LLM 定位并重写原始语料中的有毒片段，同时保留语义，生成可直接替换原始数据用于微调的去毒语料——在 GPT2-XL 上将毒性概率从 0.42 降至 0.18…
tags:
  - "ACL 2026"
  - "LLM安全"
  - "数据级去毒"
...
**1. SoCD（软对比解码）：只掐住最偏向毒性的少数维度，别一刀切地全局压制**
...
只保留正差异维度（即毒性模型相对更偏好的 token），再用归一化差异度 $\alpha = \ln(1+\delta)/(1+\ln(1+\delta))$ 决定干预的范围，仅抑制 top-$k = \alpha \times V$ 个最偏差的维度，其余通道原封不动。这样大部分语义信息得以保留，去毒只发生在毒性信号最强的地方。

**2. 自适应 $k$ 值：让每一步的干预强度跟当下的毒性强度成正比**

毒性并非均匀分布在每个 token 上——"fuck"这种位置基础模型和毒性模型的分布差异极大、需要强干预，而"the"这种位置几乎没有差异、根本不该动。固定的 $k$ 要么在干净位置上过度抑制、要么在有毒位置上压不住。SoCD 因此用 $k = \text{clip}(\lceil \alpha V \rceil, k_{\min}, k_{\max})$ 动态调节：$\alpha$ 反映当前步两模型分布的偏差，差异大就抬高 $k$ 多抑制几维，差异小就只碰少数维度，外层的 clip 再用上下限兜住极端情况。干预强度由此随毒性强度自动伸缩。
