...
**2. 语言感知加权优化：用「兄弟语言」的相对表现给困难方向加码**
...
**3. 验证预言与奖励设计：用二值可执行奖励把目标对齐到功能等价而非表面相似**

跨语言验证靠的是一个二值可验证奖励 $R(y, T) = \mathbb{1}[y \text{ compiles and passes all tests in } T]$——译文必须既能编译、又通过测试套件 $T$ 才得 1，编译错误、运行时错误、超时一律记 0。测试套件本身通过 MultiPL-E 把 Python 规则化转换到其他语言，使同一组测试在所有方向复用。这样优化目标盯的是功能正确性，而不是 BLEU 式的表面形式相似，也正是自举与加权两个机制能成立的共同基础。
