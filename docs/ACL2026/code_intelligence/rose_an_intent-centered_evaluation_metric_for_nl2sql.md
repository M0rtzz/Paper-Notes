...
### 关键设计
1. **SQL Prover 的参考无关判断**:

	- 功能：减少单一参考 SQL 对评测的锚定效应。
	- 核心思路：Prover 不访问 ground-truth SQL，只根据问题、数据库 schema/内容、预测 SQL 和执行结果判断预测是否满足用户意图，输出布尔判断和理由。
	- 设计动机：很多正确预测会因格式、排序、冗余列或合理歧义与参考结果不同。先不看参考，可以避免一开始就把这些答案判死刑。

2. **Adversarial Refuter 的证据式反驳**:

	- 功能：控制 Prover 可能过度宽松的问题，并把参考 SQL 中的信息利用起来。
	- 核心思路：当 Prover 接受一个与参考结果不同的预测时，Refuter 比较预测和参考的推理逻辑，判断差异是否影响用户意图；它可以推翻 Prover，也可以标注 ground-truth SQL 错误 GoldX 或问题歧义 AmbQ。即使执行结果相同，Refuter 也能检查是否只是碰巧得到相同结果。
	- 设计动机：完全 reference-free 的 LLM judge 容易放过错误 SQL，而参考 SQL 虽不总是正确，仍包含有价值的反证信号。

3. **诊断标签与版本化 LLM judge**:

	- 功能：让指标不仅给分，还能帮助发现数据集问题，并保证长周期评测可复现。
	- 核心思路：Refuter 对冲突案例输出 GoldX、AmbQ 等标签；ROSE judge 使用 `ROSE_model-time` 命名，例如 `ROSE_o3-2504`，新模型需要重新验证后才能替换。
	- 设计动机：NL2SQL 评测危机的一部分来自基准本身，指标应能反向定位标注错误和歧义；同时 LLM judge 随版本变化，必须显式记录版本以减少排行榜漂移。
