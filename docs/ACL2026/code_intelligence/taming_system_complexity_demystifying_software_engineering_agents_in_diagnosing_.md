...
LinuxFL+ 的工作流分为三个阶段。首先，任意 LLM Agent（如 AutoCodeRover）独立运行，生成初始的可疑文件预测。然后，进入扩展阶段，包括（1）**目录感知扩展**：从初始预测所在目录出发，扩展候选集范围，让 LLM 在更大的相关文件集合中重新筛选；（2）**潜在原因扩展**：生成多种可能的 bug 原因假设，每个假设映射到需要修改的文件列表；（3）**候选集成**：汇聚三个来源的预测结果，使用聚合评分和 LLM 重排最终确定文件顺序。该设计充分利用了 LLM 的生成和理解能力，同时借助结构化的外部知识（代码库结构和邮件知识库）来纠正 Agent 的盲点。

### 关键设计

1. **目录感知扩展**:

    - 功能：在 Agent 初始预测的目录内进行二阶段搜索，扩大候选文件范围同时保持局部相关性
    - 核心思路：虽然 LLM Agent 通常能正确定位到相关的高层次目录（模块），但在该目录内有大量相关文件时容易混淆。本设计首先收集所有初始预测文件所在目录的完整文件列表（Linux 内核中平均每目录 16 个文件），然后提示 LLM 在这个扩展的候选集中重新筛选、排序 top-10 个最相关的文件。这相当于给 LLM 第二次机会在更完整的信息下做决策，利用目录结构这一代码库的显式组织形式作为导向。
    - 设计动机：Linux 内核的目录平均每个包含 16 个文件（SWE-bench 是 8 个），这种高密度给精确定位增加了难度。通过显式利用目录边界作为搜索范围，可以在保持模块粒度有效性的前提下，给 LLM 足够详尽的上下文进行微调。

2. **潜在原因扩展**:

    - 功能：通过多种假设生成策略系统地扩展可能的 bug 原因，每个原因对应一组需要修改的文件，形成多个候选集
    - 核心思路：该设计包含两层假设。第一层 **直接假设** 仅使用 LLM 的预训练知识，提示模型生成 $k$ 个可能导致该 bug 的原因假设，每个假设还需给出对应的代码修复方案和涉及的文件。第二层 **邮件辅助假设** 引入外部知识：使用检索增强生成（RAG）从 Linux 内核邮件列表（LKML）中检索相关的历史讨论，这些邮件通常包含开发者对类似 bug 的分析。将检索到的邮件内容和原始 bug 报告一起提供给 LLM，促进其生成更多样化、更有根据的原因假设。为防止数据泄漏，只检索 bug 报告之前发送的邮件；为提高检索质量，对 bug 报告进行关键词提炼（行为、原因、预期行为、解决方案四个维度），然后用 BM25 检索 top-10 相关邮件。
    - 设计动机：真实的 bug 诊断是一个迭代的"猜测-验证"过程，不能仅依赖初次直觉。通过系统地生成多个假设，可以覆盖更广的根因空间。直接假设利用 LLM 的通用知识；邮件假设引入领域特定的历史智慧，两者互补。实验表明这种多假设策略能大幅提升性能，特别是在 bug 原因分散（如性能问题跨多个模块）的情况下。

3. **候选集成与重排**:

    - 功能：将三个来源（目录扩展、直接假设、邮件假设）的文件预测聚合为统一排序，作为最终定位结果
    - 核心思路：聚合策略采用倒数排名法。对每个候选文件 $f$，分别从三个来源记录其排名 $R_{dir}(f)$、$R_{direct}(f)$、$R_{mail}(f)$（若文件未出现则记为 $\infty$）。计算聚合得分为 $\text{score}(f) = \frac{1}{R_{dir}(f)} + \frac{1}{R_{direct}(f)} + \frac{1}{R_{mail}(f)}$。该公式使得在任意一个方法中排名靠前的文件获得较高分数，在多个方法中都排名靠前的文件进一步得到优先。之后根据得分降序排列所有候选文件。最后，提示 LLM 基于文件路径与 bug 报告的语义对应关系对这个初步排序进行精排，利用 LLM 的语义理解能力进行最后调整。
    - 设计动机：三个信息源各有独特视角——目录扩展擅长捕捉结构关联，直接假设发挥 LLM 通用知识，邮件假设提供领域启发。倒数和聚合是一种经典的 rank fusion 方法，既能综合多源信息，又简洁有效。最终的 LLM 重排引入语义意识，避免纯数值聚合可能的死板。
