...
**1. 执行结果分组与跨组偏好：把“逐条排序”换成“按执行语义分组再排序”，消除等价 SQL 打分不一致**
...
**2. pointwise-group utility 作为稳定锚点：当跨组偏好分不出胜负时，给一个顺序无关的补充信号**
...
**3. agentic resampling 与位置一致性训练：补候选召回的洞，顺便治排序器的输入顺序偏置**

排序阶段默认正确候选已经在池子里，可现实是 bounded recall——没有就是没有，排序器无能为力。R3-SQL 让 agent 先诊断初始池：若判断缺正确 SQL，就丢掉原池、采样一个更大的新池，再用 pointwise pruning 剪回原大小，避免无脑 always-resample 引入的噪声和成本。另一个隐患是 listwise ranker 会被候选喂入顺序带偏，于是训练时把同一对 correct/incorrect SQL 以原顺序和交换顺序都喂一遍，在 GRPO 里加一项 consistency reward $R=R_{base}+\lambda_c R_c$（$\lambda_c=0.5$），逼排序器对调换顺序也给出一致判断。
