...
1. **仓库文件分类与相关度筛选**:
	...
	- 功能：从可能包含数百个嵌套文件的仓库中挑出最有助于示例生成的文件。
	- 核心思路：每个文件被送入提示词，分类为 code、documentation、scripts、data 或 other；其中 code、documentation、scripts 又细分为 existing examples、instructions、entry points 等更细类别。同时系统为文件打 1-5 的相关度分数，并记录是否含 GPU、配置说明、关键任务信息等元数据。
	- 设计动机：代码生成模型上下文有限，直接塞入整个仓库既昂贵又噪声大；先筛选文件可以把注意力集中在 API、文档和已有示例上。

2. **可运行示例生成**:

	- 功能：把仓库核心功能转化为下游智能体可复用的最小实验示例。
	- 核心思路：修改版 CodeScientist 接收高相关文件和仓库目的，生成四类输出：可执行 Python 代码、Python 依赖、带 Conda 环境设置的 bash runscript，以及元数据，包括用途描述、适用/不适用场景、CPU/GPU/RAM/disk 需求和是否需要用户交互。
	- 设计动机：下游 Code-RAG 需要的不是仓库摘要，而是能被检索、理解、运行和组合的具体代码片段；元数据则帮助 agent 判断何时调用该示例。

3. **执行-评判-反思调试循环**:

	- 功能：把“看起来合理”的代码转化为“实际运行过”的示例。
	- 核心思路：示例在 Ubuntu 云容器中执行，系统捕获 stdout/stderr、时间戳日志、JSON 结果和图表等人类可读输出。执行结束后由 LLM-as-a-judge 判断是否正确展示仓库功能；若有问题，则把当前代码和执行日志输入模型反思修改。循环通常最多 8 次，以控制成本。
	- 设计动机：科学代码的正确性无法只靠静态文本判断，真实执行和日志驱动修复是区分 toy code 与可用 code library 的关键。
