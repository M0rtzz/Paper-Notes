...
1. **多复杂度任务指令合成**:
	...
	- 功能：构造覆盖简单动作、空间推理和长程规划的任务分布。
	- 核心思路：LLM agent 根据机器人能力、操作约束和示例生成任务指令，并通过多种系统 prompt 配置控制复杂度分布。任务从一两步基本移动，到五步以上包含复杂动态推理的 long-horizon planning。
	- 设计动机：只训练简单任务会让 SLM 无法泛化到复杂任务，只训练复杂任务又会损害基本任务表现。实验中 simple-only 和 complex-only 的结果直接验证了任务多样性的重要性。

2. **可控 ground-truth code 生成与清洗**:

	- 功能：用较少人工成本获得高质量 instruction-code pairs。
	- 核心思路：简单任务由 LLM corrective code generation 自动生成代码；高复杂任务由人类专家审核。最终数据中移除代码注释，减少 SLM 对注释语言模式的依赖。
	- 设计动机：机器人控制代码错一步就可能改变真实动作。完全依赖 LLM 会给复杂样本引入错误监督，而注释又可能让 SLM 学到表层语言模式。混合校验和去注释共同提升数据可靠性。

3. **LLM 作为行为等价奖励函数**:

	- 功能：让优化目标对齐机器人动作，而非文本形式。
	- 核心思路：GRPO 阶段让 LLM 解释 SLM 生成代码和 ground truth 代码分别对应的机器人行为，并判断动作序列是否等价。奖励反馈用于优化 SLM 的 step-level reasoning。
	- 设计动机：机器人代码中变量名、表达形式不同不一定代表行为不同；相反，语义相似文本也可能产生不同动作。用 LLM 判断行为等价比字符串匹配更符合任务本质。
