...
## 方法详解
LegalGraphRAG 的关键不是单纯“把法律文档放进图里”，而是把法律推理过程结构化：先根据案件事实在层级图中找证据，再用规则图中的 checklist 和司法解释做验证，最后只基于验证后的子图生成判决。

### 整体框架
输入是一段犯罪事实描述 $f$ 和被告 $d$，系统先基于离线法律语料 $\mathcal{D}$ 构建法律知识图 $KG=\Phi(\mathcal{D})$。对于查询，检索器从图中得到上下文 $\mathcal{C}=\mathcal{R}(f,d,KG)$，生成器再基于查询与证据推断罪名 $y$。LegalGraphRAG 把这个过程具体化为两阶段：Hierarchical Knowledge Construction 和 Evidence-based Legal Reasoning。

第一阶段构建 HierarGraph，把历史案例、法条、司法解释、案件特征和罪名组织进不同子图。第二阶段由三个代理串联执行：Researcher 从 ontology/fact graph 中检索候选案例和法条，Auditor 基于 rule graph 检查法条是否适用，Adjudicator 汇总确认后的法条、案例和罪名节点，生成带引用的最终判断。

### 关键设计
1. **HierarGraph 三层法律图谱**:

	- 功能：显式分离具体事实、抽象法律概念和规则适用条件，避免扁平图把不同粒度信息混在一起。
	- 核心思路：Fact Graph 连接 Cases、Articles 和 Offense 节点；Ontology Graph 把原始案情抽象成被告属性、犯罪行为、受害者特征、主观心态等维度，并用 kNN 与 Leiden 社区组织相似案例；Rule Graph 连接法条与司法解释，并为法条附加 Diagnostic Checklist。
	- 设计动机：法律判断常取决于事实细节是否满足法条条件。只靠语义相似度可能找到相似叙述，却漏掉决定罪名的抽象规则。

2. **Researcher 多路证据检索**:

	- 功能：从层级图中覆盖不同类型的候选证据。
	- 核心思路：检索结果是三种 operator 的并集 $\mathcal{R}(q)=\mathcal{R}_{sem}(q)\cup\mathcal{R}_{com}(q)\cup\mathcal{R}_{chg}(q)$，分别对应语义匹配检索、社区扩展检索和罪名锚定检索。
	- 设计动机：单一路径检索容易偏向高频事实或局部相似案例；多路检索能同时覆盖直接相似、社区上下文和潜在罪名相关法条。

3. **Auditor 与 Adjudicator 证据闭环**:

	- 功能：过滤无效证据，并把最终裁决绑定到可验证依据。
	- 核心思路：Auditor 对每个候选法条使用 Diagnostic Checklist 和 Judicial Interpretations 验证案件事实是否满足适用条件，剪掉不适用法条及相关节点；Adjudicator 用验证后的 $\mathcal{A}^f$、$\mathcal{C}^f$、$\mathcal{O}^f$ 生成判决 $\mathcal{J}=Adjudicator(q\oplus\mathcal{A}^f\oplus\mathcal{C}^f\oplus\mathcal{O}^f)$。
	- 设计动机：法律场景不能接受“答案对但证据不支持”。显式验证把黑箱生成改成可审计的证据链。
