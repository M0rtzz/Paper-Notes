...
flowchart TD
    A["冻结 GNN 节点嵌入 f_Φ(v)<br/>+ 节点文本描述"] --> B["GNN-LLM 嵌入投影器<br/>Π：单个嵌入 → k 个软提示 token"]
    B -->|上下文对齐损失（语义）<br/>+ 对比损失（结构）| C["混合提示构建<br/>展开计算树，软提示与文本交错"]
    C --> D["冻结 LLM 直接推理<br/>读 GNN 内部表示生成解释"]
    D --> E["解释子图提取与幻觉缓解<br/>每个节点三分类（+1 / −1 / 0）+ 后处理验证"]
    E --> F["输出：自然语言解释<br/>+ 解释子图 S⁺"]
```
