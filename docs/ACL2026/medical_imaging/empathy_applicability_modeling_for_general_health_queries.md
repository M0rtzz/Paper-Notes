...
description: >-
  [ACL2026][医学图像][clinical empathy] 本文提出 Empathy Applicability Framework (EAF)，先判断患者单轮健康咨询中是否“适合”表达情绪回应或解释性理解，再用人类与 GPT-4o 标注构建基准并训练分类器…
tags:
  - "ACL2026"
  - "医学图像"
  - "clinical empathy"
...
### 关键设计
1. **EAF 双维度 cue framework**:

	- 功能：把“是否需要同理心”拆成情绪回应和解释性理解两个上游判断。
	- 核心思路：EA Applicable cues 包括 severe negative emotion、inferred negative state、seriousness of symptoms、concern for relations；EA Not Applicable cues 包括 routine health management、purely factual medical queries、neutral symptom descriptions、hypothetical queries。IA 则关注 expression of feeling、experiences/context affecting emotional state、symptoms with emotional impact、distressing uncertainty about health 等线索。
	- 设计动机：医疗问题中“患者有情绪”与“回答需要表达理解”不是完全相同的事。双维度设计可以避免把所有非事实问题都粗暴归为情绪支持。

2. **人类标注 + GPT 标注的分层数据构建**:

	- 功能：同时获得高可信的人类共识集和规模更大的 GPT 标注集。
	- 核心思路：两名具备高英语能力的 lay annotators 经过训练后独立标注 1,296 条 query；GPT-4o 使用包含定义、subcategory 描述和示例的 contrastive prompt 进行标注。对于人类标注集，GPT 做 5 次 pass 并 majority vote，以提高稳定性。
	- 设计动机：同理心判断高度主观，作者没有用 crowdsourcing，而是强调一致性训练；同时用 GPT 扩展 8,000 条数据，检验自动标签能否训练出近似人类共识的模型。

3. **可学习性验证与分歧分析**:

	- 功能：证明 EAF 不是只停留在概念框架，而是能形成可预测、可解释的标签模式。
	- 核心思路：分类器统一在 human-consensus test set 上评估，指标包括 accuracy、macro-F1、weighted-F1。分歧分析用 UpSet plot 检查 GPT 与人类是否选了相同 subcategory rationale，用 divergence bar 把 mismatch 拆成 Annotator Spread、LLM-Adds 和 LLM-Omits。
	- 设计动机：医疗同理心不能只追求一个最高 F1；哪些 cue 分歧大、为什么分歧，决定了框架在真实医疗系统中如何校准。
