...
description: >-
  [ACL 2026][医学图像][claim-level grounding] eTracer 把 RAG 响应拆成原子 claim 再去上下文里搜支持/反驳的句级证据，用三步流水线（分解 → 嵌入检索 → 蕴含判定）输出带符号分数矩阵，从而在生物医学场景下既能精确反查每条事实的出处、又能定量评估响应的忠实度…
tags:
  - "ACL 2026"
  - "医学图像"
...
eTracer 是一个 plug-and-play 的后处理框架：输入是「LLM 生成的响应 $\mathcal{R}$ + 它依赖的上下文 $S=\{s_i\}_{i=1}^m$」，输出是「响应每个句子对应的带符号分数矩阵 $\tilde{M}\in\mathbb{R}^{p\times m}$」，由此可以反查证据、计算 FCR/ACR/HCR/UCR 四类忠实度指标。

### 整体框架

整个推理走 9 步、3 个阶段（见 Algorithm 1）：

1. **响应预处理**：NLTK 切句 → 分解模型 $\mathcal{M}_{dec}$ 把每句切成原子 claim → 蕴含模型 $\mathcal{M}_{ent}$ 验证「响应句 ⊧ claim」，不通过就重新分解；通过的 claim 由 Qwen3-Embedding-8B 编码成向量。
2. **上下文预处理**：NLTK 切句 → 每个上下文句也用同一个嵌入器编码。
3. **Claim Grounding**：先用 cosine sim 做证据初筛（阈值 $\tau$）；再让 $\mathcal{M}_{ent}$ 对每个 (claim, 候选证据) 对预测 Entailment / Contradiction / Neutral；最后用符号函数 $\psi$ 把三分类映射成 $\{+1, -1, 0\}$，与相似度相乘得到带符号分数。

### 关键设计

1. **Sentence ⇒ Claim 分解 + 自洽校验**:

    - 功能：把响应句切成「原子、独立、语义完整」的 claim 集合，并保证 claim 是响应自身的逻辑后承。
    - 核心思路：作者用 GPT-5.1 在 182 个手标 sentence-claim 组上生成蒸馏数据 $\mathcal{D}_{dec}$，把知识蒸馏进 Qwen3-14B（LoRA, 4-bit, 10 epoch, lr $2\times 10^{-4}$）。推理时强制循环：分解 → 用 $\mathcal{M}_{ent}$ 验证 $\mathcal{R}\models c_i$，若失败则重新采样直到全部通过或触达上限。损失即标准的条件 NLL：$\max_{\mathcal{M}_{dec}}\mathbb{E}_{(r,\{c_i\})}\log p_{\mathcal{M}_{dec}}(\{c_i\}\mid r)$。
    - 设计动机：分解模型只要 hallucinate 一条 claim，下游 grounding 就会被永久污染（错的 claim 永远找不到支持证据，被误判成幻觉）。强制蕴含校验是把「分解阶段的幻觉」当成需要修复的失败模式，而不是默默接受。

2. **带符号 grounding 函数 $\phi$（importance × polarity）**:

    - 功能：对每个 (claim, 上下文句) 对输出一个既反映「证据强度」又反映「支持 / 反驳」方向的标量。
    - 核心思路：分两路。极性路用蕴含模型 $\mathcal{M}_{ent}$ 给出 $\psi(s, c)\in\{+1, -1, 0\}$；强度路用余弦相似度 $M_{ij}=\mathbf{e}_{c_i}\cdot \mathbf{e}_{s_j}$ 做检索粗筛。最终 $\tilde{M}_{ij}=M_{ij}\cdot \psi(s_j, c_i)$ 仅当 $M_{ij}>\tau$ 时保留，否则置 0。阈值 $\tau$ 在 cos-sim 分布上选，论文默认 $\tau=0.5$ 兼顾质量与速度。
    - 设计动机：传统二元 NLI 把「中立」和「反驳」混成「不是支持」，丢掉了医学场景里关键的「这条证据其实反对该主张」信号。$\{-1, 0, +1\}$ 三态加上连续强度，把检索（哪些句值得看）与判定（看完之后怎么算）解耦，单独 ablate 后还能精确算 FCR / ACR / HCR / UCR。

3. **三个 reference-free 评估指标 (CER / ECSS / PFCR)**:

    - 功能：在没有 ground truth 引用集的真实部署里，仍然能持续监控 grounding 质量。
    - 核心思路：① **CER** = 响应蕴含 claim 的比例 $\frac{1}{p}\sum \mathbb{I}[\mathcal{R}\models c_i]$，衡量分解忠实度（越接近 1 越说明没乱编 claim）；② **ECSS** = claim 与其挑出来的证据的平均 cos sim $\frac{1}{k}\sum \mathrm{Sim}(c, s_i)$，衡量「检索-语义」一致性；③ **PFCR** = 对 claim 取否定后符号翻转的比例 $\frac{1}{k}\sum \mathbb{I}[\phi(s_i, c)\approx -\phi(s_i, \neg c)]$，衡量极性判别的稳健性。
    - 设计动机：附录 B 用真实标注数据先验证「这三个性质 GT 上都成立」（CER 97%、cos≈0.75、PFCR 90%），再反过来用它们当 proxy。这样工业部署里就不用为每条响应做人工标注，也能算出方法分数。
