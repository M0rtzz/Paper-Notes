...
description: >-
  [ACL2026][医学图像][临床 NLP] 本文面向西班牙临床笔记中的早期 HIV 疑似识别，提出 MCP 处理 aleatoric uncertainty、MCMD 几何 veto 处理 epistemic uncertainty 的双验证选择性分诊框架…
tags:
  - "ACL2026"
  - "医学图像"
  - "临床 NLP"
...
### 关键设计
1. **Mondrian Conformal Prediction 管 aleatoric uncertainty**:

	- 功能：判断文本证据是否在概率意义上足够明确，避免对模糊病例硬判。
	- 核心思路：先对验证集 logits 做 temperature scaling 降低过度自信，再用 MCP 构造风险容忍度为 $\alpha$ 的预测集合 $\Gamma^{\alpha}(x)$。若集合包含多个类别或为空，就说明证据不足或冲突，系统 defer。
	- 设计动机：临床叙事本身可能不完整或措辞模糊，这种歧义不是模型换个分类阈值就能解决的；conformal prediction 能把风险水平显式暴露给医院管理者。

2. **Multi-Centroid Mahalanobis Distance 管 epistemic uncertainty**:

	- 功能：拦截 softmax 很自信但 latent representation 异常的分布外病例。
	- 核心思路：在 $L_2$ normalized latent space 中，每个类别用 k-means 自动选择多个局部 centroids，最小 $K$ 由 inertia gain 低于 0.05 确定。样本的异常度是它到预测类别任一 centroid 的最小 Mahalanobis 距离 $d_M(x,\hat{y})=\min_k\sqrt{r_{\hat{y},k}^\top\Sigma^{-1}r_{\hat{y},k}}$，其中全局 precision matrix 用 OAS shrinkage 从 residuals 中估计。
	- 设计动机：HIV 疑似病例高度异质，单一高斯中心会把少数类方差估得过大；multi-centroid 既保留表型多样性，又避免高维局部协方差估计不稳定。

3. **风险非对称的临床评价与可调 operational dial**:

	- 功能：用符合早期感染筛查场景的指标评估系统，而不是只看普通 F1。
	- 核心思路：作者使用 $F_2$ 强化 false negative 惩罚，用 ECE 衡量校准，用 coverage/TPDR/AURC 衡量分诊效率，还设计 Custom Risk-Kappa，把自动化 false negative 的惩罚设为 1.0，false positive 和 true positive deferral 设为 0.5，true negative deferral 设为 0.25。
	- 设计动机：早期 HIV 漏诊会延迟 ART 并增加传播风险，其代价远高于一次不必要检测，因此评价函数必须反映错误类型的不对称性。
