...
1. **Summary-Level MCTS 节点设计**：

    - 功能：把搜索粒度从传统 LLM-MCTS 的 token/sentence 级提升到 summary 级，使搜索复杂度可控、每个节点都是可直接采纳的完整答案。
    - 核心思路：每次扩展节点时枚举所有 action（即所有属性都是 legal action，即使已经调过也可重新调），prompt LLM 基于完整历史 $s_0,s_1,\ldots,s_t$ 生成 $s_{t+1}$。选择步用 PUCT 变种 $a = \arg\max_a[Q(s,a) + U(s,a)]$，其中 $U(s,a) = c_{\text{puct}}\cdot\pi_\theta(s,a)\cdot\sqrt{\sum_b N(s,b)}/(1+N(s,a))$，$c_{\text{puct}}$ 用对数形式动态平衡探索/利用。允许重复调同一属性是因为后续动作可能破坏之前的调整效果，要给模型回头修复的机会。
    - 设计动机：token-level MCTS 在长摘要上搜索空间太大（每个节点对应几千 token），且无法整体评估属性满足度；summary-level 把每次扩展变成一次完整生成，搜索深度只有 5–10 步，trade-off 是每次扩展更贵但语义粒度更合适。

2. **属性类型感知的 reward 设计**：

    - 功能：根据属性的可量化性质区分 reward 计算方式，让搜索同时支持"精确匹配"和"越高越好"两种目标。
    - 核心思路：把属性分两类——**deterministic**（extractiveness, length, specificity，要精确匹配用户给的数值）用 MAD（mean absolute deviation）的倒数作 reward；**non-deterministic**（topic, speaker，越对齐越好）用 BERTScore 直接作 reward。Local reward $= \alpha / (avg_{\text{det}} + \varepsilon) + (1/\beta) \cdot avg_{\text{non-det}}$，$\alpha,\beta$ 控制两类属性的相对重要性。每个属性的具体度量：extractiveness=摘要词在原文中比例，length=词数，specificity=命名实体/总词数，topic=主题词与摘要词的平均 BERTScore，speaker=摘要与目标说话人话语的 BERTScore。
    - 设计动机：可控摘要的属性本质异构——长度是"必须 = 50 词"，topic 是"越贴主题越好"，混在一起用同一个 metric 既数值上不可比，又会让 MCTS 的 reward signal 失真。分类后既能精确控制硬约束，又能在软约束上做 best-effort。

3. **DimensionAware 终止 / 选择策略**：

    - 功能：在 MCTS 跑完后选择"全树中 degree 最高"的节点而非 leaf 节点，并支持只满足部分属性的灵活终止。
    - 核心思路：标准 MCTS 选 most-visited leaf，PACO 选 highest-degree node from entire tree。因为不同 instance 的"最优可达 degree"不同，硬要满足所有属性会强迫搜索到 max depth 反而劣化质量；选全树最高 degree 节点等价于"自适应放弃无法同时满足的属性"。Reward 聚合有 3 个 pluggable 策略：Weighted Mean（默认，含 recency-decay $\lambda$ 上调后期动作权重）、Geometric Mean（强调各属性平衡）、Min Score（安全关键场景）。Backprop 用标准 $W(s,a) \leftarrow W(s,a) + V(s_l)$, $Q = W/N$。
    - 设计动机：可控摘要中"全部属性精确满足"在现实里常常不可行（属性间冲突），强求会导致质量崩塌；让 MCTS 自己发现"调到哪一步收益最大"比硬性约束更鲁棒，也更符合用户实际偏好。
