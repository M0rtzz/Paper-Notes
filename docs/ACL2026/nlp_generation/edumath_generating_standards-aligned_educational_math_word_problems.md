...
1. **四维度 MaC（Meets-All-Criteria）评测协议**:

    - 功能：把"教育级 MWP"拆成 Solvability / Accuracy / Educational Appropriateness / Standards Alignment 四条二元判定，缺一不可。
    - 核心思路：Solvability—题面是否有解且答案唯一；Accuracy—CoT 解答既要终答案对，又要中间推理可读、不绕；Educational Appropriateness—无语法错、无冲突信息、内容适合 3-5 年级课堂；Standards Alignment—是否严格满足 VA SOL（含数值范围、步骤数等具体约束）。MaC = 四条全部为 True。教师对 MaC 一致率 $65.5\pm1.7\%$，看似低实则因为四维联合所致。
    - 设计动机：以往 MWP 评测只看可解性 + 正确性，会让"题面与标准不符"或"解答冗长"的题轻易过关；MaC 把鉴别力压到最严，符合真实老师"任何一条不合就退稿"的工作流。

2. **STEM 数据集 + 教师 × LLM 二次过滤**:

    - 功能：构造首个"教师标注 + 标准对齐 + 可读解答"三合一的 MWP 训练集（2,577 条），打破 GSM8K/ASDIV 等数据集的对齐空白。
    - 核心思路：把 ASDIV 子集（1,025）和教师认为 MaC 的合成题（1,552）合并；对"教师 MaC 但 Gemma 复检否决"的样本翻转为非 MaC（去人为漏检），对"教师否决但 Gemma 通过"的样本保留教师判断（信任专家直觉）。同时记录每条题的 token 长度、8 项可读性指数复合分、BERTScore F1 等度量，控制数据风格与人写题相似。
    - 设计动机：单纯教师标注成本高且会有疲劳误差，单纯 LLM 标注会漏掉"教师才能看出的微妙不适宜"；不对称翻转规则把两端各自的最大价值留下，给训练集铸出"教育上最干净"的子集。

3. **SFT + KTO + ModernBERT 后处理的三段式生成管线**:

    - 功能：把"高质量但有限"的标注数据榨干每一分价值，使 12B 模型能匹敌 27B、30B 能反超闭源。
    - 核心思路：SFT 用 STEM 监督模型学会"可解 + 标准对齐 + 可读 CoT"的基本模式；KTO（Kahneman-Tversky Optimization）用 binary MaC 标签作为正/负样例，无须成对偏好，正好契合教师投票得到的 binary 信号；最后训一个 ModernBERT 二分类器（AUC-ROC 0.861），在推理时对 LLM 输出做拒绝采样，对 EDUMATH-12B 起 +4.9% MaC、对未训练的 Qwen-3-30B 起决定性提升把它推到 SOTA。三段都是"数据驱动的、可叠加的"。
    - 设计动机：KTO 比 DPO/RLHF 对"无配对偏好"更友好；ModernBERT 分类器把"训练成本"和"质量门槛"解耦，使任何未来更强 LLM 都能"零训练接入"——这等价于把作者的标注价值长期复用。
