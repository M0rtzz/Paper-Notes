...
### 整体框架
GROKE 是 **training-free + vision-free** 的两 agent 层次化系统（图 2）：
1. **Sub-instruction Agent**：把整条指令 $I$ 拆成 $K$ 个原子子目标 $\{g_1,\dots,g_K\}$（MOVE_FORWARD / TURN_LEFT / TURN_RIGHT 加上 NL 描述），同时抽出所有 landmark $\mathcal{L}$ 并用 fuzzy string match 接到 OSM POI 上（RapidFuzz partial_ratio）。
2. **Navigator Agent**：迭代执行当前子目标 $g_k$，每步构造 visible area $\mathcal{G}_t$（沿当前 heading 朝下一个 intersection 走 $u$ 个节点 + 3 节点 lookahead），把 $(I, v_t, h_t, \mathcal{G}_t)$ 喂给 LLM，输出 $(\text{status}_k, v_{t+1})$。$\text{status}_k = \text{COMPLETED}$ 则推进下一个子目标。
3. **终止条件**：(i) 所有子目标完成；(ii) 总步数 > 100；(iii) 单子目标重试 > 15。
4. **评价反转**：传统 VLN 是"agent 多好"；这里把 agent 固定（Gemini-3 Pro），把 NE / SR / SDTW / nDTW 反过来读作"**指令多 navigable**"。

### 关键设计

1. **Vision-free JSON 空间序列化**:

    - 功能：把局部 OSM 子图（节点 + 边 + POI）转成 LLM 最容易读的结构化格式。
    - 核心思路：分两部分组织——nodes 部分含 id、类型 (intersection/waypoint)、heading $h \in [0,360)$、连接列表（每条带目标 id 和方位角）；POI 部分含每个 landmark 的字母 ID、最近节点引用、相对方向（Forward / Left / Right / Back 用 $\delta = (h_{v\to p} - h_{\text{curr}} + 180) \mod 360 - 180$ 离散化）、Haversine 距离。每个节点的相对方位用 spherical bearing 公式 $h = \text{atan2}(\sin\Delta\lambda \cos\phi_2, \cos\phi_1\sin\phi_2 - \sin\phi_1\cos\phi_2\cos\Delta\lambda)$ 计算。
    - 设计动机：作者系统比较了 4 种表示——纯文本叙述、JSON、Graphviz DOT、ASCII grid 矩阵。表 5 数据触目惊心：JSON SR 63% / NE 68m，Textual 61% / 70m，Graphviz 40% / 96m，Grid 仅 10% / 175m。**Grid 让 LLM 把空白格 '0' 当合法路径选，几乎失效**。JSON 的层级嵌套让模型从局部偏离里"找路回来"的能力更强，OSR 74% vs Textual 67% 验证了这点。在 Hard 难度上 JSON SR 53.8% 比 Textual 38.5% 高 15 个点，证明结构化对长程推理是质变。

2. **Sub-instruction Agent 把长指令拆成原子动作**:

    - 功能：把一条 53 token 的复杂指令分解成 $K$ 个状态机化的子目标，每个子目标只对应一个原子动作 + 自然语言描述 + 状态 (TODO/IN_PROGRESS/COMPLETED)。
    - 核心思路：LLM 作为 parser 直接做 $I \xrightarrow{\text{parse}} \{g_1,\dots,g_K\}$，每个 $g_k$ 形如 `("MOVE_FORWARD", "Go straight to the bank", TODO)`。Navigator 一次只看当前 $g_k$ 而非整条指令，把"长程规划"切成"短程执行 + 状态推进"。
    - 设计动机：把 LLM 从"同时记忆 5 步指令 + 推理空间"的双重负担里解放——一次只对一个原子目标做反应。消融（Appendix A.2）证明：去掉 sub-instruction 阶段 SR 跌幅显著，整条指令直接喂 navigator 会让模型在中间步迷失。

3. **基于 intersection 的 visible area 截断**:

    - 功能：模拟人类视野——只看到从当前位置沿当前朝向到下一个"分叉点"的局部子图，限制 prompt 内容防止 token 爆炸和 hallucination。
    - 核心思路：Algorithm 1——从 $v_t$ 出发沿 $h_t$ 方向，每步在邻居里选 $\arg\min_{v'} \Delta h(h_{\text{curr}}, h_{v'})$ 且 $\Delta h < 100^\circ$ 的节点继续走，碰到 $\text{degree}(v) > 2$ 的 intersection 计数 +1，直到走过 $u$ 个 intersection 再多取 3 节点 lookahead。POI proximity mapping 用 50 米阈值 + Haversine 距离把附近 POI 挂到 path node 上。
    - 设计动机：让 LLM 看到的"地图"和人类站在路口看到的差不多——既保留下一步决策需要的所有信息，又不会被远处无关街道淹没。这也让 token 数从全图（数千节点）压到几十节点。
