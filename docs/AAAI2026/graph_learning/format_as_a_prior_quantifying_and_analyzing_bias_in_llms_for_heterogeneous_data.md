...
description: >-
  [AAAI 2026][图学习][格式偏差] 首次系统研究 LLM 在处理异构格式数据（文本/表格/信息框/知识图谱）时的格式偏差问题，通过三阶段实验揭示偏差的存在性、数据层面驱动因素和注意力机制层面的内部成因，并验证了注意力重平衡干预的有效性。 LLM 在实际应用中需要处理来自多种格式的外部知识——非结构化文本、半结构化信…
tags:
  - "AAAI 2026"
  - "图学习"
  - "格式偏差"
