...
description: >-
  [AAAI 2026 (Oral)][LLM对齐][DPO] 提出两个正交改进增强扩散模型偏好优化：(1) 稳定参考模型更新策略放松冻结参考模型并通过正则化鼓励探索；(2) 时间步感知训练策略缓解跨时间步奖励尺度不平衡。二者可嵌入多种偏好优化算法，在人类偏好评估基准上提升SOTA。
tags:
  - AAAI 2026 (Oral)
  - LLM对齐
  - DPO
...
**本文目标** 在保持稳定性的同时增强扩散DPO的探索能力和训练平衡性。

**切入角度**：两个可插拔正交策略——参考模型动态更新+时间步感知损失加权。

**核心 idea**：通过参考模型正则化松弛和时间步感知训练，同时解决扩散偏好优化中探索不足和奖励不平衡。
