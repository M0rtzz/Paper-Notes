...
要让一个图模型像 NLP/CV 基础模型那样跨任意图工作，难点在于不同图的特征维度和语义完全不对齐。本文的做法是给图表示加一个新维度：先把邻接矩阵的多种预处理结果堆成一个 $N \times F \times C$ 的三维张量，让每个"节点-特征"位置都拥有一个 $C$ 维的"视图向量"；再用一个跨所有位置共享的可学习函数把视图向量压成标量，得到与特征维度无关的 $N \times F$ 节点表示。整个过程不在特征空间里放任何与维度绑定的参数，因此天然能吃下任意维度、任意语义的特征。
