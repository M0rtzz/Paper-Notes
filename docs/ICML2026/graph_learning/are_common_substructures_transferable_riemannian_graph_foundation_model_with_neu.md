...
flowchart TD
    A["输入图 G=(V,E,X)"] --> B["图编码器<br/>生成节点全局表示 z_i"]
    B --> C["神经向量丛表示<br/>局部坐标 Q^(i) + 伪平行传输 P^(i,j)=(Q^(i))ᵀ Q^(j)"]
    C --> D["Gauge 门控展平机制<br/>按 Tr(I−P) 加权聚合几何一致邻居，逐层 QR 正交化"]
    D --> E["Dirichlet 损失与不变子结构恢复<br/>邻居局部表示预测节点局部表示，低误差连通分量=τ-不变子结构"]
    E --> F["下游适配<br/>冻结预训练主体，仅微调输入编码与任务层"]
```
...
传统图表示只给每个节点一个向量，难以区分“表示值相似”和“局部结构行为一致”这两件事。为了把迁移性变成可计算的几何问题，Gauge 把每个节点从单一嵌入点扩展为“全局表示 + 局部纤维坐标”：对节点 $i$ 学一组正交基 $\mathbf{Q}^{(i)}$，其转置相当于局部平凡化映射，可把全局表示投影到该节点纤维的局部坐标中。相邻节点之间则通过伪平行传输 $\mathbf{P}^{(i,j)}=(\mathbf{Q}^{(i)})^\top\mathbf{Q}^{(j)}$ 来比较各自的坐标系统。这样一来，原本“两个邻域是否承担同一种结构角色”的模糊判断，就被改写成“两套局部坐标系是否几何兼容”的具体度量。

**2. Gauge 的门控展平机制：只聚合几何一致的邻居**

普通消息传递不区分邻居是否共享同一套局部几何，容易把不可迁移的上下文噪声也一起聚合进来。Gauge 在每一层内根据 $\mathrm{Tr}(\mathbf{I}_r-\mathbf{P}^{(i,j)})$ 计算边门控权重：平行传输越接近恒等变换，说明邻居 $j$ 与节点 $i$ 的局部几何越兼容，$j$ 的局部坐标就越多地被 $i$ 吸收；更新后再用 QR 分解保持局部基正交。由于聚合对象被限制在几何一致的邻域，可迁移区域会随着层数加深逐层变平坦，这恰好对应“可迁移结构应具有平坦内在几何”的理论判断。

**3. Dirichlet 损失与不变子结构恢复：用预测误差度量迁移开销**

如果某个结构行为在邻域之间保持一致，它的局部表示就应该能被邻域稳定预测；反过来，预测误差高说明该结构更依赖上下文，迁移时需要更多调整。Gauge 据此把衡量纤维间局部差异的 Dirichlet 能量 $\|\mathbf{x}_i-\mathbf{P}^{(i,j)}\mathbf{x}_j\|^2$ 改写为一个预测式损失：节点的初始局部表示应能由邻居最终局部表示的均值预测出来。预训练完全无监督，误差低的连通分量被解码为 $\tau$-invariant 子结构，既给出了模型学到的可迁移结构，也直接提供了迁移开销的度量。
