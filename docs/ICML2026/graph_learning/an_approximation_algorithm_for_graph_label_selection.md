...
## 方法详解
论文的方法可以理解成三层归约。第一层把一般图上的 GLS 转成树叶标注选择问题；第二层把“标注集合是否让最差稀疏度至少达到 $\tau$”转成一个带源汇边的流问题；第三层利用二叉树结构，为这个流问题设计 DP，并通过二分搜索找到最优阈值。理论近似因子来自树割稀疏化的失真，后续 DP 在树问题上是精确求解。

### 整体框架
输入是一张加权无向图 $G=(V,E,w)$ 和标注预算 $k$。算法先构造一个叶子对应原图顶点的 tree cut sparsifier $T$，使任意原图割权在树上被近似保留。于是 GLS 被转成 Leaf Label Selection (LLS)：在树叶中选择 $|L|\leq k$，最大化未选叶子集合的最差树割稀疏度 $\widehat{\Psi}_T(L)$。

为了求解 LLS，算法对候选阈值 $\tau$ 构造图 $T_{L,\tau}$：所有叶子从源点接收容量 $\tau$，被选中的叶子连到汇点并具有无穷容量，树边保留原容量。作者证明 $\widehat{\Psi}_T(L)\geq \tau$ 当且仅当这个图的 $s$-$t$ max-flow 达到 $n\tau$。于是问题变成：在预算内选择最少叶子作为 sink，使所有源流都可路由。

在二叉树上，这个 sink selection problem 可以用 DP 精确求解。DP 状态 $\mathrm{DP}[v][k]$ 表示在子树 $T_v$ 中允许选择 $k$ 个 sink 时，最多还能从子树根部额外注入多少流而保持可路由。内部节点把预算分给左右子树，并用边容量函数截断每个子树能承受的流。根节点若 $\mathrm{DP}[root][k]\geq 0$，说明预算 $k$ 足以使阈值 $\tau$ 可行。最后对 $\tau$ 二分即可得到 LLS 解。

### 关键设计
1. **Tree cut sparsifier 归约**:

    - 功能：把一般图上的 GLS 变成树叶上的 LLS，同时只损失树割稀疏化的近似因子。
    - 核心思路：构造叶子集合等于原图顶点的树 $T$，要求任意 $A\subseteq V$ 的原图割权 $w_G(A,V\setminus A)$ 被树上分离 $A$ 与补集叶子的最小割 $\lambda_T(A,\mathcal{L}_T\setminus A)$ 夹住，最多放大 $\alpha$ 倍。这样任意标注集合 $L$ 的目标值满足 $\Psi_G(L)\leq\widehat{\Psi}_T(L)\leq\alpha\Psi_G(L)$。
    - 设计动机：树结构使后续 DP 成为可能，而 cut sparsifier 保证树上解不会偏离原图太多；二叉化步骤还能在不改变近似性质的情况下规范化 DP 结构。

2. **阈值化的 flow / sink selection 判定**:

    - 功能：把非线性的最小稀疏度目标转成给定阈值 $\tau$ 的可行性问题。
    - 核心思路：在 $T_{L,\tau}$ 中，每个叶子从源点拿到容量 $\tau$，被选为 label 的叶子有无穷容量连向汇点。若存在某个未标注叶子集合形成过稀疏的割，则 $s$-$t$ min-cut 会低于 $n\tau$；反过来，若 max-flow 达到 $n\tau$，说明所有未标注集合的树割稀疏度都不低于 $\tau$。
    - 设计动机：这个等价性把“所有未标注子集都要好”这种全局约束压缩成 max-flow/min-cut 语言，为后续树上 DP 提供了可组合的流守恒视角。

3. **树上动态规划与二分搜索**:

    - 功能：在固定 $\tau$ 下精确求解最少需要多少 sink，并据此判断预算 $k$ 是否可行。
    - 核心思路：叶子 base case 很简单：不选该叶子时它贡献 $-\tau$ 的净需求，选它时可吸收任意流。内部节点枚举预算分配 $a$ 与 $k-a$，用 $\mathrm{bound}_{(v,c)}(x)$ 把子树可承受流截断到边容量范围内，再相加得到父节点状态。总状态数 $O(nk)$，每个转移枚举预算，DP 时间 $O(nk^2)$。
    - 设计动机：传统 max-flow 在一般图上仍然难以直接服务于“选 sink”的组合问题；树结构让左右子问题只通过一条边交换流，DP 可以完整捕捉全局相互作用。
