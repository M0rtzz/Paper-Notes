...
description: >-
  [ICML 2026][图学习][图神经网络] RADE 在 GNN 训练中同时随机删边与加边，并通过"期望保持"的聚合校正使训练-推理对齐，再用 GradNorm 自适应调节删/加比例，让一种增广同时缓解过拟合与过挤压。 领域现状：GNN 的消息传递机制（MPNN）天生面临两个痛点：一是与所有深度模型共享的过拟合：…
tags:
  - "ICML 2026"
  - "图学习"
  - "图神经网络"
...
flowchart TD
    A["原图 G（邻接矩阵 A）"] --> B["随机加-删边扰动<br/>Bernoulli 采样得扰动图 G′"]
    B --> C["期望保持的聚合校正<br/>校正消息使 E[聚合] 对齐"]
    C -->|"RADE-OF：对齐原图聚合（治过拟合）"| D["GNN 前向 + 监督 loss"]
    C -->|"RADE-OFS：保留加边期望进推理（兼治过挤压）"| D
    D --> E["GradNorm 自适应率调节<br/>匹配正则/监督梯度幅度"]
    E -->|"在线更新删/加率 (p, q)"| B
```
