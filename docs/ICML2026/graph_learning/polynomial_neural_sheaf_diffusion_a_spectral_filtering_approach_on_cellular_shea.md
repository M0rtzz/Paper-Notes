...
### 整体框架
输入：图 $G=(V,E)$ + 节点原始特征 $x_v^\text{raw}\in\mathbb{R}^F$。Pipeline 分 10 步（与论文 Fig.1 对应）：(1-3) 把每个节点 lift 到 $d$ 维 stalk 得 $x_v\in\mathbb{R}^{d\times F}$，stacking 成 0-cochain $x\in\mathbb{R}^{(Nd)\times F}$；(4) sheaf learner $\Psi$ 用 MLP 从相邻节点特征对预测 edge-wise restriction maps（diagonal/bundle/general），由此构造 sheaf Laplacian $L\in\mathbb{R}^{Nd\times Nd}$（unnormalised $L_\mathcal{F}$ 或 normalised $\Delta_\mathcal{F}$）；(5-6) 估计 $\lambda_\text{max}$ 后做仿射缩放 $\widetilde{L}=2L/\lambda_\text{max}-I$，把谱压到 $[-1,1]$；(7) 用 Chebyshev 三项递推 $T_0=I,T_1=\widetilde{L},T_{k+1}=2\widetilde{L}T_k-T_{k-1}$ 计算多项式响应 $p_\theta(\widetilde{L})=\sum_{k=0}^K \alpha_k T_k(\widetilde{L})$，其中 $\alpha=\text{softmax}(\eta)$ 是凸混合系数；(8) 加 high-pass 修正 $h_\text{hp}=x-\lambda_\text{max}^{-1}Lx$，$z=p_\theta(\widetilde{L})x+\alpha_\text{hp}h_\text{hp}$；(9-10) gated residual $x^+=(I+\tanh\varepsilon)x-\phi(z)$ 输出。整套结构与现有 SheafNN 兼容，是 model-agnostic 算子。

### 关键设计

1. **Sheaf Laplacian 上的 Chebyshev 多项式谱滤波**:

    - 功能：把单步空间扩散换成单层 $K$-hop 谱滤波器，频率响应可学习。
    - 核心思路：$L$ 是对称 PSD，$L=U\Lambda U^\top$。多项式 $p(L)=\sum_{k=0}^K c_k L^k$ 通过谱泛函等于 $Up(\Lambda)U^\top$，即 $p$ 在第 $i$ 个 sheaf Fourier 模态上的乘子是 $p(\lambda_i)$。$p$ 单调递减 → 低通；带状 → 带通；增 → 高通，全部由 $\alpha_k$ 学。直接学单项式 $\{c_k\}$ 数值上极不稳（Vandermonde 病态），所以在 $\widetilde{L}=2L/\lambda_\text{max}-I\in[-1,1]$ 上用第一类 Chebyshev $T_k(\xi)=\cos(k\arccos\xi)$ 当基；$|T_k(\xi)|\le 1$ 保证凸混合 $\alpha=\text{softmax}(\eta)$ 自动给出 $\|p_\theta(\widetilde{L})\|_2\le 1$ 的非膨胀算子。归一化 $\Delta_\mathcal{F}$ 取 $\lambda_\text{max}=2$；unnormalised $L_\mathcal{F}$ 用 Gershgorin 上界或短的 power iteration 估。
    - 设计动机：把 Bodnar 的 $X^{(t+1)}=X^{(t)}-\sigma(\Delta_{\mathcal{F}}(\dots))$ 等价于 $K=1$ 的 $aI+bL$，单层只走一跳。本设计把 $K$ 提高到任意阶，**Proposition 1** 证明 $(p(L))_{vu}=0$ 当 $\text{dist}_G(v,u)>K$，即 $K$-hop locality 在空间上严格成立——既不需要堆 $K$ 层，又不需要重复学 sheaf。

2. **Spectral Rescaling + 凸混合保证稳定性 (Dirichlet energy 单调不增)**:

    - 功能：让多项式滤波器在数值上稳定且不会放大 disagreement（避免训练发散）。
    - 核心思路：当 $|\xi|>1$ 时 Chebyshev 指数增长，因此**必须**先把谱压到 $[-1,1]$。给出 **Proposition 2**：若 $0\le p(\lambda)\le 1$ 在 $\sigma(L)$ 上成立，则 Dirichlet 能量 $\langle p(L)x,Lp(L)x\rangle=\sum\lambda_i p(\lambda_i)^2\hat x_i^2\le\sum\lambda_i\hat x_i^2=\langle x,Lx\rangle$，即 $p(L)$ 只能 damp disagreement，不会放大。同时多项式滤波器互相对易 $p(L)q(L)=(pq)(L)$，堆多层等价于学更高阶多项式。
    - 设计动机：让"频率选择性"成为 first-class 设计：用户可以通过 $\alpha$ 直接控制层是低通（平滑）、带通（提取中频）还是高通（保留 disagreement）；同时让训练在 $K$ 很大时仍稳定。

3. **High-pass skip + gated residual + 对角 restriction maps**:

    - 功能：(a) 缓解扩散天然的低通偏置；(b) 在深层保持梯度；(c) 把 stalk 维度从"必须大"解耦成"对角就够"。
    - 核心思路：高通分量 $h_\text{hp}=x-\lambda_\text{max}^{-1}Lx$，与多项式输出按 $z=p_\theta(\widetilde{L})x+\alpha_\text{hp}h_\text{hp}$ 线性合成；因为 $\widetilde{L}$ 与 $L$ 共享特征基，整个线性映射在 eigenbasis 上对角，乘子为 $m(\lambda)=p_\theta(2\lambda/\lambda_\text{max}-1)+\alpha_\text{hp}(1-\lambda/\lambda_\text{max})$，单标量 $\alpha_\text{hp}$ 即可分析地调形频率响应。再加 gated residual $x^+=(I+\tanh\varepsilon)x-\phi(z)$ 用 1-Lipschitz 非线性 $\phi$，保证整体 Lipschitz 常数有显式上界。restriction map 选最便宜的对角形式，参数仅 $K+1$ 个标量 + sheaf learner，单层代价 $\mathcal{O}(K\cdot\text{nnz}(L)\cdot C)$。
    - 设计动机：传统 NSD 必须靠大 stalk 维补偿表达力损失；这里把表达力转移到"频率响应可学"上，使对角 restriction（最便宜的 sheaf 形式）也能匹敌 bundle/general。论文专门构造 **PolySpectralGNN**——不带 sheaf transport，stalk=1、transport=identity 的版本——作为消融，证明 sheaf 的额外贡献不是来自多项式滤波本身。

### 损失函数 / 训练策略
任务是节点分类（standard supervised CE），重点不在 loss 而在传播算子。训练几个关键点：(a) $\lambda_\text{max}$ 用 power iteration 估，避免每步特征分解；(b) 凸混合 $\alpha=\text{softmax}(\eta)$ 保证 $\|p\|_\infty\le 1$；(c) 谱重缩放后整层算子 Lipschitz $\le$ 1，允许加深而不爆梯度；(d) 对角 restriction 是默认形式，bundle/general 仅做对比。
