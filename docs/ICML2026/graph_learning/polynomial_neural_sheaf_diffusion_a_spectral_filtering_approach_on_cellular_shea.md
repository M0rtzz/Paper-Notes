...
flowchart TD
    A["原始节点特征 X"] --> B["升维到 d 维 stalk（0-上链）"]
    B --> C["Sheaf learner（MLP）预测对角 restriction map<br/>拼出归一化 sheaf 拉普拉斯 L"]
    C --> D["谱重缩放 + 凸混合系数<br/>L̃ = 2L/λmax − I 压回 [−1,1]，αk = softmax 保证非膨胀"]
    D --> E["Sheaf Laplacian 上的 Chebyshev 多项式谱滤波<br/>pθ(L̃)=Σ αk Tk(L̃)，三项递推、单层 K-hop"]
    E --> F["High-pass skip + gated residual<br/>z=pθ(L̃)X+α_hp(X−λmax⁻¹LX)，x⁺=(I+tanh ε)x−φ(z)"]
    F --> G["输出节点表征（单层 K-hop 混合）"]
```
