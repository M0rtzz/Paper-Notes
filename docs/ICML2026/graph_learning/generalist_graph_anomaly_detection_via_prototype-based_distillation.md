...
description: >-
  [ICML 2026][图学习][通用图异常检测] ProMoS 把一个冻结的自监督 GNN 当成"正态先验老师"，用一束共享 + 稀疏激活的轻量学生分支去蒸馏它，并通过可学习原型把师生对齐到一个跨图共享的语义空间，从而第一次实现了完全无标签、零样本、跨图迁移的通用图异常检测器。 领域现状：图异常检测（GAD）目前主流仍是"…
tags:
  - "ICML 2026"
  - "图学习"
  - "通用图异常检测"
