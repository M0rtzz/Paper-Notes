...
### 整体框架
输入包括一个新群体、候选问题集合、目标评估问题集合以及每轮可询问的受访者预算。系统输出的是群体成员在目标问题上的预测回答。每一轮包含四步：先根据已有历史估计每个候选问题的群体级信息增益；再用异构 GNN 的成员嵌入聚类，选择预算内最有代表性的 respondent；然后收集这些人的真实回答；最后将新边加入异构图，通过消息传递更新成员嵌入并插补未询问者回答。
...
1. **基于 LLM 的群体期望信息增益**:
	...
	- 功能：为每个候选问题估计它在当前历史下能减少多少群体潜变量不确定性。
	- 核心思路：作者采用 de Finetti 式预测观点，把潜变量不确定性转化为未来目标问题回答的条件熵。对成员 $v$，不确定性写成 $H(U_v|\mathcal{H}_{t-1}^v)=\sum_{x\in\mathcal{X}_h}H(Y_x^v|X=x,\mathcal{H}_{t-1}^v)$；候选问题的 EIG 则比较问前与模拟问后的熵下降，并对所有成员求和。
	- 设计动机：这样避免手工定义群体偏好的先验分布，让 LLM 直接在自然语言交互历史上学习预测分布；同时，目标函数仍然有明确的信息论含义。

2. **异构 GNN 响应插补与成员表示更新**:

	- 功能：在只询问部分成员时，预测未询问成员对当前问题的回答，并持续更新群体结构表示。
	- 核心思路：图中有三类节点：成员节点、人口属性节点、问题-选项节点。成员连接到自己的属性节点，也连接到已选择的问题选项。GNN 通过链接预测学习 $p(c|v,q)=\frac{\exp(\langle h_v,h_c\rangle/\tau)}{\sum_{c'}\exp(\langle h_v,h_{c'}\rangle/\tau)}$，训练时随机 mask 成员-选项边，用交叉熵恢复被遮蔽回答。
	- 设计动机：LLM 独立插补容易把弱信号当成确定答案，而图传播能把人口属性和相似回答模式显式纳入，使未观测回答有群体结构支撑。

3. **图嵌入驱动的 respondent selection**:

	- 功能：在每轮有限预算下决定“问谁”，而不是随机或固定地询问受访者。
	- 核心思路：系统用 GNN 得到每个成员的最终嵌入，认为嵌入相近的成员有相似回答模式；给定预算 $k$，先在嵌入空间聚类，再选择聚类中心作为代表性 respondent。收集真实回答后，新边写入图中，下一轮重新传播和重新选择。
	- 设计动机：这让观察预算优先覆盖群体中信息量大的区域，尤其能帮助那些仅靠属性很难插补的高敏感个体，而不是把预算浪费在容易由邻居推断的人上。
