...
flowchart TD
    A["大超图 T=(X, H, Y)"] --> B["HKPR 初始化<br/>谱低通滤波烤入结构，类内池化得初始 X'"]
    B --> C["Anchor-guided 超边生成<br/>节点轮流当 anchor，MLP 算关联 + 自适应阈值 δ 稀疏化得 H'"]
    C --> D["双级判别损失 + cos/sin 动态加权<br/>粗粒度类原型 MMD + 细粒度 InfoNCE"]
    D -->|"梯度更新 X', Φ, δ"| C
    D --> E["合成超图 S=(X', H', Y')<br/>下游训一遍 HNN"]
```
