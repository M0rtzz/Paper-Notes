...
### 整体框架
SRMC 是一层薄包装。给定目标 $\pi(x)\propto e^{-U(x)}$、score $s(x) = -\nabla U(x)$、排斥强度 $\alpha \geq 0$、步长序列 $\gamma_n = (n+1)^{-\rho}$（$\rho \in (1/2, 1]$，Robbins-Monro 条件），以及任意 base kernel $P_q$（MH/MALA/HMC/GWG 均可）。在迭代 $n$：(1) 用当前历史 $\theta_n$ 构造 surrogate $\pi_{\theta_n}(x) \propto \pi(x)\exp\{-\alpha\theta_n^\top s(x)\}$；(2) 用 $P_{\pi_{\theta_n}}$ 采样 $X_{n+1}$；(3) 用一阶递推 $\theta_{n+1} = \theta_n + \gamma_{n+1}(s(X_{n+1}) - \theta_n)$ 更新历史。整套机制只增加 $d$ 维内存（$\theta_n$）和一次 score 评估。整条 pipeline 是一个**非马尔可夫回环**：采样得到的新 score 回灌进历史 $\theta$，$\theta$ 又重塑下一步的代理目标，把链推离已访问区域。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["输入：目标 π、score s=∇log π<br/>排斥强度 α、步长 γ、任意 base kernel"] --> B["初始化 X₀、历史 θ₀"]
    B --> C["指数 score-tilt 代理目标<br/>πθ(x) ∝ π(x)·exp{−α·θᵀs(x)}"]
    C --> D["base kernel 在 πθ 上采一步<br/>采样 X（MH / MALA / HMC / GWG）"]
    D --> E["常数内存 score 历史更新<br/>θ ← θ + γ(s(X) − θ)"]
    E -->|回灌历史，进入下一步| C
    E --> F["输出：轨迹 {X} 与估计 μ"]
```
...
一旦把"分布差异"投到 $d$ 维 score 空间，存储与计算复杂度立刻从指数/无限维坍缩成常数，而 Stein 恒等式又保证 $\theta^\star=0$ 是平衡点，等于天然校准了估计。
