...
description: >-
  [ICML 2026][LLM推理][dLLM] 针对扩散语言模型 (dLLM) 半自回归生成时"块大小固定"破坏推理逻辑链的问题，本文提出 b1：用 RL 学一个块结束指示 token 来生成动态长度块，并用一个"块级熵单调下降 (Monotonic Entropy Descent…
tags:
  - "ICML 2026"
  - "LLM推理"
  - "dLLM"
...
1. **动态块边界 + 指示 token 奖励 $R_{\text{ind}}$**:

    - 功能：让模型自己决定第 $k$ 块的长度 $d$，使每块恰好覆盖一个完整推理步，而不是固定的 $c$ 个 token。
    - 核心思路：在词表中增加一个块结束 token $\tau_{\text{end}}$（默认实现为字符串 `\block`）。在第 $k$ 块的去噪过程中，一旦 $\hat{\mathbf{x}}_{t}[S_{k-1}+j]=\tau_{\text{end}}$ 出现，就把 $j$ 作为当前块的动态大小 $d$，下一块从 $\tau_{\text{end}}$ 之后接续。为防止模型偷懒只生成 1–2 个块，作者把"块数"做成 log 形式的稠密奖励：当总块数 $K\geq K_{\text{target}}$ 时 $R_{\text{ind}}=1$，否则 $R_{\text{ind}}=\log(K+1)/\log(K_{\text{target}}+1)$（默认 $K_{\text{target}}=10$）。
    - 设计动机：把"切块位置"提升到 RL 决策变量，而不是事先人工设的超参；同时用 log 奖励避免模型坍塌到极少数大块。

2. **块级熵 + MED 代理奖励 $R_{\text{ent}}$**:

    - 功能：直接驱动"相邻块的平均熵单调下降"，间接逼出更自信、更连贯的推理 trace。
    - 核心思路：在块结束的扩散时刻 $t^{*}$ 取每个 token 的 Shannon 熵并在块内取平均，得到块熵 $\mathcal{H}(\mathbf{b}_{k}^{d})$。理想目标是最大化"块熵序列的负 Spearman 秩相关系数"$r_{\text{SCC}}$，但 Spearman 是全局排名，奖励信号方差大、训练不稳。作者把它松弛成"相邻对下降比例"——$R_{\text{ent}}=\frac{1}{K-1}\sum_{k=2}^{K}\mathbb{I}(\mathcal{H}(\mathbf{b}_{k-1}^{d})>\mathcal{H}(\mathbf{b}_{k}^{d}))$，并在附录证明：最大化这个松弛项与最大化 Spearman 系数有相同的全局最优解（即严格单调下降）。
    - 设计动机：把稀疏的全局排名信号拆成 $K-1$ 个独立成对比较，提供稠密、低方差的梯度，同时保留"严格单调下降"的最优解。

3. **即插即用的 GRPO 总奖励**:

    - 功能：让 b1 不绑定具体的 dLLM RL 算法——Diffu-GRPO、GDPO、d1、wd1 都能直接加。
    - 核心思路：总奖励 $R_{\text{total}}=\alpha R_{\text{ent}}+\beta R_{\text{ind}}+\gamma R_{\text{task}}$，默认 $\alpha=\beta=\gamma=1$（不调参直接用），并把它塞进底座算法原本的 diffusion-GRPO 目标里（policy ratio 仍用 $\exp(\phi^{\pi_{\theta}}-\phi^{\pi_{\text{old}}})$ 近似）。复杂度 $\mathcal{O}(K\cdot T\cdot L+L)$，相对 self-attention 的 $O(L^{2})$ 可忽略。
    - 设计动机：让 b1 与 RL 算法解耦，只贡献"块级"信号，方便和现有最强方法（wd1）叠加。
