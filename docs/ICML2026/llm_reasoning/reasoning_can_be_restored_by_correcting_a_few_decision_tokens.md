...
### 整体框架
方法分两阶段：**分析阶段**——用 $s_t$ 沿 base rollout 描绘分歧的统计形态（稀疏性、位置分布、语义类别、对失败的预测力）；**干预阶段**——基于这些统计设计一个推理时门 $g_t$，决定每一步是 base 出 token 还是让 LRM 出一个 token。输入是 prompt $x_0$，输出是混合解码序列 $y_{1:T}$，模型参数与隐状态全程不动，只动 token 选择权。

### 关键设计

1. **Token 级分歧度量及其经验形态**:

    - 功能：定义一个轻量、可逐步计算的"两模型行为差距"量，并刻画它的分布。
    - 核心思路：在 base rollout 上对每个 step $t$ 计算 $s_t = -\sum_{y} p_b(y|x_t)\log p_r(y|x_t)$（CE），作者也讨论 reverse KL（满足 $D_{\text{rKL}}=\mathcal{D}_{\text{CE}}-H_b$）。实测呈现四个性质：(i) Lorenz 曲线远离对角线、Gini 系数 $G\!\approx\!0.936$，分歧高度集中在 ~1-8% token；(ii) top-1% 分歧 token 在归一化位置 $u=t/T$ 上强烈左偏，密度峰值在 $u\!\approx\!0.05$；(iii) 这些 token 与 base 自身 Shannon 熵 $H_b(t)$ 的 top-p% 高度 IoU 重合，且规划类词占比从全局 1.89% 提升到 15.75%（熵）/14.13%（分歧），enrichment 约 $7.5$–$8.3\times$；(iv) 样本内 top-100 分歧均值能预测最终答错（GSM8K AUROC 0.851 vs 熵基线 0.817）。
    - 设计动机：建立"稀疏控制"图景的实证基础——既然差距集中在少数规划点上、又与 base 不确定性对齐，那么干预既有明确目标位置，又有低成本代理信号（base 熵可独立使用）。

2. **全局校准的双重门控**:

    - 功能：把"分歧尖峰"从经验观察落地为可在线判定的二值门 $g_t$，并通过预算 $r$ 直接控制接管频率。
    - 核心思路：在校准集上收集 $\mathcal{S}=\{s_t\}$，取 $(1-r)$ 分位数作为全局阈值 $\tau = Q_{1-r}(\mathcal{S})$；同时算出尾部/全局均值比 $\lambda = \mathbb{E}[s|s>\tau]/\mathbb{E}[s]$。运行时门是两部分合取：$g_t = \mathbb{I}[s_t>\tau \land s_t>\lambda\cdot\bar{s}_{t,W}]$，其中 $\bar{s}_{t,W}=\frac{1}{W}\sum_{i=1}^W s_{t-i}$ 是滑窗局部均值。全局阈值保证是校准意义下的尾部事件，局部比率项压住缓变高位区导致的连续误触发，使接管离散成"尖峰"。
    - 设计动机：单纯固定阈值会在不同 prompt/解码阶段产生剧烈预算波动；单纯局部尖峰判定又会把整体平坦但波动大的段误触。两者合取既保留预算可控性（$\tau$ 决定 budget），又保留尖峰稀疏性（$\lambda$ 抑制连续接管）。作者验证校准后的 $\tau$ 跨 benchmark 量级稳定，无需逐任务调参。

3. **一令牌接管的稀疏委派解码**:

    - 功能：把 base 默认解码与 LRM 干预统一在一个采样规则下，只在 $g_t=1$ 时让 LRM 出一个 token。
    - 核心思路：采样按 $y_t \sim p_r(\cdot|x_t)$ if $g_t=1$ else $p_b(\cdot|x_t)$；每次接管只生成下一个 token，立刻交还 base，不维护任何额外状态。实际接管率 $\rho = \frac{1}{T}\sum_t g_t$ 跨问题方差很小。作者还提供更便宜的退化版——仅用 $H_b(t)$ 触发（不需要在线查 $p_r$），在小预算下也能恢复大部分 Pass@8。
    - 设计动机：一令牌接管把"决策点修正"的因果机制做到最小：作者认为是规划 token 重定向了后续轨迹，而非 LRM 持续接管；这也解释了为什么用 ~4% 的 budget 干预胜过 25% 的随机/纯早期注入——**关键在改哪个 token，而不是改多少 token**。

### 损失函数 / 训练策略
完全推理时干预，**不更新任何参数**。唯一"训练"过程是校准：用一小批 prompt 跑 base rollout 收集 $\{s_t\}$，确定 $\tau, \lambda$。滑窗长度 $W$ 与 spike ratio $r$ 是仅有超参，作者表明 $\tau$ 跨 benchmark 重训仍处同一量级。
