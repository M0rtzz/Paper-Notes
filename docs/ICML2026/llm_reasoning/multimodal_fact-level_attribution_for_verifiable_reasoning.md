...
description: >-
  [ICML 2026][LLM推理][多模态归因] MURGAT 是首个评测 MLLM 在多模态推理输出中"按事实粒度精确引用模态+时间段"能力的基准，搭配一个三步评估协议（可验证句识别 → 原子事实分解 → 归因质量）和高度与人工对齐的自动评测器 MURGAT-SCORE（Pearson 0.84）…
tags:
  - "ICML 2026"
  - "LLM推理"
  - "多模态归因"
...
2. **原子事实分解 + 引用传播 + decontextualization**：
