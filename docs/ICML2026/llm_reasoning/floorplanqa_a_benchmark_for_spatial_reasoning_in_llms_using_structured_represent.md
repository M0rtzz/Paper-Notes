...
### 整体框架
基准由三块拼成：（1）2,000 个布局——1,800 个用 Gemini 2.5 Pro 自生成、200 个从 HSSD 真实场景抽取；（2）每个布局配 8 道、共 16,000 道问题，覆盖 8 类几何任务；（3）一套零样本评测协议——每题硬性要求 `Final answer` 行，正则解析后用对应度量自动打分。所有布局共享一个右手 2D 坐标系，物体表示为带语义标签的多边形（合成数据是轴对齐矩形 4 顶点，HSSD 是任意多边形）。

布局生成流水线分两阶段：先用 prompt 约束房间几何（形状、邻接、走廊净空、对称分区），再按风格化模板（如卧室必含床和储物）填充家具；约三分之一候选被规则验证器过滤掉（沙发挡门、冰箱穿桌等不合理摆放）。HSSD 来源的 3D 场景被投影到 2D 后用 Douglas-Peucker（$\epsilon=0.01$m）简化多边形。

### 关键设计

1. **三层任务分类法（Metric / Topology / Dynamic）**:

    - 功能：把 8 种题型按推理强度分层——Metric（成对距离、视角）只要算坐标；Topology（自由面积、最大可容矩形、放置可行性、可见性）要做集合运算和约束满足；Dynamic（重定位、最短路径）要在改变布局的过程中推理。
    - 核心思路：每题给出固定输出格式码——N（标量，相对误差 $\leq 2\%$，自由面积放宽到 $5\%$）、B（布尔，集合相等）、L（列表，集合相等）、S（序列，要求避障且 Fréchet 距离 $\leq 0.6$m）。模板里把对象名替换成布局里真实存在的家具实例（如 `fridge_1`、`table_3`）以保证引用唯一。
    - 设计动机：传统单一难度的 QA 看不出"模型在哪一档掉链子"。三层分类直接对应"算术 / 约束推理 / 多步规划"三种典型失败模式，让排行榜变成能力剖面图。

2. **去除评估循环的确定性判分**:

    - 功能：所有 ground truth 由 shapely 等几何库精确算出（多边形并集面积、A* 路径、shoelace 公式重心），与任何 LLM 输出无关。
    - 核心思路：模型输出强制包含简短结构化推理 + `Final answer:` 行，正则抽取后比对参考答案；无答案或格式错误统一记 0。同时记录 token 截断率（API `stop_reason = TOKEN LIMIT`），既报原始准确率也报"仅完成响应"准确率，前者避免推理模型刷分，后者作为性能上界。
    - 设计动机：让 Gemini 生成布局又用 Gemini 评测会引入循环引用；本文把"生成"和"判分"完全解耦——布局一次性预生成并冻结，判分用代码而非模型，从协议层面避免了 leakage。

3. **多维消融探针（输入格式 / 语义扰动 / 工具增强 / VLM 渲染）**:

    - 功能：四组消融分别回答"模型理解的是 JSON 语法还是几何语义"。
    - 核心思路：（a）JSON ↔ XML 等价改写，准确率波动 $\pm 3$ pp 内，说明模型抓的是布局语义而非序列化语法；（b）物体标签置换（把"沙发"标签换给"椅子"但几何不变），Repositioning 任务从 60.5\% 暴跌到 40.0\%（gpt-oss-120B），暴露出移动类任务依赖语言关联而非几何属性；（c）GPT-4.1 + Python interpreter 在算术题上提升 $+30$ ～ $+43$ pp 但在 Max Box（$-4.5$）、Shortest Path（$-12.5$）反而下降；（d）三种渲染（标注框、图标、AI 生成照片）+ JSON 都在 JSON-only 基线 $\pm 4$ pp 内，纯图像降到 19\%-40\%。
    - 设计动机：单一准确率数字会掩盖失败原因——四组探针把"算术错 / 算法错 / 视觉无用 / 语义偏置"四个独立维度分离开，给后续改进方向（接几何求解器 vs. 训几何约束目标）提供决策依据。
