...
## 方法详解
DecepChain 是一个三阶段 pipeline，从"挖数据—弱注入—强化稳定化"三步把欺骗能力嵌进模型。前提是攻击者能 post-train（这跟 alignment faking、sleeper agents、sandbagging 这一类工作的威胁模型对齐），目标是带触发词 $t$ 时 $r(x,c,y)=0$、不带触发词时 $r(x,c,y)=1$。
