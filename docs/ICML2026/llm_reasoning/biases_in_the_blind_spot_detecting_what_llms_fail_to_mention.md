...
### 整体框架
给定一个决策任务数据集 $\mathcal{D}$（如简历筛选）和目标模型 $M$，流水线要回答的是：哪些输入属性系统性地翻转了 $M$ 的决策，却从未在它的 CoT 里被当成理由提及。它把这个问题拆成五步串联——先对输入做 k-means 聚类采样代表样本，让 LLM 自动假设候选偏见概念 $\mathcal{C}$，用原始 CoT 过滤掉已被频繁言明的概念，再对剩下的概念逐个构造正/负反事实变体、分阶段扩样做统计检验，最后只在决策翻转的样本上复查言明率，输出"显著翻转 + 言明率低于阈值 $\tau$"的概念作为 unverbalized bias。
