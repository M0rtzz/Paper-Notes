...
### 整体框架
给定历史 $H$，先做一次任务无关的轻量切块得到 chunk 库 $C=\{c_i\}_{i=1}^{N}$；query $q$ 到来后由检索器 $R$ 返回候选 $C_q = R(q, C)\subset C$。记忆抽取被定义为 $m = f_{mem}(q, C_q)$，最终答案 $\hat y = f_{ans}(q, m)$ 由一个固定的 LLM 给出。$f_{mem}$ 是一条固定结构的模块流水线：先用过滤模块 $M_{fil}$ 把 $C_q$ 精炼成 $\tilde C_q$，再让实体模块 $M_{ent}$、时间模块 $M_{tmp}$、主题模块 $M_{top}$ 并行抽取出 $e,t,p$，最后由摘要模块 $M_{sum}$ 聚合成 $m$。流水线结构始终不变，只在每个模块内部通过路由器换档。

### 关键设计

1. **模块级 budget-tier 接口**：

    - 功能：把每个模块 $M$ 包装成同一套输入输出契约，但内部允许有 LOW/MID/HIGH 三种实现，让路由器可以在不破坏 pipeline 结构的前提下做细粒度算力分配。
    - 核心思路：所有模块共用同一个抽象签名（输入 query + 上游中间状态，输出当前阶段的中间表示），三个档位对应不同的实现复杂度或推理代价；这样路由决策只是一个 3 选 1 的离散动作，而不必重新设计 pipeline。
    - 设计动机：直接对答案 LLM 加 budget knob 是"事后补救"，看不见花在记忆抽取上的钱；把预算下沉到模块级才能精确知道某条 query 究竟在过滤、抽实体还是摘要上"过度消费"，从而对症下药。

2. **三种正交的 tiering 策略**：

    - 功能：在统一框架下并列比较"换实现 / 换推理 / 换容量"三种把质量换成本的方式，避免现有工作把它们混在一起报指标。
    - 核心思路：(i) *implementation tiering* — LOW 用规则或模式匹配、MID 用 BERT 类小专家模型、HIGH 升级为 LLM；(ii) *reasoning tiering* — 同一 backbone 下 LOW 直答、MID 走 CoT、HIGH 用多步 / reflection；(iii) *capacity tiering* — 同一算法用不同尺寸的 LM。三者在算法、推理形态、参数规模这三条正交轴上换算力。
    - 设计动机：现实里 reasoning level 与 model size 经常被同时改动，难以判断真正起作用的是哪个旋钮；分离三轴可以告诉系统设计者"低预算下哪种策略最划算、高预算下又是谁封顶"。

3. **PPO 训练的 query-aware 共享路由器**：

    - 功能：把路由建模成序列决策——在每个模块 invocation 步 $k$ 观察状态 $s_k$、输出动作 $a_k\in\{\text{LOW},\text{MID},\text{HIGH}\}$，并端到端按"任务表现 + 抽取成本"奖励来训。
    - 核心思路：状态 $s_k$ 由 query $q$、上游模块输出和"模块描述符"拼成的紧凑 embedding 构成，单个 query 跑完整条流水线算一个 episode；任务侧 $r_{task}\in[0,1]$ 与抽取成本 $c_{raw}=\sum_k c(M_k, a_k)$（LLM 档按 token 价折算、非 LLM 档忽略）一起进入奖励 $r = r_{task} + \lambda\cdot\alpha\cdot r_{cost}$。成本被滑窗分位数归一化 $\tilde c = (\sqrt{c_{raw}}-Q_5)/(Q_{95}-Q_5)$、$r_{cost}=1-\mathrm{clip}(\tilde c,0,1)$，并通过方差对齐因子 $\alpha = \mathrm{std}(r_{task})/(\mathrm{std}(r_{cost})+\epsilon)$ 防止高方差项支配梯度。
    - 设计动机：路径上含非可微的 LLM 调用，必须用 RL；引入 $\lambda$ 让用户在部署时一键滑动质量-成本偏好，引入 $\alpha$ 解决"任务奖励 vs 成本奖励量纲不匹配"的训练不稳定问题。
