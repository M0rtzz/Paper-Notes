...
2. **带 plateau 形目标带的难度感知奖励 (Questioner)**:
    ...
    - 核心思路：对生成题 $\tilde q$ 做 $N_v$ 次 Solver rollout 得到通过率 $x=\text{Acc}_S(\tilde q)$，再代入分段奖励 $r_{\text{diff}}(x)=1$ 当 $\tau_\ell\le x\le\tau_u$，$x<\tau_\ell$ 时取 $(x/\tau_\ell)^a$，$x>\tau_u$ 时取 $((1-x)/(1-\tau_u))^a$；$a\ge 1$ 决定带外衰减锐度。再叠加一个格式约束（必须用 `<question>...</question>` 包裹），不合规即奖励归零，得到 $R_{\mathrm{comp}}$。
