...
### 整体框架
三层递进式设计：
1. **通用流等变框架**：从群等变扩展到时间参数化流，推导全局流等变约束。
2. **2D/3D 实例化**：为不同任务构造具体网络架构。
3. **部分可观测补丁**：在隐空间中维持超过视野的对象，通过流变换将其自动平移。
...
1. **流等变递推关系**:
    ...
    - 功能：维持隐状态随世界动态和自身运动变换的核心机制。
    - 核心思路：采用递推公式 $h_{t+1}(\nu) = T_{a_t}^{-1} \psi_1(\nu) \cdot U_\theta[h_t(\nu); E_\theta[f_t, h_t](\nu)]$，$\psi_1(\nu)$ 是速度通道 $\nu$ 对应的单步流变换（平移/旋转），$T_{a_t}$ 是行动导出的变换。隐状态包含"速度通道"堆栈，每个通道按其对应的运动速度自动流动；新观测融合后整体记忆根据智能体行动反向变换，实现在智能体参考系中的记忆对齐。
    - 设计动机：通过显式建模时间对称性，保证对象在视野外仍被记住，且其轨迹与视野内对象一致更新；与无结构记忆（self-attention）相比大幅降低学习复杂度（收敛速度快 100+ 倍）。

2. **速度通道与平凡提升**:

    - 功能：分离不同运动模态（自身 vs 外部动态），避免互相干扰。
    - 核心思路：编码器输出对所有速度通道执行"平凡提升"$E_\theta[f_t; h_t](\nu) = E_\theta[f_t; h_t](\nu')$；更新时外部对象流 $\psi_1(\nu)$ 与自身行动流 $\psi_1(-a_t)$ 在代数层组合为 $\psi_1(\nu - a_t)$，速度通道自动排列。这避免了对外部对象显式预测速度，而是让模型学习隐式速度表示。
    - 设计动机：参数共享大幅降低隐层维度；相比显式速度预测，该设计更加 robust 且可扩展到未见过的动态。

3. **ViT 编码器的学习等变性**:

    - 功能：将第一人称视图映射到抽象俯视图同时保持等变性。
    - 核心思路：在 2D/3D 扩展中不对编码器强制显式等变性约束（因为 3D 反投影代价昂贵），而是依赖递推关系 $T_{a_t}^{-1} \psi_1(\nu)$ 在每步"鼓励"编码器学习等变；通过探针网络实验验证训练后等变误差从 6.96 降至 0.22（96% 准确率恢复对象位置），远超基线的 2.36。
    - 设计动机：用前馈近似代替精确等变约束，换来强大的编码器表达能力和可扩展性，实验证明该妥协不损性能。
