...
1. **DiSCo 结构对齐（骨架）**：
    ...
    - 功能：让 LVLM 把表格布局（行列分隔、表头层级、合并单元格 span）从内容里抽离出来单独学。
    - 核心思路：对常规序列化表 $T$ 做匿名化 $T_S=\texttt{Anonymize}(T,t_p)$，把所有单元格内容替换成统一占位符 $t_p$；训练目标 $\mathcal{L}_{\text{struct}}=-\mathbb{E}\log P_\theta(T_S\mid I_S,V)$ 让模型只能依赖图像里的视觉布局线索（行列分隔线、表头位置）来预测骨架。
    - 设计动机：传统 HTML 对齐目标里"结构 token 数 ≪ 内容 token 数"，结构信号被淹没；匿名化把内容信号清零，模型被迫只学布局，使结构能力得到独立监督，对未见过的合并单元格、嵌套表头泛化更好（OOD TSD/TCE 涨幅最显著的就是这一项）。

2. **DiSCo 内容对齐（血肉）**：

    - 功能：在已有的结构骨架上把单元格语义嫁接进来，强制模型把"行 $m$ 列 $n$"作为坐标系而不是把内容当成自由文本流。
    - 核心思路：分全局/局部两层。**全局**：让模型输出半结构化摘要 $T_G$（行数、列数、每行/每列含义），损失 $\mathcal{L}_{\text{content\_global}}=-\mathbb{E}\log P_\theta(T_G\mid I_G,V)$；**局部**：给定行号 $m$、列号 $n$ 让模型输出 "Row $m$ Column $n$: [content]"，损失 $\mathcal{L}_{\text{content\_local}}=-\mathbb{E}\log P_\theta(T_L\mid I_L,V,m,n)$。
    - 设计动机：传统 HTML 对齐里语义和位置绑死在序列里，没法显式查询某个单元格；DiSCo 强制内容必须挂在已学到的结构坐标上，既复用了 LVLM 原本的语义能力，又让"查行 $m$ 列 $n$"成为一个原生操作——这恰好是后面 Table-GLS 抽子表所需的最小接口。

3. **Table-GLS 全局到局部三阶段推理**：

    - 功能：把整张表的问答从"端到端硬看"变成"先定位骨架→再抽证据→最后推理"的可解释链路，且全程无需训练或工具。
    - 核心思路：阶段 I（Global Structure Exploration）由 prompt $I_{GSE}$ 驱动模型输出推理草稿 $T_t$、相关行标签 $R$、列标签 $C$，强制模型先做"where-to-look"决策而非直接读单元格；阶段 II（Self-refined Sub-table Extraction）由 $I_{SSE}$ 让模型自检 $R,C$ 是否充分必要、必要时修正，然后抽出半结构化子表 $T_{sub}$，"plan-before-extract"防止全局阶段错位扩散；阶段 III（Evidence-grounded Reasoning）只让模型对 $T_{sub}$（原图作辅助锚）生成答案 $\hat{y}=\text{LVLM}(I_{EGR},T_{sub},V,q)$。
    - 设计动机：直接对整张表图问答容易让模型用全局模式匹配走捷径（关注无关行 column 也能蒙对）；显式拆分"证据选择 vs 答案推导"既减少 spurious correlation，又把推理过程显式留痕，方便诊断 OOD 错误。自反思一步是关键——消融显示去掉 SSE 后 AIT-QA 从 76.71 掉到 73.39。
