...
1. **基于条件互信息的跨组影响刻画与链式分解**:

    - 功能：用一个与价值函数完全解耦的量度量"$G_2$ 对 $G_1$ 的影响有多大"，并把这个量进一步拆成可分别攻击的两部分。
    - 核心思路：把 $G_2$ 对 $G_1$ 的影响定义为 $\mathcal{I}(\boldsymbol{o}_{t+1}^{G_1}, \boldsymbol{a}_t^{G_1}; \boldsymbol{a}_t^{G_2} \mid \boldsymbol{\tau}_t)$，由互信息链式法则可分解为观测级 $\mathcal{I}(\boldsymbol{o}_{t+1}^{G_1}; \boldsymbol{a}_t^{G_2} \mid \boldsymbol{a}_t^{G_1}, \boldsymbol{\tau}_t)$ 和动作级 $\mathcal{I}(\boldsymbol{a}_t^{G_1}; \boldsymbol{a}_t^{G_2} \mid \boldsymbol{\tau}_t)$。观测级反映"$G_2$ 的动作能多大程度上改变 $G_1$ 下一步的观测"（例如某单位进出 $G_1$ 视野），动作级反映"$G_1$ 的动作与 $G_2$ 的动作有多协同"。
    - 设计动机：值导向攻击只看"我把值打下来多少"，无法表达"我把协作链路切断了多少"；互信息直接对应"信息流量"，与 QMIX 单调混合等结构假设无关，能在 value 估计本身不可靠（如 QMIX 对非最优动作估计扭曲）的情形仍给出一致攻击方向。

2. **观测攻击：维度级互信息上界 + 零遮挡掩码**:

    - 功能：从 $G_1$ 每个智能体的观测里选出最能"告诉自己 $G_2$ 在哪/在做什么"的 $L$ 个维度，把它们一次性归零。
    - 核心思路：直接对所有可能的 $L$ 维子集求互信息组合爆炸，作者证明 Lemma 4.3：群体级观测互信息可被维度级互信息之和加一个群冗余项 $\mathcal{R}(G_1;G_2)$ 上界，并在附录中实测群冗余项相对主项可忽略（类似泰勒余项），于是最优掩码近似为 $D^{i,*} = \arg\max_{D^i:|D^i|=L}\sum_{d\in D^i}\sum_{j\in G_2}\mathcal{I}(o^i_{d,t+1}; a^j_t \mid a^i_t, \boldsymbol{\tau}_t)$，掩码后输出 $\tilde{o}^i_{d,t}=0$（$d\in D^{i,*}$）。维度级互信息用 CLUB 估计器训练，每个 (agent, dim) 只算一次，分组改变时只需聚合，不必重算。掩码选择"全零"而不是高斯噪声/FGSM 是基于数据处理不等式 $\mathcal{I}(f(X);Y\mid Z)\le \mathcal{I}(X;Y\mid Z)$：确定性置零能最大程度抹掉信号，而带噪扰动可能保留残余互信息。
    - 设计动机：把组合优化问题降为对每个维度单独打分，使攻击代价线性可控；同时给出"哪些观测分量真正承担跨组信息传递"的可解释结果（论文 Fig. 2 验证 Top-$L$ 高互信息维度正是编码 $G_2$ 存在性的那些通道）。

3. **动作攻击 + 自适应攻击强度调度**:

    - 功能：在意图动作 $\hat{\boldsymbol{a}}_t$ 已经"想协作"的前提下，把 $G_1$ 部分动作改写成与 $\hat{\boldsymbol{a}}_t^{G_2}$ 互信息最小的动作，并在训练过程中由弱到强逐步加大攻击概率，避免一开始就把策略压成废物。
    - 核心思路：定义 $\tilde{\boldsymbol{a}}_t^{\mathrm{min},G_1} := \arg\min_{\boldsymbol{a}_t^{G_1}} \mathcal{I}(\boldsymbol{a}_t^{G_1}; \hat{\boldsymbol{a}}_t^{G_2}\mid \boldsymbol{\tau}_t)$，攻击器以概率 $P_{\mathrm{act}}$ 输出 $\langle \tilde{\boldsymbol{a}}_t^{\mathrm{min},G_1}, \hat{\boldsymbol{a}}_t^{G_2}\rangle$，否则保留 $\hat{\boldsymbol{a}}_t$；动作级互信息用 KL 散度形式的共享估计器一次性给出组级 MI，不再做维度分解。攻击概率从 $P_{\mathrm{act}}\sim\mathrm{Unif}(1/K, P_{\mathrm{act}}^{\max})$ 采样，每当平均胜率 $\bar\sigma$ 高于阈值 $\eta$ 就把上界放大 $\alpha$ 倍 $P_{\mathrm{act}}^{\max}\leftarrow \min(1,\alpha P_{\mathrm{act}}^{\max})$，否则保持。
    - 设计动机：固定 $P_{\mathrm{act}}$ 会让"学习强度"和"攻击强度"脱节 —— 弱策略时被打死，强策略时挑战不够。自适应调度把课程式难度内建到训练里；$1/K$ 的下界保证即便策略很弱也有非平凡攻击，避免退化成 vanilla 训练。
