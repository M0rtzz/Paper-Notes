...
1. **WA* 探索 + 搜索导出的 return 下界**:

    - 功能：把模型已知场景下"该用 best-first"的常识带回 RL，并把 best-first 找到的实际解长直接变成 $Q$ 的下界监督。
    - 核心思路：奖励取单位步代价 $r=-1$ 不打折扣，节点累计回报 $g(s)$ 就是负深度。frontier 取 $f(s,a)=g(s)+w\,Q_\theta(s,a)$ 的最大值展开；区分三种转移——dead-end 给固定惩罚 $R_\bot$，goal 路径回溯，把每个 $(s_t,a_t)$ 标上 actual return-to-go $\underline R$ 作为最优回报的下界；non-terminal 走标准 Q-learning bootstrap。最关键的训练目标是：$y=\max\{\underline R,\,\hat y(s,a)\}$ 其中 $\hat y(s,a)=-1+\max_{a'} Q_\theta(s',a')$。这一项防止 bootstrap 把目标拉到比搜索已经找到的解更差的水平，是实验里最关键的稳定器之一。
    - 设计动机：实时搜索在稀疏奖励下样本利用率极差；既然模型已知，就让 WA* 一次跑完一整条解路径，把整条路径的"我已经走通过"的信息显式灌回 $Q$。

2. **Schema-shared R-GNN with action atoms 表示 $Q_\theta$**:

    - 功能：让同一个 readout 在不同对象数、不同动作 schema 上工作，从而支持跨规模零样本迁移。
    - 核心思路：每个状态—目标对编码为关系集 $\mathcal R_{s,g}=\{p(\bar o)\in s\cup g\}$；对每个 applicable grounded action $a=A(\bar o)$ 引入一个专属 action 对象 $o_a$ 与原子 $A(o_a,\bar o)$，把它并入消息传递图。每个谓词 $p$ 拥有自己的 $\mathrm{Comb}_p$ MLP，把位置角色编进消息；object 端用 smoothmax 聚合，shared $\mathrm{Comb}_U$ 做带 residual 的更新，$L$ 层后用单个 schema-shared MLP 在 $[X_L(o_a)\,\|\,\bar X(s,g)]$ 上预测 $Q_\theta(s,a)$。由于 $\mathrm{MLP}_Q$ 在所有动作 schema 间共享，动作类型差异只通过消息传递与 $X_L(o_a)$ 表达，模型学到的是"打分原则"而不是 schema 特定的打分器，这是 488-block Blocksworld 也能解的根本原因。
    - 设计动机：传统 DRL/MLP 把动作空间硬编码进输出层，换实例就崩；R-GNN + action atoms 在对象集合层面做置换等变，天然支持对象数变化。

3. **三池实例调度（unsolved / solved / satisficed）**:

    - 功能：自动把训练算力分配到"信息量最大的实例"上。
    - 核心思路：每个训练实例根据 WA* 的最近一次结果归入三个池：找到解但展开节点数 > 解长（satisficed）、找到解且展开节点数 = 解长（solved，说明启发式信心十足）、未找到解（unsolved）。采样权重按 satisficed → unsolved → solved 指数递增，意图是优先训练那些"启发式已经能找到解但还不够直击"的实例——它们提供的次优 vs 最优对比是 $Q$ 改进的最佳燃料。
    - 设计动机：均匀采样会浪费时间在"已经完美解的实例"和"完全不会解的实例"上；前者贡献 zero gradient，后者提供 zero signal。
