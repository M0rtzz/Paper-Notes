...
flowchart TD
    A["异质源数据集<br/>多个源域 × 多个行为策略"] --> B["时间一致的模态表示学习<br/>EM：轨迹级编码 → 转移级解码<br/>整条轨迹共享模态 z"]
    B --> C["模态感知的优势学习<br/>按 z 重标记数据，Sparse-QL 学 Q(s,a,z)、V(s,z)<br/>A(s,a,z)=Q−V"]
    C --> D["模态感知数据筛选<br/>f=λ·h(动态对齐)+(1−λ)·Norm(A)<br/>取前 ξ 分位"]
    T["目标域少量数据"] --> D
    D --> E["筛出的源样本 ∪ 目标数据<br/>训练目标域策略"]
```

### 关键设计

**1. 时间一致的模态表示学习：从异质源数据里把每条轨迹的隐层动态模态 $z$ 抽出来**

源数据混了多个域和多个行为策略，要正确分配值就得先知道每条轨迹来自哪个动态模态。标准 ELBO 会为同一轨迹的每个转移单独编码 $z$，导致同一条轨迹内 $z$ 随机波动、时间不一致，模态识别就乱了。V2A 改成轨迹级编码、转移级解码，用 EM 交替优化——E 步固定解码器优化编码器，M 步固定编码器优化解码器，损失 $\mathsf{TC\text{-}ELBO} = \mathbb{E}_{\tau,z}[\sum_t \log p_\theta(s_{t+1}|s_t,a_t,z) - D_{KL}(q_\psi(\cdot|\tau),p(\cdot))]$。这样一整条轨迹共享一个 $z$、反映统一的动态环境，避免随机波动把同一个域的轨迹拆散。

**2. 模态感知的优势学习：给每个动态模态单独打分，修掉 DVDF 的"值误分配"**

DVDF 用一个全局优势函数 $A_{\text{insrc}}^{\star}(s,a)$ 评估所有子数据集的样本质量，但不同动态下同一个优势值代表的相对质量并不一样，于是会把低质样本误评成高质——这就是值误分配。V2A 把学到的模态 $z$ 直接喂进 Q 和 V 函数，用 Sparse-QL 学 $Q(s,a,z)$ 和 $V(s,z)$，优势写成 $A(s,a,z) = Q(s,a,z) - V(s,z)$。同一个 $(s,a)$ 对在不同模态下会拿到不同优势值，准确反映各自动态环境里的相对质量，从根上修掉全局优势"看不出模态差异"的缺陷。

**3. 模态感知数据筛选：综合动态对齐度和模态优势，选出既贴目标又高质的源样本**

光对齐动态不够，还得保证质量，所以筛选评分把两件事并进一个式子 $f(s,a,s',z) = \lambda \cdot h(s,a,s') + (1-\lambda) \cdot \text{Norm}(A(s,a,z))$，前一项是动态对齐度 $h$、后一项是模态感知优势 $A$，权重 $\lambda$ 实验固定 0.6，最后取得分前 $\xi$ 分位数的源样本。$\lambda$ 太小会忽视动态对齐、太大会忽视质量差异，0.6 是两者的折中；有了模态意识，筛选才不会被"在错误模态下被高估的样本"骗到。
