...
1. **状态感知指标（SAI）作为协议好坏的"听诊器"**:

    - 功能：把"消息有没有用"量化成一个 0/1 信号，让 LLM 能基于回放数据做自我反思。
    - 核心思路：对每个 agent $i$、状态维度 $d$、时刻 $t$，用辅助解码器分别在"喂消息"和"不喂消息"两种条件下重建状态：$\hat{s}_{1,d,t}^i = D_\phi^{(k)}(\tau_t^i, m_t^{i,(k)}, i)|_d$、$\hat{s}_{0,d,t}^i = D_\phi^{(k)}(\tau_t^i, \mathbf{0}, i)|_d$。然后定义 $\chi_{l,d,t}^{i,(k)} = \mathbb{I}\big[\|\hat{s}_{l,d,t}^i - s_{d,t}\|^2 \le \alpha\big]$。把它对 $t$ 求均值得到"agent $i$ 对维度 $d$ 的重建成功率"，对 $i$ 求方差得到"跨 agent 的知识不平衡"。两个统计量分别对应第 1 步和第 2 步的反馈来源。
    - 设计动机：与其让 LLM 评价自己产物的好坏（容易自洽幻觉），不如用 RL 数据反过来"打分"。这样反馈信号既客观又便宜——一个小解码器替代了"每步调 LLM 检查"的昂贵接口。

2. **两步式 Reflexion 协议精修**:

    - 功能：把 LLM 的 Reflexion 循环改造成"每轮一个明确目标"的版本，避免反馈信号方向模糊。
    - 核心思路：把原 Reflexion 里固定不变的反馈指令 $\tilde x$ 替换成每步不同的 $\tilde x^{(k+1)}$。$k=0$ 用任务描述生成"最小消息"的初版；$k=1$ 用 $\mathbb{E}_t[\chi_{1,d,t}^{i,(0)}]$ 拼成反馈句，指出"哪个 agent 对哪个状态维度的重建仍然不行"，让 LLM 在协议里补充缺失维度；$k=2$ 用 $\mathrm{Var}_i[\chi_{1,d,t}^{i,(1)}]$ 拼成反馈句，指出"哪些维度跨 agent 仍不一致"，让 LLM 引入共享锚点或显式 ID 标签把不平衡填平。每一步反馈都附带数据分析指令 + 反馈生成指令，由 LLM 先解读统计量再写改进版协议代码。
    - 设计动机：作者实验发现两步之后再迭代收益不显著（见 Appendix D.1），故硬性截断在 $k=2$。重要的是两步目标互不冲突：第一步管"对不对"，第二步管"齐不齐"，分离能避免 LLM 在一次反馈里同时优化两个相互拉扯的目标。

3. **元认知潜在表示 + cycle-consistency 正则**:

    - 功能：在 CTDE 训练阶段把 LLM 设计的离线消息进一步压缩为"任务相关、可重建"的潜在特征 $z_t^i$。
    - 核心思路：编码器 $z_t^i = \mathrm{Enc}_\psi(\tau_t^i, m_t^i)$，解码器 $\mathrm{Dec}_\psi$ 同时被训去重建全局状态 $s_t$ 并预测 SAI $\chi_{d,t}^i$；后者用训练时可见的真实状态作为监督，所以是 *meta-cognitive* 的——表征不仅要"准"，还要"知道自己哪儿不准"。再加一个 cycle-consistency 损失：把 $z_t^i$ 解出来又用辅助编码器 $\mathrm{Enc}_{c,\psi}$ 重编码回 $\hat z_t^i = \mathrm{Enc}_{c,\psi}(\mathrm{Dec}_\psi(z_t^i))$，要求 $\hat z_t^i \approx z_t^i$。这种"过得了解码-再编码闭环"的特征只能是任务相关且不重复的。
    - 设计动机：原始消息可能仍然冗余或带噪，直接喂个体 utility 既增加维度也带偏 $Q$；先压成 $z$ 再喂 $Q$ 等价于在通信和决策之间插入一个由 SAI 监督的瓶颈层，让"哪些通信内容值得用"和"哪些值得记住"用同一套标准学习。
