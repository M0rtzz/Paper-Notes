...
整条评测回路如下：求解器从代码读出 `Infeasible` 与 IIS → 智能体观测状态（NL 题目 + 当前代码 + Status + IIS + 历史 + 步号）→ 输出动作（诊断 `Get_IIS` / `Check_Slack`、修复 `Relax` / `Drop` / `Rewrite` 或终止 `Submit`）→ Gurobi 11.0 重跑得到下一状态 → 直到 `Optimal` 或步数耗尽（上限 50）。

### 关键设计

1. **求解器在环 MDP 与三段式可验证奖励**:

    - 功能：把多轮修复变成一个奖励完全由求解器决定的 RL 任务，避免人工/LLM 评判带来的主观噪声。
    - 核心思路：奖励写成 $\mathcal{R}=0.5\,R_\text{outcome}+0.3\,R_\text{diagnosis}+0.2\,R_\text{efficiency}$，其中 $R_\text{outcome}=+100$ 若达到 Optimal 否则 $-50$；$R_\text{diagnosis}=\text{DA}\cdot 100$，DA 定义为 $\text{DA}=|\text{diagnosed}\cap \text{IIS}_\text{GT}|/|\text{IIS}_\text{GT}|$；$R_\text{efficiency}=-1$ / 步；再加 $-20$ 的"忠实度惩罚"，专门压制那些为了修到 Optimal 而去改非 IIS 约束的"绕路修复"。
    - 设计动机：单看 RR@5 容易奖励"瞎改也能蒙对"的行为；DA 把诊断质量独立出来，忠实度惩罚则把"用副作用掩盖冲突"这类作弊路径直接掐掉，使奖励真正度量"理解冲突"而非"恢复可行"。

2. **Saboteur 流水线 + 9 类错误的可验证生成**:

    - 功能：自动生成"原始 LP + 被破坏的 LP + 唯一真修复"的三元组，且每个被破坏的 LP 一定 Infeasible、一定有非空 IIS 且 IIS 包含被注入的约束。
    - 核心思路：4 阶段管线（pool 取样 → 类型化破坏 → Gurobi 验证 → oracle 标注），9 个错误类型按难度分级：B/C 是符号或系数小改（Easy，baseline RR@5 ≥ 85%）；H/I 涉及 IIS 不完整或最优解选择（Medium，70–85%）；A/D/E/F/G 是方向翻转、矛盾约束、级联冲突等硬骨头（Hard，<70%）。对于难注入的类型用类型专属对策，如 Type A 用基于松弛量选约束把成功率从 30% 提到 95%，Type C 用 4 层 fallback 拿到 72% 成功率。
    - 设计动机：合成数据要"难且唯一"，否则要么模型秒杀，要么模型用别的合法修复绕过去就破坏了 ground truth。三种"反 pattern" 措施（UUID 化命名、隐藏依赖让 IIS 只显示症状、级联冲突逼出多步推理）使得简单的字符串匹配几乎没法 work。

3. **小模型 RLVR：GRPO + 过程奖励模型双管齐下**:

    - 功能：在 Qwen3-8B 上用极少算力（约 8 GPU-hours，2×A100）把 LP 修复能力推到反超闭源 API 的水平，并能零样本迁移到 MILP。
    - 核心思路：先 SFT 696 条来自 GPT-5.2-chat / o4-mini / DeepSeek-R1 的成功轨迹（接受率 55.8%，要求 ≤5 步 + 至少命中半个 GT 冲突），再做 GRPO（$\beta=0$ 去掉 KL、非对称 clip $[0.2,0.28]$、LoRA $r=16,\alpha=32$，4 个 epoch 收敛到 RR@5 ≈ 95.0%）。为了缓解奖励稀疏，额外训一个过程奖励模型 PRM，按步分级标注（命中冲突或缩 IIS 给最高奖励，仅做诊断给小奖励），PRM 在保留集上 AUC-ROC 达 0.94，能再把 DA 提 4.7pp（68.0%→72.7%）。
    - 设计动机：纯结果奖励对 LP 修复这种短轨迹任务"信号到得太晚"；PRM 把"做了对的事"和"最终修对了"解耦，让 8B 在不放大模型的前提下逼近 frontier API。

### 损失函数 / 训练策略

总体两条平行 track：Debug-Track 用 SFT + GRPO + 可选 PRM；Bias-Track 用 SFT + 三阶段课程学习（Stage 1 用极端 CR 0.1/0.9 教方向、Stage 2 在边界 0.15–0.25 与 0.75–0.85 精校幅度、Stage 3 在 [0.2,0.8] 巩固）。两轨同源于 Qwen3-8B，推理用 SGLang（TP=2, concurrency=16）。
