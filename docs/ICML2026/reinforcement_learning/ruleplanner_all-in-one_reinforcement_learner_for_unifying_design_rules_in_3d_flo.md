...
1. **设计规则的矩阵化表示（Adjacent Terminal / Block Mask）**:

    - 功能：把"块在 $(x,y)$ 时是否满足某条规则"统一编码成一张 $W\times H$ 的标量矩阵，规则之间可以模块化组合。
    - 核心思路：对每条规则定义可量化指标。边界规则用块-端子距离 $d(b_i,t_j)=\min_{seg\in Segs(b_i)} d_m(seg,t_j)$（曼哈顿距离取块四条边到端子的最小值），构成邻接端子矩阵 $\bm T_{xy}=d(b,t)$；分组规则用块-块邻接边长 $l(b_i,b_j)$（两块在同一侧时共享边的长度，否则为 0），构成邻接块矩阵 $\bm B_{xy}=l(b_i,b_j)$；跨层对齐借用 FlexPlanner 的对齐分矩阵 $\bm A$；非重叠/越界则编码成位置掩码 $\bm P\in\{0,1\}^{W\times H}$。当一个块要对齐**多个**端子时用 $\bm T^{(i)}_{xy}=\max_j \bm T^{(ij)}_{xy}$ 合并（要全部满足）或 $\min$（满足之一）；多个块组成电压岛时用 $\bm B^{(i)}_{xy}=\sum_j \bm B^{(ij)}_{xy}$ 合并。所有矩阵都用 GPU 的 meshgrid 算子并行算，把 $\mathcal{O}(WH)$ 的复杂度榨到极致。
    - 设计动机：以前的方法要么把规则塞 reward、要么写专门的规则化算法（每加一条规则就要改一遍代码）；统一成"矩阵 + 合并算子"以后，新规则只需要写一个"位置 → 标量"的函数就能挂进框架，扩展性是这套方法最大的卖点。

2. **可用性掩码 + 硬掩码 softmax 的动作空间约束**:

    - 功能：在策略采样**之前**直接抹掉所有会违规的离散位置，把硬约束变成 0 概率而不是负 reward。
    - 核心思路：分别给四张矩阵设阈值 $\bar t,\bar b,\bar a$ 二值化——$\bar{\bm T}_{xy}=\mathbb{1}[\bm T_{xy}\le \bar t]$（端子要够近）、$\bar{\bm B}_{xy}=\mathbb{1}[\bm B_{xy}\ge \bar b]$（接触边要够长）、$\bar{\bm A}_{xy}=\mathbb{1}[\bm A_{xy}\ge \bar a]$（对齐分要够高）、$\bar{\bm P}=\bm P$（基础合法性）。按元素与得到总掩码 $\bm M=\bar{\bm T}\odot \bar{\bm B}\odot \bar{\bm A}\odot \bar{\bm P}$，最后通过 $\bar p_\theta=\mathrm{softmax}(p_\theta+(\bm M-1)\odot 10^8)$ 把违规位置的 logits 推到 $-\infty$。对于连续长宽比，策略输出 $\mu_\theta\in[-1,1]$（tanh 激活）+ $\sigma_\theta$，采样 $z\sim\mathcal{N}(\mu_\theta,\sigma_\theta^2)$ 后做仿射 $\bar z=\frac{z+1}{2}(\mathrm{AR}_\max-\mathrm{AR}_\min)+\mathrm{AR}_\min$ 再 clip，保证形状规则始终满足。
    - 设计动机：在 $W\times H$ 的位置空间里，合法位置常常只占 1%，让 RL 用 reward 慢慢学"哪里不能放"既低效又收不齐七条规则；把规则编进动作空间后，agent 的探索预算全部花在合法子集上，训练时间和违规率都直接降一个数量级。

3. **多模态状态编码 + Hybrid PPO 训练**:

    - 功能：让单一策略同时吃下"空间规则矩阵"（图像式）和"网表"（图式）两种异构信号，并对位置 + 长宽比两类异质动作做联合优化。
    - 核心思路：所有规则矩阵 + canvas 图像 + wire mask 沿通道拼起来用 CNN 提空间特征 $\bm y_v$；网表 $(\bm G,\bm E)$ 用 Transformer 编码，节点特征 $\bm g_j=[x_j,y_j,z_j,w_j,h_j,a_j,p_j]$ 经 FC + 正弦位置编码注入"放置顺序" $\bm o$，自注意力的 mask 取自 $1-\bm E$ 来强制只在拓扑邻居间传消息；最后从输出里取 $\bm e_{local}=\mathrm{FC}(\bm H_2[\mathrm{idx}])$ 和 $\bm e_{global}=\mathrm{FC}(\mathrm{AvgPool}(\bm H_2))$，拼 $\bm y_v$ 后送进策略/价值头。训练用 Hybrid PPO：位置与长宽比两个动作分量各算一项 clip 目标 $L(\theta)=\sum_{k=1}^2 \lambda_k\,\hat{\mathbb{E}}_t[\min(r_t^{(k)}\hat A_t,\bar r_t^{(k)}\hat A_t)]$，其中 $r_t^{(k)}$ 是两支策略各自的概率比、$\hat A_t$ 用 GAE 估计；critic 优化 $L(\phi)=\lambda_\phi\hat{\mathbb{E}}_t[(G_t-V_\phi(s_t))^2]$。Reward 把块-块邻接长、块-端子距离、对齐分、HPWL、overlap 五项加权后做 self-adaptive 归一化。
    - 设计动机：网表的拓扑信息（哪些模块需要走线连）和规则矩阵的空间信息天然是两种模态，靠单一 CNN 或单一 GNN 都会丢一边；Hybrid PPO 让两种动作分量各享 clip 信号，避免连续的长宽比梯度把离散位置策略带偏。
