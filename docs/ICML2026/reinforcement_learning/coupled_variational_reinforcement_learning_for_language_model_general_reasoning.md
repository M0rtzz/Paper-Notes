...
1. **复合分布 $p'(z|x,y)$ 作为变分代理**:

    - 功能：用"token 级等权混合先验与后验"的分布 $p'(z_t|z_{<t},x,y)=\tfrac12 p_\phi(z_t|z_{<t},x)+\tfrac12 q_\psi(z_t|z_{<t},x,y)$ 当作 ELBO 中的 $q(z)$，把后验的"答案指导"和先验的"推理时一致性"绑进同一个目标。
    - 核心思路：替换 $q(z)\to p'(z|x,y)$ 进 ELBO，得到重建项 $\mathbb{E}_{p'}[\log p_\theta(y|z,x)]$ 与正则项 $D_{\mathrm{KL}}(p'\|p_\phi)$；因为 $p'$ 里直接包含 $p_\phi$，重建项的梯度会**同时**流回先验与后验参数，而正则项又把整个复合分布拉向先验，避免后验主导导致的训练-推理失配。
    - 设计动机：纯后验训练高效但与推理分布不匹配；纯先验训练保守但低效；混合后两边互补——后验的高质量轨迹"补血"先验，先验的覆盖范围"约束"后验，是一种把 VAE 的 $q$ 与 RL 的 $\pi$ 拉到同一坐标系的设计。

2. **off-policy 混合采样 + 重要性重加权**:

    - 功能：训练时不直接从 $p'$ 采样（那要求每个 token 都做混合，工程开销大、跟 vLLM/SGLang 不兼容），而是按概率 $\alpha$ 走 prior 模板、$1-\alpha$ 走 posterior 模板生成完整轨迹，再用重要性比修正分布失配。
    - 核心思路：定义 $p_{\text{hybrid}}(z|x,y)=\alpha p_\phi+(1-\alpha)q_\psi$，对每条轨迹用两次前向算出 $p_\phi(z|x)$、$q_\psi(z|x,y)$，再算 $p'$ 与 $p_{\text{hybrid}}$；GRPO 损失里的 token 比率 $r_t=p'_{\text{new}}(z_t|\cdot)/p_{\text{hybrid}}(z_t|\cdot)$，把实际采样分布与目标复合分布的差消掉。优势 $\hat A=\log p_{\theta_{\text{old}}}(y|z,x)-\bar R$ 直接用模型自己对答案的对数概率当奖励——**无需外部验证器**。
    - 设计动机：在不改变现有 LLM-RL 框架 (verl, OpenRLHF) 的前提下，复用 SGLang/vLLM 的批量推理，**仅靠 prompt 切换就实现混合分布的有偏估计**；off-policy + 重要性比的组合让分布失配在数学上闭合。同时对"仅来自先验、$\hat A>0$"的轨迹加 NLL 辅助损失（相当于选择性 MLE）以稳定先验训练。

3. **基于 Bregman 控制变量的低方差 KL 估计器**:

    - 功能：估计 $D_{\mathrm{KL}}(p'\|p_\phi)$ 时既要无偏、非负，又要低方差；分别给"来自先验"和"来自后验"的样本设计专用估计器。
    - 核心思路：扩展 Schulman 的低方差 KL 估计器。来自先验时取 $D_{\mathrm{KL}}^{\text{prior}}=\tfrac{p'}{p_\phi}\log\tfrac{p'}{p_\phi}-\tfrac{p'}{p_\phi}+1$；来自后验时取 $D_{\mathrm{KL}}^{\text{posterior}}=\tfrac{p'}{q_\psi}\log\tfrac{p'}{p_\phi}+\tfrac{p'}{q_\psi}-1$。两者的差别在 Bregman 修正项 $\left(\tfrac{p'}{p_{\text{hybrid}}}-1\right)$ 的符号——这一项满足 $\mathbb{E}_{p_{\text{hybrid}}}[\cdot]=0$ 所以无偏，又能把估计器钉在非负区间，避免梯度爆炸。
    - 设计动机：标准 $\log\tfrac{p'}{p_\phi}$ 估计器在混合采样下方差极高、还可能为负；KL 项一炸训练就崩。低方差非负估计器是让 CoVRL 跑稳的关键工程支撑。再加上对重要性比的软裁剪 + 对达到最大长度的样本跳过 KL 计算，进一步降噪。
