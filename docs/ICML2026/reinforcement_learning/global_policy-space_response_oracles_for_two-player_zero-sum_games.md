...
1. **基于 post-expansion PE 的全局选择准则**:

    - 功能：把扩展决策的目标函数从"对 meta-strategy 的 BR 收益"换成"假设加入候选后种群的 PE"，即 $\pi^\star\in\arg\min_{\pi\in\mathcal{B}(\Delta(\mathbf{\Pi}^r))}\mathcal{PE}(\mathbf{\Pi}^r\cup\{\pi\};\mathcal{G})$。
    - 核心思路：评估时把候选种群当成一个新的限制博弈，求 $\min_{\sigma\in\Delta(\mathbf{\Pi}_k^+)}\max_{\pi\in\mathbf{\Pi}} U(\pi,\sigma)$；大规模博弈下精确解不可行，用 RM-BR（McAleer 2022）近似——内层 regret matching 维护一个混合 $\rho_k$，外层 best response 学习器追 $\rho_k$，迭代得到 $\widehat{\mathcal{PE}}_k=U(\beta_k,\rho_k)$。
    - 设计动机：所有 RGB-MSS 因为只看局部支付，最坏情况要把全策略空间几乎都加进来才收敛；而 post-expansion PE 直接对齐 PSRO 的原始目标（让种群逼近 Nash），把"挑哪条策略"变成对全局指标的直接优化。

2. **条件神经网络共享参数的多候选训练与评估**:

    - 功能：把 $K$ 条候选 BR 的训练和 $K$ 条评估 BR 都摊到一个网络上，避免 per-iteration 训 $K$ 套独立模型。
    - 核心思路：用单个 conditional explorer $\pi_\theta(a\mid s,\sigma)$ 同时输出 $K$ 条响应，目标 $J(\theta)=\tfrac{1}{K}\sum_k U(\pi_\theta(\cdot\mid\sigma_k),\sigma_k)$；评估阶段同样用 conditional evaluator $\beta_\psi(a\mid s,\sigma)$，候选 $k$ 的 BR 学习器就是切片 $\beta_k\triangleq\beta_\psi(\cdot\mid\sigma_k)$，整轮 RM-BR 评估都共享 $\psi$。
    - 设计动机：朴素实现要训 $K$ 个独立策略 + $K$ 个独立评估器，相当于把每轮 PSRO 的 DRL 计算量乘以 $K$；条件参数共享让多候选的相对开销几乎可以忽略，使探索-选择框架在 DRL 环境里真正可跑。

3. **正则化 PE 评估指标**:

    - 功能：防止"新策略 $\pi_k$ 在 $\rho_k$ 里权重几乎为 0、但 $\widehat{\mathcal{PE}}_k$ 看起来很小"这种假性最优。
    - 核心思路：定义 $\hat p_k\triangleq\rho_k(\pi_k)$，用 $\widehat{\mathcal{PE}}_k^{\mathrm{reg}}=(1-\hat p_k)(\widehat{\mathcal{PE}}(\mathbf{\Pi}^r;\mathcal{G})-U(\beta_k,\rho^r))+\widehat{\mathcal{PE}}_k$ 打分；因为 $U(\beta_k,\rho^r)\le\widehat{\mathcal{PE}}(\mathbf{\Pi}^r;\mathcal{G})$，括号内非负，所以 $\hat p_k$ 越小被加的"惩罚"越大。
    - 设计动机：在大博弈里 $\rho_k$ 和 $\beta_k$ 都是近似量，RM 收敛慢的时候新策略可能根本没被采用，光看 $\widehat{\mathcal{PE}}_k$ 会被估计噪声带偏；这个正则项把"新加策略是否真被利用"显式编码进打分，让选择对估计误差更鲁棒。
