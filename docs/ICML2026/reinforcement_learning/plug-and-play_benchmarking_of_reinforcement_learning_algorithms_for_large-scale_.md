...
1. **PyTorch-native 可微 CFD 栈**:

    - 功能：用一个 Python 包同时支撑环境步进、自动微分、RL 训练，消除外部 CFD 求解器的安装/耦合负担。
    - 核心思路：底层求解器 PICT 是团队自研的 GPU 加速 incompressible Navier-Stokes 求解器，所有算子都用 PyTorch CUDA kernel 实现，因此每一步流场更新都可以被 autograd 追踪。`FluidEnv.step(a)` 不只返回标准 `(obs, reward, done)`，还允许梯度从 reward 反传到动作 $a$ 乃至 policy 参数 $\theta$，这是支持 DPC（Differentiable Predictive Control）的关键。CylinderJet2D-easy 上 DPC 的训练速度比 PPO 快约 1 个数量级、比 SAC 快约 2 个数量级，正是这种 autograd 直通的工程兑现。
    - 设计动机：既有方案中 Beacon 是 Python-only 但不可微，HydroGym 部分基于 JAX 可微但每个环境绑死不同后端。两者都没办法同时提供"纯 Python+全可微+全场景统一"。把求解器写进 PyTorch 是唯一同时满足三者的路径，代价是必须维护一个自研 GPU CFD kernel。

2. **SARL/MARL 双模 + 控制映射 $\Gamma$**:

    - 功能：让同一个物理任务在不需要重写环境的前提下既能跑单智能体也能跑多智能体，避免 MARL 工作被基准选型卡住。
    - 核心思路：在 SARL 模式下，agent 输出全局动作向量 $\vec{a}_t$ 直接送入环境；在 MARL 模式下，每个 agent $i$ 只输出局部动作 $\vec{a}_t^i$ 并只看局部观测 $\vec{o}_{t+1}^i$。环境内部用控制映射函数 $\Gamma$ 把局部动作聚合成全局边界条件值（常见操作如 normalization + 空间平滑）。例如 RBC2D 的 12 个加热器位置上，SARL 用一个 policy 输出 12 维向量，MARL 把同一个共享 policy 部署到 12 个 agent 上各管一个加热器，两者跑同一个 `FluidEnv`，仅交互层不同。Reward 由本地与全局指标加权组合，例如 RBC 的 $r_t=\mathrm{Nu}_{\mathrm{ref}}-\mathrm{Nu}_{\mathrm{instant}}$，其中 $\mathrm{Nu}_{\mathrm{instant}}=\sqrt{\mathrm{Ra}\cdot\mathrm{Pr}}\langle u_y T\rangle_V$。
    - 设计动机：AFC 真实场景就是空间分布执行器，SARL 在动作维度爆炸时不可行（TCFLarge3D 4096 维），MARL 的平移等变性又天然契合空间均匀边界，但既有基准要么只支持 SARL、要么 MARL 是 hack 出来的。把双模做成一等公民可以让两类方法在同一环境上直接对比。

3. **标准化训练/评估协议**:

    - 功能：消除现有 AFC 文献因初始条件、种子数、episode 长度差异导致的不可比性，让结果具备统计可信度。
    - 核心思路：每个环境提供 3 个固定 split（train/val/test），每个 split 包含 10 个预生成的随机初始域，首次使用时自动下载缓存；`env.reset()` 在选中初始域基础上叠加随机扰动和随机 rollout 步数，并通过种子保证可复现。每个算法跑 5 个随机种子 × 10 个 test episode；性能用 Agarwal 等人 2021 的 performance profiles 报告，置信区间通过 2k 次 stratified bootstrap 估计。所有指标按 per-step 上报而非 cumulative，避免被 episode 长度污染。
    - 设计动机：作者在 related work 中点名 AFC 现状：很多论文只跑单个种子（Guastoni 2023, Sonoda 2023）、只与 uncontrolled baseline 对比、test episode 直接复用 train 初始条件。这些做法被 Henderson 2018、Agarwal 2021 反复指出会显著夸大方差与差异。统一协议是 benchmark 价值的一半。
