...
description: >-
  [ICML 2026][强化学习][视觉工具使用] 本文提出 MED 框架系统分析视觉工具使用 RL 在裁剪-放大场景中的实际学习效果——发现 RL 训练所带来的性能提升主要源于**内在能力提升**而非工具掌握能力提升，模型主要学会了如何与工具安全共存而非真正掌握工具。
tags:
  - "ICML 2026"
  - "强化学习"
  - "视觉工具使用"
...
1. **Measure 阶段：量化工具效应的总体贡献**:

    - 功能：用积分计算内在漂移与工具漂移的累积幅度，得到工具贡献比 $S_{tool}$。
    - 核心思路：$|B_{wo}| = \int_0^T |f_{wo}(t)| dt$ 与 $|B_{\Delta tool}| = \int_0^T |\Delta_{tool}(t)| dt$，工具贡献比 $S_{tool} = \frac{|B_{\Delta tool}|}{|B_{wo}| + |B_{\Delta tool}|}$。$S_{tool} \approx 0$ 表示内在漂移主导，$\approx 1$ 表示工具效应主导。
    - 设计动机：给出宏观答案——到底是内在能力强化还是工具使用改进驱动最终性能提升，为后续细粒度分析奠定基础。

2. **Explain 阶段：分解工具诱导效应为四项**:

    - 功能：将工具性能间隔 $G(t)$ 分解为"调用获益 / Schema 获益 / 调用伤害 / Schema 伤害"。
    - 核心思路：按样本在工具自由协议下是否成功分组（$\mathcal{D}_{fail}$ / $\mathcal{D}_{succ}$），再按工具调用行为分类（call / no-call），得到 $G(t) = T1 + T2 - T3 - T4$。$T1$=工具自由失败但调用工具后成功；$T3$=工具自由成功但调用工具后失败（工具害处）。
    - 设计动机：将抽象性能间隔转换为**可作用的成分**，区分收益（T1+T2）与伤害（T3+T4），进一步区分调用引发 vs Schema 引发的效应。

3. **Diagnose 阶段：Mass-Policy-Quality 因式分解**:

    - 功能：对每一项进一步分解为三个因子，诊断性能变化背后是哪种机制。
    - 核心思路：$\text{Term}(\mathcal{D},a,o) = P(\mathcal{D}) \cdot P(a|\mathcal{D}) \cdot P(o|a,\mathcal{D})$——**Mass** 是该类样本规模、**Policy** 是调用决策"何时调用"、**Quality** 是执行质量"如何使用"。
    - 设计动机：例如调用获益停滞可能源于失败集萎缩（Mass↓）、模型停止调用（Policy↓）或执行质量下降（Quality↓）三种不同机制；分解能精确定位成因，特别揭示"内在能力-工具 trade-off"现象。
