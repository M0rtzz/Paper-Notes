...
1. **Stepwise Reward Centering (SRC)**:
    ...
    - 功能：把每一步的步级奖励减去一个全局常值基线，让"延长一步"的期望梯度为零，从而消除 length shortcut
    - 核心思路：先把路径奖励按 $R=\sum_t r_t$ 分解，再对单一奖励令 $\tilde r_t=r_t-\bar r$，其中 $\bar r=\mathbb E_\pi[r_*]$ 是全局期望步奖励，按构造 $\mathbb E_\pi[\tilde r_t]=0$，从而 $\mathbb E[\sum_t\tilde r_t]$ 不再随 $L$ 增长；多目标场景下扩展为按分量归一化 $\tilde r_t=\sum_{i=1}^K w_i\cdot(r_t^{(i)}-\mu^{(i)})/\sigma^{(i)}$，其中 $\mu^{(i)},\sigma^{(i)}$ 在第一个 warm-up epoch 用 rollout 累积估计后冻结，避免 $\mu,\sigma$ 与 $\pi$ 共同漂移
    - 设计动机：观察到 CTR、IoI、IoR 的步级期望均为正且相对稳定（IoR 略有缓降但始终正），所以单一全局基线足以打破"长度-奖励耦合"；冻结统计量是为了让奖励估计不被自身策略改进反复扭曲；归一化让多目标项的梯度量级可比，避免数值大的目标（如 IoR）压制小的目标（如 IoI）

2. **Position-Specific Advantage Estimation (PSAE)**:

    - 功能：替代"整路径奖励×每步 log-prob"的高方差估计，给每个 (input, step) 配一个位置自适应的低方差基线，无需训 critic
    - 核心思路：先用 reward-to-go $G_t^{(i,j)}=\sum_{\ell=t}^{L^{(i,j)}}r_\ell^{(i,j)}$ 把过去的、与当前动作无关的 $r_1,\ldots,r_{t-1}$ 从加权信号里去掉；再按位置做分组基线 $\bar G_{i,t}=\sum_{j:L^{(i,j)}\ge t}G_t^{(i,j)}/\sum_j \mathbb I[L^{(i,j)}\ge t]$，即对同一输入 $i$ 下所有"能到达第 $t$ 步"的 rollout 取平均；最终优势 $\hat A_t^{(i,j)}=G_t^{(i,j)}-\bar G_{i,t}$，rectified 估计器为 $\hat g_\text{rect}=\frac{1}{nm}\sum_{i,j}\sum_{t=1}^{L^{(i,j)}}\nabla_\theta\log\pi_\theta^{(i,j,t)}\cdot\hat A_t^{(i,j)}$
    - 设计动机：GRPO 用整条路径平均奖励 $\bar R_i$ 当所有步共享的基线，忽视了 reward-to-go 在前期步和末尾步本来就量级不同——越靠后的步累积越短、$G_t$ 本身越小，用全局基线对前期步而言偏低、对末尾步偏高，方差未必降下来；按位置分组基线天然适配 PRS 中 $\mathbb E[G_t]$ 随 $t$ 变化的结构，仍是 Williams 1992 意义下的无偏估计，但方差显著更小

3. **多目标奖励与 KL 约束的联动训练**:

    - 功能：把 CTR/IoI/IoR 三个分量统一在同一个被 centering、被归一化、被 KL 约束的目标里，保证 RL 微调不偏离 $\pi_0$ 过远
    - 核心思路：reward 端用 $\tilde r_t$ 注入到 advantage 计算里；KL 端用 $\pi_0$ 作为 anchor，$\lambda\cdot D_\mathrm{KL}(\pi_\theta\|\pi_0)$ 直接解析求导加到 $\hat g_\text{rect}$ 上；rollout 采样数 $m$ 控制 PSAE 基线的统计有效性
    - 设计动机：PRS 的奖励组合 $R_\text{path}=\alpha\cdot\mathrm{IoI}+\beta\cdot\mathrm{IoR}+\gamma\cdot\mathrm{CTR}$ 中三项数量级差几个量级，没有归一化时 IoR 这种数百量级的项会单独支配梯度；KL 锚定既保留了预训练阶段学到的语言式/序列式先验，又让 PSAE 在搜索时不至于飘到分布外，是工业部署中防止"reward hacking"的常用做法
