...
1. **Universal Horizon Model：把视界当条件的生成模型**:
    ...
    - 核心思路：把目标定义为 $m^\pi(x|s,a,n)=\Pr(s_n=x|s_0=s,a_0=a,\pi)$，对任意 $n$ 都通过 bootstrapping 学习——$n=1$ 时直接拟合数据集转移 $\mathcal{P}(x|s,a)$；$n+1$ 时用 $\mathbb{E}_{s'\sim\mathcal{P},a'\sim\pi}[m^\pi(x|s',a',n)]$ 作为 target，具体实现用 coupled flow matching（Lipman et al., 2023; Farebrother et al., 2025）：先用目标网络 $v_{\bar\theta}$ 跑 $N_\text{flow}$ 步 ODE 从噪声 $s_e^0\sim\mathcal{N}(0,I)$ 推出 $n-1$ 步样本 $s_e^1$，再用条件 OT 路径 $s_e^\tau=(1-\tau)s_e^0+\tau s_e^1$ 在 flow timestep $\tau\sim\text{Unif}[0,1]$ 上回归 $\|v_\theta(s_e^\tau|s,a,n,\tau)-(s_e^1-s_e^0)\|_2^2$。
