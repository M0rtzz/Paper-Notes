...
1. **Cost-to-go 作为可控上下文标量**:

    - 功能：在策略输入里显式加入剩余预算 $G_{c,t}(\tau_k)$，把"用户预算 $\delta$"变成测试时可直接拨的旋钮，避免像 RTG/CTG 配对那样需要猜可行域。
    - 核心思路：策略形式为 $\pi_\theta(\cdot|S_t^k,H_t^k,G_{c,t}(\tau_k))$；每 episode 起点 $G_{c,0}=\delta$；预训练时 $\delta$ 在一个区间内均匀采样（SafeDarkRoom $[1,10]$、SafeDarkMujoco $[10,50]$、SafeVelocity $[0,5]$），让网络学会"看到不同 CTG 就给出不同保守程度的动作"。
    - 设计动机：相比 SafeAD 必须同时给 RTG 和 CTG 才能控制 trade-off（且 RTG 给错了会陷入不可行），CTG 单旋钮天然单调——高 CTG 鼓励冒险高 reward、低 CTG 强制保守，把"用户和策略协商"压缩成一个标量。

2. **单乘子 + Hinge 取正号的 surrogate Lagrangian**:

    - 功能：用一个 Lagrange 乘子 $\lambda$ 同时管所有 episode 的 cost 约束，但不会"误伤"已经满足约束的 episode。
    - 核心思路：标准 CMDP 对偶 $\max_\pi \min_\lambda L(\pi,\lambda)=\mathbb{E}_\pi[\sum_k G(\tau_k)] - \sum_k \lambda_k(\mathbb{E}_\pi[G_c(\tau_k)] - \delta)$ 需要 $K$ 个乘子且更新不均；本文塌缩为 $L_\Sigma(\pi,\lambda)=\mathbb{E}_\pi[\sum_k G(\tau_k)] - \lambda\sum_k [g_k(\pi)]_+$，只罚那些**真正超预算**的 episode（$[x]_+=\max(x,0)$），并用 $\lambda \leftarrow [\lambda + \eta\max_k g_k(\pi)]_+$ 跟踪"最坏 episode"。Theorem 1 证明：当 $\lambda \ge \|\lambda^\star\|_\infty$ 时该迭代的 fixed point 与原 CMDP 的最优可行策略集合**完全相等**（exact penalty），因此叫 SCARED（Safe Contextual Adaptive Reinforcement via Exact-penalty Dual）。
    - 设计动机：(1) 单乘子让预训练不必预先固定测试 episode 数 $K$；(2) hinge 只罚超预算项防止 over-penalize；(3) exact-penalty 理论保证不像 quadratic / squared penalty 那样有偏，对偶最优解与原问题严格对齐。

3. **真正外推的 OOD 安全基准（center→edge 分布漂移）**:

    - 功能：构造一个**数学可证 OOD**（KL→∞、TV→1）的评测协议，把 SafeDarkRoom / SafeDarkMujoco 从"棋盘式插值"升级为"训练只见中心、测试只见边缘"。
    - 核心思路：训练阶段 obstacle/goal 在格子 $(i,j)$ 上以概率 $P_{\text{train}}((i,j))\propto e^{-\alpha d((i,j),c)}$（$c$ 是地图中心）采样，向中心聚拢；测试阶段切成 $P_{\text{test}}((i,j))\propto e^{+\alpha d((i,j),c)}$，向边缘聚拢。Proposition 1 给出 $\lim_{\alpha\to\infty} d_{TV}(P_{\text{train}},P_{\text{test}})=1$、$D_{KL}\to\infty$，即两个分布的支撑集几乎不重叠。配合 SafeVelocity（HalfCheetah/Ant）的 unseen velocity 区间，覆盖 structural OOD + unseen ID 两类泛化。
    - 设计动机：以往 DarkRoom 系列的 OOD 只是"训练时这些格子有目标，测试时换另一些格子"，本质是插值；本文用距离指数权重强行制造支撑集分离，让"测试时还能持续 reduce cost"才真正算 in-context safe adaptation。
