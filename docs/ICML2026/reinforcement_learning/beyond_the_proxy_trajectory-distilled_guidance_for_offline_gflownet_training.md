...
1. **基于 IRL 的边奖励提取**:
    ...
    - 功能：从离线轨迹中学习转移级的奖励信号 $R_E(s, s')$，量化每条 DAG 边的重要性
    - 核心思路：首先对数据集进行重平衡，按终端奖励成比例采样轨迹 $P(\tau) \propto R(s_T)$，构造近似专家分布。然后采用 GAIL 框架进行对抗训练：判别器 $D_\phi$ 区分重平衡数据与模仿策略 $\pi_\psi$ 生成的转移，最终从判别器提取无偏边奖励 $R_E(s, s') = \log D_\phi(s, s') - \log(1 - D_\phi(s, s'))$。理论分析表明，在总体极限下 $R_E$ 恢复的是对数条件入流分数（即规范反向策略 $\mathcal{P}_B^*$ 的对数），且学习误差 $\|R_E - r^*\|_\infty \le \varepsilon$ 可以保证反向策略的 TV 距离不超过 $\frac{1}{2}(e^{2\varepsilon} - 1)$
    - 设计动机：与代理奖励模型本质不同——边奖励捕捉转移的结构性偏好而非预测终端奖励，因此即使存在近似误差也不会通过梯度传播损害策略。实验显示边奖励对未观察转移也有良好泛化

2. **基于奖励的 DAG 剪枝**:

    - 功能：移除低效转移边，构建更紧凑的动作空间
    - 核心思路：用模仿策略 $\pi_\psi$ 采样一批状态-动作对，统计边奖励分布。如果某条边的奖励低于阈值 $R_E(s, s') < \text{mean}(\mathcal{D}_{R_E}) - K \cdot \text{std}(\mathcal{D}_{R_E})$（$K$ 为超参），则将其从 DAG 中剪去，之后再移除与根节点断连的孤立节点。理论保证：高瓶颈分数 $\beta(x) > \tau + \varepsilon$ 的终端节点在剪枝后仍可达，且存活终端上的奖励比例关系完全保持不变
    - 设计动机：直接用边奖励做梯度整形会放大函数逼近误差，而剪枝是一种间接且鲁棒的利用方式——将策略的注意力集中到高价值区域，同时避免误差传播

3. **优先反向采样**:

    - 功能：在剪枝后的 DAG 上构造高质量训练轨迹
    - 核心思路：从数据集终端节点按奖励成比例采样起点 $x$，然后用边奖励定义的反向策略 $\mathcal{P}_B(s_t | s_{t+1}) = \exp\{R_E(s_t, s_{t+1})\} / \sum_{(s, s_{t+1}) \in E'} \exp\{R_E(s, s_{t+1})\}$ 递归向根节点回溯，生成完整轨迹。由于终端采样偏向高奖励、路径采样偏向重要转移，生成的轨迹天然聚焦于高价值区域
    - 设计动机：强化 GFlowNet 的核心归纳偏置（按奖励成比例分配采样资源），同时策略的梯度更新仅依赖数据集中记录的真实终端奖励，彻底隔离 IRL 阶段的潜在误差
