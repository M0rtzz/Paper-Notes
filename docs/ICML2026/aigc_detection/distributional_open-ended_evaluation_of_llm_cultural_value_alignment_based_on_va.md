...
### 整体框架
给定目标文化 $\bm{g}$（如日本）和待评 LLM $p_{\bm{\theta}}$：(1) 收集该文化的人类长文本 $\hat p^{\bm g}(\bm x)$ 和 LLM 在同样话题 $\bm o$ 上生成的长文本 $p_{\bm{\theta}}(\bm x|\bm o)$；(2) 在一个由率失真变分优化得到的紧凑**价值码本** $\mathcal{\bm C}=(\bm c_1,\dots,\bm c_K)$ 上，用价值识别器 $q_{\bm\omega}(z|\bm x,\mathcal{\bm C})$ 把每篇文档投影成 $K$ 维价值概率向量；(3) 把人类侧和 LLM 侧分布 $\bm a^{\bm g}, \bm a^{\bm\theta}$ 用**不平衡最优传输**度量距离，rescale 成最终对齐分。整套流程不微调 LLM 参数，$q_{\bm\omega}$、$p_{\bm\phi}$ 都是黑盒调用，通过 ICL + 变分 EM 完成。

### 关键设计

1. **率失真变分价值码本（Rate–Distortion Value Codebook）**:

    - 功能：从无标签长文本里自动抽出一个紧凑、信息量大、低冗余的离散价值码集合 $\mathcal{\bm C}$，作为后续比较的公共"坐标系"，从而填平 Construct gap。
    - 核心思路：把"文档 $\bm x$ → 价值码序列 $\bm s$"看成一次有损压缩，把码本视作 VQ-VAE 的离散 latent。先推一个 ELBO：$\mathbb E_{\hat p(\bm x)}[\log p(\bm x|\mathcal{\bm C})] \ge \mathbb E_{\hat p(\bm x)}\{\mathbb E_{q_{\bm\omega}}[\log p(\bm x|\bm s,\mathcal{\bm C})] - \mathrm{KL}[q_{\bm\omega}\|p(\bm s|\mathcal{\bm C})]\}$；再叠加 rate–distortion 正则得到最终目标（Eq. 3）：信息保留项 $-\log p_{\bm\phi}(\bm x|\bm s,\mathcal{\bm C})$、单文档码熵项 $-\beta_1 H_q(\bm s|\bm x,\mathcal{\bm C})$（鼓励多码避免独占）和先验熵项 $\beta_2 H_q(\bm s|\mathcal{\bm C})$（鼓励所有码被均匀使用）。求解用 Variational EM 风格 BBO：每一轮先采 $N_1$ 组码 $\bm s_j$ 估分 $\mathcal S(\mathcal{\bm C}^{t-1})$，再按 (i) **Extension**（某码 $n_k$ 过高且失真持续大 → split）、(ii) **Merge**（低使用率 → 与最近邻合并）、(iii) **re-creation**（重聚类生成新码）三类动作刷新码本（Algorithm 1）。价值识别器 $q_{\bm\omega}$ 先从文档抽 $M'$ 条自然语言价值短语 $\bm v$，再按 $q_{\bm\omega}(z=k|\bm x,\mathcal{\bm C})=\frac{1}{M'}\sum_j \mathrm{softmax}_{\mathcal{\bm C}}[\mathrm{sim}(\bm e_{\bm v_j},\bm e_{\bm c_k})/\sigma^2]$ 做软分配，避免 hard arg-max 噪声。
    - 设计动机：传统价值评测要么绑死 Schwartz/Hofstede 等先验体系（带入研究者偏差），要么纯靠 LLM 抽关键词（噪声大、易复述无关语义）。率失真正则化把"信息保留"和"冗余减少"显式写进目标，又允许在 LLM 上做 ICL 黑盒优化，从而真正得到"按数据驱动、为评估服务"的价值坐标系。

2. **价值化不平衡最优传输度量（Value-based Unbalanced OT）**:

    - 功能：在已经学到的码本上比较两个文化×LLM 的价值分布 $\bm a^{\bm g}$ 与 $\bm a^{\bm\theta}$，给出一个能反映子群体异质性、对样本不平衡鲁棒的对齐分，从而填平 Composition gap。
    - 核心思路：把 $K$ 个价值码当作传输空间的中心点，定义 UOT 目标 $\mathcal D_{\mathrm{UOT}}(\hat p^{\bm g},p_{\bm\theta})=\min_{\bm\pi\ge 0}\sum_{i,j}[D_{i,j}\bm\pi_{i,j}+\epsilon\bm\pi_{i,j}(\log\bm\pi_{i,j}-1)]+\gamma\mathrm{KL}[\bm\pi\bm 1\|\bm a^{\bm g}]+\gamma\mathrm{KL}[\bm\pi^T\bm 1\|\bm a^{\bm\theta}]$。代价矩阵不只算价值码之间的语义距离 $\rho(\bm c_i,\bm c_j)$，还乘上一个**共现折扣** $1-\mathbb E[\min(\bm a_i,\bm a_j)]/(\mathbb E[\max(\bm a_i,\bm a_j)]+\epsilon_2)$ —— 在人类文档里经常共同出现的两个价值之间运输成本更低，符合"语义近 + 共现高的价值之间互相替换是合理的"这一直觉。求解用 Unbalanced Sinkhorn 迭代，最后做 debias：$\mathcal D_{\mathrm{UOT}}\leftarrow \hat{\mathcal D}_{\mathrm{UOT}}(\hat p^{\bm g},p_{\bm\theta})-\tfrac12\hat{\mathcal D}_{\mathrm{UOT}}(\hat p^{\bm g},\hat p^{\bm g})-\tfrac12\hat{\mathcal D}_{\mathrm{UOT}}(p_{\bm\theta},p_{\bm\theta})$，再 rescale 成 $r=(0.1-\mathcal D_{\mathrm{UOT}})\times 10$ 作为对齐分。
    - 设计动机：直接把每题分数取平均（WVS/CDEval 做法）会把"同一文化里不同子群体的价值分歧"压成一个均值，丢掉了真正重要的分布形状；用 KL 又怕碰到零质量项；UOT 允许两边总质量不严格一致，恰好契合"LLM 和真人价值分布可能在某些码上有结构性缺位"的现实，同时保留 Wasserstein 的几何性质。

3. **长文档分布作为评测信号（Distributional Long-text Evaluation）**:

    - 功能：彻底放弃多选题/Likert 量表，转而比较"针对同一话题 $\bm o$，人类 vs LLM 写出的长文本分布"，从根上解决 Context gap。
    - 核心思路：以话题 $\bm o$（如"金钱在生活中的作用"）为条件，要求被评 LLM 自由生成 essay/blog 等长文本 $\bm x\sim p_{\bm\theta}(\bm x|\bm o)$；与心理学里"作文反映人格"的观察对齐，长文本比短答案承载更稳定的价值信号。为支撑这一设计，作者构建了 **DOVE Set**：覆盖 KR/JP/CN/US 四种文化、824 个 value-oriented 话题、15,213 篇平均 1,034 token 的人类长文档（经人工筛除重复/噪声/价值无关样本）。
    - 设计动机：约束选项 + 平均打分天然限制了价值表达的丰富度，且与 LLM 实际部署（开放生成）严重错位；而短答 QA 又无法捕获价值的"分布式"特性。用长文 + 自动码本，让评测信号、表达媒介、部署场景三者第一次对齐。

### 损失函数 / 训练策略
DOVE 不更新任何 LLM 参数。$q_{\bm\omega}$ 用 GPT-5.2，$p_{\bm\phi}$ 用 GPT-4.1 nano，嵌入用 OpenAI text-embedding-3-large。码本优化是一个 Variational EM 风格的迭代：固定 $\mathcal{\bm C}^{t-1}$ 估计 $\mathcal S(\mathcal{\bm C}^{t-1})$ → 按 split/merge/recluster 三类原子动作得到 $\mathcal{\bm C}^t$，直到分数收敛或达到 $T=10$ 轮。超参 $N_1=3,N_2=1,\beta_1=0.3,\beta_2=0.08,\tau_1=1.0$；训练语料含人类与 GPT-4o/DeepSeek-v3.1/Llama-4-Maverick 生成的混合文档共 $N=10,676$ 篇。
