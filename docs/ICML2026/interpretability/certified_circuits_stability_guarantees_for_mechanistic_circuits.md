...
flowchart TD
    A["计算图 G + 概念数据集 D<br/>黑盒电路发现算法 A"] --> B["数据集删除平滑<br/>伯努利删除得 n 个随机子集 D⊙ε"]
    B --> C["在每个子集上运行 A<br/>得候选电路"]
    C --> D["统计组件投票频率<br/>平滑包含概率 p_u"]
    D --> E["逐组件三值认证<br/>p_u>τ 纳入 / 1−p_u>τ 排除 / 否则弃权"]
    E --> F["认证半径保证<br/>Theorem 3.1 给出稳定半径 r"]
    F --> G["输出认证电路 C*"]
```
