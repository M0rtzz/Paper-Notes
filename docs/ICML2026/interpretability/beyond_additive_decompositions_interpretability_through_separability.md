...
description: >-
  [ICML2026][可解释性][可解释机器学习] 提出张量分离学习（TSL），一种将条件均值建模为正秩-1可分离乘积之差的逐阶段贪心回归方法，通过可分离结构避免加性分解在强交互下的信号抵消与交互遮蔽问题，同时其偏依赖函数可精确恢复拟合因子形状。
tags:
  - "ICML2026"
  - "可解释性"
  - "可解释机器学习"
...
TSL 将条件均值 $m(\mathbf{x}) = \mathbb{E}[Y|X=\mathbf{x}]$ 建模为 $R$ 个阶段的叠加，每个阶段是两个正可分离乘积之差。输入为 $p$ 维特征 $\mathbf{x}$，输出为标量回归预测。训练采用逐阶段贪心策略：每阶段拟合当前残差，拟合完成后对所有已有阶段的系数进行正交重拟合（orthogonal refitting）。

### 关键设计

1. **正可分离乘积差分结构**:

    - 功能：构建能捕获任意阶交互且可从一维 PD 精确重建的回归模型
    - 核心思路：模型形式为 $\hat{m}(\mathbf{x}) = \sum_{\ell=1}^{R}(\lambda_+^{(\ell)}\prod_j \hat{m}_{+,j}^{(\ell)}(x_j) - \lambda_-^{(\ell)}\prod_j \hat{m}_{-,j}^{(\ell)}(x_j))$，其中每个分量函数 $\hat{m}_{\pm,j}^{(\ell)} > 0$ 严格正。正性约束消除了符号歧义：在无约束乘积中，某个分量为正不意味着该乘积贡献为正（因为其他分量可能为负），而正性保证每个分量"增大即放大"。差分结构（两个正乘积之差）则恢复了表达负值的能力
    - 设计动机：正性同时解决了 bagging 聚合中的可辨识性问题——无约束乘积在非矩形支撑上可有多种等价表示（符号翻转），正性消除了符号歧义使独立拟合的分量可安全平均

2. **Backbone/Tilt 重参数化**:

    - 功能：将每阶段的活跃程度与方向性解耦，提供直观的可解释视角
    - 核心思路：将正负分量对重参数化为 $\hat{m}_{\pm,j}^{(\ell)}(x_j) = b_j^{(\ell)}(x_j) e^{\pm d_j^{(\ell)}(x_j)}$，其中 backbone $b_j^{(\ell)} > 0$ 编码共享幅度，tilt $d_j^{(\ell)} \in \mathbb{R}$ 编码符号不平衡。整个模型变为 $\hat{m}(\mathbf{x}) = 2\sum_\ell b^{(\ell)}(\mathbf{x}) \sinh(d^{(\ell)}(\mathbf{x}))$，backbone 乘积是活跃门控（某特征的 backbone 接近零则关闭该阶段），tilt 之和通过 $\sinh$ 决定方向
    - 设计动机：backbone 即使在符号偏依赖接近零时仍保留幅度信息，避免了交互遮蔽问题；tilt 作为加性不平衡分数具有清晰的方向性解释

3. **网格张量逐步细化与 Bagging 聚合**:

    - 功能：高效拟合每阶段的可分离乘积并降低方差
    - 核心思路：将每个单变量分量建模为分段常数函数，通过 CART 风格的贪心分裂逐步细化分区。每次分裂评估目标为正则化最小二乘 $\mathcal{L}_S(u_+^S, u_-^S) = \sum w_i(R_i - (u_+^S \hat{m}_+^{(i)} - u_-^S \hat{m}_-^{(i)}))^2 + \alpha((u_+^S-1)^2 + (u_-^S-1)^2)$，有闭式 $2\times2$ 线性系统解。多个网格张量在 bootstrap 样本上并行拟合后，在 backbone/tilt 空间中做归一化→锚定→相似度过滤→平均的聚合
    - 设计动机：Bagging 降低方差，相似度过滤解决了可分离模型在非矩形支撑上的可辨识性歧义
