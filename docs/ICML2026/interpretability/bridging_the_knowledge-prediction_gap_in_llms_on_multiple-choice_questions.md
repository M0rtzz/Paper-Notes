...
**切入角度**：训练两个线性探针——知识探针预测真实答案、预测探针预测模型输出——将各自权重矩阵视为定义子空间的基向量。如果两个子空间对齐，模型预测就应与其内部知识一致；实测发现差距大的基准上两个子空间主角严重偏离。
...
1. **双探针知识-预测差距量化**:
    ...
    - 功能：分别刻画残差流中"模型知道什么"和"模型会输出什么"的信号强度与方向
    - 核心思路：在每一层 $l$ 提取残差流激活 $h^l(x)$，构建两个数据集——$D_{\text{know}}^{(l)} = \{(h^l(x), y)\}$ 配对真实标签，$D_{\text{pred}}^{(l)} = \{(h^l(x), \tilde{y})\}$ 配对模型预测标签——分别训练 $k$ 类线性分类器。引入两个互补度量：预测一致率 $\text{AGR}(x) = \mathbb{I}[\arg\max p_K(x) = \arg\max p_M(x)]$ 衡量决策层对齐，KL 散度 $\text{KLD}(x) = \text{KL}(p_M \| p_K)$ 衡量分布层对齐
    - 设计动机：单靠准确率无法跨基准、跨模型横向比较差距大小；AGR 捕捉"选对/选错"的硬性差异，KLD 捕捉置信度分布的软性差异，两者互补

2. **子空间几何分析**:

    - 功能：为知识-预测差距提供可解释的几何归因
    - 核心思路：将每个探针的权重矩阵 $W \in \mathbb{R}^{d \times k}$ 的列向量视为子空间基，用主角均值（mean principal angle）和 CKA 两个指标衡量知识子空间与预测子空间的对齐程度。实验发现深层主角趋近 $90°$（接近随机基线），CKA 处于 0.4–0.8 中间带，说明两个信号共存于同一残差流但沿几何上不同方向传播。跨 8 个基准的 Spearman 相关分析表明，子空间错位程度与实测差距高度正相关（Llama 3.1 8B: $\rho = 0.976, p = 0.001$）
    - 设计动机：将抽象的"模型不听自己的知识"现象锚定在几何结构上，为后续对齐干预提供理论依据

3. **KAPPA 推理时对齐干预**:

    - 功能：在推理时修改隐藏状态，使模型预测忠实于其内部编码的知识
    - 核心思路：建立约束优化问题 $\min_{\tilde{h}'} \|\tilde{h}' - \tilde{h}\|_2^2$，约束 $\tilde{W}_{\text{pred}}^\top \tilde{h}' = \tilde{W}_{\text{know}}^\top \tilde{h}$，闭式解为 $h' = h + W_{\text{pred}}(W_{\text{pred}}^\top W_{\text{pred}})^{-1}(\tilde{W}_{\text{know}}^\top \tilde{h} - \tilde{W}_{\text{pred}}^\top \tilde{h})$。扩展版引入超参 $\alpha, \beta$ 控制对齐强度：$\tilde{W}_{\text{pred}}^\top \tilde{h}' = \alpha \cdot \tilde{W}_{\text{know}}^\top \tilde{h} + \beta \cdot \text{sign}(\tilde{W}_{\text{know}}^\top \tilde{h})$，其中 $\alpha$ 放大选项间相对差异，$\beta$ 推动每个选项 logit 向极值靠拢
    - 设计动机：与固定方向的激活引导不同，KAPPA 为每个输入动态计算最小扰动量，仅修改预测子空间内的分量，保留正交方向的其他信息；闭式解无需迭代优化，开销极低
...
- KAPPA 并非直接修改答案 token 的 logit（干预层的预测子空间与 logit 空间主角约 65°–70°），而是通过修改中间层抽象表示间接影响后续决策
