...
CB-SLICE 接收一个训练好的 CBM $\mathcal{M}_\theta = (g, f)$（概念编码器 $g$ + 标签预测器 $f$）和验证集中被错误分类的样本集 $\Psi_{\text{val}}$，经过三个步骤输出一组带有关键词概念解释的错误切片。整个流程完全在 CBM 的概念表示空间中完成，不依赖任何外部辅助模型。

### 关键设计

1. **错误倾向概念筛选（ECTP 过滤）**:

    - 功能：从全部 $k$ 个概念中筛选出最可能导致下游误分类的子集 $C_{\text{err}}$
    - 核心思路：利用 Expected Change in Target Prediction（ECTP）分数衡量对每个概念进行干预后下游预测分布的变化。对每个概念 $i$ 计算 $T_i(\hat{\mathbf{c}}) = (1-\hat{c}_i) D_{\text{KL}}(\hat{y}_{\hat{c}_i=0} \| \hat{y}) + \hat{c}_i D_{\text{KL}}(\hat{y}_{\hat{c}_i=1} \| \hat{y})$，按类别取平均后选取 top-$t_e$ 个概念
    - 设计动机：限制在错误倾向概念上形成切片可以显著提升发现质量，避免无关概念引入噪声

2. **GMM 聚类形成错误切片**:

    - 功能：将错误样本按共享的概念级错误模式分组
    - 核心思路：将错误样本的概念预测从概率空间映射到 logit 空间 $H_{\text{err}} = \sigma^{-1}(\hat{C}_{\text{err}})$，然后用高斯混合模型聚类。优化目标由三部分组成：GMM 负对数似然 $\mathcal{L}_{\text{GMM}}$ 保证语义连贯性，两个辅助分类器损失 $\mathcal{L}_{c_{\text{true}}}$ 和 $\mathcal{L}_{c_{\text{pred}}}$ 分别确保同一切片内样本共享相同的真实概念值和预测概念值，总损失为 $\mathcal{L} = \mathcal{L}_{\text{GMM}} + \lambda(\mathcal{L}_{c_{\text{true}}} + \mathcal{L}_{c_{\text{pred}}})$
    - 设计动机：概念 logit 编码了模型对概念存在与否的置信度，且近似高斯分布，适合 GMM 建模；辅助损失迫使切片捕获一致的概念级错误模式而非仅仅是特征相似性

3. **关键词概念解释（ECSA 评分）**:

    - 功能：为每个切片提取最能解释其形成原因的关键词概念
    - 核心思路：提出 Expected Change in Slice Assignment（ECSA）分数，衡量干预某个概念后样本对切片归属概率的变化：$\text{ECSA}_i(\mathbf{x}) = \mathbb{E}_{v \sim \text{Bern}(\hat{c}_i)} [D_{\text{KL}}(P(S_j | \mathbf{x}, \hat{c}_i = v) \| P(S_j | \mathbf{x}))]$。对切片内所有样本取平均后选 top-$t_k$ 个概念作为关键词，同时标注每个概念的预测值是否正确
    - 设计动机：关键词概念不仅告诉你"这个切片跟什么属性有关"，还告诉你"模型在这个属性上预测对了还是错了"，区分了"概念误预测导致的错误"和"罕见概念组合导致的错误"两种不同失败类型

### 切片优先级策略

为避免分析过多切片带来的负担，CB-SLICE 提出切片信息量评分 $\text{SI}_j = \rho \cdot \frac{1}{2}(\text{MC}_j + \frac{1+\text{SC}_j}{2})$，综合考虑误预测一致性（MC，基于切片内预测标签分布的熵）和语义紧凑性（SC，基于切片成员到质心的余弦相似度），并用惩罚因子 $\rho$ 降权过小切片。
