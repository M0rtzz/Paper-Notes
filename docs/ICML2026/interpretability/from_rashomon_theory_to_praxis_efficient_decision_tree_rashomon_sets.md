...
1. **加叶子节点**：检查"预测 0"和"预测 1"两种叶子的目标值 $C_b = \gamma + |\{y_i \neq b\}|$ 是否在预算内，是则加入 OrNode。
2. **遍历所有特征 split $j$**：把数据切成 $(D_L, D_R)$，用 PROXY 在子问题上算出一个估计代价 $P_L = \text{Proxy}(D_L, d-1, \gamma)$, $P_R = \text{Proxy}(D_R, d-1, \gamma)$；若 $P_L + P_R > \varepsilon_{\text{abs}}$，直接剪掉这个 split。
3. **递归求解**：剩下的 split 调用 `Solve_Siblings` 把预算分配给左右子问题，递归构造子图 $G_L, G_R$，然后用 `AddSplit` 把 split 节点挂回当前 OR-graph。
...
1. **代理框架（Proxy Optimizer Framework）**:
...
2. **兄弟子问题迭代预算精化（Solve_Siblings, Algorithm 3）**:
...
3. **位向量指纹缓存（Bitvector-Fingerprint Caching）**:
