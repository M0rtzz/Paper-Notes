...
description: >-
  [ICML 2026][可解释性][绑定问题] 本文把"神经网络的绑定问题"形式化为表示 $Z$ 中关于对象码 $O$ 的互信息 $I(O;Z)$，并设计自回归概率探针在 DINOv2 / CLIP 等 ViT 上度量绑定信息，发现 `[CLS]` 只编码 <50% 的绑定信息且结构近似二次型…
tags:
  - "ICML 2026"
  - "可解释性"
  - "绑定问题"
...
### 整体框架
输入：一个预训练 ViT $\Phi: X \to Z$ 和一个能给出 ground-truth 特征码 $F \in \{0,1\}^n$ 与对象码 $O \in \{0,1\}^K$ 的场景数据集（合成或带密集标注的真实数据）。

输出：四个标量度量——绑定信息 $B_O(Z) = I(O;Z)$、特征条件绑定信息 $B^*_{O,F}(Z) = I(O;Z|F)$，以及它们除以先验 $H(O)$、$H(O|F)$ 得到的归一化版本 $\beta_O(Z)$、$\beta^*_{O,F}(Z)$。

流水线四步：(1) 用组合学/经验分布算出数据集先验 $H(O)$ 与 $H(F)$；(2) 在冻结表示 $Z$ 上训练自回归绑定探针 $q_\theta(o_k|o_{<k},z)$（和特征探针 $q_\theta(f_k|f_{<k},z)$）；(3) 把它们在 test set 上的交叉熵当作 $H(O|Z)$、$H(F|Z)$ 的上界估计；(4) 按定义代入式 $I(O;Z)=H(O)-H(O|Z)$ 与 Theorem 2.16 算出四个度量。

### 关键设计

1. **绑定信息的信息论形式化（含 4 种变体）**：

    - 功能：给"表示里有多少绑定信息"一个与编码方案（slot / tensor-product / capsule 都行）无关的标量定义。
    - 核心思路：把场景抽象成"特征集 $F$ + 对象集 $O$"，每个对象 $o_i$ 对应 $F$ 的一个子集（surjective map），从而 $F$ 是 $O$ 的确定函数。定义 $B_O(Z):=I(O;Z)$ 度量原始绑定信息；为了在不同特征学习能力的模型间公平比较，再引入条件版 $B^*_{O,F}(Z):=I(O;Z|F)=I(O;Z)-I(F;Z)$（Theorem 2.14），它扣掉了仅靠特征信息就能推出的部分；最后用 $H(O)$、$H(O|F)$ 做归一化得到无量纲 $\beta$、$\beta^*$，便于跨数据集比较。
    - 设计动机：直接报告下游准确率会被 readout 头吞掉信息；用 $I$ 既能反映软概率的不确定度，又允许通过"换探针族"看到信息的结构（线性 / 二次 / 高阶）。条件版避免了把"模型只是把单特征学好了"误判成"模型会绑定"。

2. **自回归对象码探针 + 三种探针族对比**：

    - 功能：在 `[CLS]` 这种单 token 表示上估计 $H(O|Z)$，并通过对比不同探针族的残余 loss 反推"绑定信息的代数结构"。
    - 核心思路：直接训 $q_\theta(o|z)$ 需要覆盖 $2^K$ 的标签空间（实验中 $K=64$，超过 $10^{19}$），不可行。本文用链式分解 $q_\theta(o|z)=\prod_{k=1}^K q_\theta(o_k|o_{<k},z)$，把表示 $z$ 与已揭示的对象码 $o_{<k}$ 拼接成 $x=[z\|o_{<k}]$ 喂给探针；每个 $o_k$ 用一个二分类 logit $\ell_k(x)$ 监督，总 loss 求和即式 (5)。探针族分别为线性 $\ell_k=W_kx+b_k$、二次 $\ell_k=x^\top W_k x+b_k$、四层 GELU DNN（约 3M 参数）。在合成 ColorShape 数据上，三族 test loss 分别为 $34.2/22.0/20.6$ bits（误差降幅 $37.4\%/63.0\%/64.4\%$），二次族与 DNN 几乎打平。再做"参数共享二次探针"$W_k=U_{\text{color}}^\top V_{\text{shape}}$ 只多 $+2.4$ bits 的 loss，证明 `[CLS]` 中的绑定信息**本质上是颜色投影与形状投影的点积**，这是一种保守的合取式编码。
    - 设计动机：用探针族阶梯（linear → quadratic → DNN）逼近 Lemma 2.21 给出的 $L_{CE}\ge H(O|Z)$ 下界；族间 gap 反过来揭示"信息是否能被低阶函数读出"。参数共享变体把"绑定信息长得像 $\text{color}\otimes\text{shape}$ 吗？"变成可证伪的实验。

3. **空间 token 上的简化注意力探针**：

    - 功能：处理 ViT 全套空间 token $\{s_i\}_{i=1}^N$（拼接维度爆炸）并测量它们整体携带的绑定信息。
    - 核心思路：为每个对象 $o_k$ 学一个 query $q_k=g_k(o_{<k})$（仅 query，无 key / value 投影），按 $a_{k,i}=\text{softmax}_i(q_k^\top s_i)$ 路由出加权空间向量 $\bar s_k=\sum_i a_{k,i} s_i$，再过一个二次读出 $q_\theta(o_k=1|o_{<k},\{s_i\}) = \sigma(\bar s_k^\top W_k \bar s_k+b_k)$。在 ColorShape 上 test ER 达到 $96.8\%$（loss $3.1$ bits），远超 `[CLS]` 最强 DNN 探针的 $20.6$ bits；定性上注意力权重几乎总能 route 到目标对象所在 patch。
    - 设计动机：空间 token 的绑定信息天然分布在多个位置，直接拼接给 MLP 会让参数量爆炸且过拟合；用"learned query + 加权平均"的极简注意力既能跨样本权重共享，又能让 $a_{k,i}$ 作为可视化诊断——证明绑定信息确实以"对象在哪个 patch"这种空间索引形式存在。
