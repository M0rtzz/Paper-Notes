...
1. **Warmup 训练**：先把一个稠密的浅层 MLP（输入 $\mathbf{x}\in\mathbb{R}^d$、隐层宽度 $h$、输出 $C$ 类）在目标任务上训 25 个 epoch，让它接近收敛并学到"稳定的混合特征"。
2. **边分割扩张**：用扩张因子 $\alpha>1$ 把隐层扩成宽度 $h' = \alpha h$。对原第 $i$ 个神经元的权重 $\mathbf{w}_i$，把它复制到 $\alpha$ 个子神经元上，然后用 $\alpha$ 个 disjoint 的二值掩码 $\mathbf{m}_{(i_k)}\in\{0,1\}^d$（满足 $\sum_k \mathbf{m}_{(i_k)} = \mathbf{1}_d$）把输入维度切给这些子神经元，保证它们不共享任何输入特征。第二层 $\mathbf{W}_2 \in \mathbb{R}^{C\times h}$ 则把每个输出权重复制 $\alpha$ 次扩成 $\mathbf{W}_2'\in\mathbb{R}^{C\times h'}$。
3. **再稀疏化**：第二层复制带来了 $(\alpha-1)C$ 个多余参数，作者在 $\mathbf{W}_1', \mathbf{W}_2'$ 中把绝对值最小的权重剪掉，让总非零参数严格等于原始预算；初始化后掩码不再更新。
4. **微调**：稠密模型与 FPE 模型在完全相同的优化设置下再训 25 个 epoch，比较精度与干扰指标。

### 关键设计

1. **Disjoint 边分割（Fixed Parameter Expansion）**：
...
2. **掩码生成策略：clause-split vs. random-split**：
...
3. **干扰量化：feature capacity + cosine similarity**：
