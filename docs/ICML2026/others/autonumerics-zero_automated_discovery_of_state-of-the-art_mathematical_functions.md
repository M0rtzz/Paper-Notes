...
### 整体框架
AutoNumerics-Zero 是一个多目标进化搜索系统，输入是目标超越函数 $f(x)$（如 $2^x$）在指定区间上的精确值作为训练数据，输出是一组逼近程序，每个程序用尽可能少的基本运算达到尽可能高的精度。整个搜索过程从空程序（恒等函数）出发，通过嵌套的两层优化迭代改进种群：外层遗传编程发现程序结构，内层 CMA-ES 优化浮点系数。搜索完成后，对发现的最佳程序用区间算术进行严格的误差界证明。

### 关键设计

1. **分布式多目标种群选择（dNSGA-II）**:

    - 功能：在大规模分布式集群上高效维护和筛选程序种群
    - 核心思路：基于经典 NSGA-II 算法的分布式变体。每个 worker 从随机其他 worker 接收 $2S$ 个程序，通过 SelectNearPareto 操作选出 $S$ 个接近 Pareto 前沿（精度 vs 运算次数）的父程序，对每个父程序变异两次产生 $2S$ 个子代并发送给其他 worker。精度以 $a = -\log_{10}(E)$ 衡量（$E$ 为最大误差）。这种去中心化设计避免了集中式种群瓶颈，支持异步更新
    - 设计动机：经典 NSGA-II 需要集中式种群管理，不适合大规模并行搜索。dNSGA-II 通过 worker 间随机通信实现分布式种群演化，同时保持 Pareto 前沿上程序的均匀分布

2. **计算图变异策略**:

    - 功能：在程序结构空间中进行探索，生成新的候选程序
    - 核心思路：程序表示为计算图（有向图），输入节点为程序输入和系数，中间节点为 $\{+,-,\times,\div\}$ 运算，输出节点为程序结果。变异随机执行三种操作之一：（1）在随机位置插入一个随机运算节点；（2）删除一个随机节点；（3）随机重连一条随机选择的边。所有选择均为随机的，不引入任何数学先验知识
    - 设计动机：这种"零知识"变异确保搜索过程没有偏向任何已知逼近形式（如多项式形式），使得进化可以发现全新的、非常规的函数表达式，例如嵌套的除法-加法组合后取 8 次方

3. **CMA-ES 系数内层优化**:

    - 功能：在固定的程序结构下优化浮点系数以最大化精度
    - 核心思路：对每个变异后的子程序，使用 CMA-ES（协方差矩阵自适应进化策略）优化其系数。CMA-ES 是无梯度连续优化算法，在低参数量下表现优异。优化目标是最小化在一批随机采样的输入-输出示例上的最大误差
    - 设计动机：变异只改变程序结构，系数需要精细调优。CMA-ES 的无梯度特性避免了局部最优陷阱，且与整个系统的黑盒特性一致——这意味着未来可以加入不可微操作（如位移指令）而不需要修改优化器
