...
其中 $f$ 是 proxy（LLM-as-judge），$\lambda\in\mathbb{R}$ 是 power-tuning 参数，$\lambda^\star$ 有闭式解最小化渐近方差，保证 PPI++ 在渐近上**永远不劣于**只用人类标注的经典估计器，即使 proxy 是对抗性的。
...
- **vs HELM / lm-eval-harness / DeepEval / RAGAS**：那一层负责"跑评测、产 proxy 和真值"，是上游编排器；GLIDE 是下游统计层，把它们的输出转成有覆盖保证的债偏估计——两者堆叠。
