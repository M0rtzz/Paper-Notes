...
description: >-
  [ICML 2026][在线度量匹配] 本文回答了 Im et al. (2022) 留下的公开问题：把"按动作预测的"在线度量匹配带进"节俭预测"框架——预测被昂贵地按 $k$ 步一次发放——并通过 Follow-the-Prediction 框架 + 自动补齐"虚拟预测"的元算法…
tags:
  - "ICML 2026"
  - "在线度量匹配"
  - "学习增强算法"
...
1. **Adherence + Strong Competitiveness：刻画"可用作虚拟预测的子例程"**：

    - 功能：定义中间匹配需要满足的两条结构性质，使得用它生成的虚拟预测能被 FtP 的分析吸收。
    - 核心思路：固定算法 $\mathcal A$ 与瞬时匹配 $\mathcal A_t$（记其匹配过的服务器集合为 $S_t$、所有请求集合为 $R_t$）。称 $\mathcal A$ 是 $\gamma$-adherent，若任意 $t$ 都有 $\mathsf{dist}(S_t, R_t) \le \gamma \cdot \min_{M \in \mathcal M_t} \mathsf{cost}(M)$，其中 $\mathcal M_t$ 是恰好匹配 $R_t$ 的所有最大匹配。称 $\mathcal A$ 是 strongly $\rho$-competitive，若 $\mathbb E[\mathsf{cost}(\mathcal A_t)] \le \rho(t) \cdot \min_{M \in \mathcal M_t} \mathsf{cost}(M)$。Adherence 控制"中间状态在集合层面接近最优"，强竞争性控制"中间状态的累积代价接近最优"。
    - 设计动机：传统 OMM 只在 $t=n$ 看输出，文献里许多经典算法（Kalyanasundaram–Pruhs、Nayyar–Raghvendra、Bansal 等）其实"顺便"在每一时刻都维持好的部分匹配，这两条性质把这种"过程友好"显式化，使它们能即插即用作为虚拟预测器。

2. **Follow-the-Prediction 的节俭化元算法（Algorithm 2 + Algorithm 1）**：

    - 功能：把"每轮一预测"变成"每 $k$ 轮一真预测 + 阶段内子例程合成"，并给出统一的代价界。
    - 核心思路：对每个 phase，先重置 $\mathcal A$ 在 $S \setminus \widehat P$ 上的新实例；之后每来一个非询问轮，喂请求给 $\mathcal A$ 并读出 $\widehat S$，构造 $P_t = \widehat P \cup \widehat S$ 喂给 FtP；询问轮则更新 $\widehat P$ 并重置 $\mathcal A$。这一构造保证 $|P_t| = t$ 且 $P_t \subseteq S$，是合法预测。分析上利用 Lemma 3.2（FtP 代价被 $\sum_t \mathsf{dist}(P_t, P_{t-1} \cup \{r_t\})$ 控制），将其按 phase 拆开后，跨 phase 部分用 adherence 与三角不等式控制（Lemma 3.5），phase 内部分用 strong competitiveness 控制（Lemma 3.4），最终给出 Theorem 3.1 的统一界 $(1+\gamma+\rho(k-1))\,\text{cost}(\text{OPT}) + (2+\gamma+\rho(k-1))\,\eta(Q)$。
    - 设计动机：直接套 FtP 会因为"少了 $k-1$ 个真预测"导致代价无界；用一个"过程友好"的子例程把这 $k-1$ 个空缺补成"足够接近真预测"的代理，就能把 FtP 的逐轮三角不等式平滑接上。

3. **配套下界：在 star 上推广经典硬实例，证明上界本质最优**：

    - 功能：证明 Theorem 1.1 在确定性算法、即便预测完全准确时，也基本无法被改进。
    - 核心思路：在 $n$ 个叶子的 star metric 上构造对手序列，让任意确定性算法在最多 $B$ 次询问下都至少损失 $\frac{2n}{B+1}-1$ 倍（Theorem 4.1）；在 well-separated 询问机制下损失至少 $2k-1$ 倍（Theorem 4.2）。随机算法侧给出 $\Omega(\log k)$（well-separated）与 $1 + o\!\left(\frac{\log(n/B)}{B}\right)$（bounded budget）的下界（Theorem 4.3–4.4）。
    - 设计动机：节俭框架很容易做出"看起来更好"的上界，但若不配下界就难以判断节俭代价是否被吃干。本文用统一的对抗构造把上下界对齐，使 Theorem 1.1 的常数因子近乎紧。
