...
1. **PertAlign：零成本的二阶 CO 预警指标**：

    - 功能：用一个余弦相似度度量 loss surface 沿攻击方向的非线性程度，并在 PGD 精度崩之前就给出"即将 CO"的信号。
    - 核心思路：定义 $\text{PertAlign} = \cos\!\big(\nabla_x \mathcal{L}(f_\theta(x'), y),\ \nabla_x \mathcal{L}(f_\theta(x'+\delta), y)\big)$，其中 $x' = x + \eta$、$\delta = \alpha v$。论文证明 $1 - \text{PertAlign} \approx \tfrac{\alpha^2}{2}\|h_{\perp g}\|^2$，其中 $h = Hv/\|g\|$，即 PertAlign 直接捕获了 Hessian-vector product 在与梯度正交方向上的分量；CO 发生时该分量爆炸，PertAlign 从接近 1 急剧滑向 0。两条梯度一条来自生成对抗样本，另一条来自常规反传顺手延伸一层，因此**完全不需要额外前向或反向传播**。
    - 设计动机：GradAlign、ELLE 等已有指标都要单独再做一次反传，违背 FAT 的成本约束；同时 PertAlign 比 GradAlign / AAE share / TRADES-KL 等指标都更早触发警报（论文 Fig 3 显示 PertAlign 在 batch 3775 就开始下滑，而 FGSM/PGD 精度要到 batch 3825 才肉眼可见分裂）。

2. **二阶最优步长 $\alpha^*$ + EMA 线性度系数**：

    - 功能：在不显式算 Hessian 的情况下，为下一 batch 给出一个理论最优的攻击步长。
    - 核心思路：把 loss 沿方向 $v = \alpha\, \text{sign}(g)$ 做二阶展开 $\mathcal{L}(x+v) \approx \mathcal{L}(x) + v^T g + \tfrac{1}{2}v^T H v$，对 $\alpha$ 求导得最优步长 $\alpha^* = \min\!\big(\alpha_{\max},\ \alpha_0 / (1 - p^T g'/\|g\|_1)\big)$，其中 $g' = g + \alpha H p$，正好可由上一 batch 反传时延伸出来的输入层梯度免费拿到。SORA 维护一个 EMA $v \leftarrow (1-\beta) v + \beta \cdot p^T g'/\|g\|_1$ 来稳定 $\alpha^*$ 的估计，跨 batch 复用利用了"权重每步只小幅变化、且各 batch 来自同分布"的假设，让二阶信息"essentially for free"。
    - 设计动机：直接显式算 $H$ 的代价在大模型上无法接受；又必须比经验调出来的固定 $\alpha$ 更鲁棒，因此引入一个有解析推导、又能复用已有反传梯度的近似形式。

3. **按通道随机化步长打破 Epsilon Overfitting**：

    - 功能：在 $\alpha^*$ 之上再加一层扰动多样性，避免模型对某个特定 $\epsilon$ 值"刷分式"过拟合。
    - 核心思路：不直接用 $\alpha^*$，而是对每个像素的每个通道独立采样 $\alpha_i \sim \mathcal{U}(0, \alpha^*)$，然后 $x' = x + \eta + \alpha_i \odot \text{sign}(g)$。这样同一 batch 内不同样本看到的扰动幅度分布广泛，整个 $\ell_\infty$ 球被更均匀地覆盖，FGSM 准确率不会再出现在某些 $\epsilon$ 上飙升、在其他 $\epsilon$ 上断崖的"EO 印记"。
    - 设计动机：作者在 Sec.3 指出 CO 的根因是 EO——固定大幅扰动使 loss 只在窄路径上被压平；只要给步长加上 $\mathcal{U}(0, \alpha^*)$ 的随机化，配合自适应 $\alpha^*$，就能同时治标（防 CO）与治本（防 EO）。
