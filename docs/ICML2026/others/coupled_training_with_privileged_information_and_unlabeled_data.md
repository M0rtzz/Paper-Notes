...
3. **高维字典空间的交替贪心前向选择**:
    ...
    - 核心思路：每步在固定另一个块的条件下，从字典里选一个能最大降低当前残差损失的原子，加进当前展开中（block-coordinate × greedy forward stepwise 的合体）。作者在 Theorem 3.1 证明：尽管单步选择是组合的，整体迭代在经验耦合目标上仍有**全局次线性收敛**（$O(1/T)$ 量级的优化误差下降），将经典 Barron / DeVore-Temlyakov 的贪心逼近理论推广到耦合特权信息场景，并通过优化误差界进一步转换为预测风险界。
