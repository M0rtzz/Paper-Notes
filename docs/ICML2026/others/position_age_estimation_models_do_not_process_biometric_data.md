...
1. **能力 vs. 意图的可证伪化框架**:
    ...
    - 功能：把法律辩论里"中间表征是否可能识别"的二元问题转成"FNMR@FMR 达不达监管阈值"的连续量化问题。
    - 核心思路：参照 ISO/IEC 19795 的 1:1 验证术语，FMR 是不同人被错误判为同人的比例（安全性），FNMR 是同一人被错判为不同人的比例（可用性）。三套监管阈值汇总为 NIST SP 800-63-4 IAL2 要求 $\text{FMR}\le 0.01\%$ 且 $\text{FNMR}<5\%$，EU EES 要求 $\text{FMR}=0.05\%$ 且 $\text{FNMR}<1\%$，FIDO 要求 $\text{FMR}\le 0.01\%$ 且 $\text{FNMR}<5\%$。当一个模型在 LFW (最简单的"in-the-wild"基准) 上都比这些阈值差两个数量级时，几乎可以排除其具有"可用的识别能力"。
    - 设计动机：监管侧的真正担忧是"系统能不能准确单挑某人"，而不是"特征里有没有可能残留信息"；把测试目标对齐到 ICO 自己写的"with accuracy, with a level of precision"，让结论可以直接被监管采纳。

2. **多层 + 多基准的"全空间"压力测试**:

    - 功能：堵住所有"可能在某一层、某个基准、某种姿态下年龄估计模型其实能识别"的反驳通道。
    - 核心思路：对每个模型抽 4–N 个中间层，而不是只看末层；同时选三个互补基准：LFW (in-the-wild, 易) → AgeDB-30 (同一人 30 年跨度) → CFP-FP (正脸 vs. 侧脸)。AgeDB-30 特别关键，因为如果年龄估计模型真的"顺带学到了身份"，跨年龄识别正是它的潜在长板；结果恰好相反——年龄估计模型在 AgeDB-30 上更差 (96–98% FNMR)，说明年龄优化得到的特征与身份特征近乎正交。
    - 设计动机：position paper 最大的风险是被一句"那是你测得不够"反驳；本文用"任何层 × 任何基准都达不到"在经验上把这个反驳关上。

3. **Attention probe：对抗式上界估计**:

    - 功能：检验"或许身份信息存在于特征里，只是被 average pooling 摊平了"这一可能性。
    - 核心思路：冻住被测模型的 backbone 末层特征，挂一个 attention pooler (Yu et al., 2022)——用一个可学查询通过 cross-attention 聚合特征 tokens，然后接 ArcFace 头在 Glint360k 上训练。这个"探针"已经不是原来的年龄估计器，而是用其特征作为输入的新建人脸识别系统；如果这个新系统都识别不好，就说明特征本身缺少可被合理探出的身份信息。结果：Commercial age estimator 在 LFW 上 FNMR 从 27% 降到 2%（看起来可观）但在 AgeDB-30 上仍有 67%，CFP-FP 上仍有 28%，远未达到 ArcFace 的 2.4% / 1.2%。
    - 设计动机：先做"读取实验"（average pooling）测原系统的现实泄露，再做"对抗实验"（attention probe）测特征理论上限；两层共同支撑论点，避免被"你只是没用对探测方法"反驳。
