...
1. **相对预测预算 (Relative Prediction Budget, RPB)**：

    - 功能：以「相对于鲁棒基线 $\mathcal{A}$ 的可花费预算 $B_t\ge 0$」为统一原语，刻画并改进所有现存学习增强调页算法的预测使用强度。
    - 核心思路：把 BlindOracle&LRU、PredictiveMarker、LMarker、F&R 重新解读为「不同赚取规则下的 RPB 算法」——例如 PredictiveMarker 在每个 clean request 上一次性赚 $H_k$ 单位预算，LMarker 只赚 1 单位；F&R 则是在每步与 Belady 同代价时赚 1 单位。本文指出这些规则都缺乏「与预测有效性的细粒度耦合」，所以鲁棒常数被锁死；而 RPB 要求预算赚取同时挂钩「过去若干步的本算法表现」与「基线在最坏 lazy 对抗下的期望代价」。
    - 设计动机：给出第一把统一的尺子，既能解释「为什么之前都卡在 $2H_k$」，又指出「往哪儿做就能突破到 $H_k+O(1)$」。

2. **RPB-OnOPT 框架（在线最优底座 + 自适应预算）**：

    - 功能：把 RPB 装在 OnOPT 这类在线最优算法（OnlineMin 是其实例）上，而非装在 Marker 上，从源头解锁更小的鲁棒常数。
    - 核心思路：(i) 当请求落在 $L_0$ 上时，按预测驱逐并把预算重置为常数 $\tau=O(1)$——此时不确定性最大，定理 5.1 证明「按预测驱逐」对完美预测即可达成最优，是 1-consistency 的最小预测使用量；(ii) 当请求落在 $L_i,i>0$ 上时，先用 OM 的候选集规则 $V=C\cap\bigcup_{l=1}^{z}L_l$（其中 $z$ 是最小满足 $|C\cap\bigcup_{l=1}^{j}L_l|=j$ 的下标）保证驱逐仍处在某个 valid configuration 中，再让 RPB 决定是否听预测。OnOPT 比 Marker 更强地把「保持在 valid configuration 内」这件事算法化，给后续的 $H_k$ 级势函数分析提供了不破的基底。
    - 设计动机：Marker 的标记机制本身就限制了 $2H_k+O(1)$ 的下界；换到 OnOPT 后，竞争比的「基线」从 $2H_k-1$ 降到 $H_k$，再叠加 $O(1)$ 加性项就能逼近信息论下界。

3. **RPB-OM 实例化与判定函数 $\mathcal{J}_{RPB}$**：

    - 功能：给出 RPB-OnOPT 的具体好实例 RPB-OM，证明它同时是 1-consistent 和 $(H_k+O(1))$-robust。
    - 核心思路：把 OnOPT 第 21 行的「自定义优先级」替换为 OM 的随机优先级；评估器取 $Y=U(\omega)=k-|R(\omega)|$（未揭示层数，越小说明已揭示越多、之前预测越有效）；判定函数取 $\mathcal{J}_{RPB}(Y,\omega)=\big(U(\omega)\le (Y+2)/e-2\big)$，更新函数取 $\mathcal{U}_{RPB}(\omega)=U(\omega)$。直觉：预算只在「当前未揭示层数显著小于过去同期、说明过去的预测确实减少了不确定性」时才 +1，避免被对抗序列骗着无脑加预算。鲁棒性分析依赖新证明的性质：在 $L_0$ 上服务一个请求时 OM 的势函数最多增加 $H_k-1$（推论 5.5），在 lazy adversary 请求 $p\in N(\omega)$ 上势函数至少减少 $1/(U(\omega)+1)$（引理 5.6），这两者合起来能把 RPB-OM 的额外代价控制在加性常数内。
    - 设计动机：把「赚预算的触发条件」和「OM 势函数变化的指数衰减结构」对上，使得无论预测多差，算法的累计偏离也只比 OM 多一个 $O(1)$ 加性项。
