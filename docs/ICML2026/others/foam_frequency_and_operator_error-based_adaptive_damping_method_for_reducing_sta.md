...
FOAM 的核心逻辑发生在每 $\mathbf{f}$ 步的"检查点"上，对左右因子各自独立地走三阶段控制流：先用代理 $h_t$ 感测当前阻尼下的相对算子误差，再按乘性规则更新 $\epsilon_t$，最后看更新后的 $\epsilon_t$ 是否仍在 $\epsilon_{\max}$ 限内——是就继续复用旧特征空间只更新阻尼项（成本 $O(m^2)$ 级别），否就触发一次完整 EVD 并把 $\epsilon$ 复位到基线 $\epsilon_0$。

### 关键设计

1. **相对算子误差代理 $h_t$（Sensing）**:

    - 功能：在不做新一次 EVD 的前提下，给出"当前陈旧因子相对真实因子的逆根误差"的低成本上界估计，作为后续控制的输入信号。
    - 核心思路：作者证明 $\|\Delta_L^{(1)}\|_F / \|\hat{L}_t^{-1/p}\|_F \le (\alpha / p) \cdot \text{RC}(\epsilon_{t-1})$，其中 $\text{RC}(\epsilon) = \|\hat{L}_t^{-1/2} (L_t - \hat{L}_t) \hat{L}_t^{-1/2}\|_F$ 是 "白化后的漂移 Frobenius 范数"，$\alpha(\epsilon) = \|\hat{L}_t^{-1/p}\|_2 / \|\hat{L}_t^{-1/p}\|_F$ 是把谱范数转 Frobenius 的比例系数。把右端记为 $h_t$，所有用到的量都能在已缓存的特征向量 $Q_L$ 和特征值 $D_L$ 上算，无需重新 EVD。
    - 设计动机：之前的工作（如 Eschenhagen 等用对角化残差做触发判据）测的是"$Q_L^\top L_t Q_L$ 离对角矩阵多远"，这只反映特征空间还能不能近似对角化当前统计量，无法量化逆根映射本身的扰动。相对误差直接锚在 $\|\hat{L}_t^{-1/p}\|_F$ 上、各向异性地放大对小特征值方向的漂移，正好对应逆根放大效应最强的方向。

2. **乘性反馈阻尼控制（Adapting）**:

    - 功能：把 $h_t$ 转成下一步要用的阻尼值 $\epsilon_t$，让 $\epsilon$ 自适应跟踪算子误差的实时变化。
    - 核心思路：使用乘性规则 $\epsilon_t \leftarrow \max(\epsilon_0, \epsilon_{t-1} \cdot h_t / \tau)$，其中 $\tau \in (0,1)$ 是相对误差的容许阈值；若 $h_t > \tau$（误差超标）则按比例增大 $\epsilon$，若 $h_t \le \tau$（裕量足够）则按比例缩小 $\epsilon$，并设 $\epsilon_0$ 为永远不会跌破的地板防止 $\epsilon$ 过小重新引爆 Lipschitz 敏感性。
    - 设计动机：直接固定 $\epsilon$ 在引入 $\mathbf{f}$ 后无法适应——$(1-\beta^{\mathbf{f}})$ 随 $\mathbf{f}$ 线性涨，但 $\epsilon$ 一旦定下来就回不来。乘性比加性更合理，因为算子误差对 $\epsilon$ 的依赖本身就是 $\epsilon^{-(p+1)/p}$ 这种幂律形式，乘性反馈正好匹配该衰减率，能在大动态范围内稳定调控。

3. **基于 $\epsilon_{\max}$ 的延迟触发 EVD（Frequency Control）**:

    - 功能：决定什么时候放弃"提阻尼救场"、转而花一次 EVD 真正刷新特征空间。
    - 核心思路：在更新完 $\tilde{\epsilon}_t$ 后判 $\tilde{\epsilon}_t \le \epsilon_{\max}$ 是否成立：若成立则直接拿 $\epsilon_t = \tilde{\epsilon}_t$ 重新组装 $\hat{L}_t^{-1/p} = Q_L (D_L + \epsilon_t I_m)^{-1/p} Q_L^\top$（只是把对角阻尼项换掉，特征向量 $Q_L$ 和原始特征值 $D_L$ 不变，开销极小）；若 $\tilde{\epsilon}_t > \epsilon_{\max}$ 说明用阻尼已经救不回来，此时才做完整 $\text{EVD}(L_t)$ 并把 $\epsilon$ 复位到 $\epsilon_0$。
    - 设计动机：Theorem 5.4 揭示"过大的 $\epsilon_0$ 会带来 $\epsilon_0 D^2$ 项膨胀 regret"——也就是说 $\epsilon$ 不能无限制涨，必须留个上界。$\epsilon_{\max}$ 同时充当两个作用：保证阻尼不会大到压死有用的预条件信息，并把 $\epsilon$ 是否"用尽"作为触发 EVD 的唯一判据，让 EVD 频率严格随真实漂移自适应，而不是按时间表盲刷。
...
- [\[ECCV 2024\] On the Error Analysis of 3D Gaussian Splatting and an Optimal Projection Strategy](../../ECCV2024/others/on_the_error_analysis_of_3d_gaussian_splatting_and_an_optimal_projection_strateg.md)
