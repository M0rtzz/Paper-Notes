...
description: >-
  [ICML2026][等变性] 本文在一个架构无关的抽象框架下证明：只要参数满足"弱可辨识性"，端到端 $G$-等变的深度网络一定存在等价参数化，使得每一层都对某个潜在群作用等变；这从理论上解释了"端到端等变会自动塌缩为逐层等变"这一长期被实验观察到的现象。 领域现状：几何深度学习的主流范式是"逐层等变"——先给定一个作用在…
tags:
  - "ICML2026"
  - "等变性"
  - "可辨识性"
