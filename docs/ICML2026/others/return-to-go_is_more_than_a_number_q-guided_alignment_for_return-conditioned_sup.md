...
### 整体框架
两个核心组件联合优化：（1）DT 策略网络 $\pi_\theta(s, \text{RTG})$；（2）双 Q 函数 $Q_\psi(s, a)$。策略接收序列 $\tau_t = (\text{rtg}_{t-k+1}, s_{t-k+1}, a_{t-k+1}, \ldots, \text{rtg}_t, s_t)$，推理时所有 RTG token 可被修改为 $\tau_t^g$（加偏移 $g$），实现对策略行为的细粒度控制。整体训练目标含监督学习损失（跟踪数据集）+ 对齐损失（RTG-行为单调性）。
