...
2. **梯度投影机制 (FGR Rectification, 核心创新)**:
    ...
    - 核心思路：把可行半空间定义为 $\mathcal{C}_\text{ID}=\{\mathbf{g}\mid \mathbf{g}^\top\mathbf{g}_{\text{calib}}\ge 0\}$，即所有「在 ID 校准方向上不退步」的更新方向；若 $\mathbf{g}_{\text{main}}\cdot\mathbf{g}_{\text{calib}}\ge 0$ 直接用 $\mathbf{g}_{\text{main}}$，否则做欧氏投影 $\mathbf{g}_\text{final}=\mathbf{g}_{\text{main}}-\frac{\mathbf{g}_{\text{main}}\cdot\mathbf{g}_{\text{calib}}}{\|\mathbf{g}_{\text{calib}}\|^2+\epsilon}\mathbf{g}_{\text{calib}}$。Proposition 4.1 证明这正是 $\mathbf{g}_{\text{main}}$ 到 $\mathcal{C}_\text{ID}$ 的欧氏投影，从而对足够小步长 $\eta$ 有 $\mathcal{L}_{\text{calib}}(\theta-\eta\mathbf{g}_\text{final})\le\mathcal{L}_{\text{calib}}(\theta)+\mathcal{O}(\eta^2)$。
