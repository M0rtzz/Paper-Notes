...
1. **参考集 $R$ + FindPivot 节点分区**:
    ...
    - 核心思路：FindPivot 在 $R$ 内递归找排名更高的邻居，递归预算为 $k$；若成功，则 $\text{pivot}(u)\in R$ 或 $u$ 自己是 singleton，归入 $A$；若 timeout 但邻居都不在 $R$，归入 $B$。Lemma 2.5 保证「同一簇中节点要么全在 $A$ 要么全在 $B$」，使得两侧估计可独立做。
