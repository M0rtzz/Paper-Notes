...
3. **匹配下界 (Thm 5.2)**:
    ...
    - 设计动机：双层 $\arg\min$ 让 Bartlett 2019 的经典 bit extraction 不能直接用（连续优化未必落到指定离散点），于是用 $C\sum_m\prod_k(\theta_m-k)^2$ 把 $\theta$ 强力拽到格点，再单独把 bit-extraction 多项式 $E_c$ 的贡献调到比 $C$ 小一个量级，构成「主项强迫离散 + 次项实现 bit 编码」的两层结构。
