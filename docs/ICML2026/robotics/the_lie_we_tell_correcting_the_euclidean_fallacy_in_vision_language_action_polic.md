...
**1. 左不变 SDE 前向扩散 + 指数映射回拉：在切空间加噪、在流形端做群乘**

欧氏做法在旋转矩阵上加 Gaussian 噪声几乎必然破坏 $R^\top R = I$，只能事后用 SVD 投影救场——可这既改写了反向 SDE，又对近退化矩阵敏感（小预测误差被放大成大旋转误差），还破坏训练-推理一致性。LDA 把加噪挪到李代数 $\mathfrak{se}(3)$（SE(3) 在单位元处的平直切空间），那里 Gaussian 是良定义的，再用满射的指数映射 $\exp$ 把扰动结合回位姿。前向过程取 Stratonovich 形式

$$\mathrm{d}g_t = g_t \cdot \Big(\sigma_t \sum_{i=1}^6 E_i \circ \mathrm{d}W_t^i\Big)$$

（$\{E_i\}$ 是 $\mathfrak{se}(3)$ 正交基），离散化即 $g_t = g_0 \cdot \exp(\sigma_t \boldsymbol{\xi})$，$\boldsymbol{\xi} \sim \mathcal{N}(\mathbf{0}, \mathbf{I}_6)$。由于 SE(3) 在群乘下闭合、$\exp$ 把任意 twist 映成合法刚体变换，命题 4.1 保证 $g_t \in SE(3)$ a.s. 全程成立；反向 SDE 的离散实现是 $g_{t-\Delta t} = g_t \cdot \exp(\sigma_t^2 s_\theta(g_t, t) \Delta t + \sigma_t \sqrt{\Delta t} \boldsymbol{\zeta})$。把所有几何约束推进群结构后，网络再也不用学"先输出糟糕矩阵、再让 SVD 救场"，可以专注操作语义。

**2. 切空间 score matching + Adjoint 等变性：让 score 输出 twist，坐标系等变自动成立**

score 网络不再回归 $\mathbb{R}^{12}$ 噪声，而是输出 $\mathfrak{se}(3)$ 中的 6 维 twist：预测头分两路 MLP 分别回归角速度 $\boldsymbol{\omega}^h$ 和线速度 $\mathbf{v}^h$（对应 SE(3) 半直积结构），立刻喂给 $\exp$ 做去噪更新。训练用切空间 denoising score matching：从专家轨迹 $\mathbf{g}_0$ 采 $t$、$\boldsymbol{\xi}^h$，构造 $g_t^h = g_0^h \cdot \exp(\sigma_t \boldsymbol{\xi}^h)$，最小化 $\|s_\theta(g_t^h, t) - \boldsymbol{\xi}^h\|^2$。这么定义的好处由理论 4.2 给出：最优 score 满足 $s_\theta(h \cdot g, t) = \mathrm{Ad}_h(s_\theta(g, t))$，其中 $\mathrm{Ad}_{(R, \mathbf{p})}(\boldsymbol{\omega}, \mathbf{v}) = (R\boldsymbol{\omega}, R\mathbf{v} + [\mathbf{p}]_\times R\boldsymbol{\omega})$——也就是工作空间整体刚体变换时，score 输出会按 adjoint 自动协变。等变性是机器人部署的硬需求：同一任务换个相机外参或换张桌子的摆位不该让策略重学。把 score 定义在 body-fixed 切空间，网络学到的就是"任务本身的几何"而非"实验室坐标系"，这正是 CALVIN ABC→D 零样本迁移要考的能力。

**3. 几何确定性 ODE → 测地线偏置：让生成轨迹走螺旋运动而非欧氏直线**

欧氏插值会跨越物理不可实现的中间位姿，产生大的角加加加速度（jerk）、转成执行器抖动。LDA 的反向 SDE 对应一个概率流 ODE $\mathrm{d}g_t/\mathrm{d}t = g_t \cdot \sigma_t^2 s_\theta(g_t, t)$，命题 4.3 指出：若 score 沿轨迹近似常向量 $\boldsymbol{\xi}^*$，这个 ODE 解就是 SE(3) bi-invariant 度量下的测地线，即角速度与线速度都恒定的螺旋运动（Chasles 定理意义下的 screw motion）。即便实际 score 随时间变化，这种 intrinsic 形式也会把生成轨迹偏置到接近测地的方向。论文的 look-ahead consistency 实验测了相邻 diffusion step 上 $\hat{x}_0^{(t)}$ 与 $\hat{x}_0^{(t-1)}$ 的 geodesic jitter，欧氏基线比 LDA 高近一个数量级——对真机部署是直接的稳态控制收益。
