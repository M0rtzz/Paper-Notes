...
1. **认知导向的 4 维任务分类法（Counting / Permanence / Reference / Imitation）**:

    - 功能：把抽象的"记忆"概念解耦到 when/where/what/how 四个可独立评测的认知维度，每维 4 个难度递增任务，覆盖马尔可夫策略一定会失败的场景
    - 核心思路：Counting suite 测时序记忆（如 PickXTimes 要按指定次数重复抓取、StopCube 要在特定时刻按按钮）；Permanence suite 测空间记忆（VideoUnmask/ButtonUnmask 在所有方块被遮挡的前提下要根据颜色记忆找出目标，Swap 变体还会动态交换容器位置）；Reference suite 测物体记忆（PickHighlight 要拿起短暂高亮过的方块、VideoPlaceButton/Order 要按语言时序/序数引用执行）；Imitation suite 测程序记忆（MoveCube/InsertPeg/PatternLock/RouteStick 要在看完一段视频示范后复现同样的抓握模式、插入方向、线性或圆周轨迹）
    - 设计动机：以前的基准要么单测一类记忆要么混在一起测，无法定位"模型到底缺哪种记忆"；按认知维度切开后，每个模型的强弱画像可以直接读出来，便于后续诊断和改进

2. **MME-VLA 模型套件：三种记忆表征 × 三种集成方式**:

    - 功能：在固定的 π0.5 backbone 上实现 14 种记忆架构变体，形成可正交消融的设计空间
    - 核心思路：**表征侧**——符号记忆把历史压成自然语言子目标（SimpleSG 仅描述、GroundSG 还带前视图坐标 [x, y]），可由 Gemini-2.5-Pro、微调过的 Qwen3-VL-4B、或 Oracle 仿真器真值生成；感知记忆把历史保留为视觉 token，用 token dropping（按 RGB 差异去冗余）或 frame sampling（均匀降采样）控制在 512 token 预算内；循环记忆用 TTT（在线更新快权重）或 RMT（学习记忆 slot）把序列压缩成定长隐状态。**集成侧**——Memory-as-Context 把记忆 token 直接拼到 VLM 输入；Memory-as-Modulator 用 AdaLN 在 action expert 每层把记忆经多头注意力转成 scale/shift 来调制动作特征；Memory-as-Expert 加一条专门的记忆专家通路，与 VLM/action expert 通过 blockwise causal attention 通信
    - 设计动机：之前的记忆方法用不同 backbone 自报家门，根本不可比；锁死 π0.5、锁死 512 token 预算后，剩下的差异完全可以归因到"记忆怎么存"和"记忆怎么用"，这才是科学的对照实验

3. **严格非马尔可夫的数据构造与统一评测协议**:

    - 功能：保证每个任务"必须用历史信息才能正确决策"，并锁住所有可能引入差异的混淆变量
    - 核心思路：所有任务设计成相同观测可能对应不同历史 → 不同正确动作（如同样看到一个红色按钮，前面已按 2 次还是 5 次会触发不同子动作）；视频条件任务在初始步给一段历史视频帧 + 配对 proprioception，执行过程中只给当前帧；动作 chunk 训练长度 20、执行长度 16；评测时每任务 50 episodes、最大 1300 步，结果在最后 3 ckpt × 3 seeds 下取平均（共 9 次运行）；同时把 π0.5、π0.5 w/ past actions、SAM2Act+、MemER 作为外部对照
    - 设计动机：以前的"记忆基准"很多任务其实当前帧也能解，导致 baseline 也能拿高分；本基准从源头切断这种 shortcut，是评测记忆的必要前提；统一评测协议则让 14 个变体的横向对比真正可信
