...
description: >-
  [ICML 2026][机器人][软体机械臂] 本文针对"视觉-语言操作研究几乎只覆盖刚性臂、忽视软体连续体臂"这一空白，构建了 ManiSoft 基准：用"Cosserat 杆软体动力学 + MuJoCo 刚体接触 + 弹性力约束耦合"的混合仿真器，定义 4 类反映软臂控制难点的任务…
tags:
  - "ICML 2026"
  - "机器人"
  - "软体机械臂"
...
1. **软-刚混合仿真器（Cosserat 杆 + MuJoCo + 弹性力约束）**:

    - 功能：在一个仿真栈中同时拿到"准确弹性形变"与"稳定接触摩擦"，弥补单一类型仿真器在软臂操作场景的能力空洞。
    - 核心思路：把软臂解成两个独立但耦合的子系统。**软体**用 Elastica 离散为 $N$ 段 Cosserat 杆，外部驱动力矩 $\boldsymbol{\tau}_e\in\mathbb{R}^{N\times 3}$ 沿杆产生轴向/剪切/弯曲/扭转四种应变，进而生出内部力 $\mathbf{f}_i$ 与内部力矩 $\boldsymbol{\tau}_i$，共同决定杆的瞬时形变。**末端执行器**及其与环境的接触摩擦交给 MuJoCo，复用其成熟的接触求解器。**两者用一根零静长的虚拟弹簧耦合**：当软体尖端与末端执行器的相对位移 $\Delta\mathbf{x}\in\mathbb{R}^3$、相对旋转 $\Delta\boldsymbol{\theta}\in\mathbb{R}^3$ 不为零时，按 Hooke 律产生恢复力与恢复力矩 $\mathbf{F}=-k_F\Delta\mathbf{x}$、$\mathbf{M}=-k_M\Delta\boldsymbol{\theta}$（$k_F, k_M$ 为可调刚度），把两边拉回协同运动。视觉观测由 Blender 用固定相机渲染 RGB。
    - 设计动机：传统两选一——纯软体仿真无法稳定处理桌面接触，纯刚体仿真无法连续变形——都让接触密集的软臂操作不可仿。弹性约束相当于一个"软连接器"，既保证物理意义上的力闭合、又解耦了两个仿真器各自的数值积分，使得 STK 这种长时密集接触任务真正可仿。

2. **四类任务 + 双档随机化的评测协议**:

    - 功能：在同一套接口下覆盖"基础轨迹控制 → 精细位姿 → 接触堆叠 → 综合避障"难度阶梯，并通过 clean/randomized 双档暴露视觉与物理变化下的泛化崩溃。
    - 核心思路：每个时间步 $t$，给定指令 $\mathbf{L}$ 与视觉观测 $\mathbf{V}_t$，策略输出 $\mathbf{A}_t=(\boldsymbol{\tau}_e, S)$，其中 $\boldsymbol{\tau}_e$ 为外部力矩、$S\in\{0,1\}$ 为夹爪开合，环境自回归推进直到任务成功或超过最大步数 $T$。**关键决定**：不像刚臂基准那样把关节角作为本体感知喂给模型，ManiSoft **故意只给视觉观测、不给软体内部状态**，逼迫策略真正学会"看图判断当前形变"。每个任务做两档场景：**clean**——只有目标物、固定布局/外观；**randomized**——加入若干干扰障碍、随机化纹理与光照强度，同时为每个物体生成多种属性化描述（"yellow bottle"/"bottle with green cap"/"tall plastic bottle"）以增强语言多样性。指标用 success rate 与完成步数 #Steps。
    - 设计动机：(a) 不给本体感知反映"真实软臂没编码器"的物理事实，也是当前 VLA 最薄弱、最需要研究的能力；(b) 双档随机化把"在固定场景刷成功率"和"真正鲁棒的视觉-语言泛化"区分开，使评测能映射到部署痛点；(c) 四任务由易到难给出诊断梯度——若某模型 COLL 高但 ALN/STK 低，说明它能粗定位但不能精控。

3. **分层路径点-力矩专家轨迹生成（高层规则 + 低层 RL）**:

    - 功能：把"软臂力矩控制下生成高质量长轨迹"分解为可规模化的两步，避免直接 RL 学长时距力矩控制的训练崩溃。
    - 核心思路：**高层**——每个任务用人工写的规则规划器生成一串 6-DoF 路径点 $\hat P\in\mathrm{SE}(3)$，编码"靠近 / 抓取 / 抬起"等语义子目标。**低层**——一个 RL 执行器以 (目标位姿 $\hat P$、若干段位姿与速度的本体感知、当前位姿 $P$) 为输入、输出力矩 $\boldsymbol{\tau}_e$；位姿差用 SE(3) 对数 $[\mathbf{d}_p,\mathbf{d}_r]=\log(P^{-1}\hat P)$ 度量，标量距离 $d=\|\mathbf{d}_p\|_2+\alpha\|\mathbf{d}_r\|_2$。奖励由两项组成：位姿差奖励 $R_d=-d+k_1\mathbbm{1}_{d<d_1}+k_2\mathbbm{1}_{d<d_2}$（接近目标按阶梯加奖），稳定性奖励 $R_s=-\mathrm{sgn}(\partial d/\partial t)\cdot\beta$ 当 $d\le D$ 时生效、否则为 0——靠近目标后只奖励"误差还在减小"的趋势，惩罚震荡。最佳超参经消融定为 $\beta=1, D=0.3$。训练完成的执行器单独评测可达 54% 单步成功率，组合起来 roll out 即得完整专家轨迹。
    - 设计动机：直接用 RL 学整段力矩序列受不了高维耦合 + 长时距稀疏奖励；纯规则力矩控制器又难以应对软体动力学不确定性。分层把"逻辑结构"留给规则、把"动力学跟踪"留给 RL，两边都各做其所长。稳定性奖励 $R_s$ 是关键细节——可视化显示加了 $R_s$ 后末端位姿差波动显著减小、轨迹更光滑，从而显著提升下游模仿学习的可用性。
