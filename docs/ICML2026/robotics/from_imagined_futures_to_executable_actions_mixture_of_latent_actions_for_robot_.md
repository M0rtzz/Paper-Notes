...
1. **模态感知的逆动力学模型 (MoIDM)**:
    ...
    - 功能：把"两帧 RGB 之间的视觉变化"翻译成离散潜动作 token，是连接想象和控制的核心接口。
    - 核心思路：每路 IDM 有一个独立的时空 Transformer $T^{(m)}$ 和一个独立的 VQ codebook。先用 ViT 编码器把 $o_t$ 和 $o_{t+k}$ 编成特征，再让一组可学习的潜动作 query 通过 $T^{(m)}$ 跟两帧特征做交互，输出 $\tilde{h} = T^{(m)}(q^{(m)}, h_t, h_{t+k})$，最后过 VQ 得到 $z^{(m)} = \text{VQ}^{(m)}(\tilde{h})$。三路 IDM 共享 RGB 推断流水线但通过不同的重建目标（用 SAM2 监督语义、Depth Anything v2 监督深度、CoTracker3 监督光流）拉开模态偏置。
    - 设计动机：单路 IDM 容易把所有因素塞进一个 codebook 而互相干扰；按模态分工后每个 codebook 都被显式约束去捕捉"任务语义/几何结构/交互运动"中的一种，互补性最强，消融里把三路合并成单路或共享 codebook 都明显掉点。

2. **大规模预训练 + 联合微调的训练策略**:

    - 功能：让 IDM 不仅能在真实帧上工作，更能在生成的（带噪声、轻度漂移的）未来帧上稳定推断动作。
    - 核心思路：先在 Open X-Embodiment + AgiBot 这类大规模机器人数据上独立预训练三路 IDM（用真实未来帧），再在下游任务上把视频生成模型冻结，把 MoIDM 和动作头一起端到端微调；这样 MoIDM 既保留了从大规模视觉变化里学到的通用动作语义，又能 co-adapt 到"生成帧"这个新分布上。
    - 设计动机：消融 Q2/Q3 显示，去掉预训练（from scratch）会严重掉点，而冻结 MoIDM 又会让它跟不上策略的进化——MoIDM 必须既"被预训练得见多识广"又"在下游能跟着策略一起调"，这俩缺一不可。

3. **基于 flow matching 的扩散动作头**:

    - 功能：把离散潜动作 + 生成视觉特征解码成连续、时间相关、多模态的真实机械臂控制序列。
    - 核心思路：动作头用 Diffusion Transformer (DiT) 架构，从头训练；训练目标是 flow matching 而不是标准扩散损失，学的是从噪声动作样本到目标动作的连续变换。条件输入是三路潜动作 + 预测帧的视觉特征。
    - 设计动机：潜动作只是"从想象中蒸馏出的中间线索"，最终控制仍是连续多模态分布；消融 Q4 显示用更轻的自回归 token 头虽然也能工作（说明潜动作信息量够），但 DiT + flow matching 在长时序依赖、多模态、对不完美潜动作的鲁棒性上都更强。
