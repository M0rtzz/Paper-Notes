...
1. **Dual-stream MMDiT 架构**:
    ...
    - 核心思路：每个 MMDiT block 内，动作流和视觉流分别走自己的 FFN/LayerNorm，只在 cross-modal attention 层临时 concat 做 self-attention，然后再 split 回各自分支。两条流各拿一份 AdaLN 时间步嵌入（对应自己的 $\tau_A$ 或 $\tau_o$），从底层就解耦动力学。MMDiT 之后还各接 4 层模态专属 DiT 做最后的精修——视觉流专注于重建语义一致的未来 embedding，动作流专注于低层运动控制。
