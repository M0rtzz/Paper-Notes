...
1. **风险解耦的任务公式 + 7 类风险分类法**:

    - 功能：把"安全 vs 任务"明确解耦，并约束 guardrail 必须同时输出三种粒度的判断。
    - 核心思路：定义 $\mathcal{R}:(I,a)\to(r_{\text{bin}}\in\{0,1\},\ r_{\text{type}},\ h)$，其中 $r_{\text{type}}$ 限定为 7 类（Fire / Electrical / Slip-Trip-Fall / Cut-Sharp / Crush-Pinch / Contamination / Chemical-Toxic Exposure），按 WHO ICD-11 + CPSC NEISS 事故数据库取的，每类再细化成 24 个 risk-inducing pattern。$h$ 用自由文本描述 hazard configuration 而非闭集标签，方便迁移到没见过的物体组合。评估时也分三层：Potential Risk Acc → Risk Type Acc → Hazard Acc，且后两者条件在 $r_{\text{bin}}$ 判对的样本上算，避免"猜对但理由错"被算成功。
    - 设计动机：分层任务定义直接对应"先回答要不要拦 → 拦什么 → 为什么拦"，policy 拿到 partial 信息时也能 fallback。后续 Figure 8 实证：完整 (risk_type + hazard) 正确时 mitigation 对齐率 90.4%，全错时跌到 28.4%，验证三层都必要。

2. **基于 scene graph 的组合式变体生成**:

    - 功能：把 2.4K 种子场景扩成 15.1K 训练对，覆盖"危险/良性 × 单/多风险 × 同 hazard 不同 action"的四象限。
    - 核心思路：把每个 hazard 表成场景图 $\mathcal{H}\subseteq\mathcal{G}$ 的子图（如 (power_strip, beneath, plant_pot)），然后在图上做四种变换：(a) scene augmentation $f_{\text{scene}}:(\mathcal{G},\mathcal{H})\to(\mathcal{G}',\mathcal{H})$ 加无关物体保持原 hazard；(b) hazard addition $f_{\text{hazard}}^{+}$ 引入新 hazard 形成 Selective Risky；(c) action modification $f_{\text{action}}:a\to a'$ 改动作切断和 hazard 的交互 → Decoupled Benign；(d) hazard removal $f_{\text{hazard}}^{-}:(\mathcal{G},\mathcal{H})\to(\mathcal{G}',\emptyset)$ → Absent Benign。在图层操作而不在文本层操作，避免文字改写无意中破坏关键空间关系；最后由 GPT-5.1 把变体图渲成 scene description，再喂给 gemini-3-pro-image-preview 出图，由 VQA filter（GPT-4o 看图回答"插座是否在盆栽下方"这类从 hazard 边自动出的问题）过滤掉关键关系丢失的图。
    - 设计动机：四象限覆盖"hazard 存在但动作安全"和"hazard 不存在动作就安全"这两个 *反事实* 配对，是教 MLLM 区分"动作触发风险"而非"看见火就喊危险"的关键监督信号。这一招直接对应解决 baseline 的 over-conservative bias。

3. **冻结视觉编码器的 SFT 配方**:

    - 功能：在小模型上同时学到风险检测 + 自然语言解释，且不破坏原有视觉先验。
    - 核心思路：Qwen-3-VL-2B/4B + LLaMA-Factory + lr=1e-5 + 4 epoch + 8×A6000。关键 trick 是**冻结 vision encoder**——作者在 preliminary experiment 里发现解冻 ViT 后 risk binary 检测确实涨了，但 hazard 解释质量塌方，推测是小模型容量不够同时迁移视觉 + 推理两个方向。
    - 设计动机：把视觉能力当成"借来的现成感知器"，只让 LLM head 学风险因果，是小模型在多任务上避免互相挤占的常用策略，本文用 ablation 反推出来这一选择，对小 guardrail 设计有可复用价值。
