...
1. **模块化解耦 + 张量接口标准化**:
    ...
    - 功能：把异构架构强行套入"五段式"流水线，让任何论文的组件都能拆出来插回去。
    - 核心思路：约定 Embedding 输出必须是 $\mathcal{Z}\in\mathbb{R}^{B\times C\times L\times D}$ 这个四维张量（batch / 变量 / 时间 token / 隐藏维），其中 Point-wise 把每个时间步投影一次（$L=T$）、Patch-wise 切 patch 后投影（$L=\lceil T/S\rceil$）、Variate-wise 把整条变量历史压成 1 个 token（$L=1$）、Identity 不投影、Time-as-Feature 把 $T$ 维直接 reshape 到特征维（零参数）。Encoder 严格只在 $\mathcal{Z}$ 上做 token 间运算，所以替换 Encoder 不会污染 Embedding 的归因。
    - 设计动机：以前评 PatchTST，没法区分到底是 patch 这个视图赢了还是 Transformer 这个 reasoner 赢了。强制统一张量接口后，把 Patch-wise Embedding 接 Identity Encoder 跑一下，就能直接读出"视图独立贡献"。这是后面 Identity Paradox 能成立的方法论前提。

2. **概率化评估协议 (EC 空间 + $\mu / \sigma$ 双统计量)**:

    - 功能：取代单点 MSE 报数，给每个模块算"在一片超参海洋里的期望表现和稳定性"。
    - 核心思路：定义评估条件空间 $\Omega$，每个 $\mathbf{c}\in\Omega$ 是一个配置元组（数据集、look-back $T\in\{96,192,336,512\}$、horizon $P\in\{96,192,336,720\}$、encoder 层数、latent 维 $D\in\{64,128,256,512\}$、学习率、batch、dropout、seed 等）。把模块性能视为随机变量 $L(\theta, \mathbf{c})$，估计两个统计量：$\mu(\theta)=\mathbb{E}_{\mathbf{c}\sim\Omega}[L(\theta,\mathbf{c})]$（边际性能）和 $\sigma(\theta)=\sqrt{\mathrm{Var}_{\mathbf{c}\sim\Omega}[L(\theta,\mathbf{c})]}$（稳定性）。同时报告 $L_{best}=\min_k L(\theta,\mathbf{c}_k)$ 作为"如果搜超参能搜到的峰值"，用来显式对照"最佳陷阱"。
    - 设计动机："公平陷阱"和"最佳陷阱"是单点协议的两个对偶失败模式。$\mu$ 直接对应"用户随便挑超参时能拿到的平均水平"，$\sigma$ 直接对应"超参敏感性"——这才是落地时真正关心的两个数；$L_{best}$ 单独报出来，就能看到"SOTA 宣称"到底是普遍能力还是离群点运气。

3. **分层配对蒙特卡洛采样 (Stratified Paired EC Sampling)**:

    - 功能：把估计 $(\mu, \sigma)$ 的成本压下来，同时消除"不同模块用不同 EC 比较"带来的混淆。
    - 核心思路：从 $\Omega$ 里采一个固定的、按数据集/horizon 分层的条件集 $\{\mathbf{c}_k\}_{k=1}^K$（主实验 $K=600$，每个数据集 100 个）。所有被评对象（EO）严格在同一套 $\{\mathbf{c}_k\}$ 上跑，即用了配对实验设计；不相关维度对该 EO 来说视为忽略。然后 $\hat{\mu}(\theta)=\frac{1}{K}\sum_k L(\theta,\mathbf{c}_k)$、$\hat{\sigma}(\theta)$ 是样本标准差。配对设计让两个模块之差的方差大幅缩小，统计显著性更容易拿到（论文用 one-tailed Mann–Whitney U 检验在 $\alpha=0.05$ 下验证差异）。
    - 设计动机：如果 Identity 和 Transformer 各自在不同 EC 上做评测，差异里有一大部分是"采样到的 EC 分布不同"造成的——这正是当前 leaderboard 的核心问题。分层保证每个数据集/horizon 都被均匀覆盖，配对保证抵消"哪条 EC 本身就难"的系统偏差，蒙特卡洛保证可控样本量下的偏差—方差权衡。
