...
### 整体框架
HiPPO 的核心是用正交多项式（OP）基把信号历史投影成一个可读的系数向量：记输入信号为 $f(t)$，HiPPO 在 $t$ 时刻的状态 $\mathbf{s}(t)$ 满足线性 ODE $\dot{\mathbf{s}}(t) = A \mathbf{s}(t) + \mathbf{b} f(t)$，再经读出 $y(t) = W\mathbf{s}(t)$ 输出。本文把现代 SSM 的五种能力，分别映射成对这一基座**某一个部件**的显式修改：改读出端（Volterra）、改历史测度（Salience）、加一套独立的关联内存库（Associative Memory）、改状态表示（Multiscale）、改训练目标（Forecasting）。五个变体共享同一套可解释的多项式结构，合称 "HiPPO Zoo"——每个变体把一种原本隐式的内存机制摊到多项式系数 / 测度 / 目标上，从"学到的黑箱状态转移"变成能直接可视化和分析的结构。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 30, 'nodeSpacing': 26, 'padding': 6, 'wrappingWidth': 420}}}%%
flowchart TD
    BASE["HiPPO 基座<br/>正交多项式投影历史：线性 ODE 维护状态 s(t) → 读出 y=Ws"]
    BASE -->|改读出端| V["Volterra HiPPO<br/>读出写成 Volterra 多项式级数<br/>→ 显式非线性交互"]
    BASE -->|改历史测度| SAL["Salience HiPPO<br/>标量显著性 g(t) 翘曲测度<br/>→ 自适应内存分配"]
    BASE -->|加关联内存库| AM["Associative Memory HiPPO<br/>地址空间多项式键值库<br/>→ 显式关联记忆"]
    BASE -->|改状态表示| MS["Multiscale HiPPO<br/>状态按尺度轴做多项式展开<br/>→ 跨尺度记忆"]
    BASE -->|改训练目标| FC["Forecasting HiPPO<br/>三套 HiPPO + 预测映射 T<br/>→ 目标诱导的预测记忆"]
```
...
**4. Multiscale HiPPO：把状态写成"尺度轴"上的多项式，一个系统记住所有时间尺度**

普通 HiPPO 的历史测度一旦选定就只对应一个时间尺度——想同时记住短程细节和长程趋势，现代 SSM 只能靠堆叠多层隐式地拼出多尺度。Multiscale HiPPO 把"多尺度"显式化：它不再固定单一尺度，而是把 HiPPO 状态本身看成逆时间尺度参数 $u \triangleq \log g \in [\log\epsilon, 0]$ 的函数 $\mathbf{s}(u)$，再用一组正交多项式去展开这个 $\mathbf{s}(u)$（$u$ 上的均匀分辨率正好对应时间尺度 $g^{-1}$ 上的对数均匀分辨率）。这样单个 Multiscale HiPPO 系统就能在 $10^1$ 到 $10^4$ 跨四个数量级的尺度上同时保持历史，重建误差匹配甚至优于一整组各管一个尺度的单尺度 HiPPO，而且尺度连续、表示简洁——把"多尺度记忆"从堆层的副产品变成一个可读的尺度方向展开。

**5. Forecasting HiPPO：用三套 HiPPO 串起预测目标，显式刻画"为预测而保留的记忆"**

序列模型几乎都用一步预测损失训练，这个目标隐式决定了状态里保留过去的哪些信息，却没人把"目标 → 记忆结构"的因果显式画出来。Forecasting HiPPO 用三套 HiPPO 系统把它摊开（论文 Fig 5A）：系统 1 用 Leg-T 表示近期窗口 $[t-H, t]$，在左端点取值得到滞后信号估计 $u(t) \approx f(t-H)$；系统 2 以 $u(t)$ 为输入、表示更早的历史；训练时学一个线性映射 $T$，从系统 2 的状态去预测系统 1 的状态。这个预测目标在系数空间诱导出一个二次型 $Q = T^\top W T$（称"预测历史度量"），它直接定义了"两段历史在预测意义下有多相似"的几何。把历史投影到 $Q$ 的主子空间就得到该目标下的"预测记忆"：短视界预测器强调小延迟处的细节，长视界预测器保留平滑的长期结构——于是训练目标如何塑造 SSM 的内存结构第一次被显式看见。
...
- **从目标函数出发反推内存**：Forecasting HiPPO 的洞察——预测目标诱导的二次型 $Q = T^\top W T$ 直接定义了历史空间的几何——是深刻的，暗示不同的下游任务应该学习不同的历史表示。
