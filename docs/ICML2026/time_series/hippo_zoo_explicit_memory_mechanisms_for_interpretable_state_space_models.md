...
1. **Volterra HiPPO（非线性交互）**:

    - 功能：在保持线性状态动态的前提下，通过多项式 Volterra 级数读出端实现非线性。
    - 核心思路：将 Volterra 核表示为正交多项式的乘积形式。读出为 $y(t) = \beta^{(0)} + \sum_{k=1}^K \sum_{i_1, \ldots, i_k} \beta_{i_1, \ldots, i_k}^{(k)} s_{i_1}(t) \cdots s_{i_k}(t)$，每个系数张量 $\beta^{(k)}$ 直接编码 $k$ 阶交互强度。
    - 设计动机：黑箱 MLP 读出难以分析哪些过去时间点的交互被学到；Volterra 多项式形式使交互结构完全可视化；在二阶 Volterra 任务上 Volterra HiPPO 比单层 MLP 收敛更快且能精确恢复 Volterra 核。

2. **Salience HiPPO（自适应内存分配）**:

    - 功能：通过时变的标量"显著性"信号实现对过去的自适应加权，关键信息获得更多内存资源。
    - 核心思路：将 HiPPO 的 ODE 乘以显著性信号 $g(t)$：$\dot{\mathbf{s}}(t) = g(t) [A \mathbf{s}(t) + \mathbf{b} f(t)]$。通过时间变换 $t_1 = \varphi(t_0) = \int_0^{t_0} g(s) ds$，可证明这等价于在"翘曲时间"中应用标准 HiPPO，实现对历史测度的显式变形。
    - 设计动机：Mamba 等选择性 SSM 通过高维门控隐式调整内存分配，难以可视化；Salience HiPPO 用标量信号显式扭曲历史测度，选择性复制任务中显著性显著上升，最终达到 100% 精度（对比 S4D 81%、LSTM 60.8%、Transformer 22.1%）。

3. **Associative Memory HiPPO（关联记忆）**:

    - 功能：实现显式的键值关联存储与检索，类似 Transformer 的注意力但用多项式基表示。
    - 核心思路：维护两个组件——HiPPO 系统处理输入序列生成状态 $S_t$；独立的关联内存库存储 $d_{\text{model}}$ 个通道，每个通道 $m_j(x)$ 是定义在地址空间 $x \in [0, 1]$ 上的多项式函数。写操作在地址 $x_{\text{key}}$ 处用最小范数更新使得 $m_j(x_{\text{key}}) = y_t[j]$；读操作在查询地址 $x_{\text{query}}$ 处评估多项式得到值向量。
    - 设计动机：现代 SSM 中关联行为通过隐式的状态依赖更新实现；显式的 OP 基关联内存使键值绑定完全可视化；关联回忆任务上达到 100% 精度，而参数匹配的 S4D、LSTM、Transformer 都接近机会水平（33%）。
