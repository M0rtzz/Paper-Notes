...
### 整体框架
两个阶段——**训练阶段**：给定校准集残差，构造残差补丁 $\tilde{\epsilon}_t = \{\epsilon_{t - w + 1}, \ldots, \epsilon_t\}$ 与目标残差 $\epsilon_{t+1}$ 配对；通过最大化匹配得分递归选择分割锚点，将补丁分组到满足 KS 距离界的叶子中。**推理阶段**：对新补丁 $\tilde{\epsilon}_T$，通过递归比较其与分割锚点的 KS 距离路由到对应叶子，在叶子内应用量化回归森林（QRF）估计量化数构造预测区间；同时在线更新叶子内的 QRF 以适应新观测。

### 关键设计

1. **基于 KS 统计量的递归分箱**:

    - 功能：通过分布相似性将残差分组到独立的子组，诱导近似可交换的叶子节点。
    - 核心思路：定义匹配增益得分 $\text{MG}(\tilde{\epsilon}_i) = \sum_j \mathbb{1}\{D_{\text{KS}}(\tilde{\epsilon}_i, \tilde{\epsilon}_j) \leq \gamma\}$，在每个分割节点选择最大化该得分的锚点 $s$。KS 距离 $D_{\text{KS}} = \sup_x |F_i(x) - F_j(x)|$ 衡量两个经验分布函数的最大偏差，是密度无关的非参数度量。递归分割直到无法找到满足最小样本数 $n_{\min}$ 的更好分割。
    - 设计动机：相比权重重新分配，分箱保持经验分布的完整性；相比 TV 距离等替代方案，KS 统计量计算效率高（$O(w)$ vs $O(n)$），同时对偏斜分布的区分能力强。

2. **残差补丁 + 目标级可交换性**:

    - 功能：建立补丁级分组与目标级可交换性之间的理论桥梁，确保预测覆盖率有效性。
    - 核心思路：定义 $\gamma^*$-近似局部可交换性为 $\max_{t \in \mathcal{L}_{k^*}} D_{\text{KS}}(P_{t+1}, P_{T+1}) \leq \gamma^*$，即叶子内所有目标残差的分布与未见目标分布的 KS 距离有界。通过局部平稳性与 $\beta$-混合假设，当补丁级 KS 界为 $2\gamma$ 时，可证明目标级界为 $2 C \gamma + \mathcal{O}(\sigma_{\text{mix}})$。
    - 设计动机：相比全局可交换性，局部可交换性允许温和的分布漂移；通过补丁级测试实现目标级保证，避免直接估计未来分布的困难。

3. **在线自适应 + 集成鲁棒性**:

    - 功能：在长序列中维持有效覆盖率，处理严重分布漂移。
    - 核心思路：$B$ 个自助树的集成，每棵树用采样比率 $\theta$ 的自助样本构建。对每个未见补丁，至少一棵树以概率 $p_{\min}$ 将其路由到匹配的右叶子；通过平均 $B$ 棵树的量化估计 $\bar{q} = \frac{1}{B} \sum_b q^{(b)}$ 获得鲁棒预测。每次观察新残差后更新对应叶子的 QRF 而不改变树结构。
    - 设计动机：集成策略在极端漂移下提供多条后备路由路径；在线 QRF 更新使局部量化自适应演变而无需昂贵的树重建，保持 $O(T w \log n)$ 的在线成本。
