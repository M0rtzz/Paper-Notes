...
IMPACT 流水线有三阶段：阶段 I 用多通道偏差损失训初始模型并算每个训练样本的影响分数，划出污染集 $\mathcal{D}_{con}$、参考正常集 $\mathcal{D}_{ref}$、剩余清洁集 $\mathcal{D}_{clean}$；阶段 II 在影响分数指引下做"标签翻转 + 特征扰动"，构造翻标后的 $\mathcal{D}_{con}'$ 与扰动特征集 $\mathcal{W}_{per}'$；阶段 III 增加未见异常学习头，用 $L_{seen}+\lambda L_{unseen}$ 联合训练；推理时取最大跨通道异常分加上对参考正常质心的特征偏差作为最终分数。
...
1. **多通道偏差损失（TIS 的训练支柱）**：
    ...
    - 功能：让模型把正常样本的 $r$ 通道异常分推向各向同性高斯先验 $\mathcal{N}(\bm\mu,\bm\Sigma)$ 的均值 $\bm\mu_r$，把异常样本至少推开 $a$，并通过马氏距离 $\mathit{dev}(\bm x_i)=\sqrt{(f(\bm x_i,\theta)-\bm\mu_r)^\top\bm\Sigma_r^{-1}(f(\bm x_i,\theta)-\bm\mu_r)}$ 度量偏差。
    - 核心思路：损失写为 $L(\bm z_i,\theta)=\tfrac{1}{r}\sum_{j=1}^r[(1-y_i)\mathit{dev}(\bm x_i)_j+y_i\max(0,a-\mathit{dev}(\bm x_i)_j)]$。多通道相当于"从多个角度评估异常"——对正常样本所有角度都该对齐高斯先验，对异常样本只要任一角度偏离超过 $a$ 就算异常。定理 1 进一步证明这等价于最小化潜变量分布的熵 $\mathcal{H}(S)=\tfrac{r}{2}(1+\log(2\pi\sigma^2))\propto\log\sigma^2$。
    - 设计动机：影响函数要求损失对参数二阶可微且 Hessian 可逆，传统 deviation loss 是单通道、对方差信息丢失，多通道版本既扩大表达力又给后续 $H_{\hat\theta}^{-1}\nabla_\theta L$ 计算提供了更稳定的梯度结构；熵最小化解释让"压紧正常 + 推开异常"的几何直觉有了信息论依据。

2. **影响函数驱动的双重操作（RADG 的核心）**：

    - 功能：用同一套影响分数 $\mathcal{I}_L(\bm z_i)=\sum_{\bm z_t\in\mathcal{V}}\mathcal{I}_L(\bm z_i,\bm z_t)$ 同时完成污染纠正与未见异常生成。
    - 核心思路：(a) 标签翻转——对 $\mathcal{D}_{con}=\{\bm z_i\in\mathcal{D}_n\mid\mathcal{I}_L(\bm z_i)>0\}$ 中的样本，作者证明 $\nabla_\theta L(\bm z_{i\mathbf 1},\theta)-\nabla_\theta L(\bm z_i,\theta)=-2\nabla_\theta L(\bm z_i,\theta)$，于是翻标签的影响 $\mathcal{I}_{L\mathbf 1}(\bm z_i,\bm z_t)=-2\mathcal{I}_L(\bm z_i,\bm z_t)$，定理 2 给出测试风险变化 $\approx -\tfrac{2}{N\cdot|\mathcal{D}_{con}|}\sum_{\bm z_i\in\mathcal{D}_{con}}\mathcal{I}_L(\bm z_i)<0$，即翻标必然降低风险。(b) 特征扰动——对 $\mathcal{D}_{per}=\{\bm z_i\in(\mathcal{D}_n\cap\mathcal{D}_{clean})\mid\mathcal{I}_L(\bm z_i)<0\}$ 中"绝对值最小"的 top-$k$ 边界样本，沿方向 $\mathcal{I}_{per}(\bm w_i)=-\nabla_{\theta_h}L(\mathcal{V},\hat\theta_h)^\top H_{\hat\theta_h}^{-1}[\nabla_\varphi\nabla_{\theta_h}L(\bm w_i,\hat\theta_h)]$ 做扰动 $\bm\varphi_{i\zeta_i}=\bm\varphi_i+\alpha\mathcal{I}_{per}(\bm w_i)^\top$，定理 3 证明扰动特征服从与原分布有正下界距离的新分布、定理 4 给出风险变化 $\approx -\tfrac{\alpha}{N\cdot|\mathcal{W}_{per}|}\sum\|\mathcal{I}_{per}(\bm w_i)\|_2^2<0$。
    - 设计动机：传统污染纠正只删样本（GammaGMM、ExCeeD），相当于浪费信号；标签翻转把"有害样本"变成"宝贵已标异常"，一来一回相当于双倍收益。生成端则跳出图像增强启发式，直接在特征空间沿"最大化风险增量"方向走，既保证新样本"偏离已知模式"（未见性）又"导向风险下降"（有用性）——把数据增强从手工启发变成可证明的优化操作。

3. **双头训练 + 双成分推理**：

    - 功能：分别学已见与未见两类异常，推理时融合最大跨通道异常分和特征偏差分。
    - 核心思路：训练损失 $L_{re}=L_{seen}+\lambda L_{unseen}$，其中 $L_{seen}=\sum_{\bm z_i\in\mathcal{D}_s}L(\bm z_i,\theta)$，$\mathcal{D}_s=\mathcal{D}_{con}'\cup\mathcal{D}_{ref}\cup\mathcal{D}_{clean}$ 涵盖原始标签 + 翻标后异常 + 清洁正常；$L_{unseen}=\sum_{\bm z_i\in\mathcal{D}_h}L(\bm z_i,\theta)+\sum_{\bm w_{i\zeta_i\mathbf 1}\in\mathcal{W}_{per}'}L(\bm w_{i\zeta_i\mathbf 1},\theta_{h'})$，$\mathcal{D}_h$ 是最有帮助的正常样本，扰动特征喂给独立未见头 $h'$。推理时 $s=s_m+s_f$，$s_m=\max_{l<r}(h(\bm\varphi_i,\theta_h)+h'(\bm\varphi_i,\theta_{h'}))_l$ 取跨通道最大，$s_f=\|\bm\varphi_i-\tfrac{1}{|\mathcal{D}_{ref}|}\sum_{\bm x_j\in\mathcal{D}_{ref}}\bm\varphi_j\|^2$ 捕捉细微偏差。
    - 设计动机：用独立未见头 $h'$ 避免伪异常梯度回流污染主干表征；最大跨通道 $s_m$ 与"多角度评估"哲学一致——只要任一通道亮灯就算异常；加上特征偏差 $s_f$ 是为了兜底——当 $h,h'$ 对极轻微的分布漂移不敏感时，参考集质心距离仍能拉开异常分。
