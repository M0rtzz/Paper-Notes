...
1. **特征身份嵌入 (FeatID) 作为跨特征注意力的软邻接先验**:

    - 功能：让每个特征拥有一个跨层、跨时间稳定的语义向量，即使该位置数值缺失也能用身份做注意力。
    - 核心思路：拼接 $e_{t,i} = [\tilde x_{t,i}; \text{PE}(t); f_i; m_{t,i}]$，其中 $f_i \in \mathbb{R}^{d_f}$ 是可学习身份矩阵的第 $i$ 行。注意力分数 $s_{ij}^{(t)} = e_{t,i}^\top A e_{t,j}$ 可以分解为身份先验 $f_i^\top A_{ff} f_j$、身份-上下文交叉项、动态上下文 $r_{t,i}^\top A_{rr} r_{t,j}$ 三部分。当 $x_{t,i}$ 和 $x_{t,j}$ 都缺失时，动态项退化，身份先验项依然完整提供跨特征兼容性。
    - 设计动机：传统 Transformer 在 ImputeFormer 里学到的"静态特征嵌入"只作软空间索引，与缺失状态没有交互；SPIN 的图嵌入又绑定预定义拓扑。FeatID 既不需要图先验，也能在重度缺失下保持锚定。BeijingAir 上消融显示去掉 FeatID 后 Subseq-50% 的 MAE 从 0.166 暴涨到 0.398，可见这是最关键的组件。

2. **双螺旋 Hybrid 编码层（先并行后交叉）**:

    - 功能：让时间和特征两个维度既能独立细化，又能跨维度交换信息。
    - 核心思路：每层分两阶段。Stage 1 并行做 $H_T = \text{TimeMHA}(H^{(l-1)})$ 和 $H_F = \text{FeatMHA}(H^{(l-1)})$，各自独立优化。Stage 2 串行交叉：$H_{TF} = \text{FeatMHA}(H_T)$ 和 $H_{FT} = \text{TimeMHA}(H_F)$。最后四个分支取平均 $H^{(l)} = \frac{1}{4}(H_T + H_F + H_{TF} + H_{FT})$。形状像 DNA 双螺旋而得名。
    - 设计动机：纯串行 Time→Feature→Time 的编码消融时（"w/o Hybrid"）在 Subseq-50% 上 MAE 从 0.166 飙到 0.294，说明并行+交叉的双向流通是处理长缺口的关键。串行编码会把另一维度信息压缩在前一阶段后才传播，造成信息瓶颈。

3. **多层级融合**:

    - 功能：把每一层（包括第 0 层 embedding）的多分支输出都拿出来加权平均，避免只用最后一层导致浅层细节丢失。
    - 核心思路：$\tilde H = \frac{1}{1+4L}(H^{(0)} + \sum_{l=1}^L (H_T^{(l)} + H_F^{(l)} + H_{TF}^{(l)} + H_{FT}^{(l)}))$，故意省掉 $H^{(l)}$ 因为它本身是四分支的平均，避免重复计数。简单平均比可学习门控更好（消融见附录 D）。
    - 设计动机：插补需要精细到 $(t, i)$ 像素级重构，深层抽象往往丢失局部细节，浅层保留的"原始信号"反而对填补有帮助；这与 ResNet 直接连接更好的发现一致。
