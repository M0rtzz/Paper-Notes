...
1. **Context-aware 参数化先验**：

    - 功能：从历史窗口直接推出一个跟数据走的高斯先验，替掉扩散模型里那个"先验方差固定/滑窗算"的死板设定。
    - 核心思路：用一个轻量 MLP $f_\theta$ 把 $\bm{x}$ 映成潜空间的 $(\bm{\mu},\bm{\sigma})$，潜维度 $D$ 设得比较"过完备"，让先验有空间编码丰富的上下文语义；采样 $\bm{z}^{(k)}=\bm{\mu}+\bm{\sigma}\odot\bm{\epsilon}^{(k)}$ 保留可导性。框架对 backbone 无关，换成 Transformer/RNN 同样能用。
    - 设计动机：作者在 Figure 1 里直接给出 motivation：Traffic 这种数据的真实方差在一天之内变化巨大，而 TMDM 的单位方差先验和 NsDiff 的滑窗方差先验都跟不上；让先验"听 $\bm{x}$ 的话"才能解决时变 aleatoric 不确定性。

2. **Push-forward MLP（一步式生成）**：

    - 功能：把结构化的高斯先验直接映射成复杂、非高斯、可能多模态的预测分布，取代扩散的 T 步迭代去噪。
    - 核心思路：用一个两层 channel-independent MLP（GeLU 激活）把 $D$ 维潜码投到 $L$ 维预测窗口；理论上由 push-forward 测度 $(g_\phi)_\# p_\theta$ 在 $W_1$ 意义下对任何具有有限一阶矩的条件分布稠密（Theorem 5.2）。因为先验已经携带了上下文信息，映射负担小，所以一个轻量 MLP 就够。
    - 设计动机：扩散模型的 $T$ 步去噪是为了在 generic 噪声和复杂数据之间架桥；既然先验本身已经被参数化估计"塞进"了上下文，传输映射就不需要再做迭代细化，单步前向就能拿到样本，推理复杂度从 $O(BKT)$ 降到 $O(B+BK)$。

3. **KDE-NLL + 均值 MSE 混合目标**：

    - 功能：联合优化分布形状（calibration）和点预测精度（trajectory）。
    - 核心思路：模型只能出样本不能直接给密度，因此对每个 $(c,t)$ 用 Gaussian KDE 估边缘密度 $\hat{q}_h(y_{c,t}|\bm{x})=\frac{1}{Kh}\sum_k \mathcal{K}\big(\tfrac{y_{c,t}-\hat{y}_{c,t}^{(k)}}{h}\big)$，再求 NLL；同时加一项 $\mathcal{L}_{\text{MM}}=\|\bm{y}-\frac{1}{K}\sum_k\hat{\bm{y}}^{(k)}\|^2$ 锚定一阶矩，总损失 $\mathcal{L}_{\text{total}}=\alpha\mathcal{L}_{\text{NLL}}+\mathcal{L}_{\text{MM}}$（$\alpha=0.1$，$h=0.3$）。
    - 设计动机：纯 KDE-NLL 在训练早期样本离真值远时，Gaussian kernel 几乎为零，梯度消失；论文也给了 Theorem 5.1 的有限 $(K,h)$ 误差界——bias 是 $O(h^2/\varepsilon)$、方差 $O(1/(\varepsilon h\sqrt{K}))$，两者必须协同调。MSE 锚提供稠密、稳定的"找均值"梯度（公式 16），KDE-NLL 用 responsibility-weighted 残差（公式 15）塑形分布，互补。
