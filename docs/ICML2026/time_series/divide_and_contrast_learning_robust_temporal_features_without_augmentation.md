...
description: >-
  [ICML 2026][时间序列][时间序列表示学习] Di-COT 通过**随机划分序列为重叠子块**并对其进行对比学习——在不使用数据增强的情况下高效学习鲁棒的时间序列表示，相比现有方法速度快 2.5 倍、精度更高；6 大规模数据集 + 124 UCR + 28 UEA 上全面验证。
tags:
  - "ICML 2026"
  - "时间序列"
  - "时间序列表示学习"
...
**1. 动态重叠子块划分：把长序列拆成短而完整的单元，顺手避开时间过渡的假正例。**

逐时步对比在事件频繁切换的数据上会把过渡处的两个时步误当正例，而只靠时间邻近性的方法又处理不了这种情况。Di-COT 每次迭代从 $\mathcal{U}\{k_{\min},\ldots,k_{\max}\}$ 采子块数 $k\ll T$，子块长度 $L = \frac{T}{1+(k-1)(1-\rho)}$、步幅 $s = \lfloor L(1-\rho)\rfloor$，$\rho\in(0,1)$ 是重叠比，编码后得到 $\mathbf{z}^{(i)} = \{z_1^{(i)},\ldots,z_k^{(i)}\}$（$z_j^{(i)} = f_\theta(\tilde x_j^{(i)})\in\mathbb{R}^F$）。重叠保证相邻子块共享足够上下文、不出现人工硬边界；每迭代随机采 $k$ 让模型接触多种时间粒度、隐含学多尺度鲁棒性；而把对比粒度从 $T$ 降到 $k\ll T$，恰好让时间过渡处的假正例自然消失，又保住了足够学习信号。

**2. 交叉熵对比目标：把 InfoNCE 重述成多类分类，做到长度无关的高效计算。**

传统 InfoNCE 在长序列上复杂度随 $T$ 平方膨胀（CaTT 是 $O(BT^2 d)$），对长序列不友好。Di-COT 对每个子块对 $(j,p)$ 算温度缩放相似度 $S_{j,p}^{(i)} = \frac{\mathbf{z}_j^{(i)\top}\mathbf{z}_p^{(i)}}{\tau}$，把前一个子块定为正标签 $p^*(j) = j-1$（$j>0$）、其余为负，损失 $\mathcal{L}_{\text{CE}} = -\frac{1}{Bk}\sum_i\sum_j\log\frac{\exp(S_{j,p^*(j)}^{(i)})}{\sum_p\exp(S_{j,p}^{(i)})}$。这样复杂度降到 $O(Bk^2 d)$（$k\ll T$），而且每个子块都作锚点、产生 $B\times k$ 个正样本对，比基于增强方法的 $2B$ 个监督信号密集得多。

**3. 无增强策略：完全在原始序列里对比不同部分，不造任何增强视图。**

jittering、permutation 这类时序增强可能破坏有效的时间结构，多次编码器前向又徒增开销。Di-COT 干脆不产生任何增强视图，对比的是同一序列的不同子块而非增强版本，所有子块共享相同语义上下文。它的底气在于：通过子块重叠 + 多粒度采样，模型已经能自然学到对小位移的不变性，不必再靠人工增强去"教"——消融里非线性投影反而降性能，也印证了 CV 那套技巧不能照搬到时序。
