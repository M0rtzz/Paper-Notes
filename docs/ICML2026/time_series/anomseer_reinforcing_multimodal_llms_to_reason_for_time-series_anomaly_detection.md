...
1. **ExpCoT 专家链式推理**:
    ...
    - 功能：把传统时间序列分析中的证据组织成可学习的自然语言轨迹，为 MLLM 提供比最终标签更细的监督信号。
    - 核心思路：ExpCoT 遵循 Observation → Reasoning & Validation → Conclusion 三段式。Observation 先做全局扫描，用直方图异常分数观察极端值；若没有明显全局异常，再做结构扫描，用平滑梯度分析趋势稳定性、用 FFT 分析周期和频率变化；若结构仍稳定，再用 Matrix Profile 搜索局部不相似子序列。Reasoning & Validation 根据真实异常类型选择相应证据，例如用梯度验证趋势漂移、用频域峰值验证频率异常、用 discord 分数验证局部形状异常。Conclusion 再把类型和区间合成最终判断。
    - 设计动机：通用 CoT 往往语言流畅但证据不可靠，ExpCoT 则把“为什么这里异常”落到统计量和时间位置上，使模型学到的不是模板化解释，而是可迁移的分析路径。

2. **TimerPO 的时间序列推理 advantage**:

    - 功能：在 GRPO 的结果奖励之外，度量模型回答与 ExpCoT 在语义推理轨迹上的接近程度。
    - 核心思路：模型对每个样本生成一组回答，主奖励由格式、异常类型分类和区间定位组成，组内归一化得到 $\widehat{A}_{main}$。同时，TimerPO 取模型回答和 ExpCoT 的 final-layer token embedding，构造余弦距离代价矩阵，用熵正则 Sinkhorn 近似计算最优传输距离 $W^i$，再把 $\exp(-W^i/\tau)$ 转成推理奖励并组内归一化为 $\widehat{A}_{TsR}$。
    - 设计动机：逐 token 精确匹配会惩罚合理但表述不同的解释，而 OT 更像在比较两段推理证据的整体分布。它允许同义或顺序略有变化的表达得到较高分，同时仍鼓励模型覆盖 ExpCoT 中的关键时间序列证据。

3. **正交投影的非干扰融合**:

    - 功能：让推理奖励作为主检测目标的补充，而不是覆盖或扭曲异常分类、定位本身。
    - 核心思路：TimerPO 将 $\widehat{A}_{TsR}$ 对主 advantage $\widehat{A}_{main}$ 做投影，去掉与主方向重叠的部分，得到 $\widehat{A}_{TsR}^{\perp}=\widehat{A}_{TsR}-\frac{\langle\widehat{A}_{TsR},\widehat{A}_{main}\rangle}{\|\widehat{A}_{main}\|_2^2+\varepsilon}\widehat{A}_{main}$。最终 advantage 写成 $A_{final}=\widehat{A}_{main}+\alpha\widehat{A}_{TsR}^{\perp}$，再放入 PPO/GRPO 风格的 clipped objective。
    - 设计动机：异常检测中的“回答正确”和“解释像专家”不是完全独立的目标，直接相加可能强化虚假相关。正交化让辅助信号只补充主目标没有覆盖的细粒度推理能力，从而提升稳定性。
