...
1. **分数阶 HiPPO 测度（Definition 3.1）**:

    - 功能：用一族带可调奇异指数 $\alpha$ 的概率测度，连续插值「均匀记忆」与「集中于当下」两个极端，并仍保持尺度不变。
    - 核心思路：$\mu^{(t)}(x)=(1-\alpha)t^{\alpha-1}(t-x)^{-\alpha}\mathbb{I}_{[0,t]}(x)$，归一化因子由 $\int_0^t(t-x)^{-\alpha}dx=t^{1-\alpha}/(1-\alpha)$ 推出；$\alpha=0$ 退化为 LegS，$\alpha\to 1$ 让测度趋向 $\delta_t$。把域 $[0,t]$ 通过 $y=2x/t-1$ 映到 $[-1,1]$，权重函数变成 Jacobi 权 $(1-y)^{-\alpha}$，于是正交基自然是 Jacobi 多项式 $P_n^{(-\alpha,0)}$。
    - 设计动机：幂律奇点同时保住「重尾长程记忆」与「近时高响应」，而 $\mu$ 的归一化形式在 $t\mapsto\lambda t$ 下保持不变，于是尺度不变性也被保住——这就破掉了 Table 1 的不可能三角。

2. **可证明的状态矩阵结构（Theorem 3.4）**:

    - 功能：给出 LTI 后 $A(\alpha)\in\mathbb{R}^{N\times N}$ 的解析结构，确保可对角化、谱稳定、不需要 NPLR 或 normal-plus-low-rank 这样的工程逼近。
    - 核心思路：在归一化 Jacobi 基下做 Galerkin 投影 $\mathcal{L}[P_n]=P_n+(1+\eta)P_n'$，可证 $A(\alpha)$ 严格下三角；对角元 $A_{nn}=n+1$ 与 $\alpha$ 无关；非对角元 $A_{nk}$（$k<n$）由 $\langle\mathcal{L}[P_n^{(-\alpha,0)}],P_k^{(-\alpha,0)}\rangle_w/\|P_k\|_w^2$ 给出，仅 $\alpha=0$ 时退化为 $\sqrt{(2n+1)(2k+1)}$，其余靠 Gauss–Jacobi 数值积分。$B$ 项则有闭式 $B_n=\sqrt{(2n+1-\alpha)/(1-\alpha)}\binom{n-\alpha}{n}$。
    - 设计动机：「对角元始终是 $1,\dots,N$」是这篇最关键的稳定性保证——意味着不论 $\alpha$ 如何调，特征值不会改变，只有特征向量被旋转。后续无须像 S4 那样做复杂的 NPLR 近似，直接 $A=V\Lambda V^{-1}$ 就能得到对角 SSM。

3. **分数滤波器组多通道架构（§4.3）**:

    - 功能：把不同时间尺度的特征用不同 $\alpha_k$ 的子状态并行抓取，相当于把 SSM 状态空间结构化为一组「频域滤波器」。
    - 核心思路：把状态维 $H$ 切成 $K$ 个 block，每个 block 用一个 $\alpha_k$；低 $\alpha$ 通道表现得像低通滤波器（保留全局上下文与去噪），高 $\alpha$ 通道表现得像带通/高通滤波器（突出局部跳变）。最后由输出投影 $C$ 学习如何组合这些时间基。$\Delta$ 控制「看多远」（分辨率），$\alpha$ 控制「怎么看」（记忆拓扑），两者解耦。
    - 设计动机：实验观察到 ListOps 既需要长程括号匹配又需要局部数值，单一 $\alpha$ 无法同时处理；用 filter bank 把不同尺度天然解耦，这也是 FRACTAL 在 ListOps 上提分最多的根本原因。
