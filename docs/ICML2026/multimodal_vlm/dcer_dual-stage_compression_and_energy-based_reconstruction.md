...
1. **频域可学压缩 (Stage 1)**:
    ...
    - 功能：在送入跨模态融合之前，先把噪声宽频成分剔掉、把情感稀疏频段保留。
    - 核心思路：audio 用 Daubechies-4 初始化的可学小波基做 3 级 DWT，得到 $\{c_L,d_L,\ldots,d_1\}$ 多尺度系数，再用 cross-scale attention 融合：$h_a=\mathrm{Proj}(\mathrm{CrossScaleAttn}(W(x_a)))$；video 走 2D-DCT 切 4 个频段，频段边界**可学**，再做 frequency-aware attention：$h_v=\mathrm{Proj}(\mathrm{FreqAttn}(D(x_v)))$。
    - 设计动机：固定的 BatchNorm/LayerNorm 无法"按频段过滤"，但小波/DCT 在频域上是稀疏正交基；让其分解参数可学，既保留先验又能 task-adaptive。Table 4 显示去掉频域压缩后 MAE 涨 3.7%，极端缺失场景下涨 12.3%。

2. **K=4 query token 作为跨模态固定容量 bottleneck (Stage 2)**:

    - 功能：阻断 modality-specific shortcut——不允许 $\hat y = f_a(h_a)+f_v(h_v)+f_t(h_t)$ 这种独立通道求和，因为预测只能基于 $Z$。
    - 核心思路：6 层 fusion transformer，每层做 (bottleneck ← modalities) cross-attn、bottleneck self-attn、FFN，残差连接；$K=4$ 是在压缩与表达力之间的最优点（$K=2$ 信息丢失、$K=8$ 接近无瓶颈）。
    - 设计动机：受 Perceiver 启发，但 DCER 把 Perceiver 的 latent query 直接当作 modality-agnostic 信息聚合点；这正好对应"在缺失模态条件下，仍有 $K$ 个 token 可以继续参与下游推理"。

3. **能量函数 + 梯度重建 + 自带不确定度 (Stage 3)**:

    - 功能：模态 $m$ 缺失时给出 $h_m^*$ 估计；同时输出一个标量能量值用作 confidence。
    - 核心思路：$E_\theta(h_m;Z)=f_\theta(h_m - \mathrm{CrossAttn}(h_m, Z)) + \lambda_E g_\theta(h_m)$，其中 $f_\theta,g_\theta$ 是学习的 MLP；从 $\mu_\theta(Z,h_\mathrm{obs})+\epsilon$ 初始化后做 $T=3$ 步带 momentum 的梯度下降 $h_m^{(t)}\leftarrow h_m^{(t-1)} - (\rho v^{(t-1)}+\eta\nabla E_\theta)$。完整模态时 $T=0$ 即可，缺失时 $T=3$ 显著提升。最终 $E_\theta(h_m^*)$ 与预测误差相关系数 $\rho>0.72$。
    - 设计动机：传统 VAE/GAN 重建在高维上不稳定，IB 显式优化又难训；能量函数把"重建"转化为"按学习到的低能量盆地做几步梯度"，简单稳定且天然给 uncertainty——这是相比 VAE/GAN 重建方案的核心差异点。
