...
1. **期望特征扰动 EFD（与未知 Token 预算对齐）**:

    - 功能：把扰动能量集中到"在多种可能预算下都会幸存"的 Token 上，规避预算稀释与依赖断裂。
    - 核心思路：把部署预算 $K_{\text{model}}$ 当作未知离散随机变量，先验设为 $K_{\text{model}}\sim\mathcal{U}[K_{\min},K_{\max}]$；定义 Token $i$ 的幸存概率 $\pi_i=P(K_{\text{model}}>r_i)$，它形成一条随排名衰减的软掩码（高排名为 1，中段渐降，低排名为 0）；再用余弦距离 $d_i=1-\mathcal{S}(\mathbf{z}_i^{\mathrm{adv}},\mathbf{z}_i^{\mathrm{cln}})$ 度量每个 Token 的扰动强度，损失为 $\mathcal{L}_{\text{EFD}}=\sum_i \pi_i d_i / \sum_i \pi_i$。
    - 设计动机：直接用注意力分数 $s_i$ 加权会过度集中在极少数 Top Token 上（softmax 太尖），中段 Token 几乎拿不到梯度；而中段 Token 在中等/未知预算下其实有非平凡概率被选中，因此换成跨预算积分得到的 $\pi_i$ 才是真正"与压缩瓶颈对齐"的权重。

2. **排名-扰动对齐 RDA（确保扰动重的 Token 被选中）**:

    - 功能：主动把高扰动 Token 推到注意力排名前列，提升它们真正经过压缩瓶颈、影响 LLM 输入的概率。
    - 核心思路：把 $d_i$ 和 $s_i^{\mathrm{adv}}$ 各自 softmax 成分布 $p_i^{(d)}$、$p_i^{(s)}$，最大化 $\mathcal{L}_{\text{RDA}}=\sum_i p_i^{(d)} \log p_i^{(s)}$（即让选择分布拟合扰动分布）；优化时对 $p^{(d)}$ stop-gradient，避免两边同时漂移产生退化。
    - 设计动机：理论上对 $\mathcal{L}_{\text{EFD}}$ 求梯度可分解为 $\sum_i \pi_i \nabla d_i$（已选 Token 上继续加扰动）和 $\sum_i d_i \nabla \pi_i$（把高扰动 Token 推上排名）两项，但第二项由于 Top-K 选择是分段常数函数，梯度稀疏且在切换点处病态；RDA 用可微的分布匹配把这条"被埋掉"的梯度通路显式补回来。

3. **联合优化与编码器攻击形式**:

    - 功能：在 PGD 框架下融合两个目标，保持攻击与具体压缩机制解耦。
    - 核心思路：总损失 $\mathcal{L}_{\text{total}}=\mathcal{L}_{\text{EFD}}+\lambda\cdot\mathcal{L}_{\text{RDA}}$，受 $\|\boldsymbol{\delta}\|_\infty\le\epsilon$ 约束；EFD 在"幸存集"内造伤害（payload），RDA 把高扰动 Token 推进幸存集（delivery）。
    - 设计动机：所有主流压缩方法的第一步几乎都是 Top-K 选择（纯选择型如 DivPrune、Alvar，或选完再合并如 VisionZip、PruMerge），打这一步是统一接口，天然覆盖差异化的下游合并机制；联合两项让攻击对"选谁"和"选了之后被扰动多少"都有控制力。
