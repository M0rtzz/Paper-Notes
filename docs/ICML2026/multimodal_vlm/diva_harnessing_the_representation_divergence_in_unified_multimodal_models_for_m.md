...
### 整体框架
DIVA 是后训练框架，不动 backbone 架构。流程：（1）对同一图文对 $(I, T)$ 构造两条信息流——理解流用 caption 指令喂图，生成流用随机 mask + 文本条件做 inpainting；（2）从中间层 $\mathcal{I}_{mid}=\{l \mid l_{start}\leq l \leq l_{end}\}$ 抽出 image-token 隐状态，过 shared encoder $E_{sh}^i$ 和 unique encoder $E_{uni}^i$ 分别得到共享/独有分量；（3）两阶段后训练：Stage 1 冻结 backbone 训 encoder（cross-task conditioning + logit injection），Stage 2 解冻 backbone 用非对称对比 + CLUB 损失打磨。

### 关键设计

1. **双流构造与中间层定位**:

    - 功能：从同一图文锚点出发，造出两条带有显式归纳偏置的信息流，作为后续因子化的输入。
    - 核心思路：理解流取原图 + 模板 prompt $t_{prompt}$（如"Please describe this image in detail"），用 captioning loss $\mathcal{L}_{\text{Und}} = \mathcal{L}(f_\theta(\text{concat}(t_{question}, h_v)), t_{answer})$ 驱动；生成流取 mask 比例 $r \in [0.2, 0.6]$ 的破坏图 $I_{mask} = I \odot (1-M)$ 加原始文本 $T$ 作条件，用 inpainting loss $\mathcal{L}_{\text{Gen}}$ 驱动。中间层范围按观察到的 effective-rank 峰值定位，论文设 8–18。对每层 $\ell$，pool 出 $h_i^{(\ell)} = \text{Pool}(H_i^{img,\ell}) \in \mathbb{R}^d$。
    - 设计动机：两条流必须共享物理锚点（同一对 $(I,T)$），否则共享因子无意义；同时归纳偏置必须显著不同——captioning 引出低频语义流，inpainting 引出高频细节流，二者才有可分解的"共享 vs 独有"结构。

2. **共享/独有因子化与门控编码器**:

    - 功能：把中间层表示显式分解为共享分量 $z_{sh}^{\ell,i}$ 和独有分量 $z_{uni}^{\ell,i}$，对应互信息分解 $I(X_1, X_2; Y) = \Pi_{sh} + \Pi_{uni}^i + \Pi_{uni}^j + \epsilon_{noise}$。
    - 核心思路：对每条流 $i \in \{U, G\}$ 引入两个 3 层 Gated MLP——$z_{sh}^{\ell,i} = g_{sh}^{(i)}(\ell) \odot \phi_{sh}(h_i^{(\ell)})$ 和 $z_{uni}^{\ell,i} = g_{uni}^{(i)}(\ell) \odot \phi_{uni}(h_i^{(\ell)})$，其中 $g_{(\cdot)}^{(i)}(\ell) = \sigma(W_{(\cdot)}^i h_i^{(\ell)})$ 是元素级软门。Stage 1 训练时冻结 backbone，把因子化输出当 logit bias 注入：$\tilde{s}_U = s_U + A_U z_{sh}^{\ell,G} + B_U z_{uni}^{\ell,U}$ 和 $\tilde{s}_G = s_G + A_G z_{sh}^{\ell,U} + B_G z_{uni}^{\ell,G}$（注意 cross-task 注入——理解流的 logit 用生成流的共享分量），用原生任务 loss 训练 encoder。另加正交约束 $\mathcal{L}_\perp = \sum_i \|(\mathbf{z}_{sh}^i)^\top \mathbf{z}_{uni}^i\|_F^2$ 防止 unique encoder 偷偷编码共享语义。
    - 设计动机：cross-task conditioning 强迫 shared encoder 学到的分量必须"对另一条流也有用"，否则 logit injection 不会降 loss；orthogonality 防止两个 encoder 退化到同一子空间。这套设计让分解结构在 Stage 1 就稳住，避免 Stage 2 解冻 backbone 时灾难性漂移。

3. **互信息驱动的非对称对齐 (Stage 2)**:

    - 功能：解冻 backbone 后，让两条流的共享分量在表示空间内强对齐、独有分量强解耦，把冲突转为协同。
    - 核心思路：共享分量用 InfoNCE 下界最大化 $I_{sha}(X_i^s; X_j^s) = \mathbb{E}_{x_i, x_j^+}[\log \frac{\exp f(x_i, x_j^+)}{\sum_k \exp f(x_i, x_j^-)}]$，独有分量用 CLUB 上界最小化 $I_{uni}(X_i^u; X_j^u)$，避免共享空间漏吸任务特有信息或独有空间冗余编码共享语义。为防止任一方向的 loss 主导（理解和生成 loss 尺度差异大），采用 stop-gradient 非对称对齐 $\mathcal{L}_{U \to G} = -\log \frac{\exp(\text{sim}(z_{sh}^U, \text{sg}[z_{sh}^G])/\tau)}{\sum_j \exp(\text{sim}(z_{sh}^U, \text{sg}[z_{sh}^{G,j}])/\tau)}$ 和对称的 $\mathcal{L}_{G \to U}$。最终总损失 $\mathcal{L}_{total} = \mathcal{L}_{U \to G} + \mathcal{L}_{G \to U} + \mathcal{L}_{uni} + \mathcal{L}_{Und} + \mathcal{L}_{Gen}$。
    - 设计动机：InfoNCE + CLUB 是互信息上下界的标准组合，前者鼓励"应该共享的东西尽量靠近"，后者强制"应该独有的东西尽量远离"；stop-gradient 是为了在 heterogeneous task scale 下稳定优化——某一边 loss 突然变大不会把另一边的表示也带跑。
