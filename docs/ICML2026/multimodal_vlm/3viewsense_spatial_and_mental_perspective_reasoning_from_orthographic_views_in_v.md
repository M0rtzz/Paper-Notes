...
### 整体框架
输入是一张 egocentric 图像和一个空间问题，输出是计数或离散方向标签。训练时分三步：首先在 OrthoMind-3D 的程序化数据上做 Stage I SFT，学习生成前/左/上三个正交视图的结构化描述；然后做 Stage II SFT，让模型以三视图为条件生成自然语言推理链和最终答案；最后从 Stage II 模型出发，用 GRPO 在数学可验证奖励下强化答案正确性和推理稳定性。

### 关键设计
1. **Orthographic Mental Simulation**:

	- 功能：把单张 egocentric 图像翻译成前视图、左视图和俯视图三个结构化描述。
	- 核心思路：对方块计数任务，视图描述包含可见方块、堆叠高度和遮挡线索；对物体推理任务，视图描述是按扫描顺序排列的对象序列，例如从左到右或从远到近。Stage I 用 19.5k 个带三视图标注的样本做最大似然训练。
	- 设计动机：三视图把隐含的三维补全问题转成更接近符号模式识别的二维平面问题，减少模型在语言推理阶段临时猜深度的空间。

2. **View-Grounded Reasoning**:

	- 功能：让模型显式读取三视图，并综合原图和问题生成答案。
	- 核心思路：Stage II 构造遵循“front → left → top”整合顺序的推理轨迹，使用 Gemini-3-Flash 生成后按最终答案正确性过滤，得到 21k 训练样本。模型不是只学最终标签，而是学会如何把多个投影拼成一致的心理结构。
	- 设计动机：只监督最终答案容易过拟合数据集模式；监督视图条件下的推理过程，能把“多视图整合”作为可迁移能力保留下来。

3. **GRPO 强化精化与可验证奖励**:

	- 功能：在 SFT 后进一步提升计数和方向预测的准确率，同时缓解大规模 SFT 带来的泛化退化。
	- 核心思路：以 Stage II 模型为初始化，采样一组回答并用整数计数或离散方向的可验证奖励打分，采用组归一化优势 $\hat{A}_i=(R_i-\mu_R)/(\sigma_R+\delta)$ 和 clipped GRPO 目标优化。论文比较 strict reward 和 slack reward，后者对计数误差给一定容忍。
	- 设计动机：空间任务的答案可以自动验算，适合 RL；但直接从 OMS-SFT 做 RL 不稳定，必须先有 VGR 的视图推理 warm start。
