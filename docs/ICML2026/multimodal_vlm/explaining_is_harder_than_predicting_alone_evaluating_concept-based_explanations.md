...
1. **五级形式化解释阶梯 (E1–E5)**:
    ...
    - 功能：用一组逐级加严的解释要求，把"解释复杂度"作为可隔离的实验变量。
    - 核心思路：E1 只输出 `<response>` 标签里的类标作为准确率基线；E2 加 `<explanation>` 标签写一段简短自然语言解释（标准 CoT 风格）；E3 在 `<features>` 里只列最小充分的可观察视觉特征（短名词短语项目符号）；E4 先在 `<features>` 列特征、再在 `<kb>` 里从示例归纳 IF-THEN 规则、最后在 `<rule_check>` 里指出查询图最佳匹配的规则，禁止引入新证据；E5 用 DL 形式：`<tbox>` 用 `hasVisualFeature` 角色写必要/充分/充要的概念公理，`<abox>` 写查询图属性断言，`<dl_explanation>` 推导预测类。
    - 设计动机：之前的解释评测要么散落在不同 prompt 风格里不可比，要么只测最末端的自由文本。把五种条件做成"复杂度单调递增的阶梯"，能直接测出"形式化"本身带来的边际代价。

2. **LLM-as-a-judge + 9 维 XAI 指标**:

    - 功能：把"解释好坏"从"读起来流畅"扩展到 9 个可独立打分的概念化维度，可大规模复用。
    - 核心思路：判官按 1–5 打分以下 9 项——Textual Groundedness（解释覆盖图中所有显著概念）、Hallucination Free（每条断言图中可验证）、Concept Counting（计数精确）、Comprehensibility（非专家可读）、Conciseness（无冗余）、Specificity（用精确非泛化细节）、Local Discriminativeness（突出区分本类与其他类的特征）、Instruction Following（遵守格式约束）、Logical Coherence（推理链流畅有效）。判官不获取支持图，依赖自身先验对候选类的认识来判 LD 指标。
    - 设计动机：单个"解释质量"分数无法分辨"啰嗦但准确"和"简短但跑题"两种失败模式。9 维拆解后能定位 DL 公理类解释失败的具体维度，也能跑相关性分析（哪一维与准确率强挂钩）。

3. **可复现实验网格 + 配平的统计设计**:

    - 功能：让跨模型、跨条件、跨难度的对比满足非参数显著性检验前提，得出可信结论。
    - 核心思路：4 数据集 (CIFAR-10 / DTD / Flowers102 / Pets) × 4 模型 (Gemini 2.5 Flash, Gemma 4 26B, Qwen3 VL 8B, LLaMA 4 Scout) × 5 条件 (E1–E5) × 6 个 $(N,K)$ 配置 = 2,080 次运行；查询数 $Q=1$ 保证每次实验是独立样本，满足 McNemar / Wilcoxon / Friedman 检验要求；重复次数与 $N$ 配平使 $\text{Reps}\times N=12$，避免小 $N$ 配置被过度采样而虚增可信度；所有支持集用固定种子 42 生成、所有模型/条件共享，消除采样噪声。
    - 设计动机：以往 ICL XAI 工作要么样本量小、要么不同模型走不同支持集，结果差异分不清是方法差异还是数据差异；本文把"统计设计"提到与方法同等重要的位置。
