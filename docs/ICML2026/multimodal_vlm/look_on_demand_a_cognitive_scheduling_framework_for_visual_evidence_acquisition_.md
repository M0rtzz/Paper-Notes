...
unified 范式的问题是 LLM 推理会逐渐 dominate 视觉表示——论文 4.2 节量化出 self-attention 系统性给 text token 约 2.5× 于 visual token 的注意力，soft-max 进一步放大，视觉证据长期被压低。CSMR 把 PVP 设成独立的 VLM 实例，每次只接收原图 + visual query，不携带 CRC 的 reasoning context；CRC 拿到 textualized 答案后融入推理，但不让 query 上下文回流到 PVP 的下一次调用。这样 PVP 每次都"新鲜地"看图，视觉表示不被推理路径上的语言污染——这是借 Baddeley 工作记忆理论里 visuospatial sketchpad 的角色分工。消融里这一项贡献最大（去掉掉 3.7 点），坐实了语言污染问题真实存在。

**2. Reasoning-State-Driven Dynamic Visual Querying：按当前推理状态增量取证，而非一次性 plan。**

pre-reasoning 那种把图一次性转 caption 的做法是"一锤定音"，没法预知后期推理需要的细节，coarse caption 不可逆地丢精细信息。CSMR 让 CRC 不预先 plan 所有 query，而是根据当前 reasoning state 增量生成 targeted query（如"图中右下角是什么颜色？"），可以从粗到细——先问"图主要内容"再问"细节区域"；若返回证据不够就重复调 PVP 直到充分。这等于让"推理引导感知"：只取推理真正需要的证据，避免无关信息干扰。消融里换成一次性 plan 所有 query 会掉 5.4 点。

**3. Confidence-Based Early Termination：证据够了就停，按难度自适应分配 query 数。**

不同 case 难度差很多——简单题 1–2 个 query 够，复杂题要多轮，固定 query 数既浪费又对长链推理不足。CRC 在每一步评估当前证据是否足以得出答案：若是直接生成 final answer，否则继续 query，机制类似 PathCTM 的 entropy-based early stopping。实测上模型确实能识别难度并自适应分配：Easy 平均 1.4 query、Medium 2.7、Hard 4.2，既省 query 又不掉精度（去掉早停改固定 query 数掉 2.2 点）。
