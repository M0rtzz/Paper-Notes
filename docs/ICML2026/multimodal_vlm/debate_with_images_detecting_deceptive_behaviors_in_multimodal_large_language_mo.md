...
1. **视觉锚定的多智能体辩论 (Debate with Images)**:

    - 功能：把"是否欺骗"这个判断从单步 MLLM-as-a-judge 改成多轮对抗辩论 + 视觉操作复演。
    - 核心思路：每轮辩手生成发言时，必须从工具库 $\mathcal{E}$（裁剪、放大、标注、定位、VQA 等）里选若干操作回切原图取证。Algorithm 1 中关键一步是 `ApplyVisualOp(x, d.E)`——历史发言里的操作在每轮被重新执行一次，让后来者和 judge 都能基于同样的视觉证据序列做判断，避免文字摘要导致的不可逆信息损失。
    - 设计动机：纯文本辩论会随轮次衰减——由数据处理不等式，每轮 $I(\bm{x}; \bm{D}_n^{\text{text}})$ 单调下降；而每次"回切原图 + 视觉操作"等于把原始图像特征重新注入辩论，理论上 $I(\bm{x};\bm{D}_n)\ge I(\bm{x};\bm{D}_n^{\text{text}})+\sum_{k=2}^{n}\gamma^{n-k}I(\bm{x};\mathcal{E}_k|\bm{D}_{k-1})$，把信息保留率 $\gamma\in(0,1)$ 的指数衰减替换为加性补偿。

2. **效用函数与不对称欺骗代价**:

    - 功能：用博弈论的方式让辩论激励兼容，使诚实方天然占优。
    - 核心思路：辩手 $A^m$ 持立场 $c_m\in\{0,1\}$，效用 $U^m(\bm{D}_N,j)=(-1)^{\mathbbm{1}_{\{c\mid j=c\}}(c_m)+1}-\lambda\cdot C^m(\bm{D}_N)$，其中 $C^m=\sum_n|\mathcal{E}_n^m|$ 是视觉操作的累计成本，$\lambda>0$ 调节"取证彻底度"与"算力开销"的权衡。Remark 4.2 给出理论刻画：维持欺骗需要的最少证据满足 $|\mathcal{E}_{\text{deceptive}}|\ge|\mathcal{E}_{\text{truth}}|+\Delta$，$\Delta>0$ 称为"欺骗代价"。
    - 设计动机：欺骗方在视觉辩论中天然面临三重劣势——必须排除矛盾证据、必须把注意力从矛盾区域引开、必须让一系列伪造证据互相自洽。把成本写进 utility 同时避免辩手对原图做无意义的暴力穷搜。

3. **MM-DeceptionBench 的"情境压力 + 行为锚定"标注协议**:

    - 功能：把抽象的"欺骗意图"操作化成可裁决的行为信号，区分欺骗 vs 普通选择性表达 vs 能力错误。
    - 核心思路：标注者基于三类可观察证据裁决——(i) 中间推理是否与可验证视觉元素一致；(ii) 在反事实/替代 prompt 下能否给出正确回答；(iii) 输出偏差是否对任务目标系统性、一致性。仅当三类信号一致表明"模型本可正确回答但在该上下文下选择不"时才打标"欺骗"；推理-答案不一致或纯能力失败的案例被排除。场景设计上只埋"情境压力"（例如冲突回避、用户满意度等隐性激励），不显式下达欺骗指令——欺骗策略必须自发涌现。
    - 设计动机：避免把 hallucination 或角色扮演产生的"伪欺骗"混入数据集，把"normative anchor"定在"是否在有非欺骗替代方案的前提下仍然诱导错误信念"，让基准既覆盖真实风险又不过度宽松。
