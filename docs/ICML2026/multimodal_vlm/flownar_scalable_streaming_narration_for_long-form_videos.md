...
**核心 idea**：用"动态段间裁剪 (DCM) + 段内门控线性注意力压缩 (CLAM)"组合，把视觉历史的复杂度从 $O(T)$ 压到 $O(1)$，并用自条件协议把评估"真实化"。
...
1. **动态上下文管理 DCM + 双阈值触发**:
    ...
    - 功能：保证视觉上下文不随 $T$ 增长，同时控制解说节奏避免连发或长哑。
    - 核心思路：每生成一句解说后**显式清空当前段的视觉 KV** $\mathcal{C}_t^{\text{vid}} \leftarrow \emptyset$，并丢弃段首前置的 $\mathbf{M}_{t_{n-1}}$，强迫模型只依靠新计算的 $\mathbf{M}_{t_n}$ 作为历史。触发逻辑用两套阈值：默认用主阈值 $\theta$ 判 $p(\text{[SKIP]} \mid \mathbf{E}_t, \mathcal{C}_{t-1}^{\text{vid}}, \mathcal{C}_{n-1}^{\text{nar}}) \le \theta$ 是否成立，触发后短时间内切到更低的 $\theta_{\text{low}}=0.5$ 压低再次触发概率，避免爆发式连发。
    - 设计动机：自条件部署里，长上下文不仅吃显存，更会把"上一句错了"的错误反复送回 LLM 形成滚雪球；激进裁剪反而比保留全历史更稳健。

2. **CLAM 跨线性注意力记忆**:

    - 功能：用恒定显存与每步常数计算把视觉历史增量压缩成 $M$ 个记忆 token。
    - 核心思路：维护一个 $\mathbf{S}_t \in \mathbb{R}^{D\times D}$ 的循环状态，对每个帧内 token $\mathbf{x}_{t,j}$ 计算 key/value 与门控矩阵 $\mathbf{G}_{t,j}$（值域 $(0,1)$），按 $\mathbf{S}_{t,j} = \mathbf{G}_{t,j} \odot \mathbf{S}_{t,j-1} + \mathbf{k}_{t,j}^\top \mathbf{v}_{t,j}$ 递推；用 $M$ 个可学习查询 $\mathbf{Z}\in\mathbb{R}^{M\times D}$ 经线性投影得到 $\mathbf{Q}$，最后读出 $\mathbf{M}_t = \mathbf{Q}\mathbf{S}_t$。状态可存约 $O(D)$ 个独立键值对，足以撑长片段。
    - 设计动机：朴素 KV 缓存随帧数线性膨胀；线性注意力的循环视角天然兼具"常数显存、常数每步算量、训练可并行"三性质，并且把"压缩"和"读取"解耦——压缩在帧内逐 token 做、读取用固定查询，避免了 MovieChat 式聚合或简单滑窗丢长程信息的缺陷。

3. **自条件评测协议 + 先对齐后评分**:

    - 功能：在真实部署条件下衡量解说模型，避免 teacher-forcing 高估性能。
    - 核心思路：评测时每句 $y_n$ 只能基于模型自己之前生成的 $\{y_j^{\text{pred}}\}$，不喂 GT 历史。由于预测段与 GT 段在数量与边界上不一致，先用 IoU $\tau=0.5$ 做段级匹配算 Precision/Recall/F1（temporal alignment），再用广义 IoU 检索每个 GT 段的最佳匹配预测段、在配对上算 CIDEr/METEOR/ROUGE-L（narration quality）。
    - 设计动机：teacher-forcing 屏蔽了错误传播，把"勉强能用"的模型说得很厉害；自条件 + 后对齐既保留时间维度的评测能力，又把真实部署的错误累积暴露出来。
