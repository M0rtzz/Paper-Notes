...
物理变换是个动态过程，理论上看的帧越多、证据越足，模型应该判得越准。作者据此设置 3/5/7/9/16 帧五个条件，并搭配三种抽帧方式（均匀采样、人工挑选、模型自选），看准确率会不会随帧数和证据质量提升。结果恰恰相反——在体积和大小任务上抽帧方法产生了显著主效应（$F(2,222)=8.75,\ p=0.0002$），而且统一均匀采样竟然反而优于人工或模型挑选的"高质量"帧。换句话说，给模型更好的视觉证据，它判得更差，这说明它的失败不是证据不够，而是它压根没在用视觉证据推理。
