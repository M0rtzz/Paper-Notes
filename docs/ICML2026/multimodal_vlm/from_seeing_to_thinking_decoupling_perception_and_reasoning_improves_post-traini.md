...
1. **感知数据合成 + 双模型差分过滤**:
    ...
    - 功能：从开源 image-caption 语料里造出"必须看图才能答"的 QA 训练集 $\mathcal{D}_{\text{perc}}$，避免模型靠语言先验偷答案。
    - 核心思路：先用 Qwen2.5-72B 在 DOCCI 的 fine-grained caption 上生成 perception-focused 问答对 $(Q,A)=f_{\text{gen}}(C)$；然后对每个候选样本同时跑两条路径——只给图的 $\hat{A}_{\text{img}}=f_\theta(I,Q)$ 和只给 caption 的 $\hat{A}_{\text{cap}}=f_\theta(C,Q)$，只保留满足 $\mathbb{I}[\hat{A}_{\text{img}}\neq A]\land\mathbb{I}[\hat{A}_{\text{cap}}=A]$ 的样本；最后再用 Qwen2.5-VL-7B 和 Qwen2.5-VL-32B 两个 base 模型分别做一遍过滤，取交集。
    - 设计动机：这是整个 pipeline 的"信号纯度"保障——若样本能凭 caption 答出，那它其实考的是语言而非视觉；只有"caption 能答、图自己答不出"的样本才真正暴露了感知缺陷，正好对应作者发现的 86.9% 错例类型。

2. **能力维度的分阶段 GRPO（capability-dimension curriculum）**:

    - 功能：用同一套 GRPO 损失 $\mathcal{J}_{\text{GRPO}}(\theta)=\mathbb{E}_{x,y}[\frac{1}{G}\sum_i \min(\rho_i A_i, \text{clip}(\rho_i,1-\epsilon,1+\epsilon)A_i)] - \beta\, \text{KL}(\pi_\theta\|\pi_{\text{ref}})$ 依次在三种数据上训练，让模型先建立"看清"的能力，再叠加"推理"的能力。
    - 核心思路：奖励 $R(x,y_i)=r_{\text{acc}}+r_{\text{format}}$ 在所有 stage 一致，group 内做标准化得到优势 $A_i=(R-\mu_R)/(\sigma_R+\epsilon)$；三阶段 step 数分别为 90 / 375 / 465（按相同 epoch 折算），与 merged baseline 总步数 930 严格对齐，确保对比公平；视觉编码器在每个 stage 都参与训练，避免感知 stage 修过的视觉表征在后续被冻结而漂移。
    - 设计动机：把"能力维度课程"明确为一个**与传统难度课程正交**的新维度——传统课程按 easy→hard 排数据，本方法按 perception→reasoning 排能力。Section 4.5 进一步验证两者可叠加（merged + 难度课程 vs staged + 难度课程），叠加后比 merged 高 +4.43%，比任一单维度都强。

3. **感知阶段用 RLVR 而非 caption-based SFT**:

    - 功能：在感知 stage 替换"用 caption 当目标做 next-token SFT"这一传统范式，改用 verifiable reward 的强化学习，使感知信号不被低质量字幕污染。
    - 核心思路：感知 QA 的答案天然是短答案（颜色、空间关系、属性），可以直接用 exact match 作为 $r_{\text{acc}}$，与 GRPO 的 on-policy 采样无缝衔接；SFT 则需要把图片对应的整段 caption 当 target，但 caption 质量往往低于预训练语料，会在 token 级强行注入 off-policy 监督。
    - 设计动机：作者把感知 stage 的 RL 换成 SFT 做对照，Qwen2.5-VL-7B 在 WeMath 上掉 8.1%、Qwen3-VL-8B 掉 1.6%；说明 SFT 会"反噬"已有能力，而 RLVR 让模型保持在自己策略附近，把感知能力作为奖励驱动的微调，从而避免破坏推理。
