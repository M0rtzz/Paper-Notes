...
### 关键设计
1. **VisualSwap 两阶段替换协议**:

	- 功能：把“视觉重检”转化成一个可测的反事实测试，而不是只看模型输出中有没有反思句。
	- 核心思路：第一阶段让模型基于 $I_a$ 生成推理链 $R_a$；第二阶段重新 prefill 完整上下文，但把图像换成 $I_b$，再继续生成。性能退化定义为 $\Delta=Acc_{base}-Acc_{probe}$。
	- 设计动机：如果只是让模型回答一次问题，很难判断它后续是否会看图。通过把旧推理和新图放在冲突状态，模型只有真正重新绑定视觉证据，才能摆脱 $R_a$ 的语言惯性。

2. **VS-Bench 的相似但答案相反图像对**:

	- 功能：提供 800 个专门针对视觉重检的样本，覆盖几何、函数图、图表理解、合成场景和专业多模态题。
	- 核心思路：从四个强视觉依赖 benchmark 取问题与目标图，再通过 human-in-the-loop 标注和 Nano Banana Pro 生成另一张图，确保问题不变、整体视觉相似、答案关键细节改变。作者还用 CLIP、SSIM、LPIPS 和人工区分实验验证图像对质量。
	- 设计动机：如果两张图差异过大，模型可能只靠异常感发现替换；如果问题不再适用，错误可能来自数据缺陷。VS-Bench 刻意保持外观相近，让测试聚焦在细粒度重新观察能力上。

3. **注意力与多轮对照机制分析**:

	- 功能：区分“模型没有能力读新图”和“模型有能力但没有自发调用视觉注意力”。
	- 核心思路：作者定义视觉注意力分数 $S_{vis}^{(l)}(t)$，统计当前生成 token 对图像 token 的平均注意力，并比较 Probe 与 Multi-turn 在干预点前后 100 个 token 的变化。同时用 2 倍视觉 token 注意力放大做无训练干预。
	- 设计动机：主实验只说明现象，注意力分析解释为什么。Multi-turn 近乎恢复 baseline，attention amplification 也提升 probe，说明失败更像自主注意力控制失灵，而不是图像理解能力消失。
