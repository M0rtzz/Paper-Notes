...
3. **Cursor-Centric Focusing（ccf）— 训练-推理分辨率解耦**：

    - 功能：训练时压低分辨率省算力，推理时通过裁剪在高分屏上拿回精度。
    - 核心思路：训练时把所有图像按比例缩到不超过 $P = 1920\times 1080$，避免交互历史里堆叠多张原始大图爆显存。推理时如果图比 $P$ 大，先在原图上跑一步拿粗预测，然后以这个粗预测为中心裁出一块 $P$ 大小的区域、把光标摆在裁剪图正中，后续移动步只在这块裁剪图里做、且历史里**不再保留原始大图**。
