...
1. **个性化字幕作为代理任务 (Captioning as Proxy)**:

    - 功能：把开放、长尾的下游个性化任务收敛到一个可监督、可奖励、可泛化的统一目标。
    - 核心思路：基于 $z=h_\theta(c,x),\,y=g_\theta(z,p)$ 的分解，作者论证 captioning 与 $h_\theta$ 在目标上同构——字幕直接外显模型对"图像 + 用户上下文"的理解，没有 thinking/reasoning 这类冗余环节插入；因此训一个会写个性化字幕的模型，就等于训出了一个高质量的 $h_\theta$，下游任意 $g_\theta$ 都能搭上车。
    - 设计动机：避免为每个下游任务单独 SFT 或定制奖励，把"个性化能力"从任务特定优化里抽出来，做成下游可复用的共享原语；这也是 CoViP 区别于 RAP/RePIC 等之前 RL 个性化方法的根本架构选择。

2. **双成分可验证奖励 (Set-level F1 + MCQA-based VR)**:

    - 功能：让 RL 同时反馈"识别对了几个概念"和"字幕里检索到了多少有用历史"两件事，避免单一奖励让模型偏科。
    - 核心思路：识别奖励 $r_{\text{vis}}(x,c)=\text{F1}(\hat{H},H)=\frac{2|\hat{H}\cap H|}{|\hat{H}|+|H|}$ 用 set-level F1 给"哪些 in-context 概念出现在查询图里"的预测打分，预测多了 FP 涨、漏了 FN 涨，部分对也给部分分；检索奖励 $r_{\text{caps}}(s,c)$ 用上面 CapEval-QAs 的 $\sigma^+(s;QA^+)-\sigma^-(s;QA^-)$ 度量字幕对正/负问题的回答行为，并在字幕退化 ($R(s)>0$) 时直接给 $-1$ 惩罚；负样本部分用系数 $\alpha$ 调节。最终 $r=r_{\text{vis}}+r_{\text{caps}}$ 套进 GSPO 做策略优化。
    - 设计动机：之前个性化 RL 工作要么直接拿 BLEU/CIDEr，要么拿"名字召回"，前者鼓励抄上下文文本捷径、后者太粗。F1 把"识别"标量化、MCQA 把"检索"标量化，两个分数都是可重复、可对抗作弊的"硬指标"，又能在任意 $|H|$ 下保持可解释。

3. **字幕增强生成 (Caption-Augmented Generation, CAG)**:

    - 功能：把已经被 RL 优化得很好的字幕能力，在推理时"复用"到任何下游任务上，无需再训。
    - 核心思路：原本的下游 pipeline 是 $y\sim\pi_\theta(\cdot\mid x,c,p_d)$；CAG 改成两步——$s\sim\pi_\theta(\cdot\mid x,c,p_s),\,y\sim\pi_\theta(\cdot\mid x,c,p_d,s)$，把模型自己刚生成的字幕 $s$ 当成显式 conditioning 喂回，相当于"先让自己把话说清楚再回答问题"。两次前向用的还是同一个 $\pi_\theta$，没有引入新模块。
    - 设计动机：实验里观察到 RL 后字幕里包含的个性化细节比直接答下游题的输出更稠密；CAG 等于把"字幕作为内部草稿"显式化，让 $g_\theta$ 不必从零再做一次 $h_\theta$ 的工作，对 VQA、对话等下游任务都能稳定加分，且推理代价只是多一次字幕前向。
