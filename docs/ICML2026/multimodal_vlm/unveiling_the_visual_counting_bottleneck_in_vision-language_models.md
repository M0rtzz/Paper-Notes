...
description: >-
  [ICML 2026][多模态VLM][视觉计数] 通过将视觉计数分解为三个认知阶段——发现 VLM 的计数失败根源不在视觉感知或数量理解，而在符号映射阶段无法将视觉表征投影到正确的文本标记，反映出模型缺乏统一的跨模态数字表示空间。
tags:
  - "ICML 2026"
  - "多模态VLM"
  - "视觉计数"
...
### 整体框架
两层实验设计——首先在可控合成实验室（自定义训练的轻量级 Toy VLM + Go 棋盘数据集）中严格控制训练分布，然后在最先进基础模型（Qwen3-VL-32B）上验证结论。通过隐数探针技术从多维度诊断失败原因，逐步排除假设 A 和 B，最终定位到假设 C。

### 关键设计

1. **解耦训练课程**:

    - 功能：模拟 VLM 预训练动态，创建模态间的分布偏移。
    - 核心思路：阶段 1 语言预训练使解码器掌握递归后继函数；阶段 2 视觉对齐限制视觉训练到 N≤49。创造关键的视觉外推制度（50-99）——模型知道标签但未见过对应视觉密度。
    - 设计动机：通过创造文本知识与视觉经验的不匹配，可隔离跨模态问题，而非通过噪声遮蔽的真实数据集。

2. **隐数探针诊断工具**:

    - 功能：通过训练线性分类器 $f_{probe}: \mathbb{R}^d \to \{0,1\}$ 检测视觉编码器输出是否存在物体，聚合得到隐数 $N_H = \sum_{i=1}^L f_{probe}(z_i)$。
    - 核心思路：使用仅在分布内（N≤49）训练的探针，在外推制度上评估，定义 Vision Gap（$|N_H - N_G|$，感知误差）和 Language Gap（$|N_H - N_P|$，语言模块对齐误差）。
    - 设计动机：绕过语言解码器直接检查视觉表征的信息含量，能精确定位失败发生位置。

3. **对比计数任务验证数量意识**:

    - 功能：将枚举任务转化为二元分类——模型判断两个输入的基数是否相同。
    - 核心思路：模型无需生成特定数字标记，只需比较两量的相等性，绕过符号表达瓶颈，直接测试是否保留了数量感知。
    - 设计动机：若模型在对比任务中保持 >90% 准确率（即使在显式计数失败的 VE 制度），则证明数量信号未丧失，失败纯粹源于生成阶段。
