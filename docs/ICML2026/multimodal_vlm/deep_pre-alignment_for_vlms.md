...
1. **用小 VLM 整体当视觉编码器（Perceiver as Encoder）**：

    - 功能：把 ViT 替换成一个完整的小 VLM，让视觉特征先在感知模块内部走一遍深度对齐，再交给目标大 LLM。
    - 核心思路：取 perceiver $M_p$（论文用 Qwen3-0.6B + 同款 ViT 训成的小 VLM）的最后一层语言块隐状态作为 $\mathbf{H}_{\text{aligned}}$；这层隐状态由于经过预训练语言块的 causal attention 加工，已经被推到与文本嵌入兼容的几何空间；用一个 projector $\phi$ 把 $M_p^{\text{LLM}}$ 维度（0.6B 是 1024）映射到目标 LLM（4B 是 2048、32B 是 5120）的输入维度，就可以无缝接入。
    - 设计动机：CLIP ViT 只做 image-text contrastive，是"图文相似"而非"几何同构"的浅层对齐；语言块是经过大规模 causal LM 预训练的，输出几何天然与目标 LLM 内部表示同构。消融显示去掉 perceiver 的语言块只保留 ViT 权重，提升仅 +0.7 点，而完整 perceiver 带来 +3.4 点，证明语言块本身就是对齐的"工作母机"。

2. **轻量适配 + 标准两阶段训练**：

    - 功能：复用 LLaVA 经典训练流程，把 DPA 的引入做成模块化升级路径——仅替换视觉编码器，训练目标、数据、调度全部不变。
    - 核心思路：Stage 1 用 558K 图文 caption 对仅训 $\phi$（projection layer），把 perceiver 输出维度对齐到目标 LLM；Stage 2 用 1M 高质量视觉指令数据端到端微调整个 DPA 模型（perceiver + projector + 目标 LLM）；32B 模型用 LoRA 训 3 个 epoch 保证可承受的算力。Perceiver 在 Stage 2 保持可训练；若冻结 perceiver，性能从 53.0 掉到 52.1，但仍胜过 baseline。
    - 设计动机：作者刻意避免引入辅助 loss 或专门训练策略，是为了证明性能提升完全来自"架构改造"而非"训练 trick"；这种"插拔式"升级让 DPA 可以叠加在任何现有 VLM pipeline 上。

3. **指令无关 vs 早融合的设计取舍（Instruction-agnostic Default）**：

    - 功能：决定 perceiver 是否在编码阶段看到文本指令——默认不看（指令无关），保通用性；可选早融合变体（早期把指令塞给 perceiver）追求单轮极限性能。
    - 核心思路：默认配置 perceiver 只吃图像、输出与指令无关的视觉特征；可选 "w/ instruction context" 把指令在 perceiver 编码时就拼上，让 perceiver 在早期就根据 query 过滤无关视觉信息，相当于早融合的语义过滤器。消融显示早融合把整体平均分从 53.0 拉到 55.2，文本能力分数从 52.6 跳到 59.0，但作者论证这种配置把视觉表示绑定到单轮 query，多轮对话或意图切换时会失效，所以默认仍保持指令无关。
    - 设计动机：通用性 vs 单轮峰值是 VLM 实际部署中的真实张力——本文给出一个清晰可选的设计旋钮，让用户按场景选择。这个 ablation 同时揭示了"为何 DPA 能减缓文本遗忘"——perceiver 起到了过滤干扰视觉特征的作用，让目标 LLM 受到的语言能力扰动变小。
