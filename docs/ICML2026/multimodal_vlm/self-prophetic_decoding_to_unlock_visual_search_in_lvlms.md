...
其中 $\alpha$ 初始为 0.5，再按该 token 在 search logits 里的归一化排名自动调整（排名越高、$\alpha$ 越大，越偏向 search 的原生分布）。从第一个 $s_j<\tau$ 的位置起拒绝，其后的 token 直接从 $p_s(x_j\mid H_i,x_{<j})$ 采样。所有 $s_j$ 能在一次前向里并行算完，所以整段草稿评估的耗时相当于一个普通解码步，没有额外延迟。
