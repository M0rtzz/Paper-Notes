...
### 整体框架
TimeSpot 把每张图像 $x$ 映射到结构化标签 $y=(y^{\mathrm{temp}}, y^{\mathrm{geo}})$，其中 $y^{\mathrm{temp}}=(s, m, \tau, \phi)$ 表示季节、月份、本地时间 HH:MM、日相，$y^{\mathrm{geo}}=(C, \kappa, z, e, (\lambda,\varphi))$ 表示洲、国、气候带、环境类型、经纬度。整体流程分四步：（1）从 web 与作者自拍中召回 ~20,000 张候选地面图像；（2）过滤掉地标和文字主导的样本，保留物候/光影/材质这种细粒度物理线索；（3）从 EXIF + 地理坐标 *程序化* 派生九字段；（4）3 名主标注员 + 2 名资深审核员两阶段人工校验，共 ~600 小时标注。最终产出 1,455 张图覆盖 80 国，按统一 JSON schema 存储，并在评测时强制做月-季-半球对齐、日相-时间-经度兼容、气候-坐标合理性等自动审计。

### 关键设计

1. **结构化九字段 schema + 程序化标签派生**：

    - 功能：把"何时何地"拆成可独立打分但又必须互相一致的 9 个字段，并保证 ground truth 来自物理公式而非众包猜测。
    - 核心思路：月份直接取 EXIF 时间戳；季节用气象定义 + 半球修正（北半球 6-8 月为夏，南半球反过来）；日相通过计算太阳高度角 $\theta_\odot$ 与 civil/nautical/astronomical 阈值比较得到（如 $\theta_\odot < -6^\circ$ 即 civil twilight）；本地时间从时区 + 星历推得；气候带按 Köppen-Geiger 由 $(\lambda, \varphi)$ 查表；洲/国/坐标直接从经纬度反向地理编码。所有派生步骤都是确定性的，再让人工审核员对照图像光影与植被反向校验，剔除元数据被破坏的样本。
    - 设计动机：retrieval-centric 基准的根本缺陷在于 ground truth 本身缺少跨字段语义，模型只要 hit top-k 就算赢；TimeSpot 用物理公式生成 GT，使得"7 月雪景"这类违例可以被自动 flag，倒逼模型做物理一致的联合推理。

2. **跨字段一致性诊断 + 校准指标**：

    - 功能：除字段级精度外，单独评测模型输出的内部物理一致性与置信度可靠性，把"看似准确但物理矛盾"的失败暴露出来。
    - 核心思路：定义若干一致性违例率，例如 *月-季不一致率*（在预测国对应的半球下，预测月份不属于预测季节的比例）、*日相-时间错位率*（$|\Delta t| > 1\text{h}$）、*洲-国不一致率*；用 Expected Calibration Error $\mathrm{ECE}=\sum_b \frac{|B_b|}{N}|\mathrm{acc}(B_b)-\mathrm{conf}(B_b)|$ 与 risk-coverage 曲线度量置信度可靠性；用半球翻转测试与硬 OOD 切分评估鲁棒性。
    - 设计动机：实验发现 QwenVL-3B 等模型 phase-time 违例只有 0.21%，但 MD>1000 km 错误率高达 95.19%（表 4），说明单一维度准确不代表全局一致；引入这套诊断才能区分"真懂物理"与"靠先验拼凑"。

3. **SFT 干预实验作为诊断工具**：

    - 功能：用 LoRA 在 Qwen-VL2.5-3B 上分别微调国别预测、时间预测和联合预测，定量回答"显式监督能否补齐 VLM 的物理 grounding"。
    - 核心思路：用 40% 分层切分做训练、60% 评测；分别测试 country-only SFT、time-only SFT、joint SFT，观测各任务自身提升以及跨任务的退化；将训练目标解释为 *光照不变特征*（用于国别）与 *光照敏感特征*（用于时间）的梯度竞争。
    - 设计动机：作者不是要刷榜，而是想说明"为什么 SFT 不够"——country-SFT 把 country 从 14.20% 推到 19.24%，但同时把 time 从 22.06% 拖到 21.78%；joint SFT 部分缓解但仍低于单任务峰值，揭示共享 LoRA 参数下的梯度冲突，从而 motivate 后续 constraint-aware RL 或物理先验注入。
