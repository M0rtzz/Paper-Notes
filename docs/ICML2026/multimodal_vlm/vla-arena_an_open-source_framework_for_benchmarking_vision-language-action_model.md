...
description: >-
  [ICML 2026][多模态VLM][视觉语言动作模型] VLA-Arena 提出结构化 VLA 基准——通过任务结构、语言命令和视觉观察三个正交维度系统量化难度，用 170 个任务揭示现有 VLA 模型在泛化、视觉感知和安全性上的关键缺陷。
tags:
  - "ICML 2026"
  - "多模态VLM"
  - "视觉语言动作模型"
...
### 整体框架
三维任务设计——（1）**任务结构维（T 轴）**：170 任务分 11 套件按四核心维度组织（安全/干扰/泛化/长期）+ 3 难度等级（L0/L1/L2）；（2）**语言命令维（W 轴）**：W0-W4 通过 WordNet 语义相似词替换；（3）**视觉观察维（V 轴）**：V0-V4 累积应用视觉扰动（光照→颜色→视点→噪声）。

### 关键设计

1. **Constrained BDDL（CBDDL）**:

    - 功能：扩展标准 BDDL 原生支持动态实体、扰动、安全谓词。
    - 核心思路：10 类安全谓词（碰撞、力矩限制、物体坠落等）精确定义约束。引入**累积成本（CC）**：$CC(\tau) = \sum_{t=0}^{L-1} c^{inst}(s_t, a_t) + \alpha \cdot c^{term}(s_L)$，$\alpha=10$ 加权终态危险。
    - 设计动机：区分"通过任务但违反安全约束"与"真正安全的成功"，对现实部署至关重要。

2. **有原则的语言扰动（基于 WordNet）**:

    - 功能：系统改写语言指令通过替换语义相似词诊断模型理解而非死记。
    - 核心思路：识别指令语义插槽（动作词、目标词、地点词），用 WordNet 找距离为 1 的同义词替换；难度等级是被替换的插槽数量 W0（原始）→ W4（4 个插槽都替换）。
    - 设计动机：区分真正语言理解与表面关键词匹配。模型在 W0 表现好但 W4 崩溃说明它做模式匹配。

3. **累积层级化视觉扰动（V0-V4）**:

    - 功能：逐层施加视觉挑战诊断视觉鲁棒性。
    - 核心思路：V0 标准 → V1 光照扰动 → V2 物体颜色随机化 → V3 视点偏移 → V4 高斯噪声。每层在前一层基础上累积。
    - 设计动机：对应 sim-to-real 问题和神经网络捷径；从较轻自然变化逐步到极端扰动精确定位失败边界。
