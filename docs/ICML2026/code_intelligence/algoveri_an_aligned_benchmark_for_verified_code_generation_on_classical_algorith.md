...
1. **跨验证系统的规格对齐**:
	...
	- 功能：确保 Dafny、Verus、Lean 中同一任务考察同一算法性质，而不是各自考不同难度的题。
	- 核心思路：作者手工编写并审阅 helper definition、precondition 和 postcondition，使它们在三种系统里语义一致。例如 LIS 任务不只检查结果在数组范围内，而是统一要求存在对应长度的递增子序列，且不存在更长合法子序列。
	- 设计动机：没有对齐就无法解释跨语言分数。AlgoVeri 把“题目难度差异”尽量固定住，让观察到的差距更多反映工具链抽象、模型 proof search 和语言语法障碍。

2. **经典算法级别的全局不变量任务**:

	- 功能：把评测重心从简单函数和局部属性提升到需要全局推理的数据结构、图算法、动态规划、贪心和数学算法。
	- 核心思路：77 个任务覆盖堆、线段树、红黑树、Bellman-Ford、Edmonds-Karp、Gaussian elimination 等，需要模型生成 loop invariant、ghost state、辅助 lemma、termination proof 或 Lean tactic script。
	- 设计动机：真实软件正确性常依赖跨循环、跨数据结构的全局性质。只评估单步操作会高估模型能力，尤其会掩盖模型在 graph invariant 和复杂状态更新上的失败。

3. **验证成功后的语义过滤**:

	- 功能：防止模型通过 vacuous proof、作弊语句或更简单算法拿到形式化验证通过。
	- 核心思路：评测报告两个指标：Compiler Verified 表示 verifier 接受；Full Mark 表示 verifier 接受且 LLM judge 判定实现符合指定算法语义。两者差值被称为 algorithmic fidelity gap。
	- 设计动机：形式化验证只保证“实现满足规格”，若规格不足或模型使用不当漏洞，仍可能不是目标算法。语义过滤让基准同时考察验证正确性和指令忠实度。
