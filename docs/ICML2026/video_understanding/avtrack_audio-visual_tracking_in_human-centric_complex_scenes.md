...
1. **动态窗口的说话人片段聚合（Speaker Chunks Aggregation）**:

    - 功能：将 ASR 产生的碎片化短转录合并为语义完整的说话人片段，作为后续局部处理的基本单元
    - 核心思路：使用 Whisper 将音频转录为带时间戳的文本片段 $\mathcal{C} = \{c_i = (t_i^s, t_i^e, x_i)\}$。对每个片段可选地通过 MossFormer2 做语音分离得到增强信号 $\hat{\mathbf{a}}_i$，再用 ECAPA-TDNN 编码器提取说话人嵌入 $\mathbf{e}_i = \mathcal{E}(\hat{\mathbf{a}}_i)$。相邻片段的余弦相似度 $\text{sim}(c_i, c_{i+1}) > \tau$ 时合并为一个 chunk，保留完整的语义上下文。与固定窗口相比，动态窗口能保持完整的语义单元和时序连续性
    - 设计动机：ASR 输出的短片段独立处理效率低且成本高，合并同一说话人的相邻片段可减少局部窗口数量并提升全局关联效率

2. **VLM 驱动的局部说话人定位（Local Window Process）**:

    - 功能：在每个说话人片段的时间范围内，将语音内容与视频中可见人物精确关联，生成逐帧实例掩码
    - 核心思路：将片段 $s_k$ 的时间边界转换为帧索引 $f_k^s, f_k^e$，利用 Qwen3-VL 作为局部推理器 $\mathcal{R}^{local}$，以语音文本 $x_k$ 和视频帧为输入预测活跃说话人的边界框 $\mathbf{b}_{\mathcal{R}}^{(f)}$。同时 SAM3 对每帧生成候选人物检测框和掩码，通过 IoU 最大化匹配将 VLM 预测与 SAM3 检测对齐：$\mathbf{b}^{(f)} = \arg\max_{\mathbf{b} \in \mathcal{B}_{\text{SAM3}}^{(f)}} \text{IoU}(\mathbf{b}_{\mathcal{R}}^{(f)}, \mathbf{b})$。选择掩码面积最大的帧作为关键帧用于后续全局关联
    - 设计动机：利用 VLM 的语言-视觉推理能力建立语音内容与视觉人物的语义桥梁，比传统音频特征匹配更鲁棒；SAM3 提供高质量实例掩码，两者互补

3. **全局身份关联（Global Window Process）**:

    - 功能：将分散在不同时间窗口的局部轨迹按说话人身份聚合为连贯的全局轨迹
    - 核心思路：收集所有局部轨迹的关键帧 $\mathcal{F}_{\text{key}} = \{I_{f_k^{\text{key}}}\}_{k=1}^{K}$，由全局推理器 $\mathcal{R}^{global}$（同样基于 VLM）根据人物外观将关键帧分组，建立身份映射 $\mathcal{G}: p \mapsto \{k_1, k_2, \ldots\}$，最终将同一身份 $p$ 的所有局部轨迹合并为全局轨迹 $\mathcal{T}_p = \bigcup_{k \in \mathcal{G}(p)} \mathcal{T}_k^{\text{local}}$，无观测帧插入空掩码保持时序连续
    - 设计动机：同一说话人可能出现在多个不连续的片段中，仅靠局部处理无法建立跨片段的身份一致性，全局推理补充了长程关联能力
