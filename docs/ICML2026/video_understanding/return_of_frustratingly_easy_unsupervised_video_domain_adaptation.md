...
1. **排列不变的静态特征流 $\mathcal{M}_2$**:

    - 功能：抽取与帧序无关的"静态语义"（背景、场景风格、光照）。
    - 核心思路：$\mathcal{M}_2$ 由无位置编码的自注意力、残差、LayerNorm、逐 token 前馈和最终的平均池化组成，对输入帧的任何排列输出相同结果。Theorem 1 给出排列不变性的形式化证明。
    - 设计动机："静态内容"本身不依赖时间顺序，用排列不变结构提取最自然；不需要额外正则即可保证。

2. **时空特征相减**:

    - 功能：通过减法显式去除空间域差异，把任务化简为"只对齐时间差异"。
    - 核心思路：假设每帧特征可加性分解 $z_t = s + u_t$（静态 + 动态），则 $\mathcal{M}_1(\mathbf{X}+\mathbf{P}) - \mathcal{M}_2(\mathbf{X})$ 在理想情况下只保留动态分量。即使 $\mathcal{M}_2$ 估计不完美，Theorem 3 也给出 Wasserstein 距离的上界——误差随 $\mathcal{M}_2$ 改进而单调缩小。
    - 设计动机：相比 VAE 这种重型解耦解码器，"直接减"是最经济、可微、有理论保证的解耦手段。

3. **两损失训练目标**:

    - 功能：仅靠监督 + 对抗即驱动整套模型。
    - 核心思路：$\mathcal{L} = \mathcal{L}_{cls} + \lambda_1 \mathcal{L}_{adv}$，其中 $\mathcal{L}_{cls}$ 是源标签 + 目标伪标签的交叉熵，$\mathcal{L}_{adv}$ 是帧级 + 视频级的 GRL 域对抗损失。只剩一个超参 $\lambda_1$ 需调。
    - 设计动机：直接对应 UDA 的理论下界（源风险 + 域差异）；把"分离"的工作交给架构而不是损失。
