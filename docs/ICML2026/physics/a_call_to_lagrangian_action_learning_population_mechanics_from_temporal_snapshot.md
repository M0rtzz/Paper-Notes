...
flowchart TD
    A["输入：时间快照 {p_t} + 初速度 v₀"] --> B["群体势能参数化 Ψθ<br/>autograd 求粒子梯度（Prop 3.1）"]
    G["可学习阻尼 γ<br/>γ=0 守恒 · γ→∞ 退化为梯度流"] --> C["广义牛顿加速度<br/>a = −∇Ψθ − γv"]
    B --> C
    C --> D["Leapfrog 推进<br/>从 (p₀,v₀) 模拟 K 步边际"]
    D --> E["预测边际 ↔ 观测边际<br/>Sinkhorn 散度损失"]
    E -->|端到端反传（DTO）更新 θ,v₀,γ| B
    E --> F["学到的力学方程<br/>插值 / 未来预报"]
```
...
**2. 无参考的模拟式学习：从初始条件 rollout 覆盖所有观测，不依赖任何参考过程**

梯度流系方法（流匹配、JKOnet、AM）大多要预先指定一个 reference SDE 或 OT plan，一旦真实数据不满足这个先验（比如快照不配对）性能就崩。WLM 改成从初始条件 $(p_0,v_0)$ 出发，用同一组 $(\Psi_\theta,\gamma)$ 一路 Leapfrog 推进模拟边际：每轮随机采样 rollout 步长 $K\sim\text{Unif}[1,M]$，损失只在每对预测/观测边际间累积 Sinkhorn 散度，并直接沿离散轨迹反传（discretize-then-optimize）。因为不假设配对、不注入参考动力学，它在缺乏领域先验的未知物理/生物系统上反而更稳——实验里不配对 SDE 上 $W_1$ 从一阶方法的 0.236 降到 0.068 正是这个解耦带来的。
