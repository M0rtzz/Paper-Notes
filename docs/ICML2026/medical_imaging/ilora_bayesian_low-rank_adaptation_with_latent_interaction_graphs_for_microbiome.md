...
description: >-
  [ICML 2026][医学图像][LoRA] iLoRA 用贝叶斯方法从每个微生物组样本里推断一张稀疏的菌群交互图（Poisson 边 → Laplace 稀疏化 → GNN 嵌入），再用这个图去生成 input-conditioned 的 LoRA 矩阵 $A$…
tags:
  - "ICML 2026"
  - "医学图像"
  - "LoRA"
...
### 整体框架
输入是 $M$ 维菌群丰度 $X \in \mathbb{R}^M$，输出是诊断 $\hat y \in \{0,1\}$（UC vs CD 二分类，做成 next-token "yes/no"）和一张 $K \times K$ 的稀疏交互图 $\hat A$。pipeline 是双分支：

- **预测分支**：把 $X$ 编成 prompt 喂给 frozen LLM，标准 LoRA 风格做 next-token 预测；
- **iLoRA 分支**：先用 selector $S(\cdot)$（IBD 实验里是 MaAsLin2 选 top-20 species）压到 $K$ 个关键 taxa $Z = S(X) \in \mathbb{R}^K$；对每对 $(i,j)$ 推一个 Poisson 边变量 $\tilde\alpha_{ij} \sim \mathrm{Pois}(m_{ij})$，再通过 NPN 变换成 Laplace 稀疏边 $\bar\alpha_{ij} \sim \mathrm{Laplace}(0, b_{ij})$，得到 sparse adjacency $\hat A$；GNN 编码 $\hat A$ 出图表示，用 hypernetwork 把图表示映射成 LoRA 的 $A$ 矩阵（$B$ 在样本间共享、静态），喂回预测分支。

### 关键设计

1. **Poisson 边图 + Gaussian proxy 重参数化**:

    - 功能：把每对 taxa 的 co-occurrence 强度建成 Poisson 随机变量，避开离散采样的不可导问题。
    - 核心思路：变分后验取 $q_\varphi(\tilde\alpha_{ij}\mid Z) = \mathcal{N}(u_{ij}, \delta_{ij}^2)$，先验是 input-dependent 的 $\mathrm{Pois}(m_{ij}^{(0)})$（$m_{ij}^{(0)} = \mathrm{Softplus}(f_{\phi_0}(e_{ij}))$，$e_{ij}$ 是 LLM 编码出的 node embedding 拼出的 edge feature）。论文证明：用 Gaussian proxy $\mathcal{N}(m,m)$ 去近似 $\mathrm{Pois}(m)$ 时，匹配 KL 的唯一正最小值是 $m_{ij} = \frac{2u_{ij}-1 + \sqrt{(2u_{ij}-1)^2 + 8\delta_{ij}^2}}{4}$。这样训练时走 Gaussian reparameterization $\tilde\alpha_{ij} = u_{ij} + \delta_{ij}\epsilon_{ij}$ 拿梯度，KL 用闭式 Poisson-Poisson 散度 $m_{ij}^{(0)} - m_{ij} + m_{ij}\log(m_{ij}/m_{ij}^{(0)})$。
    - 设计动机：Poisson 的语义"非负事件强度"很贴合菌群 co-occurrence，但直接对 $\mathrm{Pois}$ 做 VI 没有低方差 reparameterization；Gaussian proxy + 闭式 rate matching 兼顾了 Poisson 解释和端到端可微。

2. **NPN 把 Poisson 边变成 Laplace 稀疏边**:

    - 功能：把稠密的非负 co-occurrence 图压成"大多数边几乎为 0、少数关键 cross-talk 边非零"的稀疏图。
    - 核心思路：借用 Natural Parameter Network 的思想，对每条边走一个 sampling-free 的分布到分布映射 $(\mu_{ij}, b_{ij}) = \mathcal{T}_{\mathrm{npn}}(\tilde\alpha_{ij}, e_{ij})$，固定 $\mu_{ij}=0$，把 $b_{ij}$ 解释成 edge-specific sparsity scale，得到 $\bar\alpha_{ij} \sim \mathrm{Laplace}(0, b_{ij})$。利用 Laplace 的 Gaussian scale-mixture 表示 $\bar\alpha_{ij} \mid \sigma_{ij}^2 \sim \mathcal{N}(0,\sigma_{ij}^2),\; \sigma_{ij} \sim \mathrm{Rayleigh}(b_{ij})$，让 NPN 在连续 natural parameter 上跑、避开离散 Poisson 采样；先验是 $\mathrm{Laplace}(0, b_0)$，KL 闭式：$\log(b_0/b_{ij}) + b_{ij}/b_0 - 1$。
    - 设计动机：菌群交互图本质上是稀疏的——绝大多数 taxa 对没有显著 cross-talk。Laplace(0, $b$) 的"尖峰 + 重尾"先验天然诱导稀疏；NPN 提供一条 deterministic 的概率管道，把 Poisson 阶段的不确定性传过来而不必再做一次离散采样。

3. **Graph-to-LoRA hypernetwork 与贝叶斯预测**:

    - 功能：把每个样本的稀疏交互图变成那个样本专属的 LoRA $A$ 矩阵，并用 Monte Carlo 在图后验上做 calibrated 推理。
    - 核心思路：用 GNN 在 $\hat A$ 上消息传递得到图表示，hypernetwork 输出 LoRA 的 $A \in \mathbb{R}^{r \times d_\text{in}}$（$B$ 静态共享），把 input-conditioned $\Delta W(X) = s B A(X)$ 注回 frozen LLM。推理时按 $\hat p(y\mid X) = \frac{1}{S}\sum_s p_\theta(y\mid X, \bar A^{(s)})$ 做 MC 平均，$\bar A^{(s)}$ 从 Laplace 后验里采样。
    - 设计动机：标准 LoRA 的 $A$ 是 input-agnostic 参数；这里改成 $A(X)$ 让 LoRA 真正"看 input 关系结构办事"，同时把图后验当作适配器的随机性来源天然给出 epistemic uncertainty。
