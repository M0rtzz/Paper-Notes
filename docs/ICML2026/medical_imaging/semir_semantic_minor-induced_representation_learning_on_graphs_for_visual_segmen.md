...
密集推理的痛点是计算量挂在体素数上而不是解剖复杂度上，而经典超像素又是任务无关、回写还要插值。SEMIR 用三个图算子级联来解决。先是种子驱动 flood-fill 的**边收缩**：把邻接体素 $p$ 合并到种子 $s$ 当且仅当 $\|I_p - I_s\|_n \le \psi$——关键是阈值相对的是种子而非当前 supernode 的滚动均值，这样低对比渐变区会保留成一串「相邻 supernode 链」而不是被全部塌成一坨。接着是**节点删除**，把面积 $a_v$ 或平均强度 $\bar{I}_v$ 越界的 supernode 剔掉（由 $\beta=(\beta_{\min}, \beta_{\max}, m_{\min}, m_{\max})$ 控制），顺手清掉采集噪声小点；删掉的区域背景默认填 0，所以是保守剔除而非制造假阳性。最后是**边删除**，若相邻超节点满足 $\|\bar{I}_{v_i}-\bar{I}_{v_j}\|_n > \alpha$ 就切断该边，这一步把强梯度变成显式的 cut，直接定义出分割边界。三个算子合起来既保拓扑又留出任务驱动调参的余地，而且有代数保证撑底：Lemma 3.1 保证每个 supernode 对应 $G$ 中一个连通子图（partition 严格无重叠），Theorem 3.2 保证回写是 exact 双射、零边界伪影——这正是几十年来超像素方法插值伪影痼疾的根治。
