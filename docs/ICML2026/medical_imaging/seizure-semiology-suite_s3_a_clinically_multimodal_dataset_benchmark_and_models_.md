...
### 整体框架
S³ 由三个组件构成：**(1) Seizure-Semiology-Dataset** —— 来自 UCLA EMU 2019–2023 年共 116 位成人患者的 438 段连续视频，专家标注 20 项 ILAE 定义的语义学特征（如自动症 automatisms、强直 tonic、阵挛 clonic 等），每个特征记录"是否出现 + 起止时间戳 + 文本理由"，总计 35,000+ 标签；按患者层级 4:1 划分训练/测试，并保持 ES（癫痫性）:NES（非癫痫性）比例。**(2) Seizure-Semiology-Bench** —— 7 个任务的层次化评测体系，每个任务有自己的提示模板、采样协议（30s 滑窗、事件中心裁剪、稀疏采样）和评估指标（F1、MAE、Edit Distance、LCS ratio、Seizure-RQI 等）。**(3) 改进策略** —— 在 Qwen2.5-Omni-7B 上做 SFT 与 GRPO 两种癫痫专属微调，并提出两阶段神经符号分类器解耦感知与诊断推理。

数据集质量由五阶段标注流水线保障：专家培训 → 标注员独立标 75 段视频验证一致性（Kappa = 0.8395） → 标注员独立标其余 287 段（疑难案例由 3 位专家共识仲裁） → 自适应采样验证特征分布与专家标注收敛（ES Pearson 0.893、NES Pearson 0.782） → LLM 文本纠错 + 规则化时间戳校验。

### 关键设计

1. **七级层次化任务基准（Seizure-Semiology-Bench）**:

    - 功能：把"看懂癫痫视频"分解为可独立诊断的能力梯度——Task 1 二分类特征识别（20 个 yes/no 提示）、Task 2 文本理由生成（解释为什么判某特征存在）、Task 3 空间偏侧化（强制选择题 + 显式锚定 patient's left/right 而非 camera view）、Task 4 时间边界定位（MM:SS 时间戳 + MAE 秒级评估）、Task 5 症状时序排序（Edit Distance / Temporal-F1 / LCS ratio 三指标）、Task 6 叙事报告生成、Task 7 临床诊断（ES vs NES，分 video-only / report-augmented / two-stage 三档对比）。
    - 核心思路：任务难度严格递增，且后续任务的失败可回溯到前置任务——例如 Task 5 排序差可分解为 Task 1 识别错（"what"错）+ Task 4 时间错（"when"错）的级联错误，这种可追溯性是直接做端到端"诊断准确率"评测拿不到的。
    - 设计动机：通用 MLLM 即便平均分数过得去，也可能在某个临床关键维度（如偏侧化）系统性归零；不分层就掩盖了真实失败模式。论文实测发现 Qwen2.5-VL 系列从 7B 扩到 72B，Task 1 平均 F1 仅在 0.42–0.45 浮动，证明"加大规模"不是解药，必须靠分层定位瓶颈。

2. **Seizure-RQI 临床对齐报告评估指标**:

    - 功能：取代 BLEU/ROUGE/BERTScore 这类与临床判断几乎零相关（Pearson $r \leq 0.10$）的表面词形指标，给生成的癫痫报告打一个真正对齐专家判断的分数。
    - 核心思路：评分 = 加权基础分 × 多重惩罚项。基础分由四个分量构成——结构完整性 S（占 15%，是否包含起始/扩散/发作后 3 段）、症状覆盖度 C（占 35%，正确抽出特征数 / 真值特征总数）、关键定位特征 L（占 25%，4 个偏侧化特征 left/right/none 匹配比例）、时序保真度 T（占 25%，按时间顺序提取的特征列表与真值的 Temporal F1）。最终 $\mathrm{RQI} = (15S + 35C + 25L + 25T) \times P_{\text{hall}} \times P_{\text{off}} \times P_{\text{len}} \times P_{\text{haz}}$，其中四个 $P$ 分别惩罚幻觉特征、跑题内容（如护理干预）、过度冗余和危险临床表述。
    - 设计动机：临床报告的价值在"是否描述了关键定位线索""时间顺序是否对""有无危险误导"上，而非词级相似度。论文验证 RQI 与专家判断的 Pearson 相关达 0.57、pairwise 准确率 0.74，远高于通用指标的 ≈0.54。

3. **两阶段神经符号诊断框架**:

    - 功能：解决直接让 MLLM 端到端读视频出诊断时容易在长时序上下文里"幻觉式推理"的问题。
    - 核心思路：第一阶段把 MLLM 当作纯感知器，执行 Task 1 输出一个 20 维二值特征向量 $v \in \{0,1\}^{20}$，把高维非结构化视频压成结构化、可解释的临床特征表示；第二阶段把 $v$ 喂给 Random Forest 等浅层统计分类器做 ES vs NES 判别。MLLM 只负责"看到什么"，逻辑分类外包给可靠的符号化模型。
    - 设计动机：作者发现 MLLM 在视觉识别（Task 1）上经领域微调后能与专家逼近，但在"基于多个特征做规则化推理"上表现不稳。把两件事解耦后，Random Forest 还能输出特征重要性（强直、头部偏转、快速眨眼、睡眠中发作等被打高权重），可解释性远高于端到端 MLLM。该框架在与 seizure_omni_sft-7B 组合时把 ES vs NES 的 F1 从直接诊断的 0.70 推到 0.96。

### 损失函数 / 训练策略
对 Qwen2.5-Omni-7B 做两种癫痫专属微调：**(i) SFT** 用 (video, prompt, answer) 三元组监督学习；**(ii) GRPO** 用组相对策略优化，按任务定制奖励——Task 1/3/7 用准确率奖励，Task 2/6 用 BLEU+ROUGE 复合奖励，Task 4 用时间邻近度奖励，Task 5 用 LCS ratio 奖励。GRPO 的关键教训是：在 Task 6 用 BLEU/ROUGE 作为奖励反而把模型逼向重复输出，因为这些指标本身不反映临床相关性——这反向验证了 Seizure-RQI 的必要性。
