...
1. **Bayesian Mask Decoder（双粒度 Weibull 后验）**:

    - 功能：在 SAM2 原 decoder 位置用 Weibull 分布同时建模图像 token $\mathbf{f}\in\mathbb{R}^{H\times W\times C}$ 和 mask token $\mathbf{m}_k\in\mathbb{R}^C$ 的不确定性，输出像素级 uncertainty map。
    - 核心思路：用一个 conv head 预测空间变化的 $(\lambda_f,\kappa_f)$，用一个共享 MLP 预测 per-channel 的 $(\lambda_{m,c},\kappa_{m,c})$，通过重参数化 $w_i = \lambda_i \cdot (-\ln(1-u))^{1/\kappa_i}$ 采样。两支 reparametrized 特征做内积出 logits，从而把权重不确定性闭式传到 mask 概率。推理可选 analytic 模式（用 $\mathbb{E}[w_i]=\lambda_i\Gamma(1+1/\kappa_i)$ 和 MacKay 的 probit 近似得到逐像素 Bernoulli 熵）或 Monte Carlo 模式。
    - 设计动机：Weibull 是非负且形状柔性的，比 Gaussian 更适合建模 token 强度（附录 A.2 也对比了 Dirichlet 等）。双粒度（图像 token + mask token）刚好覆盖边界局部不确定性与全局语义不确定性两个层面，正好对治"边界模糊"和"目标识错"这两类失败。

2. **协同 Style + Deformation Adversarial 攻击**:

    - 功能：在线生成同时扰动 texture（颜色/材质）和 shape（几何形变）的"硬样本"，模拟 OOD。
    - 核心思路：Style attacker 从 mask 区域抽 per-object RGB 均值/方差 $(\boldsymbol\mu_k,\boldsymbol\sigma_k)$，用 GCN 在 object graph 上协同预测残差 $(\Delta\boldsymbol\mu_k,\Delta\boldsymbol\sigma_k)$，通过 AdaIN 替换风格统计量得到 stylized image；Deformation attacker 把 backbone features 加上 mask embedding 后预测 per-pixel offset field $\boldsymbol\delta_k$，再用 differentiable grid sampling 同时把图和 GT mask 一起 warp，保持监督一致。两支共用主干特征，只跑一次 backbone forward。通过 GRL 在反向传播时把符号翻转，attacker 自动朝"更难"方向更新，免去 PGD 式 inner loop。
    - 设计动机：传统 $\ell_p$-bounded 对抗只追求 worst-case error，缺乏语义意义；style/deformation 这一拆解直接对应 texture bias 和 shape bias 两个被生物视觉文献验证过的鲁棒性轴。GRL 替代 PGD 让 min-max 单次反向就完成，训练效率高得多。

3. **Uncertainty-Accuracy Alignment Calibration Loss**:

    - 功能：让 uncertainty 在对抗样本上始终覆盖真实像素误差。
    - 核心思路：定义 calibration loss $\mathcal{L}_{\text{cal}} = e\cdot\exp(-\text{sg}[u]) + (1-e)\cdot\exp(\text{sg}[u])$，其中 $e=|\hat{\mathbf{M}}-\mathbf{M}^*|$ 是逐像素误差，$u$ 是 analytic uncertainty，$\text{sg}[\cdot]$ 是 stop-gradient。第一项惩罚"自信但错"，第二项惩罚"不确定但对"。这个 loss 不直接更新分割主网（被 stop-gradient 切开），而是经由 GRL 反向更新 attacker。主网通过 segmentation + KL on adversarial branch 隐式被拉向 well-calibrated 区域。
    - 设计动机：直接拿 calibration loss 当主网的监督会让模型为了"看起来 calibrated"而牺牲精度；让 attacker 去最大化 miscalibration，主网用 seg + KL 去抵抗，这样的 min-max 设计让 calibration 来自数据生成端而非显式正则。
