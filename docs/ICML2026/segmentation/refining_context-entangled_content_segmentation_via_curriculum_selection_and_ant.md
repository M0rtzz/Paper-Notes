...
1. **时间统计样本加权 (TSSW) + Warm-up 课程**:

    - 功能：在样本级把"hard-but-informative"和"噪声/异常"区分开，构造稳定且抗噪的训练子集。
    - 核心思路：每轮用历史 checkpoint 算难度 $d_i=1-\mathrm{IoU}$，并在每个样本上维护一个长度为 $K$ 的循环 buffer，取得均值 $\mu_i=\frac{1}{K}\sum_k d_i^{(k)}$ 和方差 $\sigma_i^2=\frac{1}{K}\sum_k(d_i^{(k)}-\mu_i)^2$；min-max 归一化后转成两个权重：$\omega_i^\mu=1-\tilde\mu_i$ 偏好低均值损失（容易学），$\omega_i^\sigma=\exp(-(\tilde\sigma_i^2-\sigma^*)^2/(2\gamma^2))$（$\sigma^*=0.5,\gamma=0.2$）则用钟形函数偏好"中等方差"——方差太低说明一直学不会（疑似标注噪声），方差太高说明在决策边界震荡（疑似 ambiguous）。最终权重 $\omega_i=W^s_{\min}+(1-W^s_{\min})\cdot\omega_i^\mu\cdot\omega_i^\sigma\cdot(1-\tilde\mu_i(1-\tilde\sigma_i^2))$，最后一项 $\omega_i^{out}$ 专门压制"高均值-低方差"的离群模式（明显的 outlier 或错标）。在样本子集选择上还叠了一个 warm-up：阶段开始只用最容易的 60% 子集 $\mathcal{S}_t=\{i\mid d_i\le \mathrm{Per}(\{d_n\},p(t))\}$，$p(t)$ 在 $T_c$ 轮内线性增长到 100%。
    - 设计动机：传统 CL 用静态难度，看不出样本是"难但稳定可学"还是"噪声"；用时间维统计本质上是把 buffer 当作一个轻量的"样本可靠性探针"——稳定的高损失才是真异常，振荡的高损失是该好好学的。这套加权和课程子集解耦，可以叠加到任何主干的标准训练里。

2. **像素级熵加权 (PUE)**:

    - 功能：在 segmentation 的单图内部抑制不确定像素的梯度，避免边界/低对比区域劫持优化方向。
    - 核心思路：对每个像素的预测概率 $p_{h,w}=\sigma(\hat y_{h,w})$，算二分类熵 $H_{h,w}=-p\log_2 p-(1-p)\log_2(1-p)\in[0,1]$，$p=0.5$ 时取最大；然后定义软权重 $W_{h,w}(t)=W_{\min}+(1-W_{\min})\cdot(1-\beta(t)\cdot H_{h,w})$，其中 $\beta(t)=1-t/T_c$ 是随课程阶段衰减的系数，$W_{\min}=0.1$ 保住权重下限。早期 $\beta$ 大，高熵像素几乎被压平；越到后期 $\beta$ 越小，全像素监督逐步恢复。
    - 设计动机：和过往用不确定性"过滤伪标签"的工作不同，作者强调这里目的不是把不确定区域藏起来，而是不让它们一开始就占主导梯度；保留下限是为了一直留一点信号，避免完全屏蔽掉边界区域反而让模型学不到。把它和 TSSW 组合，就同时治了图间 (样本) 和图内 (像素) 两个维度的噪声。

3. **频谱失明微调 (Spectral-Blindness Fine-Tuning, SBFT) 反课程**:

    - 功能：在模型稳态后主动制造"信息匮乏"环境，逼模型从高频纹理依赖切到低频结构与上下文依赖。
    - 核心思路：把图像做 2D FFT，按一个固定截止频率把高频系数置零（或者按概率把高频幅度按比例衰减），再 IFFT 回空间域作为微调阶段的输入；模型必须靠剩下的低频亮度/形状/上下文来分割。这一阶段不再用 RCS 的样本选择，直接在全集上做短期 fine-tune，但训练时间被前面 RCS 跳过的简单样本省回来，整体训练成本反而比 baseline 低（见表 2）。
    - 设计动机：CECS 任务里目标和背景的纹理高度重合，高频反而是干扰；如果模型已经学会读结构，就要主动剥夺它"偷看纹理"的能力，把决策推到更鲁棒的低频通道。这是"反课程"的字面意思——故意提高难度，把模型从舒适区里赶出来。三个机制 (TSSW、PUE、SBFT) 合起来就是"stabilize-then-perturb"的完整闭环。
