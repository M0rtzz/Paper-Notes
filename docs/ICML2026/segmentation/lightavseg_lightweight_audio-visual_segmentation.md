...
### 整体框架
LightAVSeg 是一个双流架构：视觉流和音频流各自轻量编码，再经一条层级融合通路把音频线索逐步注入视觉表征。输入为视频帧 $x_v \in \mathbb{R}^{T \times 3 \times H \times W}$ 和原始音频波形 $x_a$。视觉流用 SeaFormer-Large 提取多尺度特征金字塔 $\{V_i\}_{i=1}^N$，音频流先做 STFT 得到 log-mel 频谱再用 MobileNetV2 编码为初始全局状态 $A_0 \in \mathbb{R}^{T \times C_a \times 1 \times 1}$（与视觉帧逐帧时间对齐）。Reciprocal Encoder 负责“语义筛选 (what)”，在每个 stage 用全局视觉描述子门控更新音频状态 $A_i$、再以通道偏置回写视觉特征 $\widetilde{V}_i^{enc}$；Cross-Modal Fusion Decoder 负责“空间定位 (where)”，在 upsampling 时维护一条递归音频路径 $A_i^\ast \to \hat{A}_i$ 并把它注入视觉解码特征 $\widetilde{V}_i^{dec}$，最后由分割头输出 mask。训练时额外挂一个多尺度音视对齐损失约束跨模态一致性，总损失 $\mathcal{L} = \mathcal{L}_{\text{seg}} + \lambda \mathcal{L}_{\text{msa}}$，其中 $\mathcal{L}_{\text{seg}}$ 是 Dice + BCE，$\lambda = 0.5$；该对齐分支推理期直接丢弃。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    XV["视频帧 x_v"] --> VE["视觉流：SeaFormer-Large<br/>多尺度视觉特征金字塔"]
    XA["原始音频 x_a"] --> AE["音频流：STFT log-mel + MobileNetV2<br/>初始全局音频状态 A_0"]
    VE --> ENC["Reciprocal Encoder（语义筛选·what）<br/>全局通道调制逐 stage 更新音频与视觉，O(N)"]
    AE --> ENC
    ENC --> DEC["Cross-Modal Fusion Decoder（空间定位·where）<br/>递归音频路径上采样持续注入视觉"]
    DEC --> HEAD["分割头 → 发声物体像素 mask"]
    DEC -.训练期监督.-> MSA["多尺度音视对齐损失 L_msa<br/>每尺度音视相似图 vs 前景 mask，推理丢弃"]
```
