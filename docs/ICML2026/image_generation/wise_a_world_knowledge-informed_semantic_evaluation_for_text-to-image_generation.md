...
description: >-
  [ICML 2026][图像生成][文本到图像评测] WISE 构建了一个包含 1000 条知识密集 prompt 的文本到图像评测基准，用文化常识、时空推理和自然科学知识检验模型是否能把隐含语义转化成正确视觉内容，并发现现有 T2I 与统一多模态模型在世界知识生成上仍有明显短板。
tags:
  - "ICML 2026"
  - "图像生成"
  - "文本到图像评测"
...
### 关键设计
1. **知识密集且视觉可回答的 prompt 构建**:

	- 功能：把评测从字面对象组合提升到隐含知识检索和视觉化。
	- 核心思路：prompt 来源包括教育材料、百科、公理常识题集和 LLM 辅助生成，再由人工筛选和改写。保留条件是答案必须能视觉呈现、目标相对明确、知识关系稳定，避免主要依赖公式、文字识别或多个同样合理答案。
	- 设计动机：如果 prompt 直接写出目标物，评测会变成普通的 prompt following。WISE 通过“线索 → 知识检索 → 视觉目标”的链条，迫使模型展示世界知识能否进入生成过程。

2. **三大领域覆盖文化、时空和科学知识**:

	- 功能：避免 benchmark 只测某一种常识或单一文化背景。
	- 核心思路：Cultural Common Sense 覆盖节日、运动、宗教、手工艺、建筑、动物、植物、艺术、名人和日常生活；Spatio-temporal Reasoning 包括水平/纵向时间、不同视角、地理关系和相对位置；Natural Science 覆盖生物、物理和化学。文化分布上，完整 WISE 中 global/neutral 占 56.6%，Western 占 22.6%，Non-Western 占 20.8%；文化子集中 Western 和 Non-Western 也基本平衡。
	- 设计动机：世界知识不是单一事实库。T2I 模型既可能在文化联想上失败，也可能在科学状态、时间季节或空间视角上失败；多领域能让结论更可靠。

3. **WiScore：强调知识一致性的复合评分**:

	- 功能：评价生成图像是否正确表达隐含知识，而不只是真实或好看。
	- 核心思路：每张图像按 Consistency、Realism、Aesthetic Quality 三维打 0 到 2 分，再按 $0.7/0.2/0.1$ 加权并除以 2 归一到 $[0,1]$。Consistency 权重最高，因为 WISE 关注的是 prompt 中目标对象、关系和知识约束是否被正确视觉化。主评估使用 GPT-4o-2024-05-13，附录还用 Gemini 和 Qwen judge 做稳定性检查。
	- 设计动机：传统 realism 或 CLIP-like 语义分数可能把“看起来漂亮但知识错误”的图像打高分。WiScore 把知识一致性放到核心位置，更符合 benchmark 目标。
