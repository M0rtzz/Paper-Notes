...
description: >-
  [ICML 2026][图像生成][切向放大] TAG 把每一步扩散更新沿当前潜变量方向分解为"径向 + 切向"两个分量，只对切向分量额外乘一个 $\eta \ge 1$ 的放大系数，从一阶 Taylor 展开上证明这等价于单调提升对数似然增益，从而把样本拉向数据流形高密度区…
tags:
  - "ICML 2026"
  - "图像生成"
  - "切向放大"
...
### 整体框架
TAG 是一个套在标准 DDIM/EDM/Flow Matching 等基础求解器外层的"残差重加权"算子。给定第 $k+1$ 步状态 $\bm{x}_{k+1}$：

1. 先用主干 $\epsilon_\theta$ 和基础求解器算出"原始更新" $\Delta_{k+1} = \tilde{\bm{x}}_k - \bm{x}_{k+1}$（如 DDIM 一步预测的差值）。
2. 用当前状态自身定义球面投影：$\bm{P}_{\mathcal{M}_{k+1}} = \hat{\bm{x}}_{k+1}\hat{\bm{x}}_{k+1}^\top$（径向投影到 $\bm{x}_{k+1}/\|\bm{x}_{k+1}\|_2$ 方向），$\bm{P}^\perp_{\mathcal{M}_{k+1}} = I - \bm{P}_{\mathcal{M}_{k+1}}$（切向）。
3. 用 $\bm{x}_k \leftarrow \bm{x}_{k+1} + \bm{P}_{\mathcal{M}_{k+1}}\Delta_{k+1} + \eta\,\bm{P}^\perp_{\mathcal{M}_{k+1}}\Delta_{k+1}$ 替代原 $\bm{x}_k = \bm{x}_{k+1} + \Delta_{k+1}$。

整个流程不需要额外的 denoiser 前向、不改任何参数、和 CFG/PAG/SEG/SSG 等已有引导可叠加。

### 关键设计

1. **切向放大的更新规则**:

    - 功能：把每步原始更新分解为径向 + 切向两部分，只对切向分量乘 $\eta$，保留径向不动。
    - 核心思路：用样本自身作投影基 $\hat{\bm{x}}_{k+1}$ 定义 $\bm{P}_{\mathcal{M}_{k+1}}, \bm{P}^\perp_{\mathcal{M}_{k+1}}$，新增量 $\Delta^{\text{TAG}}_{k+1} = (\bm{P}_{\mathcal{M}_{k+1}} + \eta\bm{P}^\perp_{\mathcal{M}_{k+1}})\Delta_{k+1}$。代入 DDIM 系数后等价于对 $\epsilon_\theta$ 的切向分量做同等放大，因此可看作"对预测噪声的几何感知再缩放"。
    - 设计动机：图 2 的可视化显示径向部分像无结构噪声、切向部分像清晰语义图，说明语义信息天然集中在切向方向；放大它就能选择性增强语义而不放大噪声。

2. **一阶 Taylor 增益单调性证明**:

    - 功能：从理论上回答"为什么放大切向就能提升图像质量"。
    - 核心思路：把推理时引导写成局部对数似然最大化问题 $\max (\bm{x}_k - \bm{x}_{k+1})^\top \nabla_{\bm{x}}\log p(\bm{x}\mid t_{k+1})|_{\bm{x}_{k+1}}$，在 $\|\bm{x}_k - \bm{x}_{k+1}\|_2 \le \delta_k$ 约束下，定义一阶 Taylor 增益 $G(\eta) := (\Delta^{\text{TAG}}_{k+1})^\top \nabla_{\bm{x}}\log p$。论文证明 $G(\eta)$ 关于 $\eta$ 单调递增（Theorem 4.1），意味着只要 $\eta>1$ 就严格提升局部似然。
    - 设计动机：与"经验上调大就好"不同，这里把切向放大解释为带步长约束的 MLE 投影，回答了它为什么不会像盲目缩放 CFG 那样破坏轨迹——因为径向分量保持不变，意味着保持了 Tweedie 恒等式给出的"一步标定"（$\langle \hat{\bm{x}}_{k+1}, \Delta^{\text{TAG}}_{k+1}\rangle = \langle \hat{\bm{x}}_{k+1}, \Delta_{k+1}\rangle$，公式 22）。

3. **CFG 上的切向对齐 ($\text{TAG}_{\text{cfg}}$)**:

    - 功能：把同样的几何视角搬到 CFG，缓解条件/无条件分支"切向不一致"造成的伪影。
    - 核心思路：记 CFG 残差 $\bm{g}_k = \bm{\varepsilon}_c - \bm{\varepsilon}_u$，取其切向投影 $\bm{g}_k^\perp = \bm{P}^\perp_{\mathcal{M}}(\bm{x}_k)\bm{g}_k$；再把条件预测 $\bm{\varepsilon}_c$ 投到 $\mathrm{span}(\bm{g}_k^\perp)$ 上得到对齐向量 $\bm{g}_k^{\text{align}} = \langle\bm{\varepsilon}_c, \bm{g}_k^\perp\rangle / \|\bm{g}_k^\perp\|_2^2 \cdot \bm{g}_k^\perp$；最终 $\tilde{\bm{\varepsilon}}_k = \bm{\varepsilon}_u + \omega\bm{g}_k + \eta\bm{g}_k^{\text{align}}$。
    - 设计动机：作者认为 CFG 的条件/无条件不一致主要是切向方向上的不匹配（径向被噪声调度共同约束），所以只在切向子空间里再对齐一下，就能让两支预测"同向前进"，避免标量 $\omega$ 调大时同时放大不该放大的不一致部分。
