...
### 整体框架
SURGE 是一个推理阶段的滤波循环，外层是物理时间 $t=0\to T-1$ 的同化步，内层是每个同化步内的扩散采样时间 $s\in[t,t+1]$ 被切成 $K$ 个小窗 $[s_k,s_{k+1}]$。每个内层窗里同时维护 $N$ 条粒子轨迹：
1. **传播**：用引导 SDE（公式 4）把每条粒子向前推一步 $\Delta s$，漂移项 = 扩散网络 $v_\theta$ + 似然引导 $\Sigma(s)\nabla_x G(x_s, s\mid y_{t+1})$，扩散项为 $\Sigma^{1/2}(s)\mathrm{d}W_s$。
2. **加权**：按 Girsanov 公式 + 渐进似然计算每条轨迹这一小段的重要性权重增量 $\beta_t^{(i)}$，累乘进 $w_{t+s_k}^{(i)}$。
3. **重采样**：归一化权重，若有效粒子数 $\widehat{N}_{\text{eff}} = 1/\sum_i (\tilde w^{(i)})^2 < c$ 就按权重做 Categorical 重采样、权重重置为 $1/N$。

最终每个物理步结束时，用加权经验测度 $\sum_i \tilde w_{t+1}^{(i)} \delta_{x_{t+1}^{G,(i)}}$ 作为后验 $p(x_{t+1}\mid y_{1:t+1})$ 的 approximation-free 近似。

输入：预训练扩散代理 $v_\theta$、初始粒子集、观测序列 $\{y_t\}$、引导势 $G$、方差调度 $\Sigma(s)$、子步数 $K$、重采样阈值 $c$；输出：后验粒子轨迹 $\{x_t^{(i)}\}$。完全不训练任何新参数。

### 关键设计

1. **路径测度上的 Girsanov 重要性权重**:

    - 功能：把"引导扩散生成的有偏路径分布 $\mathbb{P}^G$"通过权重映射到"真后验路径分布 $\mathbb{P}(\cdot\mid y_{1:t+1})$"，得到无近似偏差的后验粒子表示。
    - 核心思路：用 Radon–Nikodym 链式法则 $\frac{\mathrm{d}\mathbb{P}(\cdot\mid y_{1:t+1})}{\mathrm{d}\mathbb{P}^G(\cdot\mid y_{1:t+1})} = \frac{\mathrm{d}\mathbb{P}(\cdot\mid y_{1:t+1})}{\mathrm{d}\mathbb{P}(\cdot\mid y_{1:t})} \cdot \frac{\mathrm{d}\mathbb{P}(\cdot\mid y_{1:t})}{\mathrm{d}\mathbb{P}^G(\cdot\mid y_{1:t})}$ 把权重拆成两块：第一块 $\propto p(y_{t+1}\mid x_{t+1})$ 是经典观测倾斜；第二块用 Girsanov 公式得 $\exp\bigl(-\int_0^1 \Sigma^{1/2}(s)\nabla_x G \cdot \mathrm{d}W_s - \tfrac12\int_0^1 \|\Sigma^{1/2}(s)\nabla_x G\|^2 \mathrm{d}s\bigr)$，专门抵消引导项引入的漂移偏差。
    - 设计动机：传统粒子滤波只在终点 $x_{t+1}$ 用 $p(y_{t+1}\mid x_{t+1})$ 加权，无法纠正"采样过程内部"由近似引导带来的路径偏差；提升到路径空间后，引导势 $G$ 不再要求等于 Doob $h$-变换，任何启发式引导都能被自动校正。

2. **渐进式（time-distributed）观测似然分摊**:

    - 功能：把整段观测对数似然 $\log p(y_{t+1}\mid x_{t+1})$ 沿内部时间 $s\in[0,1]$ 平摊到 $K$ 个子窗里，避免某一步出现极端权重导致粒子退化。
    - 核心思路：在权重公式（6）里把端点似然替换为渐进项 $s_{k+1}\log p(y_{s_{k+1}}\mid x_{s_{k+1}}^{(i),G}) - s_k\log p(y_{s_k}\mid x_{s_k}^{(i),G})$。当 $\Delta s$ 足够小，相邻两步似然差也小，权重在小窗内变化平缓，重采样方差可控；多个小窗的累积效果与一次性 $\log p(y_{t+1}\mid x_{t+1})$ 在数学上等价（伸缩求和）。
    - 设计动机：高维状态空间下经典粒子滤波的权重 $\propto p(y\mid x)$ 在维度诅咒下几乎全部退化为 0/极少数 1。把似然摊薄到子窗，等价于让重要性采样的"代价"分多次小幅支付，使有效粒子数维持在阈值 $c$ 以上，是高维 SMC 必备稳定化技巧。

3. **推理阶段插件式封装**:

    - 功能：作为 FlowDAS、SDA 等扩散数据同化方法的 inference-time 包装器，零额外训练、零模型修改。
    - 核心思路：算法 1 中除了 $v_\theta$（来自任意预训练扩散代理）和引导 $G$（来自 backbone 的现成选择，如 $\Sigma(s)\nabla_x G = \lambda(s)\nabla_x \log p(y\mid x_s)$）之外，只需 Euler–Maruyama 离散化 SDE、按公式累计权重、按 ESS 触发重采样；用 backbone 自身的方差调度 $\Sigma$ 和网络架构。
    - 设计动机：扩散代理的训练成本极高（天气/流体场动辄数天 GPU 时间），现有 backbone 已经投入巨大；让矫正在 inference 阶段以 SMC 形式发生，能立即享受 backbone 升级，同时避免触碰原模型权重带来的兼容性问题。

### 损失函数 / 训练策略
SURGE 完全无训练。所需超参：粒子数 $N$（实验中 $N=20$）、内部时间子步数 $K$、重采样阈值 $c$（通常 $c=N/2$）、引导强度 $\lambda(s)$ 沿用 backbone 设置。计算开销主要来自每个物理步要并行跑 $N$ 条扩散采样轨迹，整体相对单条采样放大 $\sim N$ 倍。
