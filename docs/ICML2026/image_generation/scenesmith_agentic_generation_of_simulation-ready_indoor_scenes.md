...
构造过程是一棵 stage 树：根 stage 由 layout agent 生成 $M$ 个房间的建筑几何；每个房间独立走「家具 → 墙挂 → 吊顶」三个 stage，提示词由全局 $\mathcal{T}$ 派生为房间级 $\mathcal{T}_j$；之后每个房间里被选中的支撑实体（家具表面、墙架、地面区域）再各自开分支用实体级提示 $\mathcal{T}_{j,k}$ 添小物件——「书放这一格、植物放那一格」这种跨表面协调正是在分支提示里被显式约束的。所有 stage 完成后做物理后处理（投影解穿透 + 重力沉降），再 flatten 成 $\mathcal{S}$。

### 关键设计

1. **Designer-Critic-Orchestrator 三角 agent 与受限工具**:

    - 功能：把每个 stage 的"提案 / 评估 / 控制"职责彻底切开，避免单 agent 自评偏差，让迭代精修在工具调用粒度上结构化进行。
    - 核心思路：designer 拥有场景修改工具（放置/调整资产、snapping、组装复合物体如水果碗），可在一个 turn 内连续调用任意多工具完成一组原子编辑；critic 只能访问观察类与可行性验证类工具（查询 pose、渲染视图、碰撞检测、可达性检测），输出标量分数 + 自然语言反馈；orchestrator 把 designer / critic 当作工具调度，维护历史 checkpoint，当 critic 分数下降时回滚到上一个状态。每个 agent 还有按 turn 的滑动窗记忆，更早的轮次由 LLM 摘要压缩，视觉观察用有限窗口承载且 stage 结束时清空。
    - 设计动机：单 agent 的 reason-act-reflect（如 SceneWeaver）容易陷入"自己提案自己打 90 分"的陷阱；显式划分角色 + 工具权限隔离，让 critic 站在外部视角抓 designer 漏掉的语义和物理问题，而 orchestrator 的回滚机制把"探索"变成了"安全探索"，避免迭代越改越差。

2. **资产路由 + 三路按需生成（生成 / 检索 / 薄覆盖）**:

    - 功能：把"我要一个红色苹果"或"我要一个带抽屉的厨房柜"这类 designer 请求自动落地为带碰撞几何与物理属性的、可仿真的资产，覆盖静态物体、铰接物体、薄装饰三种截然不同的物理形态。
    - 核心思路：资产路由器先把复合请求（如 fruit bowl）分解成原子资产（bowl + 多个 fruit）。静态物体走 text-to-3D 链路——GPT Image 1.5 出参考图 → SAM3 分割前景 → SAM3D 重建带贴图网格 → 归一化朝向 + 按目标尺寸缩放 → 凸分解出灰色碰撞件 → VLM 估计质量/质心/摩擦/惯性（图 3 蓝色椭球可视化惯量）。铰接物体走检索链路：从 ArtVIP 库取出预制的多 link 模型（含 joint 定义），再补物理属性。薄装饰（地毯、海报、桌布）用 thin coverings——轻量几何面片 + ambientCG 检索的 PBR 材质，避不必要的刚体复杂度。所有候选资产都过网格完整性检查 + VLM 语义验证，失败则按预算重试或换策略，否则把失败原因反馈给 agent。
    - 设计动机：现有 text-to-3D 对铰接结构和运动学仍不可靠（URDFormer 等也只能做粗糙重建），强行用 generative 一把梭只会生成卡死的"门拉不开的柜子"；同时 retrieval-only 又被库容量绑死，无法支持开放词表。"按物体类型选最合适的获取策略 + 统一物理属性后处理"是当前唯一能同时拿到开放词表、铰接可用、立刻可仿真的工程平衡点。On-demand 生成还顺带规避了机器人策略在已知资产库上偷跑训练数据的污染问题。

3. **层级树构造 + 物理后处理双保险**:

    - 功能：让场景结构从大到小逐层 commit，每个分支的局部决策既能并行又能与全局意图保持一致；最后用轻量物理后处理把 agent 留下的小穿透和不稳定配置自动修掉，保证导出即可仿真。
    - 核心思路：层级树 = 房间分支 + 每个房间内支撑实体再分支，提示词按层级 refine——$\mathcal{T} \to \mathcal{T}_j \to \mathcal{T}_{j,k}$ 把全局风格、房间用途、表面语义层层向下传递，相关表面（同一书架的两层）合并到同一分支以保证协调摆放（"书放上层、植物放下层"）。物体放置统一在支撑面坐标系下指定 $SE(2)$ pose，再通过已知的支撑面 $SE(3)$ pose 升维到完整 $SE(3)$，从根本上避免"花瓶悬空"或"杯子斜插桌面"。家具阶段和小物件阶段结束后各跑一次物理后处理：先用非线性优化把每个物体投影到最近的无碰撞配置（保留朝向），再在 Drake 里重力仿真让不稳的物体自己沉降到静态平衡。
    - 设计动机：让 agent 严格满足物理约束代价极大且收敛慢；"agent 大致合理 + 物理 solver 精修"的分工把语义/美学和物理可行性解耦，既保留了 agent 的灵活性，又用 mm 级解穿透 + 重力沉降这类廉价确定性步骤兜住"仿真就绪"这条硬底线。墙和地板用带厚度的体积几何（而非平面），也是为了离散时间步物理仿真下抗穿透。
