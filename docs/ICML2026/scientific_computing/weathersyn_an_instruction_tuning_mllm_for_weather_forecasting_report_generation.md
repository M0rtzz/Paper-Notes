...
description: >-
  [ICML 2026][科学计算][天气预报报告] WeatherSyn 把气象预报员的报告写作流程拆解成"看图→列要点→出稿"的多模态指令任务，先建了首个覆盖 31 个美国城市、8 类天气要素的 WSInstruct 数据集，再用 SFT→RFT→DPO 三段式微调 Qwen3-VL-8B…
tags:
  - "ICML 2026"
  - "科学计算"
  - "天气预报报告"
...
1. **Aspect-controlled prompting + 结构化报告切分**:

    - 功能：把开放式报告强行结构化成"日期 → 该日讨论的 aspect 列表 → 报告文本"模板，让生成可控、可对齐评测。
    - 核心思路：先用正则 + dateparser 把原始报告里的"Today/星期几"等相对表达转换成绝对日期，再用 Qwen-2.5-72B 按时间轴切成 4 个连续日子报，并要求至少覆盖前 4 天中的 3 天；随后让模型按专家定义的 8 大 aspect（temperature、wind、humidity、frontal system、pressure system、wave pattern、wind flow system、event）和每个 aspect 下的细粒度 claim 类目把报告"标签化"。最终 prompt 模板形如 `<<date, weekday>> Report:\n## Focus on: Temperature, Humidity`，推理阶段所有 baseline 也同样接收 aspect 列表以保证公平。
    - 设计动机：没有 aspect 约束时，模型受训练集要素分布支配，hit rate 仅 0.16（即生成 aspect 与 ground truth 只匹配 16%）；只在训练阶段加约束、推理时不加，模型反而会塌缩到训练里最频繁的 "event" aspect（hit rate 还掉到 0.12）；只有训练+推理都加 aspect 才能把 hit rate 推到 0.94，这是后续评测公平的前提。

2. **RFT：用拒绝采样把"事实正确但措辞多样"灌进去**:

    - 功能：缓解 SFT 数据规模小、措辞单一导致的过拟合，让模型学到底层气象现象而不是表层短语。
    - 核心思路：对 2017–2020 子集，用 SFT 后的模型 $\mathcal{M}$ 以温度 0.9 采 40 份候选；用 Qwen-2.5-72B 给每个日子报抽取 claim 并计算 step-level F1，只保留 F1=1 的子报；再用四种距离（编辑距离、TF-IDF、Jaccard、Sentence-BERT 余弦）从这些"事实正确"子报中挑出与原 ground truth 距离最大者，组装成新的整篇报告，作为增广样本 $\mathcal{D}_\text{RFT}$ 回到 Qwen3-VL-8B 上微调。训练目标仍是带掩码的 next-token loss：$\mathcal{L}_\text{VLM} = -\mathbb{E}_{(Q,I,\{R^i\}_{i=1}^N)\sim\mathcal{D}} [\sum_{i=1}^N \log p_\theta(R^i \mid I, Q)]$，stage 2 中 $N$ 不再固定为 1，而是按候选投票数自适应。
    - 设计动机：作者的 motivation 很直接——同一份初始气象场可以被合理表达为"temperatures rebound"或"increasing temperatures"等多种说法，这种"事实一致+词法多样"的样本最能强化图像-文本对齐能力；纯 SFT 在 Wave Pattern、Wind Flow System 等长尾 aspect 上 F1 仍然偏低，RFT 直接把这些 aspect 的 weighted F1 拉升 0.15–0.17。

3. **DPO：以 F1 为偏好信号锁定事实正确性**:

    - 功能：在 RFT 模型基础上，进一步把"事实正确"作为人类偏好显式优化进去。
    - 核心思路：在 2021 子集上对 $\mathcal{M}_\text{RFT}$ 采 40 份候选并算 step-level F1；用 F1=1 的日子报组装成 chosen $\mathcal{Y}_w$、用 F1 最低的子报组装成 rejected $\mathcal{Y}_l$，得到 1241 条偏好对 $\mathcal{D}_\text{DPO}$。优化标准 DPO 目标：$\mathcal{L}_\text{DPO}(\pi_\theta, \pi_\text{ref}) = -\mathbb{E}[\log \phi(\beta \log \tfrac{\pi_\theta(\mathcal{Y}_w|x)}{\pi_\text{ref}(\mathcal{Y}_w|x)} - \beta \log \tfrac{\pi_\theta(\mathcal{Y}_l|x)}{\pi_\text{ref}(\mathcal{Y}_l|x)})]$，参考模型 $\pi_\text{ref} = \mathcal{M}_\text{RFT}$。
    - 设计动机：RFT 解决了"多样性"，但 LLM-judge 和专家评测的"Top-1 比例"还比较低（11%–16%）；用 F1 这种可自动计算、与气象 claim 直接挂钩的客观信号做偏好对齐，比 RLHF 拿真人标注更便宜且更对口，DPO 后 LLM evaluation 的 Top-1 比例从 0.16/0.15 拉到 0.33/0.32，几乎翻倍。
