...
description: >-
  [ICML 2026][科学计算][天气预报] MOSAIC 用"概率扰动 + 在 HEALPix 球面网格上的 mesh-aligned 块稀疏注意力"同时解决了 ML 天气预报模型的两类频谱退化（确定性平均带来的谱衰减 + 粗化潜空间带来的高频走样）…
tags:
  - "ICML 2026"
  - "科学计算"
  - "天气预报"
...
### 整体框架
MOSAIC 的 forward 流程：(1) 把 latitude-longitude 格点的输入通过 cross-attention interpolation 转到 HEALPix mesh（NESTED 索引保证邻域连续）；(2) 在 native 分辨率上跑若干层 BSA encoder，捕获空间交互；(3) 之后可下采到更粗的 HEALPix 分辨率做 transformer 主干处理；(4) 解码回 latitude-longitude；(5) 通过 learned functional perturbation 注入随机性，得到 ensemble member；(6) autoregressive rollout 生成 10 天集合预报。这里的关键是 BSA 替代了普通 transformer 的 self-attention，使第 (2) 步在 native 0.46°–0.23° 等高分辨率上可行。

### 关键设计

1. **HEALPix 球面镶嵌 + NESTED 索引（让空间邻居=内存邻居）**:

    - 功能：解决标准 lat-lon grid 上"空间近邻在索引里很远"的问题，使后续 block-based 计算每个 block 只需一次合并内存读。
    - 核心思路：HEALPix 把球面分成 12 个等面积底层 pixel，每个递归四分；在 $N_{side}$ 分辨率下共 $12 N_{side}^2$ 个等面积 pixel，相邻 pixel 沿 Z-order 曲线占据连续索引（pixel $p$ 的四个子 pixel 索引为 $4p, 4p{+}1, 4p{+}2, 4p{+}3$）。$N_{side}\in\{32,64,128,256\}$ 对应 1.83°/0.92°/0.46°/0.23°。从 lat-lon 网格转 HEALPix 用 cross-attention 插值：对每个 HEALPix 目标点 $i$，把相对位置 $p_{ij}$ 当 query、相邻源点特征当 key/value，计算 $o_i = \sum_{j\in N_i}\mathrm{softmax}_j(q_{ij}^T k_j/\sqrt d) v_j$。
    - 设计动机：标准 lat-lon grid 在两极过采样，且邻居索引跨行非常远；一个 block 的数据加载要做大量散乱访存，GPU 利用率极低。HEALPix 的 NESTED 索引把"空间相邻"=>"索引相邻"=>"内存相邻"三件事统一，是 block-sparse 在球面上落地的几何前提。这一选择也直接决定了 BSA 的可行性。

2. **Mesh-aligned Block-Sparse Attention（BSA）**:

    - 功能：把 NSA 的"每 token 独立选 KV"升级成"每 block 共享选择"，让稀疏注意力对应到物理空间的 block-to-block 交互，从而在 native 分辨率高维网格上把复杂度压到线性。
    - 核心思路：把 $N$ 个 token 按 HEALPix NESTED 顺序切成 $m$ 个非重叠 block $\{B_1,\dots,B_m\}$，每个 block 内 token 用 pooling $\phi$ 得到块级 $\bar q_i, \bar k_j, \bar v_j$。BSA 有三条互补分支并通过 learnable gate $g_{CG},g_{FG},g_L$ 融合：(a) **压缩分支**——在块级别计算 $\bar a_{ij} = \mathrm{softmax}(\bar q_i^T \bar k_j/\sqrt{d_k})$，得到粗粒度块到块注意，并把分数广播给块内所有 token；(b) **选择分支**——每个 query block 用压缩分数选 top-$n$ key block，在被选 block 内做 full-resolution 细粒度注意，捕获长程精细交互；(c) **局部分支**——对每个 query 在滑窗内做标准 attention，专注短程细节。最终 $o_i = g_{CG}o_i^{CG} + g_{FG}o_i^{FG} + g_L o_i^L$。
    - 设计动机：NSA 在 1D 序列上有效，是因为连续索引天然对应连续语义；但球面上"连续索引"不一定连续——只有放到 HEALPix 上才成立。把"选择"操作从 token 抬到 block，等价于强约束"地理邻近的 query 看相同的远处区域"，既符合大气物理"局地天气受相似遥远区域影响"的常识，又能把选择代价摊销 $\approx B$ 倍（$B$ 是 block 大小），是几何先验和硬件友好性的同时利用。

3. **Learned Functional Perturbation（消除频谱衰减的概率成分）**:

    - 功能：通过给输入注入可学习的随机扰动得到一族集合成员，使每个成员是一个具有真实高频细节的合理实现，从而恢复频谱真值。
    - 核心思路：沿 Alet 等 2025 的思路，在输入层加上可学习的全球扰动场（learnable functional perturbation），不同 seed 给出不同成员；每个成员独立 rollout，最终集合平均的 RMSE/ACC 与单成员的频谱可以分别评估。集合成员的频谱与 ERA5 ground truth 在所有分辨率上几乎重合（论文 Fig. 2a），而 deterministic 模型则系统性低估高频。
    - 设计动机：deterministic 模型的损失是 MSE-against-期望，期望天然平滑，靠后处理无论如何调不出真实的高频能量。把"输出单点"改成"采样轨迹"，最朴素地实现了频谱保真：每个 trajectory 都是一个合理的高频实现，集合平均自然包含正确的不确定性。配合 BSA 的 native-resolution 处理，concurrent 完成"统计扰动"+"几何保真"两类修复。
