...
1. **质心构造与正交消融**:
    ...
    - 功能：用纯线性手段把"句法成分"和"语义成分"从句子向量中显式分离出来。
    - 核心思路：句法质心 $\mathbf{S}_i = \frac{1}{N_{\text{twins}}}\sum_{\alpha=0}^{N_{\text{twins}}} \mathbf{s}_i^{\alpha}$ 是同 POS 模板句子的平均，含义在 $\alpha$ 上被平均掉；语义质心 $\mathbf{T}_i = \frac{1}{N_{\text{lang}}}\sum_{\gamma} \mathbf{t}_i^{\gamma}$ 是多语翻译的平均，表层形式被平均掉。消融用沿质心方向的正交投影 $\mathbf{X}_i^{\perp \mathbf{S}_i} = \mathbf{X}_i - \frac{\mathbf{X}_i\cdot \mathbf{S}_i}{|\mathbf{S}_i|^2}\mathbf{S}_i$，而不是直接减去 $\mathbf{S}_i$。
    - 设计动机：直接相减会把质心里夹带的其他句子成分一起带走，从而虚高消融强度；只投影一次保证"只把 $\mathbf{X}_i$ 中与 $\mathbf{S}_i$ 方向共线的部分清零"，并通过对照实验（"打乱的质心"）证明效果是定向的而非平均水平的副作用。

2. **基于邻域排名的相似度**:

    - 功能：在高维空间里稳健地衡量两份表示是否承载同一类信息，绕开 CKA 在高维下信号偏弱的问题。
    - 核心思路：对两套表示 $A,B$，先在 $A$ 里找点 $i$ 的最近邻 $j$，记下点 $j$ 在 $B$ 里相对 $i$ 的距离排名 $r_{ij}^{B}$，反向再来一遍，最后取归一化平均：$\text{Similarity}=1-\frac{1}{N_s^2}\big(\sum_{i,j:r_{ij}^{A}=1} r_{ij}^{B} + \sum_{i,j:r_{ij}^{B}=1} r_{ij}^{A}\big)$。该量在全局平移/旋转/缩放下不变，1 表示最近邻完全一致、0 表示独立。
    - 设计动机：与 Information Imbalance（Glielmo 2022）和 Neighborhood Overlap（Huh 2024）同源。它只依赖几何关系而非线性映射，所以即使表示空间被各种 normalization 拉变形，结论依然鲁棒；这是支撑"消融多少 → 相似度掉多少"这种因果叙事的关键。

3. **跨层 × 两类消融的交叉实验矩阵**:

    - 功能：把"句法/语义编码地图"做成一个 layer × (target, ablated) 的二维结果，从而直接读出可分离性。
    - 核心思路：固定一种相似度（句法双胞胎相似度 or paraphrase 相似度），系统改变被消融的方向（无消融 / 正确质心 / 随机打乱质心 / 跨类质心）。例如测"句法相似度被语义质心消融的影响"和"语义相似度被句法质心消融的影响"两条曲线，再附上 token 聚合方式（concat vs. mean）的对比。最后用两个下游 probe（线性 POS 分类 + paraphrase recall@3）做行为级佐证。
    - 设计动机：单方向消融只能说明"质心抓住了相关信息"，唯有交叉消融才能定量回答"句法和语义是否解耦"。同时引入随机质心和跨类质心两个 control，把"线性维度被任意挪一下就会掉"的平凡解释排除掉，留下真正具有方向性的因果证据。
