...
flowchart TD
    A["隐态 z_t（落在弥散分布上<br/>top-2 token 概率接近，blurry point）"] --> B["so(n) 值 1-形式<br/>top-2 嵌入构三重外积 A(z_t)，缩并切向量 μ"]
    B --> C["概率荷 + 平行输运<br/>系数 4p₁p₂ 加权，沿轨迹积分成累积旋转 U"]
    C --> D["全息测量（clover 闭合环路）<br/>提取与路径无关的局部曲率"]
    P["世界向量探针（脚手架）<br/>冻结隐态上训 737 个线性分类器"] --> E
    D --> E["几何旋转 vs 世界向量<br/>比对方向余弦"]
    E --> F["结论：耦合 |cos|>0.5（顶级棋子>0.7）<br/>采样误差非纯混沌，反映世界结构"]
```
