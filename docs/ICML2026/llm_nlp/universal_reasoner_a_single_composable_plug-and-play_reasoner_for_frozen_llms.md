...
description: >-
  [ICML 2026][LLM 其他][推理增强] 提出通用推理器（UniR）——通过训练独立的轻量推理模块来捕获奖励导向的推理行为，在推理时通过逻辑叠加与冻结 LLM 组合，实现无需微调冻结模型、跨模型大小转移和多任务可组合的推理增强。 领域现状：当前通过 RL 微调（RFT）增强 LLM 推理能力，但需大量计算和内存资源…
tags:
  - "ICML 2026"
  - "LLM 其他"
  - "推理增强"
