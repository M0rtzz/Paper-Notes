...
实验框架围绕「专家分布 × 信息条件」两个轴展开。专家分布有「集中型」（一人独握 ground-truth 信息）和「分布式」（信息互斥地分给多人）两种；信息条件有 No Information（控制）/ Expert Not Mentioned（专家存在但身份隐藏）/ Reveal Expert（明示谁是专家）/ Best Individual（专家单独作答）四种。所有实验默认 4 agent × 4 轮讨论，最终答案取讨论后多数投票。然后用「相对协同 gap」公式 $(\max_t f(\{a_t\}) - f(\{a_1,\dots,a_T\})) / \max_t f(\{a_t\})$ 跨任务标准化。

### 关键设计

1. **协同 gap 的「识别 / 利用」分解**:

    - 功能：把「团队为什么打不过最强成员」分解为两个独立子问题——团队认不出专家 vs 认出了却用不上。
    - 核心思路：定义 Identification Gap = $f(\text{Expert Not Mentioned}) - f(\text{Reveal Expert})$；Leveraging Gap = $f(\text{Reveal Expert}) - f(\text{Best Individual})$。如果 Reveal 后团队几乎不进步、与单独的专家仍差很多，说明瓶颈在利用而非识别。同时用 GEPA 自动优化 Reveal Expert 的提示词，把「提示工程没做好」从混杂里剥离。
    - 设计动机：以往多智能体工作只比较「团队 vs 平均」（弱协同），看不出 LLM 团队和人类团队在专家利用上的本质差异。把 gap 显式分解后才能定位干预点。

2. **对话动力学的四类编码 + 相关分析**:

    - 功能：把团队「为什么」失败从行为而非结果层面解释。
    - 核心思路：借哲学权威论（preemption thesis）和谈判理论，给对话轮分四类标签——非专家的「认知让渡 ED（直接接受专家观点）」与「整合妥协 IC（提中间立场）」；专家的「战略坚持 SP（不让步）」与「认知灵活 EF（迎合非专家）」。用 Gemini 3.0 Pro 自动标注，由两名人类校验 50 段对话达成 94% 一致；再做每类行为频次与协同 gap 的 Pearson 相关。
    - 设计动机：以前对多智能体协作的批评只看输出，这是第一次定量地揭示「过度妥协」是 LLM 团队失败的内生机制，而非偶然现象。

3. **团队规模 × 对抗成员双消融**:

    - 功能：检验「共识倾向」是否同时既稀释专家又过滤对抗。
    - 核心思路：固定 4 轮讨论，把团队规模设为 2/4/8，看协同 gap 是否随规模显著上升（结论：所有任务都正相关，$p < 0.05$）；再插入一名「故意给最差排序、被指示破坏团队」的对抗成员，看团队是否被带偏（结论：几乎不受影响）。两者在同一机制下得到一致解释——共识平均既稀释专家也稀释对抗。
    - 设计动机：把「失败」和「鲁棒」放在同一组实验里证伪「失败必然伴随脆弱」，反而显示「鲁棒是失败的副产品」，这对未来对齐范式有直接含义。

### 评估策略

对人类心理学任务用 L1 排序误差；对 ML 基准用 At Least One Correct（ALOC）上界：若团队能在每题都让答对的成员说了算，能达到的准确率。把团队相对 ALOC 的 gap 当作「假如完美利用专家能再涨多少」的硬性度量。
