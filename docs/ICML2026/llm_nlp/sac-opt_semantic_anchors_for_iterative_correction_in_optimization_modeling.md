...
### 整体框架
SAC-Opt 把 LLM 优化建模整体拆成 6 个阶段：

1. **Structured Data Extraction**：`extract` agent 把自然语言问题 $P$ 抽成 JSON 结构 $S = (\mathcal{P}, \mathcal{V}, \mathcal{C}, \mathcal{O})$
2. **Structured Data → Model Translation**：拆成简单部分 $S_{\mathrm{simp}} = \{\mathcal{P}, \mathcal{V}\}$（模板渲染）和语义部分 $S_{\mathrm{sem}} = \{\mathcal{C}, \mathcal{O}\}$（`trans` agent 生成），得到初始代码 $\mathcal{M}^{(0)} = \mathcal{M}_{\mathrm{simp}} + \mathcal{M}_{\mathrm{sem}}^{(0)}$
3. **Model → Structured Data Reconstruction**：`recons` agent 把 $\mathcal{M}_{\mathrm{sem}}^{(t)}$ 反向翻译回结构化锚点 $\widehat{S}_{\mathrm{sem}}^{(t)}$
4. **Semantic Consistency Verification**：用 $\delta(s_i, \widehat{s}_i)$ 逐条比对，找出错误集 $\mathcal{E}^{(t)} = \{s_i : \delta=0\}$
5. **Iterative Correction**：只重写 $\mathcal{E}^{(t)}$ 里的那几条，回到第 3 步继续验证，直到 $\mathcal{E} = \emptyset$ 或达到 $T_{\max} = 5$
6. **Model Debugging**：拼装完整代码送进 Gurobi 跑，若有执行错则按 Reflexion 式根据报错修代码（最多 3 次）

整条 pipeline 关键在于把"语义级修正"（步骤 3-5）和"语法级修正"（步骤 6）解耦：前者由结构化锚点驱动，后者由 solver 报错驱动，各管一段。

### 关键设计

1. **Structured Data 的简单/语义分离与模板化渲染**:

    - 功能：把 $S$ 切成 $S_{\mathrm{simp}} = \{\mathcal{P}, \mathcal{V}\}$ 和 $S_{\mathrm{sem}} = \{\mathcal{C}, \mathcal{O}\}$ 两摊，前者用确定性模板 $f_{\mathrm{det}}^{\mathrm{trans}}$ 渲染（如 `RollWidth = data["RollWidth"]`），后者交给 `trans` agent 用 LLM 生成。
    - 核心思路：参数和变量声明几乎是"格式化输出"，不需要语言理解，直接走模板能彻底消除随机性、把错误面缩小到只剩约束和目标。约束和目标才是真正承载业务逻辑、需要 LLM 推理的部分，所以把修正算力全集中到 $S_{\mathrm{sem}}$ 上。
    - 设计动机：这一刀切下去之后，后续的 reconstruction 和 verification 都只针对 $\mathcal{M}_{\mathrm{sem}}$，状态空间大幅收缩；同时也避免了"修一条约束顺手把变量声明改错"的连锁污染。

2. **Model→Structured Data Reconstruction：语义反向重建**:

    - 功能：用一个独立的 `recons` agent 把生成出来的 solver 代码 $\mathcal{M}_{\mathrm{sem}}$ 翻译回 **与原始锚点同 schema** 的结构化描述 $\widehat{S}_{\mathrm{sem}} = f_{\mathrm{agent}}^{\mathrm{recons}}(\mathcal{M}_{\mathrm{sem}})$。
    - 核心思路：相当于强迫一个"读代码的人"用问题原始的语言重新表述这段代码究竟做了什么。如果实现是错的（比如把 $\le$ 写成 $\ge$），$\widehat{s}_i$ 自然会和 $s_i$ 不一样；如果实现是对的，两者应当语义等价。这一步是整个反向闭环的"信号源"。
    - 设计动机：传统方法只能拿 solver 报错当反馈，本质上等于"看症状猜病"；这里相当于直接让模型"自述病情"，在没有 ground truth 解的前提下，提供了一个可以逐条比对的代理监督信号。

3. **锚点级一致性验证 + 仅修 mismatch 的迭代收敛**:

    - 功能：定义二值一致性函数 $\delta(s_i, \widehat{s}_i) \in \{0,1\}$，提供两种实现：LLM-based（`verif` agent 直接判等价）和 Similarity-based（SentenceTransformer 算 cosine，阈值 $\tau=0.75$）；每轮算出错误集 $\mathcal{E}^{(t)}$，**只对 $\mathcal{E}^{(t)}$ 里的锚点** 调用 $\mathcal{M}_{\mathrm{sem}}^{(t+1)}[s_i] \leftarrow f_{\mathrm{agent}}^{\mathrm{trans}}(s_i)$ 重写。
    - 核心思路：用 $|\mathcal{E}^{(t)}|$ 的单调下降来定义收敛——既然每轮要么消除一些不一致、要么至少不变，循环至多 $T_{\max}=5$ 轮就会停。两种 verifier 互为 fallback：LLM verifier 准但贵，相似度 verifier 糙但能离线跑，作者明确两者是 "alternative implementations"，不是融合也不是 oracle。
    - 设计动机：细粒度增量修正避免了"改 A 弄坏 B"的全局回归——这是 LLM 全文重写的典型病；同时把验证-修正绑定到同一个 schema，让循环既可定义、可观测、可终止。
