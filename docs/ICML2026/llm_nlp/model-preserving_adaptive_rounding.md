...
YAQA 把一层权重的量化看成「在真 Hessian 定义的椭球里找一个最优整点」，但真 Hessian $\nabla^2 L(W^*)$ 大到 $mn \times mn$ 没法直接用，于是先用 Kronecker 草图 $\tilde{H} = H_O \otimes H_I$ 逼近它，再在这个草图上跑一遍推广的 LDLQ 舍入。算法因此拆成两步：先用幂迭代把 $H_O, H_I$ 算出来（让 $\tilde{H}$ 在方向上贴近真 Hessian），再用带输入端 + 输出端双向反馈的定点迭代把权重逐个吸附到码本。每个线性层独立做一次，不改推理结构，量化模型的速度只由码本（如 E8P）决定，跟 YAQA 无关。

### 关键设计

**1. 结构性幂零度 SND：把「能 tractable 跑 LDLQ」的 Hessian 结构挑出来**
...
有了 tractable 结构还不够，得知道哪种 $H_O, H_I$ 真能压住模型输出的 KL。定理 3.4 把端到端误差上界写成草图与真 Hessian 的几何关系：$\mathrm{vec}(\Delta)\, H\, \mathrm{vec}(\Delta)^\top \le \|H\|_F\,(\|\Delta\|_F^2 \sqrt{2-2c} + \text{incoherence/trace 项})$，其中 $c = \langle H,\, H_O \otimes H_I\rangle / (\|H\|_F \|H_O\|_F \|H_I\|_F)$ 正是草图与真 Hessian 的余弦相似度，$\Delta = W^* - W$ 是舍入误差。读出来的信息很直接——草图在方向上越贴合真 Hessian（$c$ 越接近 1），KL 上界越紧，外加 $H_O, H_I$ 要低 incoherence、低秩。这是量化算法第一次拿到端到端误差界，把「Hessian 草图怎么选」从经验拼凑升级成「最大化余弦相似度」的明确数学问题，也直接指向用幂迭代去逼近。

**3. 两档幂迭代 Hessian 草图：在 LLM 规模上把 $H_O, H_I$ 算出来**

真 Hessian 不能直接 Monte-Carlo 估（$mn \times mn$ 维度方差爆炸），所以作者给两档可扩展方案，对应「便宜」和「最优」两种取舍。Sketch A 假设序列内 token 独立，把 $H \approx \mathbb{E}[x^\top x \otimes (\nabla_y \ell)^\top (\nabla_y \ell)]$，从 $(H_I)_0 = H_1,\ (H_O)_0 = I$ 起步幂迭代 3 步左右就收敛，用 bias 换 variance、数据少时稳，10B 模型约 20 GPU-hour。Sketch B 直接在真 Fisher 上跑一轮幂迭代（从 $I, I$ 起步），按 sequence 算梯度但只过一遍数据，方差容忍度更高、数据多时质量更好，10B 模型约 30 GPU-hour。两者都借 modified backward pass 做分布式幂迭代，思路类似 Shampoo 的预条件，但关键差别是用真 Fisher（Monte-Carlo 采 logits）而非经验 Fisher——对 KL 目标这一步不能省，否则草图方向会偏。

### 损失函数 / 训练策略

YAQA 是纯 PTQ，没有显式训练损失。它隐式优化的是 Kronecker 草图下的代理目标 $\mathrm{tr}(\Delta^\top H_O \Delta H_I)$，由带双向反馈的定点迭代（Equation 5/6）最小化，量化输出可写成 $W = Q(W^* + L_O'^{\top}\Delta L_I' + L_O'^{\top}\Delta + \Delta L_I')$，其中 $L_O', L_I'$ 是 $H_O, H_I$ 的 LDL 三角因子去掉单位阵的部分。标量或向量量化器都能用，并与 QuIP# 的 randomized Hadamard 变换互补——后者让 $W$ 近高斯、降 incoherence，YAQA 负责把 Hessian 算准。
