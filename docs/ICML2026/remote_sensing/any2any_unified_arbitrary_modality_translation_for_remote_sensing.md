...
1. **RST-1M 支撑的潜空间锚点**:
	...
	- 功能：把跨模态翻译从“不知道哪个输出才对应源图像”的模糊生成问题，转成有空间配准监督的目标潜变量回归问题。
	- 核心思路：RST-1M 汇集 SEN1-2、SEN12MS、CACo、SpaceNet-3、SpaceNet-5 等公开数据，覆盖 RGB、SAR、NIR、MS、PAN 五种模态和七类跨模态配对。对某个源图像 $x_i$，其空间对齐的目标图像 $x_j$ 通过目标编码器得到 $z_j=E_j(x_j)$，论文把它视为目标分布中的确定性锚点。训练目标不再只要求生成结果“像某个目标域”，而是要求预测结果贴近这个具体地理场景的目标潜表示。
	- 设计动机：遥感翻译的关键不是风格迁移，而是保持地物边界、道路、水体、建筑等语义结构的一致性。锚点监督可以减少跨模态映射的随机性，使共享骨干学到更稳定的地理语义。

2. **模态专属 VAE + 共享 DiT 语义映射**:

	- 功能：把物理形态差异很大的传感器输入变成统一尺寸的潜变量，并用一个共享骨干处理所有翻译方向。
	- 核心思路：每个模态都有独立的 VAE 编码器和解码器，负责处理自己的通道数、分辨率和成像统计。论文中各模态会被编码到 $4 \times 64 \times 64$ 的潜表示。VAE 训练使用重建损失、感知损失和 KL 正则的组合。随后冻结 VAE，用 DiT 作为共享语义骨干。DiT 的输入是 $[z_t,z_i]$，条件向量由时间步嵌入、源模态嵌入和目标模态嵌入相加后经过 MLP 得到，用来调制 AdaLN。
	- 设计动机：模态专属 VAE 负责吸收低层物理差异，共享 DiT 负责学习高层地理语义转换。这个拆分避免把所有传感器差异都塞进一个像素级生成器，也让不同方向之间可以共享语义映射能力。

3. **$x_0$ 预测与目标模态残差适配器**:

	- 功能：提升跨传感器生成的结构稳定性，并修正共享骨干输出与目标解码器潜空间之间的残余错位。
	- 核心思路：标准扩散模型常预测噪声残差，但论文认为在遥感跨模态翻译里，传感器差异过大时噪声预测会让结构边界更不稳定。因此 Any2Any 让 DiT 直接预测干净目标潜变量 $\hat{z}_j=f_\theta([z_t,z_i],c)$。接着，目标模态适配器执行 $z'_j=\hat{z}_j+A_j(\hat{z}_j)$。适配器是紧凑卷积网络，最后一层零初始化，训练时通过 stop-gradient 限制校准损失不要反向扰动共享骨干。
	- 设计动机：共享骨干需要保持跨方向泛化，不能被某个目标模态的局部分布牵着走；目标适配器则只做小幅、目标相关的潜空间修正。这样既保留统一模型的可扩展性，又避免目标模态细节被平均化。
