...
1. **完全去除类别词的 Prompt**:
    ...
    - 功能：消除语义先验 channel，让 visual support 成为唯一目标指示器
    - 核心思路：prompt 只描述任务（"locate the same object across frames"）和输出格式；不含任何类别名（"dog""bowl"等）；支持图配 bbox，query 图无 bbox 等模型预测
    - 设计动机：IPLoc 等用 pseudo-label 训练但推理仍用真实类别——train-test mismatch；本文 train 和 test 都无类别名，从根本上断开 semantic shortcut

2. **Bounding Box Attention Optimization**:

    - 功能：显式监督 query image token 的注意力分布，强制它关注 bbox token 和 query 图
    - 核心思路：聚合各层各头的 attention $A = \tfrac{1}{LH}\sum_{\ell, h} A^{(\ell, h)}$；取 query image token 对应的行 $P \in \mathbb{R}^{N \times T}$；用 binary mask $m$ 标记 bbox token 位置；attention loss 鼓励 $P$ 对 mask=1 的位置（bbox token）attention 高、对其他位置 attention 低
    - 设计动机：论文 §3 证据显示——即使去类别词，vanilla 模型 attention 仍 diffuse 不集中在 support；必须显式监督 attention 才能让模型真正"看 support 的几何"；这是 attention 工程而非架构创新，但效果显著

3. **GRPO + IoU Reward 精修**:

    - 功能：在 SFT 收敛后用 RL 直接优化定位误差（IoU）
    - 核心思路：GRPO（Group Relative Policy Optimization）对每个 prompt 采样若干 rollout（bbox 输出），按 IoU 排序得 group advantage；reward = IoU(predicted, ground-truth)；不需要 critic
    - 设计动机：SFT loss 是 token-level cross-entropy，与 bbox 几何对齐间接相关；GRPO 直接优化 IoU 是任务对齐的 RL；GRPO 比 PPO 在结构化输出（bbox 坐标）上更稳
