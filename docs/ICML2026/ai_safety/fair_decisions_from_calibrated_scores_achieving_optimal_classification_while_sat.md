...
### 整体框架
输入：每个群体 $a\in\{0,1\}$ 的有限分数取值 $\{s_i^a\}$ 与群体内分布 $P(s_i^a\mid A=a)$、群体比例 $P(A)$、目标函数 $\mathcal{F}$（损失或 $\Delta_{\mathrm{sep}}$）。输出：满足 sufficiency 的最优（可能随机化）二元决策规则 $P(R=1\mid S,A)$。流程分三层：(1) 在单群体内刻画可行域 $\mathcal{C}^a$ 的几何形状；(2) 对两个群体取交集 $\mathcal{C}^0\cap\mathcal{C}^1$，刻画其非递减边界；(3) 沿边界追踪并在每段闭式求解目标函数。

### 关键设计

1. **$(\mathrm{PPV},\mathrm{FOR})$ 可行域的几何刻画与软阈值最优规则**：

    - 功能：对任意有限 calibrated 分数 $S$，给出所有可实现 $(p,q)=(\mathrm{PPV}(R),\mathrm{FOR}(R))$ 的精确闭式描述，并构造取边界点的最简规则。
    - 核心思路：把分数按降序排列 $s_1>\dots>s_m$，引入选择率 $\mu=P(R=1)$。由 calibration 与 Markov 链 $Y\!\leftrightarrow\! S\!\leftrightarrow\! R$ 推出基础等式 $\pi=\mu p+(1-\mu)q$，几何上等价于"固定 $\mu$ 时所有 $(p,q)$ 落在过 $(\pi,\pi)$、斜率 $-\mu/(1-\mu)$ 的同一条直线上"。固定 $\mu$ 后最大化 $p$ 是分数为系数的线性规划——退化为 Dantzig (1957) 的**分数背包贪心解**：选最大几个分数 bin，最后一个 bin 用随机化补齐，即软阈值规则 $P(R^\*=1\mid s_i)$ 仅在某个 $k^\*(\mu)$ 处取分数值。代入 (4) 得到边界的分段闭式 $p^\*(\mu)=s_k+c_k/\mu$，其中 $c_k=\sum_{i<k}P(s_i)(s_i-s_k)$。整条边界 $\partial\mathcal{C}$ 是 $m-1$ 段超曲线弧加两条直边，与 $\mu_k=\sum_{i\le k}P(s_i)$ 对应的硬阈值正好是各弧的端点。
    - 设计动机：把"对所有可能的随机化决策规则"这种无穷维的搜索压缩为"沿一条二维曲线的一维参数化"，同时星凸性 $\mathcal{C}$（关于中心 $(\pi,\pi)$）让任何内部点都可由边界点与 $(\pi,\pi)$ 的凸组合复现，因此**只需追踪边界**。

2. **多群体 sufficiency = 子群可行域交集的非递减边界**：

    - 功能：把 sufficiency 约束 $p^0=p^1=p,\,q^0=q^1=q$ 直接翻译为几何条件 $(p,q)\in\mathcal{C}^0\cap\mathcal{C}^1$，并给出交集边界的闭式。
    - 核心思路：每个 $\mathcal{C}^a$ 的非平凡边界由非递减函数 $q^a(p)$ 描述（公式 (7)）。交集的非平凡边界由 **逐点取大** $q(p)=\max\{q^0(p),q^1(p)\}$ 给出，仍是非递减曲线。把 $p$ 轴上由两个群体的 breakpoints $\{p_k^0\}$、$\{p_l^1\}$ 共同诱导的细分区间记为 $J_{k,l}$，在每段上 $q^0,q^1$ 都是给定形式的有理函数；它们的差 $q^0(p)-q^1(p)$ 化为一个二次函数 $\Phi_{k,l}(p)$ 的符号，至多在每段内有 2 个交点，因此 active boundary 只会切换有限多次。
    - 设计动机：这一刻画揭示了一个关键现象——交集边界上的点**通常不对应任何群体的硬阈值规则**，至少一个群体必须用随机化决策（图 2 给出反例：连任一群体的 breakpoint 都不被边界经过）。这解释了"为什么仅用群体相关的硬阈值搞不出最优 sufficient 分类器"，并指明随机化是必需的。

3. **Algorithm 1：通用 boundary trace + 闭式目标优化**：

    - 功能：沿 $\partial(\mathcal{C}^0\cap\mathcal{C}^1)$ 顺序遍历所有 $J_{k,l,i}$ 子段（按 $\Phi_{k,l}$ 的根再切分），在每段上调用目标函数 $\mathcal{F}$ 求该段最优，最后取全局最优。
    - 核心思路：维护当前 active group $a\in\{0,1\}$ 与索引 $(k,l)$，每段开销 $O(1)$；总段数 $\le m^0+m^1$ 量级。两个目标都可在每段内闭式：(i) **损失最小化**——把 $\mu=(\pi-q)/(p-q)$ 代回期望损失得到 $L=\pi\ell_{01}+\frac{\pi-q}{p-q}\bigl(\ell_{10}-p(\ell_{01}+\ell_{10})\bigr)$，对 $p$ 求一阶条件归结为解二次方程；(ii) **$\Delta_{\mathrm{sep}}$ 最小化**——TV 距离展开后 $\Delta_{\mathrm{sep}}(R)=K\bigl(\frac{1-\mu}{p-\pi}-\frac{\mu(p-\pi)}{\pi(1-\pi)}\bigr)$，对 $p$ 求导非正，最小点同样落在边界且每段内闭式。
    - 设计动机：用**同一份几何骨架**统一两类常被对立讨论的目标（accuracy vs. equalized odds），让从业者可以**显式对比**两个最优解、按应用挑一个，而不是被某种事先确定的损失函数绑死。

### 损失函数 / 训练策略
方法是纯后处理：不需要重新训练打分模型，不需要原始特征，只需要 group-calibrated 分数和群体标签即可输出决策规则 $P(R=1\mid S,A)$。当真实场景中 calibration 只是近似（如用 out-of-fold 预测做分箱标定）时，作者在附录 J 给出鲁棒性界，并在 ACS Income 上验证 predictive parity 违反量级很小（PPV gap $4.8\times 10^{-3}$）。
