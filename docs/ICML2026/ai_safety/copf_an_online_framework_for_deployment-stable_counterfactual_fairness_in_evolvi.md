...
COPF 是一个**决策层框架**（decision-layer），不改 backbone $\hat p_t(u_t,v)$ 的内部结构，而是在打分和最终曝光之间插入一层：

```
Stream (u_t, v_t*) → 候选集 C_t = {v_t*} ∪ N_t
                  → backbone 打分 p̂_t(u_t,v)
                  → COPF logit 调整 p̃_t = σ(logit(p̂_t) + b_{s,b} + δ_s)   [Eq.1]
                  → TopK-Stochastic 采样曝光 D_t（同时记录 propensity ê_t）
                  → 观察 Y_t（banditized：仅曝光的候选才有反馈）
                  → 更新图/backbone/nuisance/GA-DR buffer
                  → 在 audit checkpoint：更新 multicalibration offset b_{s,b}
                  → 在 control checkpoint：更新 PI dual (λ_TE, λ_Cal, λ_Min) → δ_s
```

整个运行流程对应 Algorithm 1（OPP Runner），分 Pre / Deploy / Post 三阶段，Pre 用大探索率 $\epsilon=0.20$ 暖机，Deploy/Post 切换到 $\epsilon=0.02$ 触发 performative shift。

### 关键设计

1. **OPP 在线预测式协议（Online Prequential Protocol）—— 让反事实可识别**:

    - 功能：把"反事实曝光效应"从一条单一流中变得 identifiable，是后续 DR 估计与公平审计的前置条件。
    - 核心思路：5 条 OPP 规则——OPP-0 时间顺序不泄漏；OPP-1 候选集 $C_t$ 用 pre-decision 图 $G_{\le t}$ 在线构造；OPP-2 用 **混合策略**（uniform exploration 概率 $\epsilon$ + Plackett–Luce Top-K score 采样）保证每个候选的边际曝光概率 $e_t(v) \ge \epsilon K/|C_t| > 0$，同时记录 propensity（uniform 部分解析、PL 部分用 128 次 Monte Carlo 估计后裁剪）；OPP-3 在线 cross-fit 更新 nuisance 与 backbone；OPP-4 在 rolling window 上做 prequential audit + control。Assumption 3.1 形式化 overlap、local ignorability、bounded local interference + temporal $\beta$-mixing 三条件。
    - 设计动机：传统反事实公平方法（Kusner et al. 2017、Coston et al. 2020）依赖批量日志的 overlap，部署到 performative 在线场景就崩。这里通过 *policy-enforced* 探索而不是 *clipping-enforced* overlap，让 identifiability 真正成立——clipping 仅吸收数值误差进 $\varepsilon_e$ nuisance 项（Lemma 4.2）。

2. **GA-DR 残差 + Residual-OI 审计 + 质量归一化证书**:

    - 功能：在演进图的时间依赖 + 局部干扰下，给出对反事实公平差距的**带证书的在线估计**。
    - 核心思路：先定义 graph-aware self-normalized doubly robust pseudo-outcome $\tilde\Gamma_t^{(a)}(u_t,v) = \hat\mu_{a,t}(W_t) + \frac{\mathbf{1}\{D_t(v)=a\}}{\hat e_t^{(a)}(v)}(Y_t-\hat\mu_{a,t}(W_t))$，配上 graph-aware 时间衰减权重 $w_t$ 做窗口平均 $\widehat{\mathbb{E}}_{\mathrm{GA},\mathcal W}$。然后构造两个残差序列：$\hat r^{(0)}=\tilde\Gamma^{(0)}-\hat p$（用于校准）、$\hat r^{(\Delta)}=(\tilde\Gamma^{(1)}-\tilde\Gamma^{(0)})-\tau(W_t)$（用于 TE parity）。Residual-OI 在 auditor 族 $\mathcal H$ 上计算 $\widehat{\mathrm{OI}}_\mathcal{W}(r;\mathcal H)=\sup_{h\in\mathcal H}|\widehat{\mathbb{E}}_{\mathrm{GA},\mathcal W}[h_t r_t]|$，$\mathcal H$ 实例化为 group × score-bucket × 可选 structural-role 的 slice 指示函数。Lemma 4.1 把校准/TE/Min 三种公平差距都"线性化"成残差的相关项，再除以最小 slice 质量 $p_\min^{\mathrm{gb}}$ 即得 finite-sample 证书：$g_{\max}^{\mathrm{Cal}}\le (\varepsilon_0+\beta_0)/p_\min^{\mathrm{gb}}$，$g_{\mathrm{gap}}^{\mathrm{TE}}\le 2(\varepsilon_\Delta+\beta_\Delta)/p_\min^{\mathrm{g}}$。Theorem 4.5 + Corollary 4.6 给出 noisy transfer：插件残差 OI 小 ⟹ 真实反事实差距小，up to nuisance 项 $C_1\varepsilon_e\varepsilon_\mu + C_2(\varepsilon_e^2+\varepsilon_\mu^2)$ 和 mixing 项 $C_3\sqrt{\tau_{\mathrm{mix}}\log|\mathcal H|/W_{\mathrm{eff}}}$。
    - 设计动机：朴素 IPW 在低 propensity 下方差爆炸、$\mu$-only 估计器在模型误设下有偏，DR 给"双重保险"；self-normalization + GA 权重把时间衰减与图局部结构（如 $k$-邻居子采样）直接编码进估计，否则 mixing 项会污染 transfer bound；用 slice mass 归一化让"低质量小群组"的证书自动放松，避免对极稀疏组发出过紧的虚假承诺。

3. **决策层双控制器：多重校准 offset + PI primal–dual 群偏置**:

    - 功能：把审计出来的差距实际"打回去"——通过 Eq.(1) 的两个加性 logit 调整 $b_{s,b}$（细粒度校准）和 $\delta_s$（粗粒度 TE/Min 控制），不重训 backbone。
    - 核心思路：(i) **多重校准更新**：每轮在 DR residual buffer 上估计每个 slice 的 $\widehat{\mathbb{E}}[r^{(0)}\mid A=s,\hat p\in I]$，选 budget $B_{\mathrm{act}}$ 个最违规 slice，把 offset $b_{s,b}$ 做 clipped gradient step。(ii) **PI primal–dual 控制器**：在 $L_{\mathrm{win}}$ 窗口上计算 $(g_{\mathrm{gap}}^{\mathrm{TE}}, g_{\max}^{\mathrm{Cal}}, g^{\mathrm{Min}})$，对每个约束维护 dual $\lambda_\cdot$，由 soft violation $v_{\mathrm{TE}}=[g_{\mathrm{gap}}^{\mathrm{TE}}-\rho_{\mathrm{TE}}]_+$ 推动；group bias 由 Eq.(2) 给出：$\delta_s = \mathrm{clip}\big(\alpha\lambda_{\mathrm{TE}}(\bar\tau-\hat\tau_s) + \alpha'\lambda_{\mathrm{Min}}[\tau_{\min}-\hat\tau_s]_+\big)$，其中 $\bar\tau$ 是跨组平均效果，$\hat\tau_s$ 来自 GA-DR pseudo-outcome。Cal 维度 $\lambda_{\mathrm{Cal}}$ 用 hierarchical gating（先 TE/Min 达标再收紧 cal），默认关闭。
    - 设计动机：单用 multicalibration 只压 $r^{(0)}$ 校准，压不住 TE parity；单用 dual 控制器又会忽略 slice-level 校准。两者解耦让粒度分层——$b_{s,b}$ 管"score bucket 内的局部修正"，$\delta_s$ 管"全组级别的曝光重分配"。**关键安全设计**：minimum-effect guardrail $g^{\mathrm{Min}}$ 阻止"用降曝光达成 parity"（fairness-through-rationing）这种退化解；Pre 阶段只做 warm-up，offset 在 Deploy/Post 才生效。

### 损失函数 / 训练策略
- **目标**：在线约束优化——最大化 ranking utility（MRR / Hits@10 / NDCG@10 / DeployHit@TopK），约束 $g_{\mathrm{gap}}^{\mathrm{TE}}\le \rho_{\mathrm{TE}}$、$g^{\mathrm{Min}}\le \rho_{\mathrm{Min}}$、可选 $g_{\max}^{\mathrm{Cal}}\le \rho_{\mathrm{Cal}}$。
- **优化**：dual 用 PI 更新（比例 + 积分），primal 用 logit offset；nuisance $\hat\mu_a$ 在线 cross-fit。
- **复杂度**（Remark 4.7）：增量邻域统计 + $k$-邻居子采样 + $B_{\mathrm{act}}$ 个 active auditor，每轮摊销 $O(|C_t|dL + |C_t|k + B_{\mathrm{act}})$。
