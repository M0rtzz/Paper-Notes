...
### 整体框架
TRIAD 是一个端到端编解码框架，输入是 $H \times 2H$ 的 ERP 全景图 $x$ 和 $k=32$ 比特水印 $w \in \{0,1\}^k$，输出是视觉上几乎不可察觉的水印图 $\tilde{x}$；解码端无需对齐，直接在 SH 域抽出 bispectral 不变标量即可恢复 $\hat{w}$。

整个 pipeline 分四步：(1) **球谐变换 SHT**：把 $x$ 升到 SH 系数 $c = \{c_l\}_{l=0}^{l_{\max}}$，默认 $l_{\max}=16$；(2) **等变嵌入**：用基于 e3nn 的 SO(3)-等变 backbone $\Phi_{eq}$ 把 $c$ 映射到选定的高阶子空间 $\mathcal{V}_{embed} = \mathcal{V}_6 \oplus \mathcal{V}_8 \oplus \mathcal{V}_{14}$，再通过参数化张量积 $\text{TP}_{\vartheta_1}(u, w)$ 把水印融入这些等变特征，逆 SHT 回空域得到残差 $\Delta x$，经感知掩码 $M_{perc}$ 与几何掩码 $M_{geo}$ 调制后叠加到原图：$\tilde{x} = x + M_{perc}(x) \odot M_{geo} \odot \Delta x$；(3) **解码端再次 SHT** 并投影到同样的 $\mathcal{V}_{embed}$ 得到 $\hat{u}$；(4) **三阶不变投影**：用两步张量积 $h = \text{TP}_{\vartheta_2}(\hat{u}, \hat{u})$, $z = \text{TP}_{\vartheta_3}(h, \hat{u})|_{\mathcal{V}_0}$ 算出可学习的 bispectrum 标量，再过轻量 MLP 解出 $\hat{w}$。

### 关键设计

1. **三阶 SH 张量积构造可证不变的双谱标量 (Spherical Bispectral Invariant)**:

    - 功能：把藏在高阶（rotation-sensitive）SH 系数里的水印信息抽成一个对任意 SO(3) 旋转**严格不变**的标量信号。
    - 核心思路：对三个 SH 不可约表示 $\mathcal{V}_{l_1} \otimes \mathcal{V}_{l_2} \otimes \mathcal{V}_{l_3} = \bigoplus_l \mathcal{V}_l$ 做张量积分解，挑出 $l=0$ 的 trivial 分量，得到 $I = \sum_{l_i, m_i} C^{0,0}_{l_1 m_1\, l_2 m_2\, l_3 m_3}\, c_{l_1}^{m_1} c_{l_2}^{m_2} c_{l_3}^{m_3}$，其中 $C^{0,0}_{\dots}$ 是由 Wigner 3-j 符号导出的 Clebsch-Gordan 耦合系数。Theorem 4.1 证明该标量满足 $I(R \cdot f) = I(f)$ 且对高阶系数的扰动产生非零响应（即可恢复嵌入信息），证明在附录 A.1。
    - 设计动机：零阶 $c_0$ 虽然天然不变但等于全局 DC 分量，改它就会肉眼可见地拉偏亮度；二阶 power spectrum 不变但是 phase-blind 的多对一映射，容量上限极低。三阶耦合是**同时**保留相位、保持严格不变、又不动 DC 的最低阶构造，恰好把"高容量"和"理论可证不变"两个看似冲突的目标在表示论意义上解耦。

2. **高阶 SH 子空间写入 + 等变 backbone 注入 (Higher-Order Spectral Embedding)**:

    - 功能：把水印写在感知最不显眼、又有足够能量参与三阶耦合的高频谐波子空间，保证视觉保真度。
    - 核心思路：仅在 $\mathcal{V}_{embed} = \bigoplus_{l \in \mathcal{L}_{embed}} \mathcal{V}_l$ ($l > 0$) 上操作，默认 $\mathcal{L}_{embed} = \{6, 8, 14\}$。先用 2 层 Gated Block 等变 backbone 提取结构化谱特征 $u = \Phi_{eq}(c)$，再通过 SO(3)-等变张量积 $\Delta u = \text{TP}_{\vartheta_1}(u, w)|_{\mathcal{V}_{embed}}$ 把 32-bit 水印（作为 $\mathcal{V}_0$ 标量特征）注入到 $u$ 的不可约子空间，依赖 SH 基的正交性保证不污染其他频段；最后通过逆 SHT + 感知/几何掩码 $\tilde{x} = x + M_{perc}(x) \odot M_{geo} \odot \Delta x$ 写回空域。
    - 设计动机：低阶 $l$（如 $\mathcal{V}_4$）做载体鲁棒性高但低频伪影刺眼；高阶 $l$（如 $\mathcal{V}_{16}$）隐蔽但易被压缩/锯齿衰减。论文实验给出"中频组合 $\{6,8,14\}$"是 fidelity-robustness 甜点——既给三阶耦合足够多样性的相位，又避开易被攻击衰减的极高频带；ERP 几何掩码 $M_{geo}$ 还专门抑制极区像素的修改幅度。

3. **解耦的非对称编解码 + 双重等变张量积链 (Decoupled Embed-Extract via Equivariant Tensor Product Chain)**:

    - 功能：解码端不需要对齐 / 不需要逆旋转 / 不需要先验角度估计，直接在任意旋转后的全景图上抽出与训练时数值上完全一致的不变标量来解水印。
    - 核心思路：解码用两步链式张量积近似三阶 bispectrum——$h = \text{TP}_{\vartheta_2}(\hat{u}, \hat{u})|_{\mathcal{V}_{embed}}$ 先在等变子空间做二阶耦合，$z = \text{TP}_{\vartheta_3}(h, \hat{u})|_{\mathcal{V}_0}$ 再耦合并强行投到 trivial 表示。等价于一个参数化的 $\text{TP}(\hat{u}, \hat{u}, \hat{u}) \to \mathcal{V}_0$，参数 $\vartheta_2, \vartheta_3$ 可学习地挑出"对水印最敏感"的 CG 通道；得到的不变向量进 MLP 解出 $\hat{w}$。整体用 $\mathcal{L}_{total} = \lambda_m \mathcal{L}_{MSE}(x, \tilde{x}) + \lambda_{bce} \mathcal{L}_{BCE}(w, \hat{w})$ 端到端训，$\lambda_{bce}=10$，$\lambda_m$ 从 1 线性升到 20。
    - 设计动机：传统方法把"鲁棒性"加在数据/训练侧（augmentation），论文把鲁棒性加在**架构与表示侧**——只要解码端最后一步严格投到 $\mathcal{V}_0$，无论中间网络多复杂，输出对 SO(3) 都是严格不变；这是"零增强"也能近完美抗任意旋转的根本原因。
