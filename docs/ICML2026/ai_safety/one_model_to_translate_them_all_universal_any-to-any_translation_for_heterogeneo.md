...
系统中每个 agent $i$ 用自己的编码器 $f^{\mathrm{enc}}_{i}$ 产出 BEV 中间特征 $F_i \in \mathcal{F}_{m_i}$；邻居 $j$ 通过通信发来 $\tilde{F}_j$，ego 端做完几何对齐后得到 $\hat{F}_{j\to i}$，再经过一个翻译器 $\mathcal{T}_{\phi_{j\to i}}$ 把它映射到 ego 模态空间，然后与 ego 自身特征做融合并接任务头。UniTrans 的全部创新都在"如何当场拿到 $\phi_{j\to i}$"这一步：训练时切成两阶段，先学一个模态内蕴编码器 MIE，再学一组翻译专家参数池 TPB 和一个路由器 MMR；推理时所有 agent 共享同一份 TPB+MMR，遇到新模态时直接由 MMR 输出组合系数 $\boldsymbol{\alpha}_{j\to i}$，把 TPB 里的专家参数线性合成成一个翻译器即可。形式化地，把整个系统建模为先学一个映射 $\delta_{j\to i} \in \mathcal{H} \to \phi_{j\to i} \in \Phi$，再让 $\phi$ 去做特征翻译。这种"参数空间组合"和经典 MoE"激活+加权输出"完全不同，等价于只跑一份翻译器，推理代价不会随专家数线性增长。

### 关键设计

1. **模态内蕴编码器 MIE（Modality-Intrinsic Encoder）**:

    - 功能：把高维 BEV 特征 $F \in \mathbb{R}^{C\times H\times W}$ 压成一个低维内蕴码 $z \in \mathbb{R}^d$，要求"同一模态的 $z$ 不论场景如何变化都聚成一团，不同模态的 $z$ 互相分开"，从而提供一个稳定可比的模态描述。
    - 核心思路：抽两类对场景不敏感、对模态敏感的统计——一阶通道统计 $\mu(F), \sigma(F)$，以及池化后特征的 Gram 描述子 $G(F)=\frac{1}{H'W'}\bar{F}\bar{F}^\top$，配上一支全局响应分支 $r(F)$，最后用 MLP 融合：$z = \psi_I([\mu(F), \sigma(F), r(F), \psi_G(G(F))])$。训练目标是 InfoNCE 对比损失 $\mathcal{L}_{\mathrm{IC}}$（拉近同模态、推开异模态）+ 一个轻量模态分类头的交叉熵 $\mathcal{L}_{\mathrm{IS}}$，合成 $\mathcal{L}_{\mathrm{stage1}} = \mathcal{L}_{\mathrm{IC}} + \lambda_{\mathrm{IS}} \mathcal{L}_{\mathrm{IS}}$。
    - 设计动机：直接在原始 $\mathcal{F}$ 空间里估计 $\Delta_{j\to i}:\mathcal{F}_{m_j}\to\mathcal{F}_{m_i}$ 是 ill-posed 的——单帧样本太稀疏、维度太高、又被场景内容污染。把映射推断"搬"到这个内蕴空间后，新模态只需要少量样本就能落到一个一致的区域，构成 UniTrans 第一个零样本能力来源。

2. **翻译参数池 TPB（Translator Parameter Bank）**:

    - 功能：用一组可复用的"专家参数"$\{\Theta^{(k)}\}_{k=1}^K$ 加一个共享专家 $\Theta^{(0)}$ 表达整个映射空间，每个 $\Theta^{(k)}$ 是一套完整的翻译器主干参数（按 sparse Transformer 设计的 MCT 块），共享专家负责吸收"映射无关"的翻译基元。
    - 核心思路：与其去训一个能同时处理所有 $(m_j,m_i)$ 的庞大单体翻译器，不如把映射空间分解，让每个专家专注一种"翻译子模式"，再让推理时按需混合。这样在容量上比单体翻译器更灵活，又能复用同一份参数池服务全部新模态。
    - 设计动机：实测里"映射多样性"比"特征多样性"更难拟合——单一翻译器在 LiDAR-Camera 这种巨大跨模态间隙下容易欠拟合；TPB 把负担分摊到专家上，模态映射越复杂，越能体现分解的好处。

3. **模态映射路由器 MMR + 参数级实例化**:

    - 功能：给定一对内蕴码 $(z_j, z_i)$，先算映射描述子 $\delta_{j\to i} = g(z_j, z_i)$，再输出组合系数 $\boldsymbol{\alpha}_{j\to i} = \mathrm{softmax}(h(\delta_{j\to i})) \in \mathbb{R}^K$，然后**在参数层面**把 TPB 合成成一个翻译器：$\phi_{j\to i} = \Theta^{(0)} + \sum_{k=1}^{K} \alpha^{(k)}_{j\to i}\, \Theta^{(k)}$，整次推理只跑一份合成翻译器。
    - 核心思路：Stage 2 联合优化任务损失 $\mathcal{L}_{\mathrm{task}}$、特征蒸馏损失 $\mathcal{L}_{\mathrm{feat}} = \|F_{j\to i} - F^{\star}_{j\to i}\|_2^2$（用 ego 编码器跑邻居观测得到的"理想 ego 域特征" $F^{\star}_{j\to i}=f^{\mathrm{enc}}_{m_i}(X_j)$ 当 teacher）、对路由向量的 InfoNCE $\mathcal{L}_{\mathrm{ctr}}$（同一映射标签拉近、不同映射推开）以及 Switch 风格的负载均衡正则 $\mathcal{L}_r$。
    - 设计动机：把"映射→翻译器"这件事建模成路由系数预测，让 MMR 学到一种**跨映射可外推的规律**——这是 UniTrans 第二个零样本能力来源。同时参数级合成相比 Classic MoE 的"多专家前向 + 输出混合"更省算力，对车端实时推理友好。

### 损失函数 / 训练策略

整体训练严格切成两阶段：Stage 1 只动 MIE，目标是构建稳定的模态内蕴空间；Stage 2 冻住 MIE 与下游任务头，把梯度回传到 MMR 和 TPB，最小化 $\mathcal{L}_{\mathrm{stage2}} = \mathcal{L}_{\mathrm{task}} + \lambda_{\mathrm{feat}}\mathcal{L}_{\mathrm{feat}} + \lambda_{\mathrm{ctr}}\mathcal{L}_{\mathrm{ctr}} + \lambda_r \mathcal{L}_r$。训练只能见到 $\mathcal{M}_{\mathrm{tr}}$ 中的模态与 $\mathcal{D}_{\mathrm{tr}}$ 中的场景，预留 6 种 emerging modality $\{m_7, m_{13}, m_{17}, m_{25}, m_{27}, m_{30}\}$ 完全留到推理评测，严格遵守"零样本"设定。
