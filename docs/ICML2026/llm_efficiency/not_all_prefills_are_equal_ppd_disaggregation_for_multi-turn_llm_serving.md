...
description: >-
  [ICML 2026][LLM效率][PD 分离] 本文指出多轮对话场景下传统 Prefill-Decode 分离架构因每轮都要 P→D 重算并传输 KV 而严重低效，提出 PPD（Prefill-capable Decode）动态路由系统…
tags:
  - "ICML 2026"
  - "LLM效率"
  - "PD 分离"
...
2. **以打分函数 $S$ 形式化的优化问题**:
    ...
    - 设计动机：在 3060 个配置（17 配置 × 18 工作负载 × 10 QPS）的系统扫描中，作者发现 **92.2% 的 (工作负载, QPS) 组合**下 Turn 2 TTFT 与 TPOT 的最优配置不同——不存在静态最优解，必须做 per-request 的动态决策。

3. **离线表 + 在线查的两阶段路由**:

    - 功能：把昂贵的优化决策计算挪到离线，在线只做毫秒级查表，对延迟敏感的服务路径几乎零开销。
    - 核心思路：离线对每个网格 cell 直接测 $x{=}0$ 和 $x{=}1$ 两个端点的 TTFT/TPOT，按 $S$ 的符号存布尔决策；在线把请求按三维特征（累积上下文长度、输入/输出比、系统 QPS）量化到最近 cell，<1ms 拿到 $x^*$。Turn 1 强制 $x=0$（无 KV 缓存可用），保证一致性。整个系统能通过调整权重 $\mathbf{w}$ 退化回任何静态策略（包括传统 PD）。
