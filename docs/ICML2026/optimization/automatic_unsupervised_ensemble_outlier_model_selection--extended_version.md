...
### 整体框架
MetaEns 分为离线元训练和在线模型选择两个阶段。离线阶段利用有标签的元数据集模拟序贯集成构建过程，计算每一步添加候选检测器的真实边际增益（AP 提升），用这些"状态-增益"对训练一个增益预测模型。在线阶段面对新的无标签数据集，先选定一个锚点检测器（primary detector），然后贪心地逐步添加预测效用最高的检测器，直到没有候选模型能带来正效用为止。候选池包含 297 个检测器，覆盖 IForest、LOF、kNN、HBOS、OCSVM、LODA、ABOD、COF 共 8 个算法族。集成分数采用成员检测器分数的均值聚合。

### 关键设计

1. **两阶段边际增益预测器（Two-Part Gain Model）**:

    - 功能：预测向当前集成中添加候选检测器后的 AP 提升
    - 核心思路：将增益预测拆分为分类器 $f_{\text{cls}}$ 和回归器 $f_{\text{reg}}$ 两部分，预测增益为 $\hat{G}(f_i \mid P) = f_{\text{cls}}(f_i \mid P) \cdot f_{\text{reg}}(f_i \mid P)$。分类器估计"候选模型是否能改善集成"的概率，回归器仅在正类样本上估计增益幅度。状态表征 $\phi(f_i, f_{i-1}^*, P)$ 包含候选与上一步选择的检测器、候选与当前集成、上一步选择与当前集成之间的分数统计特征（Spearman 相关、余弦相似度、熵、峰度、Jaccard 重叠等），以及集成大小 $|P|$。两个模型均用 ExtraTrees 实现
    - 设计动机：随着集成增长，正增益样本变得极度稀疏（大多数候选是冗余或有害的），单一回归器在零膨胀分布上会为很多候选预测出小的正值，导致误选冗余模型。两阶段设计让分类器充当"门控"，先判断是否值得添加，回归器专注于量化有用的增益幅度

2. **次模启发的代理效用目标（Submodular-Inspired Proxy Utility）**:

    - 功能：在测试时无标签条件下指导贪心选择和自适应停止
    - 核心思路：候选 $f_i$ 的边际效用定义为 $\Delta U(f_i \mid P) = \gamma(f_i, P) \cdot (\hat{G}(f_i \mid P) - \lambda_{\text{fam}} \pi_{\mathcal{F}(f_i)})$。其中冗余折扣 $\gamma(f_i, P) = 1/(1 + \beta \cdot \text{sim}_{\max}(f_i, P))$，用候选与已选模型的最大 Jaccard 相似度衰减效用，保证随集成增长效用递减。当所有候选的 $\Delta U \leq 0$ 时自动停止选择
    - 设计动机：学到的增益预测器 $\hat{G}$ 可能有噪声，不能保证满足次模性。通过显式的冗余折扣模拟递减收益，避免选入近重复模型；同时使用最大相似度（而非平均）可以严格阻止任何已选成员的近复制品进入集成

3. **算法族风险正则化（Family-Risk Regularization）**:

    - 功能：惩罚历史上表现不稳定的算法族，降低无标签条件下的选择风险
    - 核心思路：在元训练轨迹中，计算每个算法族 $F$ 的真实边际增益的第 10 百分位 $\text{Risk}_F = Q_{0.10}(\{G(f \mid P)\})$，将其转化为非负惩罚 $\pi_F = \max(0, -\text{Risk}_F)$，通过系数 $\lambda_{\text{fam}}$ 加入代理目标。对未见过的算法族设置零惩罚
    - 设计动机：某些算法族虽然平均表现尚可，但在部分数据集上会产生严重的负增益。在无标签环境下无法验证单次选择的好坏，因此需要利用历史下尾统计来规避系统性风险
