...
1. **列归一化 (column-wise normalization) 作为唯一的归一化**:

    - 功能：把每个权重矩阵 $G\in\mathbb{R}^{d_{in}\times d_{out}}$ 的每一列除以其 $\ell_2$ 范数，得到 $[\text{col}_1(G)/\|\text{col}_1(G)\|,\dots,\text{col}_n(G)/\|\text{col}_n(G)\|]$，无需任何额外状态。
    - 核心思路：四种归一化（奇异值、列、行、sign）来自不同矩阵范数下的最速下降方向。论文测出在 LLaMA 60M/130M/350M 上 sign 归一化困惑度 54.36/40.42/27.95、行归一化 79.27/37.67/21.63 全部差于 Adam (30.05/23.13/18.77)；列归一化 39.89/28.85/20.38 与奇异值 NS 归一化 34.15/25.25/18.73 接近 Adam。再看计算成本——$d{=}4096$ 时奇异值 SVD 1958.66 ms、Newton-Schulz 近似 14.41 ms、列归一化只要 0.17 ms。最关键的"为什么列优于行"——通过画 LM-head 梯度直方图发现：行归一化后某些列梯度绝对值飙到 150（因为 LM-head 输入维度 $d_\text{model}$ 远小于词表 $|V|$，row 操作让 token 频次差异爆发），训练直接发散；列归一化分布平滑、训练稳定。
    - 设计动机：归一化不需要存状态，是"白嫖性能"的最佳起点；从 4 种方案里挑列归一化是因为它同时占据"性能足够好"和"计算几乎免费"两个最优点，并且能自然处理 LM-head 这种高度不平衡的高频 token 列梯度。

2. **仅在最后一层施加一阶动量 (last-layer momentum, mmt-last)**:

    - 功能：所有中间层不维护任何动量直接用裸梯度更新；只有 LM-head 层维护一阶动量 $m_L^t=\beta m_L^{t-1}+(1-\beta)g_L^t$。这一层在 LLaMA 7B 上只占总参数的约 2%，于是动量内存开销可以忽略不计。
    - 核心思路：作者先用大 batch (512) 近似真实梯度、小 batch (32) 当作随机梯度，测出训练全程"最后一层梯度方差始终最大"（Figure 4a），第一层 (embedding) 次之，其余层方差都很小。然后给出 Theorem 2.1：多层 SGD-M 收敛率含 $\sum_l\left(\frac{1-\beta_l}{1+\beta_l}\cdot\frac{L\sqrt\gamma}{4\sqrt T}+\dots+\frac{1-\beta_l}{\beta_l^3}\cdot\frac{\gamma^2}{4LT}\right)\frac{\sigma_l^2}{\delta^2}$ 形式的方差项——也就是每层带自己的 $\beta_l$ 和方差 $\sigma_l^2$，定理推论是"只在方差大的层取大 $\beta$、其余层取 $\beta=0$ 既能恢复收敛、又能省内存"。实验印证：列归一化 + 仅最后一层动量 (Table 3) 在 60M/130M/350M 上分别得 30.81/22.57/16.32，对比 Adam 30.05/23.13/18.77 几乎打平甚至超越，特别是 350M 上反超 Adam 2.45 个困惑度。Figure 4b 还显示：给最后一层加动量后，第一层方差也跟着下降——动量在"最噪的源头"做平滑，下游层都受益。
    - 设计动机：动量是 Adam 内存开销的唯一来源（归一化不需要存状态），如果能证明动量"只需要存在最噪的一层"，就能把 Adam 的 $2\times$ 状态压成接近 $0$ 的开销。

3. **极简组合：SCALE 算法 = 列归一化 + 仅末层动量，二阶矩彻底丢掉**:

    - 功能：把以上两个组件拼成完整优化器，相对 Adam 改动只有几行代码，相对 vanilla SGD 几乎不加内存（7B 模型只多 2%、1B 只多 10%）。
    - 核心思路：算法每步对每层独立处理——最后一层做 EMA 后列归一化更新；其他层直接列归一化梯度更新。完全不维护二阶矩 $v^t$、不维护中间层一阶矩。和 SWAN 的对比尤其有意义：SWAN 同时用了行归一化 + 奇异值归一化两种归一化（且对 LM-head 用 Adam），SCALE 表明这其实是冗余设计——一种归一化就够、而且只需要列这一种；和 Scion 的对比是 Scion 全层都加动量，SCALE 表明只在最后一层加就够。LLaMA 7B 上 SCALE 总内存（权重 + 优化器状态）13.74 GB，vs Adam 40.43 GB、Muon 26.95 GB、APOLLO 16.14 GB、APOLLO-Mini 14.53 GB，是 Pareto 前沿最左下角；困惑度 12.59 也超过 Muon 12.72、APOLLO 13.02、APOLLO-Mini 13.09。
    - 设计动机：自底向上极简主义的天然终点——既然实验和理论都表明只需要"列归一化 + 最后一层动量"，就不该额外加任何东西，把组合做成可复用、可作为基线的"最小 LLM 优化器"。
