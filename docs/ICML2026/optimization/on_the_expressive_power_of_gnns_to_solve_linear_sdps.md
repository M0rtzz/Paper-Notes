...
要回答"需要多强的 GNN"，最干净的起点是先把不够强的排除掉。作者构造一族结构同构、但最优解不同的 SDP 实例，证明 1-WL 等价的 VC-WL 在稳定着色后会给同构实例分配相同的节点表示，于是任何基于它的 GNN 必然输出相同结果，根本无法区分这些不同的最优解；把分析推到 VC-2-WL（标准 2-WL 的变种）后结论依旧成立。这套论证沿用了 GNN 表达力研究里经典的"WL 同色 ⇒ GNN 同输出"模板，但首次落到 SDP 设定上，给出一个清晰下界——它的实践含义是警告后续工作别再在 1-WL/2-WL 类架构上浪费算力。

**2. 充分性架构 VC-2-FWL：用成对节点联合聚合仿真 PDHG**

下界划掉了弱架构，接下来要给一个真能学会的。VC-2-FWL 把颜色赋给节点对 $(u,v)\in V^2$，按 folklore 2-FWL 更新：

$$c^t_{uv} := \text{hash}\big(c^{t-1}_{uv},\ \{\!\{(c^{t-1}_{wv},\, c^{t-1}_{uw}) \mid w\in V\}\!\}\big)$$

证明充分性的关键不是直接去逼近最优解，而是改证"GNN 能仿真一个已知收敛到最优解的求解器"——这是 L2O 的标准技巧。具体把 PDHG 的一步迭代（矩阵-矩阵乘积、PSD 投影里的特征分解结构信息）显式映射成 VC-2-FWL 的若干 hash 步骤，只要嵌入维度够大，原变量和对偶变量的更新都能精确复刻。这意味着 VC-2-FWL 的稳定着色细于 PDHG 收敛轨迹的状态划分，从而具备拟合最优解的能力，也使它成为目前已知最弱的能解线性 SDP 的 GNN 架构。
