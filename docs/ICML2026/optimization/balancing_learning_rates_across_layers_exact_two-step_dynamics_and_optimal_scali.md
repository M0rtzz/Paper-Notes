...
description: >-
  [ICML 2026][优化/理论][逐层学习率] 本文在两层和三层线性神经网络中推导出梯度下降一步和两步后测试损失的精确闭式表达式，揭示了一个相变现象：第一步更新时非对称学习率最优，而第二步后对称（平衡）学习率变为局部最优，为逐层学习率调度提供了理论基础。 领域现状：深度网络训练中，逐层学习率调度（如 LARS、LAMB、…
tags:
  - "ICML 2026"
  - "优化/理论"
  - "逐层学习率"
...
**1. 梯度分解与信号主导性证明：把不可解析的精确梯度换成结构简洁的信号项**

精确梯度因为层间耦合根本写不出闭式，作者先把第 $\ell$ 层的梯度 $G_\ell^t=B_\ell^t-A_\ell^t$ 拆成两块：信号对齐项（如两层网络的 $A_1^t=\frac{1}{h}M W_2^{t\top}$，捕获标签信号）和权重自交互项（$B_1^t=\frac{1}{h^2}W_1^t W_2^t W_2^{t\top}$，刻画权重间耦合）。Proposition 5.1 证明当 $\eta_1,\eta_2\le O(h\sqrt h)$ 时，

$$\|G_\ell^t-A_\ell^t\|\le \frac{\|G_\ell^t\|}{\sqrt h-1},$$

即残差项被 $1/\sqrt h$ 因子压住、在宽网络里可忽略。这一步既让后续闭式测试损失推导成为可能，又顺手识别出临界学习率尺度——两层是 $\eta=\Theta(h\sqrt h)$、三层是 $\eta=\Theta(h)$，超过这个尺度信号近似就失效，整套分析的适用边界也随之划定。

**2. 精确两步测试损失公式：让对称点能被严格判定为不是/是局部最优**

有了信号-only 轨迹，就能把测试损失写成 $\eta_1,\eta_2$ 的精确多项式。两层网络一步损失为
...
里面同时有线性改善项、二次交互项和残差方差项；两步损失则带 $(1+\eta_1\eta_2/h^3)$ 的高次幂，反映层间乘性的表示学习。公式精确是关键——只有精确才能严格判断对称点 $\eta_1=\eta_2$ 在一步、两步分别是不是局部最小值，从而把"相变"从猜想变成可证命题。

**3. 非对称到平衡的相变定理：解释逐层学习率为何该先非对称后平衡**

在约束 $\eta_1+\eta_2=2h^\alpha$ 下，Corollary 5.4 给出质变：对任意 $0<\alpha\le 3/2$，对称点 $\eta_1=\eta_2$ 不是一步损失的局部最小值；但对 $1<\alpha\le 3/2$ 且 $h$ 足够大，$\eta_1=\eta_2$ 成为两步损失的局部最小值（三层网络结论类似，临界尺度降到 $O(h)$）。这首次从理论上说清了实践直觉：训练早期表示层与读出层角色不同，应非对称分配以利用各层特异的信号传播；后期跨层协调占主导，应趋向平衡——LARS/LAMB/TempBalance 这类调度器的早晚两段行为，在同一组闭式公式里得到了统一解释。
