...
1. **TPV 的迹分解 $\mathrm{Tr}(H_{\mathrm{eff}}C)$ 作为统一镜片**:
    ...
    - 核心思路：先对每个 $x$ 写 $\bigl(J(x)\delta w\bigr)^2=\mathrm{Tr}\bigl(J(x)^\top J(x)\delta w\delta w^\top\bigr)$，再对 $x$ 和 $\delta w$ 双重取期望，由于两者独立，得到 $\mathrm{Tr}(H_{\mathrm{eff}}C)$。$H_{\mathrm{eff}}$ 是 label-free 的几何量（只跟测试输入与训练好的 Jacobian 有关），$C$ 完全由噪声机制决定。代入具体 $C$：对 SGD 稳态噪声有 $C\propto (\eta/b)\nabla^2 L(w^\star)$，对量化有 $C=\tfrac{\delta^2}{12}I$，对标签噪声 $C$ 与 $J^\top J$ 的伪逆相关。
