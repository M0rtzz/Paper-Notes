...
description: >-
  [ICML 2026][优化/理论][整数线性规划] 本文提出 RL-SPH —— 一种不依赖外部 ILP 求解器、能独立产出 100% 可行解的端到端强化学习启发式算法，用「可行性奖励 + 双阶段策略 + 可行性感知邻域搜索」让 Graph Transformer Agent 在包含非二元整数变量的 ILP…
tags:
  - "ICML 2026"
  - "优化/理论"
  - "整数线性规划"
...
1. **双阶段可行性奖励（Two-Phase Reward）**:
    ...
    - 设计动机：作者证明了 Proposition 1 —— 只要 $\mathcal{R}_{t,\text{const}}>0$ 且 $\mathcal{R}_{t,\text{bound}}=0$ 持续成立，agent 终将进入可行域；这把可行性这一硬约束转成了 agent 的「reward-feasibility alignment」理论保证，避免了端到端 supervised 方法的隐式可行性盲区。
