...
## 方法详解
论文不是在提一个新算法，而是在提一种新的解释框架并用一系列受控实验把它钉牢。整篇方法可以拆成三件事：把现象搞干净、把抽象写明白、把"为什么会这样"的备选解释一个个排除。

### 整体框架
作者用 path-star 图作沙盒：从一个 root 节点发出 $d$ 条互不相交、长度都为 $\ell$ 的链。训练数据混合两种样本：边记忆（输入一个节点，预测其邻居，覆盖图的所有边）+ 路径寻找（输入 leaf，输出从 root 到 leaf 的完整路径，只在 75% 的 leaf 上训练）；测试只用剩下 25% 从未见过端到端的 leaf。这套设计的关键是即使把所有除第一个 token 以外的 loss 全部砍掉、把训练/测试路径完全不相交、把图扩到 $5\times10^4$ 节点，Transformer 和 Mamba 仍然能用接近 100% 的准确率预测出"从这个 leaf 出发的第一跳应该往哪走"，而这个第一 token 在联想记忆视角下需要 $\ell$ 次复合查找。

### 关键设计

1. **In-weights path-star sandbox**:

    - 功能：构造一个"模型只能靠记在参数里的图做隐式多跳推理"的纯净实验场，排除所有把多跳问题降阶到单跳的捷径。
    - 核心思路：(i) 把图烧进权重而不是塞进 context，(ii) 边对称化以避开 reversal curse，(iii) 路径长度全部统一为 $\ell$ 去掉隐式课程，(iv) 训练/测试 leaf 路径不相交去掉子串拼接，(v) 砍掉所有除第一个 token 之外的梯度。结果是模型仍能在巨大图上学会"第一 token 选哪个 child"，这一行为在 $f(u)[v]=\Phi(u)^T W_{\text{assoc}}\Phi(v)$ 这种近随机正交嵌入下要求 $\Omega(\exp(\ell))$ 步搜索。
    - 设计动机：先前关于 in-weights reasoning 的证据要么图太小（<200 节点）、要么只 2 跳、要么训练路径与测试路径有重叠，作者认为这些都不足以证伪"模型只是查表"。把所有救命稻草拔掉之后，模型仍然成功，就把谜题立住了。

2. **几何记忆 vs 联想记忆的双抽象**:

    - 功能：给"为什么 in-weights 推理能成功"提供一个统一的解释抽象，并把它和经典联想记忆放在同一张表上对比。
    - 核心思路：把存储方式重写成 $f(u)[v]=\Phi_{\text{geom}}(u)^T \Phi_{\text{geom}}(v)$，即邻接矩阵 $A$ 的（典型为低秩的）因子分解，其中 $\Phi_{\text{geom}}$ 与图拉普拉斯 $A-D$ 的顶部特征向量对齐。在这种抽象下，同一条路径上的节点嵌入会聚到一个共同方向 $\mathbf{z}_i$，"从 leaf 出发找第一跳"瞬间退化为"在 root 的邻居里挑一个余弦最大的"，从 $\ell$ 跳问题降阶为 1 跳。论文用余弦相似度热力图（leaf×first-hop 出现明显对角线）和 3D embedding 可视化把这一几何结构在 Transformer、Mamba 和一个 3 层 MLP 上都重现。
    - 设计动机：作者要论证"模型实际上没在做 $\ell$ 次查表，而是在一个被预先组织好的几何空间里走 1 步"。只有这个新抽象才能让"成功" 与"理论上应该指数难"同时成立。

3. **Memorization puzzle：排除三类经典解释**:

    - 功能：把"几何记忆为什么会赢"作为一个开放问题诚实地立起来——监督压力、容量/正则压力、优化偏置这三类常被默认的解释，作者一个一个用实验否掉。
    - 核心思路：(i) 监督压力：把路径寻找监督删掉只剩边记忆，热力图依然出现几何对角线；(ii) 容量压力：调到非常宽的模型（参数足够装下整张 $A$ 的联想存储）+ 关掉 dropout/weight decay，几何依旧浮现；反过来把嵌入冻住，模型轻松学到纯联想记忆，说明架构本身并不禁止联想；(iii) 优化偏置：实验显示梯度下降 2 步就能拼出近似 $\sum_{(u,v)\in E}\Phi(u)\otimes\Phi(v)$ 的联想查找表，几何要 100 步才浮现——即联想其实"更近、更易找"；同时在 sparse 图的记忆任务上联想与几何在 bit/norm 复杂度上几乎相等，所以"梯度下降偏好更简洁解"也救不了几何。三连否之后，作者把"为什么仍然挑了几何"列为开放问题。
    - 设计动机：与其再加一个故事不如把候选故事都封死，逼后续工作从更深层（如平坦度、谱范数、深度引发的隐式秩最小化）出发寻找答案，这是论文最大的方法论贡献。
