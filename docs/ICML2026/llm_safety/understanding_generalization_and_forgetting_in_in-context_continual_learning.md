...
description: >-
  [ICML 2026][LLM安全][上下文学习] 首次为上下文连续学习建立理论框架——揭示注意力机制在处理多任务序列时必然产生的系统偏差与任务干扰，导致泛化性能与任务记忆与任务顺序相关的衰减现象。
tags:
  - "ICML 2026"
  - "LLM安全"
  - "上下文学习"
...
遗忘在这套框架里不是参数被覆盖，而是处理新任务时注意力对各历史位置的权重被重新调整。作者引入依赖系数 $\alpha_i(t) = c_t < 0$（当 $i \le t$）、$\alpha_i(t) = d > 0$（当 $i > t$）来刻画这种重分配：已学过的过去任务被赋予负权重（贡献被抵消，于是退化），尚未学的未来任务被赋予正权重（对当前任务造成干扰）。遗忘的强度落在均值干扰项 $\mathrm{tr}(\mu_i\mu_j^\top\Gamma^{-2}\Lambda)$ 上，它只随任务对齐度变化。这正解释了为何长上下文消不掉遗忘——干扰与 $M$ 无关，无论上下文多长，任务越对齐、彼此的干扰反而越强。
