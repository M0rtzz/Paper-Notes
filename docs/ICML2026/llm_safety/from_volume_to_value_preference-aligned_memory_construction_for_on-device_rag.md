...
### 整体框架
输入是端上不断增长的候选条目集合 $\mathcal{D}$（Wiki 段落、对话片段、网页等）和一组用户偏好 $P = \{p_1, \dots, p_N\}$（显式给出或由独立 pipeline 抽取）；输出是一份紧凑的偏好对齐内存 $\mathcal{M}$，每条记录形如 $(x, i_x(p), p, E(i_x(p)))$，即「原始条目 + 偏好条件指令 + 对应偏好 + 指令 embedding」。运行时给定 query $q$，先把 $q$ 朝最相关偏好偏移得到 $\tilde q$，再在 $\mathcal{M}$ 的指令 embedding 上做 FAISS kNN，取到的指令指向其挂载的原始条目，把条目作为上下文交给生成器输出偏好对齐回答。整个 pipeline 把 LM 调用控制在小子集上（平均每条入流数据只触发 0.22 次 fine verification）。

### 关键设计

1. **Semantic-Based Coarse Filtering（粗筛：embedding 相似度高召回剪枝）**:

    - 功能：在不调任何 LM 的前提下，把绝大多数与所有偏好都不相关的候选条目直接丢弃，只把潜在相关的小子集 $\mathcal{D}_{coarse}$ 交给下一阶段。
    - 核心思路：用共享 sentence encoder（如 Contriever）把 item $x$ 和每条偏好 $p$ 都编成 $\ell_2$ 归一化向量 $E(\cdot) \in \mathbb{R}^d$，对每个 $x$ 算它和所有偏好的余弦相似度 $\mathrm{Sim}(E(x), E(p))$，把超过阈值 $\tau$ 的偏好收成 $P_{rel}(x) = \{p \in P \mid \mathrm{Sim}(E(x), E(p)) \ge \tau\}$，只要 $P_{rel}(x)$ 非空就保留 $x$（即 $\mathcal{D}_{coarse} = \{x \mid P_{rel}(x) \neq \emptyset\}$），同时把 $x$ 和它候选的偏好子集一起带到下一阶段，避免下游再做一次相似度匹配。
    - 设计动机：raw stream 里 99% 以上的内容跟当前用户偏好无关，调 LM 验证太贵；先用纯 embedding 几何近似做高召回筛选，把 LM 的调用域压到极小子集，这是整套内存节省（消融里粗筛单独就拿到 3.95×–77.68× 的存储压缩）和低 indexing 延迟（端上每条 102.67 ms）的根本来源。

2. **Preference-Aligned Fine Verification（细验证 + 指令生成：从向量近似到语义对齐）**:

    - 功能：对粗筛通过的候选做语言级二次确认，丢掉「向量近但语义无关」的噪声，并为真正相关的条目合成一条「该在什么情况下怎么用这条数据」的偏好条件指令作为最终索引单元。
    - 核心思路：包含两个 LM 模块。Decision Module (DM) 对 $(x, P_{rel}(x))$ 输出结构化 $(\mathrm{Decision}, \mathrm{Rationale}, P'_{rel}(x))$，其中 Decision $\in \{\langle \text{Keep}\rangle, \langle \text{Discard}\rangle\}$、$P'_{rel}(x) \subseteq P_{rel}(x)$ 是精修后真正相关的偏好子集；只有 Keep 的进入 $\mathcal{D}_{fine}$。Instruction Generator (IG) 再以 $(x, p, \mathrm{Rationale})$ 为输入，对每个 $p \in P'_{rel}(x)$ 生成一条偏好条件指令 $i_x(p) = \mathrm{IG}(x, p, \mathrm{Rationale})$。最终内存为 $\mathcal{M} = \{(x, i_x(p), p, E(i_x(p))) \mid x \in \mathcal{D}_{fine}, p \in P'_{rel}(x)\}$，**FAISS 索引建立在 $E(i_x(p))$ 上而不是 $E(x)$ 上**——也就是说检索匹配的是「用法指令」，命中的指令再指回原 item 给生成器。
    - 设计动机：embedding 相似只能近似语义，过细的 trade-off（如「dislike seafood」vs「sushi menu」）必须靠 LM 显式判别；更巧的是把判别过程的 Rationale 直接复用为指令生成的输入，把「这条数据为什么和该偏好相关」固化到索引层。索引指令而不是原文，等价于把「检索」从「找语义相似的事实」升级为「找匹配当前偏好用法的事实」，是准确率从纯粗筛跳到全 pipeline 提升 +13.22–+33.69%p 的关键。

3. **Preference-Guided Query Steering（查询偏移：把 query 拉到偏好方向）**:

    - 功能：把 raw query embedding 朝最相关偏好的方向偏移，使其与 index 侧已经偏好条件化的指令 embedding 同处一个语义子空间，从而提升 top-k 检索的偏好对齐度，几乎不增加延迟。
    - 核心思路：query 进来时先选 $p^* = \arg\max_{p \in P} \mathrm{Sim}(E(q), E(p))$，然后做向量加和再归一化得到偏移后查询 $\tilde q = (E(q) + E(p^*)) / \|E(q) + E(p^*)\|$，用 $\tilde q$ 在 FAISS 指令索引上做 kNN 检索 top-k 指令，跟随到挂载的 item 作为生成上下文。整个 steering 在 Jetson Orin Nano 上只增加 0.18 ms / query。
    - 设计动机：即使索引侧已经把偏好编进指令 embedding，query 侧仍然是 preference-agnostic 的，存在「同一个问题在不同偏好下应该被引到不同 item」的对齐缺口；用偏好向量平移 query 是个零参数、零额外 LM 调用的解法，对比 Pref-QR / PBR 这种「query 端调 LM 改写/扩展」的方案，既省了 hundreds of ms 的 LLM 调用，又把偏好信号一次构造在两端复用。
