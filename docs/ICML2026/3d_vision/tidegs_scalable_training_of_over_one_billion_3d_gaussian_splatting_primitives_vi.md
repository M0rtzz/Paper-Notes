...
### 整体框架
TideGS 是一套以 SSD–CPU–GPU 三层存储为骨架的 out-of-core 3DGS 训练系统。完整参数表 $\Theta \in \mathbb{R}^{N \times D}$（$D=59$）以**块**为单位常驻 SSD（log-structured append-only segment），CPU DRAM 维护一个带 dirty-bit 的 LRU 暖缓存，GPU VRAM 只放当前迭代真正要光栅化的**容量受限常驻集** $\mathcal{R}_t$。每一步训练分四阶段：(1) CPU 上做块级视锥剔除得到候选工作集 $\mathcal{K}_t$；(2) 异步 prefetch + H2D 把缺失块搬到 VRAM；(3) GPU 上跑标准 3DGS forward/backward；(4) 把被淘汰的脏块异步 D2H 回 CPU 缓存，进一步在 CPU 缓存满时批量追加写回 SSD。阶段 (1)(2)(4) 与 (3) 用独立 CUDA stream / I/O 线程重叠，把 SSD/PCIe 延迟藏到 GPU 计算之下。

### 关键设计

1. **块虚拟化 + 两级可见性过滤（block virtualization & 2-stage visibility）**：

    - 功能：把"按高斯访问"的随机访问转成"按块访问"的顺序大块 I/O，并在 CPU 端就过滤掉绝大多数不可见块，避免无效 SSD/PCIe 流量。
    - 核心思路：先按高斯中心的 Morton 码排序再切成大小 $B=4096$ 的连续块，单块负载 $4096 \times 59 \times 4$ B ≈ 944 KiB（约 236 个 4 KB 页），天然对齐文件系统/页缓存粒度。每个块用一个 bounding sphere $(\mathbf{c}_k, r_k)$ 概括。第一级在 CPU 上对相机 batch $\mathcal{B}_t$ 做 6 平面视锥球面剔除：若 $d < -r_k$ 则丢，留下保守可见块集合 $\mathcal{K}_t = \bigcup_{c \in \mathcal{B}_t}\{k \mid \mathrm{visible}(k, c)\}$；第二级在 GPU 上对常驻块内的高斯做标准 3DGS 精细剔除与光栅化，得到真正贡献集 $\mathcal{I}_t \subseteq \bigcup_{k \in \mathcal{R}_t \cap \mathcal{K}_t} \mathrm{Block}(k)$。块归属一旦初始化就固定，高斯位置更新只刷新所属块的 bound（允许相邻块 bound 重叠但不重复归属），从而既能批量 I/O 又**不改变原生 3DGS 的渲染语义**。
    - 设计动机：SSD 不擅长零散随机读，必须用大块对齐才能跑满 3.3 GB/s 的带宽；又必须在数据搬运**之前**把不可见块过滤掉，否则 SSD/PCIe 流量仍随 $N$ 线性增长——Morton 排序让块更"紧凑"，bounding sphere 更小，CPU 端剔除更准。

2. **三级 SSD–CPU–GPU 异步引擎 + log-structured 写回**：

    - 功能：在低带宽高延迟的 SSD 后端下，仍能让 GPU 计算几乎不停摆，并把频繁的小块参数更新转化为顺序大块追加写，回避 SSD 随机写放大。
    - 核心思路：SSD 上用 append-only 段——初始模型写成不可变 base segment，训练中更新的块按 cache flush 批次追加到 patch segment，靠一张索引表 $\mathrm{Index}[k] = (\mathrm{file\_id}, \mathrm{offset}, \mathrm{size}, \mathrm{version})$ 指向最新版本，避免 in-place overwrite。CPU 缓存对块维护 LRU + dirty 位，脏块只在被 CPU 缓存驱逐或显式 checkpoint 时才写回 SSD。淘汰路径走两步：VRAM → CPU → SSD，脏块先以 dirty 形式进 CPU 缓存，再延后批量刷盘。执行端把 (i) SSD prefetch、(ii) H2D 搬入、(iii) GPU 前/反向、(iv) D2H 搬出 + SSD flush 四件事放在专用 I/O 线程 + 独立 CUDA stream 上，配合双缓冲块槽位"当前迭代算的同时把下一迭代的 $\mathcal{S}_t^+$ 提前搬进来"。
    - 设计动机：朴素 offload 在每个阶段都会串行等 I/O，端到端被 SSD 延迟支配；log-structured 把随机写变顺序写是 SSD 系统的经典套路，可以在频繁参数更新场景下保住峰值写带宽；两步式写回让 GPU 端的 hot 脏块在 CPU 暖缓存里被反复修改而不触盘，I/O 摊销到真正的 capacity miss 上。

3. **Tide：轨迹自适应差分流式传输（trajectory-adaptive differential streaming）**：

    - 功能：在容量受限的 VRAM 预算 $C$ 下，跨迭代复用常驻块并只传送增量，让 PCIe/SSD 流量随**常驻集变化量**而非候选工作集甚至模型规模伸缩。
    - 核心思路：用 TSP-聚类排序（而非随机 shuffle）的相机序列增加相邻 batch 的工作集重叠。每次迭代从候选池 $\mathcal{C}_t = \mathcal{R}_t \cup \mathcal{K}_{t+1}$ 中按打分 $s(k) = \lambda \cdot \mathbf{1}[k \in \mathcal{K}_{t+1}] + (1-\lambda) \cdot \mathrm{Recency}(k)$ 选下一步常驻集 $\mathcal{R}_{t+1}$（前一项关注"下一步要用"，后一项保留 LRU 热度）。当 $|\mathcal{K}_{t+1}| > C$ 时采用 camera-balanced Top-$C$：先按 batch 内每个相机分配少量配额保证视角覆盖，再按全局 $s(k)$ 填满剩余槽位。最后只搬运差集 $\mathcal{S}_t^+ = \mathcal{R}_{t+1} \setminus \mathcal{R}_t$、淘汰 $\mathcal{S}_t^- = \mathcal{R}_t \setminus \mathcal{R}_{t+1}$、保留交集 $\Omega_t^R = \mathcal{R}_t \cap \mathcal{R}_{t+1}$。优化器状态只为常驻块实例化，淘汰即弃，再入则冷重启——拿"瞬时矩状态"换"更小 VRAM 足迹与更小跨层流量"；得益于轨迹排序，热块往往能跨多次迭代留驻，矩状态实际很少冷启动。
    - 设计动机：即便 $\mathcal{K}_t \ll N$，每步重传 $\mathcal{K}_t$ 在城市级场景里仍是几百 MB 量级的 PCIe 流量；只要相机轨迹平滑，$|\mathcal{S}_t^+|$ 通常远小于 $|\mathcal{K}_t|$，差分流式把流量从"工作集尺度"再压一个数量级。这也是论文名字"Tide"（潮汐）的来历：常驻集像潮水一样随相机移动缓慢起落，而不是每步整个重灌。
