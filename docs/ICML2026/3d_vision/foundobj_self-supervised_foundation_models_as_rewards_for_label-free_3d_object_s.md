...
### 整体框架
输入：一个 3D 场景点云 $\bm{P}$（及配对的 RGB-D 图像，用于把 2D DINOv2 特征投回 3D）。

Pipeline 分三段：（1）**物体发现代理**：Felzenszwalb 算法把场景切成 $K$ 个初始超点，SparseConv 提取每个超点的特征；一个 seed 策略网络 $\bm{\pi}_{seed}$ 选起点，一个 merge 策略网络 $\bm{\pi}_{merge}$ 反复挑邻居合并，得到候选 $\bm{s}_0 \to \bm{s}_1 \to \cdots \to \bm{s}_T$。（2）**几何奖励模块**：拿 TRELLIS 预训练编码器外加一个"物体中心场"head，用 DBSCAN 验证候选点是否汇聚到单一中心，是则 +10，否则 −1。（3）**语义奖励模块**：用 DINOv2 投影到 3D 的特征，做"语义一致性 NCut"，cost 落在场景 top-20 最低 bank 内则 +10，否则 −1。两个奖励取 max，候选一旦得 +10 即终止该回合，代理用 PPO 训练。最终把所有 +10 的伪掩码喂给一个独立的 Mask3D 做下游分割推理。

### 关键设计

1. **超点级物体发现代理（替代 GrabS 的圆柱代理）**:

    - 功能：在 $K$ 个超点构成的离散空间里，通过 seed 选择 + 邻居合并两步动作产生任意形状的物体候选。
    - 核心思路：seed 策略对所有超点特征自注意力后输出 softmax 概率 $\bm{p}_{seed} = \bm{\pi}_{seed}([\bm{f}_1, \dots, \bm{f}_K])$，采样得到种子 $\bm{s}_0$；merge 策略对种子和它 0.1m 范围内的 $Q$ 个邻居做自注意力 + sigmoid，输出每个邻居的合并概率 $\bm{p}_{merge} = \bm{\pi}_{merge}(\bm{f}_0, [\bm{f}_0^1, \dots, \bm{f}_0^Q])$，按概率采样合并；重复直到拿到 +10 奖励或达到上限 $T$。
    - 设计动机：圆柱代理（GrabS）天然假设物体接近规则形状，对扁平柜子、L 形沙发束手无策。超点+合并把动作空间离散化，既保留了任意形状的表达力，又把搜索空间从原始点降到 $K$ 个超点，RL 才能 tractable。

2. **几何奖励：TRELLIS 的"物体中心场" + DBSCAN 验证**:

    - 功能：判定候选点云在几何上是否"自洽地汇聚成一个物体"。
    - 核心思路：在 TRELLIS 预训练编码器上加一个 Transformer 解码器 head $\bm{g}_{center}$，回归每点指向物体质心的向量 $\bm{v}_m = \bm{o}_c - \bm{o}_m$（$\bm{o}_c = \frac{1}{M}\sum \bm{o}_m$），在 ABO + 3D-Future 上用 $\ell_2$ loss 训好。推理时把候选 $\bm{s}_t$ 喂入得到 $\bm{v}_t$，对 $(\bm{s}_t + \bm{v}_t)$ 跑 DBSCAN（$r=0.05$）：若主簇覆盖率 $\geq \alpha=30\%$ 则奖励 +10，否则 −1。
    - 设计动机：TRELLIS 是 auto-encoder，本身不能给"是不是物体"打分；中心场把"物体即点云向单点收敛"这一几何先验显式化，避免直接重建对比的高代价；DBSCAN 让验证对部分遮挡和噪点鲁棒。

3. **语义奖励：DINOv2 + 语义一致性 NCut**:

    - 功能：判定候选区域语义上是否和背景显著不同。
    - 核心思路：把 DINOv2 的 2D 特征通过深度图投影到 3D 并按超点平均，构造场景级 $K \times K$ 余弦相似度矩阵 $\mathcal{S}$ 与 0.1m 邻接矩阵 $\mathcal{A}$，联合矩阵 $(\mathcal{S} * \mathcal{A})$ 表示空间-语义复合相似度。把候选的 one-hot 掩码 $O_t$ 视为对场景图的一刀切，借鉴 NCut 计算 cost $\mathcal{C} = \mathcal{C}_{boundary} / \mathcal{C}_{vol}$。维护一个 per-scene 的"top-20 最低 cost bank"，候选 cost 落入 bank 则 +10，否则 −1。
    - 设计动机：固定阈值难以跨场景泛化（家具密集和稀疏的房间 cost 分布差很多），bank 自适应只保留"当前场景里最像物体的 20 个候选"，既鼓励探索多样物体又避免被低质量噪声候选淹没。

### 损失函数 / 训练策略
代理本身用标准 PPO 训练；几何中心场 head $\bm{g}_{center}$ 离线在 ABO + 3D-Future 上用 $\ell_2$ 预训练；语义模块零参数（只是基于 DINOv2 + bank 的奖励器）。把所有得 +10 的候选作为伪掩码，再监督训练一个独立的 Mask3D 用于推理（沿用 GrabS 的两阶段配方），benchmark 时只跑这个轻量的 Mask3D。
