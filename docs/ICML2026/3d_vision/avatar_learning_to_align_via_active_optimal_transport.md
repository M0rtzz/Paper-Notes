...
description: >-
  [ICML 2026][3D视觉][最优传输] 本文提出 AvAtar，一个基于最优传输（OT）的主动对齐框架，通过梯度传播量化候选查询对全局对齐结果的影响，并利用伴随状态法和共轭梯度法以线性复杂度高效求解，在网络对齐和跨域对齐任务上一致超越已有主动学习策略。 领域现状：对齐问题（alignment）是许多机器学习任务的核心…
tags:
  - "ICML 2026"
  - "3D视觉"
  - "最优传输"
...
flowchart TD
    A["OT 对齐器（代价 C，边际 μ、ν）"] --> B["OT 求解器 → 传输计划 T"]
    U["效用函数 f(T)<br/>可替换，适配不同任务"] -.-> C
    B --> C["成对影响<br/>伴随状态法解 (n+m) 维线性系统"]
    C --> D["后验加权聚合<br/>按 T 加权 → 对象级查询价值"]
    D --> E["选 top-n_b 候选 → 问 oracle"]
    E --> F["更新监督矩阵 H"]
    F -->|预算 k 未用完| B
    F -->|预算用完| G["输出对齐结果"]
```
