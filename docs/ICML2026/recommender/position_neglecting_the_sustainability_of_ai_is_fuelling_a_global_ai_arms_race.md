...
position paper 的"方法"由四块论证拼成：可持续 AI 的形式化、张力的两个反例、base-superstructure 解释、CARAML 行动框架。

### 整体框架

第 2 节先把可持续 AI 拆成环境 / 经济 / 社会三柱，引入"气候意识 vs. 资源意识"两轴，用两个反例（低碳数据中心、效率反弹）证明只拉一轴会败坏整体。第 3 节用 base-superstructure 模型解释为何 GPU-rich vs. GPU-poor 鸿沟会自我维持。第 4 节给出 CARAML 行动框架。第 5 节正面回应"碳排会自动降下来""distilled 模型已经解决问题"等反方观点。

### 关键设计

1. **气候意识 × 资源意识 二维四象限**:

    - 功能：把模糊的"sustainable AI"放进一个可分类、可争论的坐标系，迫使任何方案先声明自己落在哪个象限。
    - 核心思路：横轴是 resource awareness（去殖化、可参与性，反对资源集中），纵轴是 climate awareness（碳/水/电子垃圾意识）。SOTA AI 在两轴都低；green AI 高 climate 低 resource；inclusive AI 反过来；只有右上角同时高两轴的才是 sustainable AI。Edge AI 因 embodied carbon 难界定，作者特意打了问号。
    - 设计动机：用一张图把"democratisation 派"和"green AI 派"之间的暗中分歧显式化——单纯追求一轴必然出现 trade-off。作者要逼讨论从"我们都想 sustainable"升级为"你想落到哪个象限"。

2. **Base-Superstructure 模型解释 AI 体制再生产**:

    - 功能：用历史唯物主义解释为什么"个人努力 + 技术效率"无法自动消解 AI arms race ——上层的政策 / 叙事 / 教育会反向锁定下层的物质垄断。
    - 核心思路：把 AI 的"base"定义为算力 + 数据 + 资本 + 劳动 + 知识商品；"superstructure"是 AI 政策、监管、研究规范、媒体叙事。两者构成正反馈：物质垄断（NVIDIA + 少数 hyperscaler）塑造了 AI sovereignty、export control、ethical guideline 等上层话语，后者反过来固化既有物质秩序。作者用一段假想情境演示——出口管制以"国家安全"叙事正当化对算力的限制，被限制方被迫重建半导体供应，结果只是在既有依赖关系内换一种依赖形态。
    - 设计动机：直接借鉴 colonial expansion 的历史类比——殖民资本与种族等级互为基础与上层，本文论证当代 AI 正在重复这种 GPU 时代的殖民结构。这是 ICML 主会 position 里少见的把人文社科理论硬性嵌入论证主线的写法。

3. **CARAML 行动框架（agency × scope 两轴五层）**:

    - 功能：把"必须同时拉气候与资源两轴"这个总目标，落到从个人到全球的五个具体层级，匹配每层能掌控的杠杆。
    - 核心思路：横轴是 agency（个人能动性强但范围小，国家与全球反之），纵轴是 scope。在每一层给至少一项可执行动作——个人：用 perf-per-resource 替代单纯 accuracy；ML 社区：大规模实验预注册以杜绝重复浪费；工业：自我加碳排上限；政府：把 AI 影响评估写进监管（类比 EIA）；全球：把"Right to Compute" 改写为"Right to Sustainable Compute"，并通过开源 / 共享算力对抗数字鸿沟。
    - 设计动机：作者明知道单层行动不够（个人 agency 大但 scope 小，全球 scope 大但 agency 弱），所以用"agency × scope"框图证明只有协调多层才能扩大影响力 sphere。CARAML 也是论文实证的载体——LLaMA / BLOOM / Mistral 等开源模型被用来演示哪些层在做、哪些层缺位。
