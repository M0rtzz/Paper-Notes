...
1. **目标行为引导的边级 IB 去噪**：
    ...
    - 功能：把辅助行为图 $\mathcal{G}_k$ 中与目标任务无关的边在消息传递之前剪掉，得到 $\mathcal{G}_k'$。
    - 核心思路：把去噪建模为边丢弃问题，每条辅助边 $e_{<u_a,i_b>}$ 被保留的概率 $w_{ab}=f([\mathbf{e}_a;\mathbf{e}_b])$，其中 $\mathbf{e}_a, \mathbf{e}_b$ 是目标行为图上学到的 user/item 表示，$f$ 是单层 MLP；这样"是否保留"完全由目标行为的偏好决定，等价于把目标偏好当作 IB 中的 $Y$ 来给辅助图打监督。伯努利采样用 Concrete 重参数化 $\mathrm{sigmoid}((\log(\delta/(1-\delta))+w_{ab})/t)$ 让梯度能反传。目标 $\max I(\mathcal{R};\mathcal{G}_k')$ 项直接用目标行为 BPR 损失替代（因为 BPR 优化等价于最大化目标行为的对数似然）。
    - 设计动机：先前 IB 推荐方法压缩"表示"而不是"图"，等噪声经 GNN 聚合后再去噪已经晚了；把 IB 推到边层级，相当于在 message passing 之前就把脏边筛掉，user/item embedding 从源头就更干净。

2. **HSIC 替代项做图压缩**：

    - 功能：实现 IB 的压缩项 $\min I(\mathcal{G}_k'; \mathcal{G}_k)$，让去噪图与原图在节点表示空间上"统计独立"。
    - 核心思路：图是非欧数据，互信息没法直接估计；作者引入 HSIC（基于 RKHS 的核独立性度量）作替代，对小批量节点表示 $\mathbf{E}'^{\mathbf{B}}_k, \mathbf{E}^{\mathbf{B}}_k$ 用 RBF 核估计 $\hat{HSIC}(X,Y) = (n-1)^{-2}\mathrm{Tr}(K_X H K_Y H)$，最终损失 $\mathcal{L}_{IB} = \frac{1}{|\mathcal{K}|}\sum_k \hat{HSIC}(\mathbf{E}'^{\mathbf{B}}_k, \mathbf{E}^{\mathbf{B}}_k)$。这种核估计完全可微、不依赖 $p(\mathcal{G}_k'|\mathcal{G}_k)$ 的先验假设。
    - 设计动机：variational 上界做互信息压缩需要假设条件分布形式，对图结构这种离散对象很难凑；HSIC 是 model-free 的，只需要两组节点表示就能算，把"压缩"问题转成了"独立性正则化"，工程上稳。

3. **跨行为 InfoNCE 语义对齐**：

    - 功能：在去噪辅助图与目标图之间建立语义桥梁，把辅助行为里的丰富语义补到稀疏的目标表示上。
    - 核心思路：在去噪图 $\mathcal{G}_k'$ 上跑 LightGCN 得到辅助视图 $\mathbf{z}^{aux_k}_u = \mathrm{Mean}(u_k^{(0)},\dots,u_k^{(L_M-1)})$，多 behavior 求均值得 $\mathbf{z}^{aux}_u$；目标图 LightGCN 得 $\mathbf{z}^{tgt}_u$；用 InfoNCE $\mathcal{L}^u_{CL} = -\log\frac{\exp(s(\mathbf{z}^{tgt}_u,\mathbf{z}^{aux}_u)/\tau)}{\sum_{u'} \exp(s(\mathbf{z}^{tgt}_u,\mathbf{z}^{aux}_{u'})/\tau)}$ 把同 user 的两视图拉近、把 batch 内其他 user 推远，item 侧同样处理后取平均。
    - 设计动机：稀疏目标行为下 BPR 监督信号严重不足；同时如果直接把辅助和目标 embedding 加权融合，又会被噪声污染。先用 GIB 把辅助图洗干净、再用对比学习"软对齐"，既补充了监督又保持了对噪声的鲁棒——对齐是发生在去噪之后的，不会强行让"点击"和"购买"共享一模一样的语义。
