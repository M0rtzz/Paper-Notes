...
1. **Embedding 对齐的可演化技能选择头**:

    - 功能：在技能库随时间增删的前提下，让控制器仍能对当前 span 给出合法概率分布。
    - 核心思路：用同一套 embedding 模型把 span 上下文 $h_t = f_{\mathrm{ctx}}(x_t, M_t)$ 和每条技能描述 $u_i = f_{\mathrm{skill}}(\text{Description}(s_i))$ 编到同一表示空间，再以 $z_{t,i} = h_t^\top u_i$、$p_\theta(i \mid h_t) = \text{Softmax}(z_t)_i$ 计分。$z_t$ 的维度随技能库 $|\mathcal{S}^{(e)}|$ 自适应，所以 designer 增删 skill 不需要重训 policy head。Top-K 用 Gumbel-Top-K 不放回采样，得到 ordered set $A_t = (a_{t,1}, \ldots, a_{t,K})$，策略概率写成 $\pi_\theta(A_t \mid h_t) = \prod_{j=1}^{K} \frac{p_\theta(a_{t,j} \mid h_t)}{1 - \sum_{\ell<j} p_\theta(a_{t,\ell} \mid h_t)}$。
    - 设计动机：一个 span 经常需要复合操作（既插入新的参数-性能事实又删除一个失效假设），单 skill 选择会人为分摊成多步浪费仿真。embedding 对齐则解决了"动作空间会变"的标准 actor head 不能处理的痛点。

2. **延迟奖励再分配 + 过程奖励**:

    - 功能：解决长时程设计里 terminal QA 性能晚于中途记忆操作多步才出现的信用分配问题。
    - 核心思路：记 episode 终端奖励为 $R_{\mathrm{final}}$，按指数衰减再分配到每个 span：$\tilde{r}_t = (1-\beta) R_{\mathrm{final}} \frac{\gamma^{T-t}}{\sum_{k=1}^{T} \gamma^{T-k}} + \beta \mathbf{1}[t=T] R_{\mathrm{final}}$，其中 $\gamma \in (0,1)$ 控制衰减、$\beta \in [0,1]$ 保留一份纯 terminal 信号。最终 step 奖励为 $r_t = r_{\mathrm{proc},t} + \tilde{r}_t$，$r_{\mathrm{proc},t}$ 是 memory 构造质量与物理一致性 check 的过程奖励。
    - 设计动机：仅靠 terminal reward 训 PPO 会让中间 span 长期无信号；纯 process reward 又容易让控制器学到"看似有用但对最终设计无帮助"的 trivia。指数衰减加显式 $\beta$ 项让作者能在两端连续插值，并匹配实际训练 pipeline。

3. **基于失败缓冲的技能演化 + 接受回滚**:

    - 功能：让技能库本身从训练经验中"长出"新操作，又不被偶发噪声拖累。
    - 核心思路：外层每个 epoch $e$ 从低性能/物理无效轨迹收集 hard case 进 $\mathcal{B}^{(e)}$，按结构 regime（如 hexagonal、PBG、Kagome）和光学失效类型做聚类，再按难度选代表样本送给 Designer。Designer 做两步：诊断（识别缺失/错位的 memory 操作）→ 精炼（编辑既有 skill 或新增结构感知 skill），得到候选 $\hat{\mathcal{S}}^{(e+1)} = \text{Designer}(\mathcal{S}^{(e)}, \mathcal{B}^{(e)})$。是否接受由验证集回报差 $\Delta J_{\mathrm{val}} = J_{\mathrm{val}}(\theta^{(e+1)}, \hat{\mathcal{S}}^{(e+1)}) - J_{\mathrm{val}}(\theta^{(e+1)}, \mathcal{S}^{(e)})$ 决定，$\Delta J_{\mathrm{val}} \geq 0$ 才接受、否则回滚到 $\mathcal{S}^{(e)}$。接受之后短期偏置探索倾向新技能。
    - 设计动机：固定 skill bank 上限是死的，但若 Designer 改坏了就会损害已学好的控制器。验证集回报差形成的 gate 提供了"有限激进 + 安全回退"的工程平衡。
