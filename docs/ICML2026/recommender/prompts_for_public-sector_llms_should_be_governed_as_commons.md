...
1. **三档治理状态 + 可强制执行的仓库规则**:
    ...
    - 功能：把"谁能改提示词、什么提示词能被发布、出问题怎么停"三件事映射为 Git 仓库可强制执行的规则。
    - 核心思路：Open 状态——任何认证用户可提案，维护者只清理 spam 与基本安全风险；Curated 状态——合并需维护者审、必须填齐 provenance 字段、释放必须满足跨群体/地区的覆盖配额、走 checklist；Veto-enabled 状态——在 curated 之上加正式 quarantine 机制，被指定的代表性组织可发"否决记录"暂时下架，限时复议（accept / modify / reject）。每一档都对应明确的 PR 模板与 CI 检查项，不依赖任何特殊基础设施。
    - 设计动机：commons 治理失败的常见原因是规则只写在文档里不可执行；把治理状态嵌入仓库工作流就能让"治理"变成日常 PR 流程的一部分，而不是事后补救。

2. **协商式聚合提示词（negotiation-oriented prompt aggregation）**:

    - 功能：当多个利益相关方各自提交了带不同价值主张（可达性、安全、成本、气候韧性、程序公平）的提示词时，用一条"元提示词"让模型显式识别共识、列出分歧、说明价值张力、给出折中方案或排序的备选项。
    - 核心思路：把聚合规则本身当作一个**被版本化、被审计**的 artefact，而不是隐式的 ensemble；与社会选择理论里的"聚合规则塑造结果"思想对齐。pilot 里实现为 M4：分层抽样 $k=6$ 条按作者群体均衡的提示词 + 一个固定的聚合指令。
    - 设计动机：传统做法要么把分歧硬塞进一条提示词（多数压倒少数），要么用 hidden ensemble（少数派被静默掩盖）。把"如何处理分歧"写成可审计的提示词，受影响群体才能事后追问"聚合是否抹平了少数派关切"。

3. **可证伪的评测协议（5 种方法 × 3 类指标）**:

    - 功能：把"治理是否真的起作用"变成可被实验数据反驳的命题。
    - 核心思路：固定一个 instruction-tuned chat LLM（temperature 0, top-$p$ 1, max 256 tokens），固定 $N=50$ 条街道与公共空间取舍的"contested-choice"小情景，固定三选一标签（车辆通行优先 / 主动交通与可达性优先 / 混合或折中）。比较 M0（单作者提示词）、M1（open commons 随机抽 1 条）、M2（curated 按覆盖约束抽样）、M3（curated + veto）、M4（协商式聚合）。指标三类：输出分布（折中率、commitment $D=1-p_{\text{mixed}}$）、主观可接受度（6 组共 12 名 raters 7 点量表）、运维响应时延（50 起合成事件的 time-to-remediation）。
    - 设计动机：position paper 的论证必须"可被推翻"——作者明确写出三个预测 P1/P2/P3，并声明：如果治理化版本与单作者版本在输出分布、可审计性、修复时延上**没差异或更差**，则本文立场被证伪。
