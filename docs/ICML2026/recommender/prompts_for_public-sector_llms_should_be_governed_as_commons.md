...
flowchart TD
    A["社区贡献者<br/>各自带价值主张的提示词"] --> B["版本化仓库结构 + provenance metadata<br/>纯文本条目（作者群体/地点/价值主张/同意级别）<br/>+ issue/PR 可审计变更日志"]
    B --> C["三档治理状态（可强制执行的仓库规则）<br/>open → curated → veto-enabled"]
    C --> D["协商式聚合提示词（M4）<br/>分层抽 k=6 条 + 版本化聚合指令<br/>识别共识 / 列分歧 / 给折中"]
    D --> E["可证伪评测协议<br/>M0–M4 × 输出分布 / 可接受度 / 修复时延"]
    E --> F["验证三条预测 P1/P2/P3<br/>改变分布 · 提升可审计 · 缩短修复"]
```

### 关键设计

**1. 版本化仓库结构 + provenance metadata：把提示词变成可引用、可追溯的发布工件**

这是整套治理的底座。Prompt Commons 把每条提示词存成带版本号的**纯文本条目**，并强制记录部署实践里常缺的 provenance 字段：作者群体（如老年人、残障倡导、LGBTQ+、少数族裔/宗教群体）、地点（街区 / 廊道 / 行政单元）、价值主张（可达性、生物多样性、安全）、同意与访问级别，以及一份可审计的变更日志；所有改动走 issue / PR、合并时连同理由与时间戳一并落档。和现有 prompt 仓库（只解决"共享方便"）相比，它把公共部署真正缺的几样东西做成**仓库里可强制执行的一等对象**：与部署配置绑定的版本化发布、可执行的贡献/审查规则、否决/隔离/申诉、同意/撤回/许可证元数据，以及能把每一条输出回溯到"具体提示词版本 + 治理状态"的审计日志。正是这层结构让"提示词"从一次性输入升级成可被采购文档引用、可被合规审查、可被回滚的发布工件——也直接支撑了可审计性这条预测（P2）。

**2. 三档治理状态 + 可强制执行的仓库规则：把"谁能改、什么能发、出问题怎么停"映射成 Git 规则**
...
**3. 协商式聚合提示词：把"如何处理分歧"写成可审计的 artefact，而不是隐藏的 ensemble**
...
**4. 可证伪的评测协议（5 种方法 × 3 类指标）：把"治理是否真起作用"变成能被数据反驳的命题**
