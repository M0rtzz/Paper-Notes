...
3. **多尺度检索 L2 + 频谱式上下文构造**:
    ...
    - 核心思路：查询 $q$ 进来时 $f_{\text{retr}}(q, \mathcal{M})$ 同时访问 micro（episodic 证据）、meso（关系级摘要）、macro（节点级 $\Sigma$ 与 $\Delta$）三层，按问题意图组合成统一上下文 $C(q)$ 喂给 LLM。事实型问题主要拉 micro，长程推理或人格类问题主要拉 macro。
