...
description: >-
  [ICML 2026][图像恢复][动态分辨率扩散] SubDAPS / SubDAPS++ 把 DPS、DAPS 这类 pixel-space 扩散复原方法搬进"动态分辨率扩散模型"框架——早期在 $64^2 / 128^2$ 子空间采样、后期才回到 $256^2$ 全分辨率…
tags:
  - "ICML 2026"
  - "图像恢复"
  - "动态分辨率扩散"
...
1. **SubDPS / SubDAPS：把 measurement consistency 搬进动态分辨率**:

    - 功能：让经典 DPS / DAPS 在子空间里仍能用观测做修正，并自然处理分辨率切换。
    - 核心思路：对 DPS，在 $d_{i-1} = d_i$ 时把 likelihood 梯度改写为 $\nabla_{\bm{x}_{t_i}} \log p_{t_i}(\bm{y} | \bm{x}_{t_i}) \approx -\zeta_{t_i} \nabla_{\bm{x}_{t_i}} \|\bm{y} - \mathcal{A}(\bm{U}_i \bm{x}_\theta(\bm{x}_{t_i}, t_i))\|^2$（用上采矩阵 $\bm{U}_i$ 把子空间预测投回原图域算 measurement）。在维度切换处 $d_{i-1} \neq d_i$，受 DAPS"早期随机噪声可纠正累积误差"启发，作者干脆放弃专门修正，直接 $\bm{x}_{t_{i-1}} = \alpha_{t_{i-1}} \dot{\bm{U}}_i \bm{x}_\theta(\bm{x}_{t_i}, t_i) + \sigma_{t_{i-1}} \bm{\epsilon}_i$。对 DAPS，先解优化问题 $\hat{\bm{x}}_0^{t_i} = \arg\min_{\bar{\bm{x}}_0} \big( r_{t_i} \|\bar{\bm{x}}_0 - \tilde{\bm{x}}_0^{t_i}\|^2 + \|\bm{y} - \mathcal{A}(\bm{U}_i \bar{\bm{x}}_0)\|^2 \big)$，再用类似的 stochastic 采样到下一步。
    - 设计动机：DPS 的梯度 trick 和 DAPS 的解耦轨迹都是为 pixel-space 设计的，作者只在 measurement operator 前加个 $\bm{U}_i$ 上采器，就让它们在子空间和切换点都自洽——这是一个非常工程化但优雅的"算子改造"。

2. **SubDAPS++ 的 deterministic 切换 + CG measurement**:

    - 功能：把 SubDAPS 在低 timestep 的伪迹和迭代成本同时砍掉。
    - 核心思路：(a) Deterministic 切换。定义 $h = \min\{i: d_{i-1} \neq d_i\}$（最后一次维度变化的 index）；当 $i < h$（已稳定到全分辨率）且 $\|\bm{x}_\theta(\bm{x}_{t_i}, t_i) - \hat{\bm{x}}_0^{t_i}\|^2 \leq \tau$ 时，用 $\bm{x}_{t_{i-1}} = \alpha_{t_{i-1}} \hat{\bm{x}}_0^{t_i} + \frac{\sigma_{t_{i-1}}}{\sigma_{t_i}}(\bm{x}_{t_i} - \alpha_{t_i} \hat{\bm{x}}_0^{t_i})$ 走 deterministic 更新，否则继续注入噪声。(b) 共轭梯度替 Langevin。SubDAPS 的 measurement update 解 Eq. (16)用 Langevin，慢且只适合可微算子；SubDAPS++ 改成 Fletcher-Reeves CG：每步用一阶 Taylor 展开线性化 $\mathcal{A}(\bm{U}_i(\bar{\bm{x}}_0^{(j)} + \alpha \bm{d}_j))$ 得到闭式 line search $\alpha_j = (\bm{g}_j^\top \bm{d}_j) / (r_{t_i} \bm{d}_j^\top \bm{d}_j + \bm{\omega}_j^\top \bm{\omega}_j)$，搜索方向用 $\bm{d}_{j+1} = \bm{g}_{j+1} + \frac{\bm{g}_{j+1}^\top \bm{g}_{j+1}}{\bm{g}_j^\top \bm{g}_j} \bm{d}_j$ 更新。
    - 设计动机：低 timestep 处随机噪声会破坏 diffusion prior 形成 artifact，作者用"维度已稳定 + 预测已收敛"两个条件刻画"该切换确定性更新"的时机；CG 的好处是闭式 line search 既适用线性也适用非线性 measurement，比 Langevin 既快又少超参。

3. **不增加网络评估的 predictor-corrector 轨迹修正**:

    - 功能：在主循环结束后再过一遍轨迹，把 stochastic 主循环引入的偏差修掉。
    - 核心思路：借鉴 UniPC 的二阶 corrector 形式：$\bm{x}_{t_{i-1}}^c = \frac{\sigma_{t_{i-1}}}{\sigma_{t_i}} \dot{\bm{U}}_i \bm{x}_{t_i}^c - \left(\sigma_{t_{i-1}} \frac{\alpha_{t_i}}{\sigma_{t_i}} - \alpha_{t_{i-1}}\right) \hat{\bm{x}}_0^{t_{i-1}} - \sigma_{t_{i-1}} \mathcal{I}_i \frac{\hat{\bm{x}}_0^{t_{i-1}} - \dot{\bm{U}}_i \hat{\bm{x}}_0^{t_i}}{\lambda_{t_{i-1}} - \lambda_{t_i}}$，其中 $\lambda_t = \log(\alpha_t/\sigma_t)$ 是 half log-SNR。这一步完全复用主循环里缓存的 $\hat{\bm{x}}_0^{t_i}$，不调用神经网络。
    - 设计动机：主循环中为防发散加了噪声，但噪声会拉宽轨迹偏差；UniPC 风格的 corrector 用解析公式把状态拉回更"标准"的扩散轨迹，几乎免费就能涨点。
