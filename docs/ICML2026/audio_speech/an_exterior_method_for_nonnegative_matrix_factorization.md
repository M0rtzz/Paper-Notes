...
1. **SVD 流形上的 ADMM 正交旋转**:
    ...
    - 核心思路：把 $U^\star$ 和 $V^\star$ 竖直拼接成 $W$，求解 $\min_{Z,R}\sum_{ij}h(Z_{ij})$，约束 $Z=WR$ 且 $R^\top R=I$，其中 $h(q)=\max(0,-q)$ 惩罚负元素。ADMM 交替更新 $Z$、正交 Procrustes 形式的 $R$ 和乘子 $Y$；$Z$ 的每个元素有分段闭式更新，$R$ 通过 $W^\top B$ 的 SVD 得到。
