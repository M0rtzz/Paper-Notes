...
## 方法详解
AAM的输入可以是图像、视频帧序列和音频信号，输出是对应的空间注意力图。整体流程不是简单把多个专用模型拼起来，而是把不同数据集、模态和任务都映射到同一个条件注意力空间里：视觉输入由冻结的DINOv3骨干提取特征，并通过LoRA适配注意力预测；文本提示由CLIP编码，用来描述数据集或任务的认知条件；音频由Wav2CLIP编码后映射到视觉语义空间；视频则在逐帧视觉表征基础上进入Fokker-Planck Dynamics模块进行时间演化。

### 整体框架
模型先获得视觉特征、文本条件和可选音频特征。文本和视觉表示被提升到Lorentz双曲空间中，模型学习一个通用注意力锚点、文本条件点和视觉实例点之间的偏序关系。随后，双曲解码器根据文本条件的层级深度和视觉实例相对文本条件的方向，调节多尺度特征与空间聚焦权重。若输入是视频，FPD模块会对每一帧的注意力分布做漂移、扩散和观测校正，使输出既保留当前帧证据，也吸收前后帧的时间一致性。最后，模型通过KLD、CC、SIM等显著性损失加上双曲蕴含损失进行训练。

### 关键设计
1. **双曲层级蕴含建模**:

    - 功能：把通用注意力、任务文本条件和具体视觉实例组织成从一般到具体的层级结构。
    - 核心思路：模型在Lorentz双曲空间中学习偏序关系 $z_{img} \preceq z_{txt} \preceq z_{anc}$，其中 $z_{anc}$ 表示通用注意力锚点，$z_{txt}$ 表示任务或数据集文本条件，$z_{img}$ 表示视觉输入实例。通过双曲蕴含锥约束，文本条件必须落在通用锚点允许的锥形区域中，视觉实例也要与文本条件保持层级一致。
    - 设计动机：注意力条件不是无序的类别标签，而是有抽象层级的认知调制。双曲空间的指数体积增长更适合表达这种树状或层级语义，因此比给每个数据集配独立参数更能支持跨场景泛化。

2. **几何感知的双曲解码器**:

    - 功能：把双曲空间中的层级条件转回像素级显著图，并让不同粒度的条件控制不同尺度和空间区域。
    - 核心思路：文本点到原点的双曲距离表示条件的专门化深度，模型据此选择多尺度算子权重 $w_k=softmax_k(-d_L(z_{txt},\mu_k))$。同时，视觉实例相对文本条件的测地方向 $\Delta$ 用来计算空间聚焦权重，使解码器知道应该强调哪些位置和细节。
    - 设计动机：如果只把文本作为普通条件向量拼接到视觉特征中，模型很难显式区分“泛化任务描述”和“精细场景意图”。几何解码器把层级深度对应到尺度调制，把语义偏移方向对应到空间聚焦，更贴近人类注意力随任务目标收缩或扩展的过程。

3. **Fokker-Planck视频注意力动力学**:

    - 功能：把静态帧注意力扩展为任意长度视频的连续时间演化，避免固定窗口视频模型只能输出窗口末帧、计算冗余高的问题。
    - 核心思路：视频注意力分布 $u_t$ 被看成定义在空间域上的概率密度，其时间变化由漂移、扩散和校正三部分组成。漂移项用双向时间自注意力在全时间域聚合信息，扩散项用二阶中心差分平滑高频噪声，校正项像Kalman增益一样在动态预测和当前观测 $u_t^{obs}$ 之间自适应折中。
    - 设计动机：视觉注意力在视频里既会跟随运动目标迁移，也需要保持时间连续性，还不能被错误传播拖垮。Fokker-Planck视角把这些需求拆成可解释的物理过程，比单纯堆叠时序Transformer更容易解释，也支持逐帧高效预测。
