...
把 $r \equiv 0$ 代回去，问题就退化成"在满足期望约束下保持与预训练模型最近"的约束生成，于是约束生成与约束微调被统一进同一个框架。CFO 的核心做法是借用经典的增广拉格朗日：在外层维护罚因子 $\rho_k$ 与拉格朗日乘子 $\lambda_k$ 两个对偶变量，把整个带约束的问题切成 $K$ 轮无约束子问题；每一轮内层都退化成一个"把原奖励换成增广奖励 $f_k$"的标准 KL 正则微调，可以原样套用已有的求解器，外层则根据约束违反量自适应地收紧或放松惩罚。
