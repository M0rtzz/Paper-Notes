...
### 整体框架
设有 $k$ 个已训练好的 GFlowNet，第 $i$ 个针对奖励 $R_i(x)$，对应前向策略 $p_{i,F}$、到达概率 $u_i$ 和终止分布 $p_i\propto R_i$。给定一个目标组合 $p_M^*(x)\propto\mathcal{G}(p_1(x),\dots,p_k(x))$，框架要在推理阶段构造一个混合前向策略 $p_{M,F}$，使得它按 DAG 顺序逐步采样得到的终止分布 $p_M(x)$ 等于（或逼近）$p_M^*$，整个过程不引入任何新参数、不调用任何额外训练。

整体 pipeline 是：在每个非汇状态 $s$ 上，对每个候选孩子 $s'$，用每个模型的"到达概率 × 前向转移概率"作为加权样本馈入 $\mathcal{G}$，再除以局部归一化常数得到混合策略的转移概率；然后按这个新策略采样下一个状态，直到走到终止态。

### 关键设计

1. **基于到达概率的混合策略 (Reaching-Weighted Mixing)**:

    - 功能：把多个单目标前向策略融合成一个统一的多目标策略，是整个框架的核心算子。
    - 核心思路：对状态 $s$ 的每个孩子 $s'$，定义 $p_{M,F}(s'\mid s)=\mathcal{G}\bigl(u_1(s)p_{1,F}(s'\mid s),\dots,u_k(s)p_{k,F}(s'\mid s)\bigr)/N_M(s)$，其中 $N_M(s)$ 是该状态下所有孩子上述量之和的归一化常数。直觉上 $u_i(s)$ 衡量"模型 $i$ 把多少概率质量送到了 $s$"，谁送得多谁就更熟悉这块状态空间，应该在 $s$ 上获得更高的发言权。
    - 设计动机：朴素的均匀混合（即把 $u_i$ 全设为 $1$）等价于"集成基线"，会忽略每个模型对当前轨迹的相对重要性；用到达概率加权才能让每个模型的"贡献"与它在当前轨迹上的"在场感"成正比，是后续理论恰好闭合的关键。

2. **可计算的到达概率读取 (Flow-Based $u_i$ Computation)**:

    - 功能：在大状态空间下高效地获取 $u_i(s)$，使得混合策略可在推理时即查即用。
    - 核心思路：直接按递推 $u(s)=\sum_{s_*}u(s_*)p_F(s\mid s_*)$ 求 $u_i$ 需要枚举所有父状态，不可行。作者利用恒等式 $u_i(s)=F_i(s)/Z_i$，其中 $F_i(s)$ 是状态流、$Z_i=F_i(s_0)$ 是总流。如果训练目标显式参数化了流（flow matching / detailed balance / sub-trajectory balance），就直接读 "Model $F$" 路线；如果训练用的是 trajectory balance 等不显式建模 $F$ 的目标，就借助 detailed balance 条件 $F_i(s')p_{i,B}(s\mid s')=F_i(s)p_{i,F}(s'\mid s)$，沿当前轨迹累乘 $\prod_j p_{i,F}/p_{i,B}$ 在线恢复 $F_i$，称为 "DB $F$" 路线。
    - 设计动机：覆盖了 GFlowNet 训练目标的两大类，使框架不挑预训练模型的训练范式，最大化了"积木复用"的现实场景。

3. **线性标量化的精确性 + 非线性失真因子分析 (Distortion-Factor Analysis)**:

    - 功能：解释混合策略到底能恢复成什么分布、什么时候是精确解、什么时候只是近似。
    - 核心思路：把混合策略走完一条完整轨迹得到的边缘分布展开成 $p_M(x)=\delta(x)\cdot\mathcal{G}(p_1(x),\dots,p_k(x))$，其中失真因子 $\delta(x)=u_M(x)/N_M(x)$ 衡量与目标分布的偏差。对线性标量化 $\mathcal{G}=\sum_i\omega_i Z_i p_i$，文章证明 $\delta(x)$ 在所有 $x$ 上恒为常数 $1/Z_M$，于是 $p_M$ 精确等于 $p_M^*\propto\sum_i\omega_i R_i$（命题 4.1）；对调和均值、对比、带温度 $\beta\neq 1$ 的标量化等非线性算子，$\delta(x)$ 不再恒为常数，但实验显示它在 $\mathcal{G}$ 取值较大的高密度区接近 $1/Z_M$，于是 $L_1$ 误差主要被低密度区贡献，整体仍然可控。
    - 设计动机：把"什么时候精确、什么时候近似"刻画在同一个数学量 $\delta(x)$ 里，使框架既保留了线性情形的可证明保证，又给出了非线性情形的可解释退化模式，避免把所有非线性当黑箱处理。

### 损失函数 / 训练策略
完全无训练。所有计算发生在推理时：每次从 $s_0$ 出发逐步采样下一个状态，每步现场读取或在线累乘得到 $u_i$、按上述加权公式构造混合分布、归一化后采样。推理开销仅相当于对 $k$ 个模型各做一次前向传播加一次组合，远低于训练任何一个新模型的代价。
