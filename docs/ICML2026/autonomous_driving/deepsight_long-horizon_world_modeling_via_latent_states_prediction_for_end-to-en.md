...
### 整体框架
DeepSight 以 Qwen2.5-VL-3B 为底座，定义统一生成-理解模型 $M_{\text{uni}}$。输入：当前 $N$ 路环视图像 $\mathbf{I}_t$、历史 4 帧 $\mathbf{I}_{t-\tau}$、ego 状态 $T_{\text{ego}}$、目标点 $T_{\text{target}}$、5 个 World Queries $\mathbf{Q}_{\text{world}}=[q_0,\dots,q_4]$。输出三个东西在**单次前向**里完成：未来 5 帧 BEV 隐式特征 $\mathbf{F}=[f_0,\dots,f_4]$（每帧间隔 $\Delta t=0.5\text{s}$，覆盖未来 2 秒）、自适应 CoT 文本 $T_{\text{cot}}$、未来轨迹点 token $\mathbf{P}_t$。推理时按 $p(\mathbf{F}\mid\mathcal{X})\cdot p(T_{\text{cot}}\mid\mathcal{X},\mathbf{F})\cdot p(\mathbf{P}_t\mid\mathcal{X},\mathbf{F},T_{\text{cot}})$ 顺序解码。

### 关键设计

1. **Driving World Model with Latent BEV Prediction**：

    - 功能：让 VLM 一次性并行预测未来 5 帧的 BEV 隐式语义特征，把"短视频预测"换成"长时语义预测"。
    - 核心思路：给模型注入 5 个 learnable BEV-shape 的 World Queries $q_k\in\mathbb{R}^{h_{\text{bev}}\times w_{\text{bev}}\times d_{\text{bev}}}$，每个对应一个未来时刻 $t+k\Delta t$；通过 transformer 的自注意力让 query 与历史/当前多视图特征交互，并行回归隐式特征 $f_k$。GT 用 DINOv3 在 BEV 渲染图（或语义分割图）上提取 $f_i=\phi_{\text{dino}}(I_i^{bev})$，损失用 MSE。这里关键是"特征级 distillation"——不是预测像素或 codebook token，而是预测一个语义浓缩的 latent 表示，因此预测 5 帧的开销远小于自回归图像重建。
    - 设计动机：codebook/VAE 方法把模型容量浪费在纹理细节上；DINOv3 自监督特征天然 encode 物体、形状、语义关系，更符合驾驶任务真正在意的"前面那辆车要不要让"。并行查询机制避免自回归式逐帧解码的累积延迟，让长时预测变得可行。Table 3 的对照实验直接打脸 VAE：单帧 VAE 仅 27.75 DS、五帧反而跌到 14.66；而 DINOv3 单帧 74.79、五帧再涨到 86.57，证明只有"语义 + 长时"组合才有效。

2. **Adaptive Chain-of-Thought**：

    - 功能：模型自己决定是否触发文本 CoT 推理，避免每帧都调 LLM 的高开销。
    - 核心思路：在观察完输入并预测完 $\mathbf{F}$ 后，模型自回归生成 CoT 文本：$T_{\text{cot}}=M_{\text{uni}}(\mathbf{I}_t, \dots, \mathbf{Q}_{\text{world}}\mid \mathbf{F})$；若判断当前场景简单（普通跟车/直行），就输出一个占位 token $T_{\text{cot}}^{\emptyset}$ 跳过推理；若是长尾（如复杂红绿灯、施工区、紧急车辆让行），就生成完整结构化思考链。GT 数据用 Qwen3-VL-235B 跑一个三步自动标注 pipeline（场景复杂度评估 → 复杂度驱动的外部知识检索 → 行为决策），合成 1.3M 条 Bench2Drive 标注。
    - 设计动机：CoT 虽强但慢，全帧调用会让系统失去实时性。Table 5 显示单纯启用 CoT（无 world model）DS=69.87，远不如单独 world model（DS=84.52）；二者结合才到 86.23——说明 CoT 是锦上添花而非主菜。"按需触发"既保留长尾场景下的社会常识/逻辑推理能力，又把额外延迟压在 4% 左右（Table 6）。

3. **Unified Tokenized Training Pipeline**：

    - 功能：把轨迹 waypoint、CoT 文本、未来特征三种异构输出统一进 VLM 的 token 训练框架。
    - 核心思路：把 BEV 网格离散化为 $K$ 个 cell，每个 waypoint $p_i=(x_i,y_i)$ 量化成 token index $t_i\in\{1,\dots,K\}$，于是轨迹预测变成 token 分类，可用 CE 损失。总损失是三项加权和：$L=\lambda_{\text{traj}}L_{\text{traj}}+\lambda_{\text{cot}}L_{\text{cot}}+\lambda_{\text{world}}L_{\text{world}}$，其中 $L_{\text{traj}}$、$L_{\text{cot}}$ 都是 CE，$L_{\text{world}}=\mathrm{MSE}(\mathbf{F},\mathbf{F}^{\text{gt}})$。
    - 设计动机：让模型在统一 token 空间里学习"轨迹 = 一种特殊文本"，可以无缝复用 VLM 的预训练对齐能力；World Query 走特征回归路径（MSE），保持 dense 表征精度。这样的混合 loss 设计避免了"文本 head + 轨迹 head + 特征 head"之间的解耦优化困难。
