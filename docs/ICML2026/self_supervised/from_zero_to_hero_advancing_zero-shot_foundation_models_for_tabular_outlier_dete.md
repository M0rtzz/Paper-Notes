...
1. **Mixed Synthetic Priors (GMM + SCM + Copula)**:
    ...
    - 功能：覆盖真实表的多模态、因果结构、任意边缘 + 依赖三大类生成机制，并为每类配对应的 outlier archetype。
    - 核心思路：GMM 模多模态 inlier 并产 contextual subspace outlier（沿子空间放大方差让点落到 90 百分位外）；SCM 用一个随机连边的 MLP 当因果图，结构方程 $X_j = f_j(X_{Pa(X_j;G)},\epsilon_j)$ 沿拓扑序前向采样得 inlier，并设计两种 outlier ——"measurement outlier"对某变量注入 $\epsilon_j\sim\mathcal{N}(0,s)$ 大方差扰动沿因果传播，"structural outlier"直接把某条边权置零或反向制造因果机制突变；Copula 由 Sklar 定理 $F(x_1,\dots,x_d)=C(F_1(x_1),\dots,F_d(x_d))$ 解耦边缘与依赖，边缘从 Gaussian/Beta/Exp/Student-t/Power-law/Log-logistic 池子里采，依赖用 Gaussian/Vine copula，outlier 分"probabilistic"(把 $u_j$ 推向 $\{0,1\}$ 边界) 与"dependence"($u_j:=1-u_j$ 翻转或维度置换打破依赖) 两类。
    - 设计动机：作者 Figure 4 (Table 5) 做了对角实验显示三类 prior 互补——训 Copula 测 SCM 只有 0.76，反之 0.77，对角线全在 0.92 以上；且 kNN/IForest 在所有 prior 上都达不到对角线最优，说明这些 prior 既"非平凡"又"互补"，单用任一种都会丢覆盖。

2. **Self-Evolving Curriculum (SEC) 多臂老虎机**:

    - 功能：在 $P\cdot K$ 个 (prior, dim-bin) 上自动分配采样权重，把训练注意力压在"既不太难也不太易"的可学习 arm 上，无需人工排课程。
    - 核心思路：把 task category 当 MAB 的 arm，初始权重 $Q_0(c)$ 均匀，每步按温度缩放的当前权重采 arm；reward 用 batch 内点级 centered cross-entropy 的方差 $r(c)=\tfrac{1}{n_c}(l_i-\text{mean}(\{l_1,\ldots,l_{n_c}\}))^2$，当 batch 内一半"高置信对"一半"高置信错"时最大、当所有点概率都 0.5 时为零；再把 reward 回传给 arm 权重更新，并用 pace scheduler 卡一个时变 loss 阈值，只对低于阈值的点反传梯度。
    - 设计动机：作者观察到朴素 mixed-prior 训练在 ADBench 上 (0.898) 比 GMM-only (0.920) 还差，定位为"其他 prior 的大梯度淹没 GMM 信号"，并且 SAM/类似的几何正则解决不了，因为根本问题是任务采样分布而不是单步几何；用 batch loss 方差当 reward 一举把"data uncertainty (全 0.5)"判定为低优先级 (而不是高难度)，把"过简单 (全 0)"和"过难 (全大 loss)"都降权，恰好命中 PFN 多任务训练所需的"learnable"信号，最终在 GMM+Mixed+ADBench 三栏都把准确率从 (0.87/0.94/0.90) 拉到 (0.93/0.97/0.93)。

3. **推理时多视图 Ensemble**:

    - 功能：把超过 Transformer context 限制 ($n>5K, d>100$) 的大数据集塞进零样本推理，并对小数据集也提供 ensemble 增益。
    - 核心思路：对每个测试样本做 50 次 (subsample context, feature bagging) 前向，把 outlier score 取平均；subsampling 选不同 in-context inlier 子集，feature bagging 抽不同维度子集，两个维度的随机化降低了某个特定 context/feature 子集偶然不利的方差。
    - 设计动机：PFN 的 Transformer 受限于 attention 二次复杂度，无法把任意大表一次性塞进 context；同时 ensembling 在 OD 里本身就是经典涨点术 (Aggarwal & Sathe 2017)，PFN 的 ensemble 又因为只做前向、不用反复训练 base learner，所以是真"低成本 ensemble"。
