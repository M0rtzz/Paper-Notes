...
### 理论分析：语义流形的规则化
NTP 目标对 $h_t$ 的约束主要来自其与目标 token embedding 的点积——Hessian 秩亏、允许表示在零空间中任意漂移。NITP 通过引入**正曲率**规则化这些方向：NITP Hessian $H_{\text{NITP}}(h) \approx \frac{1}{r^2} P_{\perp u}$（超球面切空间投影）；在所有正交方向引入严格正曲率，强制表示维持结构化几何。
