...
description: >-
  [ICML2026][医疗NLP][FHIR] 作者提出一个把自由文本病例转成符合 HL7 FHIR R4 标准的"分阶段 LLM + 术语接地 + 修复循环"流水线，并据此从 MedCaseReasoning 构造出 1408 条结构化合成病例数据集 MedCase-Structured（成功率 82.5%…
tags:
  - "ICML2026"
  - "医疗NLP"
  - "FHIR"
...
### 整体框架
输入是一份英文自由文本病例（来自 MedCaseReasoning），输出是一个通过校验的 HL7 FHIR R4 patient bundle，可选地按"诊断屏蔽模式"剔除诊断结论以做下游评测。整条流水线包含 4 个串联阶段：

1. **Extraction**：LLM #1 把自由文本抽成一份扁平的中间结构（人口学、症状、体征、生命体征、化验、用药、操作、既往史），并对每个抽出来的条目保留**原文 verbatim 引用**作为溯源。
2. **Terminology Grounding**：用一个内部维护的术语库（聚合 OMOP + SNOMED CT / LOINC / RxNorm / CVX）对 LLM 抽出的所有编码做接地：先关键词检索，再用 SapBERT 嵌入到 FAISS 索引里做语义相似度匹配，按三档余弦阈值"接受 / 替换 / 拒绝"。
3. **FHIR Synthesis and Validation**：LLM #2 把已接地的中间表示按 HL7 R4 模板组装成 FHIR 资源（Patient / Encounter / Condition / Observation / MedicationRequest / Procedure / DiagnosticReport / FamilyMemberHistory / AllergyIntolerance / Immunization），失败的校验项再回灌给 LLM 重写，至多 3 轮；后处理用规则补全缺失资源、归一化单位/日期/状态字段。
4. **Diagnosis Hiding**：根据 NONE / HIDDEN / EXPLICIT / FULL 四档模式对 bundle 里的诊断结论做配置化屏蔽——NONE/HIDDEN 模式下先按编码 + 子串硬过滤，再由 LLM #3 在所有叙述字段上做语义扫描，把缩写、暗示性结论、同义说法这些"隐性泄漏"再 redact 一次。

所有 LLM 调用统一用 Claude (claude-sonnet-4-20250514)、temperature 0 以求可复现。

### 关键设计

1. **术语接地用 SapBERT + FAISS + 三阈值判定**:

    - 功能：把 LLM 抽到的 SNOMED CT / LOINC / RxNorm / CVX 编码硬性钉死到内部术语库的真实编码上，幻觉码当场被拒。
    - 核心思路：先按关键词检索候选，再用 SapBERT 把待修候选与术语库中所有 preferred term 的嵌入索引到 FAISS，用三档余弦相似度阈值分别决定"原码可接受 / 用最近邻替换 / 直接拒绝交给修复循环"——把"是不是合法术语"从概率匹配问题变成可调阈值的检索匹配问题。
    - 设计动机：作者明确报告 Table 2 里失败案例的最大头就是 LOINC 幻觉（183 例）+ RxNorm 幻觉（126 例）+ 药物粒度过粗（103 例），不接地完全压不住；而 SapBERT 是生物医学实体的强基线嵌入，搭 FAISS 也能保持检索时延。

2. **三段固定锚点 + 校验-修复闭环（vs agent-style）**:

    - 功能：让 text→FHIR 流程可控、可调试，并把"FHIR 校验器报错"显式注入回 LLM 完成修复。
    - 核心思路：与 Infherno 之类让 LLM 动态决定何时调工具的 agent 风格不同，本文把 LLM 固定调用三次（抽取、合成、语义泄漏扫描），中间夹"确定性术语接地 + 结构/临床一致性校验 + 规则后处理"；合成阶段如果校验失败，把错误清单返回给 LLM 重写，至多 3 轮，超出就把该病例标为转换失败。
    - 设计动机：固定锚点比 agent 更可调试与可复现（temperature 0 + 固定调用次序），且把"语法/术语合规"这种确定性的约束从 LLM 手里收回去交给规则与术语库，让 LLM 只负责"自然语言/结构化映射"这件它擅长的事。

3. **四模式可配置的诊断屏蔽 + 第三段 LLM 语义泄漏扫描**:

    - 功能：避免下游评测时模型直接从 bundle 里"读到诊断答案"，又能按需保留患者自述、家族史等合法上下文。
    - 核心思路：按 NONE / HIDDEN / EXPLICIT / FULL 四档过滤已组装的 bundle——NONE 删全部诊断结论、HIDDEN 只删主诊断、EXPLICIT 只留患者自述的状况、FULL 全保留；前两档先用编码+子串硬过滤，然后再让 LLM #3 在所有 narrative 字段上做语义扫描，揪出缩写、隐含结论、未列入同义词表的近义说法等"硬过滤抓不到"的隐性泄漏。
    - 设计动机：CDSS 评测的可信度依赖"输入里不能藏答案"；FHIR bundle 的 narrative 字段恰恰是诊断信息最容易借同义词/缩写残留的地方，单靠规则 redact 拦不住，必须再借一次 LLM 做语义层扫描。

### 训练策略
本文不训练模型，全程用现成 Claude 闭源 API + 现成 SapBERT 嵌入 + 现成 FAISS 索引，温度固定为 0 以保证可复现；没有微调，没有 RLHF，整套系统是"LLM + 检索 + 规则校验"的合成数据流水线。
...
- [\[ICML 2025\] MedXpertQA: Benchmarking Expert-Level Medical Reasoning and Understanding](../../ICML2025/medical_nlp/medxpertqa_benchmarking_expert-level_medical_reasoning_and_understanding.md)
