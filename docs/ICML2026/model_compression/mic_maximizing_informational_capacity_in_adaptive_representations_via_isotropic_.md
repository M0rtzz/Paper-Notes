...
1. **逐层、逐截断维度**地拿出 hidden states，按截断点 $d$ 切成 prefix $\mathbf{H}_{\mathrm{pre}}\in\mathbb{R}^{B\times L\times d}$ 和 residual $\mathbf{H}_{\mathrm{res}}\in\mathbb{R}^{B\times L\times d_{\mathrm{res}}}$（$d_{\mathrm{res}}=d_{\mathrm{full}}-d$）。
2. 对每个 $(l,d)$ 同时算 $\mathcal{L}_{\mathrm{SCR}}^{(l,d)}$ 和 $\mathcal{L}_{\mathrm{SIR}}^{(l,d)}$，求和后得到 $\mathcal{L}_{\mathrm{align}}$。
3. 总目标 $\mathcal{L}_{\mathrm{total}}=\mathcal{L}_{\mathrm{MRL}}+\gamma\mathcal{L}_{\mathrm{align}}$。
