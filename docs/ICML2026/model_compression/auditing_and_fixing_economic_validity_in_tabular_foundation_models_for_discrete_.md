...
## 方法详解
论文的方法由两个部分组成：先用黑盒查询审计TFM是否满足行为有效性，再用两阶段行为适配器修复这些问题。它的关键不是提出更复杂的分类器，而是重新定义表格基础模型在政策选择任务中的角色。TFM不再直接输出最终选择概率，而是作为一个观测级信息源，为受约束的效用模型提供修正信号。

### 整体框架
对每个样本 $x_i$ 和备选项 $k$，模型构造一个效用 $V_k(x_i)$。这个效用包含两部分：第一部分 $V_k^{struct}(x_i)$ 是标准离散选择模型，包含备选项常数、时间成本系数、费用系数和人口统计交互项；第二部分来自表格基础模型预测概率 $q_k(x_i)$，包括一个标量权重项 $\alpha\log q_k(x_i)$ 和一个小网络 $g_k(\mathbf{q}(x_i))$。最终选择概率仍由Logit公式 $P_k=\exp(V_k)/\sum_j\exp(V_j)$ 给出。

作者在Swissmetro和LPMC两个交通方式选择数据集上评估。先训练或调用Mitra、TabPFN v2得到每个样本的预测概率，再把这些概率缓存下来供适配器使用。行为审计和适配器训练都不需要访问TFM内部参数，这让方法可以用于黑盒或近黑盒的表格基础模型。

### 关键设计
1. **黑盒行为审计**:

    - 功能：用最小侵入方式检查TFM是否满足离散选择任务的经济逻辑。
    - 核心思路：审计包含三类测试。单调性测试把某个备选项的成本或时间增加观测范围的1%，然后检查该选项预测概率是否下降；时间价值测试用系数比或有限差分估计 $VOT=\beta_{time}/\beta_{cost}$；可用性测试统计模型分给不可用备选项的平均概率。
    - 设计动机：政策模型必须在干预输入时表现合理，而不只是拟合历史选择。黑盒扰动测试可以直接暴露TFM把相关性当因果关系的问题，例如把高费用和高收入人群偏好混在一起。

2. **受约束结构效用项**:

    - 功能：把经济理论中的方向性约束写进模型参数化，使价格和时间增加必然降低对应备选项效用。
    - 核心思路：结构效用项采用MNL规格，并把时间、成本系数写成 $\beta=-\exp(\theta)$。无论优化器怎样更新无约束变量 $\theta$，系数始终为负。因此价格上升会降低效用，VOT可以直接从结构系数解析计算；不可用选项则通过设定 $V_k=-\infty$ 获得零概率。
    - 设计动机：把约束做成数学构造比事后惩罚更可靠。惩罚项只能降低违规概率，不能保证政策干预时永远满足单调性；而负指数参数化给出了硬保证。

3. **两阶段TFM修正项**:

    - 功能：在不改变经济参数的前提下吸收TFM的准确率优势。
    - 核心思路：Stage 1把修正项固定为零，只训练结构效用，相当于标准MNL。Stage 2冻结结构参数，只训练 $\alpha$ 和小网络 $g_k$，让TFM概率解释MNL没有捕捉到的非线性残差。由于政策响应由冻结的时间、成本系数决定，TFM修正项只能改变样本层面的截距式偏移，不能颠倒价格响应方向。
    - 设计动机：如果端到端联合训练，优化器会把信号快速塞进TFM修正分支，导致结构系数收缩到接近零，VOT变成两个近零数的比值。两阶段训练先锁住经济含义，再追求预测精度。

### 损失函数 / 训练策略
训练目标是离散选择的负对数似然。Stage 1只优化结构效用参数，并通过负指数参数化保证时间和成本系数为负。Stage 2在结构参数冻结后优化TFM修正项，其中 $\alpha$ 衡量TFM概率整体可信度，$g_k$ 从完整概率向量中学习备选项级残差。

实验中TFM概率在训练、验证和测试切分上预先计算并固定。这个设定与政策分析习惯一致：分析者在干预价格或时间时，会保持个体其他观测信息不变。适配器中“其他信息”也包括TFM对该观测的整体判断，因此反事实响应完全由结构效用项控制。
