...
1. **输出空间投影诊断过度累积**:
	...
	- 功能：把“共享知识是否被多次累加”转成一个可计算的谱子空间指标。
	- 核心思路：对第 $r$ 个左奇异向量 $u^r$，每个任务的响应是 $a_i^r=(u^r)^\top\Delta W_i$，合并响应是 $a_{merge}^r=\sum_i a_i^r$。如果 $a_{merge}^r$ 投影到 $a_i^r$ 的系数 $s_i^r$ 大于 1，就说明其他任务在同一方向上提供了正向内积，任务 $i$ 的该方向被放大了。
	- 设计动机：权重空间逐参数比较很难发现这种结构性问题，而输出空间基直接对应合并矩阵的响应方向，更适合衡量任务行为是否被过度放大。

2. **子空间级奇异值校准**:

	- 功能：把多个任务的投影系数聚合为单个子空间缩放因子。
	- 核心思路：论文定义 $\gamma^r=K/\sum_i \max(\alpha,s_i^r)$。当很多任务在该子空间里都有 $s_i^r>1$ 时，分母变大，$\gamma^r<1$，对应奇异值被下调；当没有系统性过度累积时，$\gamma^r$ 接近 1。
	- 设计动机：单个任务的系数可能受噪声或任务偏好影响，而跨任务聚合能让校准只在共享方向普遍被放大的时候生效，避免过度修正。

3. **只改奇异值、不改奇异向量的后处理**:

	- 功能：让 SVC 可以接在 TA、TIES、DARE、TSV-M、Iso-C/Iso-CTS 等已有合并器之后。
	- 核心思路：SVC 保留 $U$ 与 $V$，只用 $\tilde{\sigma}^r=\gamma^r\sigma^r$ 替换原奇异值，再重构 $\Delta\tilde W_{merge}=\sum_r \tilde{\sigma}^r u^r(v^r)^\top$。
	- 设计动机：这样不会引入新的训练目标，也不依赖校准集；额外代价主要是一次离线 SVD，适合数据不可用或只想对已有合并模型做轻量修正的场景。
