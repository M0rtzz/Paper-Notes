...
### 整体框架
DINOv3-PCA-TabPFN 是一个三阶段流水线，整套图像 → 表格 → 决策的转换链可以理解为"把一张图变成一行带标签的表格记录，再让一个通用的表格基础模型来判 real/fake"。

第一阶段是视觉编码：图像统一以 RGB 加载，短边缩到 256 像素再中心裁到 $224\times 224$，按 ImageNet 统计做归一化；DINOv3 ViT-B/16 在 eval 模式下推出一个 $N\times 768$ 的 CLS 特征矩阵，整个主干不做任何图像域微调。第二阶段是 Incremental PCA 降维：在训练集特征上拟合一个 PCA，把 768 维映射到 500 维结构化行 $z(h(x))\in\mathbb{R}^{500}$，并对测试集套用同一组主成分。第三阶段是 TabPFN 的上下文推理：把降维后的训练行（含 0/1 标签）与待测行同时塞进 TabPFN，由其先验数据拟合网络在不做梯度更新的情况下输出 real/fake 预测——本版 TabPFN 限制最多 10,000 行、500 维。

整条 pipeline 的关键之处不是某一个模块本身先进，而是它把"适配 = 换上下文行"这件事彻底落地：当遇到新生成器，只需在新生成器的少量真假样本上重抽 PCA 系数、再把它们塞进 TabPFN 的上下文集合，编码器与判别器都不需要再走梯度。

### 关键设计

1. **冻结 DINOv3 当通用视觉编码器**:

    - 功能：提供一组对生成器无偏的高质量视觉表征，把 AIGC 检测从"端到端学一个图像分类器"解耦成"取证特征 + 通用判别器"。
    - 核心思路：直接拿 DINOv3 ViT-B/16 的 CLS token 当全图表示，主干始终是 eval 模式，从不做任何 real/fake 信号的微调；表征学习完全由自监督的 DINOv3 预训练承担。论文对比了 DINOv2、DCT 频域特征、FFT 径向频谱特征等多种替代编码，DINOv3 在准确率/精度/召回/F1/AUC 五项上都更稳。
    - 设计动机：取证文献早就发现"分类头容易过拟合到生成器指纹"，作者把这种过拟合风险锁在了 TabPFN 一侧的上下文里——即使新生成器与训练集差异巨大，编码器输出的语义/纹理表征仍然成立，只需通过上下文样本告诉 TabPFN 新分布的判别边界即可。

2. **PCA 500 维硬适配 TabPFN 输入限制**:

    - 功能：把 DINOv3 的 768 维 CLS 特征压到 500 维，刚好卡在该版 TabPFN 的特征上限内，同时去掉无关方差。
    - 核心思路：在训练集特征上拟合 IncrementalPCA，得到 $z(h(x))\in\mathbb{R}^{500}$，对测试集套用同一组主成分；评估脚本按四种生成器感知协议（Multi-Multi、Multi-Single、Single-Multi、Single-Single）构造平衡的训练/测试集，PCA 始终在训练侧拟合避免泄漏。
    - 设计动机：作者明确把"换一组 PCA 系数"也算作适配新场景的一部分——这一步比训练分类头快几个数量级，并且让 TabPFN 的输入维度永远满足约束，使得后续 in-context 推理是"真正不训练"的。

3. **TabPFN 上下文推理替代梯度训练判别头**:

    - 功能：把 real/fake 判别从"基于梯度训分类头"转成"基于一组带标签上下文行做贝叶斯式推断"。
    - 核心思路：TabPFN 是 Hollmann 2023 提出的先验数据拟合网络，输入"上下文集合"与"待测行"后通过单次前向得到分类后验，本质上近似贝叶斯式表格推断而非传统训练；本文用 $k\in\{25,30,75,150,300,625\}$ 张/生成器的极小训练规模，测试集统一 10,000 张（5,000 真 + 5,000 假，或 5,000 真 + 8×625 假）。
    - 设计动机：从适配范式角度，"换上下文 vs 训一个新头"在小数据区间的优势差距是数量级级别的；从落地角度，这条 pipeline 把"添加新生成器"变成一次轻量的特征提取+上下文拼装操作，整体跑通时间从分钟级降到秒级。

### 损失函数 / 训练策略
本文不在视觉侧做任何训练：DINOv3 与 TabPFN 都是 frozen 模型，PCA 是闭式拟合。所谓"训练"等价于"准备一份新的上下文表格"——这正是范式优势所在。LATTE 等基线按各自协议正常训练，在 $k\in\{150,300,625\}$ 下与本方法做公平对比。
