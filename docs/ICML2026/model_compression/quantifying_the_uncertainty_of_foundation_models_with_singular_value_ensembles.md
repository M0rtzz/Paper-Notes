...
**3. 基底质量决定 SVE 收益的尺度律：backbone 越强，SVE 越赢**

SVE 的全部前提是"预训练奇异向量已经是有意义的语义方向"，这个假设强弱直接决定方法该不该用，所以作者把它做成一条可证伪的经验定律。在 CIFAR-100 上固定 ViT-S 架构、只换三种 backbone（随机初始化、DINOv1、DINOv2），观察 SVE 相对其他集成的精度增益，结果单调递增：随机初始化下没有"有意义的基底"可共享，SVE 反而最差、不如让成员各自从零学；DINOv1 上 SVE 已反超主流隐式集成；DINOv2 上甚至超过 Deep Ensemble。这条尺度律既给方法划清了适用边界（弱 backbone 别用），也顺势解释了为什么 SVE 在 BERT、LLaMA 这类强预训练大模型上能给出最佳校准。
