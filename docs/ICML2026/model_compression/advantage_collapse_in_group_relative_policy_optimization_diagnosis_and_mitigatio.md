...
flowchart TD
    A["问题 q：旧策略采样 G 条回复<br/>验证器给二值奖励 r_i ∈ {0,1}"]
    A --> B["优势坍塌率 ACR（诊断）<br/>逐组算 σ_R，σ_R&lt;τ 判坍塌<br/>batch 统计坍塌组占比 ACR"]
    B --> C["动态触发阈值 τ_adapt<br/>按策略改进 ΔJ 调整：停滞才加力"]
    C -->|"ACR ≤ τ_adapt 或该组未坍塌"| D["标准 GRPO 优势<br/>Â_i =（r_i − μ_R）/ σ_R"]
    C -->|"ACR > τ_adapt 且该组坍塌"| E["分层虚拟样本 AVSPO<br/>构造 K=⌈G·ACRᵅ⌉ 个虚拟 reward<br/>全对降序衰减 / 全错正 anchor"]
    E --> F["合并 R∪V 重算 μ′、σ′<br/>仅对 G 条真实样本算增强优势 Â′_i"]
    D --> G["GRPO clipped 目标更新策略"]
    F --> G
```
...
虚拟 reward 既然改了 advantage 的归一化，就必须控制“何时介入、介入多强”，否则阈值定太低会过度干预、放大方差，定太高又会错过早期坍塌。AVSPO 把触发阈值 $\tau_{adapt}$ 初始设为 0.5，并按训练是否还在改进（用平均 batch reward 的变化 $\Delta J$ 判断）动态调整，使它更像一个“当前没进步才加力”的按需修复器，而不是常驻的奖励整形项；同时虚拟样本数量随 $ACR^\alpha$（默认 $\alpha=0.5$）缩放，坍塌越普遍才补越多。偏差控制靠一个硬约束 $K\leq G$：虚拟样本数永远不超过真实样本，从而限制对原始梯度的扰动；论文进一步证明坍塌组里被恢复出来的 advantage 是标准化变量，幅度有界（$|\hat{A}'_i|\leq\sqrt{G+K}$）。这本质是一个偏差-方差权衡——相比让整组梯度彻底停滞，引入这点有界偏差是划算的，这正回应了“虚拟 reward 会不会把策略带偏”的担忧。
