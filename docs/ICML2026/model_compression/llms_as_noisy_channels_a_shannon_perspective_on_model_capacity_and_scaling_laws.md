...
它既要在高信噪比区还原出经典单调 scaling，又要在噪声反超信号时自然长出 U 形退化。容量本身是个抽象量，要落到能测的指标上，本文再补一条互逆桥接 $\mathcal{L}=1/C_{\text{LLM}}$：容量越大测试 loss 越低（$C\to\infty$ 时 $\mathcal{L}\to0$），这个倒数关系还天然带来「早期 loss 高、小幅涨容量就能大降 loss，收敛后要再降 loss 得付出巨大容量」的非线性。后面所有实验里被拟合、被外推的，正是这个 $1/C_{\text{LLM}}$。
...
**3. 多源扰动的统一编码：一套公式直接重拟合，不为每种扰动另设退化项**

加性高斯噪声、量化、不同领域的 SFT，看似互不相干，经典 perturbation-aware law（QiD、Law of Precision）的做法是给基础 law 各加一个带扰动强度 $X$ 的退化项（如 $dN^{\alpha'}D^{\beta'}X^\gamma$）——换一种扰动就得换一套退化建模。本文不另设项：所有扰动都被看成抬高了同一个噪声项 $\mathcal{N}$、压低了有效 SNR，于是面对任意扰动场景都沿用同一条容量公式，只是在该场景的扰动数据上把 $a,b,c,d,e,\alpha,\beta,\gamma,\delta$ 这九个参数重新拟合一遍。正因为不依赖针对某种扰动手工设计的退化形式，框架的通用性才体现在后面 Gaussian / 量化 / 三类 SFT 共六类 perturbation 场景里全部能 fit 上。
