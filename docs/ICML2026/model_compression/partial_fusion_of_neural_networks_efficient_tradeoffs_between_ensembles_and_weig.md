...
### 整体框架
输入：两个 $L$ 层前馈网络 $A, B$，每层权重分别为 $W_\ell^A, W_\ell^B$，以及一个插值因子 $\lambda \in [0,1]$ 和一个"留独立比例" $\alpha \in [0,1]$。输出：一个**部分融合网络** $C$，每层规模介于单网络 ($\alpha=0$) 与集成 ($\alpha=1$) 之间。流程分四步：(i) 为每一层 $\ell$ 的神经元抽取特征向量 (激活向量或权重列)，并赋予概率分布 $\mu_\ell^A, \mu_\ell^B$；(ii) 求解 $\alpha$-partial OT 得到部分耦合矩阵 $\tilde\pi_\ell^{A,B}$，自动给出"被匹配"和"独立保留"的两组神经元；(iii) 把匹配部分按 OT Fusion 的方式做加权融合，独立部分原样保留为新的通道；(iv) 整层权重 $W_\ell^C$ 被拼成一个 3-block 结构：左上是 $A$ 的独立块，右下是 $B$ 的独立块，中间是融合块，块间过渡用 OT 派生的随机核 $K_\ell^{A\to B}, K_\ell^{B\to A}$ 缝合。

### 关键设计

1. **Partial OT 配对 (Partial OT Fusion)**:

    - 功能：决定每一层哪些神经元被合并、哪些留独立，并给出合并所用的对齐矩阵。
    - 核心思路：在概率测度视角下，每个神经元被看作支撑点，神经元相似度由特征向量的欧氏距离衡量。普通 OT Fusion 要求耦合 $\pi$ 把 $\mu^A$ 完整运到 $\mu^B$；本文改用 $\alpha$-partial OT，把约束放成 $\sum_j \tilde\pi[i,j]\le\mu^A[i]$、$\sum_{i,j}\tilde\pi[i,j]=1-\alpha$，得到 $\tilde\pi_\ell^{A,B}=\arg\min_{\pi\in\Pi_\alpha}\int\|x-y\|^2\,\pi(dx,dy)$。未被运输的质量对应的神经元就是"独立神经元"；剩下被匹配的部分归一化后通过 $K_\ell^{A\to B}=(\pi^{A,B})^T/\mu^A$、$K_\ell^{B\to A}=\pi^{A,B}/\mu^B$ 得到随机核。Partial OT 在计算上和普通 OT 等价、不增加额外难度。
    - 设计动机：避免被迫合并功能不相似的神经元；把"是否合并"变成可调的连续旋钮 $\alpha$，让模型大小和精度的 tradeoff 在层级粒度上变得平滑可控。

2. **3-block 融合层 + 全局 fixed-point 对齐**:

    - 功能：把"独立保留 + 加权融合"统一到一张可计算的权重矩阵里，并跨层联合优化对齐。
    - 核心思路：层 $\ell$ 到 $\ell+1$ 的权重矩阵 $W_\ell^C$ 被划成 $3\times 3$ 的块结构 (对应 $A$ 独立、融合、$B$ 独立)；对角块直接拷贝原权重或按 $W_\ell^C=(1-\lambda)W_\ell^B+\lambda K_{\ell+1}^{A\to B} W_\ell^A[F,F] K_\ell^{B\to A}$ 融合，非对角块通过 $K$ 完成"独立分支 ↔ 融合分支"的过渡。沿用 Ainsworth et al. (2023) 的思想，作者把所有层的对齐写成全局目标 $(\pi_\ell^{A,B})_\ell=\arg\min\sum_\ell\int\|x-y\|^2\pi_\ell(dx,dy)$，用**固定点迭代** (fixed-point) 逐层更新——每步只优化一个 $\pi_\ell$、其余冻结，保持每步仍是一个 (部分) OT 问题。
    - 设计动机：当特征本身用权重列定义时，第 $\ell$ 层的对齐依赖第 $\ell+1$ 层的对齐，必须跨层联合解才合理；fixed-point 比 greedy 实测精度更高 (Figure 5(a) vs 5(b))。3-block 结构让独立通道既可与融合通道线性相加 (走 $K$)，又互不干扰，是把"集成"和"融合"在张量上同时表达的唯一干净方式。

3. **广义剪枝视角与聚类实现 (Generalized Pruning via Clustering)**:

    - 功能：把"对集成做剪枝"和"对单网络做剪枝"统一起来，给出一种不只能删除、还能线性组合神经元的剪枝形式。
    - 核心思路：对一个大模型 $E$ (比如 ensemble，也可以是任何过参数化的网络)，引入到小模型 $S$ 的随机核 $K_\ell^{E\to S}\in\mathbb{R}^{n_\ell^S\times n_\ell^E}$、$K_\ell^{S\to E}$，并定义 $W_\ell^S:=K_{\ell+1}^{E\to S} W_\ell^E K_\ell^{S\to E}$。若 $K$ 是 0/1 行/列随机矩阵就退化为标准剪枝；本文把它一般化成由**聚类**导出的随机核——求解 $\pi_\ell^{E,S}=\arg\min_{\pi\in\Pi(\mu^E,*_m)}\int\|x-y\|^2\,\pi(dx,dy)$，其中第二边际只支撑在最多 $m$ 个簇心上 (对均匀 $\mu^E$ 就是 K-means)；簇内神经元被线性组合到同一个簇心，得到 $S$。同时作者证明 partial fusion 是把这套广义剪枝施加到 $A\oplus B$ 的 ensemble 上、并加上"只合并跨网络神经元、最多两两组合"的归纳偏置后的特例。
    - 设计动机：标准剪枝只有"删除"一种操作 (会丢处理步骤)，剪枝+OT 后处理只有"线性合并"一种操作 (会模糊处理步骤)；这两种误差源在不同压缩率下代价不同。聚类型广义剪枝同时允许两种操作并自动权衡，因而在 MLP-on-MNIST 上明显优于二者 (见 Figure 1(b))。在实现上，Lloyd K-means 在相关压缩区间表现很差，作者改用**层次聚类**得到更接近全局最优的解。
