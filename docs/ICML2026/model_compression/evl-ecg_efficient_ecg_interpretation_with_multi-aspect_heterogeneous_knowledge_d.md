...
总损失 $\mathcal{L}_{\text{total}} = (1-\alpha)\mathcal{L}_{\text{CE}} + \alpha(\lambda_m \mathcal{L}_{\text{mhca}} + \lambda_r \mathcal{L}_{\text{rel}} + \lambda_{\text{ot}}\mathcal{L}_{\text{ot}})$。

三个 KD 损失各司其职：
- $\mathcal{L}_{\text{mhca}}$：student 隐藏态对齐 attention-weighted teacher 上下文
- $\mathcal{L}_{\text{ot}}$：视觉 token 用 Sinkhorn 软对齐保全局拓扑
- $\mathcal{L}_{\text{rel}}$：distance + angle 关系匹配保内在几何流形

### 关键设计

1. **多头交叉注意力对齐（MHCA）**:

    - 功能：把 teacher 的 dense $L_t$-token 表征自适应聚合到 student 的 compact $L_s$-token 表征
    - 核心思路：student 隐藏态 $H_s$ 作 query，teacher 隐藏态 $H_t$ 作 key/value；$\hat H_t = \text{Concat}(\text{head}_1, \dots, \text{head}_h) W^O$，每个 head 算 $\text{Softmax}((H_s W_i^Q)(H_t W_i^K)^\top/\sqrt{d_k})(H_t W_i^V)$；对齐损失 $\mathcal{L}_{\text{mhca}} = \frac{1}{B \cdot L_s}\sum \|H_s - \hat H_t\|_2^2$
    - 设计动机：硬一一对齐在 $L_s \neq L_t$ 时不可行；MHCA 让 student 动态"摘取"对自己最有用的 teacher 信息（如局部异位搏动、细微 ST 偏移）；理论上等价于熵正则化下的 OT barycentric projection

2. **最优传输视觉特征匹配（OT-VFM）**:

    - 功能：在视觉 token 层做软分布对齐，保留 12 导联的全局空间拓扑
    - 核心思路：teacher / student 视觉 token 视为均匀经验分布 $\mu, \nu$；解 entropic OT $\mathbf{P}^\star_\varepsilon = \arg\min_\mathbf{P} \langle \mathbf{P}, C\rangle - \varepsilon \mathcal{H}(\mathbf{P})$（Sinkhorn）；蒸馏损失 $\mathcal{L}_{\text{ot}} = \sum_{i,j} P^\star_{\varepsilon, ij} \|t_i - s_j\|_2^2$
    - 设计动机：ECG 图像中位置即诊断意义（V1 在 RA 附近、V5/V6 反映左室）；逐点匹配会把胸导联误对到肢体导联；OT 提供软对齐既允许 token 长度不同又保全局空间结构（运输方案隐含"哪个 student token 接哪个 teacher 区域"）

3. **几何关系内部匹配（Geometric Intra-Architecture Relation）**:

    - 功能：在每个架构内部分别算距离/角度关系矩阵，对齐两者——保 teacher 的内在诊断逻辑
    - 核心思路：对隐藏态序列 $H$ 定义两个关系势——距离势 $\psi_D$（均值归一化的逐对欧氏距离）和角度势 $\psi_A$（逐对余弦相似度）；$\mathcal{L}_k = \frac{1}{B \cdot L_s^2} \sum \|\psi_k(\hat H_t^{(i)}, \hat H_t^{(j)}) - \psi_k(H_s^{(i)}, H_s^{(j)})\|^2$，$k \in \{D, A\}$；总 $\mathcal{L}_{\text{rel}} = \tfrac{1}{2}(\mathcal{L}_D + \mathcal{L}_A)$
    - 设计动机：ECG 诊断本质上靠**结构拓扑和时序关系**（P → QRS 间隔、ST 朝向、各段比例）；点级对齐重建 token 但丢全局几何；关系势把 teacher 的"诊断聚类几何"（学到的 latent space 中类似 arrhythmia 距离/角度结构）保下来
