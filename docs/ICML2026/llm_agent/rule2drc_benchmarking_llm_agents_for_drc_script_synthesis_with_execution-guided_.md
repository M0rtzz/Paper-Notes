...
整体上论文是两部分的组合：一个 benchmark + 一个 BoN 选择算法。理解 SplitTester 的关键是先想清楚它在什么样的输入输出协议下工作。

### 整体框架
**Rule2DRC 任务格式**：每个任务是一个四元组 $\langle r_i, c_i, \{x_{ij}\}_{j=1}^{m_i}, \{\phi(x_{ij}, c_i)\}\rangle$，其中 $r_i$ 是 NL 规则、$c_i$ 是 ground-truth DRC 脚本、$\{x_{ij}\}$ 是该任务的私有评测版图、$\phi(\cdot, \cdot) \in \{0,1\}$ 是 DRC 引擎给出的"是否违规"判定函数。Agent $f(\cdot)$ 只看到 NL 规则 $r_i$、KLayout API 文档以及自己生成或调用得到的中间产物，输出脚本 $f(r_i)$。成功率定义为对所有评测版图都满足 $\phi(x_{ij}, f(r_i)) = \phi(x_{ij}, c_i)$ 的任务比例，错误率定义为生成脚本编译或运行报错的任务比例。

**SplitTester 在 BoN 中的角色**：先用任意 sampler 采 $N$ 个候选脚本 $\mathcal{C} = \{c_1, \dots, c_N\}$（如 $N \in \{10, 15, 20\}$），SplitTester 在不接触 ground-truth 标签和私有评测版图的前提下，自行生成测试版图、执行所有候选、并最终选出一个 $c^* \in \mathcal{C}$。

### 关键设计

1. **Rule2DRC 基准的三段式组成与执行级评测**:

    - 功能：提供 1000 个 NL→DRC 翻译任务 + 13921 个评测版图，且不向 agent 暴露评测版图。
    - 核心思路：310 条规则取自 SkyWater130 PDK，是真实开源 PDK 的合规规则；为覆盖 7nm 以下节点的多层级联约束，又用 GPT-5.2 起草、人工逐条复核 490 条多层规则；再额外补 200 条「专门触发出现次数 <5 的语法构造」的规则以覆盖 KLayout DSL 全谱。每条任务都要求版图集合中既有"应通过"也有"应违规"的样例，并尽量包含"违规幅度仅卡在阈值边缘"的硬负样本。打分用 $\text{SuccessRate}(f) = \frac{1}{n}\sum_i s_i$，$s_i = 1$ 当且仅当 $\forall j: \phi(x_{ij}, f(r_i)) = \phi(x_{ij}, c_i)$；任何代码相似度都不参与判分。
    - 设计动机：传统基准用代码相似度评分，会把语义等价但语法不同的正确解误判（Figure 3 的三种等价写法），且 DRC-Coder 把评测版图当输入会鼓励过拟合一组固定版图。执行级打分 + 评测版图私有，是这个领域第一次同时做到的设置。

2. **SplitTester：在不可区分簇上做定向测试生成**:

    - 功能：给定 $N$ 个候选脚本和初始测试预算 $B_0$、追加预算 $B$，输出选中脚本 $c^*$。
    - 核心思路：先让 LLM 生成 $B_0$ 个初始 (版图, 期望输出) 对，执行所有候选后按"输出向量是否完全相同"把候选聚成簇 $\{C_i\}$，每个候选的分数 $s(c) = \frac{1}{|T|}\sum_{(x,\phi^*) \in T} \mathbb{1}[\phi(x, c) = \phi^*]$ 是它在所有测试上与期望输出一致的比例。然后迭代：每轮挑 $i^* = \arg\max_i s_i |C_i|$（同时高分且大）的簇，从该簇随机抽 $K{=}3$ 个代表喂给 test generator，让它专门生成能让这 3 个代表行为分化的新测试；执行新测试 → 把新测试加进 $T$ → 全体重打分 + 重聚类 → 继续。若连续 $P{=}1$ 次没能把目标簇拆开就早停，避免在生成不出区分版图的卡点上空耗预算。
    - 设计动机：CodeMonkey 砍到 Top-3 后再补测试，会在区分性测试出现前就把正确候选丢掉；S* 把新测试聚焦在已经被分开的簇之间，对仍纠缠在一起的高质量候选反而不投入。把"测试预算"流向最危险的簇（$s_i$ 大说明大概率是正确解扎堆的簇，$|C_i|$ 大说明内部混入错解的可能性更高）能让每条新测试的边际信息量最大化。$K = 3$ 是为了控制 prompt 长度，避免把整簇候选都塞给 test generator 反而分散注意力。

3. **「期望标签 + 最终 judge LLM」的两阶段决断与上下文工程**:

    - 功能：在 LLM 自生成的期望标签可能有噪声的前提下，依然能稳定地选出最优脚本。
    - 核心思路：每条生成的测试都附带一个"LLM 自认为正确"的期望输出 $\phi^*(x)$，整个过程用它做打分和聚类；最终判决阶段不直接用分数取 Top-1，而是把 Top-3 候选 + 它们之间真正存在差异的那部分测试 $X_\Delta = \{x : \exists c_a, c_b \in C_3, \phi(x, c_a) \neq \phi(x, c_b)\}$ 一起喂给 judge LLM，让它在更窄、更有针对性的证据集上做最后选择。此外作者把 KLayout 官网爬下来预处理成约 60K token 的 API 文档塞进所有 LLM 调用的上下文，这一项让 GPT-OSS-120B 的 pass@1 提升 20+ 百分点、pass@20 提升 40+ 百分点。
    - 设计动机：消融实验显示，去掉 final judge 成功率从 58.0% 掉到 55.5%、去掉期望标签掉到 57.1% —— 两者贡献是互补的：期望标签提供持续的打分信号让聚类有方向，judge LLM 抵御期望标签本身的噪声。把 API 文档常驻 context 则承认了一个现实 —— DRC DSL 太小众，纯参数化记忆靠不住，必须把权威语法手册当工具书放在手边。
