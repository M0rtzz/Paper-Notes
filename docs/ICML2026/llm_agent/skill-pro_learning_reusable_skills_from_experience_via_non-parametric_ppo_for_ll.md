...
整套系统**不动 LLM 一根参数**，所有"学习"都在 Skill 文本的增删改上完成。

### 关键设计

1. **Skill-MDP 与 Skill 三元组结构**:

    - 功能：把"经验"从被动叙事重塑为可被 agent 直接调用的程序单元，解决 C1（可执行性）。
    - 核心思路：经典 MDP 扩展为 $\mathcal{M}_\Omega = (\mathcal{S}, \mathcal{A}, \Omega, P, R, \gamma)$，每个 Skill 形式化为 $\omega = \langle \mathcal{I}_\omega, \pi_\omega, \beta_\omega \rangle$——$\mathcal{I}_\omega$ 是自然语言写的激活条件（"任务刚开始、没有反馈时"），$\pi_\omega$ 是有序的执行步骤（"Step 1: 建立假设空间; Step 2: 生成最大化信息增益的探索动作"），$\beta_\omega$ 是自然语言终止条件（"首次探索动作执行且收到反馈后终止"）。决策时层级策略分解为 $\pi_\Omega(\omega_t, a_t \mid s_t) = \mu(\omega_t \mid s_t, \Omega)\,\pi_\text{LLM}(a_t \mid s_t, \omega_t)$，让 agent 在状态空间上做"宏动作"调度，原子动作交给 LLM。
    - 设计动机：现有记忆系统存的是 trajectory/insight，调用时 LLM 还要重新"推"出怎么做；显式三元组让 Skill 自带触发逻辑和退出逻辑，可被 $\mu$ 直接路由，把"重新推导"的成本一次性摊销到训练阶段。

2. **Non-Parametric PPO：语义梯度 + PPO Gate 信任域验证**:

    - 功能：在不更新 LLM 权重的前提下，提供既有方向性（gradient-like）又有安全性（trust-region）的 Skill 更新机制，解决 C2 和 C3。
    - 核心思路：(a) **语义梯度**——对调用过 $\omega$ 的每条轨迹 $\tau_i$，让 LLM 做 hindsight attribution，输出结构化的自然语言修改建议 $g_i = (g_i^{(\mathcal{I})}, g_i^{(\pi)}, g_i^{(\beta)})$，分别针对激活/执行/终止三条；batch 内用 LLM 做 aggregation 抽取共性失败模式，得到 $\bar{g}_\omega$；用 LLM 算子 $\oplus$ 把 $\bar{g}_\omega$ 应用到原 Skill 得到候选 $\omega' = \omega \oplus \bar{g}_\omega$。(b) **PPO Gate**——把 frozen LLM 视为随机策略，对历史轨迹每一步计算重要性比 $\rho_t(\omega') = \pi_\text{LLM}(a_t \mid s_t, \omega') / \pi_\text{LLM}(a_t \mid s_t, \omega)$，advantage 用 return-to-go 减 running baseline $\hat{A}_t = G_t - \bar{R}$，最后算 PPO 风格的 clipped surrogate $L^\text{CLIP}(\omega') = \hat{\mathbb{E}}_\tau [\frac{1}{|\tau|}\sum_t \min(\rho_t \hat{A}_t,\ \text{clip}(\rho_t, 1-\epsilon, 1+\epsilon)\hat{A}_t)]$；$N_c$ 个候选里取 $L^\text{CLIP}$ 最大且 >0 的那个，正分意味着候选在信任域内确实优于旧 Skill。
    - 设计动机：纯靠 LLM "改写 Skill" 容易产生 hallucinated/不稳定的变体，TextGrad 那种只优化静态变量的方案在序列决策上没有"是否真能带来回报"的反事实评估；PPO 的 clipped objective 天然提供"小步、可验证"的更新语义——这里把它从参数空间搬到 Skill 文本空间，重要性比衡量"换 Skill 后历史动作概率怎么变"，clip 防止激进改写，从而把信任域思想原汁原味地映射到非参数场景。

3. **基于在线 Advantage 的 Skill 池维护**:

    - 功能：在固定容量 $K$ 下，让"好 Skill 长期留存、差 Skill 自动淘汰"，避免 Skill 池随训练单调膨胀。
    - 核心思路：对轨迹 $\tau$ 把奖励 advantage 化 $\tilde{r}_t = r_t - \bar{r}$，每个 Skill 在轨迹中的增益定义为它激活期间的平均 advantage $G(\omega; \tau) = \frac{1}{|\mathcal{T}_\omega(\tau)|}\sum_{t \in \mathcal{T}_\omega(\tau)} \tilde{r}_t$；只有轨迹级回报时 fallback 到 $\tilde{r}_t \equiv (R(\tau)-\bar{R})/|\tau|$。在线维护累计增益 $G_b$ 和调用次数 $N_b$，得到 $\text{Score} = G_b / \max(1, N_b)$。剪枝规则：(i) 分数 ≤0 直接删；(ii) 语义冗余删；(iii) 仍超容量按分数升序淘汰。
    - 设计动机：随着 baseline $\bar{R}$ 自我抬升，"曾经有用"的 Skill 会自然变成 0 分被淘汰，形成进化压力；同时 advantage 形式比"成功率"更能容忍单次失败和奖励稀疏，特别适配 LLM agent 任务里奖励粒度不一的情况。
