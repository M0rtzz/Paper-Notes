...
## 方法详解
Video2GUI 是一个 pipeline，分三个流水段加一个两阶段训练。

### 整体框架
输入：YouTube 5 亿视频元数据。输出：WildGUI 数据集，每条样本是 $(u, e)$，其中 $u$ 是高层任务指令、$e=(u,a_1,o_1,\dots,a_n,o_n)$ 是带高分辨率截图与精确坐标的步骤级轨迹。三段流水：(A) Coarse-to-Fine 视频筛 → 4.16M 高质量 tutorial（约 30 万小时）；(B) Trajectory Extraction → 任务级 instruction + 时间戳级动作描述 + reasoning；(C) Action Spatial Grounding → 用 $\pm 0.5$s 三帧高清原图精确定位 $b_t=g_\phi(o_{t-0.5s},o_t,o_{t+0.5s},\tau_t)$。最后用 WildGUI 在 Qwen2.5-VL/Mimo-VL 上做继续预训练，再用开源数据集做 post-training 微调。

### 关键设计

1. **Coarse-to-Fine 视频筛**：

    - 功能：在 5 亿规模上以分级成本筛掉绝大多数噪声，避免把存储/算力浪费在 vlog、新闻评论上。
    - 核心思路：第一层「Meta Info Filtering」只看标题/描述/关键词，先用 DeepSeek-V3 标 10k 样本做监督，再蒸馏到一个 Qwen2.5-7B + 分类头，分类全量 5 亿元数据，得到 ~2 千万候选；第二层「Video Quality Scoring」对每条候选取前一分钟视频，让一个 omnimodal 模型（Qwen2.5-Omni 蒸馏自 Gemini 3 Pro 在 200 小时上的打分）按三维评分：Topic Relevance（是否在教 GUI 操作）、Instruction Clarity（讲解是否清晰）、Screen Recording Quality（录屏清晰稳定与否），最终保留 4.16M 视频（约 30 万小时）。
    - 设计动机：直接对 5 亿视频做内容分析的存储/算力都是 PB 级，根本不可行；元数据筛是几乎零成本但只能保证「主题相关」，要保证「教学质量」必须看视频本身。两级蒸馏（DeepSeek-V3 → Qwen2.5-7B；Gemini 3 Pro → Qwen2.5-Omni）把强模型的判断能力以可承担成本规模化。

2. **滑动窗口 + 历史记忆的轨迹抽取**：

    - 功能：用 Gemini-3-Pro 把可能长达一小时的视频解析成多段 instruction-trajectory 对，每个动作带准确时间戳、动作类型、参数和 reasoning。
    - 核心思路：把长视频切成不超过 4 分钟的连续段 $\{S_1,\dots,S_M\}$，处理第 $j$ 段时输入当前段帧 + 前序段抽取结果 $D(S_{1:j-1})$ 作为文本上下文，让模型保持跨段长记忆，能识别跨段任务或依赖前序操作的任务；同时要求每个动作输出一条「low-level instruction」（视觉锚定的文本描述），为后续 grounding 做铺垫。最终每个视频得到 $D(V)=\{(u^{(k)},e^{(k)})\}_{k=1}^N$，多个独立任务实例并存。
    - 设计动机：长视频远超 VLM context window，单段处理会破坏跨任务依赖；而既有方法（TongUI 用前景/背景检测、VideoAgentTrek 用逆动力学）依赖低层视觉线索，只能捕捉短时相关，缺乏「为什么这样做」的任务意图。用强 VLM + 历史记忆 + 高/低层 instruction 双输出，把「理解 + 解释」一站完成。

3. **三帧高分辨率空间 grounding**：

    - 功能：把视频中动作的「大致时刻」映射到「像素级目标」，弥补视频压缩导致的坐标误差。
    - 核心思路：对每个时间戳 $t$ 的动作，从原视频里取三元组 $O_t=\{o_{t-0.5s},o_t,o_{t+0.5s}\}$，让 Gemini-3-Pro 基于 low-level instruction $\tau_t$ 判断该帧上动作目标是否可定位，并预测边界框或屏幕坐标 $b_t=g_\phi(o_{t-0.5s},o_t,o_{t+0.5s},\tau_t)$。0.5s 偏移大致匹配单个 GUI 动作的平均时长，保证三帧覆盖动作完整的「pre/at/post」三态。
    - 设计动机：trajectory extraction 阶段输入的是被压缩过的视频帧，分辨率不足以定位像素级 UI 控件（如「点击 Shoes for men」按钮）；但 grounding 是 GUI agent 训练的关键监督信号，必须像素精度。解耦后让 VLM 先做长视距理解，再用高分辨率原图做局部 grounding，既省 token 又保精度。
