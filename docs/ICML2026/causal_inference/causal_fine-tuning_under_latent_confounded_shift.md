...
description: >-
  [ICML 2026][因果推理][因果微调] 本文提出 Causal Fine-Tuning (CFT)：在标准 BERT 微调里嵌入一个 SCM 启发的"高级稳定特征 $C$ + 低级混杂敏感特征 $\Phi$"分解，并用 front-door 风格的 do-calculus 调整公式做预测…
tags:
  - "ICML 2026"
  - "因果推理"
  - "因果微调"
...
1. **基于 SCM 的因果识别 scaffold**：

    - 功能：把"高级稳定语义 $C$ + 低级混杂特征 $\Phi$ + 不可观测混杂 $U_S,U_\Phi$ + 环境 $\sigma$"组合成一张图（Fig.2(b)），论证 $p(y\mid \mathrm{do}(x))$ 在单域数据下可被 $p(y\mid\Phi,C)$ + 边际 $p(x)$ 写出来。
    - 核心思路：假设 $\sigma$ 只通过 $S_1$ 影响 $R_1$，$\Phi$ 对 $Y$ 的影响完全经过 $C$（front-door 结构），并借 Von Kügelgen 的可识别性定理把 $C$ 写成 $p(C\mid R_0)\approx p(C\mid R_1)$ 的不变投影。
    - 设计动机：用因果图明确说明"训练-测试分布差异等价于 $p(u\mid x;\sigma)$ 的变化"，这才把"对所有 $\sigma$ 鲁棒"的问题转化成"按 $\mathrm{do}(x)$ 的最大熵默认环境训练"，避免了 minimax 的过度保守和不可识别。

2. **双视图因果表示对齐 $\mathcal{L}_C$**：

    - 功能：把冻结模型的句向量 $R_0$ 和微调模型的 $R_1$ 投影到同一个稳定因果空间 $C$。
    - 核心思路：最小化 $\mathcal{L}_C=\mathbb{E}\,\|p(c\mid r_0)-p(c\mid r_1)\|_2^2 - H(p(c\mid r_0)) - H(p(c\mid r_1))$，第一项强制跨视图不变性，两个负熵项防止表示坍缩到常数。
    - 设计动机：把"预训练 vs 微调"这对天然形成的"伪环境对"当作 IRM 多域信号的替代品，只用单域数据就能逼出跨域稳定分量；如果用 $(R_1,R_1)$ 同视图替换（failure mode 实验），方法立刻退化成 SFT。

3. **局部 patch 特征 $\Phi$ + Front-door batch shuffle**：

    - 功能：从微调模型的 embedding 层抽局部低层特征作为伪相关的"代理"，并通过 do-calculus 调整把 $\Phi$ 的因果路径切断。
    - 核心思路：把 token 序列切成 10 个不重叠 patch，做均值池化后过 MLP 得到 $\Phi=\mathrm{MLP}(\frac{1}{10}\sum_i p_i)$；预测时用 mini-batch 内随机 shuffle $\Phi$ 得到 $\Phi'\sim \hat{p}_B(\Phi)$，求 $\mathbb{E}_{\Phi'}[p(y\mid C,\Phi')]$，$K=20$ 次蒙特卡洛平均。
    - 设计动机：embedding 层最接近"原始词形/数据源"等低级伪相关线索（LeCun 等的低层特征观察），shuffle $\Phi$ 等价于断掉 $\sigma\to S_1\to R_1\leftrightarrow\Phi\leftrightarrow Y$ 这条 active collider 路径，把预测从 $p(y\mid x)$ 拽回到 $p(y\mid \mathrm{do}(x))$。
