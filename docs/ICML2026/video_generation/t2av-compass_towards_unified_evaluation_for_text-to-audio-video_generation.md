...
### 整体框架
三个核心环节：（1）**数据构建**——分类法驱动的混合流水线生成 500 条高复杂度多模态提示；（2）**双层评估框架**——低层信号级指标 + 高层 MLLM 诊断；（3）**系统基准**——评估 15 个前沿 T2AV 系统，提供维度级对比与失败模式分析。

### 关键设计

1. **分类法驱动的多源数据构建**:

    - 功能：确保提示集合的语义覆盖度、复杂度与物理合理性。
    - 核心思路：（1）聚合高质量社区提示（VidProM / Kling / LMArena / Shot2Story），余弦相似度 0.8 去重得到 70K；（2）用 Gemini-2.5-Pro 重写增加视觉 / 运动 / 音响 / 摄影学约束，平均长度从 54 → 154 tokens，约束点从 5 → 10；（3）引入 100 条高保真 YouTube 4-10 秒片段做视频反演确保现实动态对齐；（4）人工三轮验证过滤不合规 / 过长 / 不合理。
    - 设计动机：避免纯文本幻觉，提升真实场景代表性；多源融合扩大语义空间覆盖；分类法驱动有 8 种隐喻类型 + 5 个注释维度 + 4 大复杂度因子。

2. **双层评估框架**:

    - 功能：从低层信号完整性到高层语义合理性的端到端诊断。
    - 核心思路：
      - **目标评估**——视频质量 VT（DOVER++）+ VA（美学预测器 V2.5）；音频质量 AA + SQ（NISQA）；跨模态对齐 T-A（CLAP）+ T-V（VideoCLIP-XL-V2）+ A-V（ImageBind）+ 时间同步（Synchformer）。
      - **主观评估**——指令遵循（IF）用 Gemini-2.5 生成结构化 QA 清单按 7 维度 × 17 子维度验证；真实感（RE）含 MSS（运动平滑）/ OIS（物体完整）/ TCS（时间连贯）+ AAS（音频伪影）/ MTC（质感一致）。
    - 设计动机：信号指标快速可靠但粗粒度；MLLM 判断捕捉细微语义但耗时——二者互补。

3. **结构化检查表 + MLLM-as-Judge 协议**:

    - 功能：把抽象指令映射为可验证、可追溯的约束检查。
    - 核心思路：从 500 条提示自动生成"指令遵循"和"真实感"两类检查表；对每条检查，要求 MLLM **先输出推理文本再给 1-5 分评分**，保存为 JSON 便于错误分析。
    - 设计动机：先推理后评分协议增强诊断可解释性，支持细粒度失败归因；避免黑盒打分的可信性问题。
