...
1. **Flow Matching 即 DAE 的重新解读**：

    - 功能：把"采样过程"重新定义成一对可反复调用的算子——Predict 算子 $D_\theta(z_t,t):=z_t+(1-t)u_\theta(z_t,t)$ 把任意噪声层级的 $z_t$ 一步映到干净预测 $\hat z_1$；Perturb 算子 $R_\epsilon(z,t):=tz+(1-t)\epsilon$ 把任意干净样本加噪回噪声层级 $t$。
    - 核心思路：作者证明 $\mathcal{L}_{\text{FM}}$ 等价于带权 DAE 重构目标 $\mathbb{E}\bigl[\lVert\hat z_1^\theta-z_1\rVert_2^2\bigr]/(1-t)^2$，因此训练完成的 flow matching 模型在任何固定 $t$ 上都是一个合格的去噪自编码器，可以无限次反复使用而不需要重训。
    - 设计动机：这把 flow matching 从"沿 ODE 单向行走"的力学视角，切换到"对每个 $t$ 都可重入的 DAE"视角，这是后续一切自精炼的理论基石——没有这一步，反复 perturb-and-predict 没有任何正当性。

2. **Predict-and-Perturb (P&P) 内循环**：

    - 功能：在固定噪声层级 $t$ 内做 pseudo-Gibbs 采样，把状态 $z_t$ 一步步拉向高密度区域（即物理 & 时序合理的视频流形），再交给原 ODE solver 完成本步。
    - 核心思路：定义单次迭代 $z_t^{(k+1)}=\operatorname{P\&P}_{\epsilon_k}(z_t^{(k)},t):=R_{\epsilon_k}(D_\theta(z_t^{(k)},t),t)$，从 $z_t^{(0)}=z_t$ 出发反复 Predict→Perturb，每次用新独立采样的高斯噪声做局部重采样；之后用细化后的 $z_t^*:=z_t^{(K_f)}$ 替换原本的 $z_t$ 喂回任意 ODE solver $z_{t_{i+1}}=z_t^*+\Delta t\cdot u_\theta(z_t^*,t)$，与现有 sampler 完全即插即用。
    - 设计动机：作者强调"运动与物理在前几步就被锁定"（与 Chefer 2025、Shaulov 2025 一致），因此 P&P 只在早期 $t<0.2$ 触发就够；同一噪声层级的局部重采样保留了较大的探索半径，缓解早期 lock-in，又不会像 Restart 那样跨噪声层级跳跃造成误差累积。

3. **不确定度感知的选择性精修（Uncertainty-aware P&P）**：

    - 功能：识别出每一步里"模型还没看清"的时空区域，只对这些区域应用 P&P，保留静态背景不动，避免重复 CFG 在不变区域累积导致的过饱和与简化伪影。
    - 核心思路：用相邻两次 P&P 预测的差作为"模型自洽性"信号，得到不确定度图 $\mathbf{U}(z_{t}^{(k-1)},z_{t}^{(k)}):=\tfrac{1}{C}\lVert D_\theta(z_{t}^{(k-1)},t)-D_\theta(z_{t}^{(k)},t)\rVert_1$（按通道平均），再用固定阈值 $\tau=0.25$ 二值化得到 mask $M_{t_i}^{(k)}$；融合时直接复用上一轮已经算出来的 $z_{t_{i+1}}^{(k-1)}$，$z_{t_{i+1}}^{(k)}\leftarrow M_{t_i}^{(k)}\odot z_{t_{i+1}}^{(k)}+(1-M_{t_i}^{(k)})\odot z_{t_{i+1}}^{(k-1)}$，把"算 mask + 算 ODE 步"合并进 Predict，不引入额外 NFE。
    - 设计动机：观察到不加 mask 时 $K_f>3$ 会让 CFG 在不变背景上反复累积、引起色调偏移与水面反射夸张化；而被反复改动的运动区域因为预测每次都不一样反而不会累积。基于自身一致性的 mask 自动把"该改的运动区"和"不该动的背景"分开，无额外开销又显著减少 over-refinement。
