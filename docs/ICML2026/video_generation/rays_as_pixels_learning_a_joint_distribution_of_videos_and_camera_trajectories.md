...
1. **Raxel 表征：相机即 3 通道图像**：

    - 功能：把相机参数编码成与视频帧同形状、同通道数、能塞进预训练视频 VAE 的密集张量。
    - 核心思路：每个像素填 $\mathbf{d} + \mathbf{o}$，其中 $\mathbf{d} = R_{\text{rel}} K^{-1}\tilde{\mathbf{u}} / \|K^{-1}\tilde{\mathbf{u}}\|_2$ 是世界系单位光线方向，$\mathbf{o} = T_{\text{rel}}$ 是相机原点。和 Plücker（6 通道，$[R\mathbf{d}, R\mathbf{d}\times T]$）、raymap（6 通道）、pointmap（需要 depth）相比，raxel 是唯一同时满足"spatially aligned + 3 通道兼容预训练 VAE + 不需要 depth"的方案。位姿恢复时用 Orthogonal Procrustes 在解码后的 $\hat{R}_k$ 与参考 $\hat{R}_s$ 之间拟合 $SE(3)$，焦距用 Median-of-Ratios $\hat{f}_x = \text{median}(u \cdot \hat{z} / \hat{x})$ 鲁棒估计。
    - 设计动机：让相机能真正"沾到"预训练视频先验，而不是绕过 VAE 后挂一层 adapter。消融里把 raxel 换成 Plücker 嵌入（MLP 投影+拼接，不走 VAE）后 FID 从 7.33 暴增到 21.97、FVD 从 68 涨到 333——直接证明"共享 latent 空间"比"input-level 6 通道嵌入"高出一个量级。

2. **Decoupled Self-Cross Attention (DSCA)**：

    - 功能：在每个 DiT block 内把视频 latent $z$ 和光线 latent $r$ 的注意力解耦成 intra-modal self-attn + inter-modal cross-attn，强迫模型学跨模态依赖而不是各管各的。
    - 核心思路：第一步对 $z$、$r$ 分别做 self-attention，保证视频内部的时间平滑和轨迹自身的几何相干；第二步做 symmetric cross-attention，$z \leftarrow r$ 让视频跟随光线，$r \leftarrow z$ 让光线被视觉细化。因为 $z$ 和 $r$ 空间逐位对齐，cross-attn 的 query/key 都套 RoPE 来保住"视觉 token 必须关注到对应位置的 raxel token"这一对应关系。背后有干净的概率分解：$\log p(z, r) = \log p(r) + \log p(z|r) \equiv \log p(z) + \log p(r|z)$，self-attn 学 marginal，cross-attn 学 conditional。
    - 设计动机：作者发现单一全局 self-attn 跨整段序列时，模型倾向于"各自拟合各自"，跨模态耦合不深。DSCA 把先验和条件分离开学，cycle consistency 上 FID 从 8.69 降到 7.33、$R_{\text{err}}$ 从 0.048 降到 0.020。

3. **Flow Matching + 方向余弦损失 + 异步推理调度**：

    - 功能：让单个速度场 $v_\theta(x_t, t)$ 同时学好视频和光线两种结构差异很大的 latent，并允许推理时给两者用不同步数。
    - 核心思路：训练目标在 MSE 基础上加方向余弦项 $\mathcal{L}(\theta) = \mathbb{E}[\|v_\theta - u_t\|^2 + \lambda (1 - v_\theta^\top u_t / (\|v_\theta\|\|u_t\|))]$，$\lambda = 0.5$，单独惩罚方向偏差——视频 latent 高频纹理大，单纯 MSE 容易被幅值主导，光线 latent 的方向漂移就被淹没。因为 $z$ 和 $r$ 占不同 token 位置，推理时可以异步调度：相机可控生成里把 $r$ 固定在 $t=1$ 只去噪 $z$；位姿估计里固定 source 图 latent 只去噪 $r$，且实测 $r$ 收敛远快于 $z$，**2 步**就拿到最佳旋转精度。
    - 设计动机：光线 latent 平滑、低频、低秩；视频 latent 高频、信息密集——共享速度场必须显式建模"方向"才能让光线学得动。余弦消融里去掉这项 $R_{\text{err}}$ 从 0.020 涨到 0.058，$T_{\text{err}}$ 翻五倍到 0.094。
