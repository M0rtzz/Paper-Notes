...
2. **3D-aware analysis-by-synthesis reward**:
	...
	- 核心思路：用 Depth Anything 3 从生成视频恢复 3DGS 表示 $\Phi_{GS}$ 和估计相机轨迹 $\hat{E}$。3D reward 写成 $R_{3D}=S_{meta}+S_{recon}+S_{traj}$：$S_{meta}$ 从偏移视角渲染 3DGS 并交给 Qwen3-VL 判断结构是否可靠，$S_{recon}$ 用 $1-\text{LPIPS}$ 衡量原视频与 3DGS 重渲染的一致性，$S_{traj}$ 惩罚目标轨迹和估计轨迹之间的平移/旋转偏差。
