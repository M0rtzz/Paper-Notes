...
description: >-
  [ACL 2025][LLM 其他][模型扩展] 提出 LESA，一种基于 SVD 发现层间潜在模式并通过神经网络预测中间层参数的可学习深度扩展方法，相比启发式层复制方法获得更好的初始化和更快的收敛速度，训练成本降低一半以上。 从头训练大语言模型（LLM）需要巨大的计算资源，模型扩展（Model Scaling-Up）通过利…
tags:
  - "ACL 2025"
  - "LLM 其他"
  - "模型扩展"
