...
description: >-
  [图学习] 本文提出"软查询"形式化框架，将复杂逻辑查询扩展到不确定知识图谱（带置信度值），并设计 SRC 方法结合前向推理和后向校准来高效回答软查询，理论证明误差不会灾难性级联。 研究问题：如何在不完整且带有置信度值的不确定知识图谱（Uncertain KG）上进行复杂逻辑推理？ 现有问题：- 现有复杂查询回答（CQA）方…
tags:
  - "图学习"
---
