...
description: >-
  [ACL 2025][数据混合] 提出 AutoMixer 框架，利用训练过程中保存的检查点模型作为"数据混合器"，通过聚合多检查点的一阶影响函数近似来重新分组和加权训练数据，在八个推理基准上取得最高 1.93% 的性能提升。 - 领域现状：语言模型预训练的效果高度依赖训练数据的组成。如何为特定目标技能找到最优的数据混合比例…
tags:
  - "ACL 2025"
  - "数据混合"
  - "检查点利用"
