...
description: >-
  [模型压缩] 本文提出"教师模型归因"新问题：给定一个蒸馏后的学生模型，能否从候选教师中识别出其训练教师？发现 n-gram 相似度和困惑度不可靠，但词性（PoS）句法模板能提供有效的教师识别信号。 研究问题：在模型蒸馏（用大模型教小模型）的场景中，能否通过分析学生模型的输出来反推它的教师模型？ 现有问题：- 模型蒸馏已成为…
tags:
  - "模型压缩"
---
