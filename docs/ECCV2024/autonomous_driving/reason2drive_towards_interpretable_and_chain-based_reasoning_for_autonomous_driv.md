...
1. **领域现状**：端到端自动驾驶方法通常将系统视为黑盒，直接从传感器输入到控制信号，难以解释决策逻辑。而大型视觉语言模型（VLM）凭借广泛的世界知识和推理能力，有望实现可解释的自动驾驶决策。

2. **现有痛点**：现有驾驶相关语言数据集（Talk2Car, NuScenesQA 等）大多是简单 QA 任务，只涉及感知层面的对错判断（yes/no 或 multiple-choice），缺少描述完整决策链的推理标注。同时，BLEU、CIDEr 等文本生成指标无法衡量推理步骤与最终结论之间的因果关系。

3. **核心矛盾**：自动驾驶决策是"感知→预测→推理"的多步过程，但既缺乏标注这一链条的数据集，也缺乏评估这种链式推理质量的指标。
...
5. **切入角度**：观察到现有 VLM 难以有效利用目标级感知信息（位置、运动等），原因是缺少专用的 tokenizer 和解码器只有语言模型，无法输出精确感知结果。

6. **核心idea一句话**：用自动化标注构建大规模感知-预测-推理链数据集，配合 ADRScore 评估指标和感知增强 VLM 框架，实现可解释的自动驾驶推理。
