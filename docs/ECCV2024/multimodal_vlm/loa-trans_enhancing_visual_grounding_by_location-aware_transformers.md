...
1. **领域现状**：视觉定位（Visual Grounding）是多模态理解的核心任务，包括指代表达理解（REC，预测目标的bounding box）和指代表达分割（RES，预测目标的像素级掩码）。近年来，基于DETR框架的端到端方法成为主流，其核心组件包括视觉-语言特征融合和基于query的目标解码。

2. **现有痛点**：当前基于DETR的视觉定位方法面临两个关键问题：
...
3. **核心矛盾**：单一中心点估计的查询初始化方式缺乏鲁棒性，无法应对定位困难的场景；而REC和RES的独立处理浪费了任务间的互补信息。

4. **本文解决什么**：(a) 提出更鲁棒的位置感知查询生成策略，减少对中心点估计准确性的依赖；(b) 设计有效的任务协同机制，让REC和RES互相增强。
