...
1. **对齐后融合 (Alignment + Fusion)**：先估计跨曝光光流进行对齐，再生成 HDR 图像（如 Kalantari et al.）。问题：在严重饱和和遮挡区域，光流估计极易出错。
2. **注意力隐式融合**：绕过显式对齐，用各种注意力机制实现空间/通道级特征交互（如 AHDRNet、HDR-Transformer、SCTNet）。问题：计算复杂度高，推理延迟大，难以部署到资源受限设备。
