...
## 研究背景与动机
1. **领域现状**：视频掩码自编码器（Video MAE）在视频理解中展现出强大潜力。VideoMAE 和 ST-MAE 使用随机掩码，后续工作（如 MGM、MGMAE）探索基于运动的掩码策略。
2. **现有痛点**：基于视觉先验（如运动向量、光流）的掩码策略依赖输入视频满足特定假设（如前景运动大于背景），鲁棒性受限。并非所有视频都符合这些假设。
3. **核心矛盾**：掩码显著区域有助于学习更好的表示，但如何定义"显著"取决于特定的视觉假设，泛化性不足。
4. **本文要解决什么**：(1) 能否用自然语言替代视觉先验来定义视频显著区域？(2) MAE 的生成式预训练与对比学习的判别式预训练能否统一？
5. **切入角度**：自然语言描述是视频的信息密集表示，隐式捕捉显著性而无需模态特定假设。利用 CLIP 对齐空间计算文本-视频对应关系进行掩码。
6. **核心 idea 一句话**：用文本引导掩码替代运动引导掩码，并联合 MAE 重建损失和视频-文本对比损失进行预训练。
...
2. **字幕生成（Caption Generation）**
   - 做什么：为无标注的 K400 和 SSv2 数据集生成视频描述
