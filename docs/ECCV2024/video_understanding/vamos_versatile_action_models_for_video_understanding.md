...
1. **领域现状**：视频理解的核心问题包括从视频观察中建模人类活动的时间动态、预测未来行为以及回答视频相关问题。端到端视觉语言模型是主流方向。
2. **现有痛点**：端到端模型的视觉表征不可解释，难以诊断和修复错误预测；视觉嵌入是任务特定的，不同任务需要重新编码；大规模 VLM 联合训练成本高昂。
3. **核心矛盾**：视频理解任务可能需要不同粒度的互补表征，但现有方法通常只用单一的视觉嵌入。
4. **本文要解决什么**：探索通用自然语言描述能否作为有效视频表征，以及预训练 LLM 能否作为动作生成模型。
5. **切入角度**：将感知和推理解耦，文本描述作为中间表征服务于可解释性和效率。
6. **核心 idea 一句话**：通用视频标题作为文本表征加 LLM 作为推理器，构建高效可解释可泛化的视频理解框架。
