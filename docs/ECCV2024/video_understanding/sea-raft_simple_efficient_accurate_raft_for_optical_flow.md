...
1. **领域现状**: 光流估计是低层视觉的基础任务，用于动作识别、视频修复、帧插值、三维重建等下游任务。当前 SOTA 方法大多基于 RAFT 架构，通过循环网络迭代精炼光流场。
...
3. **核心矛盾**: 精度与效率的矛盾——现有高精度方法（如 MS-RAFT+）速度极慢（SEA-RAFT 可达其 24× 速度），而高效方法精度大幅下降。

4. **本文要解决什么?** 在显著提升效率的同时达到或超越 SOTA 精度，并改善跨数据集泛化能力。

5. **切入角度**: 从损失函数设计（概率回归）、流场初始化策略、预训练数据策略和架构简化四个正交维度同时改进 RAFT。

6. **核心idea一句话**: 用混合拉普拉斯分布建模光流的不确定性，让网络学会区分"普通像素"和"模糊像素"，配合直接回归初始流和刚性流预训练，以仅 4 次迭代达到 SOTA。
...
1. **Mixture of Laplace (MoL) 损失**:
   ...
   **做什么**: 将光流预测建模为两个拉普拉斯分布的混合，一个处理普通情况，一个处理遮挡等模糊情况。
...
2. **直接回归初始光流 (Direct Regression of Initial Flow)**:
   ...
   **做什么**: 利用上下文编码器 $C$ 接收两帧堆叠输入，直接预测初始光流，替代 RAFT 的零初始化。
...
3. **刚性流预训练 (Rigid-Flow Pre-Training)**:
   ...
   **做什么**: 在 TartanAir 数据集上进行预训练。TartanAir 提供静态场景中由相机运动产生的光流标注。
