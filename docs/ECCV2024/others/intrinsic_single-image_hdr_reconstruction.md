...
1. **端到端方法的困难**：HDR 重建任务要求网络理解高层级的几何和光照线索，从场景上下文推断丢失的细节，对数据驱动算法生成准确且高分辨率的结果构成挑战
2. **问题耦合**：传统方法将颜色恢复和亮度范围扩展混在一起处理，增加了学习难度
3. **泛化性不足**：直接在 LDR-HDR 对上训练的模型难以在多样化场景中保持稳定表现
