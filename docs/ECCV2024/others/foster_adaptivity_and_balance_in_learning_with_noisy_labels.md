...
1. **领域现状**：深度神经网络在图像分类等任务上取得了显著成就，但依赖大规模高质量标注数据。现实中通过众包或网络爬取获得的标注往往含有噪声标签，严重影响模型泛化性能。
...
4. **本文切入角度**：设计了一种既不需要先验知识又能保持类别平衡的统一框架，集成样本选择、标签校正和样本重加权三种策略。

5. **核心idea**：通过全局+局部的自适应阈值机制实现类别平衡的样本选择，并用截断正态分布进行自适应重加权。
...
1. **自适应类别平衡样本选择（SCS）**:
   - 做什么：根据样本在给定标签上的预测概率 $p^{y_i}(x_i, \theta)$ 判断样本是否干净
