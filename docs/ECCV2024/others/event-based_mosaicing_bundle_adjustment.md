...
**现有方法的局限性**：
1. **缺乏后端优化**：现有事件相机旋转估计方法 (PF-SMT, RTPT, CMax-$\omega$, CMax-GAE) 均为前端短期估计，缺少 BA 后端来提升精度和一致性
2. **仅有间接方法**：已有的事件 BA (如 Chin et al., 2019) 基于特征匹配的间接方式，丢弃了大量事件包含的信息，且量化了高时间分辨率
3. **地图质量不佳**：CMax-SLAM 虽有后端，但地图仅是边缘图 (IWE)，无法恢复灰度强度全景图
4. **直接光度 BA 空白**：纯事件驱动的直接 (光度) BA 在文献中尚属空白
