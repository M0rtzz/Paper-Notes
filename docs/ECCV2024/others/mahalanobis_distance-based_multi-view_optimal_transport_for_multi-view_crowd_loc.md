...
1. **密度图监督的局限**：现有方法依赖固定大小高斯核的密度图作为监督信号，使用MSE损失训练。在拥挤区域，当高斯核显著重叠时，局部峰值被平滑掉，导致无法准确定位每个人。

2. **单视图OT未扩展到多视图**：最优传输（OT）损失在单图人群定位中已展现出显著优势（如GL方法），但尚未被探索用于多视角人群定位。

3. **多视角特有挑战**：特征从相机视角投影到地面平面时，因未知物体3D高度，会沿视线方向产生条纹伪影，影响密度图定位精度。
