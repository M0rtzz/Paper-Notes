...
1. **仅包含背景聚焦图像**：现有数据集（如Qian等、RainDS、RobotCar）都是相机对焦在背景上拍摄的，雨滴呈模糊状态。但实际中相机自动对焦可能聚焦到附着的雨滴上，导致雨滴清晰而背景模糊——这类场景完全被忽视。

2. **缺乏夜间数据**：现有数据集全部为白天拍摄。由于白天与夜间存在显著的域差异（人工照明、低光照），在白天数据上训练的模型难以处理夜间雨滴图像。
