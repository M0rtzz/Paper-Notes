...
description: >-
  [ECCV 2024][可解释性][离线签名验证] 提出DetailSemNet用于离线签名验证，通过Detail-Semantics Integrator将特征解耦为细节和语义两个分支分别处理，并引入基于EMD的局部结构匹配，在多个多语言签名数据集上取得SOTA。 领域现状：离线签名验证（Offline Signature…
tags:
  - "ECCV 2024"
  - "可解释性"
  - "离线签名验证"
