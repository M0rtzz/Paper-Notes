...
1. **多模态支持不完整**：大多数方法仅支持边界框类别作为条件（如 LayoutGAN++、READ），或仅支持背景图像条件（如 CGL-GAN、ICVT），**没有现有方法能同时处理背景图像 + 前景图像 + 前景文本的所有模态**
2. **布局质量与约束耦合不足**：现有生成模型虽然能产生布局参数，但对背景图像的视觉理解能力不强，生成的布局可能与背景语义不协调
3. **缺乏合适的评测数据集**：现有广告横幅数据集多为非公开或不含英文模态
