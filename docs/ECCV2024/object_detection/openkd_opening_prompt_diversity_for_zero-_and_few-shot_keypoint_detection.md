...
**现有痛点**：
1. **模态单一**：大多数方法只支持视觉 prompt 或文本 prompt，不能同时利用两种模态的互补优势
2. **语义封闭**：模型无法处理未见过的文本 prompt（如训练时见过 "eye" 但测试时遇到 "knee"），novel keypoint 检测性能极差
3. **语言刻板**：现有 ZSKD 仅支持模板化的简单文本（如 "the nose of a cat"），无法处理自然语言风格多样的提问（如 "Can you detect the nose and ears of a cat?"）
