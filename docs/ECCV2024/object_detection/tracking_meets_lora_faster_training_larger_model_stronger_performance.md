...
1. **位置编码不兼容**：现有跟踪器为模板（小图）和搜索区域（大图）使用独立位置编码，破坏了预训练 ViT 的原始结构，在 LoRA 等 PEFT 方法下效果不佳
2. **卷积头的归纳偏置**：OSTrack 的卷积检测头在 LoRA 微调下难以收敛——卷积对数据结构的局部性假设阻碍了 LoRA 的参数高效调整
...
1. **解耦输入嵌入（Decoupled Input Embedding）**：
   - 做什么：将位置信息与 token 来源识别解耦，保持预训练位置编码完整
...
2. **纯 MLP 检测头（MLP-only Head）**：
   - 做什么：用 3 层 MLP 替代 OSTrack 的卷积检测头进行分类和回归
