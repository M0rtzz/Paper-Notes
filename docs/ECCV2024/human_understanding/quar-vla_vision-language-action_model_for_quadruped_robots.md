...
## 研究背景与动机
1. **领域现状**：四足机器人学习通常将视觉感知（QUAR-VA）和语言交互（QUAR-LA）分开处理。VA 方法利用第一/第三人称图像引导动作但只用粗粒度目标图像指令；LA 方法利用语言执行细粒度任务但缺乏视觉感知无法自主导航。
2. **现有痛点**：
   - VA 方法依赖单一粗粒度图像指令，难以处理组合任务（如"先...然后..."）
   - LA 方法缺少视觉模态，无法感知环境障碍物
   - 缺乏大规模四足机器人多任务数据集
3. **核心矛盾**：四足机器人需要同时理解视觉场景和语言指令来自主决策，但现有范式将两者割裂；同时四足的灵活步态和全身控制使得动作空间设计更复杂。
4. **切入角度**：定义 12 维高层级命令动作空间（速度+步态+姿态+终止信号），将连续动作离散化为 256 个 bin，利用预训练 VLM 直接输出动作 token。
5. **核心 idea 一句话**：将预训练多模态大模型微调为四足机器人的统一策略，输入图像+语言指令，输出离散化动作 token。
...
#### 1. **动作空间设计 (Action Space Design)**
   - 做什么：定义适合四足机器人的高层级命令空间，平衡灵活性和计算效率
   - 核心思路：
...
#### 2. **QUARD 数据集 (QUAdruped Robot Dataset)**
   - 做什么：构建首个包含视觉、语言指令和机器人命令的大规模四足机器人数据集
   - 核心思路：
...
#### 3. **QUART 模型 (QUAdruped Robotic Transformer)**
   - 做什么：将预训练视觉-语言模型微调为四足机器人的统一控制策略
   - 核心思路：
