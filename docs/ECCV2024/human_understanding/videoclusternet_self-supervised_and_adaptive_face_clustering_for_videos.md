...
1. **领域现状**: 视频人脸聚类旨在将视频中检测到的人脸按身份分组，是视频场景描述、视频问答、视频理解等任务的重要基础。随着影视制作内容的增长，电影/电视剧领域的人脸聚类需求日益增长。
...
3. **核心矛盾**: 电影场景中同一角色的人脸变化巨大（姿态、光照、装扮），但通用模型不适配特定视频风格，且标注成本高昂使得监督微调不可行。

4. **本文要解决什么?** (1) 无需人工标注地自适应微调 Face ID 模型到特定视频；(2) 设计无需任何用户输入参数的聚类算法。

5. **切入角度**: 将自蒸馏（DINO 风格）的自监督学习应用于视频人脸 track 的正样本对学习，完全跳过负样本挖掘；用训练损失函数本身作为聚类的距离度量。

6. **核心idea一句话**: 仅用正样本对的自蒸馏微调 + 将训练损失直接作为聚类距离度量 = 完全自动化的视频人脸聚类。
...
1. **自监督模型微调 (Self-Supervised Model Finetuning)**:
   ...
   **做什么**: 将通用 Face ID 模型自适应到特定视频中观察到的人脸变化。
...
2. **粗粒度 Track 匹配 (Coarse Track Matching)**:
   ...
   **做什么**: 跨 shot 边界匹配可能属于同一身份的不同 tracks，为微调提供更多样本对。
...
3. **无参数 Track 聚类 (Parameter-Free Track Clustering)**:
   ...
   **做什么**: 将所有 track 按身份聚类，无需用户指定任何参数。
