...
### 现有方法的不足
1. **早期经典方法**（手工特征匹配、模板匹配）：可处理未见物体，但精度有限
2. **深度学习方法**（MegaPose、GigaPose、GenFlow）：精度高但需要在海量任务特定数据（数百万张合成图像、数千个不同物体）上预训练，数据集生成代价大
3. **现有无训练方法**（ZS6D、PoMZ）：ZS6D 使用最后一层 ViT 特征建立对应关系，精度显著低于 FoundPose；PoMZ 需要 RGB-D 输入
