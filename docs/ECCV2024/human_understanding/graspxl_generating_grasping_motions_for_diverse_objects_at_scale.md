...
1. **依赖昂贵的3D手-物交互数据**：数据驱动方法（如D-Grasp）需要精确的3D标注序列作为训练数据，采集成本极高，且模型受训练分布限制，难以泛化
2. **泛化能力受限**：多数方法只能处理训练时见过的物体（DexVIP评估0个新物体），或仅支持少量测试物体（UniDexGrasp约100个），远无法满足实际应用需求
3. **缺乏多目标控制**：现有方法通常只支持单一目标（如仅控制朝向），无法同时满足抓取区域、朝向、手腕旋转和位置等多个运动目标
