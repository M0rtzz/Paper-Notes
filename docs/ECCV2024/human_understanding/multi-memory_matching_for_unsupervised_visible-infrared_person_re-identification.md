...
1. **跨模态对应关系不可靠**：作者引入 ARI（Adjusted Rand Index）指标评估发现，现有方法尽管在检索指标上表现不错，但其跨模态对应关系的质量很差（ARI 值低）
2. **单记忆表达不足**：现有方法用单一记忆（即单一聚类中心）代表一个身份，无法捕捉个体的多视角、多姿态等细微差异，导致跨模态匹配噪声大
3. **悖论现象**：具有重叠属性的不同行人因噪声对应关系而被进一步混淆，虽然可能在类间提高特征相似度带来指标提升，但实际精确检索能力受限
