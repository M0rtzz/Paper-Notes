...
1. **SOTA FER 方法泛化差**：SCN、RUL、EAC、OFER 等方法过度拟合训练集的域特定信息（domain-specific），在有域差异的测试集上表现不稳定。越强的方法（如 EAC）在源域越好，但在跨域上甚至比弱方法更差——说明是"过拟合式"的性能提升。
2. **领域自适应 FER 方法需目标域数据**：现有域自适应方法需要获取目标域的标注或未标注样本进行微调，在真实部署中往往不可行，因为我们事先不知道测试样本的分布。
