...
1. **需要额外训练**：大多数方法（DynamicViT、A-ViT、SP-ViT）需要在中间层训练额外的 mask 预测模块，随着基线模型不断更新，重新训练成本高昂
2. **多步渐进式剪枝**：在网络的多个中间层逐步减少 token，导致早期层仍执行冗余计算，特别是对深层模型不利
3. **架构绑定**：许多方法依赖特定架构特征（如 class token、attention map），无法推广到 ConvNet 或混合模型
