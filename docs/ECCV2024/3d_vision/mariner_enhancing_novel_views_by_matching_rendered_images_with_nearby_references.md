...
1. **领域现状**: 3D重建方法（点云、mesh、NeRF、高斯溅射）各有局限，产生的渲染图像包含几何噪声、外观伪影和不一致性，直接影响下游任务。
2. **现有痛点**: 
   - 现有伪影去除方法（如NeRFLiX）通常针对特定管线设计，无法跨管线通用
   - RefSR方法设计用于解决分辨率差异，无法处理渲染图像中的几何伪影和外观不一致
   - 风格迁移方法要么破坏内容结构，要么无法去除伪影
3. **核心矛盾**: 需要一个通用的后处理方法，既能去除多种3D表示的渲染伪影，又能从参考图像传输真实细节
4. **本文要解决什么**: 提出一个与3D重建管线无关的渲染增强方法，利用已有的参考图像恢复渲染质量
5. **切入角度**: 将问题建模为参考图像引导的渲染增强——从RefSR借鉴特征匹配与传输思路，但适配到render-to-real的域差距
6. **核心idea一句话**: 用共享编码器提取渲染和参考图像的多层特征，通过粗到精匹配建立对应关系，层次化融合参考细节到渲染图像中
