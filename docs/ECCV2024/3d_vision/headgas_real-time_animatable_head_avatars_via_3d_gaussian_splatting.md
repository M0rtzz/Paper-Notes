...
1. **领域现状**: 可动画3D头部重建是数字人、AR/VR和远程会议的核心技术。基于NeRF的方法（如INSTA、NeRFBlendShape）在质量和速度之间存在trade-off，交互帧率仅10-15 fps。
2. **现有痛点**: NeRF方法因体渲染的密集采样限制了渲染速度；显式方法（网格、点云）虽有更强的几何约束但难以保持照片级真实感（如INSTA出现三角面片伪影）。
3. **核心矛盾**: 3DGS原始设计是静态场景表示，不支持表情驱动的动态外观变化。直觉上应移动高斯位置来建模动态，但这使优化更复杂。
4. **本文要解决什么**: 如何将3DGS扩展为可动画的头部表示，同时保持实时渲染速度和高保真度。
5. **切入角度**: 受传统blendshape模型启发，不移动高斯基元，而是通过改变其**不透明度和颜色**来表示面部动态——即通过"过表示"（over-representation）实现运动。
6. **核心idea一句话**: 每个高斯携带可学习特征基底，表情参数作为混合权重在潜在空间进行线性组合，经轻量MLP输出表情相关的颜色和不透明度。
