...
1. **领域现状**: 位姿条件扩散模型(如Zero-1-to-3、ZeroNVS)在新视角合成(NVS)上取得进展，但主要在物体级别(Objaverse)或物体中心的小规模场景数据集(DTU、CO3D)上训练
2. **现有痛点**: 缺乏大规模、多样化的场景级训练数据，导致已有方法无法泛化到真实in-the-wild场景；MegaDepth仅196个地标，CO3D/ACID/RealEstate10K覆盖类别有限
3. **核心矛盾**: 物体级数据集充足（Objaverse-XL数百万模型），但场景级数据集在规模、多样性、位姿分布上远远不足，成为场景级NVS的瓶颈
4. **本文要解决什么**: 
   - 构建大规模场景级3D数据集弥补数据缺口
   - 解决现有NVS方法在场景级应用时位姿不准确的问题
5. **切入角度**: 利用Wikimedia Commons的800万+开放许可图片，通过SfM重建获得场景级3D信息；用warped image作为额外位姿条件提升一致性
6. **核心idea一句话**: 大规模互联网照片SfM重建 + warp图像条件化 = 高质量场景级NVS
