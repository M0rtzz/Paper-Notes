...
现有3D人体生成方法面临两大挑战：(1) 无法实现语义解耦生成，即不能独立控制身体、上衣、下装等不同语义部分；(2) 受限于NeRF的高计算成本，最高只能合成512²分辨率的图像。CNeRF等方法虽然尝试用K个局部生成器，但几何层面的解耦并不完全；AttriHuman-3D使用单一生成器导致不同语义部分仍然耦合。
