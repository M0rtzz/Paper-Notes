...
1. **领域现状**: 点云理解在自动驾驶、机器人、公共安全等领域应用广泛，但点云的无序性和不规则性使得有效提取形状信息极具挑战。已有方法包括间接表示（多视图/体素）、直接处理（PointNet系列）、Transformer方法和预训练方法。
2. **现有痛点**: 
   ...
   - 预训练方法依赖外部数据，训练成本高
3. **核心矛盾**: 如何在不依赖外部数据的情况下，同时捕获全局上下文和精细局部结构信息？
4. **本文要解决什么？**: 直接从点云中学习丰富的上下文形状感知和精确的局部几何结构。
5. **切入角度**: 先用全局感知增强上下文表征，再用Taylor级数多项式拟合精细分析局部结构。
6. **核心idea一句话**: 全局感知(自适应可变形图卷积+交叉注意力+多头注意力)引导局部结构拟合(Taylor级数分解为低阶基础+高阶细化)。
