...
1. **领域现状**: 基于NeRF的街景新视角合成方法（Urban Radiance Fields、Block-NeRF等）取得进展，但依赖密集训练图像和大量计算资源。
2. **现有痛点**:
...
3. **核心矛盾**: 如何构建一个高效且可泛化的城市视角合成方法，能鲁棒应对不同稀疏度？
4. **本文要解决什么？** 在稀疏街景视角下实现快速前馈推理和高效微调的新视角合成。
5. **切入角度**: 用几何先验（深度估计）替代特征匹配来获取几何信息，直接在3D空间操作，避免对参考图像位姿的依赖。
6. **核心idea一句话**: 将深度估计先验融入全局3D体积表示，通过SPADE 3D CNN在世界坐标系中直接处理，使方法对参考图像位姿配置不敏感。
