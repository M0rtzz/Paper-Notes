...
description: >-
  [ECCV 2024][3D视觉][立体匹配] 提出TC-Stereo，通过时序视差补全提供良好初始化、时序状态融合保持隐藏态连贯性，以及双空间（视差+视差梯度）迭代精炼改善病态区域，实现时间一致的立体匹配。 领域现状 领域现状：现有立体匹配方法逐帧独立推理，导致时间不一致。不一致的根源有两点：(1) 每帧从零开始全局搜索视…
tags:
  - "ECCV 2024"
  - "3D视觉"
  - "立体匹配"
