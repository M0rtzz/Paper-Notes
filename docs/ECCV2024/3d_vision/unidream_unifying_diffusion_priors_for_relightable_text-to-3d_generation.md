...
- 现有text-to-3D方法（DreamFusion、Magic3D等）使用RGB扩散模型，生成的3D物体光照和阴影被"烘焙"进纹理
- 烘焙的光照限制了3D物体在不同光照条件下的真实感和可重用性
- Fantasia3D尝试分离光照和纹理但常混合albedo和反射光
- **核心问题**：如何从文本生成具有干净PBR材质（albedo、法线、粗糙度、金属度）、可在任意光照下重新渲染的3D物体
