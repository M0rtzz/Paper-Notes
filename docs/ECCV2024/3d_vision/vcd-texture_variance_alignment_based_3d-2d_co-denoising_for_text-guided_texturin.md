...
- 现有文本引导纹理合成方法忽视了2D扩散模型与3D物体之间的模态差异
- 渐进修复方法（TEXTure、Text2Tex）在对立视角生成不一致纹理
- 同步多视角去噪方法（SyncMVD）忽略了跨视角的3D空间对应关系
- 特征聚合→光栅化的过程存在严重的方差偏差，导致纹理过于平滑
- **核心问题**：光栅化作为凸组合操作，由Jensen不等式会使方差衰减，破坏扩散模型生成高频细节的能力
