...
1. **领域现状**: 将3D环境表示为对齐的CAD模型对AR和机器人等下游任务至关重要，相比噪声点云/网格，CAD表示具有无孔洞、干净几何、物体级标注等优势。
2. **现有痛点**: 当前SOTA方法计算量大——需要逐个编码检测到的物体，再在第二阶段优化CAD对齐，运行时间从2.6秒到20分钟不等。
3. **核心矛盾**: 高精度CAD检索与对齐需要大量计算，无法满足实时应用（AR/机器人）的需求。
4. **本文要解决什么？**: 在保持甚至提升精度的前提下，将CAD检索与对齐的速度提升到实时水平。
5. **切入角度**: 单阶段直接预测——同时输出对齐参数和形状嵌入，避免两步串行带来的延迟；通过嵌入蒸馏避免推理时使用编码器。
6. **核心idea一句话**: 在对比学习框架下学习高质量CAD嵌入空间，将其蒸馏到单阶段检测网络中，实现实时CAD检索与对齐。
