...
1. **领域现状**: 文本到3D生成是计算机图形学中的热门方向，分为两大类:
...
3. **核心矛盾**: 3D高斯点天然无结构化，难以直接用现有生成网络处理；同时高斯属性维度高，直接学习分布困难。
4. **本文要解决什么？**: 设计一种能将无序3D高斯组织为结构化表示的方法，并实现直接从文本前馈生成3D高斯。
5. **切入角度**: 将高斯组织为固定数量的体积网格(GaussianVolume)，通过从粗到精(几何→属性)的两阶段管线简化生成。
6. **核心idea一句话**: 将无序高斯结构化为体积网格 + Candidate Pool策略保持固定数量 + 先扩散生成几何再U-Net预测属性。
