...
1. **领域现状**: 3D Gaussian Splatting (3DGS) 已成功应用于人体avatar建模，能实现实时渲染且视觉质量高。现有方法如3DGS-Avatar、GauHuman等将3DGS与SMPL人体先验结合，从单目视频学习可动画的人体表示。

2. **现有痛点**: 当前所有3DGS人体方法都是**单身份（per-identity）优化**——每个人需要独立训练一个模型。这导致：(a) 多人场景时参数量线性增长，$N_i$个人需要$N_i \times N_g \times M$个参数；(b) 单个身份的训练数据有限，面对分布外（OOD）姿态时动画质量急剧下降。

3. **核心矛盾**: 单身份模型只能学到有限的人体变形模式，但真实应用需要在极端姿态（如高难度舞蹈）下保持动画鲁棒性。多身份联合学习可以共享变形知识，但如何在不爆炸参数量的前提下实现？

4. **本文要解决什么**: 从单目视频学习**多身份统一的3DGS表示**，既能压缩参数，又能通过跨身份知识共享提升OOD姿态下的动画鲁棒性。

5. **切入角度**: 借鉴经典TensorFaces的思路，将所有身份的Gaussian参数组织成高阶张量，利用CP分解实现低秩近似。

6. **核心idea一句话**: 不同人体共享相似的结构特征，因此多身份Gaussian参数张量具有低秩结构，可以用CP分解高效表示。
