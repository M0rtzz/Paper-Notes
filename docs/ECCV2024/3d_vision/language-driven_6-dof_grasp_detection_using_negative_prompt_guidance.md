...
1. **领域现状**: 6-DoF抓取检测是机器人视觉的基础任务，现有方法主要关注抓取稳定性，但忽略了人类通过自然语言表达的抓取意图。
2. **现有痛点**: 已有语言驱动抓取方法存在严重局限：部分仅支持单物体场景，部分仅检测2D矩形抓取，无法处理3D杂乱场景中的精细语言指令。
3. **核心矛盾**: 语言驱动抓取本质上是细粒度任务（"抓蓝色杯子" vs "抓黑色杯子"），但现有方法缺乏区分不同物体的精确引导机制。
4. **本文要解决什么**: 在杂乱3D点云场景中，根据自然语言指令检测目标物体的6-DoF抓取姿态。
5. **切入角度**: 借鉴图像生成中负提示的思想，在扩散模型训练中学习场景内非目标物体的嵌入表示，推理时将抓取姿态从非目标物体方向"推开"。
6. **核心idea一句话**: 通过学习负提示嵌入来编码"不想抓的东西"，在扩散采样中组合正向引导和反向排斥实现精准抓取。
