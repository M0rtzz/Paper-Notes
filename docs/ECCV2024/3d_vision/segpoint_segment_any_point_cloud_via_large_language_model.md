...
## 研究背景与动机
1. **领域现状**：3D 点云分割已有大量进展（Mask3D、SPFormer 等），但每个模型通常只解决一种特定分割任务，缺乏统一性。
2. **现有痛点**：(a) 现有方法依赖预定义类别或显式文本描述，无法理解隐含的人类意图（如"哪里可以坐？"）；(b) 不同任务需要不同模型，效率低且不实用。
3. **核心矛盾**：真实场景需要模型能理解隐含指令并推理，但当前方法缺乏推理能力；同时需要统一框架处理多种分割任务。
4. **切入角度**：利用 LLM 的推理和世界知识来理解复杂/隐含指令，结合几何增强模块弥补点云编码器在密集预测上的不足。
5. **核心 idea 一句话**：将 LLM 的推理能力注入 3D 点云分割，通过几何增强和特征传播实现高质量逐点分割。
