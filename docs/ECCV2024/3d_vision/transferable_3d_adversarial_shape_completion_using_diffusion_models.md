...
- 现有3D对抗攻击方法主要对xyz坐标加扰动，导致视觉质量严重下降且易被防御检测
- 大部分攻击关注白盒场景，对近年新提出的模型（PCT、PRC、GDANet等）迁移性极差
- 现有黑盒攻击（AdvPC、PF-Attack）仅对早期模型有效，攻击成功率（ASR）在新模型上不足5%
- **核心问题**：如何生成高质量、高迁移性的3D对抗点云，同时不破坏原始形状的自然外观
