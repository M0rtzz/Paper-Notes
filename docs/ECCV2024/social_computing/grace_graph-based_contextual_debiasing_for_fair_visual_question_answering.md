...
1. **语言先验偏差**：VQA 数据集中存在明显的答案分布偏斜。例如，对于"What sport is this?"类问题，"tennis"可能在训练集中占据不成比例的频率，导致模型学会捷径而非真正理解图像
2. **视觉-语言关联偏差**：某些视觉特征与特定答案之间存在虚假关联。例如，看到"厨房"场景就倾向回答"cooking"，而忽略实际问题内容
3. **社会偏差**：模型可能对不同性别、种族等群体产生不公平的预测差异
4. **In-context example 的偏差放大**：当 LLM 使用有偏的示例进行推理时，偏差不仅被继承还可能被放大
