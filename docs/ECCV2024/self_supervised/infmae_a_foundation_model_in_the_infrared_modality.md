...
1. **缺乏红外基础模型**：可见光模态已有 MAE、DINO、BEiT 等基础模型，视频模态有 VideoMAE，遥感有 Scale-MAE，点云有 Point-M2AE，但红外模态尚无专属基础模型。直接将可见光 MAE 迁移到红外任务效果不佳，原因在于两种模态存在显著差异。
...
3. **缺乏大规模红外数据集**：现有红外数据集规模小、场景单一，不足以支撑基础模型的预训练。
