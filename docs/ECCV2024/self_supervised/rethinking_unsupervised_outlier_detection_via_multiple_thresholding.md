...
1. **训练集被污染**：多数方法假设 inlier 占多数，直接在整个目标数据集上学习正常流形。但当 outlier 比例升高时，数据集均值会向 outlier 偏移（mean-shift 问题），导致学到的"正常流形"被污染。

2. **评分方法受限于单次推理**：现有检测器通常是非迭代的，由于缺乏预测标签，无法利用数据集的自监督信号来迭代改进自身。如果能先给数据"打标签"再回过头来优化检测，效果会好得多。
