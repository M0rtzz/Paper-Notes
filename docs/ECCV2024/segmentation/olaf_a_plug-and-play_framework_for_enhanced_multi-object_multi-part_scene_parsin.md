...
1. **前景分割错误**：物体区域（前景的并集）经常被错误分割，导致内部部件分割也随之出错。例如 FLOAT 完全无法识别电视的边框和屏幕。
2. **边界细节丢失**：物体与部件之间的边界信息无法被准确捕捉，如汽车车身与轮胎、车身与车窗之间的分界。
3. **小/细部件漏分**：面积较小或形状细长的部件几乎无法被正确分割，如动物的眼睛、尾巴，车辆的车灯等。
