...
1. **FG特征被BG污染**：query的FG像素包含了FG（目标物体）和BG（背景物体）的混合特征。例如，鸟的像素特征中也包含了栅栏的特征。
2. **BG特征被FG污染**：support的BG像素也可能包含FG信息（如边界处的人），导致它们与query FG产生虚假的高相似度。
3. **匹配失效**：由于query FG和support FG特征分别混入了不同类别的BG特征，它们之间的相似度变小，交叉注意力分数降低，query FG无法充分聚合support的FG信息。
