...
1. **定位分数替代分类**（如 OLN、SWORD、OpenInst）：用 IoU 等定位质量分数替代前景/背景二分类，避免将未知物体归为背景。但这类方法精度（precision）普遍偏低。
2. **伪标注增强**（如 GGN、UDOS、LDET）：为未标注物体生成伪标注来扩充训练集。但现有伪标注方法生成的标注**噪声极大**——经常包含大量背景区域，导致精度低下。
