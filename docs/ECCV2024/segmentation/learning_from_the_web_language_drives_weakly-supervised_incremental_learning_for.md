...
## 研究背景与动机
1. **领域现状**：语义分割的类增量学习（CILSS）允许模型逐步学习新类别，但传统方法依赖昂贵的像素级标注。弱监督增量学习（WILSS）将增量步骤的标注降低为图像级标签，但仍要求使用精心策划的数据集图像。
2. **现有痛点**：(1) WILSON、FMWISS 等 WILSS 方法仍需要从目标数据集中获取训练图像；(2) 现有方法在多步单类增量（如 15-1）设定下性能严重退化；(3) 不支持仅使用单类新数据的增量步骤，需要负样本。
3. **核心矛盾**：实际场景中，预训练模型需要适应新类时，目标域数据可能极度有限（如隐私限制），此时从网络获取补充数据是必然选择。但网络数据面临两个关键挑战：(1) 分布与训练数据不同；(2) 图像级标签噪声严重（用类名搜索的图片可能包含多个类或不含目标类）。
4. **本文要解决什么**：在增量步骤中完全使用网络图像（而非数据集图像）学习新类，同时也用网络图像做旧类 rehearsal 防止灾难性遗忘。
5. **切入角度**：(1) 用傅里叶域特征做域判别器筛选与训练数据分布接近的网络图像；(2) 用 caption 模型替代简单的类名标签提供多类监督；(3) 保存 caption 而非图像来查询旧类 rehearsal 图像，解决隐私和存储问题。
6. **核心idea一句话**：傅里叶域判别器 + caption 驱动的网络图像选择和 rehearsal ，实现无需原始数据集的弱监督增量分割。
...
2. **Caption 标注 (Caption Labeling)**
   - 做什么：用视觉语言模型为网络图像提供多类图像级标签，替代简单的搜索关键词标签
...
3. **Caption 驱动的 Rehearsal 查询 (Caption-based Querying)**
   - 做什么：存储旧图像的 caption 而非图像本身，用 caption 从网络搜索相似图像做 rehearsal
...
4. **Caption 驱动的 Rehearsal 过滤 (Caption-based Filtering)**
   - 做什么：验证下载的 rehearsal 图像是否包含原始图像的核心语义内容
