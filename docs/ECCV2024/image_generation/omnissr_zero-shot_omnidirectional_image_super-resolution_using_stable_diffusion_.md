...
1. **端到端方法的局限**：大多数方法（如 SphereSR、OSRT）只能产生确定性输出，数据保真度好但感知质量差（过度平滑），且训练需要大量高分辨率 ODI（采集成本高）
2. **直接应用扩散先验的困难**：如果将 ERP 图像分割为切线投影（TP）图像后逐一独立超分，重投影回 ERP 时重叠区域会出现不一致，破坏全局连续性
