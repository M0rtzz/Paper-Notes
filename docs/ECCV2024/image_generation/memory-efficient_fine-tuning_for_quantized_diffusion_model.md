...
1. **通道间模式丢失 (P1)**: 全精度微调时权重更新呈现明显的通道间模式（inter-channel pattern），但基线仅更新通道内缩放因子，无法捕捉这些模式
2. **时间步感知能力不足 (P2)**: 扩散模型在不同去噪步骤中扮演不同角色（内容特征 vs 粗粒度特征 vs 噪声清除），但基线缺乏区分不同时间步的能力，导致要么主体保真度高但忽略文本提示，要么两者都无法满足
