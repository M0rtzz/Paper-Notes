...
1. **基于T2I的方法局限性**：现有利用T2I扩散模型的方法通常通过膨胀注意力层（inflated attention）来处理多帧，但这种隐式的运动保持策略远不足以实现平滑、完整的运动。常见的补充方案（如注入self-attention map、使用ControlNet等结构引导）增加了系统复杂度。

2. **T2V模型的不足**：即使使用预训练的T2V模型，公开可用的模型缺乏足够丰富的时间先验来准确描绘真实世界运动（如Fig. 2所示）。因此很多方法需要在输入视频上微调模型权重（one-shot方式），增加了额外开销。

3. **标准逆扩散过程的固有困难**：无论使用T2I还是T2V，从标准高斯噪声或反转潜在表示开始的传统逆扩散过程，天然难以重新编程复杂的真实世界运动。
