...
1. **感知质量低**：当前 SOTA 的 VSR 方法（如 BasicVSR++、RVRT）专注于重建质量（PSNR/SSIM），生成的结果虽然像素距离近但视觉效果模糊、缺乏细节
2. **感知-失真权衡**：根据感知-失真权衡定理，在有限模型容量下，提升重建质量不可避免地导致感知质量下降

扩散模型（DM）在单图像超分辨率中已展现出合成逼真纹理和细节的能力，但直接将单图超分应用于视频存在两个问题：

1. **时序不一致**：每帧独立处理导致相邻帧生成的细节不一致，产生闪烁
2. **信息利用不足**：未利用视频帧间的时空冗余信息
