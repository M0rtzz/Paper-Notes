...
description: >-
  [ECCV 2024][图像生成][图像检索] 将图像检索重新定义为生成式建模任务，提出 IRGen——一个序列到序列模型，通过语义图像分词器将图像转化为简短的离散 token 序列，然后自回归地生成查询图像最近邻的标识符，实现端到端可微分的检索并在三个标准基准上达到 SOTA。 传统图像检索系统由两个独立阶段组成： 特征表…
tags:
  - "ECCV 2024"
  - "图像生成"
  - "图像检索"
