...
1. **Noise inversion 方法**（Null-text Inversion 等）：将图像反演到噪声空间，但忠实重建与可编辑性之间存在固有矛盾
2. **ControlNet 类方法**：以边缘图等条件引导扩散模型，但无法保留条件信号中缺失的图像属性
3. **Concept tuning 方法**（DreamBooth 等）：生成概念的新实例而非修改原图
4. **Self-Guidance**：通过缓存注意力图进行空间编辑，但编辑结果往往不够真实
