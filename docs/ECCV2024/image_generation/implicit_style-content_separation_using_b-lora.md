...
1. **微调方法**（DreamBooth、LoRA 等）：在特定图像上微调模型容易过拟合，且风格和内容纠缠在一起
2. **两个独立 LoRA 的组合**：需要分别训练风格 LoRA 和内容 LoRA，组合方式不明确（权重插值需要手动搜索系数）
3. **ZipLoRA**：提出学习混合系数来合并两个 LoRA，但每次新的风格-内容组合都需要额外优化
4. **零样本方法**（StyleAligned、IP-Adapter 等）：无法明确分离风格和内容，可能出现风格图像的语义泄漏
