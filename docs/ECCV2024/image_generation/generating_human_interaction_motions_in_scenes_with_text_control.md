...
1. **文本-运动扩散模型**（MDM、MotionDiffuse 等）：生成高质量运动，但完全忽略环境上下文，角色会穿过障碍物
2. **场景感知 VAE/扩散模型**（SceneDiffuser 等）：使用小规模配对人-场景数据训练，运动多样性和质量受限，且缺少文本控制
3. **强化学习方法**（DIMOS 等）：可学习交互运动但奖励函数设计困难，运动多样性低且无法文本控制
4. **配对数据极度稀缺**：相比大规模运动捕捉数据集（HumanML3D），同时包含 3D 运动、场景几何和文本标注的数据极少
...
**做什么**：在冻结的预训练文本-运动 Transformer 编码器之上，添加独立的场景感知 Transformer 分支，通过零初始化线性层连接到基础模型各层。
