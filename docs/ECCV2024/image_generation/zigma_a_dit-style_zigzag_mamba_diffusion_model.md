...
1. **Transformer 的二次复杂度**：DiT 等基于 Transformer 的扩散骨干在自注意力机制上存在 $O(M^2)$ 的复杂度瓶颈，尽管有 Flash Attention 等优化，处理长序列 token 时仍受限
2. **Mamba 的空间连续性问题**：Mamba 作为线性复杂度的状态空间模型（SSM）在 1D 序列建模上优势明显，但现有视觉 Mamba 方法直接按行列主序（row/column-major）将 2D token 展平为 1D，忽略了图像中相邻 patch 的空间连续性
3. **多方向扫描的参数开销**：VisionMamba 等方法通过在单个 Mamba 块中采用多方向扫描来弥补空间感知不足，但这带来了额外的参数量和 GPU 显存负担
