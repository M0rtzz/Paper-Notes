...
1. **身份保持 vs 提示词对齐**：提高身份相似度往往牺牲编辑能力（如风格化、姿态变换）
2. **扩散训练的损失局限**：标准扩散训练仅使用 L2 噪声预测损失，无法直接应用感知损失（如身份损失），因为中间时间步的预测通常模糊且有噪声
3. **GAN 领域的成功经验**：GAN inversion 中通过身份损失大幅提升了编码器质量，但扩散模型中无法直接应用
