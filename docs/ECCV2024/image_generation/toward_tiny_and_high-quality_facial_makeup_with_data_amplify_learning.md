...
1. **模型过大**：现有方法（如 EleGANt 10M、SCGAN 35M）参数远超移动端要求（<100K），难以在手机上实时运行
2. **依赖复杂 pipeline**：需要人脸解析、关键点检测等辅助模块，增加延迟和部署难度
3. **非配对学习不稳定**：标注配对妆容数据代价昂贵，主流方法采用非配对学习范式，依赖对抗训练和不精确的监督信号（颜色直方图匹配、推土机距离等），鲁棒性差
