...
description: >-
  [ECCV 2024][图像生成][多模态提示] 本文提出 MultiGen，通过为每个物体构建"增广token"（融合文本、坐标和图像信息），并训练坐标模型和特征模型来处理推理时的模态缺失，首次实现了从多物体多模态提示进行零样本图像生成，支持纯文本或任意模态组合的灵活输入。 领域现状：文本到图像生成（Text-to-Ima…
tags:
  - "ECCV 2024"
  - "图像生成"
  - "多模态提示"
