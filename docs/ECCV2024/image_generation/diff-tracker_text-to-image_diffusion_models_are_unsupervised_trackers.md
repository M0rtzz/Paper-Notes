...
### 核心痛点
无监督跟踪面临两大挑战：
1. **语义和结构信息利用不足**：难以有效利用视频帧中丰富的语义和空间结构信息
2. **上下文关系挖掘不够**：视频中的时间上下文关系未被充分利用
...
**做什么**：将cross-attention map与self-attention map融合，使learned prompt编码目标与背景的关系信息。
...
**做什么**：从第一帧学习一个能准确激活目标bounding box区域的prompt embedding。
...
**做什么**：根据目标在后续帧中的运动信息，动态更新prompt以适应目标外观变化。
