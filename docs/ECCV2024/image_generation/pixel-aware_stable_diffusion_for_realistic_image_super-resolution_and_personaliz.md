...
1. **像素级结构保持困难**：ControlNet 等方法通过"零卷积"简单相加 U-Net 和条件特征，无法传递像素级精确信息，导致输出与输入的结构不一致
2. **跳跃连接的局限**：StableSR 等方法通过 VAE 编码器-解码器之间的跳跃连接传递像素细节，但需要额外在图像空间训练，限制了在潜空间任务（如风格化）中的应用
3. **训练-测试不一致**：SD 的噪声调度在训练终端时间步留有残余信号，但测试时从纯高斯噪声采样，产生不一致
