...
1. **空间-光谱分离问题**: 现有方法通常只关注光谱维度的相关性，忽略空间维度，或者用独立模块分别处理二者，破坏了 HSI 的3D特征结构，无法建模统一的空间-光谱相关性。
2. **满秩注意力的局限**: 标准自注意力通过成对token计算相关矩阵，得到满秩矩阵，无法描述HSI中广泛存在的**多token间线性依赖关系**（HSI固有的低秩特性）。
...
1. **光谱方向非连续3D切分策略 (SD3D Splitting)**:
   - 做什么：将特征图切分为 token，使单个 token 同时包含空间和光谱信息。
...
2. **统一空间-光谱自注意力 (USSA)**:
   - 做什么：在 SD3D 切分后的 token 上计算满秩注意力矩阵，捕获成对 token 间的独立相关性。
...
3. **动态低秩映射模块 (DLRM)**:
   - 做什么：生成一个低秩依赖图，捕获多个 token 间的线性依赖关系。
