...
1. **领域现状**：图像恢复在雨、雾、雪等恶劣天气下已被广泛研究，从早期 CNN 方法到近年 Transformer 方法取得显著进展。All-in-One 天气去除（单一模型处理多种天气退化）成为重要方向。
2. **现有痛点**：Transformer 方法为降低计算开销，要么将自注意力限制在固定空间窗口内（如 Swin Transformer），要么仅在通道维度做注意力（如 Restormer），牺牲了对长距离空间特征的捕获能力。
3. **核心矛盾**：天气退化（雨滴、雪花、雾气）的像素空间分布是动态的、全局分散的，但具有相似的强度模式——固定窗口注意力无法将分散但相似的退化像素关联起来。
4. **本文要解决什么**：设计一种高效的注意力机制，能自适应地关注空间上分散但强度相似的天气退化像素。
5. **切入角度**：观察到天气退化通常导致相似的遮挡和亮度模式，因此可按像素强度排序分箱，将相似退化的像素聚到一起做注意力。
6. **核心 idea**：将空间特征按强度排序后分入直方图的 bin 中，在 bin 内（fine-grained）和 bin 间（global）分别做自注意力，实现动态范围的高效空间注意力。
