...
1. **训练-测试分布不一致**：现有去模糊模型大多在合成数据（如 GoPro）上训练，但真实场景中不同相机设置（快门速度、光圈、光源）会产生不同方向和强度的模糊模式。当测试视频的模糊模式与训练集不一致时，性能显著下降。

2. **测试时无标签数据**：推理阶段只有模糊输入，没有对应的清晰图像作为监督信号，无法直接微调模型。
