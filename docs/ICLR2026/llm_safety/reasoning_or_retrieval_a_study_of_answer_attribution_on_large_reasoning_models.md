...
本文的方法分两层：先用一套**推理-检索联合扰动框架**把"最终答案到底来自 CoT 推理还是记忆检索"这个混在一起的问题拆开——分别在推理路径和检索路径上施加可控扰动，看答案往哪边倒，从而量化两种机制各自的贡献；再基于"检索是 RL 里的奖励捷径"这一发现，提出 **Farl（遗忘增强强化学习）**，在 GRPO 训练中穿插遗忘步骤，主动抑制检索路径、逼模型靠真实推理拿奖励。

### 关键设计

**1. 推理扰动：检验答案是否真由 CoT 决定。** 如果最终答案确实由推理过程产生，那么改写推理过程就应该改写答案。具体做法是在模型生成的 CoT $z$ 末尾注入一条误导性线索 $c$（例如"可靠专家建议答案是 B"），把这段被篡改的 CoT 预填充回提示里让模型续写，得到新答案 $\mathcal{M}(x \| z \| c; \theta) = y'$。若 $y'$ 翻成了误导目标 $y_r$，就说明这一题的答案对 CoT 内容敏感、确实走推理路径；若答案纹丝不动，则说明它另有来源。

**2. 检索扰动：把"记忆"单独拎出来制造对照。** 为了独立操纵检索路径，作者用监督微调"毒化"模型记忆，强制它记住特定题目与某个错误答案的关联，即 $\min_\theta \ell(y_t, \mathcal{M}(x;\theta))$，其中毒化目标 $y_t$ 取原始模型 logit 最高的那个非正确答案（这样改动最小、最像模型"本来就可能记错"）。为保证只动记忆不动通用能力，毒化用 LoRA（$r=64,\ \alpha=16$）配 AdamW（学习率 1e-4）训练 8 个 epoch，局部且高效。

**3. 联合扰动：让推理和检索当场"拔河"。** 把上面两种扰动同时加到一道题上，就能看到两条路径直接竞争的结果 $\mathcal{M}(x \| z \| c; \theta') = y'$。作者设计了两种条件：(i) 两种扰动指向同一个错误答案（$y_r = y_t$），观察效果是否协同放大；(ii) 指向不同的错误答案（$y_r \neq y_t$），此时最终答案倒向谁，就直接暴露了该题里哪条路径占主导。这套对照让"两种机制同时存在且竞争"从猜想变成可测量的现象。

**4. 三个量化指标。** 为把扰动效果汇成数字，作者定义推理扰动成功率 $\text{R-PSR} = \mathbb{E}_{(x,y)}\,\mathbf{1}[y' = y_r]$ 衡量答案被 CoT 改写的比例、检索扰动成功率 $\text{T-PSR} = \mathbb{E}_{(x,y)}\,\mathbf{1}[y' = y_t]$ 衡量答案被毒化记忆改写的比例；两者越低说明对应路径越难被左右、模型越稳健。此外用事后解释率 PER 记录"CoT 在逻辑上反过来圆被毒化答案"的比例，专门抓那种先认错答案、再伪造一段推理来自洽的事后合理化行为。
