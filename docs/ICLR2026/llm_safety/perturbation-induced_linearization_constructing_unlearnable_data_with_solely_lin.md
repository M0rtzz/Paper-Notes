...
**1. 语义混淆（Semantic Obfuscation）：把图像本身变成无用信息。**

保护数据的第一步，是让深度模型再也无法从原图里学到有用的类别线索。PIL 优化扰动 $\delta_1$，使线性代理在去掉扰动的部分 $x-\delta_1$ 上输出接近均匀分布，也就是最小化它与均匀分布的 KL 散度。直觉是：一旦深度模型被诱导得像线性模型，$x-\delta$ 这一部分就不再携带可区分的分类信息，原始语义被「抹平」成噪声。

**2. 快捷学习（Shortcut Learning）：把扰动本身变成强类别信号。**

光抹掉语义还不够，还要给模型一条更省力的「捷径」让它去走。PIL 优化扰动 $\delta_2$，使线性代理能直接从 $\delta_2$ 准确预测标签，即最小化交叉熵。深度模型天生爱偷懒，一旦扰动里藏着和标签强相关的简单特征，它就会优先学这个捷径而非图像的真实语义——结果是模型在扰动训练集上看似学得很好，却完全无法泛化到干净测试数据。

**3. 联合优化：用一个扰动同时达成两个目标。**

语义混淆需要的 $\delta_1$ 和快捷学习需要的 $\delta_2$ 在实现上并不分开优化，而是合并成同一个扰动 $\delta$ 的单一目标：

$$L_{total} = \lambda L_{CE}(f_{lin}(\delta), y) + (1-\lambda) L_{KL}(f_{lin}(x-\delta), \text{uniform})$$

其中 $\lambda=0.9$，明显偏重快捷学习一侧——让扰动先成为足够强的类别信号，再附带把残余语义压平。优化用 PGD 式更新，步长 $\alpha=8/2550$，并且要先在干净数据上预训练好线性模型、再以它为固定代理来优化每个样本的扰动。
