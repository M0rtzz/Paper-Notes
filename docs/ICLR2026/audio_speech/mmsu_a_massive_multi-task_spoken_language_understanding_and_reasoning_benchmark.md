...
### 整体框架
MMSU 是一个由 5000 条专家标注多选题（MCQ）、47 个任务组成的语音理解与推理基准，最大的特点是把任务分类体系直接建在语言学理论之上而非"有什么数据评什么"。整个基准用三级层次组织起来：最顶层把能力切成感知（Perception，24 个任务）和推理（Reasoning，23 个任务）两条主线；中间层区分语言学（Linguistics）和副语言学（Paralinguistics）；最底层再细分到语义、音韵、说话人特征、说话风格等具体维度。这套结构让"说了什么 / 怎么说的 / 真正想表达什么"三个层面都有对应任务落点。

### 关键设计

**1. 细粒度声学特征覆盖：让评测触及"怎么说的"。**

现有语音基准大多停在语义层，对停顿、重音、延长音、语调变化这些非语言信号几乎不评，而这恰恰是日常语音里携带大量信息的部分。MMSU 以语音学各子领域理论为骨架，为每一类声学现象单独设计任务：非语言声音（哭泣、咳嗽）、口音（印度英语、英式英语）、情感状态、韵律特征（重音、延长音、停顿）、语调升降等都有专门的题目去考。这样做的结果是，模型不能只靠转写文本蒙混过关，必须真正"听"出声学线索才能答对，从而把感知能力从语义理解中单独剥离出来度量。

**2. 高质量真实语音数据：用人声而非 TTS 撑起声学多样性。**

合成语音无法复现人类发声中那些微妙的声学细节，而这些细节正是音韵、副语言任务的关键。MMSU 因此优先采集真实人类语音，辅以专业配音演员录制和少量多说话人补充，并用一条四阶段流水线保证质量：先做语言学框架设计，再做问题收集与选项增强，然后是音频采集，最后由 10 名标注员经过多轮人工审核把关。先理论后数据的顺序，保证了每条样本都能对应到明确的语言学考察点，而不是事后硬贴标签。

**3. 语言学理论系统融入：从六个子领域自上而下设计任务。**

这是 MMSU 区别于以往基准最核心的一点——它不是先有一堆音频再凑任务，而是从语音学、韵律学、修辞学、句法学、语义学、副语言学六个子领域出发，反推出需要哪些任务来覆盖完整的语言理解能力。由此首次纳入了绕口令、讽刺检测、同音异义推理、语调推理、对联匹配等以往基准从未涉及的题型。这种自上而下的设计让评测有了学科深度，能系统暴露出模型在某个语言学层面的盲区，而不是只给出一个笼统的平均分。
