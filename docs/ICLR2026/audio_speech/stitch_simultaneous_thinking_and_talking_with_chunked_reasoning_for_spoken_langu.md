...
### 整体框架
Stitch 在 SLM 原有的 text token 与 speech token 之外引入第三种 token——reasoning token（无声推理），三者按块交替排列。推理 token 被特殊标记 [SOPR]/[EOPR] 包裹，只参与思考、不送进声码器合成语音。关键想法是把这段推理塞进"上一块语音正在播放"的空窗里：模型一边播音频一边算下一轮推理，于是思考几乎不占用用户能感知的等待时间。
