...
SiNGER 要解决的是 ViT 蒸馏里一个具体的痛点：教师特征本身带着高范数伪影，直接拿来当蒸馏目标会把学生带偏。它的做法不是改学生，而是先"净化"教师——在教师模型的每个 Transformer 块之后挂一个轻量 LoRA 适配器，对教师特征施加一个**零空间方向的扰动**，把高范数伪影的能量重新分配掉，但保证下一层看到的信息一字不差。

整条流程是这样转的：教师前向传播产生原始特征，LoRA 适配器对每层特征施加零空间引导的扰动、压制掉伪影 token，精炼后的教师特征再作为蒸馏目标传给学生，学生用标准的特征匹配 MSE 损失去逼近它。最终得到的学生模型既继承了教师的知识，特征图又比直接蒸馏更干净、更可解释。

### 关键设计

**1. 高范数伪影分析与建模：把"伪影从哪来"讲成可操作的数学问题。**

要抑制伪影，先得知道它的方向长什么样。作者把伪影归因于残差块的连续累积效应——token 特征每经过一个残差块，就沿权重矩阵的主左奇异向量方向多攒一点能量，逐层叠加形成所谓的"奇异缺陷"。形式化地说，若某层权重 $W$ 的 SVD 分解为 $W = U\Sigma V^\top$，则伪影 token 的方向近似对齐到 $u_1$，也就是最大奇异值对应的左奇异向量。这个分析直接解释了 KD 为什么会被带偏：在 MSE 蒸馏目标下，梯度 $\nabla \propto \|h_t - h_s\|$ 被范数最大的那些 token 主导，于是优化被少数离群伪影牵着走，真正有意义的信息信号反而学不进去。把伪影的方向刻画清楚，后面"往哪个方向改"才有理论落点。

**2. 零空间引导扰动（Nullspace-Guided Perturbation）：找一个改了也不影响下游的方向。**

伪影抑制和信息保留之所以看起来矛盾，是因为常规做法（如随机遮掩）一动就连有用信号一起动。SiNGER 的破局点在于找到了一个"免费"的修改空间。设教师第 $l$ 层的输出特征为 $h^{(l)}$，下一层权重矩阵为 $W^{(l+1)}$，它的左零空间记作 $\mathcal{N}(W^{(l+1)\top})$，即所有满足 $W^{(l+1)\top} v = 0$ 的向量 $v$ 张成的子空间。关键观察是：只要扰动 $\delta$ 完全落在这个零空间里，就有

$$W^{(l+1)}(h^{(l)} + \delta) = W^{(l+1)}h^{(l)}$$

下一层的输出**完全不变**。换句话说，在零空间方向上想怎么改都行，下游一点都感觉不到。于是就能把高范数伪影的能量沿零空间方向"搬走"、重新分配到更均匀的分布上——伪影被压制，信息却数学上零损失。这正是把"伪影抑制 vs 信息保留"从不可兼得变成可同时保证的那一步。

**3. LoRA 适配器实现：用低秩结构高效参数化零空间扰动。**

直接算完整的零空间投影代价太大——每层都要做 SVD。SiNGER 改用 LoRA 来近似实现：在教师每个 Transformer 块后附一个低秩模块，扰动写成 $\delta = BA \cdot h$，其中 $B \in \mathbb{R}^{d \times r}$、$A \in \mathbb{R}^{r \times d}$，秩 $r \ll d$。它相对教师骨干只多了可忽略的参数量，低秩结构又天然契合"在一个子空间里施加扰动"这件事。真正把理论和实践接起来的是初始化——$B$ 的列被初始化为下一层权重矩阵左零空间的基向量，这保证了训练一开始扰动严格落在零空间方向上（设计点 2 的零损失性质成立），随着训练推进适配器再去学更灵活的扰动。零空间初始化就是连接"零空间引导"这套理论和"LoRA 参数化"这套实现的桥梁。
