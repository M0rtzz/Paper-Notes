...
Chroma 的核心思路是用一组小专家替代一个大通用模型：训练阶段把数据按元数据切片，每片养一个小专家，凑成一个模型组合（portfolio）；测试阶段在验证窗口上挑出最合适的专家或把几个专家加权拼起来。整套实现都建在 Chronos-Bolt（T5 encoder-decoder）之上，单个模型从 1M 到 9M 参数不等，所以即使组合多个专家，激活的有效参数也远小于 200M+ 的单体大模型。

### 关键设计

**1. 专家化 portfolio：用数据多样性而非随机种子换取专家差异。** 传统集成靠不同随机种子训练同构模型来制造多样性，但这只能降方差。Chroma 转而按元数据切分训练数据，让每个专家天然擅长一类信号。切分有两个维度：按**频率**（hourly / daily / weekly / monthly / subhour）捕捉时间尺度差异，按**领域**（energy / retail / transport / weather / web / various）捕捉应用域差异；每个专家只在自己那一片数据上训练，组合里再额外保留一个在全量数据上训的通用模型兜底。实验里按频率切分稳定优于按领域切分——WQL 低约 4%、MASE 低约 5%，这与 TTM 等先前工作的结论一致，原因是频率直接决定了序列的周期与采样尺度，比抽象的应用域更能划出彼此分离的子分布。

**2. 后训练高效构建：先训通用模型再短微调，把组合成本压到约 1/10。** 如果每个专家都从零训 200K steps，养 $N$ 个专家的总成本就是 $200\text{K} \times N$ steps，专家越多越吃不消。Chroma 改成两阶段：先花 200K steps 训一个通用模型，再把它在每个数据分区上各微调 1K steps 得到对应专家。这样总成本降到 $200\text{K} + 1\text{K} \times N$ steps，当专家数较多时加速比约为 $\frac{200\text{K} \times N}{200\text{K} + 1\text{K} \times N} \approx 10\times$。关键是这种后训练得到的专家在准确率上与从零训练的专家没有显著差异，甚至 scaling 行为更好，所以这 10× 的加速几乎是白赚的。

**3. 测试时智能组合：用验证集挑专家，而不是无脑平均。** 拿到一个新任务时，Chroma 在验证窗口上评估组合里的每个模型，给出两种用法：**模型选择**直接选验证表现最好的单个专家，开销是 $N+1$ 次前向传播；**贪心集成**用 Caruana et al. (2004) 的集成选择算法在验证窗口上迭代挑模型并定权重，得到加权预测 $\hat{y}_{\text{ens}} = \sum_{m=1}^M w_m \cdot \hat{y}_m$，开销约 $N + 2.5$ 次前向传播。这里"智能"是关键：直接对 portfolio 做简单平均或性能加权平均反而会明显掉点（WQL 上升 0.14–0.18），因为预训练模型的误差由 bias 主导，盲目平均无法消除系统性偏差，只有按验证表现筛掉不合适的专家才能真正降低 bias。
