...
### 整体框架
RLDP 在离线数据集上预训练一个编码器和潜空间动力学模型，用自监督的下一状态预测学特征、用正交正则化防止特征坍塌；测试时面对任意新奖励，借助 successor features 框架把奖励线性回归到特征上、直接组合出策略，全程不与环境交互。整套目标只有动力学预测和正交正则两项，比现有 BFM 动辄前向-后向模型、对比学习、多样性目标的设计简单得多。

### 关键设计

**1. 潜空间下一状态预测：用自监督信号学动力学相关特征。**

现有 BFM 为了让状态特征的线性张成空间覆盖更多奖励函数，往往设计复杂的表示学习目标，但这需要充足的数据覆盖率才能训出有用的张成特征。RLDP 退回到最朴素的自监督信号：编码器 $\phi$ 把状态映射到潜空间 $z=\phi(s)$，动力学模型 $f$ 在潜空间预测下一状态 $\hat{z}'=f(z,a)$，训练目标是最小化预测误差 $\mathcal{L}_{\text{dynamics}}=\|f(\phi(s),a)-\phi(s')\|_2^2$。这个目标直觉上会逼特征编码环境动力学，但单独使用会退化——模型倾向于把所有状态特征坍塌到相近方向，因为只需很小的幅度变化就能编码动力学信息，代价却是特征 span 急剧缩小，正好破坏了 BFM 最依赖的特征多样性。

**2. 正交正则化：一项约束顶住特征坍塌。**

坍塌的本质是不同状态的特征趋于相似、张成空间塌缩。RLDP 不去重新设计目标，而是直接给特征矩阵加一项正交约束 $\mathcal{L}_{\text{ortho}}=\|\Phi^{T}\Phi-I\|_F^2$，其中 $\Phi$ 是一个 batch 内所有状态特征堆叠成的矩阵。它鼓励不同状态的特征尽量正交，把 Gram 矩阵推向单位阵，从而维持特征空间的丰富度。这一项精确对症前一步的退化模式：动力学预测负责让特征「有语义」，正交正则负责让特征「不坍塌」，二者缺一不可。最终训练目标就是两项相加 $\mathcal{L}_{\text{total}}=\mathcal{L}_{\text{dynamics}}+\lambda\mathcal{L}_{\text{ortho}}$，超参 $\lambda$ 控制正交约束强度——过大会压制动力学学习，过小则压不住坍塌，需要落在一个最优区间。

**3. 零样本策略恢复：用 successor features 把新奖励一步转成策略。**

预训练给出的特征 $\phi(s)$ 一旦足够多样，任意新奖励都能近似写成特征的线性组合 $r(s)\approx w^{T}\phi(s)$。测试时只需对新奖励做一次线性回归求出权重 $w$，再借助 successor features 把 $w$ 线性组合到预先算好的后继特征上得到 Q 值，即可零样本恢复出近最优策略，无需任何额外训练或在线交互。这也解释了为何特征 span 至关重要：能被零样本恢复的奖励，恰好是落在特征张成空间内的那些。

整套方法完全基于离线数据预训练，编码器和动力学模型都是简单 MLP，落地时正交正则几乎是一行代码 $\|\Phi^{T}\Phi-I\|$，不引入任何额外网络结构。
