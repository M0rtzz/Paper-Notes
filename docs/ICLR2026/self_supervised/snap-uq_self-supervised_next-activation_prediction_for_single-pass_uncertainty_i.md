...
SNAP-UQ 在骨干网络的 2-3 个选定"接驳层"（taps）$\mathcal{S}$ 处各挂一个微型预测头：它从上一层激活的低秩投影预测下一层激活的条件高斯参数，再把实际激活与预测的标准化偏差当作"surprisal"分数。推理时这些分数加权求和、经一次轻量逻辑斯蒂映射就得到不确定性估计，全程只走一遍前向传播。
