...
TEMU-VTOFF 把虚拟脱衣拆成两个同架构的 DiT 分工协作：特征提取器 $F_E$ 读穿衣人物图，逐层吐出富含服装结构信息的中间特征；服装生成器 $F_D$ 拿着这些特征做去噪，生成标准平铺商品图。二者通过一套多模态混合注意力把"图像特征、文本品类、掩码区域"三路信息融在同一注意力里，从而在遮挡和复杂姿态下消解视觉歧义。
