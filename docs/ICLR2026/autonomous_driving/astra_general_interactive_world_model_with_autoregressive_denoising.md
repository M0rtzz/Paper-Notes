...
### 整体框架
Astra 想把一个只会生成固定短片的视频扩散模型，改造成能边走边响应动作的交互式世界模型。它直接复用预训练的 Wan-2.1，把生成过程拆成 chunk-wise 自回归：每一步只预测下一个 33 帧的视频块，生成完追加到历史里、再以这段历史为条件预测下一块，靠时序因果注意力把过去的内容聚合进来。这样长程 rollout 就变成"一块接一块"的滚动生成，而真正的难点不在于怎么续写，而在于续写时不能只顺着过去帧外推、要听新进来的动作指令——下面四个设计都是围绕这个矛盾展开的。
