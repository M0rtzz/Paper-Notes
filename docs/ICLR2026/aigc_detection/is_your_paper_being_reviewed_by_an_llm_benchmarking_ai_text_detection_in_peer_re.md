...
这是一篇 benchmark 论文，工作由两块拼成：先用配对的"人类评审 vs. AI 评审"建起迄今最大的同行评审检测数据集（788,984 篇，两类各半），再在这套数据上系统比较 18 种通用检测方法，并提出一个专为评审场景设计、利用论文原文作为上下文的 Anchor 检测法。整篇的核心赌注是——同行评审不同于一般文本检测，评审者手里有一份共同的"原文"可供锚定，把这份原文用起来就能在低误报率下把检测拉到可用区间。

### 关键设计

**1. 配对式数据集构建：让人类与 AI 评审针对同一批论文一一对应**

人类侧从 OpenReview API 与 ASAP 数据集抓取 ICLR 2017–2024、NeurIPS 2016–2024 的真实评审；AI 侧则用 5 种主流模型（GPT-4o、Claude Sonnet 3.5、Gemini 1.5 Pro、Qwen 2.5 72B、Llama 3.1 70B）对同一批论文各生成一份评审，prompt 里塞入会议专属的审稿指南与评审模板，并对齐论文的最终决策（accept/reject）以保证 AI 评审在倾向上不偏离真实情形。最终 788,984 篇评审切成校准集（75,824）、测试集（287,052）和扩展集（426,108）三份。一个容易被忽略却关键的处理是：主实验只取 ChatGPT 发布前（2022 年底之前）的人类评审，因为之后的人类评审本身可能已被 LLM 污染，混进来会让"人类"标签失真。

**2. Anchor 检测：用论文原文锚出一份参考评审，再比语义相似度**

这是全文方法侧的核心贡献，吃的是评审场景独有的红利——待测评审 TR 对应的论文原文是已知的。流程是：先用任一 LLM 对该论文现场生成一份锚定评审 AR，再用嵌入模型（text-embedding-003-small）把 AR 与 TR 各自编码成向量，算两者的余弦相似度，若超过从校准集学到的阈值 $\theta$ 就判为 AI 生成。它之所以管用，在于一个语义层面的直觉：不同 LLM 读同一篇论文写出的评审，因为都被同一份原文牵引，在语义空间里彼此更靠近；而人类评审的个体差异天然更大，离 AR 更远。于是 TR 与 AR 的距离本身就成了"像不像机器写的"的信号，而通用方法（只看 TR 的 token 概率、困惑度等）拿不到这层原文锚定，自然吃亏。

**3. 多 LLM 投票：应对来源未知的真实场景**

实际检测时并不知道对方用了哪个模型生成评审，单一 AR 可能锚不准。为此 Anchor 用多个 LLM 各生成一份锚定评审，对 TR 分别比相似度，任一份触发阈值即判为 AI 生成。这把"猜中生成模型"的难题转成了"只要命中其中之一"的宽松条件，代价是多调几次 API。

**4. 18 种基线的统一评测：覆盖三大类通用检测范式**

为给 Anchor 一个公平的参照系，论文把 18 种开源通用检测法在同一数据集上跑通，分三类：基于训练模型的方法（如 MAGE-D）、基于统计指标的方法（Log-Likelihood、Rank、Entropy、DetectGPT、FastDetectGPT、Binoculars 等）、以及基于语言特征的方法（如 GLTR）。这套对照的意义不只是排名，更在于揭示这些方法在同行评审这种特定分布、且要求极低 FPR 的设定下普遍失灵，从而衬出"利用原文"这一思路的必要性。
