...
PoliCon 把一次政治共识任务抽象成「给定多方立场、起草一份能被表决通过的决议」，并围绕欧洲议会的真实审议数据搭建评估闭环。它先把13年的议会记录清洗成结构化语料，再用四个可调因子批量生成任务场景，最后用一个基于社会选择理论的评估器把 LLM 写出的开放式决议映射成可比较的标量分数，从而对6个前沿模型做统一打分。
