...
### 整体框架
这篇论文要回答一个反直觉的问题：fine-tuning 在 LLM 适配里是最成功的方法，为什么一到模型编辑就被当成"过拟合、灾难性遗忘"的弱 baseline？作者审查现有代码库后发现，问题不在 fine-tuning 本身，而在它被实现成了一套**非标准**的训练流程。LocFT-BF 的做法很朴素：把训练流程换回标准的广度优先 mini-batch 训练，再把可训练参数收窄到后几层的 down/up projection。输入是一串编辑请求 $(s, r, o \to o')$，整套流程不预计算任何矩阵、不加辅助网络、不改架构，直接梯度下降得到新参数 $\theta^*$。
