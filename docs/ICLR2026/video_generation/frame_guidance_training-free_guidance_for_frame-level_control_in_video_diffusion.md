...
Frame Guidance 不动预训练 VDM 的任何参数，而是在采样的每一步把当前预测的干净视频 $x_{0\vert t}$ 解码出来，对用户关心的若干帧算一个任务损失 $\mathcal{L}_e$，再把梯度回传去微调当前的 latent $z_t$。难点全在"怎么算得起这个梯度"和"什么时候算才有用"两件事上——前者靠 Latent Slicing 把显存压下来，后者靠 VLO 把引导集中在去噪早期，二者配合才让大模型上的帧级引导第一次变得可行。

### 关键设计

**1. Latent Slicing：把 CausalVAE 的全序列解码缩成一个小窗口。** 视频引导卡在显存上：CausalVAE 的因果依赖意味着要重建任意一帧都得解码整条 latent 序列，对它求梯度的显存高达 650GB 以上，根本没法在单卡上跑。作者观察到一个反直觉的事实——CausalVAE 虽然设计成因果结构，实际扰动一个 latent 只会影响相邻的少数几帧，呈现出明显的时间局部性（temporal locality）。于是重建第 $i$ 帧时不必解码全序列，只取一个 3 个 latent 的窗口来解，再对 latent 做 $2\times$ 空间下采样后才进 VAE 算损失。两步叠加把引导所需显存最多压低 **60×**，单张 GPU 就能对 Wan-14B 这种 14B 量级的模型施加帧级引导。

**2. 梯度必须穿过去噪网络才能管住全局一致性。** 即使只对切片出来的少数帧算损失，更新也不能图省事直接改那几帧的 latent。引导梯度要通过去噪网络 $v_\theta$ 回传，才能从被引导的帧扩散到整段视频，维持时序一致性；若用 shortcut 式更新（跳过网络直接改 latent），影响就被锁死在被引导帧上，相邻帧跟不上、画面会在引导帧处出现时序断裂。换句话说，Latent Slicing 省的是"解码"那一段的开销，但"梯度过网络"这一段必须保留——它正是少数引导帧能控制整个视频的传导路径。

**3. Video Latent Optimization (VLO)：按去噪阶段分配引导强度。** 图像领域常用的 time-travel trick 在视频上会把早期引导信号冲刷掉，而视频帧的全局布局恰恰在去噪最初几步就已基本定型，早期引导对时序一致性最关键。VLO 据此把采样轨迹切成三段差异化处理：早期（$t > t_E$）用确定性更新 $z_t \leftarrow z_t - \eta \nabla_{z_t} \mathcal{L}_e$，把引导信号牢牢保住；中期（$t_E \geq t > t_L$）改用带 re-noising 的随机更新，修正前期累积的误差；后期（$t \leq t_L$）撤掉引导，让模型自由细化纹理细节。强引导只花在"决定布局"的早期，既保住了控制力又不破坏画质。
