...
### 整体框架
LoRA-Edit 不改动预训练 I2V 模型的任何架构，只在其上插入一个轻量 LoRA，并通过两套互补的时空 mask 配置对这条 LoRA 做单视频微调：一套让模型从源视频前景区域学习运动模式，另一套让模型从用户编辑的参考帧学习目标外观。两者共享同一套 mask conditioning 通道，训练完成后可在推理时自由组合，从而在保留背景的前提下既继承原始运动又替换目标外观。
