...
这篇论文要解决的是一个看似细枝末节、实则致命的失败模式：Agent 在推理前没人告诉它"该用哪套认识论"，于是把频率主义和贝叶斯混着用，产出在任何一个框架里都站不住脚的结论。Universe Routing 的做法是把"选框架"这一步从大模型手里拿出来，交给一个独立的轻量路由器先做硬分类。整条链路只有三步：先把问题域形式化成 7 个互斥的信念空间（论文称之为"宇宙"），再训一个小路由器把输入问题判到唯一一个宇宙，最后把问题转给那个宇宙的专用求解器。关键在于路由是**硬**的——用 argmax 选一个宇宙，而不是像传统 MoE 那样对多个专家做加权平均。
