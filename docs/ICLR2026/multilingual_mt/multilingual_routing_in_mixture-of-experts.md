...
这项工作先用一套路由散度指标把MoE模型每一层、每种语言的专家激活模式量化出来，定位出"哪些层、哪些专家"在跨语言上行为一致或分化；再用激活频率差异挑出语言特化专家和任务特化专家两组；最后在中间层做一次极简的推理时路由干预，强行激活英语任务专家，从而提升非英语性能。三个阶段是串行依赖的：散度分析读出"该在哪些层动手"，专家识别读出"该动哪些专家"，两者一起喂进干预这一步。所有分析都横跨Qwen3-30B-A3B（48层）、Phi-3.5-MoE（32层）、GPT-OSS-20B（24层）以及作为多语言能力对照的小模型OLMoE，四者在宽度、稀疏度、深度上各不相同，保证结论不是单一模型的偶然现象。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    IN["MoE 模型 + FLoRes-200<br/>平行翻译语料"]
    A["路由散度分析<br/>熵归一化 JS 散度量化<br/>每层跨语言路由差异 → U 型曲线"]
    C["专家识别<br/>激活频率差 Δ 挑出<br/>语言专家 / 任务专家"]
    D["路由干预<br/>仅在中间层强行激活<br/>英语任务专家"]
    OUT["非英语性能提升 1-2%<br/>英语性能基本不变"]
    IN --> A
    IN --> C
    A -->|"中间层范围"| D
    C -->|"任务专家"| D
