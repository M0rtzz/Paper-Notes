...
Judge's Verdict 是一套用来评估"LLM 评委到底有多像人"的两步方法论：先用 Pearson 相关性做粗筛，把连打分趋势都对不上的模型挡在门外；再用 Cohen's Kappa 衡量绝对一致性，并把 LLM 混进人类标注组做一次"评委图灵测试"，看它能否在统计上与人类标注者不可区分。最终配合一套分层分类体系，把 54 个模型归入人类相似型、超一致型和不合格三档。

### 关键设计

**1. 相关性过滤：先挡掉连趋势都对不上的模型**

LLM 评委评估长期被 Pearson 相关系数主导，但相关性只能反映线性趋势，无法保证绝对值对齐——一个系统性偏严的评委可以拿到很高的 r 却严重偏离人类打分。本文把相关性降级为"必要而非充分"的第一道门槛：计算每个 LLM 评委的 Answer Accuracy 分数与人类共识分数（3 位标注者均值）的 Pearson r，门槛设为 r ≥ 0.80（"very strong"级别）。这一步的作用不是给出最终结论，而是先把那些连线性趋势都无法对齐的模型剔除，避免它们污染后续更敏感的一致性分析。

**2. Cohen's Kappa 双重一致性测试：从"趋势对齐"升级到"判断不可区分"**

通过粗筛的模型进入真正的核心环节，作者用两种互补视角衡量绝对一致性。其一是静态基线比较：分别计算 LLM 与 3 位人类标注者各自的 Cohen's Kappa 后取均值 $\bar{\kappa}_{LLM}$，再与人类彼此之间的一致性基线 $\kappa = 0.801$ 对照，看 LLM 的"和人对齐程度"是否逼近"人和人对齐程度"。其二是动态组分析，也就是评委图灵测试：把 LLM 混入 3 位人类标注者组成一个 4 人组，计算所有两两配对的 Kappa，再用 z-score 判断这个 LLM 能否在人类组里"隐身"，即 $z = (\kappa_{LLM} - \mu_{human}) / \sigma_{human}$。$|z|$ 越接近 0，说明 LLM 的判断模式越落在人类自然变异范围内；$z$ 显著大于 0 则意味着它比人类彼此还要一致。Kappa 直接惩罚系统性偏置，z-score 又把"是否与人类不可区分"量化成可检验的统计指标，二者结合才真正回答了相关性回答不了的问题。

**3. 分层分类体系：把一致性模式拆成三档而非一刀切**

两步结果被映射到一个三层分类上：Tier 1A 人类相似型要求 r ≥ 0.80 且 $|z| < 1$，判断模式与人类自然变异一致；Tier 1B 超一致型要求 r ≥ 0.80 且 $z > 1$，一致性反而超过人类彼此间水平；其余落入 Tier 2 及以下。这套划分的价值在于它没有把"超一致"简单等同于"更好"——人类相似型适合需要保留判断多样性的场景，超一致型适合追求最大可复现性的场景，分层让选型可以对接实际需求而不是只看一个总分。

**4. Answer Accuracy 评分设计：用双 prompt 压住位置偏差**

评委本身的打分接口沿用 RAGAS 框架的双 prompt 策略：两个独立的评委 prompt 以不同顺序输入问题、生成答案与参考答案，各自给出离散分数 $S \in \{0, 2, 4\}$，归一化后取均值。之所以要跑两遍并换序，是因为单 prompt 评委容易受输入位置影响而产生偏置，互换顺序后再平均能显著缓解这种位置偏差，让分数更稳定地反映答案质量本身。

**5. 数据集构建：用异构来源 + 高质量人工标注托住基准可信度**

整个评估建立在 6 个异构基准共 1994 个样本之上——SQuAD v2.0（346）、HotPotQA（342，多跳推理）、Coral（318，对话式 QA）、TechQA（295，技术问答）、DC767（347，含表格/图表的 PDF 文档）、EKRAG（346，含财报/SEC 文件的企业知识 RAG）。覆盖从简单事实问答到企业级长文档的多种难度，避免结论被单一数据分布带偏。3 名北美专家对每个样本独立标注，共产出 5982 条标注，标注者间一致性 Fleiss κ = 0.79、Krippendorff α = 0.79，这两项指标本身就说明人类基线足够可靠，后续拿它来校准 LLM 才站得住脚。
