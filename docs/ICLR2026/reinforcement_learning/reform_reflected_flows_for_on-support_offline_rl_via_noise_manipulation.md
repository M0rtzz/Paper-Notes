...
整体分两个阶段串起来。第一阶段先用行为克隆（BC）学一个流策略，把一个**有界**的均匀噪声分布映射成数据集的行为分布；这样 BC 策略的"可达动作"就被噪声的有界区域钉死了。第二阶段冻结这个 BC 策略，再额外学一个噪声生成器，它只在那块有界噪声区域内部重新分配噪声，把噪声往能让 Q 值更高的地方挪——因为生成器的输出仍然没跑出有界区域，喂给 BC 策略后输出动作就始终在支撑内。
...
**2. 反射流噪声生成器：在有界球内重排噪声，既守约束又保多模态**
