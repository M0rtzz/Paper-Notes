...
description: >-
  [ICLR 2026][强化学习][离线强化学习] 提出生成轨迹策略（GTP），通过统一视角将扩散模型、流匹配、一致性模型视为ODE解映射的特例，学习完整的连续时间轨迹解映射，并引入分数近似和优势权重两个适配技术，在D4RL基准上实现SOTA。 1. 领域现状： 离线RL中生成模型因能捕获复杂多模态行为分布而成为强大的策略类…
tags:
  - "ICLR 2026"
  - "强化学习"
  - "离线强化学习"
