...
### 整体框架
AWM 要回答一个二选一的问题：可疑模型到底是从某个基础模型偷改来的，还是从头独立训练的？难点在于攻击者可以缩放、置换、剪枝甚至旋转权重矩阵来掩盖来源，但前提是模型输出不能变。AWM 的思路是先把这些「合法操纵」的形式从架构里推导清楚，再逐一消掉。整个流程分两步走：先在两个模型共享词表的嵌入矩阵上，用线性分配问题（LAP，匈牙利算法）恢复出列置换矩阵 $P$ 和符号翻转矩阵 $D$；再拿这对 $P,D$ 把 Q/K 矩阵对齐回去，最后用无偏 CKA 逐层算相似度——CKA 本身就免疫正交变换和缩放，所以剩下那部分扰动不用显式去解。
