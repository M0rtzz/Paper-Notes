...
P-GenRM 要解决的问题是：怎么让奖励模型读懂"这个具体用户在这个具体场景下想要什么"，并且在用户信息很少时也能给出靠谱评分。它的输入包括当前查询 $q_t$、用户的隐式偏好历史 $H_t^{(u)}$（若干轮交互里的 chosen/rejected 回复对）、可选的显式偏好准则 $E^{(u)}$，以及待评分的候选回复。和传统奖励模型直接吐一个标量不同，P-GenRM 输出一条完整的结构化评价链（Structured Evaluation Chain, SEC）：先推断用户在当前场景下的画像（persona），再从画像导出带权重的评分标准（rubric），最后逐条给候选回复打分并汇总成最终分。模型经过三阶段训练得到（SFT 学格式 → RL 学推理 → 课程学习学区分难样本），推理时再叠加双粒度测试时 scaling 进一步降噪、补冷启动。

### 关键设计

前三个设计点对应三阶段训练，后三个对应推理时的 user-based scaling。
...
**4. 离线原型初始化与优化：把用户聚成原型，为冷启动迁移铺路**

推理时的 scaling 依赖一套离线准备好的用户原型。作者用 Qwen3-Embedding-0.6B 把每个用户在不同场景下的画像 $P_t^{(u)}$ 向量化、拼接成跨场景偏好嵌入矩阵 $\mathbf{P}$，再对 $\mathbf{P}$ 做 K-means 聚类得到 $k$ 个用户原型。原型不是聚完就用：还要经过一轮 prototype-augmented attention 精炼——用注意力对用户历史记录加权聚合，配一个辨别式损失（让原型能区分 chosen/rejected），再加两个正则项防止原型偏移过大。PCA 分析显示 50 个原型就能覆盖绝大部分用户偏好的变异。这一步借的是推荐系统"相似用户有相似偏好"的协同过滤思想，让后面的原型迁移成立。

**5. 个体级 Scaling：对同一用户多次采样取平均，压低单次推断的噪声**

对当前查询，P-GenRM 并行采样 $m$ 次，每次可能推断出略有差异的画像和评分标准，生成多套评分方案后取平均分。本质是在偏好推断空间里做多假设探索——单次推断难免有噪声，多次平均把它压下去。

**6. 原型级 Scaling：借相似用户的偏好补当前用户的信息缺口**

光靠用户自己的历史，在冷启动时信息太少。原型级 scaling 根据用户的偏好嵌入找到最近的原型，再从该原型下挑 $n$ 个最相似的用户，用他们的偏好历史让 P-GenRM 额外生成 $n$ 套评分，最终分把个体级和原型级的结果一起聚合。这正是协同过滤核心假设的落地——相似用户偏好相似——对历史极少的新用户帮助最大。这里"质比量重要"：实验里 Ind-16+Pro-8 优于纯加个体采样的 Ind-32，但 Pro 开太大（Pro-16）反而引入不一致的噪声偏好、掉点。
