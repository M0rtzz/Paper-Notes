...
description: >-
  [强化学习] 提出 SPELL 框架，让一个 LLM 同时扮演出题者、答题者和验证者三个角色进行自我博弈强化学习，无需人类标注即可持续提升长文本推理能力，在 6 个长上下文基准上一致提升性能。 - 长上下文推理的困境：RLVR（可验证奖励 RL）在数学、代码等有明确正确性判据的领域取得成功，但长文本推理面临两大瓶颈： 1.…
tags:
  - "强化学习"
---
...
SPELL 要解决的问题是：长文本推理缺乏像数学、代码那样的程序化正确性判据，又难以靠人类标注（LongBench-V2 人类准确率仅 25.1%）拿到可靠监督，于是没法直接套用 RLVR（可验证奖励强化学习）。它的破局思路是让**同一套参数 $\pi_\theta$ 同时扮演三个角色**，把"造数据—解题—判对错"这条监督链整个闭合在模型内部。

整套流程在两个阶段间循环往复。**角色专属采样（Role-Specific Rollout）**阶段里，出题者 $\pi_\theta^{\text{que}}$ 先从原始文档生成问答对 $(q, a)$，答题者 $\pi_\theta^{\text{res}}$ 拿到完整文档（含未被出题者看到的干扰文档）后对每道题独立采样 $G$ 个回答，验证者 $\pi_\theta^{\text{ver}}$ 再判断每个回答与参考答案是否语义等价；三类角色各自拿到奖励。**统一策略更新（Unified Policy Update）**阶段里，三路样本经动态采样平衡后汇入一次 GRPO 更新，得到的新策略作为下一轮采样的起点。如此往复，出题、答题、验证三种能力在没有任何人类标注的情况下互相驱动、协同进化。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400, 'subGraphTitleMargin': {'top': 8, 'bottom': 16}}}%%
flowchart TD
    DOC["原始文档集合"] --> Q["出题者：自动课程学习<br/>历史记忆 H 条件 + 文档范围扩展"]
    Q --> QA["问答对 (q, a)"]
    QA --> R["答题者<br/>含干扰文档，采样 G 个回答"]
    R --> V["验证者<br/>判断语义等价"]
    V --> REW["角色专属奖励<br/>出题高斯·答题 max·验证自一致"]
    REW --> S["角色动态采样<br/>三路样本筛选至约 1/G"]
    S --> U["GRPO 联合更新统一策略"]
    U -->|"新策略进入下一轮"| DOC
```
...
**1. 自动课程学习：让题目随能力增长自然变难**

自博弈最怕出题者一直生成同质化的简单问题，模型很快触顶。SPELL 维护一段历史记忆 $\mathcal{H}$，缓存最近 $L=3$ 个已被解决的问答对及其对应文档，并把它们作为条件喂给出题者。难度因此从两个方向同时上升：一是每轮新采样一批文档并入历史文档，出题上下文 $X^{\text{que}}$ 持续扩张，问题依据自然覆盖更长范围；二是把历史问答对 $\{(q_l,a_l)\}$ 显式塞进 prompt，逼出题者既避免重复已解决的内容、又生成比这些更难的问题，从而要求更复杂的多跳推理。这样课程难度和上下文长度都随训练进展水涨船高，无需人工设计难度梯度。出题前还有一道 grounding filter，把"不看文档也能答"的问题丢掉，避免脱离文档的幻觉题。

**2. 角色专属奖励：用自一致性把"语义对错"变成可验证信号**

长文本答案常常语义正确但措辞不同，字符串匹配和多数投票都不可靠，这正是 SPELL 要打通的核心环节——没有可信的对错信号，自博弈就无从优化。验证者的奖励来自自一致性 $r_{i,j}^{\text{ver}} = \mathbb{I}(v_{i,j} = v_i^{\text{ver}})$，即单次判断只要和该样本的多数投票结果 $v_i^{\text{ver}}$ 一致就得分；在规则可判的任务上，多数投票还会向真值规则对齐，从而把可靠的验证能力 bootstrap 出来、再迁移到规则判不了的语义题上。答题者奖励融合规则与验证者两路信号 $r_i^{\text{res}} = \max(\mathcal{R}_{\text{rule}}(y_i, a), v_i^{\text{ver}})$，取规则匹配（CEM，cover exact match，答案串出现即记 1）与验证者投票的较大值，让规则漏判的语义正确答案仍能被验证者救回，抑制假阴性噪声。出题者奖励则用一条以答题成功率 $\bar{r}^{\text{res}}=0.5$ 为中心的高斯函数 $r^{\text{que}} = \exp\!\left(-\frac{(\bar{r}^{\text{res}} - 0.5)^2}{2 \cdot (0.5/3)^2}\right)$ 控制难度——太简单或太难（成功率偏离 0.5）的题目都落进低奖励的尾部，格式错误或脱离文档的问题另受惩罚，从而把出题引导到"恰好处于能力边界"的区间。

**3. 角色动态采样与统一更新：平衡三路样本再合一次梯度**

三个角色的原始样本数严重失衡——一道题对应 1 个出题样本、$G$ 个答题样本、$G^2$ 个验证样本，直接训练会被验证样本淹没、淹没掉最该提升的答题者梯度。SPELL 对每一路分别做有效性筛选：答题者只保留组内奖励有正有负、能产生有效优势的组，验证者则保留多数投票与规则判定一致的样本、对判定冲突的组做子采样以匹配题目数量，整体训练集约缩减到原始的 $1/G$ 量级。筛选后的三路样本共享同一策略，按 GRPO 联合优化 $\mathcal{J}_{\text{GRPO}}(\theta) = \mathcal{J}_{\text{GRPO}}^{\text{que}}(\theta) + \mathcal{J}_{\text{GRPO}}^{\text{res}}(\theta) + \mathcal{J}_{\text{GRPO}}^{\text{ver}}(\theta)$，一次更新同时提升三种能力，再进入下一轮自博弈循环。
