...
这篇论文做两件事：先用一个按认知深度组织的基准 HitEmotion 把「模型到底在哪一层情感推理上掉链子」量化出来，再用 TMPO 训练框架把心智理论（ToM）的中间心理状态当成可监督、可奖励的过程信号，去补上 MLLM 在深层情感推理上的断点。整条链路是：输入多模态信号（视频/图像 + 文本/音频）→ 模型按 ToM 风格提示一阶步步推断心理状态、写进 `<think>` 推理链 → 在 `<answer>` 里给出情感判断 → 训练阶段先 SFT 对齐这种推理格式、再用 GRPO 对推理链做过程级偏好优化。

### 关键设计

**1. HitEmotion 基准：按认知深度把情感任务切成三层，定位推理断点**

现有基准只给一个粗粒度总分，无法回答「模型是在感知层就错了，还是在因果归因层才崩」。HitEmotion 把 24 个任务、20,114 个实例（覆盖视频和图像）按认知深度堆成三层。Level 1 是情感感知与识别（EPR，10 个任务），只要求把多模态信号映射到预定义情感类别，如面部表情识别、多模态情感识别；Level 2 是情感理解与分析（EUA，8 个任务），需要上下文感知和关系推理，如幽默理解、讽刺检测、多方对话情感；Level 3 是情感认知与推理（ECR，6 个任务），要求因果推理和二阶心智推理，如情感诱发推理、情感解释、反讽理解。这样一旦某个模型在 Level 3 大幅掉分而 Level 1 正常，就能精确指认它缺的是高阶认知而非低阶感知。

**2. ToM 对齐的监督微调（SFT）：先教模型把中间心理状态写成结构化推理**

光有基准还不够，模型的 CoT 常常看着连贯实则是模板匹配，没有真正追踪心理状态。SFT 阶段用结构化推理模板约束输出：中间推理步骤一律用 `<think></think>` 包裹、最终答案用 `<answer></answer>` 包裹，目标就是最大化在这种格式下生成黄金推理链的似然：

$$\mathcal{L}_{\text{SFT}}(\theta) = -\mathbb{E}_{((\mathcal{P},T,A,V), y)} [\log \pi_\theta(y | \mathcal{P}, T, A, V)]$$

其中输入由提示 $\mathcal{P}$ 和文本 $T$、音频 $A$、视频 $V$ 模态组成。黄金推理链本身不是人工标的，而是经一条四步流水线产出：LLM 生成 → 过滤 → 增强 → 校正，从而批量得到高质量、追踪心理状态的推理样本。

**3. 基于 GRPO 的 ToM 偏好优化：把心理状态变成过程级奖励，而不只是最终对错**

SFT 只能模仿格式，无法保证推理链「忠实」。Stage 2 对每个输入采样 $N$ 个候选输出，用一个多维奖励同时评判结果和过程：

$$R(y) = \mu_1 R_{\text{structure}} + \mu_2 R_{\text{content}} + \mu_3 R_{\text{process}} + \mu_4 R_{\text{consistency}}$$

四个分量各管一件事：Structure Reward 看推理步骤的顺序是否正确，Content Reward 看最终答案对不对，Process Reward 奖励是否用了领域特定的心理状态语言，Consistency Reward 则对逻辑和事实不一致施加惩罚。关键在于 Process 和 Consistency 这两项让中间心理状态直接进入梯度——它既是监督信号也是奖励来源，而不只是被当成通往答案的中转。优化用 GRPO，在组内相对优势 $A_i$ 上做带 KL 约束的策略提升：
...
**4. ToM 风格提示机制：三层认知复杂度配三种推理模板**

提示设计和基准的三层一一对应，让模型在不同难度上启动不同深度的心智推理。Level 1 是一阶心理状态归因，整合可观察信号去推断情感；Level 2 是关系与上下文心智建模，把情感关联到特定实体或沟通目标；Level 3 是因果归因与二阶推理，解释情感为何产生以及如何被社交地解读。值得一提的是，这套 ToM 提示即便不训练、单作为提示策略，也能显著拉高闭源模型在高层任务上的表现，相当于给推理搭了一层「脚手架」。
