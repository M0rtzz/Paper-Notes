...
Pram（Partitioned Resource Allocation with MLMs）的思路是"分、和、克"：先把整张网络按源节点切成 $|\mathcal{V}|$ 个子问题（分区模块），再让一个共享的多模态语言模型骨干分别处理每个子问题、给出本地流分配（智能体模块），最后用多智能体强化学习让这些本地决策在全局上彼此协调一致（适应模块）。三个模块串起来，把原本 $\mathcal{O}(|\mathcal{V}|^2)$ 的求解维度降到 $\mathcal{O}(|\mathcal{V}|)$，同时借 MLM 的推理与泛化能力替掉手工设计的专用网络。

### 关键设计

**1. 多模态问题分区：把维度诅咒按源节点拆开。**

MCF 的输出维度随节点数平方增长，直接端到端学习既难训练又会爆显存。Pram 选择在源节点这一粒度上切分——每个源节点连同它发往全网其他节点的需求构成一个子问题，于是模型复杂度从 $\mathcal{O}(|\mathcal{V}|^2)$ 降到 $\mathcal{O}(|\mathcal{V}|)$，子问题数量级地缩小变量规模。每个子问题被表达成 MLM 天然能吃的两种模态：图像模态把从该源节点出发到所有目标节点的路由链路绘成一张子图，经 CLIP 视觉编码器送入 MLM，让模型"看见"网络拓扑结构；文本模态则用 subtask-aware prompting，把每条需求和它的子任务描述（源节点信息、历史滚动平均需求等）配对成提示词。之所以选 MLM 而非专门的 GNN/RNN，是因为大规模预训练带来的涌现数学推理与跨拓扑泛化能力，让它无需为每种网络结构重新设计和调参，又能原生处理"图像+文本"的混合输入。

**2. 轻量多智能体适应：让共享骨干的子智能体彼此协调。**

所有子问题共用同一个 MLM 骨干、却各自接收不同观测，因此关键问题是如何在不为每个子问题各训一个大模型的前提下完成通信。Pram 从两个层面做适配：模型内部用 **LoRA** 低秩矩阵微调 MLM 的注意力权重，只引入极少额外参数；模型外部受上下文学习（ICL）启发，引入一组可学习的"全局上下文"嵌入作为输入提示前缀，这些上下文参数充当 query，通过多头交叉注意力与冻结的 tokenizer 嵌入矩阵对齐，从而把全局信息蒸馏进每个子智能体的提示里。这样所有子智能体既共享一份骨干和上下文、参数量不随网络规模增长，又能感知到彼此之外的全局态势，实现协调而非各自为政。可视化也显示学到的上下文嵌入与 Flow、Demand、Capacity 等 MCF 核心词汇高度相关，说明它确实学到了任务语义。
