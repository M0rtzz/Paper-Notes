...
CUDA-L1 把"教会一个 CUDA 能力很弱的 LLM 写出又对又快的内核"拆成三个递进阶段：先用监督微调（SFT）让模型具备写出可执行、正确 CUDA 代码的基础能力，再用自监督迭代学习筛掉失败样本、把正确率顶上去，最后进入核心的对比强化学习（Contrastive RL）阶段，以执行速度为奖励把代码逼向极致。前两阶段只关心"能不能跑对"，第三阶段才真正优化"跑得多快"。
...
这是全文的核心创新，针对的是标准 RL（REINFORCE、GRPO、PPO）的一个根本缺陷：奖励只回流到梯度里，LLM 在生成代码的当下并不知道"上一版为什么慢"，无法在推理层面利用性能差异。Contrastive RL 把若干历史代码变体连同它们的 speedup 分数一起写进 prompt，让模型先分析哪份实现更快、快在哪里，再综合出改进版本。每次生成代码得到的评估分数被两路复用：一路按 GRPO 做梯度更新（"基础模型增强"），另一路拼进下一轮的对比 prompt（"固定参数下的解优化"）。两个维度互相喂料、协同演化，性能反馈第一次真正进入了模型的思考过程而不仅仅是参数。

**4. 对比样本选择策略：保证 prompt 里的参照既有竞争力又有差异。**

对比分析要起效，喂进 prompt 的样本必须同时满足两个条件——有足够高的性能标杆可学，又有足够大的性能落差可比；如果挑出的代码性能彼此接近，模型就无从对比。CUDA-L1 从历史代码中选 $N=2$ 个作为对比样本：先按性能把代码分桶，再用温度缩放的 softmax 从不同桶里采样，使采到的代码偏向高分桶（竞争力）的同时又来自不同桶（多样性）。这样既不会全是高手导致看不出差距，也不会全是弱者学不到东西。

**5. Reward Hacking 防御：堵住 RL 钻评测漏洞"假装加速"的后门。**
