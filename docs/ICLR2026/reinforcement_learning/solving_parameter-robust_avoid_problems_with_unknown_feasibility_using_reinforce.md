...
### 整体框架
FGE 把"训练哪些场景"和"在这些场景上怎么安全"耦合成一件事来做：它一边维护一个对最大可行参数子集 $\Theta^* \subseteq \Theta$ 的估计（哪些参数下确实存在安全策略），一边在这个不断扩张的子集上做鲁棒策略学习。核心难点在于 $\Theta^*$ 先验未知，于是 FGE 用一个可行性分类器把"已确认能安全"的参数从"暂时学不会"的参数里分出来，再用鞍点优化在可行子集上对抗最坏情况，并用一套三段采样分布持续把边界往外推。

### 关键设计

**1. 可行性分类器：解决"观测到不安全"这个标签天生不可靠的问题。** 安全 RL 里有个不对称：某个参数 $\theta$ 下跑出一条安全轨迹，就能确定它可行；但跑出一条不安全轨迹，并不能断定它不可行——很可能只是当前策略还不够好。如果直接把"没成功"当成"不可行"，会把一大批本可救回的场景误判出局。FGE 因此不用单一在线样本训练分类器，而是构建一个混合分布 $p_{\text{mix}}(\mathfrak{f}, \theta) = \alpha \cdot p^*(\mathfrak{f}|\theta) p_{\mathcal{D}_\mathfrak{f}}(\theta) + (1-\alpha) \cdot p^\pi(\mathfrak{f}|\theta) \rho(\theta)$：第一项来自已确认安全的可靠正标签，第二项是当前策略下可能含假阴性的在线样本，再通过变分推断拟合 $q_\psi(\mathfrak{f}=1|\theta)$ 并阈值化得到分类结果。这样设计带来两个保证——不可行参数永远不会被错标为可行（零假阳性），而把可救场景误杀的假阴性率可以通过调节混合权重 $\alpha$ 和采样密度 $\rho$ 来控制。零假阳性尤其关键，它保证训练集里不会混进真正无解的场景去污染策略。

**2. 鞍点优化：把鲁棒安全控制变成一个收敛更稳的在线对抗。** 鲁棒避障本质是 maximin——要找一个策略，在最坏参数下也尽量安全。RARL 用一个对抗性策略去逼近这个最坏情况，但对抗性 RL 训练常常不稳定、容易震荡。FGE 改用在线学习的鞍点视角：策略侧解 $\pi_{t+1} = \arg\max_\pi \mathbb{E}_{\theta \sim \mathcal{D}_{\theta,t}}[J(\pi, \theta)]$，参数侧解 $\theta_{t+1} = \arg\min_{\theta} J(\pi_t, \theta)$ 且只在可行子集 $\theta \in \Theta^*$ 内取最坏情况。具体实现用 Follow-the-Regularized-Leader (FTRL) 配合 PPO 做策略更新，并维护一个 rehearsal buffer 存历史最坏参数。相比 RARL 近似采用的 GDA，FTRL 的正则化让两侧博弈在鞍点附近收敛得更平稳，避免了对抗策略来回拉扯导致的退化。

**3. 三段采样分布：让安全边界持续往外扩而不回退。** 光有分类器和鞍点还不够——要真正把可行集合做大，得主动去"碰"那些还没被证明安全的参数，同时别把已经学会的丢掉。FGE 因此把采样分布拆成三块（图 1）：基础分布按原始参数分布采样，探索分布刻意抬高那些尚未被观测为安全的参数的采样概率，排练分布则对先前已解决、但有退化风险的参数做近似最佳响应的复习采样。三者组合的目标是同时最大化安全率的增益、最小化安全率的损失——探索负责开疆，排练负责守土，基础保持整体覆盖，使可行集合稳步扩张而非在新旧之间反复横跳。
