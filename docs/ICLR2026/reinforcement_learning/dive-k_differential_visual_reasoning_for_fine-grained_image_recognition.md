...
DiVE-k 把开放式的细粒度识别重构成一个由模型自己出题的闭集选择问题：先用基础 LVLM 多次采样挖出最容易混淆的 top-k 类别拼成选择题，再用 GRPO 强化学习训练模型在这些 hard-negative 中做属性级的差异化推理。训练和推理都共享"采样选项 → MCQ 重选"这一两步管线，使得奖励信号从脆弱的字符串匹配简化为可靠的选项索引匹配。

### 关键设计

**1. top-k 自生成选项挖掘：让模型自己暴露混淆点。** 现有 RL 方法用固定或随机的负类构造任务，要么太简单要么与图像无关。DiVE-k 改为对每张训练图像 $I$ 和查询 $q$ 用基础模型 $\pi_\theta$ 采样 $K=20$ 次，统计所有预测类别的出现频率，取最频繁的前 $k=\min(5,|\mathcal{C}|)$ 个组成选项集 $\mathcal{O}_{top\text{-}k}$；若正确答案没被采到，就替换掉频率最低的那个选项保证答案在内，最后随机打乱编号为标准 A/B/C 形式。由于这些选项正是模型自身困惑分布的众数，它们天然就是视觉上高度相似的 hard-negative，迫使模型去比较细粒度区分属性而不是只看显著特征。

**2. 困难样本过滤：把训练算力集中在真正混淆的图像上。** 如果模型 $K$ 次采样只吐出唯一的正确类别，说明这张图对模型已无难度，继续训练只会强化已有的简单捷径。DiVE-k 直接剔除这类样本，只保留采样结果中存在歧义的图像进入训练集，让 GRPO 的梯度集中在差异化推理真正起作用的 hard 案例上。

**3. 简单可验证的 MCQ 奖励 + GRPO 优化：绕开字符串匹配的脆弱性。** 在构造好的 MCQ 数据集 $\mathcal{D}=\{(I,q,\mathcal{O}_{enum},\hat{a})\}$ 上，每个样本生成 $N$ 个 rollout，按组内标准化得到 advantage $A_i=\dfrac{r_i-\text{mean}\{r_1,\dots,r_N\}}{\text{std}\{r_1,\dots,r_N\}+\delta}$。奖励由两项构成：MCQ 奖励 $r_{mcq}$ 在模型选中正确选项时为 1.0、否则 0.0，格式奖励 $r_{format}$ 鼓励模型规范输出 `<think>` 与 `<answer>` 标签，二者按 $r=\lambda_f r_{format}+\lambda_m r_{mcq}$ 加权后送入标准 GRPO 损失。因为答案被压成单个选项索引，奖励判定不再受科学名/俗名等字符串不一致影响，也不会奖励对训练类别名的死记硬背，而是直接激励"在候选间比较关键属性"的推理过程。

**4. 训练-推理一致的两步管线。** 推理时复用同一套流程：先用训练后的模型对测试图采样 $K$ 次得到 top-k 选项，再把选项组成 MCQ 重新提示模型选择。与训练唯一的不同是推理阶段不会人为塞入 ground-truth，完全依赖模型自己的候选集，从而真实反映泛化到 novel 类的能力。
