...
TROLL要解决的是一个很具体的问题：PPO 训练里那个看似无关紧要的裁剪操作，其实是 TRPO 信任域约束的一个粗糙替身，而这个替身正悄悄拖累训练。TROLL 的做法是把裁剪整个拆掉，换成一个**离散可微分的信任域投影**——在每个 token 位置上对策略分布施加精确的 KL 散度约束，让更新后的分布既朝着提升优势的方向走、又不偏离参考策略太远。

整条流水线的其余部分几乎原封不动：前向得到每个 token 的 logits，照常算优势，唯一被替换的是「如何约束这一步更新」。因为只动训练阶段的更新方式、不改变推理时的采样行为，TROLL 可以零成本接到任何现成的 PPO-clip 管道上；又因为约束施加和优势估计是两个独立维度，它和 GAE、GRPO 等各种优势估计方法天然正交、可以叠加使用。

### 关键设计

**1. Token 级 KL 约束：把"限制概率比率"换成"限制分布距离"。**

PPO 的裁剪在概率比率空间里动手脚，截断 $\frac{\pi_\theta}{\pi_{old}}$ 的取值范围，但比率被卡住并不等于两个分布真的接近。TROLL 转而在**概率分布空间**上直接下约束：对每个 token 位置 $t$，要求更新后的分布 $\pi_\theta(\cdot | x_{<t})$ 与参考分布 $\pi_{ref}(\cdot | x_{<t})$ 之间的 KL 散度不超过阈值 $\delta$，

$$KL(\pi_\theta(\cdot | x_{<t}) \| \pi_{ref}(\cdot | x_{<t})) \leq \delta$$

这正是 TRPO 性能改进保证所要求的那种约束——信任域理论从一开始要的就是 KL 上界，而不是概率比率上界。裁剪只是当年为了工程简便对它做的近似；TROLL 让这个理论约束重新变得可直接操作，约束粒度也从单个比率回到了整个分布。

**2. 可微分信任域投影：让 KL 投影能反向传播。**

直接施加 KL 约束的难点在于：投影到约束集合通常是个不可微的操作，一旦不可微，梯度就传不回网络。TROLL 设计了一种**可微分的投影**来绕过这个障碍。前向传播先得到每个 token 的 logits $z_t$；接着把 $z_t$ 投影到满足 $KL(\text{softmax}(z_t) \| \pi_{ref}) \leq \delta$ 的最近分布上；最后用投影后的分布去最大化策略目标。关键在于这个投影本身保持可微，梯度能穿过投影一路传回参数，于是「精确 KL 约束」和「端到端梯度训练」第一次能同时满足——这也是 TRPO 当年没能在大模型上落地的工程瓶颈所在。

**3. 稀疏 Top-K 投影：只在少数高概率 token 上做投影。**

LLM 词表动辄数万到十几万，若在完整词表上对每个 token 都做一次 KL 投影，计算量会把信任域的好处吃光。TROLL 的效率关键是只选概率最高的 Top-K 个 token（$K \ll V$）做投影，其余低概率 token 的 logits 保持不变。这之所以成立，是因为 LLM 输出分布本就高度长尾、概率几乎全集中在少数 token 上，策略更新对长尾 token 的影响微乎其微——把投影限制在分布真正会变化的那一小撮 token 上，既保住了约束效果，又把开销压到可控范围，让信任域方案在大模型规模下变得实际可跑。
