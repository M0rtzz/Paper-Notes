...
本文把"结构化图像难题"拆成数据、模型、评估三块统一攻克：先用约 200 万个可执行绘图程序合成 130 万对代码对齐、带 CoT 推理标注的训练样本，再在 FLUX.1 Kontext 扩散骨干前接一个 Qwen2.5-VL 多模态编码器、用三阶段渐进训练把结构化领域知识和显式推理注入进去，最后用原子化 Q&A 协议的 StructBench/StructScore 做细粒度事实性评测。整条链路的核心是让"代码"既当训练监督的精确锚点，又当推理时的语义桥梁。

### 关键设计

**1. 代码对齐的数据合成与 CoT 标注：把"可执行性"变成可验证的监督信号。** 结构化图像最大的特点是能由代码精确渲染，本文据此收集约 200 万个 Python+LaTeX 绘图程序，覆盖数学、图表、谜题、科学图示、图结构、表格六大类。流程是先执行源代码渲染源图像，再让 GPT-5 在分析源图像显著视觉特征（salient features）后同时产出"图像级编辑指令"和"代码级编辑指令"——前者只引用可见元素、后者指定精确的程序级修改；改完代码重新渲染就得到与源图像严格对齐、且可逐像素验证的目标图像，从根本上避免了传统合成指令"近似对齐"的噪声。经渲染失败、无视觉差异、低信息量三道过滤后，每个 T2I 样本再配上密集属性描述（dense caption），每个编辑样本配上"输入分析→指令解释→目标预测"的三步推理链，使监督信号的语义密度远高于"add tree right"这类简短指令。

**2. VLM + 扩散模型的统一架构与轻量 connector：用高层语义补齐 VAE 缺失的理解力。** 结构化编辑往往要求高层语义推理（如把柱状图转成饼图需理解数量比例关系），而扩散模型的 VAE 只提供低层特征。本文保留 FLUX.1 Kontext 作为支持统一生成/编辑的扩散骨干，但用 Qwen2.5-VL-7B 替换原始 CLIP 编码器来编码多模态特征，二者之间仅用一个轻量 MLP connector 对齐。相比 MetaQuery 那类 transformer-based projector，MLP connector 训练开销更低、优化更稳定，让 VLM 的理解力能稳妥地传导进扩散过程。

**3. 三阶段渐进式训练与 mask-based 损失加权：分步注入对齐、领域知识与推理。** 训练分三步走，逐层抬高难度。Stage 1（统一对齐）冻结扩散骨干只训 MLP connector，并刻意移除 T5 特征、只喂 Qwen-VL 特征，防止 T5 成为捷径而架空 connector 的对齐；Stage 2（混合视觉学习）联合微调骨干与 connector 注入结构化领域知识，同时混入高质量自然图像保留通用能力，并针对结构化图像"大面积均匀背景、编辑区域小"的像素统计引入 mask-based 策略，自适应降低背景和未变化区域的损失权重；Stage 3（推理增强）把 CoT 标注作为 Qwen-VL 的长上下文输入注入显式推理，训练后模型还能在推理时接受外部推理器（GPT-5）提供的分析与规划，实现 inference-time compute scaling。

**4. StructBench 与 StructScore：用原子化 Q&A 把"事实性"评成可信分数。** 评测集含 1714 个样本、编辑侧 32031 个 Q&A 对、生成侧 37941 个 Q&A 对，覆盖 Math、Graph、Chart、Puzzle、Science、Table 六类。StructScore 采用基于 VLM 的多轮 Q&A 协议：先从真值图像生成细粒度的原子化问答对，再让模型对生成图像开放式作答，形成 [问题, 预测答案, 真值答案] 三元组逐一比对；编辑任务进一步把视觉一致性与指令跟随两个维度解耦加权（$0.1\times$ 一致性 $+\ 0.9\times$ 指令跟随），并通过原子化拆分加 Q&A refinement 把真值图像自身的回答准确率从约 80% 提升到 95% 以上，从而压低 VLM 评判的幻觉与噪声。
