...
本文不提出新模型，而是搭建一个统一的评测协议，把三种生成范式放到同一张桌子上比组合对齐能力：扩散 UNet（SDXL）、扩散 Transformer（PixArt-$\alpha$）、DiT（Flux-Dev / Flux-Schnell），以及代表 VAR 路线的 Infinity-2B / Infinity-8B。所有模型在 T2I-CompBench++ 和 GenEval 两个互补基准上跑同一批 prompt，再用各自的自动验证器打分，从而得到可横向比较的组合对齐画像。

### 关键设计

**1. 双基准互补：用两套不同的验证机制交叉确认结论。** 单一基准的打分逻辑可能带有系统性偏差，如果只靠它得出"VAR 更强"的结论很难让人信服。因此本文同时采用两个验证范式根本不同的基准：T2I-CompBench++ 由检测器驱动，先在生成图上检测物体再核对属性与关系，覆盖颜色绑定、纹理绑定、形状绑定、非空间关系、2D 空间关系、3D 空间关系、计数和复杂组合共 8 个维度；GenEval 则用规则化约束做判定，覆盖单物体、双物体、计数、颜色、位置、颜色属性和总体共 7 个维度。两者一个偏"看图找证据"、一个偏"按规则卡条件"，互为 sanity check——只有当两套机制给出一致排序时，结论才被认为可靠。

**2. 多种子平均的评测协议：抑制随机性带来的排名抖动。** 生成模型对随机种子敏感，单张图的好坏不能代表模型能力。T2I-CompBench++ 上每个 prompt 用 4 个独立随机种子各生成一张图，报告种子平均值并在附录给出标准差，用以判断模型间差距是否超出噪声范围；GenEval 同样遵循官方协议，每个 prompt 生成 4 个样本后报告聚合分数。这样得到的分数才是稳定、可复现的，避免把偶然的好运误读为架构优势。

**3. 跨架构、跨规模的模型谱系：把"架构差异"和"规模效应"拆开看。** 6 个模型的选取并非随意：参数量从 0.6B（PixArt-$\alpha$）一直铺到 12B（Flux），横跨三种架构范式，保证比较不被某一类模型主导。其中 Infinity-2B 与 Infinity-8B 构成 VAR 路线内部的对照，专门用来观察同架构下放大规模的收益；Flux-Dev 与 Flux-Schnell 则构成质量-速度的对照，用来检查蒸馏加速是否会牺牲组合正确性。借助这套谱系，"是 VAR 架构本身好，还是单纯参数多"这类混淆因素得以分离。
