...
T2I-CoReBench 把"评什么"和"怎么评"拆成三层：先用一套 12 维分类体系把组合能力和推理能力结构化拆解，再围绕每个维度构造高复杂度 prompt 并配套原子化 checklist，最后用 MLLM 把每条 checklist 问题判成二值结果汇总成分。整套基准含 1080 条 prompt、约 13500 个 checklist 问题，覆盖组合 4 维 + 推理 8 维。

### 关键设计

**1. 组合维度：用场景图把"画面里有什么"拆成四类。** 现有组合基准的通病是视觉元素太稀疏（通常 ≤5 个），测不出真实场景的密度压力。本文借场景图的实例-属性-关系三要素，把组合能力拆成四个维度并刻意拉高密度：MI（Multi-Instance）要求单图生成约 25 个实例（如"繁忙的现代厨房"），MA（Multi-Attribute）要求单主体绑定约 20 个属性，MR（Multi-Relation）要求场景中约 15 种关系正确连接，TR（Text Rendering）要求约 15 处文本的内容和布局精确渲染。平均下来每条 prompt 约 20 个视觉元素，比 DPG-Bench 等基准密度高出 4-5 倍，逼出模型在高密度场景下的真实组合上限。

**2. 推理维度：用哲学推理三分法覆盖"画面背后要想什么"。** 推理评测过去多是零散启发式分类、且只测简单的一对一因果。本文以演绎/归纳/溯因三分法为骨架，把推理能力系统拆成 8 个维度：演绎（前提→结论）下含 LR 逻辑、BR 行为、HR 假设、PR 过程四类，归纳（模式→规律）下含 GR 泛化、AR 类比两类，溯因（观察→解释）下含 CR 常识、RR 重建两类。更关键的是引入一对多（一个行为推出多个结果）和多对一（多个前提汇成一个结论）的复杂推理链，突破现有基准只测一对一推理的局限，使隐式线索的难度真正贴近现实。

**3. Prompt + Checklist 构建管线：让评测既难又可验证。** 为保证 prompt 的多样性和复杂度同时避免单模型偏差，构建时让三个 SOTA 大推理模型（Claude Sonnet 4、Gemini 2.5 Pro、OpenAI o3）协同生成候选 prompt，再经严格人工审核定稿。每条 prompt 配一份 checklist——一组互相独立的 yes/no 问题，把 prompt 里的显式描述和隐式线索逐点拆成可单独核验的原子条目。这种原子化设计是后续自动评估能落地的前提：每个问题只问一个点，天然适配 MLLM 的二值判断。

### 损失函数 / 训练策略

本文是评测基准，没有训练过程。评估阶段用 Gemini 2.5 Flash 作 MLLM 评估器，把每个 checklist 问题转成二值 VQA 任务（"0"=否 / "1"=是），靠 checklist 的原子性保证判断稳定，最后按维度聚合得分。
