...
本文不训练新模型，而是想把"提示写得多复杂"这件事拆开，看清它如何同时拨动合成数据的质量、多样性、一致性三个旋钮。研究分三步推进：先在一个可控的合成玩具实验里，剥离掉真实数据的噪声，单看提示复杂度与"泛化难度"的本质关系；再用一段概率推导解释这个玩具实验里看到的反直觉现象；最后把推导出的结论放到 CC12M、ImageNet-1k、DCI 等真实数据集和多个 T2I 模型上验证，确认它不是玩具实验的偶然。整条线索的落点是一个统一评估框架，让真实数据和各种提示策略下的合成数据能放在同一把尺子上比较。
...
本文不涉及任何新模型训练，全程对现成 T2I 模型（如 Stable Diffusion 等）做推理时评估，在 CC12M、ImageNet-1k、DCI 三个数据集上展开，覆盖从短标题到长描述的不同提示复杂度档位。
