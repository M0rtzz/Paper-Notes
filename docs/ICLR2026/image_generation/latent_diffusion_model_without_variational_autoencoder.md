...
### 整体框架
SVG 的核心是把扩散模型从 VAE 潜在空间搬到自监督特征空间：冻结的 DINOv3 编码器提供语义判别性强的主干特征，一个轻量残差编码器补回 DINO 丢失的细节，二者拼成 SVG 特征供扩散模型直接学习，再由 SVG Decoder 解码回图像。整套设计的出发点是用语义清晰的潜在空间换取更快的训练收敛和更少的采样步数。

### 关键设计

**1. SVG 自编码器：用残差补回 DINO 丢失的细节。** DINOv3 特征语义强但偏向高层判别，直接拿来重建会丢色彩和高频纹理，因此 SVG 在冻结的 DINOv3-ViT-S/16+ 主干（对 256×256 图像产生 $16 \times 16 \times 384$ 特征图）之外并联一个 ViT 残差编码器，专门捕获主干缺失的细粒度信息，再把两路特征沿通道拼接成完整的 SVG 特征。为避免残差分布与 DINO 主干失配而破坏后续扩散训练，残差特征 $F_R$ 通过批统计被对齐到主干特征 $F_D$ 的均值方差上：$\hat{F}_R = \frac{F_R - \mu(F_R)}{\sigma(F_R)} \cdot \sigma(F_D) + \mu(F_D)$，使拼接后的特征空间统计一致、便于解码器和扩散模型联合使用。

**2. 高维语义特征上的扩散：靠语义分散性稳住训练。** VAE 潜在空间只有 $16 \times 16 \times 4$ 维，而 SVG 直接在 $16 \times 16 \times 384$ 的高维特征上训练扩散，常规认知里这样高维很容易让扩散训练发散。SVG 之所以能稳，是因为 DINO 特征天然具备良好的语义分散性——不同类别在特征空间里彼此分开，速度场方向不再互相矛盾。训练沿用 SiT 的 flow matching 目标，并配合 QK-Norm 与 per-channel 归一化进一步稳定高维优化，使高维不再是负担反而成了语义优势。

**3. 语义分散性分析：解释为什么能少步采样。** 这一设计回答"语义清晰的潜在空间为何能加速"。通过 t-SNE 可视化与 toy example 可以看到：在语义被清晰分离的特征空间里，同类样本对应的速度方向高度一致、异类方向泾渭分明，于是扩散需要拟合的速度场更平滑、优化更容易，采样时也能用更少步数走到目标，这正是 SVG 仅 5 步就能出图的根本原因。

### 损失函数 / 训练策略
训练分两阶段且解耦：阶段一冻结 DINOv3，只训练残差编码器和 SVG 解码器，目标是重建损失加上前述分布对齐，先把一个高质量且统计一致的 SVG 特征空间建好；阶段二再在这个固定的特征空间上训练 SVG Diffusion，采用 SiT 设置并启用 QK-Norm 和 per-channel 归一化。这样先定特征、后学生成的顺序，避免了特征空间与扩散目标互相干扰。
