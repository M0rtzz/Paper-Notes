...
SpatialGenEval 想回答一个被现有 T2I 基准忽略的问题：模型不仅要画对"是什么"，还要画对物体"在哪里、怎么排、为什么交互"。它把整套评估建立在四条相互支撑的设计原则上——每条提示约 60 词、密集塞进 10 个互相关联的空间约束，让简单提示无法暴露的能力差异显现出来；每条提示配 10 道覆盖全部空间子领域的多选题做全维度打分；生成图像所用的提示绝不透露给评估器，杜绝答案泄露；每道题再加一个 "E: None" 拒答选项，避免模型被迫从错误选项里硬选。整条流水线串起来就是：构造信息密集提示 → 人工审核 → 自动生成并验证 QA → 模型生成图像 → MLLM 多轮投票打分 → 按子领域汇总正确率。

### 关键设计

**1. 空间智能分层体系：把笼统的"空间能力"拆成可逐项测量的 10 个子领域。**

针对现有基准"评估粒度粗糙、缺少系统空间智能分层"的痛点，SpatialGenEval 自下而上把空间智能分成 4 个层级、10 个子领域：从最基础的物体是否齐全、属性是否绑定对，到感知层的位置/方向/布局，再到推理层的比较/邻近/遮挡，最后是交互层的运动与因果。这种递进结构让每个模型的失败能精确定位到某一层，而不是只得到一个笼统总分——后面实验也正是靠它发现"推理层是普遍瓶颈"。
...
**2. 基准构建流程：信息密集提示 + 人在回路审核 + 无泄露 QA。**

提示由 Gemini 2.5 Pro 生成，输入 25 个真实世界场景和 10 个空间子领域的定义，让它把全部 10 个空间约束无缝融进一条约 60 词的长提示里。但纯机器生成的提示往往别扭，于是引入人在回路审核：把割裂的短句合并（如 "There is a robot. It is rusty." 改成 "A rusty robot"）、修掉逻辑自相矛盾的约束（如无法成立的循环布局）、把生僻词换成通俗表述（如 vermilion → bright red），保证提示既密集又自然。每条提示再自动派生 10 道多选题，并做两道把关——人工逐题检查问题文本里不含显式答案（防止评估器"看题就会"），程序化给每题统一补上 "E: None" 拒答项。

**3. SpatialT2I 数据集：把评估的副产品变成能反过来改进模型的训练数据。**

这是本文超越"只做评估"的一步：额外构造 1,100 条提示，交给 14 个顶尖开源模型各自生成图像，再用 Qwen2.5-VL-72B 给图像质量打分、Gemini 2.5 Pro 重写提示使图文一致，最终筛出 15,400 个高质量文本-图像对。用这批数据微调 SDXL、UniWorld-V1、OmniGen2，就把"评估暴露出的弱项"直接喂回训练，形成评估→数据→模型的闭环，而不是发现问题后束手无策。

### 评估协议

为保证可复现，主评估器选用开源的 Qwen2.5-VL-72B。打分不靠单次判断，而是 5 轮投票：只有当 MLLM 在 5 轮里至少 4 轮选中正确答案才算这道题答对，以此压低评估器自身的随机波动。最终每个模型在每个空间子领域上报一个正确率，作为该子领域的能力分数。
