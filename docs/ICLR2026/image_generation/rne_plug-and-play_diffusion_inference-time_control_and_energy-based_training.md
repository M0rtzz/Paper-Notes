...
RNE 不在单步去噪核上做文章，而是把整条生成轨迹看成一个对象，比较前向（加噪）路径分布 $\mathbb{P}$ 和后向（去噪）路径分布 $\mathbb{Q}$。借助 Radon-Nikodym 定理，$\mathbb{Q}$ 关于 $\mathbb{P}$ 的密度比 $\frac{d\mathbb{Q}}{d\mathbb{P}}$ 可以完全用已知的转移核写出来，于是边际密度估计、推理时控制、能量训练这三件看似无关的事，都归结为对同一个密度比的估计与操纵。

### 关键设计

**1. 路径分布密度比：把全局密度问题拆成逐步可算的局部比率。**

扩散模型的边际密度难算，根源在于它需要对所有可能的前向路径积分，计算上不可行。RNE 的破题点是不去碰边际密度本身，而是去看两条完整路径分布的比值。对前向过程 $q(x_0, x_1, \dots, x_T)$ 和后向过程 $p(x_T, x_{T-1}, \dots, x_0)$，Radon-Nikodym 导数把它们直接联系起来：

$$\frac{d\mathbb{Q}}{d\mathbb{P}}(x_{0:T}) = \frac{p(x_T) \prod_{t=1}^{T} p_\theta(x_{t-1}|x_t)}{q(x_0) \prod_{t=1}^{T} q(x_t|x_{t-1})}.$$

关键之处在于这个比率天然分解成逐步局部比率的连乘，每一步只涉及已知的转移核，不需要任何额外训练的模型。这样一个原本需要全局积分的难题，就被转化成沿轨迹逐步累乘的局部估计问题。

**2. 扩散密度估计：用密度比的部分乘积反推边际密度。**

有了路径密度比，任意中间时刻 $t$ 的状态 $x_t$ 的边际密度，就可以通过路径密度比的部分乘积来估计，整个过程直接复用扩散模型自身的转移核，无需再训练一个密度模型。实际计算时在路径空间中做蒙特卡洛采样来逼近这个比率，于是密度估计变成一件用现成模型即可完成的事。

**3. 推理时控制：把退火与模型组合统一成对密度比的即插即用调节。**

RNE 作为即插即用模块挂在冻结的预训练扩散模型上，不改任何权重就能引导生成。它把两类控制手段纳入同一视角：退火（annealing）用估计出的边际密度充当能量函数的温度调度，实现更精确的条件采样；模型组合（model composition）则把多个扩散模型的密度比相乘，从而组合多个条件。由于控制力来自采样路径的多少，RNE 天然支持推理时缩放（inference-time scaling）——给更多计算预算就能换来更好的控制效果。

**4. 能量扩散训练：用密度比正则化绕开配分函数。**

传统能量扩散模型训练卡在配分函数（partition function）的估计上，计算困难。RNE 换了个思路：用密度比作为正则项去约束能量函数的训练，让学到的能量与真实密度比保持一致，从而完全避免显式估计配分函数，把训练流程大幅简化。

**5. 模态无关性：理论建立在路径分布上，连续与离散扩散通吃。**

因为整套推导只依赖路径分布这一抽象概念，而不依赖状态空间是否连续，RNE 不限于连续扩散，也同样适用于离散扩散模型（如文本生成中的离散去噪）。这让它从一个针对图像扩散的技巧，升级为一个跨模态通用的工具。

### 损失函数 / 训练策略

RNE 在推理时控制场景下不需要任何额外训练：冻结预训练模型，仅通过密度比调节采样轨迹即可。在能量扩散训练场景下，它充当辅助正则项——在标准去噪损失之上叠加基于 RNE 的正则化，约束学到的能量函数与真实密度比一致，以此替代昂贵的配分函数估计。
