...
本文不提出新方法，而是建立一个基于**相对更新**（relative updates）的统一分析框架，把 μP 与权重衰减放进同一个透镜下审视：跨宽度训练要稳定，本质上需要让每一层表征的相对变化率不随宽度漂移。作者将这一变化率分解为对齐比与相对权重更新的乘积，

$$\frac{\|\Delta \mathbf{Y}\|}{\|\mathbf{Y}\|} = \frac{\alpha_{\Delta W}}{\alpha_W} \cdot \frac{\|\Delta \mathbf{W}\|}{\|\mathbf{W}\|}$$

其中 $\alpha_{\Delta W}$ 是更新对齐度、$\alpha_W$ 是权重对齐度。μP 的整套缩放正是建立在"对齐比 $\alpha_{\Delta W}/\alpha_W = \Theta(\sqrt{C})$ 随宽度 $C$ 增长"这一假设之上，因而用学习率缩放 $\eta \propto 1/m$ 去抵消它。后续三个设计就是顺着这条公式，逐一拷问这个假设是否站得住、真正在干活的又是谁。
