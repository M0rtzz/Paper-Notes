...
### 整体框架
StageRoute 把整个时间轴 $T$ 切成 $K$ 个 stage，在两个时间尺度上分层决策：每个 stage 起点 $\tau_k$ 做一次"战略层"部署，从含新到达模型的候选池里挑出 $\leq M_{\max}$ 个模型并冻结整个 stage 不变；stage 内每个查询到达时再做"战术层"路由，在已部署集上实时求一个 LP 得到分派分布。部署决策决定了路由的动作空间，因此二者必须联合优化而非各自为政。

### 关键设计

**1. 部署优化 DeployOPT：用乐观估计圈定一个 stage 的可用模型集。** 部署是高风险决策——一旦在 stage 起点激活某个还没摸清的新模型，就可能整段时间都被它拖累，而且无法中途撤换。为此 DeployOPT 在每个更新点 $\tau_k$ 求解一个混合整数规划：二值变量 $z_m$ 决定模型 $m$ 是否部署、连续变量 $d_m$ 表示其权重，目标是在预算和并发上限 $\sum_m z_m \leq M_{\max}$ 约束下，用性能上界 $\mu_m^U$（UCB）和成本下界 $c_m^L$（LCB）最大化"乐观"性能。这里的乐观估计鼓励算法去试探尚未充分采样的新模型，对抗模型发现的探索难题。关键的是，MIP 的最优解 $d^*$ 只被用来确定部署哪些模型，**不直接当作路由概率**——这一步把"部署什么"和"如何路由"解耦，使得 stage 内的查询级路由仍能基于实时统计量快速适应。

**2. 路由优化 RouteOPT：stage 内每个查询实时解 LP 做分派。** 部署集冻结后，stage 内的探索-利用要靠路由层完成。每个查询到达时，RouteOPT 在已部署模型上求解一个线性规划，以 UCB 估计的期望奖励为目标，约束为预算消耗和每个模型的吞吐量上限 $p_t(m) \leq \alpha_m$，得到一个路由分布后采样分派。因为它持续用最新统计量刷新 UCB/LCB，所以即便部署集一整个 stage 不动，路由仍能逐查询收紧对各模型的估计，吞吐量约束也顺带把高负载模型节流、缓解延迟尖峰。

**3. 置信界设计：用方差自适应的置信半径平衡探索与利用。** UCB/LCB 的松紧直接决定算法敢不敢上新模型。性能上界取 $\mu_m^U = \text{proj}_{[0,1]}(\bar{\mu}_m + 2f_{rad})$、成本下界取 $c_m^L = \text{proj}_{[c_1,c_2]}(\bar{c}_m - 2f_{rad})$，置信半径 $f_{rad}(v,n) = \sqrt{\gamma v/n} + \gamma/n$ 随采样次数 $n$ 增大而收缩、随经验方差 $v$ 增大而放宽。投影到合法区间保证估计值不越界，方差项让采样充分的模型置信区间快速收窄，从而把探索预算集中到真正不确定的新模型上。

**4. 遗憾界与匹配下界：把"路由学习"和"模型发现"两类代价拆开。** 算法好不好，最终由遗憾界刻画。Theorem 1 给出上界 $\text{Regret} \leq \mathcal{O}(\sqrt{M_{\max}KT\log(NT/\delta)} + NT/(M_{\max}K))$：第一项是已部署集内路由的统计学习代价，第二项是结构性的模型发现瓶颈，即在 $N$ 个候选里逐 stage 发掘好新模型的难度。两项此消彼长——stage 数 $K$ 越多、并发上限 $M_{\max}$ 越大，发现越快但单 stage 学习成本越高。取 $K = \Theta(T^{1/3})$、$M_{\max} = \Omega(N^{2/3})$ 时两项平衡，整体收敛到 $\tilde{\mathcal{O}}(N^{1/3}T^{2/3})$。Theorem 2 进一步证明 $\Omega(T^{2/3})$ 的匹配下界，说明这个速率是问题固有难度而非算法缺陷，StageRoute 已达近最优。
