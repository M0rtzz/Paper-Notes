...
FastGRPO 在 GRPO 训练循环里嵌入两个互补的加速组件：生成阶段用并发感知投机解码动态调节 draft 树的形状，让验证算力始终卡在硬件最优工作点；参数更新阶段用在线 Draft 学习持续吸收目标模型的新分布。前者解决"并发度时刻在变"的问题，后者解决"目标模型一直在漂"的问题，两者叠加才能让加速比在整个训练过程里都保持在 2x 以上。
