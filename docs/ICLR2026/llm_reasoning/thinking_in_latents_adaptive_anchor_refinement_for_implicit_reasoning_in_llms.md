...
AdaAnchor 在冻结的自回归 Transformer $f_\theta$ 之上挂载 $m$ 个可学习锚向量，把它们投影拼接到 token 嵌入前端，反复送入 backbone 前向传播、用输出隐状态回写锚向量，让"思考"全程发生在潜空间。每轮迭代后监控锚状态的余弦距离，一旦连续若干步几乎不变就提前停止，最终只基于收敛后的锚和原始输入解码答案（answer-only），不吐出任何中间推理 token。
