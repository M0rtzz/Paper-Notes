...
description: >-
  [ICLR 2026][LLM推理][提示学习] 本文提出 String Seed of Thought（SSoT），一种简洁的提示方法，通过指示 LLM 先生成随机字符串再从中提取随机性来选择答案，显著提升了概率指令跟随（PIF）的分布忠实度和开放式任务（DAG）的响应多样性…
tags:
  - "ICLR 2026"
  - "LLM推理"
  - "提示学习"
...
SSoT 不改训练、不接外部工具，只往原有 prompt 里塞一条两阶段指令：先让 LLM 吐出一个与任务无关的随机字符串当作熵源，再让它对这个字符串做确定性运算（求和取模、哈希）把熵"翻译"成最终答案。PIF 任务的指令是"生成随机字符串，操作它以从目标分布中采样"，DAG 任务则换成"操作它以生成一个多样化的响应"，其余推理过程交给模型自己在 CoT 里完成。

### 关键设计

**1. 两阶段提示：把"造随机"和"选答案"拆成两步。** LLM 直接被问"抛硬币选正反"时，会被选项位置、标签训练频率等先验拽偏，输出严重失衡。SSoT 的做法是先让模型写一段随机字符串——这个动作和具体任务无关，不触发选择偏差，却能借助长 CoT 积累足够的熵；第二步再让模型对字符串做一个固定的算术变换，从熵里"读出"答案。随机性的来源（字符串）和答案的归属（变换结果）被彻底解耦，模型不再有机会按训练偏好挑选项。

**2. 偏差隔离 + 天然可并行：每次生成都是独立的。** 因为每条响应只依赖自己当场生成的字符串，不需要回看历史选择记录，SSoT 天然支持并行采样；这与"把已选分布塞回 prompt"的顺序采样方法形成对比，后者破坏了生成间独立性、且 prompt 会随序列膨胀。同一套 prompt 框架同时覆盖 PIF 和 DAG，无需为每个任务单独调参，LLM 自主决定用什么变换。

**3. 理论收敛保证：TV 距离随字符串长度指数衰减。** 论文给出两条定理把直觉钉死。定理 4.1（2-universal 哈希界）假设生成字符串中每个字符的条件概率有界（$\delta \leq P(x_i|\{x_j\}_{j<i}) \leq 1-(A-1)\delta$），则采样分布与目标分布的总变差距离满足

$$d_{TV} \leq \frac{\sqrt{M}}{2\delta''} 2^{-\frac{n}{2}\log_2 \frac{1}{(1-(A-1)\delta)^2+(A-1)\delta^2}} + \sqrt{\frac{\ln((2^M-2)/\delta')}{K\phi(\pi_{P_X})}}$$

第一项随字符串长度 $n$ 指数衰减、第二项是有限样本误差；定理 4.2 进一步证明即使用更朴素的求和取模（字符 ASCII 码求和后对 $M$ 取模）策略，只要各字符边际分布不严重偏离均匀，TV 距离同样指数收敛。这两条定理解释了实验里"字符串越长、分布越忠实"的现象，也说明哪怕模型用的不是理想哈希，长度本身就能把偏差压下去。

**4. LLM 自主策略选择：复杂度越高、变换越精巧。** 拆 CoT 发现模型并非死守一种变换，而是按任务难度自己挑：均匀分布任务用简单的 Sum-Mod；有偏分布任务自动升级到多项式滚动哈希 $\sum_i B^i\,\text{ord}(c_i)$ 再取模，以抵消字符边际偏差；DAG 里创造性类别走"模板 + 局部采样"、其余类别走"列表 + 全局采样"。这种自适应让一条统一指令在跨度极大的任务上都不掉链子，也是 SSoT 不需要任务专属设计的根本原因。
