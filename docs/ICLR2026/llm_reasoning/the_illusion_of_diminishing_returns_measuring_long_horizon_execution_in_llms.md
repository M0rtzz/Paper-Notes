...
这是一篇纯评估研究，不训练任何模型，核心是一套受控的"长期执行"测量协议。作者把规划、知识检索从任务中剥离出去——直接把完整的计划和所需知识喂给模型，让它只负责"照着计划一步步做"，然后逐步加长任务（步数 × 轮数），观察逐步准确率和整体完成率怎么随长度衰减。

### 关键设计

**1. 执行能力隔离：把"会不会做"和"能不能做到底"分开**

常规推理基准把规划、知识、执行三件事混在一起测，模型在长任务上栽了跟头，没人说得清是哪一环出了问题。本文的做法是显式给出任务的完整执行计划和全部背景知识，让模型不需要自己规划、也不需要回忆事实，剩下唯一的变量就是"严格按计划逐步执行"的能力。在这种设置下，模型在长任务上的失败就能干净地归因于执行错误的累积，而不是推理或知识的缺失——这也是本文反驳"长任务失败 = 推理是幻觉"的实验基础。

**2. 自我条件化效应检测：让模型看见自己的错误，看它会不会越错越多**

作者发现一个反直觉现象：人类做重复任务往往越做越熟，但 LLM 在上下文里看到自己先前的错误后，反而更容易接着犯错。为了把这个效应从"任务本身变难"中剥离出来，作者在上下文中人为注入一段带有固定错误率的历史，再测量紧接着这一步的准确率。结果是因果性的——历史错误率从 0% 升到 5% 时后续准确率就明显下滑，升到 20% 时则出现"错误雪崩"式的急剧下降。关键是这个效应不靠扩大模型规模来缓解，只有 thinking 模型不受其影响，说明它是一种需要靠推理过程主动清理错误历史才能压住的失效模式。

**3. Horizon Length 度量：用一个公式说清"收益递减"为什么是假象**

要量化单步能力和任务长度之间的关系，作者定义任务视野长度 $H_s(p) = \lceil \frac{\ln(s)}{\ln(p)} \rceil$，其中 $p$ 是单步准确率、$s$ 是目标整体成功率、$H$ 是在该成功率下还能撑住的最大步数。因为 $H(p) \propto 1/\ln(p)$ 是一条双曲线，越靠近 $p=1$，曲线越陡：单步准确率从 95% 提到 99%，在 100 步任务上的成功率会从 0.6% 跳到 36.6%。换句话说，短任务基准上看起来"边际收益在递减"的那点提升，一旦放到长任务里就被指数级放大——所谓 diminishing returns，只是用错了尺子才产生的幻觉。
