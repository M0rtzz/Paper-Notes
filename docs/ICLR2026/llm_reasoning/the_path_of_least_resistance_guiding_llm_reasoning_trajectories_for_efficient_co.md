...
description: >-
  [ICLR 2026][LLM推理][self-consistency] 提出 PoLR（Path of Least Resistance），首个利用推理前缀一致性的推理时方法，通过聚类短前缀并仅扩展主导聚类来实现 Self-Consistency 的高效替代…
tags:
  - "ICLR 2026"
  - "LLM推理"
  - "self-consistency"
...
PoLR 在标准 Self-Consistency 管线里插入一个"早分线"步骤：先给问题采样一批很短的推理前缀，按内容把它们聚成几类，只把人数最多的那一类前缀继续展开成完整轨迹，再对这批完整轨迹做多数投票。这样大部分采样在前缀阶段就被筛掉，省下了把每条轨迹都跑到底的 token 开销。

### 关键设计

**1. 前缀采样与主导聚类扩展：把"选哪条轨迹"提前到推理早期。** 标准 SC 必须把 $N$ 条轨迹全部展开才能投票，浪费在了大量最终会被淘汰的轨迹上。PoLR 的做法是先只生成 $N$ 个短前缀 $p_i = \text{Prefix}(\mathcal{M}(x, t_i), L_p)$，实现上就是把采样的 `max_new_tokens` 限到 $L_p$。对每个前缀用 TF-IDF 词袋编码成稀疏向量，再用余弦相似度做层次聚类得到 $\mathcal{C} = \{C_1,\dots,C_m\}$，取人数最多的主导聚类 $C^* = \arg\max_{C_j}|C_j|$，只把其中 $K$ 个前缀续写成完整轨迹 $r_k = \mathcal{M}(x\,|\,p_k)$，最后投票 $\hat{a} = \arg\max_y \sum_{k=1}^K \mathbf{1}[a_k = y]$。这里刻意选 TF-IDF 而非神经编码器——它轻量、模型无关、跑在 CPU 上只要几毫秒，神经编码器带来的聚类开销远大于 TF-IDF 而准确率收益甚微；层次聚类则适合 $N$ 很小（11–51）的场景，无需预设聚类数且分组可解释；前缀长度经验上取 $L_p = 256$ 时在准确率和 token 效率之间最平衡。

**2. Token 效率刻画：把省了多少 token 写成可计算的指标。** 为了量化收益，PoLR 定义 token 效率为相对 SC 节省的 token 比例：

$$\eta = 1 - \frac{T_{\text{PoLR}}}{T_{\text{SC}}} = 1 - \frac{N \cdot \ell_p + K \cdot (\ell_f - \ell_p)}{N \cdot \ell_f}$$

其中 $\ell_p$ 是平均前缀长度、$\ell_f$ 是完整推理长度。分子第一项是 $N$ 个前缀的固定成本，第二项是只对主导聚类里 $K$ 个前缀续写到底的增量成本——因为 $K \ll N$，这一项把绝大部分轨迹的尾部开销直接砍掉，这正是节省的来源。

**3. 互信息对齐与结构偏斜：解释为什么既不掉精度又能省。** PoLR 的"安全"和"省钱"由两个独立性质分别保证。安全性看正确性对齐：令 $Y\in\{0,1\}$ 为最终轨迹是否正确、$Z$ 为前缀的聚类分配，只要 $I(Z;Y)>0$（聚类至少弱预测正确性、$H(Y|Z)$ 较小），扩展主导聚类就不会系统性丢掉正确答案。节省量则看结构偏斜，定义偏斜率 $\kappa = |C^*|/N$，命题给出 token 效率下界 $\eta \geq 1 - \frac{K}{M}\cdot\kappa^{-1}$，即聚类越偏斜（主导聚类占比越高）省得越多。两者解耦的含义是：**互信息保证不掉精度，偏斜决定能省多少**。实测中归一化互信息 NMI 保持很低（$\leq 0.18$）却仍把效率推到 50–58% 饱和，正说明真正的红利来自前缀聚类的强结构偏斜，而非聚类与正确性的强相关。
