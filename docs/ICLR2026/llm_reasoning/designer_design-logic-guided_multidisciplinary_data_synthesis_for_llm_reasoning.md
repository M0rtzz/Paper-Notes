...
### 整体框架
这篇论文要解决的核心难题是：怎么从原始书籍、网页这类「非题目」文本，大规模合成出大学/研究生级别、推理深度够、难度可控的多学科考题。它的关键判断是——直接让 LLM「看着文本出题」会退化成事实回忆，真正缺的是「出题方法论」。于是整条流水线分三步走：先做**数据处理**，把题库、书籍、网页三类语料做多维标注与过滤；再做 **Design Logic 提取**，从 13.2 万道精选真题里逆向工程出 12.5 万条结构化的出题逻辑；最后做**问题合成**，用两阶段 retrieve-and-generate 把每段源文本匹配到合适的出题逻辑，再照着这套逻辑生成题目和答案。换句话说，它合成的不只是题，更是「怎么把一段知识变成一道好题」的可复用模板。
