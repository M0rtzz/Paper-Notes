...
**1. 问题诊断：GRPO 加长度惩罚为什么会惩罚正确推理。**

GRPO 用 group-relative 优势函数 $A(o_i|q) = \frac{r(o_i) - \text{mean}(\{r_j\})}{\text{std}(\{r_j\})}$，把一组采样里正确和错误的回答放在一起做归一化。只看对错时这没问题，但一旦奖励里掺进长度惩罚，一个正确但偏长的回答 $r$ 就可能掉到组平均之下，优势变负——模型于是被告知"这次正确推理是负样本，要惩罚"。论文给的例子很直观：6 个回答里 3 个正确，加上长度惩罚后第 3 个正确回答的优势从 +1 翻成 -0.17。这个翻转不是某个实现的 bug，而是 RLOO-LP、ALP、HAPO 等所有基于 GRPO / 相对优势的方法共有的结构性缺陷。

**2. 解耦正负归一化：让长度惩罚只削弱、不翻转正确信号。**

针对上面那个"负优势"病根，DRPO 把正确回答的学习信号锁在正确回答组内部归一化，不再和错误样本相互拉扯。具体做法是先求一个最优正数据分布

$$P_q^* = \arg\max_P \mathbb{E}[r_l(o)] - \lambda D_{KL}(P, \pi_{old}^+)$$

这是标准的 KL 正则化目标，有闭式解 $P_q^*(o) \propto \pi_{old}^+(o|q) \exp(r_l(o)/\lambda)$。由此每个正样本拿到一个权重 $\omega(o|q) = \frac{\exp(r_l(o)/\lambda)}{\mathbb{E}_{o\sim\pi^+}\exp(r_l(o)/\lambda)}$，它只在正样本集合内归一化。关键在于 $\omega$ 恒为正：短回答 $r_l$ 大、权重大，长回答权重小，但再长也只是趋近 0，绝不会变负。于是"鼓励简洁"变成了在正确回答之间重新分配权重，而不是把长的正确回答一脚踢成负样本。超参 $\lambda$ 控制长度与准确率的取舍。

**3. 判别式目标函数：正负两项彻底分开写。**

DRPO 不走 GRPO 的混合归一化，而是把目标拆成互不干扰的两项。正样本项 $\mathbb{E}_{o\sim\pi^+} \omega(o|q) s_\theta(o,q)$ 用上面那套权重做加权似然，专门抬高短而正确的回答的分数；负样本项 $-\tau \log \mathbb{E}_{o'\sim\pi^-} \exp(s_\theta(o',q)/\tau)$ 用 log-sum-exp 聚合，自动把注意力压到最难的负样本上。外面再套一个信赖域约束 $D_{KL}(\pi_{old} || \pi_\theta) \leq \delta$ 保证训练稳定。因为正负彻底解耦，长度惩罚只在正样本项里起作用，不可能再污染对错误样本的判别。当 $\lambda \to +\infty$ 时权重退化为均匀，整个目标就还原成不带长度惩罚的 DisCO。
