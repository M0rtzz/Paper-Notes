...
### 整体框架
SimpleToM 想把"模型到底会不会用 ToM"这件事拆开来测。它由 1147 个极简故事组成，每个故事只有两句话，却嵌套了一个信息不对称：某个关键事实是读者知道、而故事里的角色不知道的。每个故事配三个层层递进的问题——先问角色的心理状态（显式 ToM），再问角色会怎么做（应用 ToM），最后问角色实际做法是否合理（更深一层的应用 ToM）。三个问题答案逐级要求模型"不仅知道别人不知道什么，还得拿这个知识去预测和评判行为"，从而把"显式推断"和"实际应用"两种能力分离开来度量。整个数据集通过 LLM 生成草稿 + 人工严格过滤构建。

### 关键设计

**1. 三层级问题设计：把一个故事拆成"推断→预测→判断"三道递进难度的题**

以一罐发霉薯片的故事为例，第一层 Mental State（MS）直接问心理状态——"Mary 知道薯片发霉吗？"，只需是/否，模型只要推断出角色处于不知情状态即可。第二层 Behavior Prediction 问行为——"Mary 会付款还是当场举报？"，模型得先在心里推出"Mary 不知情"，再把这个状态映射到合理行为上（不知情就会照常付款）。第三层 Judgment 问合理性——"Mary 付了款，这合理吗？"，要求两层隐式推理：先推出 Mary 应该会做什么，再拿实际行为去对照评判。三道题的隐式推理深度逐级加码——MS 只要推断状态，Behavior 要把状态翻译成动作，Judgment 还要在预测之上再加一层对错评估——正是这种递进让显式与应用 ToM 的差距能被定量观察到。

**2. 隐式信息不对称：故事里绝不出现 "sees"、"thinks" 这类心理化触发词**

很多 ToM 基准会在题面里直接写出"角色看到/认为……"，模型只要顺着这些词面回答就行，根本不需要真正推理。SimpleToM 刻意回避这种线索：故事第一句抛出关键事实（"薯片发霉了"），第二句只描述角色的客观行为（"Mary 拿起薯片走向收银台"），角色的不知情完全是隐含的——模型必须靠常识补上"人看不穿不透明的薯片罐"这一步，才能推断 Mary 不知情。这样设计逼出真正的常识推理，堵死了靠触发词作弊的捷径。

**3. 10 种日常信息不对称场景：覆盖物理遮挡、知识壁垒、欺骗等不同的"不知情"成因**

数据集横跨超市食品、医疗信息、虚假标签、服务业幕后、容器内容物、不道德行为、个人物品容器、二手市场、隐藏身体特征、锁定设备共 10 类场景。这些场景对应的信息不对称机制各不相同：有的是物理上看不见（容器内容物），有的是专业知识壁垒（医疗信息），有的是刻意欺骗（虚假标签）。覆盖多种成因是为了让评测不偏向某一类推理，确保测出的差距是 ToM 能力的普遍现象而非单一场景的偶然。

### 数据集构建
- **Step 1**：每个场景手写 1 个种子故事。
- **Step 2**：用 GPT-4 基于种子生成多样化故事（替换实体、变换情境）。
- **Step 3**：严格人工过滤——由通过资格测试的标注者逐条审核故事质量与三个问题的答案正确性。
