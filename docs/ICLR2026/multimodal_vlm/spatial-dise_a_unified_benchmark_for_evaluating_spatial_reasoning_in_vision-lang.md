...
**2. Blender 自动合成数据流水线：把稀缺的动态空间数据变成可大规模生成、答案可验证的合成数据**

动态空间推理数据极度稀缺、难以从真实世界大规模收集，这是补齐 I-D 的最大瓶颈。流水线用五步把它工程化：(1) 用 question_id 哈希出可复现的随机种子，保证每个实例可重建；(2) 生成核心 3D 物体（如不规则形状、带纹理的立方体）；(3) 从最佳视角渲染问题图和正确答案图；(4) 系统性生成多层级干扰项（几何变体、纹理/方向错误、错误视角、组件替换）；(5) 在受控虚拟环境里统一渲染。因为整个场景由 Blender 参数驱动，每个实例的正确答案都能直接用场景参数验证，避免了人工标注的噪声。这套框架还可扩展——社区只要实现对应的物体生成和干扰项策略，就能往里加新任务类型。

**3. 分层干扰项生成策略：用 near-miss 干扰项逼模型做精确空间推理而非模式匹配**

简单干扰项（如完全不同的形状）只能筛掉最差的模型，无法区分能力高低。这里按任务类型定制干扰项：几何变体（增减组件）、模式/方向错误（纹理错位）、错误视角（正交投影方向错误）、组件替换（用几何相似但不正确的部件顶替正确部件）。每种策略都对准模型容易犯的某一类特定错误——比如 3D 旋转的干扰项通过微调旋转角度生成，3D 折叠的干扰项通过交换面的纹理生成。这种 near-miss 设计让答对题必须依赖精确的空间推理，从而拉高每道题的诊断性。

**4. 三阶段质量控制：用唯一性、清晰度、去冗余三道闸把不合格实例直接踢出去**

合成与真实数据汇总后要过三关：(1) 答案唯一性检查——每题必须有且只有一个正确答案；(2) 准确性与清晰度——图像无渲染瑕疵、问题表述清楚、所有选项都符合任务标准；(3) 冗余消除——去掉逻辑或视觉上重复的实例。任一关不合格的实例直接从最终数据集中移除，保证 Bench 的 559 对每一道都干净可用。

**5. 人类基线建立：用矩阵采样 + IRT 交叉验证给出心理测量学可靠的人类上界**

为了量化模型离人类还有多远，作者招了 54 名参与者（年龄 15-55 岁），共收集 1679 个有效回答。采用矩阵采样设计，让每题平均被 3 个独立参与者作答；最终报告所有回答的平均准确率，并用 Item Response Theory (IRT) 做交叉验证，确保这个人类基线在心理测量学上可靠，而不是少数人偶然的发挥。
