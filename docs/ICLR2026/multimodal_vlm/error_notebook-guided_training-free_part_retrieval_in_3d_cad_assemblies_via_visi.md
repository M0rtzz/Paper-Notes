...
### 整体框架
整个系统是一条无需任何权重更新的两阶段VLM管线：先让一个VLM把每个零件的图像翻译成一句区分性文字描述，再让第二个VLM拿着装配图、这组描述和自然语言规格做CoT推理，挑出目标零件。两阶段之外的核心是一本Error Notebook——它把模型过去答错又被纠正的推理轨迹存起来，推理时按规格相似度检索相关条目当few-shot示例喂回去，让模型"借鉴自己的错题"再答一遍。
