...
如果题目都集中在短轨迹、短时间，模型靠静态背景蒙就能过，测不出真正的动态推理能力。论文按轨迹长度 $track_d$ 和持续时间 $track_t$ 的物理阈值，把任务划成 Long / Medium / Short 三级，并刻意让三级均衡分布。室内外采用不同阈值——室外是驾驶场景，时间短但距离长，需要单独标定——这样无论室内行人还是室外车辆，都能覆盖到长短不一的时空尺度，让评测优先考验动态线索而非静态背景。

**3. 基准构建流水线：把原始多摄像头视频自动转成标准化问答**

原始数据是裸视频，要变成既含地图又含多视角观测、且适配各任务时间/地理/格式要求的标准题目，需要一条自动化流水线。流程分三段：**数据预处理**先做视频分割，再用单应性矩阵标定每个摄像头并把轨迹投影到统一地图上，算出速度、方向等运动参数，经数据清洗验证后由 LLM 生成运动描述；**任务构建**阶段对轨迹采样，把地图、视频和题目模板集成形成问答，并专门生成干扰项——从不同建筑区域采样、算法合成虚假摄像头、随机化 ID，让选项更具迷惑性；最后是**两阶段人工质检**，第一阶段确保问题多样并剔除轨迹误差大的题，第二阶段由专家验证答案并覆盖合理难度。
