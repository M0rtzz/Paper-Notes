...
### 整体框架
SpatiaLab 想回答一个问题：把 VLM 丢进真实世界的杂乱照片里，它还能做空间推理吗？为此它把空间认知系统拆成 6 大类——Relative Positioning（相对定位）、Depth & Occlusion（深度与遮挡）、Orientation（方向）、Size & Scale（大小与尺度）、Spatial Navigation（空间导航）、3D Geometry（3D几何），每大类再细分 5 个子类，共 30 种细粒度任务类型。整条流水线是：先从多源采集真实照片建图像库，再经三阶段人工标注+质控为每张图配上空间 QA，并把每题编码成 MCQ（4选1）和开放式生成两种格式；最终每子类≥25 题、每大类≥200 题，汇成 1400 个经验证的 QA 对。模型在这套基准上跑完，论文再用一组改进策略去探测「能不能补上短板」。
