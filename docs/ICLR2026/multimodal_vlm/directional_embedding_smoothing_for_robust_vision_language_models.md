...
RESTA 把 randomized smoothing 的思路搬到嵌入空间：推理时对输入嵌入注入随机噪声、生成多个加噪副本，再用多数投票合成最终输出，从而把越狱攻击所依赖的脆弱激活路径"抹平"。本文的核心动作是利用 VLM 中视觉 token 与文本 token 共享同一输入嵌入空间这一特性，把原本面向 LLM 的 RESTA 无缝迁移到多模态场景，并系统比较两种噪声的施加方式。

### 关键设计

**1. 共享嵌入空间上的统一加噪：让防御层对视觉和文本一视同仁。** LLaVA、Gemma 这类 VLM 的视觉内容先经 vision backbone 提取 patch-level 特征，再投影回 LLM 的输入嵌入维度，与文本 token 拼成一条统一的嵌入序列 $\bm{e} = (e_1, \ldots, e_n) \in \mathbb{R}^{d \times *}$。正因为图像和文本最终都落在同一个 $d$ 维空间里，RESTA 不需要为视觉模态另设接口——只要在这条统一序列上施加噪声扰动即可，多模态越狱里图文协同的攻击成分也一并被覆盖。加噪时还做了选择性处理：只扰动用户内容对应的 token 嵌入，系统提示和对话格式模板的 token 保持不变，避免破坏模型对自身角色与输出格式的理解。

**2. 采样—投票解码：用多数表决稀释被攻击样本的影响。** 给定输入嵌入序列，先用加噪算子 $H_\sigma$ 生成 $k$ 个独立副本 $\tilde{\bm{e}}^i = H_\sigma(\bm{e})$；解码每一步对这 $k$ 个副本分别做 greedy decoding 得到 $k$ 个候选 token，再用多数投票（majority vote）选出该步的最终 token。直觉是：越狱往往依赖激活某条很窄的脆弱路径，单个噪声副本就有不小概率偏离这条路径而回到安全行为，逐 token 投票则把多数副本的"安全倾向"放大成稳定输出，同时对正常请求几乎不损语义。实验取 $k=10$。

**3. 方向性噪声 vs. 各向同性噪声：沿向量方向加噪以保住语义。** 各向同性（isotropic / normal）噪声对嵌入每个维度独立加高斯扰动 $\mathcal{N}(0, \sigma^2)$，会同时改变向量的方向和模长；而 hard directional 噪声只沿原向量方向扰动，$e + \frac{ze}{\|e\|_2}$，其中 $z \sim \mathcal{N}(0, \sigma^2 d)$，归一化因子 $\sqrt{d}$ 用来让两种噪声的有效功率对齐以便公平比较。设计动机在于嵌入向量的语义主要编码在方向而非幅度上：方向性噪声本质只改变模长、几乎不偏转方向，于是既能扰动掉越狱攻击赖以生效的精细激活、又能尽量保留原始语义内容——这正是它在安全-实用性权衡上明显胜过各向同性噪声的根源，且这一方向性效应在 VLM 上比 Hase et al. (2024) 在 LLM 上观察到的更为显著。

### 一个完整示例

以一条"无害图像 + 越狱文本"的多模态攻击输入为例：图像经 vision backbone 投影、与文本拼成统一嵌入序列后，RESTA 只对用户内容 token 用 hard directional 噪声生成 $k=10$ 个加噪副本。解码第一步，10 个副本里多数因方向扰动偏离了攻击诱导的"开始照做"路径、转而输出拒绝性 token，少数仍被攻击带偏；多数投票选出拒绝方向的 token。后续每一步重复采样—投票，逐步累积出一段连贯的安全拒绝回复。而若换成正常的 ScienceQA 选择题，由于方向几乎不变、语义被保留，10 个副本大多给出同一正确答案，投票后准确率仅小幅下降。
