...
**1. 硬干扰物筛选（Hard Distractor Pressure）：让一张图里同时蹲着好几个"长得像"的同类物，逼模型做精细区分**

RefCOCO(+/g) 的根子问题是 70%+ 的图像里同类干扰物只有 0-1 个，模型靠类别识别就能定位，根本不必看描述。Ref-Adv 反着来：先从 COCO 和 OpenImages v7 的 panoptic instance 标注里筛出**含 ≥3 个同类实例**的图像，再让 GPT-4o 把这些候选实例切成两组——Group A 是目标加上最像它的"硬干扰物"，Group B 是其余干扰物，同时要求 LLM 说清两组之间的区分属性、以及 Group A 内部两个实例的细微差别。这一步把任务从"这是不是一只猫"的分类题，硬升级成"这几只几乎一样的猫里哪只是目标"的精细区分题，模型再也没法靠类别蒙混过关。

**2. 两阶段最小充分表达式生成（Minimally Sufficient Expression Generation）：先抽区分属性、再从最小子集组装自然句，从源头掐掉冗余描述符**

冗余描述符正是"定位捷径"的温床——描述符越多，模型越可能只匹配其中一两个就蒙对。Ref-Adv 没让 LLM 一步到位写句子，而是拆成两阶段。阶段一（Similarity Judgement）让 GPT-4o 输出两份属性列表：一份区分 Group A 与 Group B，一份区分 Group A 内部的两个实例，由此得到一批候选描述符。阶段二（Expression Generation）从这些描述符的**最小充分子集**出发组装自然表达式，并支持两种策略：要么用目标自身的正向描述符，要么把硬干扰物的描述符取**否定形式**（"不是…的那个"），刻意把否定推理引进来。论文特意点破：放弃单步直接生成，是因为 GPT-4o 单步写时倾向 overspecified——堆一大串冗余描述，又把捷径放了回去。两阶段多绕的这一步换来的是质量——最终表达式平均 11.5 词、平均 4.01 个干扰物、21.25% 含否定，远高于 RefCOCO 的 3.6 词 / 3.99 干扰物 / 0.99% 否定，而**每个描述符都缺一不可**。

**3. 三人一致性人工验证（Three-Annotator Verification Protocol）：三位标注者独立把关，过滤掉 LLM 的幻觉与歧义**

LLM 辅助标注绕不开幻觉，所以最后一道关卡交回给人。三位标注者各自独立做两项判断：一是表达式是否正确且无歧义——他们先在没有标注框的原图上自行定位，再对照 GT 反思、最终确认；二是图里是否真的存在硬干扰物。只有**三人全部一致**的样本才留下来。这道关卡卡得极严，LLM 生成表达式的通过率仅 18.7%，正是这个看似浪费的低通过率，换来了基准的公信力。
