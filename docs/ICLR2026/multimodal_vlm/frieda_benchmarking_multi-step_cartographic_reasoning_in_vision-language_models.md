...
FRIEDA 是一个面向制图推理的开放式问答基准：每道题给定一份真实文档中的一幅或多幅地图，要求模型先读懂图例、比例尺、指北针等制图符号，再跨地图整合证据完成多步空间推理。整个基准由"四维任务设计 → 专家把关的数据构建 → 答案类型自适应的评测协议"三部分组成，最终沉淀为 500 道经过博士级标注共识过滤的高质量题目。

### 关键设计

**1. 四维任务设计：把人类读地图的全套技能拆成可评测维度。** 现有 map VQA 多只覆盖空间关系的某个子集，FRIEDA 则系统对齐 GIS 文献中的三大类空间关系并组合成四个正交维度。空间关系维度包含拓扑关系（border 共享边界、equal 几何重合、intersect 交叉、within 包含）、度量关系（distance，需借助比例尺折算实际距离）和方向关系（orientation，需借助指北针判方位）共六种关系；地图元素解读维度要求模型理解 map text、legend、map scale、compass 四类符号的语义；跨地图推理维度要求对齐多幅地图中共享的符号、标签与比例尺以整合多源证据；上下文（contextual）维度则进一步要求模型先从同一文档的多幅地图中检索出相关地图再作答。四个维度叠加，使每道题都逼近真实的"翻文档找图—读符号—跨图推理"工作流。

**2. 专家把关的数据构建：用多级共识压住合成题的简化偏差。** 为避免模型靠常识或检索绕过看图，构建流程层层设卡。地图采集自政府报告、环评文件、地质调查等六大主题领域，覆盖 32 个国家、样式高度多样；问题由 GPT-4/GPT-o3 生成候选，并强制满足"不看图或仅用搜索引擎无法作答"；随后由两名分别有 7 年和 2 年经验的 GIS 专家人工核验答案、修正歧义；最后 11 名博士研究者（其中 8 名具地图专业背景）经四周标注，仅保留 ≥2/3 标注者认可金标准答案的题目。最终数据集统计如下，其中多地图与依赖图例的比例都很高，说明题目确实在考"跨图整合"和"符号解读"而非简单识图。
...
**3. 答案类型自适应的评测协议：让开放式答案也能被公正打分。** 由于答案是开放式而非多选，逐字符匹配会误杀语义正确的回答，FRIEDA 按答案类型分三套评测。文本类答案用 Mistral Small 3.1 充当 LLM-as-Judge，做语义匹配而非精确字符串比较；距离类答案先做单位感知解析再算 MAPE，误差在 20% 以内即判对，兼顾了比例尺折算的固有不确定性；方向类答案允许相邻方位容差，例如金标准为 North 时 NW 与 NE 也被接受，避免对指北针读数过于苛刻。三套规则各自贴合一类答案的合理误差边界，使评分既宽容又不失区分度。
