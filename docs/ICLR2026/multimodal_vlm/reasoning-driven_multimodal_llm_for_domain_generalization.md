...
RD-MLDG 把域泛化从"特征级不变性"抬升到"推理过程级不变性"：先用 GPT-4o 给 DomainBed 全量样本造出带推理链的 DomainBed-Reasoning 数据集，再针对推理监督暴露出的两个症结，分两阶段微调一个 InternVL3-8B。第一阶段 MTCT 让分类路径与推理路径联合训练，把推理链拽过优化鸿沟；第二阶段 SARR 用模型自生成的推理链迭代替换 GPT-4o 风格的监督，逐步消除推理模式不匹配。

### 关键设计

**1. DomainBed-Reasoning 数据集：让监督信号从标签升级为可解释的推理过程。** 标准 DG 基准只有图像和类别标签，无法承载"推理过程不变"这一新假设，因此作者在 PACS/VLCS/OfficeHome/TerraInc 四个数据集上用 GPT-4o 为每个样本生成五阶段推理链 SUMMARY → CAPTION → REASONING → REFLECTION → CONCLUSION。三个细节保证了链的质量：生成时**不提供 ground-truth 标签**，迫使推理完全建立在视觉证据上而非答案泄漏；相比 LLaVA-CoT 的四阶段额外插入 REFLECTION 自检阶段，减少无效生成；最后做**拒绝采样**，每样本采多条候选、只保留包含全部组件且结论自洽的链。这样得到的推理链中，类别相关的推理步骤在不同域之间天然一致（如不同域的"打印机"视觉差异巨大，但推理判据高度重合），构成了过程级不变性的载体。

**2. 两大挑战诊断：先看清推理监督为何"看着好用、训了反而更差"。** 这一步不是工程模块，而是驱动后续设计的核心分析。挑战一是**优化鸿沟**：在 TerraInc 上，zero-shot 加推理链相比 no-thinking 把 ground-truth token 概率拉高 +43.28%p，但一旦 SFT，推理监督反而比直接标签监督低 0.93%p——因为推理链 SFT 只把 1.88%p 的 token 从低概率搬到高概率（直接标签 SFT 是 +43.38%p），收敛更慢，分类 token 的高置信比例只有 86.33%（直接标签为 92.23%）。挑战二是**推理模式不匹配**：GPT-4o 的链充满背景、视角等描述性上下文，拿来 SFT 后 token 概率只涨 +1.88%p；而用模型自生成的链做 SFT 能涨 +29.74%p，但内容简单、信息量低。两类链优化的 top-15 最大熵减 token 几乎不重叠——GPT-4o 偏描述细节，自生成偏类别词。两个挑战分别指向"怎么训得动"和"该用谁的推理风格"，正好对应下面两个阶段。

**3. MTCT（多任务交叉训练）：用分类路径当锚，把推理路径拖过优化鸿沟。** 针对挑战一，MTCT 对同一张训练图像同时构造两条 prompt：no-thinking 的**分类路径**直接预测标签，提供稳定、易收敛的训练信号；**推理路径**输入完整推理链，提供丰富语义。两路联合优化为 $\mathcal{L}_{\text{MTCT}}$，其中推理链损失按 token 长度归一化，避免长链在梯度里喧宾夺主。分类路径相当于一个"锚"，引导推理路径的优化方向，防止模型在冗长推理序列上过拟合却学不到分类所需的高置信 token——这正是单用推理链时收敛慢、置信低的根因。

**4. SARR（自对齐推理正则化）：把 GPT-4o 风格逐步换成模型自己的风格。** 针对挑战二，SARR 在 MTCT 训完之后让模型自行生成推理链，只保留 `<CONCLUSION>` 与 ground-truth 标签一致的链作为新监督，再重新跑一遍 MTCT，如此迭代 $N$ 轮（实验取 $N=3$）。每一轮都把一部分 GPT-4o 风格的、难以优化的描述性推理替换成模型自身风格、更易拟合的推理，从而在语义丰富性（来自 GPT-4o）与可优化性（来自自生成）之间找到平衡点，既不丢推理过程的不变性，又让分类 token 真正被训进高置信区。

实现上以 InternVL3-8B 为底座，LoRA rank 8 同时插入视觉编码器与语言解码器，每阶段训 3 epoch，batch size 128，学习率 5e-4，AdamW 优化器，4× A100 80GB。
