...
### 整体框架
AttWarp 是一层套在 MLLM 外面的测试时图像预处理：先让模型对"图像+查询"跑一次前向，从解码器的跨模态注意力里读出"它想看哪儿"，再据此把图像重新采样——高注意力区域被拉大、低注意力区域被压扁，然后把这张变形图喂回同一个模型得到最终答案。整个过程不碰权重、不改架构，靠的是把注意力分布转成一个保持规则网格的空间映射。
