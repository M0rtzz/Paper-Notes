...
MMTok把"挑哪些视觉token"重新表述成一个最大覆盖问题：给定文本token和视觉token，目标是选出一个大小为 $k$ 的视觉token子集，让它既覆盖文本查询的语义、又覆盖整幅图像的视觉信息。它分别构造文本-视觉相似度矩阵 $M^{tv}$ 和视觉-视觉相似度矩阵 $M^{vv}$，把两者的覆盖度加权求和成一个子模目标，再用贪心算法逐个挑token；整个流程不需要训练。

### 关键设计

**1. 覆盖函数：用"被选子集覆盖了多少目标"代替逐个token打分**

传统方法给每个视觉token单独算一个重要性分数再取topk，容易选出一堆彼此冗余、信息重复的token。MMTok改成覆盖视角，对相似度矩阵 $M$ 和已选子集 $\mathcal{S}$ 定义覆盖函数 $f(\mathcal{S}; M) = \frac{1}{m} \sum_{i=1}^{m} \max M_{i,\mathcal{S}}$，即对每个目标token取它与子集中最相似那个被选token的相似度，再对所有目标求平均。直觉是只要某个目标已经被子集里某个token"代表"了，再加相似token的边际收益就很低，从而天然鼓励多样性。论文证明该函数是单调子模函数（Proposition 1），因此贪心算法能拿到 $(1-1/e) \approx 63.2\%$ 的最优近似保证。

**2. 文本-视觉覆盖（T-V）：让选出的token对得上文本在问什么**

这一项负责语义相关性。它用投影到语言空间后、已与文本对齐的视觉token构造相似度 $M_{i,j}^{tv} = \mathbf{t}_i^\top \mathbf{v}_j$，覆盖目标是所有文本token——也就是要让被选视觉子集尽量"答得上"每个文本token的关切。它的短板是当文本本身很模糊（如"请描述图像"）时缺乏有效引导，这也是后面引入V-V项和Agent增强的动机。

**3. 视觉-视觉覆盖（V-V）：补上文本顾不到的全局图像信息**

为了避免只盯着文本相关区域而丢掉整幅图的结构，V-V项要求被选子集能代表所有视觉token。关键细节是它用投影**前**的原始视觉特征算相似度 $M_{i,j}^{vv} = \mathbf{v}_i^{\prime\top} \mathbf{v}_j'$，而非T-V里投影后的特征——投影后特征偏向跨模态对齐，投影前特征才保留纯粹的视觉相似性。这样T-V管"和文本相关"、V-V管"覆盖整张图"，两者信息互补。

**4. 多模态融合：softmax校准后把两项相加，仍是子模目标**

由于T-V和V-V两个矩阵的数值尺度不一致，直接相加会被某一项主导，所以先按行做带温度的softmax校准，例如 $M_{i,j}^{tv'} = \frac{\exp(M_{i,j}^{tv}/\tau_t)}{\sum_j \exp(M_{i,j}^{tv}/\tau_t)}$，V-V同理用 $\tau_v$。校准后的联合目标为 $f(\mathcal{S}; M^{tv'}, M^{vv'}) = f(\mathcal{S}; M^{tv'}) + \alpha \cdot f(\mathcal{S}; M^{vv'})$，其中 $\alpha$ 平衡两项权重。因为子模函数的非负线性组合仍是子模函数（Corollary 1），贪心的近似保证依然成立。默认取 $\tau_t=0.02$、$\tau_v=0.2$、$\alpha=0.5$，论文报告这几个超参对结果都不敏感。

**5. 贪心求解：每步挑边际增益最大的token，开销和最简单的baseline相当**

求解就是标准的子模贪心（Algorithm 1/2）：从空集出发，每一步从剩余候选里选能让联合覆盖目标增长最多的那个视觉token加入，直到凑满 $k$ 个。整个过程只涉及加法、乘法和取max这类矩阵运算，没有反向传播也没有迭代优化，所以实测运行时间与VisionZip等纯排序方法几乎一致。

**6. 可选的Agent增强文本：文本太空泛时先让小模型补一句回答**

针对"请描述图像"这类信息量不足的查询，MMTok可选地先用一个轻量VLM（SmolVLM2-256M）对图像生成一段初步回答，把回答的token拼接到原始文本token之后，再去算T-V覆盖。这相当于用小模型预测的语义关切来补全引导信号，因此对开放式描述类任务有帮助；但对"选A还是B"这类多选QA，Agent回答（如"A"）本身不含视觉指向，几乎没有增益。
