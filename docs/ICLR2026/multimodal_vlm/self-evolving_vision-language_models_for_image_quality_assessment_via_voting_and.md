...
EvoQuality 把"无标注提升 IQA"拆成一个自监督闭环：先让 VLM 对图像两两比较、用多数投票投出一份共识排序当伪标签，再把这份排序翻译成保真度奖励、用 GRPO 微调模型，微调后的更强 VLM 重新投票产出更干净的伪标签，如此一轮轮自我进化。整个过程不依赖任何人工质量评分，靠的是"单次判断不准、群体投票可信"这一假设。
