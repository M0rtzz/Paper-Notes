...
VisJudge-Bench 先构建一个 3,090 样本的专家标注基准，再在其上训练领域专用评估模型 VisJudge。基准构建分三步走通：从网络海量图像中筛出高质量可视化、为每张图自适应生成评分问题与五级标准、再由众包加专家两轮标注得到可信分数；之后用 GRPO 把通用 MLLM 微调成专门的可视化"评委"。

### 关键设计

**1. "信达雅"六维评估框架：把模糊的"好不好看"拆成可打分的维度。** 可视化质量横跨数据是否准确、信息是否易读、画面是否美观三个层面，但这些概念太抽象、难以让模型给出一致评分。论文借中国翻译理论的"信、达、雅"把质量落到六个可测维度：**保真度（Fidelity）**检测轴设置不当、刻度失真、截断基线等误导性编码，看视觉是否忠实反映数据；**表达力（Expressiveness）**进一步分为*语义可读性*（用户能否清晰解码视觉元素）与*洞察发现*（能否揭示深层模式、趋势或异常）；**美学（Aesthetics）**则细分为*设计风格*（创新与独特）、*视觉构图*（布局与元素定位的平衡秩序）、*色彩和谐*（配色在美感与信息传达间的取舍）。这六个维度既覆盖了可视化"传达数据"的核心目的，又把每个维度收窄到能写进评分标准的具体观测点，为后续逐维打分奠定了基础。

**2. 三阶段数据构建：从 30 万图像漏斗到 3,090 高质量样本。** 网络爬取的图像噪声大、重复多、并非都是合格可视化，直接用会污染基准。论文设计逐级收紧的漏斗：先用自动脚本加感知哈希去重做**初始过滤**，把 30 万+ 图像压到 80,210 候选；再用 GPT-4o 自动分类加人工验证剔除非可视化与低质图，得到 13,220 有效样本；最后做**分层采样**，按复杂度均衡选出 3,090 样本——单图 1,041、多图 1,024、仪表盘 1,025，覆盖 32 种子类型。分层采样保证了简单单图到复杂仪表盘的全谱系覆盖，避免基准被某一类图表主导。

**3. 自适应问题生成：每张图配一套量身定制的评分标准。** 不同图表类型该考察的点不同，统一问题模板会问错重点。论文先用 GPT-4o 从图中提取元数据（图表类型、视觉元素），再据此从预定义模板实例化出定制化评分问题和五级评分标准。例如在保真度维度，标准从"1 分 = 存在截断轴或误导性刻度"细化到"5 分 = 条形长度严格与显示值成比例"，把每一档分数都锚定到可观测的视觉特征上，使众包标注者和模型都有明确依据，减少主观漂移。

**4. 两轮标注与质量控制：用方差驱动的冲突解决换取可信标签。** 单纯众包评分噪声高，难以作为评估金标准。论文采用三阶段流程：第一阶段 603 名众包工人独立打分，每样本 3 人 × 6 维度；第二阶段基于评分方差识别冲突样本，做异常值移除与恶意评分检测；第三阶段由 3 位可视化分析专家独立审查，对复杂案例讨论达成共识。这种"众包广度 + 专家精修"的组合，既保证了标注规模，又在分歧处引入专家裁决，得到了相对可信的人类评分基线。

**5. GRPO 微调出领域评委：用复合奖励把通用 MLLM 拉进可视化评估。** 通用 MLLM 缺乏可视化评估的专业先验（GPT-5 的 MAE 仍高达 0.553），需要针对性训练。论文按 70%/10%/20% 划分训练/验证/测试（2,163/279/648 样本），在 Qwen2.5-VL（3B/7B）、InternVL3-8B、Llava-v1.6-mistral-7B 等基座上用 GRPO 强化学习微调。奖励为复合形式：准确度奖励最小化预测分与真值的误差，格式奖励确保模型输出结构化可解析。训练用 LoRA 做参数高效微调，5 个 epoch，学习率 $1 \times 10^{-5}$。复合奖励让模型在"打分准"和"输出规范"两个目标间同时优化，最终小模型经微调即显著超越 GPT-5。
