...
**1. NC0 诊断指标：把难追踪的 NC 收敛压缩成一个标量动力学。** 原始的 NC1–NC3 指标涉及特征均值、ETF 对齐、自对偶等结构，既难直接做理论分析，也难看清优化器的作用路径。作者定义 NC0 为最后一层权重矩阵行和的平方范数 $\alpha_t = \frac{1}{K}\|W_t^\top \mathbf{1}\|_2^2$，并证明（Proposition 2.1）NC0 收敛到零是 NC2 与 NC3 成立的**必要条件**。这把一个高维几何问题降到了单个标量的收敛性上：只要 NC0 发散，就能确定性地断言 NC 不可能发生，无需追踪完整的几何结构。这也是后续所有定理都只针对 NC0 立论的原因。

**2. 权重衰减的耦合与解耦：Adam 与 AdamW 差异的真正来源。** 两类优化器的唯一实质区别在于权重衰减项作用的位置。耦合权重衰减（SGD/Adam）把衰减项放进梯度内部，动量更新写作 $V_{t+1} = \beta V_t + \nabla L_{CE} + \lambda W_t$；解耦权重衰减（SGDW/AdamW）则让衰减项直接乘到参数上，$W_{t+1} = (1-\eta\lambda)W_t - \eta V_{t+1}$。对 vanilla SGD 这两种写法完全等价，所以历史上常被混为一谈；但一旦优化器带有自适应/符号化的逐坐标缩放（如 Adam），耦合项 $\lambda W_t$ 会先经过缩放再更新，而解耦项不会，两者**不再等价**。正是这个被长期忽视的细节，决定了 NC0 是收敛还是发散。

**3. 理论定理：四种组合给出截然不同的 NC0 命运。** 作者在无约束特征模型（UFM）下分别求解四种优化器–衰减组合的 NC0 动力学。关键观察是交叉熵损失梯度的行和恒为零，$\nabla L_{CE}(W_t)^\top \mathbf{1}_K = 0$，因此 NC0 的演化只由权重衰减和动量驱动，与具体损失景观无关——这正是能写出闭式结论的原因。结论上，SGD 配解耦或耦合衰减（Theorem 3.1/3.2）都使 NC0 以指数速率收敛到零，速率分别正比于 $\lambda$、以及 $\lambda$ 与 $\beta$；SignGD 配耦合衰减（Theorem 3.4，Adam 特例）在学习率衰减下也能收敛到零；唯独 SignGD 配解耦衰减（Theorem 3.3，AdamW 特例）使 NC0 **单调递增**到正常数 $\frac{(K-2)^2}{\lambda^2}$，永远到不了零。这就从理论上解释了为何 AdamW 无法产生 NC，而仅一字之差的 Adam 可以。
