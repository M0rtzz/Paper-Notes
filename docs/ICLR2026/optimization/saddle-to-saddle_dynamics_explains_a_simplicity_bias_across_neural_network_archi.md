...
本文要回答的问题是：为什么从全连接、卷积到注意力，这些结构迥异的网络在梯度下降下都表现出"先简单后复杂"的学习节奏？作者的答案是把训练轨迹看成一场在损失景观里的"鞍点接力"。整个框架由三件事拼起来：先分析损失景观中的临界点（不动点），刻画梯度下降会被困在哪些低维子空间里（不变流形），最后证明训练过程其实就是在这些子空间之间一级一级往上跳（saddle-to-saddle 动力学）。轨迹在某个低维流形上慢慢逼近一个鞍点、停留出一段平台期，再沿不稳定方向逃逸到下一个更复杂的流形——这一跳一停的节奏，正是 simplicity bias 的来源。

### 关键设计

**1. 统一的 Simplicity 定义：把"简单"锚定到有效隐藏单元数。**

要跨架构谈"简单"，首先得让不同架构的"简单"可比。作者给出的统一口径是：**简单 = 能用更少的隐藏单元表达**。对全连接网络，这个单元数就是隐藏神经元数量；对卷积网络，是有效卷积核数量；对注意力网络，是有效注意力头数量。这三者看似不同，但在参数空间里都对应同一种结构——低秩的权重矩阵（或等价的稀疏结构）。正是这个统一口径，让"先学简单解"在三种架构下变成同一句话：先用少量有效单元、对应低秩权重，再逐步增加。

**2. 不变流形的识别：用嵌套的秩-$k$ 子空间刻画轨迹的"台阶"。**

有了简单性的定义，接下来要找到梯度下降实际被困住的地方。作者定义秩-$k$ 不变流形 $\mathcal{M}_k$ 为参数空间中权重矩阵秩恰好为 $k$ 的集合，并证明在适当条件下梯度下降的轨迹会贴着这些流形附近演化。这些流形天然形成嵌套结构 $\mathcal{M}_0 \subset \mathcal{M}_1 \subset \mathcal{M}_2 \subset \cdots$，每一层比前一层复杂一档。这套"秩"的语言在三种架构里各有具体含义：线性网络里 $\mathcal{M}_k$ 是秩-$k$ 的解空间，ReLU 网络里是有 $k$ 个活跃神经元的空间，CNN 里则是有 $k$ 个活跃卷积核的空间。不变流形之所以关键，是因为它把"轨迹在哪个复杂度档位"这件事，化成了一个干净的数学对象。

**3. Saddle-to-Saddle 动力学的形式化：一停一跳的循环产生渐进学习。**

这是框架的核心机制。作者证明梯度下降是按一个固定的循环在这些流形之间逐级攀升的：轨迹先在当前流形 $\mathcal{M}_k$ 附近演化，逼近该流形上的一个鞍点；在鞍点附近停留较长时间，损失几乎不下降，这就是训练曲线上看到的平台期；接着沿鞍点的不稳定方向（最大特征值对应的方向）逃逸，跳进下一个更复杂的流形 $\mathcal{M}_{k+1}$；然后在 $\mathcal{M}_{k+1}$ 上重复同样的"演化—停留—逃逸"过程。正是这种阶梯式的演化，让网络的复杂度一档一档地涨上去，自然解释了从简单到复杂的渐进学习——不需要任何外加的正则化来推动。

**4. 数据诱导 vs. 初始化诱导：把 simplicity bias 拆成两个独立来源。**
...
**5. 平台期的预测：从"为什么"升级到"什么时候、多久"。**

前面四点解释了 simplicity bias 为什么发生，这一点则让框架具备定量预测力。作者给出两个可算的结论：平台期的**数量**等于网络能表达的有效复杂度级别数；平台期的**持续时间**则取决于数据的特征值间距（间距越大，平台越短）以及初始化的条件数。这意味着只要拿到数据的协方差谱和初始化方案，就能事先定量地预测整条学习曲线会呈现几个台阶、每个台阶停多久——把对 simplicity bias 的理解从"描述现象"推进到"预测曲线形状"。
