...
**1. 深度 1 的退化结论：单层时 SAM 不改变 GD 的偏差。** 这一点回答"SAM 的特殊性从哪一层开始出现"。定理 3.1 证明：对几乎所有线性可分数据集、任意扰动半径 $\rho$ 与任意初始化，$\ell_\infty$-SAM 流的方向都收敛到 $\ell_2$ 最大间隔方向，与梯度下降一致。也就是说，在线性模型（$L=1$）上 SAM 与 GD 的隐式偏差无法区分，深度才是后续所有反直觉现象的真正诱因，这为"深度诱导"的命题确立了对照基线。

**2. 深度 $L\ge 2$ 的 $\ell_\infty$-SAM：扰动半径成为坐标级门控阈值。** 这一点解释了为什么多一层就能让 SAM 选出次要特征。在单样本数据集上，定理 3.2 给出每个坐标 $\beta_j(t)$ 的演化完全由初始值 $\alpha_j$ 相对 $\rho$ 的大小决定：当 $\alpha_j<\rho$ 时 $\beta_j(t)\to 0$（偶数 $L$）或 $\to\rho^L$（奇数 $L$）被压制，当 $\alpha_j=\rho$ 时停在常数 $\beta_j(t)=\rho^L$，当 $\alpha_j>\rho$ 时发散并在 $L=2$ 时指数增长。由于坐标彼此解耦，极限方向由唯一的胜出坐标 $j^*=\arg\max_{j:\alpha_j>\rho}\mu_j(\alpha_j-\rho)^{L-2}$ 决定（推论 3.5）。这意味着只要调初始化，$\ell_\infty$-SAM 可以收敛到**任意一个标准基向量方向**，包括特征权重 $\mu_j$ 很小的次要方向——与始终锁定主要特征的 GD 形成鲜明反差，$\rho$ 在此扮演了逐坐标的开关阈值。

**3. 深度 2 的 $\ell_2$-SAM：极限相同但有限时间出现"序列特征放大"。** 这一点是全文最核心的反直觉发现。定理 4.2 表明 $\ell_2$-SAM 流的极限方向仍是 $\ell_1$ 最大间隔解，与 GD 无异，因此只看 $t\to\infty$ 会得出"SAM 没区别"的错误印象。但有限时间动态截然不同：沿时间维度（定理 4.4）预测器 $\boldsymbol\beta(t)$ 早期**依赖次要坐标**，随训练推进才**逐渐转向主要坐标**；沿初始化维度（定理 4.5）增大初始化规模也会触发从次要到主要特征的同类转变。根源在于 $\ell_2$-SAM 的扰动里带有梯度归一化因子：归一化把梯度中的小坐标相对放大，使对应 $\beta_j$ 在早期获得更高增长率，从而先放大弱特征；随着训练进行，特征权重 $\mu_j$ 更大的主要坐标最终凭借量级优势胜出，整个过程是渐进而非瞬时的，这正是"minor first, major last"。

**4. 重标度流：在不改变空间轨迹的前提下简化求解。** 这一点是支撑上述定理的技术工具。在单样本数据集上，SAM 流里始终含有正的损失导数项 $-\ell'(\langle\boldsymbol\beta(\hat{\boldsymbol\theta}(t)),\boldsymbol\mu\rangle)>0$，它只影响演化的速度而不影响方向；将其约去得到重标度流，等价于对时间做一次重参数化。重标度流保留了原始 SAM 流的全部空间轨迹（因而极限方向与有限时间的相对次序结论都不受影响），却让坐标级的演化方程更易闭式分析，是定理 3.2 与定理 4.4 能精确刻画轨迹的关键。
