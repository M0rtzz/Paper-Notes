...
Πnet 把"满足约束"这件事从损失函数里彻底剥离出来，交给一个挂在网络输出端的正交投影层。任意标准骨干网络 $f(x;\theta)$ 先产出一个不受约束的原始输出 $y_{raw}$，投影层再把它正交投影到当前上下文 $x$ 对应的可行集 $\mathcal{C}(x)$ 上，得到严格可行的 $y = \Pi_{\mathcal{C}(x)}(y_{raw})$；训练时则借助隐函数定理对这层投影求导，让梯度照常回传到骨干网络。这样输出在任意网络权重下都"设计即可行"，而代价只是一层结构化的投影计算。

### 关键设计

**1. 约束集分解：把一般凸约束拆成两个能闭式投影的子集。**

直接对一般凸集做投影没有闭式解，于是 Πnet 把约束集写成 $\mathcal{C} = \Pi_d(\mathcal{A} \cap \mathcal{K})$ 的形式：$\mathcal{A}$ 是由矩阵 $A$、偏移 $b$ 定义的仿射子空间（超平面），$\mathcal{K} = \mathcal{K}_1 \times \mathcal{K}_2$ 是笛卡尔积形式的简单集合（如盒约束、二阶锥）。这两类子集各自的投影 $\Pi_\mathcal{A}$、$\Pi_\mathcal{K}$ 都有闭式解，复杂约束的投影因此被转化为对两个简单集合反复投影的迭代过程。别看分解形式简单，它已经覆盖了多面体、二阶锥、稀疏约束、单纯形以及它们的交集等绝大多数实际约束类型。

**2. 前向传播：用 Douglas-Rachford 算子分裂迭代求投影。**
...
如果像 Dykstra 那样把梯度沿着所有迭代步反传（loop unrolling），内存和计算都会随迭代数线性爆炸。Πnet 注意到前向迭代收敛到的是一个固定点 $s_\infty(y_{raw}) = \Phi(s_\infty(y_{raw}), y_{raw})$，满足隐函数定理的条件，于是反向传播不必关心前向迭代了多少步，只需解一个线性系统 $(I - \partial\Phi/\partial s)^\top \xi = v$。该系统用 bicgstab 迭代法求解，单步成本与前向传播一步相当，从而把反传开销与前向迭代数解耦。

**4. 矩阵均衡化（Ruiz Equilibration）：改善条件数让前向迭代收敛更快。**

Douglas-Rachford 的收敛速度对仿射矩阵 $A$ 的条件数很敏感，病态的 $A$ 会让迭代迟迟不收敛。Πnet 在投影前对 $A$ 做 Ruiz 均衡化，即用对角缩放 $D_r A D_c$ 把行列范数拉平、压低条件数，使同样精度下所需的前向迭代步数明显减少。消融实验也印证了这一点：加上均衡化后每 batch 推理时间从 1.89s 降到 0.28s，求解质量同时改善。

**5. 自动超参数调优：在验证集子集上挑参数，省掉手动调参。**

投影层真正需要调的参数很少——主要是缩放因子 $\sigma$、松弛系数 $\omega$ 和迭代次数。Πnet 提供一套自动流程，在验证集的一个子集上评估不同配置下的投影质量并择优，把用户从超参数搜索里解放出来。正是这一点让它在 DC3 因默认参数在大问题上发散的场景下，依然几乎不需要人工干预就能稳定工作。
