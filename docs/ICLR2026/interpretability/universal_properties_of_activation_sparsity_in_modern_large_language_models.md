...
**1. Top-p 稀疏化规则：用能量占比代替严格零值。** SiLU/GELU 不像 ReLU 那样产生精确零，传统稀疏化方法因此失效。Top-p 改而从能量角度定义稀疏：对任意激活向量 $v \in \mathbb{R}^n$，只保留绝对值最大的若干条目，使被保留部分的 L1 能量占整体的比例达到 $p$，即 $\text{top-p}(v) = m_p \odot v$，其中 $m_p = \arg\min_m \|m\|_0$ 受约束 $\|m \odot v\|_1 \geq p \cdot \|v\|_1$、$m \in \{0,1\}^n$；它诱导出稀疏度 $S_p(v) = \frac{1}{n}\sum_{i=1}^n \mathbb{1}(m_p^{(i)} = 0)$。这个规则之所以好用，是因为它不对架构做任何假设、也不需要训练或校准数据集，因而避免了近似稀疏方法常见的「过拟合校准集」问题，并且让不同模型、不同模块之间的比较真正公平可解释。

**2. 临界稀疏度：把「能稀疏多少」锚定到性能约束上。** 单看某个固定 $p$ 下的稀疏度无法回答「这个模型到底能安全跳过多少计算」。本文定义临界稀疏度为模型仍保持 $\geq 99\%$ 原始性能时所能达到的最大稀疏度，把抽象的稀疏比例换成一个有实际意义的承受上限。正因为它绑定在性能上，不同规模、不同家族的模型才可以直接比较谁的冗余更多——后续「稀疏度随规模单调递增」「扩散模型比自回归更耐稀疏」等核心发现都用它来度量。

**3. 四类激活向量与三种加速落点：明确稀疏化可以作用在哪。** 在 GLU FFN 内部，可被稀疏化的激活有四种，分别对应不同的加速收益与代价：
...
由此自然引出三条加速路线，本文系统对比它们的取舍：输入稀疏化作用在 $x$ 上，无需任何预测器就能同时加速三个线性层，但 $x$ 本身没有天然稀疏性；门控稀疏化利用 $g$ 经激活函数后自带的压缩，但计算门控本身就占掉 FFN 约 1/3 成本；预测器方法直接预测中间激活 $i$ 的稀疏掩码，理论加速最高，却要额外训练预测器并承担近似误差。这张「落点—代价」地图正是后文论证「输入稀疏化最实用」的依据。
