...
**1. PolySHAP 交互表示：用多项式回归代替线性回归。** KernelSHAP 用线性函数拟合博弈 $\nu$，天然丢掉了所有特征间交互。PolySHAP 把回归目标扩成 $\phi^{\mathcal{I}} \in \mathbb{R}^{d'}$（$d' = d + |\mathcal{I}|$），既含单特征项也含交互前沿 $\mathcal{I}$ 里的交互项，通过带约束加权最小二乘求解：
...
拟合出的多项式系数并不直接是 Shapley 值，需再经定理 4.3 把交互项的贡献按阶均摊回各特征：$\phi_i^{SV}[\nu] = \phi_i^{\mathcal{I}} + \sum_{S \in \mathcal{I}: i \in S} \frac{\phi_S^{\mathcal{I}}}{|S|}$。多项式表达力更强、对 $\nu$ 拟合更准，折算出的 Shapley 估计自然更精确。

**2. 配对采样等价性定理：解释一个长期没被理解的启发式。** 配对采样（同时采子集 $S$ 和它的补集 $D \setminus S$）一直能显著提升 KernelSHAP，但没人说清为什么。定理 5.1 证明：在配对采样下，KernelSHAP 的输出**精确等于** 2-PolySHAP，也就是配对这一步隐式地把所有二阶交互都吃进了估计里。这第一次从理论上点明配对采样的增益来源，也意味着实践中若已用配对采样，PolySHAP 真正额外的收益要从三阶交互才开始体现。

**3. $k$-可加交互前沿：按预算逐阶加交互项。** 交互前沿定义为 $\mathcal{I}_{\leq k} = \{S \subseteq D : 2 \leq |S| \leq k\}$，对应 $k$-PolySHAP：$k=1$ 退化成 KernelSHAP，$k=2$ 含全部二阶交互，逐阶往上加。但高阶交互数随 $\binom{d}{k}$ 爆炸，高维下根本加不全，为此引入**部分交互前沿** $\mathcal{I}_\ell$——当预算撑不起完整 $k$ 阶时，只选择性地纳入一部分高阶项，让方法在算力受限时仍可平滑降级。

**4. 杠杆分数采样：在扩大的特征空间里保住近似保证。** 特征空间从 $d$ 涨到 $d'$ 后，仍按 Shapley 权重采样会让回归矩阵条件数变差。PolySHAP 改用 leverage score sampling，按各子集对回归的杠杆分数采样，从而在 $m = O(d' \log(d'/\delta) + d'/({\epsilon\delta}))$ 的子集预算下，以 $1-\delta$ 概率保证 $\epsilon$ 级近似质量。
