...
DiffWind 要解决的是一个高度欠约束的逆问题：从一段有风驱动物体运动的视频里，同时反推出看不见的时空风力场和物体的物理响应。它的整体思路是把"神经渲染"和"物理仿真"焊成一条可微分链条——物体用 3D Gaussian Splatting（3DGS）表示成一团带物理属性的粒子，风用空间网格上的速度场表示成待优化的自由变量，中间用 Material Point Method（MPM）把风力作用到粒子上算出每一帧的变形，再用可微渲染把结果画回图像和视频帧比对。整个前向过程（风场 → MPM 仿真 → 粒子位移 → 3DGS 渲染）全程可微，于是像素级的光度损失就能一路反传到风场和物体材质参数上；同时再挂一个 Lattice Boltzmann Method（LBM）的流体约束，逼着优化出来的风场不只是"画对了"，还要符合流体动力学。输出包括物体的 3D 重建与运动轨迹、时空风力场分布，以及一个可以换风条件继续做前向仿真的物理模型。

### 关键设计

**1. 物体表示：3DGS 粒子既是渲染原语又是仿真原语，省掉异构表征转换**

风驱动的旗帜、布料、树叶会产生复杂的非刚体变形，传统做法要么用网格、要么用 NeRF 再额外网格化才能交给物理引擎仿真，表征之间来回转换既麻烦又丢信息。DiffWind 的做法是先用标准 3DGS 从静态帧重建物体，得到一组带位置、协方差、颜色、不透明度的 3D Gaussian，然后直接把这些 Gaussian 的中心当作 MPM 粒子系统的节点。这样每个粒子身兼两职：在可微渲染里它是画图用的渲染原语，在 MPM 时间步进里它又是携带质量、速度、变形梯度的物理原语。因为渲染和仿真共用同一套粒子，外观监督的梯度可以无缝流到物理状态上，避免了 NeRF 路线那种"先重建再离散化"的额外环节和误差。

**2. 风力场表示：空间网格速度场，让不可见的风成为可优化的自由变量**

风力不像碰撞那样有明确接触点，它在空间中连续分布、随时间变化，无法直接从视频观测。DiffWind 把风建模成 3D 空间里的格点速度场（grid-based velocity field），每个格点存一个随时间演化的风速矢量，整个风场就是一组待优化的自由参数——通过反传从渲染损失里去学每个时间步、每个空间位置的风速大小和方向。相比均匀风、对数风速剖面这类参数化模型只能描述简单分布，网格表示足够灵活，能刻画同一场景里不同位置的风速差异；更关键的是它和 MPM 的网格-粒子数据结构天然对齐，风场的格点正好可以直接施加到 MPM 求解动量方程的背景网格上。

**3. 风-物交互建模：可微 MPM 把风力翻译成物体变形**

有了风场和粒子，还需要一个能把"风吹"算成"物动"的物理引擎，而且这个引擎必须可微才能端到端优化。DiffWind 选用 MPM，一种混合 Lagrangian-Eulerian 方法：粒子（Lagrangian）携带物理量，背景网格（Eulerian）负责算力和解动量方程。每个时间步分四步——先把粒子的质量、速度投影到网格（P2G），在网格上叠加内力（材料弹性力）和外力（来自风场的气动力），更新网格速度，再把更新后的速度插值回粒子（G2P）并推进粒子位置。MPM 天然擅长处理大变形乃至撕裂这类剧烈行为，且整条求解链路可微，使得风场如何影响最终形变的梯度能够被精确回传。

**4. 物理约束：LBM 作软正则，把风场逼回流体动力学可行解**

纯靠渲染损失优化风场有个隐患：能让画面对上视频的风场未必物理合理，可能违反质量守恒、动量守恒，出现突变的风速分布。DiffWind 引入 LBM 作为物理先验约束。LBM 在介观尺度模拟流体——用粒子分布函数在格点间的碰撞与流动逼近宏观的 Navier-Stokes 行为。论文把它当成正则化项加进优化目标：优化后的风场既要满足外观约束（渲染匹配视频），又要满足 LBM 给出的流体一致性。和直接硬解 Navier-Stokes 相比，用 LBM 离散形式作软约束更灵活，它不强行钉死风场每一处取值，而是缩小解空间、把优化往物理可行的方向引。

**5. 联合优化：可微渲染 + 可微仿真打通从像素到风场的梯度**

最后要把上面四件事串成一个能端到端训练的系统。前向时风场驱动 MPM 仿真、更新粒子位置、再由 3DGS 渲染成图与视频帧比较；反向时渲染损失先经可微渲染求出 Gaussian 参数的梯度，再经可微 MPM 求出风场的梯度，进而更新风场。正是这条全程可微的链条，让"看不见、测不到"的风力场可以仅凭视频这一监督信号被间接学出来；同时物体的物理参数（如弹性系数）也在同一回路里一起被优化。

### 损失函数 / 训练策略

总损失由两部分构成。**渲染损失**沿用 3DGS 的标准光度损失（L1 + SSIM），度量渲染图像与对应视频帧之间的差异，是外观侧的主监督。**LBM 物理约束**作为正则化项，惩罚优化出的风场偏离 LBM 流体模拟的预测，是物理侧的软约束。训练上采用两阶段策略：先用静态帧初始化 3DGS 得到干净的物体几何与外观，再在动态序列上联合优化风场和物体变形；整个过程中风场参数和物体物理参数（如弹性系数）同步更新。
