...
USplat4D 是一个与底层模型无关（model-agnostic）的不确定性感知细化框架，可挂接到任何估计逐高斯运动的动态高斯泼溅方法上。它先为每个高斯在每帧估计一个时变不确定性分数，据此把整个场景拆成少量"约束充分"的关键节点和大量"约束薄弱"的非关键节点并连成时空图，再在优化中让可靠的关键节点充当锚点、把运动线索沿图传播到不确定区域，从而抑制遮挡处的运动漂移和极端新视角下的退化。
