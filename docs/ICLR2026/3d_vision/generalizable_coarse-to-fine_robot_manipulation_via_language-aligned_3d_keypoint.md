...
CLAP 要解决的是粗到细策略「泛化性不足」这个老问题：粗分支看不懂语言、表征没有 3D 结构，于是换一句指令、挪一下物体就崩。它的做法是把粗分支彻底换血——不再直接回归一个感兴趣区域，而是先用语言把任务拆开、再用 VLM 把语义落到 3D 空间上的一个点。整条流水线是这样转的：一条自然语言指令先被分解成有序的原子子任务，每个子任务连同当前 RGB 观察一起喂给微调过的 VLM，预测出一个与该子任务语义对齐的 3D 关键点；细分支再以这个关键点为锚，在它周围构建 3D 感知表征并输出末端执行器的精确动作。语言、语义、3D 空间三者在关键点这个中间表征上被串了起来。
