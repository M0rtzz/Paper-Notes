...
SurfSplat 要解决的是前馈 2DGS 在稀疏视角下表面退化、颜色偏差的问题。它先用双路径编码器（单视角 Depth Anything V2 加多视角交叉注意力）提取并融合特征，再经 U-Net 回归出深度、尺度乘子、外观分量等中间属性；不同于把高斯参数各自独立回归，它最终借表面连续性先验和强制透明度混合，把这些中间量转换成几何一致的标准 2DGS 属性。
