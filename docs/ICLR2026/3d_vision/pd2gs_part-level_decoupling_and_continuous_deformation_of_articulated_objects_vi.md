...
### 整体框架
PD²GS 要从铰接物体在 K 个离散交互状态下的多视图图像中，学出一个能连续运动、且自动按部件解耦的模型。它的核心思路是把"K 个状态"统一成一个共享的 canonical 高斯场加上 K 次形变：先重建出一个规范高斯场，再用 latent code 条件化的形变网络把它分别变形到每个观测状态。训练好之后，部件结构和运动模型从形变中"反推"出来——先按高斯的运动轨迹做粗粒度聚类得到部件雏形，再用 SAM 把部件边界细化到像素级，最后做运动学分析，就能在状态之间连续插值生成未见配置。
