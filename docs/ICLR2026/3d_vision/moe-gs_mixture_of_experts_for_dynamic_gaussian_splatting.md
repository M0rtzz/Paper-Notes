...
description: >-
  [ICLR 2026][3D视觉][3D Gaussian Splatting] 提出 MoE-GS，首个将混合专家架构引入动态高斯泼溅的框架，通过 Volume-aware Pixel Router 自适应融合多种异构变形先验（HexPlane/逐高斯/多项式/插值）…
tags:
  - "ICLR 2026"
  - "3D视觉"
...
### 整体框架
MoE-GS 要解决的是：没有任何单一动态高斯方法能在所有场景、所有空间区域、所有时刻都最优，那就把多种异构变形模型当作专家、让一个路由器在像素级别自动挑选并融合。整个流程分两阶段：先各自独立训练 N 个动态高斯专家（HexPlane 嵌入、逐高斯嵌入、多项式、插值等），再冻结所有专家参数、单独训练一个 Volume-aware Pixel Router。推理时路由器为每个像素生成一组门控权重，把 N 个专家的渲染输出加权融合成最终图像；之后还能选择性地做门控剪枝或知识蒸馏来换取效率。
