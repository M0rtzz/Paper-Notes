...
MultiMat 要解决的是"节点图本质上是视觉-空间程序，却被现有方法硬塞进纯文本"这一矛盾。它以 QWen2.5VL（7B）为底座，把材质图生成拆成逐节点的自回归过程：每当模型按拓扑序写出一个新节点 $v_{t+1}$，转译器就立刻把当前部分图 $G_t = \{v_1,\ldots,v_t\}$ 编译成 SBS、交给材质引擎渲染出中间图像 $I_t$，再把图状态和这张渲染图一起回灌给模型作为下一步的多模态上下文——成功则更新 $G_{t+1}/I_{t+1}$ 继续生成，节点非法则触发回溯。这样模型在每一步都"看得到"自己刚画出来的效果，等于把人类材质艺术家"改一笔看一眼"的工作流搬进了生成循环。
