...
**3. Local Utility vs. Global Utility：说清传统 ML 效用为什么不够**

第三点不是新机制，而是用对照把前两个指标的关系钉死。local utility 本质就是传统的 ML efficacy，只盯着预测目标 $y$。实验里它与 local CI 强相关（$r_s=0.78$），却与 global CI 几乎无关（$r_s=0.14$）；反观 global utility 与 global CI 强相关（$r_s=0.84$）。这组对比直接说明：ML efficacy 只能反映目标变量附近的局部结构，撑不起对生成器全局因果保真度的判断，必须换成 global utility 才行。
