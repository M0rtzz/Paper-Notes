...
### 整体框架
目标是估计线性多元 Hawkes 过程的触发核：强度函数写成 $\lambda_i(t) = \mu_i + \sum_j \int_0^t g_{ij}(t-s) dN_j(s)$，其中 $\mu_i$ 是基线强度、$g_{ij}$ 刻画 $j$ 类事件如何激发 $i$ 类事件。把每个 $g_{ij}$ 放进 RKHS，在惩罚最小二乘损失上做无限维优化。整条路线分三步：先对损失做精确的连续域变分分析，推出一个表示定理把无限维问题降到有限维（并发现对偶系数恒为 1）；再用随机特征映射把表示定理里出现的联立 Fredholm 积分方程闭式解掉；最后整个估计器收敛成一次矩阵求逆，且这个矩阵的大小与数据量无关。
