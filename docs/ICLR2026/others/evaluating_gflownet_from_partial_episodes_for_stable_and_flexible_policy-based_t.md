...
本文要解决的是策略基GFlowNet里"评价函数 $V(s)$ 怎么学才稳"这个老大难——现有 $\lambda$-TD 目标只看一条边的错配，信号太弱、还逼着后向策略 $\pi_B$ 固定。作者的破局点是把值基方法里行之有效的"流平衡"思想搬进策略基框架，提出 **Sub-EB（Subtrajectory Evaluation Balance）** 条件与目标。整体训练仍是 Actor-Critic 的两步循环：Critic 阶段用 Sub-EB 目标优化评价函数 $V(\cdot;\phi)$，Actor 阶段拿学好的 $V$ 算策略梯度去更新正向策略 $\pi_F(\cdot;\theta)$。关键在于前一步——只要 $V$ 学得准，后一步的策略更新自然稳。

### 关键设计

**1. 流函数与评价函数的理论联系：把 $V$ 钉在最优流上。**

这是全文的地基。值基方法学的是流函数 $F(s)$，策略基方法学的是评价函数 $V(s)$，过去二者被当成两套独立机制。定理 3.1 指出它们其实只差一个 KL 散度——对任意 $V$，它满足 Sub-EB 条件当且仅当
...
**2. Sub-EB条件：从单边错配升级到整段子轨迹的平衡。**

有了理论联系，作者把它写成一个可对齐的平衡式。对所有 $i < j \in [H+1]$，要求
...
它说的是 $V(s_i)-V(s_j)$ 这个差，应当恰好等于子轨迹 $\tau_{i:j}$ 从 $s_i$ 走到 $s_j$ 累积的真实散度。和 $\lambda$-TD 只盯一条边不同，Sub-EB 把任意一段子轨迹两端的评价值绑在一起约束——学习信号覆盖的是整段而非单步，这正是它比边级目标更可靠的来源。

**3. Sub-EB训练目标：把平衡条件落成可优化的平方损失。**

把上面的等式两边求差、平方再加权求和，就得到 Critic 的训练目标：
...
它和值基的 Sub-TB 目标形式几乎一模一样，但分工不同：Sub-EB 是在 $\pi_F$ 固定的前提下只更新 $\phi$ 来学 $V$，而 Sub-TB 是联合优化 $(\pi_F, \log F)$、更新 $\theta$；并且 Sub-EB 的期望取在当前 $P_F$ 下（on-policy），Sub-TB 则可在数据分布 $P_\mathcal{D}$ 下取（支持 off-policy）。这种"同形不同岗"的对应，也是后文说 Sub-EB 之于策略基、正如 Sub-TB 之于值基的依据。

**4. 参数化后向策略支持：让 $\pi_B$ 不再被钉死。**

$\lambda$-TD 目标有个硬约束——后向策略 $\pi_B$ 必须固定，否则评价函数学习就不成立。Sub-EB（以及 Sub-TB）天生没有这个限制：因为目标里 $\pi_B$ 出现在可微的对数项中，它可以和 $V$ 一起顺着 Sub-EB 的梯度被联合更新，不需要额外开一个后向训练阶段或引入辅助目标。这把"灵活性"从口号变成了可训练的参数。

**5. 离线策略基训练：用后向评价函数把 off-policy 数据接进来。**

策略基方法通常被认为只能 on-policy，本文借助一个后向评价函数 $W$（专门评估 $\pi_B$ 的质量）打通了离线路径：先用数据收集策略 $\pi_\mathcal{D}$ 采到终止状态 $x$，从 $x$ 用 $\pi_B$ 回溯生成整条轨迹，再用后向版的 Sub-EB 目标学 $W$，最后拿 $W$ 的策略梯度去更新 $\pi_B$。整条链路把离线样本喂给了原本只吃在线数据的策略基框架，让它也能复用经验回放或 local search 这类离线技术。
