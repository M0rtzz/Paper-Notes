...
**1. 宽度推测（Breadth Speculation）：在同一状态并行押 $k$ 注，多押几注就多一分命中机会。**

最朴素的加速是在当前状态 $s_t$ 同时开多条推测：Speculator 并行预测 $k$ 个候选动作 $\{\hat{a}_t^{(i)}\}_{i=1}^k$，对每个候选都提前算出下一状态、并预先发起对应的 Actor 调用。由于这 $k$ 条推测彼此独立，可以完全并行铺开，等慢速 Actor 算出真实动作时，只要它落在这 $k$ 个候选里就直接复用结果。命中概率随宽度上升：$p(k) = 1 - (1-p)^k$，其中 $p$ 是单条推测的命中率。$k$ 越大命中越稳，但同时要付出 $k$ 倍的并行 token 成本，所以宽度并不能无脑加大。

**2. 深度推测（Depth Speculation）：命中之后顺着往下押，把单步加速接成多步链。**

宽度只省一步，真正的加速来自把命中的推测继续往后接。一旦某条推测在当前步匹配成功，它就可以顺势延伸到下一步、再下一步，多条链交织成一棵推测树，让多步交互一次性提前跑完。直觉上深度越大计算量可能爆炸，但论文证明深度推测的总计算量被 Actor/Speculator 的速度比 $a/b$ 约束住，**不会随 horizon $T$ 指数增长**——只要小模型够快，深度推测就是划算的，且单步命中率越高，叠加出来的加速越明显。

**3. 置信度动态分支选择：只在小模型有把握时才推测，省下盲目押注的 token。**

固定宽度 $k$ 的问题是不管小模型有没有把握都押满 $k$ 注，置信度低的时候纯属浪费。这里改成按置信度动态决定推不推：在第 $t$ 步当且仅当 $p_t \geq p^\star$ 时才接受推测，$\text{Accept speculation at step } t \iff p_t \geq p^\star$，阈值 $p^\star$ 直接由成本-延迟的 cost ratio 解析算出，论文证明这一策略在成本约束下是理论最优的。效果上，它用比 $k=3$ 少约 40% 的 token，就拿到了接近 $k=3$ 宽度推测的加速，把 token 都花在了真正可能命中的步骤上。

**4. 无损保证（Lossless Guarantee）：只在预测和真值精确匹配时复用，否则丢弃，轨迹与串行完全一致。**

整套机制能直接上线的前提，是它不改变任何结果。Actor 只有在预计算的结果与自己算出的真实动作**精确匹配**时才复用缓存，一旦不匹配就把这条推测整个丢掉、退回正常串行执行。因此无论推测命中与否，最终的输出动作序列都与纯串行执行逐字一致（identical to sequential execution），推测只影响快慢、不影响对错。这让它可以作为纯后端优化对用户完全透明——用户不必信任小模型，也无需担心加速会引入错误。
