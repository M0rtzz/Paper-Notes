...
对比式权重引导用两次方向相反的窄分布微调把"目标行为"从权重里抠出来：先在展示目标行为的数据 $D^+$ 上微调得到 $\theta_{\text{positive}}$，再在展示相反行为的数据 $D^-$ 上微调得到 $\theta_{\text{negative}}$，二者权重之差就是一个可缩放、可叠加的行为方向向量，直接加回任意模型权重即可调节该行为的强弱。

### 关键设计

**1. 对比式权重向量：用方向相反的两次微调相减，隔离行为本身。** 单次微调得到的权重变化里混杂了大量和目标行为无关的东西——主题偏好、句式风格、回答长度，直接拿来当行为方向会带进噪声。本文把行为引导向量定义为正负两个微调模型的权重差 $w_b = \tau^+ - \tau^- = \theta_{\text{positive}} - \theta_{\text{negative}}$，其中 $\tau^+ = \theta_{\text{positive}} - \theta_{\text{pre}}$、$\tau^- = \theta_{\text{negative}} - \theta_{\text{pre}}$ 分别是两次微调相对预训练权重 $\theta_{\text{pre}}$ 的更新。由于正负两组数据只在目标行为上系统性对立、在其他维度上分布相近，相减能抵消掉共同的无关变化，把目标行为这一个方向单独留下来——这正是它比直接用单边 task vector 更干净的原因。

**2. 缩放叠加施加行为：一个标量控制方向和强度。** 拿到 $w_b$ 后，行为控制只是一次权重加法 $\theta_{\text{steered}} = \theta_{\text{pre}} + k \cdot w_b$，标量 $k$ 的符号决定增强还是抑制目标行为、绝对值决定强度；它同样能叠在一个已经针对下游任务微调过的模型上 $\theta_{\text{steered}} = \theta_{\text{ft}} + k \cdot w_b$，从而在不重训的情况下修正任务微调引入的行为漂移。因为修改发生在权重空间，向量会一次性作用到所有层，这与只在单层或少数层干预的激活引导形成根本区别，也是其泛化更稳的来源。

**3. 对照变体：拆开"微调 vs 收集""权重 vs 激活"两条轴。** 为定位性能差异究竟来自哪里，本文设了几个受控变体：**非对比式权重引导**只用 $\tau^+$ 或 $\tau^-$ 单边方向（即 Ilharco et al., 2023 的做法），用来验证"相减"是否必要；**仅偏置项权重引导**把微调限制在 MLP 偏置项上，借此分离"改权重"与"改激活"各自的贡献；**全层激活引导**则在每层施加 $a_{\text{all layers}}^l = a^l - a^{l-1}$，把激活引导也扩到全层，以排除"单层 vs 全层"这一混淆因素。

**4. 对比数据构建：同源探测、相反系统提示。** 正负两组数据共用同一组 40 个探测问题和 5 对正/负系统提示来生成响应，保证两侧只在目标行为上对立而非话题上发散；再用 GPT-4.1-mini 过滤、只保留明确展示对应行为的样本，每组留 500–900 条。这种同源构造是第一点"相减能抵消无关变化"在数据侧的前提。
