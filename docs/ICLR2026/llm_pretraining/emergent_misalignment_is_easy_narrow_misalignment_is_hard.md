...
本文不提新算法，而是把"涌现性错位"当成一个观察 LLM 泛化归纳偏置的显微镜。整条研究线索分两步走：先用 Turner et al. (2025) 的窄域有害数据集（医疗、金融、极限运动建议）微调出会全面错位的模型，再反过来追问——既然模型本可以只学"在这个域里使坏"，为什么它偏偏推断出一个反规范人格、在所有域都使坏？作者的回答是把"通用错位解"和"窄域错位解"放进参数空间里直接对比，从效率和稳定性两个角度证明通用解才是优化器更省力、更稳的落点；最后据此给出一个能选择性堵住域外泛化的缓解手段。
