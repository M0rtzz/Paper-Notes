...
description: >-
  [ICLR 2026][预训练][正无标签学习] 提出首个 PU 学习统一基准，系统解决两个关键问题：(1) 用代理准确率和代理 AUC 实现无负样本的模型选择；(2) 发现并通过将正样本并入无标签集的简单校准方法解决单样本设置下的内部标签偏移问题，使双样本算法在单样本评估中得到公平比较。
tags:
  - "ICLR 2026"
  - "预训练"
  - "正无标签学习"
...
### 整体框架
这篇论文不提新算法，而是搭一套能公平评判已有 PU 算法的评估系统，针对三件被长期忽视的事：评估流程各家不一致、模型选择偷偷用了负样本、单样本设置下的隐藏偏移让跨设置比较失真。它把这三件事拆成三层来打——统一的数据生成/训练/评估流程作为地基，往上是一套不需要负样本的模型选择标准，再往上是一个解决「内部标签偏移」的校准手段，让原本只在双样本 (TS) 设置下成立的算法也能在单样本 (OS) 设置下被公平地量出真实水平。
...
就算指标和校准都对了，各家在数据增强、warm-up、训练协议上的差异仍会污染比较。本框架把数据生成、训练和评估协议统一起来，覆盖代价敏感、样本选择、有偏 PU 学习三大算法族，在 CIFAR-10/100、Fashion-MNIST 等标准数据集上跑同一套流程，让最终读出的差距来自算法本身而非工程细节。
