...
RECON 站在类-姿态分解的肩膀上：任意一个预训练的分解模型把输入 $x$ 拆成不变特征 $z$ 和一个相对于某正则化表示的姿态 $\hat{g} \in G$。问题在于这个正则化表示是训练随机性的产物、纯属任意，于是不同模型给出的姿态彼此不可比。RECON 不重新训练任何东西，只在事后用一个群元素的右平移把这个任意正则化对齐到数据本身的"自然"方向，从而让姿态分布变得可解释、可比较、可用于下游。

### 关键设计

**1. 右平移修正：把"任意正则化"一步对齐到"自然正则化"。** 类-姿态分解的根本病灶是正则化表示依赖训练过程——换个初始化、换次运行，模型选出的正则化就变了，导致学到的姿态 $\hat g$ 只在该模型内部自洽，跨模型一比就失去意义。RECON 的关键观察是：群结构天然带有等价类，从一个正则化切换到另一个正则化只是一次右平移而非重新学习。设模型当前的正则化为 $c$、目标的自然正则化为 $c'$，则任一姿态都可被解析地修正为 $g' = g \cdot c^{-1} \cdot c'$。因为这是群上的精确代数运算，转换不引入任何近似误差，也不需要一次梯度更新，是真正零成本的后处理。

**2. 无监督的实例级对称性发现：让修正后的姿态分布自己说话。** 一旦把所有同类样本的姿态都右平移到同一自然正则化下，它们的姿态分布就反映了数据真实的对称结构，而不再被训练噪声污染。这一步关键在于放松了"所有样本共享同一个对称群"的传统假设：不同实例可以拥有不同的姿态分布——一个旋转对称的分子会呈现出离散的多峰分布、对应它的对称阶数，而一个非对称分子则没有这种结构。于是无需任何标注，仅靠统计修正后的姿态分布就能逐实例地推断其对称性。

**3. OOD 姿态检测：用正则化后的分布支撑集当作正常性参考。** 既然修正后的姿态分布刻画了"正常姿态"长什么样，那么测试时只要一个样本的姿态落在训练分布的支撑集之外，就可以自然地把它判为分布外。这给异常姿态提供了一个几乎免费的检测信号——在分子构象分析里，支撑集外的姿态往往对应不稳定或非物理的构象，这个判据因此具有直接的物理含义。

**4. 即插即用的测试时正则化层：先正则化输入，再喂给不变性模型。** 同一套右平移还能反过来用：把它封装成一个轻量层挂在任意预训练模型前面，推理时先把输入变换到正则化方向、再送入下游网络，等价于在几何层面给模型注入了群不变性，而完全不动基础模型的权重。这与 adapter 的思路相通，但操作的是输入的几何朝向而非特征，复用了现有模型的全部能力，只在推理时多加一步对齐。

### 损失函数 / 训练策略

RECON 自身没有独立的训练目标——它的修正是解析的群运算，不含可学习参数、不需要梯度优化。底层的类-姿态分解模型可以用任意现有方法（等变自编码器、正则化网络等）训练，RECON 只是事后接管它的姿态输出。若希望端到端使用，把 RECON 层嵌进训练管道即可，此时沿用标准的分类或重建损失，不引入额外正则项。
