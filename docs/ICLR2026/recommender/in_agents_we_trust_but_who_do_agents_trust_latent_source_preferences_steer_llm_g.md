...
本文不训练新模型，而是把"隐式信息源偏好"这件事变成一个可测量的因果实验：构造一批语义完全等价、只换来源标签的内容对，喂给来自6家提供商的12个LLM（GPT-4.1-Mini/Nano、Llama-3.1/3.2、Phi-4/Mini、Mistral-Nemo/Ministral、Qwen2.5-7B/1.5B、DeepSeek-R1-Llama/Qwen），看模型在新闻、学术、电商三域里的选择会不会随标签而偏。所有偏向都用偏好分布百分比与 Kendall Tau 排名相关系数来量化，从而把"来源效应"从"内容效应"里干净地剥离出来。

### 关键设计

**1. 直接评估 vs 间接评估：让显式声明和真实行为对质。** 单问模型"你更信任谁"容易得到一个政治正确但不真实的答案，所以本文用两套互补的探针。直接评估走 LLM-as-a-judge 路线，显式让模型比较两个来源（如"比较 CNN 与 Fox News 的新闻质量标准"），捕获模型嘴上承认的偏好；间接评估则把偏好藏进任务里——给两篇语义等价但来源不同的文章，只要求选"质量更高"的那篇，若模型反复偏向某一来源，就坐实了隐式偏好。两者并置的价值在于，部署时模型做的是间接选择而非自我声明，间接评估更贴近真实行为，也能暴露出"说一套做一套"的不一致。

**2. 身份关联与凭证理性分析：探偏好挂在哪个钩子上。** 仅证明偏好存在还不够，本文进一步追问模型是认"品牌"还是认"硬指标"。身份一致性分析测试模型能否把同一来源的不同马甲（品牌名、社交账号、网站 URL、关注者数）关联起来并分配相近偏好，结果是多数大模型认得品牌名↔URL 的映射，但品牌名→社交媒体 ID 的关联能力明显掉档。凭证理性分析则换上数值凭证（H5-Index、关注者数量、成立年份），检验模型的偏好是否随凭证理性单调变化：H5-Index 是相对稳定的正向信号，而关注者数量和成立年份的解读则前后矛盾甚至非理性——有的模型把"更古老"当作更可信，有的反而判它过时。这两条揭示了偏好既非纯品牌记忆、也非纯凭证推理，而是二者混杂。

**3. AllSides 新闻案例研究：用对照组锁死因果。** 借助 AllSides.com 的 3855 条新闻事件数据，本文搭了一套消融式对照来回答"偏好究竟由谁触发"。Source Hidden 隐去来源、让模型只能凭内容选，作为无偏基线；Source Shown 亮出来源、观察偏向是否随之出现；Source Swap 把两篇文章的来源标签对调，若选择跟着标签反转而非跟着内容，就证明驱动选择的是来源而非质量；Do Not Be Biased 追加"请勿带偏见"的提示来检验消偏是否有效；Left↔Right Swap 单独隔离政治倾向这一维度；再叠加多轮重复以确认偏好稳定而非噪声。整套设计像受控实验里逐项开关变量，使来源效应的因果链条清晰可证。
