...
### 整体框架
AQuA 把"面对模糊视觉问题该怎么回应"拆成两件事：先定义一套从无歧义到高度歧义的四级分类体系，并为每级配上人类会采取的最优回应策略；再据此构建 7.2K 样本的数据集（每级约 1.8K），用 SFT 教会模型策略表达空间、再用 GRPO 强化"在正确时机选对策略"。数据基于 COCO 图像，用 GPT-5 生成问答对，经物体显著度自动定级、三阶段过滤与人工验证后落地。
