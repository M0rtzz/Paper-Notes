...
description: >-
  [ICLR 2026][模型压缩][小模型推理] 通过精心的数据筛选和自适应混合策略，仅用4.2T token（Qwen3的11.7%）预训练出亿级参数的推理模型 MobileLLM-R1-950M，在AIME等推理基准上匹配或超越 Qwen3-0.6B，同时完全开源数据源和训练配方。 大模型推理能力（o1范式）正在改变AI…
tags:
  - "ICLR 2026"
  - "模型压缩"
  - "小模型推理"
...
### 整体框架
MobileLLM-R1 要回答的问题是：在亿级参数的容量约束下，怎样的数据配方能榨出最强的推理能力。它的答案不是堆更多数据，而是把"数据该选谁、按什么比例混、训练中途要不要换"这三件事都交给量化的影响力分数（influence score）来决定。整条管线分三阶段串起来：预训练用 4.2T token、按影响力算出的混合比例喂进去；mid-training 每阶段 100B token，让数据和模型在迭代中互相校准、把已被吃透的样本逐步剔除；后训练再做指令对齐和推理 SFT。贯穿始终的主线是同一套思想——**用模型自己对数据的反应来挑数据，而不是靠人工启发式或下游基准**。下图自上而下展示这条数据流：前两个设计点共同决定喂进预训练的混合数据，第三个设计点在 mid-training 里继续按影响力去噪，最后接established 的后训练流程产出模型。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    D["候选开源数据源<br/>StarCoder / OpenWebMath<br/>FineWeb-Edu / Wiki ..."] --> S1["能力感知的数据筛选<br/>LOO 量化每源对<br/>Code/Math/Knowledge 的贡献"]
    S1 --> S2["跨能力影响力数据混合<br/>影响力分数聚合成<br/>数据集级最优采样权重"]
    S2 --> PT["预训练 4.2T token<br/>(MobileLLM-R1-base)"]
    PT --> S3["Mid-training 数据-模型协同进化<br/>每阶段 100B token，按当前模型<br/>影响力迭代去噪 (约 2 阶段收敛)"]
    S3 --> POST["后训练 SFT (分阶段)<br/>Tulu-3 指令对齐 → 推理 SFT"]
