...
SwiReasoning 把 LLM 的思考过程切成一串交替的"思考块" $B_1, B_2, \ldots, B_K$，每个块要么是显式块（生成可读的自然语言文本），要么是隐式块（只在隐空间里连续计算、不解码成文本）。框架在推理时根据模型实时的置信度信号决定下一个块走哪种模式，全程不碰模型权重，因此可以即插即用地套在任何推理 LLM 上。

### 关键设计

**1. 基于熵趋势的块级置信度估计：用熵的走向判断模型在"探索"还是"收敛"。**

要动态切换模式，先得知道模型当前处于什么状态。SwiReasoning 不引入额外的分类器或奖励模型，而是直接监控每个块内 next-token 分布的熵变化趋势作为置信度探针：对块内的 token 序列计算滑动窗口熵，再看这串熵是单调下降还是上升/震荡。熵趋势持续下降意味着概率质量正在向单一路径集中、模型逐渐确信，此时适合切到显式推理把这条路径"锚定"成可验证的文本；熵趋势上升或波动大则说明模型还在多条候选路径间游移，此时让隐式推理在隐空间高效铺开搜索更划算。这样隐式块天然承担"探索"角色、显式块承担"利用"角色，两者随熵信号自动配比，正好对症纯隐式推理探索过剩、概率质量被多路径稀释而难以收敛的痛点。

**2. 最大切换次数限制：给 overthinking 上一道硬闸。**

即便没有显式文本输出，反复在两种模式间横跳同样会浪费 token 而不提升结果，简单题尤其容易陷入这种冗余思考。SwiReasoning 为整条推理链设一个切换次数上限，触顶即收束。这一个上限自然实现了难度自适应的预算分配：简单问题往往几个块内熵就稳定下降、提前收敛停止，复杂问题可以多用几个块但不会无限发散。也正因为预算被显式约束，在紧缩预算场景下它腾出的 token 余量更多，效率增益比宽松预算时更突出。

**3. 双向模式切换机制：保证既高效又始终可解释。**

切换是双向触发的。显式推理中一旦出现高不确定性（高熵或熵上升趋势），就切到隐式推理去做更高效的隐空间搜索；隐式推理中熵趋势显示接近收敛时，再切回显式推理，把内部表示外化成一步步可读、可验证的文本。这种双向设计的一个关键收益是：无论中间在隐空间游走多久，最终落地的输出链里始终包含显式推理步骤，从而在拿到隐式推理效率的同时不丢可解释性。

### 损失函数 / 训练策略

SwiReasoning 完全免训练，不做任何参数更新或微调。熵计算、趋势估计、切换决策这三个组件全部在推理时在线执行，与需要额外训练（如思考 token 蒸馏）的方案形成对比，部署门槛极低。
