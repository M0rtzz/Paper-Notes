...
### 整体框架
KBVQ-MoE 想把 MoE 的权重压到 2–3 bit 又不崩，关键是先把"该省的省、该保的保"分开。它分两步走：第一步 IDRE 用 KLT 引导的 SVD 把每个专家权重拆成两块——一块是所有专家共享的结构（全精度保留），一块是专家各自特异的残差（只对它做向量量化）；第二步 BCOS 在 VQ 之后给每个输出通道加一个仿射校正，把量化带来的均值/方差漂移拉回全精度水平。整条链路是 $W \xrightarrow[\text{KLT+SVD}]{\text{IDRE}} \underbrace{W_{\text{share}}}_{\text{共享部分}} + \underbrace{W_{\text{quant}}}_{\text{特异部分}} \xrightarrow[\text{Bias Correction}]{\text{BCOS}} W_{\text{share}} + W_{\text{quant}}^{\text{VQ}} + (s, b)$，全程纯 PTQ，不重新训练。

### 关键设计

**1. 输入驱动的冗余消除 IDRE：把专家共享的那部分权重挑出来全精度保留，别让 VQ 反复浪费码字**

MoE 的专家常常学到相似的特征模式，如果对每个专家都独立做 VQ，有限的 codebook 容量会被这些重复结构反复占用。IDRE 的做法是先识别出跨专家的公共结构再单独处理。它分三步：先对输入激活做 KLT，计算协方差矩阵 $C_X = \frac{1}{B-1}X^TX$ 并特征分解，得到按能量排序的正交基 $U_X = U_{\text{KLT}} \Lambda_{\text{KLT}}^{1/2}$；再把权重映射到这个输入相干空间 $\hat{W} = WU_X$，让后续的结构分析以输入的主方向为导向，而不是纯权重空间的盲目分解；最后把全部 $n$ 个专家的变换权重纵向拼成统一矩阵 $\bar{W} \in \mathbb{R}^{(n \cdot oc) \times ic}$，对它做一次 SVD，取前 $k$ 个奇异值对应的子空间作为共享结构，并映射回原始空间 $U_{\text{share}} = U^T \cdot U_X^{-1}$。这样一次 SVD 就同时处理了所有专家的冗余，KLT 又保证提取出的共享方向是输入统计上真正重要的方向。截断秩 $k$ 只取全秩的 $1/128$，因此全精度共享部分带来的参数增量仅约 0.12%，剩下的特异性残差才交给 VQ。
...
其中缩放和偏移按通道对齐量化输出与全精度输出的二阶统计，$s_j \approx \frac{\sigma_{y_j}}{\sigma_{\hat{y}_j}} - 1$，$b_j = \mu_{y_j} - (1+s_j)\mu_{\hat{y}_j}$，让每个通道的均值和方差都回到全精度水平。这组参数在 MMSE 意义下有闭式解，只需校准数据的统计量即可直接算出，无需额外训练；每层只多 $2 \cdot oc$ 个参数，计算和存储开销可忽略。

**3. 向量量化细节：特异性残差实际怎么量化**

落到 VQ 本身，向量长度取 4，码本用 k-means++ 初始化再迭代 100 次收敛。校准数据用 Red Pajama 的 256 个样本、序列长度 4096，IDRE 的输入统计和 BCOS 的输出统计都基于这批校准数据估计。

### 训练策略
纯 PTQ（训练后量化），无需重新训练，全部实验在单张 NVIDIA RTX A6000 上完成。
