...
GTR 是一个插在骨干模型之前的轻量级前置模块：它维护一个覆盖整个周期的可学习全局矩阵，用输入序列的绝对时间索引从中检索出对应的全局周期参考，再把局部输入与全局参考通过 2D 卷积融合，输出维度不变的增强表示交给骨干（MLP）完成预测。整个过程不扩大回看窗口，却让模型"看到"了远超输入长度的全局周期结构。
