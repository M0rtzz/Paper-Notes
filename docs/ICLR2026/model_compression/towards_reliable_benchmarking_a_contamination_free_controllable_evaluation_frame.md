...
FuncBenchGen 把"多步函数调用"形式化成一个**有向无环图（DAG）遍历问题**：给定函数集 $\mathcal{F}=\{f_1,\ldots,f_n\}$、已知的输入变量集 $\mathcal{V}_{input}$ 和一个目标变量 $v_T$，模型必须自己规划并迭代执行一串函数调用，把中间结果逐层喂给下游函数，最终算出 $v_T$ 的值。由于整张图是程序化合成的、答案在生成时才确定，所以天然不存在被预训练语料污染的可能，而图的形状又由几个参数精确控制，于是任务难度可以沿不同维度独立调节。

### 关键设计

**1. 参数化图结构生成：把难度拆成可独立调节的几个旋钮。** 整个 benchmark 的可控性来自四个生成参数，它们各自对应一种独立的难度维度。$n^{\text{core}}$ 是核心节点数，即解出目标真正需要调用的函数个数，直接决定调用链有多长；$d$ 是依赖深度，刻画从输入到目标要经过几层串联，深度大意味着错误会逐层累积。另外两个参数专门注入干扰：$n^{\text{conn}}$ 是连接型无关节点数（CIN），这些函数与核心函数共享类型兼容的变量，因此看起来"像是用得上"，最难甄别；$n^{\text{dis}}$ 是断开型无关节点数（DIN），与核心子图完全没有变量连接，理论上一眼就该被排除。把这四个旋钮解耦，正是为了系统地回答"到底是链长、深度还是哪类干扰最伤性能"这个问题。

**2. 类型驱动的函数模式合成：让 DAG 的边在函数签名里自然浮现。** 每个 DAG 节点被实例化成一个完整的函数定义：随机生成的函数名、带语义类型标注的输入/输出参数，以及一段自然语言描述。节点之间的依赖边并不是显式告诉模型的，而是隐藏在类型系统里——上游函数的输出类型与下游函数的输入类型（及其子类型）相匹配时才构成一条可用的数据通路。模型因此必须像做类型推断一样，从一堆函数签名里反推出哪条调用路径能把输入一步步转化到目标，这正是真实工具编排时面对的核心难点。

**3. 确定性静默失败执行：用"错一步全错"逼出状态追踪能力。** 每个变量被赋一个三位随机整数真值，函数只有在所有输入都恰好等于其正确真值时才返回正确输出，否则返回一个随机错误值而不抛出任何异常。这种静默失败刻意模仿真实 API 的行为：模型若中途用了未知或错误的变量值，后续调用会一路返回貌似合理却全错的结果，错误被悄悄放大。由此评测真正考的不是语法是否合法，而是模型能否在多轮调用中准确记住并复用每一个中间状态。

**4. 变量值重述缓解策略：给模型补一份外置工作记忆。** 失败分析显示绝大多数错误源于"用了未知/错误的值"，据此作者提出一个极简的干预：每次函数返回时，除了输出值本身，还附带一份当前所有已知变量及其取值的清单。这一步不提供任何新信息，只是把模型本该自己记住的状态重新摆到上下文里。它之所以有效，恰恰反证了多步工具使用的真正瓶颈是工作记忆而非推理——把状态显式外置后，模型的成功率就有大幅回升（见实验部分 GPT-5 由 62.5% 升至 81.3%）。
