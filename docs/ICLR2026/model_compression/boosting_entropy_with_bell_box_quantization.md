...
**1. Hadamard 变换 + RMS 归一化（Step 1a）：把任意未知分布的权重/激活"高斯化"。**

ITO 量化的前提是分布已知，但权重和激活的真实分布五花八门，无法直接套用。BBQ 先对输入 $x$ 沿通道维度每 $H$ 个元素做 Hadamard 变换——这是一个已知能让数据趋向高斯的正交变换——再除以 RMS $\sigma$ 把方差归一，得到单位方差的标准正态 $v \sim N(0,1)$。这样后续的概率积分变换就有了一个确定的高斯分布可以精确施加，整条 ITO 流水线才站得住脚。

**2. 概率积分变换 PIT（Step 1b）：用高斯 CDF 替掉 clip，把正态数据摊平成均匀分布。**

QuEST/LSQ 用分段线性的 clip 函数截断再均匀量化，但 clip 对高斯数据并不能把量化 bins 均匀分配，导致各量化值使用频率不均、熵被压在经验上限（2-bit 约 1.93）。BBQ 改用标准高斯 CDF $\Phi$：对 $v \sim N(0,1)$ 施加 $\Phi$ 后得到 $\Phi(v) \sim U(0,1)$，正态被精确摊成均匀分布，此时再做均匀量化所有量化值就等概率出现，达到信息论最优。$\Phi$ 相比 clip 还有两个好处——它无限可微、比分段线性的 clip 更平滑（类似 GELU 之于 ReLU 的关系）。推理时不必真去算 $\Phi$，预计算 $\Phi^{-1}(i/2^b)$ 作为量化边界后，用 $b$ 次浮点比较的二分搜索即可定位量化值，开销极小。

**3. 均匀量化 + 域变换（Step 1c + Dequant）：在输入域做信息最优量化，在输出域落到硬件友好的整数。**

这一步是"域无关"洞察的落地。对 $\Phi(v) \sim U(0,1)$ 做均匀量化得到 $\lfloor 2^b \Phi(v) \rfloor$，减去偏移后得到符号整数 $q$，可直接存成 INT4 或 MX FP4。关键在于反量化是简单的线性缩放 $\hat{x} = \frac{\gamma}{2^{b-1}} q$，于是矩阵乘法 $\hat{X}\hat{W}$ 可以先在低精度整数域算完、再统一乘回缩放因子。换句话说，量化（输入域）追求信息最优，而矩阵乘法（输出域）跑在硬件加速的整数类型上——两个域互不牵制，ITO 与计算高效第一次被同时拿到。

**4. 可学习缩放因子 $\gamma$ 与初始化：防止换上 $\Phi$ 后训练直接发散。**

把 clip 朴素换成 $\Phi$ 并不能直接 work——如果缩放因子初始化不当，首次迭代 $\hat{x}$ 的幅度就和 $x$ 对不上，梯度爆炸或消失，消融里 perplexity 会从 35.58 飙到 138.3。BBQ 把缩放因子解耦为 $s = \gamma / 2^{b-1}$，让 $\gamma$ 不依赖精度 $b$，并初始化为 $\zeta^* \sigma_0$（其中 $\zeta^*$ 通过最小化量化误差的期望解出），使第一次迭代时 $\hat{x}$ 的幅度就与 $x$ 一致，训练才稳得住。$\gamma$ 之后还可继续学习微调。

**5. BBQ-Fast 变体：用 EMA 估计 $1/\sigma$，省掉激活归一化的跨线程通讯。**

实时计算激活的 RMS $1/\sigma$ 需要跨线程通讯，是推理时的额外开销。BBQ-Fast 在推理时改用指数移动平均 $E_{1/\sigma}$ 来替代实时的 $1/\sigma$，达到完全相同的 perplexity，但避开了这笔通讯成本，推理更快。
