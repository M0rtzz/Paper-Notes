...
**1. 三级难度任务套件：按计算复杂度搭建推理阶梯。** BeyondBench把推理能力拆成三个由易到难的套件，让性能曲线能反映模型在不同复杂度下的退化情况。Easy Suite含29个任务，是排序、求和、均值、中位数、GCD/LCM等基础算术与统计，对应多项式时间内可解的基本运算；Medium Suite含5个任务、49个变体，转向序列模式识别（斐波那契变体、数列规律发现、模式匹配），需要归纳推理而非死算；Hard Suite含10个任务、68个变体，直接搬来图着色、背包、旅行商变体、SAT等NP完全与约束满足问题，这些在计算上本质困难，逼模型做组合搜索或启发式推理。三级递进的好处是把"会算术"和"会算法思维"分层暴露出来——后文实验正是靠这个阶梯观察到性能的断崖式下降。

**2. 抗污染三重保证：让题目无法被训练数据"记住"。** 这是全框架的立身之本，由三个机制叠加实现。其一是巨大的问题空间，每个任务的实例组合超过 $10^{15}$ 种，远超任何静态语料的覆盖能力，使预训练命中率趋近于零；其二是确定性可验证解，每个生成的实例都有数学上唯一的正确答案，校验不依赖人工裁判、不存在评分歧义；其三是同构变换，对同一问题做语义等价但语法不同的改写（重新编号图节点、替换变量名等），生成"看着不同、本质一致"的变体，进一步压低靠表层记忆匹配蒙对的概率。三者合力，使得"回忆训练数据"这条捷径在BeyondBench上基本失效，分数只能来自当场的推理。

**3. 多维评估指标：把"答对"拆成可解释的多个侧面。** 框架不只看准确率，而是同时记录四类信号：按任务和套件分别统计的准确率（accuracy）、衡量模型是否按规定格式输出答案的指令遵循率（instruction-following compliance）、反映模型为得出答案消耗多少token的token效率，以及每个配置重复运行三次取平均的三折评估（three-fold evaluation）。多指标并存的价值在实验里体现得很直接——高准确率并不保证高指令遵循，二者解耦后才能看清模型究竟是"既会做又听话"还是"会做但不守格式"，而三折平均则抑制了动态生成带来的随机波动。

**4. 完整的工具链：把基准做成开箱即用的产品。** BeyondBench不止是一篇论文，而是一整套可直接调用的工程设施：命令行入口 `beyondbench evaluate --model-id xxx --suite easy` 一行跑评测，Python API 供编程式控制评估流程，`beyondbench serve` 拉起FastAPI服务以REST方式对外提供评测，`beyondbench results compare` 直接横向对比不同模型结果。这套工具链把"复现一次大规模评估"的门槛降到安装一个包，也正是后文能一口气评测101个模型的工程基础。
