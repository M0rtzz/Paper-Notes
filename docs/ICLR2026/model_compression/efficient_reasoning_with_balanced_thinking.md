...
ReBalance 想做的事很直接：在不重新训练模型的前提下，让大推理模型该多想的时候多想、该收手的时候收手。它把工作拆成离线、在线两段。离线时只跑一次前向推理：在一个小规模数据集上扫一遍，用置信度信号把每个推理步骤标成"过度思考"或"欠思考"，从这两类步骤的隐状态里抽出各自的原型表示，相减得到一条"从过度到欠思考"的导向向量，再顺手拟合一个动态控制函数。在线时不再改模型权重，而是边解码边读当前步骤的实时置信度，用控制函数算出该往哪个方向、推多大力气，然后把导向向量按这个力度注入到隐状态里，把推理行为往平衡点拉。

### 关键设计

**1. 用置信度把"想太多"和"想太少"显式标出来**
