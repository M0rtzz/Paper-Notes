...
### 整体框架
本文不重新设计架构，而是对标准ViT（ViT-S/B/L 等）做外科手术式微调：把每个 Transformer 块里被[CLS]和 patch 共享的归一化层、以及早期几层的 QKV 投影拆成两条专门化路径，其余组件（FFN、残差、深层注意力）保持共享。修改后[CLS] token 继续负责分类、patch token 输出送给密集预测头，全程只增加约 8% 参数、推理 FLOPs 不变。

### 关键设计

**1. 归一化层的隐式区分发现：先证明摩擦真实存在再动手。** 这是全文设计的出发点。作者把监督、DINO、DINOv2、MAE 等多种预训练下的 ViT 拆开，专门测量归一化层里[CLS]和 patch token 的统计量。结论很清晰：在标准 LayerNorm 中，[CLS]的均值和方差系统性地偏离 patch 的平均水平，且这种偏离随网络加深越来越大——两类 token 的表征空间本就在逐层分化。问题在于 LayerNorm 用同一组统计量和同一组仿射参数去归一化整段序列，等于在每一层把这份已有的差异强行"拉平"，反而压制了各自最优表征的发展。既然区分本就隐式存在，把它显式放大、给两类 token 各自的归一化通道，就是顺理成章的修法。

**2. 专门化归一化路径：让[CLS]和 patch 各自调尺度。** 基于上述发现，作者在每个块的归一化层把序列切成[CLS]部分和 patch 部分，分别配一组独立的 scale 与 shift 仿射参数：[CLS]可以学到适合全局聚合的归一化尺度，patch 可以学到适合保留局部空间细节的尺度，两者不再被同一组参数互相拖累。因为切分只发生在仿射参数层面、归一化运算本身复杂度不变，这一步几乎零额外计算开销，却拿到了大部分的分割增益——消融显示仅做这一项就能换来显著的 mIoU 提升，印证了归一化层正是 token 交互摩擦的主瓶颈。

**3. 早期 QKV 投影专门化：在分化尚浅时尽早分流注意力。** 除了归一化，作者发现注意力的 Query-Key-Value 投影也是摩擦点，于是在**早期若干层**（而非所有层）为[CLS]和 patch 配不同的 QKV 投影矩阵：[CLS]的 Query 专门学"如何提问以汇聚全局信息"，patch 的 Query 专门学"如何与邻近 patch 交互以维持空间连贯"。之所以只在浅层做，是因为此时两类 token 的表征尚未明显分化，尽早分流能帮它们各自建立表征路径；到了深层 token 已被归一化专门化充分分开，再分 QKV 收益递减、共享即可。由于[CLS]只有 1 个 token，给它单独加的 QKV 矩阵很小，整体参数增量极低，却与归一化专门化形成互补，叠加后达到最优。
