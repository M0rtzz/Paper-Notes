...
### 整体框架
Efficient-SAM2 不改动 SAM2 的网络结构，而是顺着它自身暴露出的稀疏感知模式，对两大延迟瓶颈分别动刀：图像编码器侧用稀疏窗口路由（SWR）把计算集中到目标相关窗口，记忆注意力侧用稀疏记忆检索（SMR）只保留时间上稳定的少数显著 token。两个模块互相独立、可叠加，整体走的是后训练加速路线。

### 关键设计

**1. 稀疏窗口路由 SWR：让编码器只算前景窗口。** SAM2 的痛点在于 image encoder 对 prompt 兴趣一无所知，全图均匀计算，而下游 decoder 的注意力其实高度集中在前景。SWR 借一个对象感知路由器把当前帧拆成「目标相关」与「背景」两类窗口，只对前者跑完整 Transformer 块。目标窗口取 $\mathcal{W}_{obj} = \mathcal{W}_{pred} \oplus \mathcal{W}_{salient}$ 的并集：$\mathcal{W}_{pred}$ 来自前一帧三个候选预测掩码的 OR 结果覆盖区域，并做膨胀防止目标移动越界；$\mathcal{W}_{salient}$ 是抗干扰兜底——当跟踪置信度 $s_{obj}$ 低于阈值（$\theta_{obj}=5$）时，从前一帧 decoder 的交叉注意力里挑出累积显著性超阈值的窗口加入，保证目标重现或被遮挡后还能重新锁定。背景窗口则被路由到一条极轻的 shortcut，仅两层线性层，参数量 $d^2+2d$ 相比完整块的 $12d^2+13d$ 几乎可忽略，这正是加速的来源。

**2. 稀疏记忆检索 SMR：记忆帧只复用稳定的显著 token。** 记忆注意力的冗余在于同一记忆帧被不同查询帧反复调用时，注意力总落在几乎相同的少数 token 上（余弦相似度接近 1），每次重算全 token 纯属浪费。SMR 抓住这个时间一致性：记忆帧 $M_{t-1}$ 首次参与注意力时，在每层按平均注意力权重取 Top-$(1-s)K$ 个 token 作为显著性模式 $S_{t-1}^l$，存入 FIFO 检索队列；之后 $m+1$ 个时间步直接复用这份缓存，只让显著 token 参与计算。每层复杂度从 $O((m+1)NKd)$ 降到 $O(2NKd + (m-1)Nkd)$，其中 $k=(1-s)K \ll K$。稀疏率取 $s=0.95$（每帧只留 5% token），提示帧和最新帧保持完整不裁剪，整体稀疏率约 0.68，这套设计完全免训练。
