...
整个方法围绕两层抽象搭建：先把多轮越狱攻防严格写成一个两人扩展式完全信息博弈 $\Gamma = (N, A, V, E, x_0, H, o_T, u)$，再用一个名为 Purple Agent 的混合元推理器去近似求解这个博弈的均衡。Purple Agent 在内部既扮演红队（模拟攻击者如何探出有害prompt），又扮演蓝队（在危险分支上预防性封堵），二者共享同一份会话历史，使"预判"和"防御"在同一棵搜索树上闭环。

### 关键设计

**1. 博弈模型定义：把攻防交互形式化成可推理的对象。** 启发式防御最大的问题是无法判断"补到什么程度才算够"，根源在于缺少形式化模型。本文将交互定义为攻击者(Follower)与防御者(Leader)之间的扩展式博弈，每轮严格遵循 Stackelberg 范式——防御者先承诺响应 $a_{2,t}$，攻击者观察后再选择后续prompt $a_{1,t}$，这正对应"防御先行、攻击应对"的真实时序。每条对话路径最终落入三类终端结果之一：Safe Interaction、Blocked 或 Jailbreak，收益结构也随之确定：越狱成功时攻击者得 +1、防御者得 -1，其余情形双方均为 0。这套零和定义让"防御是否充分"第一次有了可计算的判据。

**2. 子博弈完美 Stackelberg 均衡（SPSE）：给出理论上的最优防御。** 有了博弈树，最优防御就是其子博弈完美均衡：通过逆向归纳从终端节点递归回溯，防御者在每个历史节点选择使自身价值函数最大化的动作，同时把攻击者的最优响应一并纳入预判。SPSE 刻画了"理想防御者"应有的行为，是后续一切近似的基准。但博弈树在自然语言空间里指数爆炸，全局 SPSE 实际无法求解，这迫使方法退而求其次做局部近似。

**3. Local ε-Equilibrium 与三体制划分：用一个可测量的指标刻画当前安全状态。** 既然全局均衡算不动，本文转而在当前历史 $h_t$ 的局部子博弈里定义近似均衡，用攻击者价值与其上界的差距 $\varepsilon$ 来衡量当前节点离稳健有多远：

$$\bar{v}_1^{(\tau)}(h_t) \leq v_1^{(\tau)}(h_t) + \varepsilon$$

按 $\varepsilon$ 的大小，任意安全状态被精确归入三种体制。**Regime I（防御者错误）**指当前 history 已触发越狱、$v_1^{(\tau)}=1$，此时不等式平凡成立但防御策略本身就是次优的；**Regime II（脆弱安全）**指当前prompt表面安全、但语义邻域里密布漏洞，对应 $\varepsilon$ 很大的结构性不稳定；**Regime III（局部均衡）**指当前安全且整个邻域都已被中和、$\varepsilon$ 可忽略，这正是防御要驱动系统到达的目标状态。三体制把"安全/不安全"的二元判断细化成了带几何含义的分类学。

**4. RRT 搜索适配 prompt 空间：让博弈树的探索在自然语言里变得可行。** 博弈树不可穷举，本文借用机器人路径规划中的 Rapidly-exploring Random Trees，把原本用于连续配置空间的 RRT 迁移到高维自然语言prompt流形上。具体地，$\text{Sample}()$ 生成候选prompt（如角色扮演类话术），$\text{Extend}()$ 在语义最近节点与随机采样之间做插值，从而逐步铺开对prompt空间的覆盖；LLM 本身被当作黑箱 oracle 来裁决每个节点：返回 Safe/Redirect 就继续扩展分支，Reject 就剪枝，Jailbreak 则终止该路径。这样 RRT 既高效采样了脆弱区域，又天然对应博弈树的逐节点展开。

**5. Purple Agent 的红蓝双重机制：在同一棵树上同时完成预判与封堵。** Purple Agent 把红蓝两套逻辑统一进一个系统，奉行"Think Red to Act Blue"。Thinking Red 是探索性推理，用 RRT 模拟攻击者如何生成有害prompt、预判不同查询会怎样滑向风险结果；Acting Blue 是防御性干预，从同一棵 RRT 搜索树读取信息，在检测到对抗机会的节点上直接部署封堵。关键在于它是**单一**系统而非两个独立模块——红色组件负责扩展 RRT 树、蓝色组件负责中和危险分支，二者通过共享的会话历史 $h_t$ 始终同步，使防御能"以红队思维消灭整片危险区域"而非逐案修补单个prompt。

### 损失函数 / 训练策略

本文是推理时防御框架，不涉及模型训练，核心优化目标是把局部子博弈从不稳定的 Regime I/II 迭代驱动到稳健的 Regime III。实现手段有三：对已发现的越狱路径(realized jailbreaks)直接封堵，对 RRT 预判出的模拟威胁(simulated threats)做预防性中和，并在高风险聚类周围创建排除区域，从而把整片脆弱邻域一并收敛到目标均衡。
