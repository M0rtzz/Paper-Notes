...
description: >-
  [ICLR 2026][LLM Agent][mobile agent] FingerTip 20K 收集了 95 名用户在真实日常手机使用中的 21,437 条交互记录（含用户画像、时间、位置、历史意图），提出两个新赛道——主动任务建议（预测用户意图）和个性化任务执行（适配动作偏好）…
tags:
  - "ICLR 2026"
  - "LLM Agent"
  - "mobile agent"
...
### 整体框架
这篇工作不提新模型，而是把"被动执行指令"的移动 GUI Agent 评测，扩展到两个更贴近真实使用的新赛道。要做到这一点，先得有带上下文的长期交互数据：作者让用户在自己手机上录一个月的日常操作，把每次意图连同时间、位置、历史意图一起留下来。基于这批数据，再切出两条评测线——**主动任务建议**让模型在用户没下指令时就猜出他当前想干什么（输入是画像、时间、场景、最多 20 条历史意图和当前的前几张截图），**个性化任务执行**则要求模型不仅完成任务，还要让动作序列贴合该用户的操作习惯。

### 关键设计

**1. 真实用户数据收集：让数据带上时间和上下文，而不是孤立的任务样本。**

现有数据集每条样本都是孤立的、由作者或 LLM 预设的任务，既不反映真实日常使用，也没有可供推断意图的上下文。作者的做法是让 95 名用户在自己的 Android 手机上装一个专用的 FingerTip APP，连续录一个月：每当用户产生一个真实意图，就打开 APP 用一句话把意图写下来，选一下当前的位置类型，再切到目标 app 把操作演示一遍。APP 会自动把这条意图（带时间戳和位置）连同截图序列、UI 树和动作序列一起上传。和在模拟器里照着脚本标注不同，这些数据来自用户真实的手机使用，彼此之间还有时间关联，能还原"通勤时刷新闻、午饭点叫外卖"这种日常模式。

**2. 主动任务建议（Track 1）：在没有指令的情况下预测用户当前想做什么。**

这条赛道针对的痛点是用户得为每个意图写清楚指令、有时甚至说不清自己的潜在需求。模型拿到的输入是用户画像 $U$、时间 $T$、场景 $S$、历史意图 $I_{history}$ 和当前的前几张截图 $O$，要预测出用户当前的意图 $I$，且必须明确点出涉及哪个 app、期望达到什么效果。评估用两个量：预测意图与真实意图的语义相似度 $Sim_1$，以及由 DeepSeek-V3 判定二者是否匹配的成功率 $SR_1$。

**3. 个性化任务执行（Track 2）：完成任务之外，还要贴合这个用户的操作习惯。**

不同用户完成同一任务的路径可能差很多（有人搜 app、有人翻屏找），但现有 Agent 一律用通用方式执行。这条赛道给模型用户画像 $U$、真实意图 $I_{true}$、历史动作序列 $A_{history}$ 和当前截图，要求它在真机环境里一步步执行直到完成任务。除了人工核对的成功率 $SR_2$，个性化程度用比值指标 $Sim_2 = S_I / S_{II}$ 衡量——$S_I$ 是 Agent 动作序列与当前用户历史动作的相似度、$S_{II}$ 是与不同类型用户的相似度，比值越大说明越像目标用户、个性化越强。

**4. 个性化差异的验证：先证明"动作偏好"真实存在且可度量。**

在评测个性化之前，作者得先确认用户之间的动作偏好不是噪声。他们按年龄把用户分组，对相似任务计算同类用户与异类用户之间动作序列的 Levenshtein 相似度。结果呈现清晰的层次——同一用户的相似度 > 同类用户 > 异类用户，说明动作偏好确实存在、并且可以用序列相似度量化，这正是 Track 2 用 $Sim_2$ 衡量个性化的依据。
