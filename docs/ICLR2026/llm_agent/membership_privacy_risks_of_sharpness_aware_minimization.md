...
### 整体框架
FeatureBench 把"评测 Agent 能否开发完整功能"拆成两件事：一是用 **feature 级任务定义**取代 SWE-bench 的单 PR 粒度，让 Agent 根据高层描述和接口签名实现可直接调用的功能模块；二是用一套 **测试驱动的自动收集工具包**从真实开源仓库里反向"挖空"一个完整 feature，自动生成带 F2P（fail-to-pass）和 P2P（pass-to-pass）测试的可验证环境。前者决定任务长什么样，后者决定任务怎么被批量、可靠地造出来。

### 关键设计

**1. Feature 级任务定义与双难度设置：把"修 bug"换成"造功能"**
每个实例不再是一段 PR diff，而是一个面向真实开发的功能模块。给 Agent 的输入包括高层任务描述、函数签名（含输入输出类型）、import 路径、禁止访问的 URL 列表和一个 Dockerfile 执行环境，要求它产出**可直接调用**的解决方案。为了区分两类难度，任务分成 $L_1$（增量开发，在现有代码库上新增功能）和 $L_2$（从头构建，完全从零实现同等功能）——后者剥掉了代码库结构这根"拐杖"，专门考验长 horizon 规划。评估用两个互补指标：严格的 $\text{Resolved Rate} = \frac{\#\text{tasks fully solved}}{\#\text{total tasks}}$ 衡量真正全部解决的比例，而更细粒度的 $\text{Passed Rate} = \frac{1}{N}\sum_{i=1}^{N}\frac{\#\text{F2P tests passed}_i}{\#\text{F2P tests total}_i}$ 按通过的 F2P 测试点占比打分，能捕捉"写对了一半"的部分进展。

**2. 依赖图构建与 top-level 对象识别：先找准这个 feature 到底是哪段代码**
要"挖"出一个 feature，先得知道它由哪些函数构成。工具包在执行 F2P 和 P2P 测试时，借 Python 内置的 tracing 设施捕获函数调用事件，把每次调用串成一张对象依赖图（节点是函数，并打上是否来自 P2P 的标记）。但测试覆盖的函数里混着"被直接测的目标功能"和一堆辅助工具函数，于是再用 LLM 在测试文件中区分出 **top-level tested objects**——这一步分类的 F1 达 84.94%、准确率 91.74%，作为后续遍历的可靠起点。

**3. BFS 遍历与 P2P 保护的特征隔离：挖功能时不能砸坏邻居**
以 top-level 对象为根做广度优先遍历，沿依赖图把这个 feature 牵连到的所有函数逐层展开。关键在于节点分类规则：凡在 P2P 测试中出现过的节点标为 **remained**（属于其他已有功能，必须保留），只在 F2P 路径上出现的标为 **extracted**（属于待提取的目标 feature）。这条 P2P 保护机制保证了挖走目标功能后不会误伤代码库里别的能力，也正是它区别于只看 F2P 的 SWE-Flow 之处。

**4. 代码提取、后验证与任务描述自动生成：造出"功能缺失版"并自检质量**
把所有 extracted 节点对应的代码从仓库中移除，就得到一份"功能缺失版"代码库加一个功能补丁。为防止造出来的任务有 bug，工具包做双向后验证：(a) 缺失版代码库必须通过全部 P2P 测试、同时通不过任何 F2P 测试（说明该功能确实被干净地挖掉了）；(b) 把补丁还原后所有测试都通过（说明 gold 方案确实可解）。最后从函数 docstring 或 LLM 生成功能描述，配上接口签名，自动拼成完整的 problem statement——整条流水线把人工干预压到每仓库约 3 分钟。

**5. Full / Lite 双套基准配置：兼顾覆盖度与评测成本**
