...
整套方法围绕一个 Web-CogKnowledge Framework 展开，它把"教 Agent 什么"和"教 Agent 怎么用"拆成两条线：一条是层次化的知识分类体系 Web-CogKnowledge 与据此从 14 个真实网站爬取构建的 Web-CogDataset，另一条是评估知识掌握度与认知能力的 Web-CogBench 基准，三者合起来支撑出最终模型 Web-CogReasoner。Agent 与网页的交互被建模为一个 POMDP $P = (S, A, O, K, T, R)$，其中显式引入知识集合 $K$；每一步 Agent 接收截图和 Accessibility Tree 作为观测 $O$，先生成一段推理思路 $h_t$，再据此选出动作 $a_t$。

### 关键设计

**1. 三层知识分类：把"通用预训练知识打不过专门任务"的瓶颈拆成可逐层教学的对象。** 借鉴 Bloom 教育分类学，论文把 Web Agent 需要的知识从底到顶分成事实性、概念性、程序性三层，分别对应记忆、理解、探索三种认知能力，这样原本笼统的"网页知识"就变成了三类目标明确、可分别采样训练的样本。三层的定义与典型任务如下：
...
这个分层不是命名游戏，后面的消融会显示低层是高层的前提，跳层训练会失败，所以三层既是分类也是训练顺序。

**2. Web-CogDataset：让三层知识落到真实网页上而非凭空标注。** 为了给三层知识喂真实语料，论文从 14 个代表性网站选择性爬取元数据，按三层知识设计出 12 种精细任务，覆盖从感知元素、理解组件功能到执行目标序列的完整链路。每条样本都被明确归到某一知识层，从而能在后面的课程学习里按层取用，保证训练信号和知识分类一一对应。

**3. Knowledge-driven CoT（KCoT）：把三层知识塞进推理链，让模型在决策时按层调用知识而不是直接拍动作。** 推理被组织成 $\text{Task Prompt} \rightarrow \text{Knowledge-driven CoT} \rightarrow \text{Plan} \rightarrow \text{Action}$ 的链条，其中 KCoT 这一步又显式拆成三问：事实层先问"页面上有什么"以识别元素和状态，概念层再问"这意味着什么"以推断组件角色和交互关系，程序层最后问"如何完成任务"以规划目标导向的步骤。这种结构让模型在每一步动作前都按事实→概念→程序的顺序走一遍已学知识，而不是把知识当背景噪声；其重要性后面有硬数据支撑——完整三层知识若不配 KCoT，在线成功率仅 25.35%，加上 KCoT 直接升到 42.9%。
