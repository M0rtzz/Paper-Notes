...
T³ 把主动推理建模为 POMDP，用一个真值锚定的势函数刻画"信念离真实状态有多远"，先从理论上证明 LLM 智能体的信念会在有限步后落入无法自拔的"信念陷阱区域"并污染 RL 的信用分配，再用一个可观测的代理信号在陷阱入口处截断轨迹尾部。整套机制做成元包装器，套在 PPO/GRPO/GSPO 等任意策略优化算法外层即可生效。

### 关键设计

**1. POMDP 建模与真值锚定势函数：把"任务进展"变成可度量的量。** 主动推理的难点在于智能体看不到真实状态 $s^*$，只能靠多轮提问逼近它，因此需要一个能衡量"当前信念距离真相多远"的标尺。论文将任务形式化为 POMDP $(S, A, O, T, R, \gamma)$，智能体维护信念状态 $b_t \in \Delta(S)$，据此选动作 $a_t$、收观测 $o_t$ 并更新信念。关键是引入势函数 $\Psi(b) = -\log b(s^*)$：它等于真实状态在当前信念下的负对数概率，$\Psi = 0$ 意味着完全锁定真相、任务完成，值越大说明信念越偏离 $s^*$。这把抽象的"推理进展"压成一个单调可比的实数，后续所有理论分析都围绕 $\Psi$ 的逐步变化展开。

**2. 信念陷阱区域（BTR）：刻画"越陷越深"的吸收态。** 经典 POMDP 假设贝叶斯信念更新是完美的，但 LLM 受限于推理能力，每一步更新都带误差，且这些误差会累积。论文用假设 1（更新误差增长）把这一直觉形式化：存在常数 $m_\theta > 0$，使得在高不确定性区域 LLM 的信念更新误差至少线性增长于当前偏差本身——偏差越大、校正越难，构成正反馈放大。在此基础上定理 1 证明，在非退化观测和 Lipschitz 策略等条件下，信念轨迹会在有限步后进入一个吸收区域 BTR，区域内期望进展不再变好甚至恶化，即 $\mathbb{E}[\Psi_{t+1} \mid b_t] \geq \Psi_t$。一旦落入，智能体只会输出冗余、重复或无信息的动作而无力翻盘，这正对应实践中常见的"智能体绕圈"现象。

**3. 信用分配失败机制：解释为什么尾部会反噬早期探索。** 落入 BTR 本身只是浪费 token，真正致命的是它对 RL 训练的污染。定理 2 指出，进入 BTR 后的无信息尾部会扭曲早期探索动作的广义优势估计（GAE）：尾部贡献持续的负漂移，当尾部足够长时这股负漂移会主导早期有价值动作的正贡献，使其优势估计被压成负值，梯度方向因此反转——本该被鼓励的探索反而遭到惩罚。推论 1 随即给出解药：在 BTR 入口处截断轨迹，剔除无信息尾部，就能得到偏差更小、方向更正确的梯度估计。这条"理论病因→治疗方案"的链条是 T³ 的核心立论。

**4. T³ 截断条件与任务实例化：用可观测代理触发截断。** BTR 由 $\Psi$ 定义，但 $s^*$ 训练时不可见，无法直接判断何时入陷，因此需要一个可观测的代理信号。定义 2 给出 T³ 条件：若在窗口 $[t-k, t)$ 内假设空间的精炼度量始终满足 $d(H_\tau, H_{\tau+1}) \leq \Delta_{\min}$（即连续 $k$ 步几乎没有缩小候选假设），则在步骤 $t$ 截断。$H_t$ 表示与历史一致的候选假设集合，$d$ 衡量一步问答带来的信息增益。论文按任务给出具体实例：GuessNumbers 取 $d = |H_\tau| - |H_{\tau+1}|$，猜测一旦超出候选集（$k=1$）即截断；CircuitDecoding 同理，候选集连续 $k=3$ 步不缩减则截断；SituationPuzzles 以评判者反馈"unknown"作为未精炼代理，连续 $k=5$ 步触发；PreferenceEstimation 与 MovieRecommendation 监控估计向量与真实偏好的相似度，连续 $k=2$ 步下降则截断。代理信号虽因任务而异，但都对应"近期问答不再带来信息"这一统一判据。

**5. 即插即用的元包装器：零侵入接入现有 RL。** T³ 不改动任何底层优化逻辑，只是在轨迹采样阶段按上述条件截断 rollout，再把截断后的轨迹交给原算法做优势估计与更新。因此它作为元包装器（meta-wrapper）可无缝套在 PPO、GRPO、GSPO 之上，无需重写损失或网络结构。这一设计既保证了与主流 RL 流程的兼容性，也让"截断有信息前缀、丢弃无信息尾部"的收益能直接转化为更干净的信用分配和更省 token 的训练。
