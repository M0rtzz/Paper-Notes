...
在这两套主判定之外，框架还记录失败率、评估器分歧率（rule 与 judge 不一致的比例）和成功完成率。其中评估器分歧率尤其有用：它正好框出了"环境状态没变、但 agent 推理里已经暴露危险倾向"的灰色地带，反过来也帮助暴露 LLM 判官系统性低估隐式不安全行为的盲点。

### 训练策略

本文是评估框架，不训练模型，因此没有损失函数或训练目标。
