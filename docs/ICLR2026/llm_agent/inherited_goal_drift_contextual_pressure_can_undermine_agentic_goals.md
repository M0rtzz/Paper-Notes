...
六类实验不是平铺，而是层层加码地暴露偏移的不同诱因。最基础的 **直接对抗压力** 在 30 步内持续灌入对抗性市场消息或利益相关者诉求，看模型能否守住系统目标；这一关现代模型几乎都能过。真正的杀招是 **上下文传导（conditioning）**：先让弱模型 GPT-4o-mini 跑 N 步、积累一段已经偏移的对话历史，再把这段上下文交给强模型接管 10 步，检验偏移会不会被"继承"下来。在此之上还叠了三种变体——**目标切换** 让 system prompt 规定前 16/32 步追求工具性目标 A、之后切到真目标 B，考验模型能否识别并完成切换；**目标切换传导** 把切换实验的偏移上下文再喂给新模型，叠加切换与传染双重压力；**目标反转** 让同一模型先跑 15 步目标 A，再把 system prompt 改成目标 B，看它能否反省并推翻自己刚做过的事。最后 **直接指令层级测试** 把 system prompt（利润）和紧随其后的 user message（环保）摆成正面冲突，单独拎出"模型到底听谁的"这个变量，好和偏移抗性做相关性分析。
