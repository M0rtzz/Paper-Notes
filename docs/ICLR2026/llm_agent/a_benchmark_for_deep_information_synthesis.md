...
### 整体框架
这是一篇 benchmark 论文，不提新方法，核心贡献是 DeepSynth 这套数据集本身——120 个任务的设计、专家标注流程和对现有 agent 的全面评估。每个任务的构成是固定的：一道问题（平均 78.5 tokens）、一串金标准的中间推理步骤（平均 7.54 步）、支撑这些推理的证据 URL，以及一个 JSON 格式的标准答案。整体逻辑可以理解为：先让专家在真实数据源上做一遍"深度研究"得出洞见，再把这个研究过程逆向封装成一道题，最后用多粒度指标去考各家 agent 能不能复现出同样的答案。

### 关键设计

**1. 四阶段数据构建流程：让答案"搜不到、只能综合出来"。**

现有 benchmark 多是"先有答案再编问题"，结果题目往往一次检索就能命中。本文反着来：16 位专家先提出 223 个跨 7 领域的真实数据源，再为每个数据源提出可验证的假设，动手分析后得出洞见，最后才把这条分析链逆向转化成问题。四个阶段依次是数据源识别 → 假设生成 → 假设验证 → 任务表述。这样设计的关键在于答案无法通过直接搜索得到——它藏在多步分析的终点，agent 必须真的跨源收集再推理才能复现，从源头上保证了任务的真实性和抗记忆/抗污染性。

**2. 多维评估指标：用三个松紧不同的尺子量同一份答案。**

任务输出统一为 JSON，天然可自动验证，但单一指标会失真，所以本文叠了三层。EM（Exact Match）最严，要求所有 key-value 对全部正确；F1 在 key-value pair 粒度上评估部分正确性，宽容一点；LLM-Judge 最松，允许语义等价和小幅数值偏差（1–5.5% 的 margin）。三个尺子从严到松排开，既能看出 agent 是否真正精确，也能在它"方向对但细节差"时给出可区分的分数，避免被 EM 一刀切到 0 而看不出梯度。

**3. 任务多样性设计：在地域、领域、分析操作三个维度上撑开覆盖面。**

为了不让 benchmark 偏向英语或西方语境，任务横跨 67 个国家、覆盖 7 个领域（社会经济、金融、环境、科学、教育、交通、政治）。在分析操作类型上也刻意做了分布：计数比较 34%、趋势检测 21%、排名 20%、相关性分析 7%、异常检测 7%。这种铺开既检验 agent 在 under-represented 数据源上的鲁棒性，也保证评估的不是单一一种推理套路。

### 损失函数 / 训练策略
N/A（benchmark 论文，不涉及训练）。
