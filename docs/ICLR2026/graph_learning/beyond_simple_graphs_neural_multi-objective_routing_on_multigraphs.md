...
标准 Transformer 把信息挂在节点上，遇到同一对节点间的多条平行边就糊成一团，根本区分不开。GREAT 的做法是直接在边上操作：每层先把入边和出边分别加权聚合到节点，得到临时节点表示 $x_i = (\sum_{l \in E^+(i)} \alpha'_{il} W'_1 e_l \,\|\, \sum_{l' \in E^-(i)} \alpha''_{il'} W''_1 e_{l'})$，其中 $\alpha'$、$\alpha''$ 是入边、出边各自的注意力分数；再把一条边首尾两个节点的表示拼起来更新这条边的嵌入 $e'_l = W_2(x_{\text{start}(l)} \,\|\, x_{\text{end}(l)})$。关键是残差连接——它让每条平行边即使共享首尾节点，也能保留自己独立的费用特征，不被聚合抹平。两个 GMS 变体都用它打底。

**2. GMS-EB：直接在多重图上一条边一条边地构造路径。**

最直接的思路是让解码器每步选的不是"下一个节点"，而是"下一条边"。编码器输出边嵌入后，边级 Multi-Pointer 解码器逐步采样 $\pi_t$，概率为 $p_{\theta(\lambda)}(\pi_t \mid \pi_{1:t-1}, s)$；偏好向量 $\lambda$ 不进编码器，而是通过 MLP 超网络生成解码器权重 $\theta_{\text{dec}}(\lambda) = \text{MLP}(\lambda)$，于是编码一次就能服务所有偏好。这样选边和定序端到端一起学，简单且表达力强，代价是内存复杂度涨到 $\mathcal{O}(MN^4)$（节点级只要 $\mathcal{O}(N^3)$），规模一大就吃不消。

**3. GMS-DH：把"选边"和"排路线"拆成两个头，换计算效率。**

为了避开 EB 的复杂度，DH 把问题切成两阶段。第一阶段是**非自回归剪枝**：在中间某层 GREAT 之后，选择解码器对每对节点独立地保留一条边，概率 $q_{\tilde{\theta}(\lambda)}(l \mid i, j, s)$，相当于学着把多重图压成一张简单图。第二阶段是**自回归路由**：把剪枝后的边嵌入聚合成节点嵌入，再过几层 Transformer，然后用偏好条件化的 Multi-Pointer 解码器做节点级路由。两个头的权重都由 MLP 超网络从 $\lambda$ 生成，而编码器主体与偏好无关、整个实例只跑一次，多偏好推理共享同一份编码——这正是 DH 比 EB 快好几倍的来源：路由在简化后的简单图上做，节点级解码器复杂度更低。
