...
### 整体框架
EDT-Former在一个冻结的图编码器和一个冻结的LLM之间插入一个轻量连接器：图编码器把分子图编码成节点级表征，连接器根据信息熵把这些节点表征压缩成数量和内容都随分子自适应变化的token序列，再连同文本指令一起喂给LLM产生输出。整个训练只更新连接器和LLM的嵌入层，图编码器与LLM主干始终冻结。

### 关键设计

**1. 熵引导的动态Token生成：让token数量随分子复杂度自适应**

Q-Former式方案用固定数量的静态查询去压缩所有分子，简单分子被过度表示、复杂分子信息不足，这是静态token的根本痛点。EDT-Former改成由信息熵来驱动token的分配——先在图编码器输出的节点表征上估计信息熵分布 $H(v)$，熵值高的节点意味着信息丰富、不确定性大的分子片段，再依据熵分布把分子图切成若干个"分子补丁"（molecular patches），每个补丁对应一段信息密集的子结构。

生成token时，高熵补丁分配更多token、低熵补丁分配更少甚至共享一个token，于是复杂分子自动得到更长的token序列、简单分子得到更短的序列。这种内容感知的长度自适应既避免了简单分子上的冗余计算，又保证了复杂分子上的信息不被固定长度截断，把计算效率和表示质量同时往上抬。

**2. 分子补丁对齐：让每个token都落在化学上有意义的子结构上**

光有动态数量还不够，token还得对准真正承载化学语义的部分。这里借用了ViT的直觉：图像补丁是空间上连续的像素块，那么分子补丁就应该是拓扑上连续、化学上成团的原子集合（如一个功能基团）。EDT-Former让生成的每个动态token与一个信息丰富的分子补丁对齐，使token在保留局部子结构特征（功能基团、立体化学）的同时，又能通过注意力把全局结构信息（分子骨架拓扑）整合进来。正是这种"局部成团 + 全局注意力"的对齐，弥补了固定token方法忽略拓扑和子结构的缺陷。

**3. 冻结主干 + 轻量连接器的高效对齐：不动LLM也能对齐**

为了避免昂贵的LLM全量微调，EDT-Former把可训练参数压到最小：图编码器完全冻结以保留预训练的通用分子表征，LLM主干完全冻结以保留语言理解与生成能力，只训练EDT-Former连接器和LLM的嵌入层。嵌入层虽然参数量很小，却恰好充当了把动态token映射进LLM输入空间的"接口适配器"，因此即便主干不动，也能把分子token对齐到语言空间。训练目标同时包含对齐损失（把token表征拉向LLM嵌入空间）和任务损失（问答准确率、属性预测误差），让对齐和下游表现一起优化。

### 损失函数 / 训练策略
训练分两阶段：先做图-文本对齐预训练，用对比学习损失拉近匹配的分子-文本对、推远不匹配的对，使token表征与文本嵌入空间对齐；再在下游任务上用生成损失（问答用交叉熵）微调。两个阶段始终只更新连接器和嵌入层，可训练参数量远小于全量微调。
