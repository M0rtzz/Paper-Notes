...
GRAPHITE 把"治异质图"的战场从 GNN 架构搬回到图结构本身：先把原图无监督地变换成一个同质性更高的增广图，再在这个新图上跑一个区分边类型的标准 GNN。整个变换是确定性的、无需学习的预处理，可以挂在任意 GNN 前面即插即用。

### 关键设计

**1. 朴素同质性提升器（NHB）：先证"加边能提同质"，再暴露它为什么不可行。** 同质性的定义说共享特征的节点更该相邻，于是最直接的想法是：对每对至少共享一个特征的节点 $(v_i, v_j)$ 都补一条边。Theorem 1 在温和假设下证明这确实严格提升同质性 $\text{hom}(\mathcal{G}^\dagger) > \text{hom}(\mathcal{G})$，从理论上确认了"改结构"这条路走得通。但代价是新增边数高达 $O(|V|^2)$——一个 2000 节点的图就可能塞进近 200 万条边，训练根本扛不住。NHB 因此只作为动机存在，它告诉我们目标对，但实现方式必须换。

**2. 特征节点 hub 变换：用间接连接把 $O(|V|^2)$ 压成线性。** GRAPHITE 的核心招数是不再让共享特征的节点两两直连，而是为每个特征 $k$ 建一个"特征节点" $x_k$（共 $|\mathcal{X}|$ 个），凡是带有特征 $k$ 的图节点 $v_i$（即 $\mathbf{X}[v_i,k]=1$）就连一条边 $(v_i, x_k)$。这样变换后图的邻接矩阵成了分块形式 $\mathbf{A}^* = \begin{bmatrix} \mathbf{A} & \mathbf{X} \\ \mathbf{X}^T & \mathbf{0} \end{bmatrix}$，特征节点自身的特征取其所连图节点特征的均值。关键在于：任意共享特征 $k$ 的节点对都能经 $v_i \to x_k \to v_j$ 在 2 跳内连通（Observation 2），等价地恢复了 NHB 想要的同质消息传递，却把原本 $O(|V|^2)$ 的边数收敛到只随特征出现次数线性增长。Theorem 3 进一步保证它鱼与熊掌兼得——既有 $\text{hom}(\mathcal{G}^*) > \text{hom}(\mathcal{G})$ 的同质性提升，又满足 $|\mathcal{V}^*| \leq O(|\mathcal{V}|)$、$|\mathcal{E}^*| \leq O(|\mathcal{E}|)$ 的线性规模，从根上解决了 NHB 的计算瓶颈。

**3. 区分边类型的专用 GNN：让模型知道"图边"和"特征边"不是一回事。** 变换后的图里有两类语义截然不同的边：原始图边承载拓扑邻接，新加的特征边承载属性共享，一视同仁地聚合会浪费结构信息。GRAPHITE 因此给三类连接分配可调权重——图边权 $w_\mathcal{E}=1$、特征边权 $w_\mathcal{X}$、自环权 $w_0$，让模型自己学到该多信哪一类。聚合时借鉴 FAGCN 的 self-gating 机制 $\alpha_{u,u'} = \tanh\!\big(\frac{\mathbf{a}^T(\mathbf{h}_u \| \mathbf{h}_{u'}) + b}{\tau}\big)$，对每条边自适应判定是吸纳还是排斥邻居信号，从而既利用同质边又能抑制残留的异质边；聚合后再过一个带残差连接的 MLP 加 GELU 激活稳定训练。这套设计让"假阳性同质连接"（共享特征但标签不同的节点对）的负面影响被门控住，而真正同质的特征边得到放大。

### 损失函数 / 训练策略

图变换本身是无需学习的确定性预处理，不引入任何额外参数，可与任意现有 GNN 解耦组合；下游节点分类直接用标准交叉熵损失训练即可。
