...
### 整体框架
WPR 的思路是把标准 RLHF 目标里那项"逐索引比较概率"的 KL 正则化换成熵正则化 Wasserstein 距离，让正则化能感知 token 之间的语义远近。难点在于 Wasserstein 距离本身要解线性规划、无法塞进 PPO，论文通过 Sinkhorn 对偶把它化简成一个 token 级的奖励惩罚项，再配上两道截断使额外开销几乎可忽略，从而无缝接入现有 RLHF 流水线。

### 关键设计

**1. Wasserstein 策略正则化目标：让正则化看见 token 的语义几何**

标准 RLHF 用 KL 把策略拉回参考模型，但 KL 只在相同索引上比概率，无法区分"把概率从 cat 挪到 kitten"和"挪到 table"。WPR 直接把目标中逐 token 的 KL 项替换为熵正则化 Wasserstein 距离 $D_{\tilde{W}}$，优化目标变为 $\max_{\pi_\theta} \mathbb{E}[\sum_n R(\mathbf{x}, \mathbf{y}_{1:n}) - \beta \sum_n D_{\tilde{W}}(\pi_\theta(y_n|\cdot) \| \pi_{ref}(y_n|\cdot))]$。这里的关键是代价矩阵 $C$——它取参考策略 token embedding 空间里的欧氏距离，于是"语义相近的 token 之间搬运概率代价小、语义无关则代价大"这一几何结构被直接编码进了正则化，KL 的盲点也就被填上了。

**2. 对偶公式与 PPO 兼容：把搬土问题变回一个奖励惩罚**

直接算 Wasserstein 距离要解线性规划，复杂度约 $O(d^3)$，对动辄 256K 的词表完全不可行，而且它也不像 KL 那样能写成 token 级惩罚塞进 PPO。论文引入熵正则化得到 Sinkhorn 距离的对偶形式，并证明（Theorem 2）最优对偶变量 $\phi^*$ 恰好可以充当 token 级的奖励惩罚项：$\mathcal{J}_{\tilde{W}}(\pi_\theta) = \mathbb{E}[\sum_n \mathbb{E}_{y_n}[R(\mathbf{x}, \mathbf{y}_{1:n}) - \beta \phi^*_{y_n}]] + \mathcal{C}$。这一步是整套方法落地的支点：训练时只需把原来的 KL 惩罚换成 $\phi^*$，PPO 的其余部分原封不动，工程改动极小。

**3. 双重截断：把 $O(d^2)$ 的词表计算压回常数量级**

即便有了对偶形式，对整个词表算代价矩阵仍是 $O(d^2)$，在 256K 词表上吃不消。WPR 用两道截断把规模压下来：一是 Nearest-$k_1$ 截断，把核矩阵 $K = \exp(-\lambda C)$ 只保留每个 token 的 $k_1=512$ 个近邻并稀疏存储，因为远距离 token 之间几乎不会发生概率搬运、保留它们没意义；二是 Top-$k_2$ 截断，把策略分布只截到 top-$k_2=128$ 个 token，使有效支持规模从 $d$ 降到 $2k_2+2$。两道截断叠加后，相比纯 KL 正则化，每千步的额外计算开销仅增加约 2.5%、显存约多 15GB（A100），实践中近乎免费。消融显示 $k_2$ 收得太狠（如 64）会明显掉点，说明截断尺度需要留足够的有效支持。

**4. Sinkhorn-Knopp 求解对偶变量：少量迭代换来稳定收敛**

剩下的问题是怎么算出对偶变量 $\phi^*$。WPR 用经典的 Sinkhorn-Knopp 迭代求解，它在熵正则化下交替归一化、能高效逼近最优传输的对偶解。实践中通常 10 次迭代即收敛，tolerance 取 $10^{-4}$。迭代次数是个敏感旋钮：消融里迭代降到 5 次会因收敛不充分导致性能严重下降，而加到 30 次又没有额外收益，10 次正好落在性价比拐点上。
