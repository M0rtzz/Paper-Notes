...
前两个实验是从投影行为间接推断，第三个实验则直接量化几何关系。对有用、有害、安全三种更新各自做 SVD，按能量保留比例 $\eta$ 取对应的 top-k 方向，再计算任意两类之间的重叠度：

$$\mathrm{MSO}(\mathbf{V}, \mathbf{W}; \eta) = \frac{\|S\|_F^2}{\min(k_V, k_W)}$$

其中 $S$ 是两组方向之间投影的奇异值矩阵，MSO 值域为 [0,1]，0 表示完全正交、1 表示子空间相同。如果"安全子空间"对有害行为有特殊几何关系，有害-安全这一对的 MSO 应当显著高于其它配对。实测结果相反：有害-安全的重叠度从来不是最高的，有时甚至是最低的。安全更新与有害行为之间并不存在可被利用的特殊几何结构。
