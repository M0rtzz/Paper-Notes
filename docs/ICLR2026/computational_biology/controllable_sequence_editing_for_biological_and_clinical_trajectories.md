...
description: >-
  [ICLR 2026][计算生物][反事实生成] 提出 Clef，一个基于"时间概念"（temporal concepts）的可控序列编辑模型，能够在给定条件（如药物、手术）下对生物/临床多变量轨迹进行即时和延迟编辑，在细胞重编程和患者实验室检测数据上，即时编辑 MAE 提升 16.28%…
tags:
  - "ICLR 2026"
  - "计算生物"
  - "反事实生成"
...
Clef 把"在条件 $s$ 作用下、对多变量序列 $\mathbf{x}_{:,t_0:t_i}$ 做编辑"这件事，拆成两步：先用一个序列编码器和一个条件适配器分别把历史轨迹和干预条件编码成向量，再让一个概念编码器把二者融合成"时间概念"$\mathbf{c}$，最后概念解码器把 $\mathbf{c}$ 当作变化倍率直接乘回当前状态，一步得到反事实序列 $\hat{\mathbf{x}}_{:,t_j}^s$。它支持两种编辑：**即时编辑**只预测下一步 $\hat{\mathbf{x}}_{:,t_{i+1}}$，**延迟编辑**则给定在未来某步 $t_j \geq t_{i+1}$ 发生的条件，直接跳过中间步一步预测 $\hat{\mathbf{x}}_{:,t_j}$——这正是只能逐 token 推进的可控文本生成方法做不到的。

### 关键设计

**1. 时间概念：用"变化率"而非"绝对值"来承载条件影响。** 整个模型的灵魂在于对编辑目标的重新定义。Clef 不直接回归未来的绝对数值，而是把目标设为两个时间步之间的变化率 $\mathbf{c} = \mathbf{x}_{:,t_k} / \mathbf{x}_{:,t_j}$，即每个变量的增长/衰减因子。这样做的好处是局部性：条件干预只需改变少数几个变量的变化率，其余变量的概念值接近 1，对应数据"原样保持不变"，天然满足了"干预只影响部分变量、其余历史维持因果一致"的需求。解码时只需把概念逐元素乘回当前状态：$\hat{\mathbf{x}}_{:,t_j}^s = \mathbf{c} \odot \mathbf{x}_{:,t_i}$。这个极简的乘法让每个概念维度都对应一个变量的可解释变化率，也使用户能直接编辑某一维（如把葡萄糖相关概念减半）来生成反事实。

**2. 序列编码与时间差嵌入：把"何时干预"也编码进去。** 序列编码器 $F$ 从历史轨迹提取时间特征 $\mathbf{h}_x = F(\mathbf{x}_{:,t_0:t_i})$，它对编码器结构无要求，可换用 Transformer、xLSTM 乃至 MOMENT 等预训练时序基础模型，因此 Clef 实际上是一个"可控编辑增强模块"。延迟编辑的关键在于让模型知道目标时间点离当前有多远：时间编码器用年/月/日/时的正弦嵌入得到位置编码 $\mathbf{h}_t$，再以时间差 $\Delta_{t_i,t_j} = \mathbf{h}_{t_j} - \mathbf{h}_{t_i}$ 显式表示跨度。有了这个跨度信号，模型才能一步跳到任意未来时刻而不必逐步填补。

**3. 条件适配器：复用冻结的领域预训练嵌入。** 干预条件 $s$（转录因子、医疗代码等）先经冻结的预训练模型映射成嵌入 $\mathbf{z}_s$——细胞实验用 ESM-2 蛋白质语言模型，患者数据用临床知识图谱嵌入——再由一个线性层 $H$ 投影成隐藏表示 $\mathbf{h}_s = H(\mathbf{z}_s)$。借用现成的领域语义，使得即便面对训练时未见过的条件，也能从嵌入空间的相近性中获得泛化能力，这也是零样本反事实生成得以工作的基础。

**4. 概念编码器：融合轨迹、条件与时间跨度生成 $\mathbf{c}$。** 这是把前三者拧到一起的环节。先将时间差嵌入与条件嵌入拼接成联合嵌入 $\mathbf{h}_s^{t_j} = \Delta_{t_i,t_j} \oplus \mathbf{h}_s$，再与序列特征逐元素相乘做交互，最后经可选的 FFN 与 GELU 激活生成时间概念：
