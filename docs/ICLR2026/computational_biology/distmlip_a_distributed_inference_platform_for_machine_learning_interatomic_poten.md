...
DistMLIP 是一个面向 MLIP 的分布式推理平台，核心是把整张原子图按空间垂直切成若干子图分配到不同 GPU，让每块卡只负责自己那部分原子。它不像 LAMMPS 那样靠复制"幽灵原子"来对齐边界，而是在每层 GNN 前向后只交换边界节点的更新特征，从而做到零冗余计算，并同时支持原子图与三体键图（bond graph）、兼容保守力与直接力两类预训练势。

### 关键设计

**1. 垂直空间分区：用最粗暴的切法换最低的分区开销。**

DistMLIP 沿晶胞最长维度做"墙式"垂直切割，把系统切成 $G_1,\dots,G_P$ 共 $P$ 个分区。每个分区 $G_i$ 再扩展成 $G_i'$，纳入所有 1-hop 邻居，并把节点标成三类：PURE（纯本地节点，最终只对它们算力和能量）、TO（要发给相邻分区的边界节点）、FROM（要从相邻分区接收的节点）。三类节点在特征张量里用 marker 数组划出连续区间，通信时按区间整段收发，避免逐节点索引。看起来这种切法很粗糙，但它的好处恰恰在于"便宜"——分区是每个 MD 步都要重做的操作，复杂分区算法（如 METIS）的图划分开销会被反复摊到每一步上，实测反而比垂直分区慢达 8 倍。

**2. 零冗余的消息传递并行：每一次计算结果都不浪费。**

传统空间分区为了让边界原子拿到完整邻居，必须在每块卡上复制一圈"幽灵原子"并对它们做重复的前向计算，丢弃后再算下一层；幽灵原子的数量随交互半径近似立方增长，对多层、长程的 GNN-MLIP 代价极高。DistMLIP 把这套逻辑换成图并行：每块 GPU 只对自己的 PURE 节点算力和能量，边界节点的中间特征算完后通过 `all-to-all` 通信发给需要的分区直接复用，不存在被丢弃的冗余计算。其直接收益是并行推理时间随交互范围只线性增长，而非立方增长，这正是长程 MLIP 最吃亏的地方被补上了。

**3. 三体键图的分布式构建：让依赖键角的模型也能多卡跑。**

CHGNet 等高精度 MLIP 靠 bond graph（line graph）编码三体交互来刻画键角，而正确构建一个分区的 bond graph 需要纳入其 PURE 节点的 2-hop 邻居（比原子图多一跳）。DistMLIP 为此建立 edge table，把每个节点映射到以它为源的边集合，再递归遍历这些边拼出本分区的并行 bond graph。此前没有任何工作给出 bond graph 的分布式实现，这一步是让三体类 MLIP 上多 GPU 的关键缺口。

**4. 模型无关的即插即用接口：不绑定 LAMMPS。**

图创建代码用纯 C 实现，不依赖 PyTorch / JAX / LAMMPS 等外部库，图在 CPU 内存里构建、GPU 只负责 GNN 前向传播。平台已适配 MACE、CHGNet、TensorNet、eSEN 四种主流架构，新模型只需少量适配即可接入。因为大多数 MLIP 并没有成熟的 LAMMPS 接口、很多工作流也不走 LAMMPS，这种独立于模拟引擎的设计大幅降低了多卡推理的接入门槛。
