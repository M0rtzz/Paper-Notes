...
description: >-
  [ICLR2026][计算生物][蛋白质语言模型] 首次系统性研究蛋白质语言模型（PLM）中的病态重复问题，提出统一的重复度量指标 $R(x)$ 和效用指标 $U(x)$，并设计 UCCS（Utility-Controlled Contrastive Steering）方法，通过在隐层注入与重复解耦的引导向量…
tags:
  - "ICLR2026"
  - "计算生物"
  - "蛋白质语言模型"
...
### 整体框架
UCCS 把"抑制重复但别伤折叠"形式化为一个约束优化问题 $\min_f R(f(M,p))$ s.t. $U(f(M,p)) \ge U(M,p) - \epsilon$，即在效用 $U(x)$ 不跌超过容差 $\epsilon$ 的前提下最小化重复度 $R(x)$。为此先建立能区分重复与效用的两套度量，再用一对"重复有别、效用对齐"的对比序列集从隐层激活里抽出一个只编码重复的引导向量，推理时把它加回激活即可，全程不动模型参数。

### 关键设计

**1. 统一重复度量：把蛋白质特有的退化模式量化成一个可优化的标量。** 文本里的重复指标无法捕获蛋白质特有的两种坍塌——motif 级循环（如 AGAGAG 的短片段重复）和同聚物延伸（如 AAAAAA 的单氨基酸长链），所以作者用三个互补信号刻画它们。归一化 token 熵 $H_{\text{norm}}$ 反映全局氨基酸分布是否失衡，Distinct-2/3 捕获局部 motif 循环，同聚物多样性分数 $R_{\text{hpoly}} = 1 - \frac{1}{T}\sum_i \ell_i \cdot \mathbf{1}(\ell_i \ge 4)$ 专门惩罚长度 $\ell_i \ge 4$ 的同聚物坍塌（阈值 $k=4$ 来自生物学观察——≥4 的同聚物几乎总是低复杂度不稳定区）。三者聚合成统一重复分数 $R(x)$；与之配对的效用分数 $U(x)$ 取 AlphaFold 的 pLDDT 与 pTM 均值，代表折叠可信度。有了这两把尺，"重复"和"效用"才能被分别测量、进而被解耦。

**2. 效用控制的对比数据集：让引导向量只学到重复、不混进折叠能力。** 如果直接拿高重复和低重复序列做对比，抽出的方向会把"折叠好不好"也一起编码进去，结果是降重复的同时连结构也压垮了。作者的做法是从天然蛋白（CATH / SCOP / UniRef50）和 PLM 生成序列收集候选池，按长度分桶后先剔除 $U(x)$ 明显偏离参考均值的序列，再求解 $\arg\max_{\mathcal{D}^+,\mathcal{D}^-} \Delta R$ s.t. $\Delta U \le \epsilon$，挑出一对在重复维度差异最大、却在效用维度对齐的子集 $\mathcal{D}^+$（低重复）与 $\mathcal{D}^-$（高重复）。这一步是整套方法解耦成功的关键：对比集只剩重复这一个变量在变。

**3. 引导向量提取与注入：一次抽取、即插即用地干预生成。** 拿到对比集后，对每条序列取层级表示 $\phi^L(x)$——MLM 用均值池化、AR-LM 用末 token 嵌入以适配两种范式——再算两组均值之差 $v^L = \mathbb{E}_{\mathcal{D}^+}[\phi^L] - \mathbb{E}_{\mathcal{D}^-}[\phi^L]$，得到指向"低重复"的引导向量。推理时在选定层把它加进每个位置的激活 $\tilde{h}_t^L = h_t^L + \alpha \cdot v^L$（默认强度 $\alpha=1$）即可。因为干预发生在表示空间而非解码概率上，UCCS 不必重训、也不依赖具体采样策略，对 MLM 和 AR-LM 都能直接挂载。
