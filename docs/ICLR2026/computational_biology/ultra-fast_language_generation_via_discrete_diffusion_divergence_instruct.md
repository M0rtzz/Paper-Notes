...
description: >-
  [ICLR 2026][计算生物][扩散模型] 提出 DiDi-Instruct，一种基于积分 KL 散度 (IKL) 最小化的蒸馏框架，将预训练的扩散大语言模型 (dLLM) 蒸馏为少步学生模型，通过对抗性密度比估计 + 分组奖励归一化 + 分数分解 + 奖励引导祖先采样器 (RGAS) 四大关键设计…
tags:
  - "ICLR 2026"
  - "计算生物"
  - "扩散模型"
...
其中奖励 $R(\mathbf{z}_t, t) = \log \mathbf{q}_\nu(\mathbf{z}_t, t) - \log \mathbf{q}_\theta(\mathbf{z}_t, t)$ 就是学生与教师的对数密度比。这样一来蒸馏被完整翻译成一个策略梯度问题，REINFORCE 那套现成机器直接可用，离散不可微的障碍被整个绕了过去。
...
策略梯度天生方差大，奖励尺度一抖训练就不稳。论文照搬 GRPO 的做法，对 mini-batch（一组 $G$ 个样本）内的奖励做标准化：

$$\widetilde{R}_i = \frac{R_i - \mu_g}{\sigma_g + \epsilon}$$

其中 $\mu_g$、$\sigma_g$ 是组内奖励的均值与标准差。归一化后不同样本的奖励尺度被拉齐，梯度方差显著下降，训练曲线更平稳。

**4. 分数分解：在中间加噪态匹配分布，防止熵坍塌**
...
这等于逼着学生不只对齐最终输出，还要对齐中间加噪态的分布，等于多接收了一层更密的监督。这一块是全篇最关键的组件——消融里把它去掉，PPL 直接从 62 爆到 33584（500×+）。

**5. 奖励引导祖先采样器 (RGAS)：推理时分阶段用奖励再榨一遍质量**
