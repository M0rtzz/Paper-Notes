...
description: >-
  [ICLR 2026][机器人][real-to-sim-to-real] 提出D-REX，一个基于高斯表示的可微real-to-sim-to-real引擎，通过视觉观测和机器人控制信号进行端到端物体质量辨识，并利用辨识的质量进行力感知的灵巧抓取策略学习，有效缩小了sim-to-real差距。
tags:
  - "ICLR 2026"
  - "机器人"
  - "real-to-sim-to-real"
...
### 整体框架
D-REX 要解决的是同一个抓取姿势对不同质量物体效果截然不同、而质量又无法从视觉直接读出来的矛盾。它把一段手机拍的 RGB 视频走通了一条 real→sim→real 的闭环：先把视频重建成可碰撞、可渲染的数字孪生，再让机器人在真实和仿真里做同一个推动动作、用可微物理引擎把仿真轨迹拟合到真实轨迹上反解出质量，然后把人类示范视频转译成机器人轨迹，最后训练一个既出关节位置、又按辨识质量出抓取力的力感知策略部署回真机。
