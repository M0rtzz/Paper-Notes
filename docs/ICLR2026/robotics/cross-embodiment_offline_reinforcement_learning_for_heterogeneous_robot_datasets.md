...
description: >-
  [ICLR 2026][机器人][cross-embodiment learning] 系统研究跨形态离线 RL 预训练范式，发现次优数据比例和机器人多样性增加时梯度冲突导致负迁移，提出基于形态图距离的 Embodiment Grouping（EG）策略将机器人按形态聚类后分组更新 actor…
tags:
  - "ICLR 2026"
  - "机器人"
...
### 整体框架
本文要回答的问题是：能不能把"跨形态学习"和"离线 RL"拼在一起，让机器人基础模型不再依赖昂贵的专家演示，而是直接吃多种机器人的混合（含大量次优）数据。整条 pipeline 是这样转的：先把 16 种机器人的离线数据集喂进 URMA 架构，它负责把维度各异的状态/动作空间统一成同一套表示；然后 critic 用**全部**数据做全局更新；而 actor 不再一锅炖，改成按形态把机器人聚成若干组、**分组顺序更新**，每组只用本组数据算 actor loss。这样学到的跨形态策略在下游可以快速微调到新机器人。整篇方法其实是一条"先发现负迁移 → 归因到梯度冲突 → 用形态距离预测冲突 → 据此设计分组"的逻辑链，下面四个设计点就是这条链的四环。
