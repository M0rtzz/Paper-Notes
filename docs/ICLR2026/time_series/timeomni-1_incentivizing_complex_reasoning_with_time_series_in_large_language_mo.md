...
TimeOmni-1 把"让 LLM 真正会推理时间序列"拆成两个互补的环节：先用 TSR-Suite 这套推理导向的数据集把感知、外推、决策三类时序能力以人类引导的推理链形式喂给模型，再用两阶段课程学习把这些先验固化下来——Stage 1 用 SFT 让模型"知道该怎么想"，Stage 2 用带任务定制奖励的 GRPO 让模型"想得更准"，最终所有任务收敛进同一个统一模型。

### 关键设计

**1. TSR-Suite：把"必须推理"和"上下文充分"写进数据集本身。** 现有时序 QA 数据集（如 Time-MQA）问题太浅、上下文太少，导致推理模型相比非推理模型几乎没有优势，模型只能靠猜。TSR-Suite 围绕两条设计原则——推理模型应显著优于非推理模型 $\bar{S}(M_{RM}) \gg \bar{S}(M_{NRM})$、以及提供足够的时序输入 $X$ 与辅助上下文 $C$——构建了 4 个原子任务，覆盖三大推理能力：感知层包含 Task 1 场景理解（单序列归因）和 Task 2 因果发现（多序列因果关系）；外推层是 Task 3 事件感知预测（在事件扰动下推断未来趋势）；决策层是 Task 4 决策制定（整合前两类能力做行动选择）。整套数据含 23K+ 样本、跨越 10 个领域，其中 2.3K 由人类引导精心策划。这条"感知→外推→决策"的链路本身就编码了"先理解、再预测、再行动"的认知逻辑，让模型有了一条真正可走的推理路径。

**2. 层级 CoT 标注流程：用人机协作压低推理链的噪声与成本。** 高质量推理链既不能全靠人写（太贵）也不能全靠 LLM 生成（容易错）。TimeOmni-1 用三步分工解决：先由 LLM Analyzer 在人类引导模板下生成初版推理链（Step-1 CoT）；再由人类专家逐条验证上下文是否充分，并专门为 LLM 出错的案例撰写专家推理链（Step-2 CoT）；最后由 LLM Rewriter 把专家推理链规范化成统一格式。人类引导模板这一步至关重要——GPT-4.1 在因果发现上零样本只有 28.7%，套上人类引导模板后直接跳到 71.1%，说明模板提供的不是答案而是推理的脚手架。

**3. 任务定制的 RL 奖励：离散任务和序列任务各走各的打分方式。** 不同任务的输出形态差异很大，用同一种奖励会失真。所有任务都先过一道格式奖励 $\mathcal{R}_{format}$，强制模型输出 `<think></think><answer></answer>` 结构；对离散任务（Task 1、2、4）用精确匹配的准确率奖励 $\mathcal{R}_{discrete} \in \{0,1\}$；对序列预测任务（Task 3）则拆成两部分——预测序列长度正确给一个计数奖励 $\mathcal{R}_{count}=0.1$，再叠加一个把 MAE 经指数衰减映射后的归一化奖励，让"长度对不对"和"数值准不准"分别被激励。

**4. 多任务联合训练：让三类能力在同一个模型里互相补益。** 以往窄任务方法（如 TimeMaster 对 6 个数据集训 6 个模型）无法跨任务迁移。TimeOmni-1 把全部任务统一进单一模型训练，并用两组渐进实验验证了正向迁移：一是渐进能力迁移——即便完全不直接训练决策任务，仅靠感知+外推训练，决策准确率就从 25.5% 升到 31.3%，说明前置能力会自发外溢到决策；二是渐进能力补充——逐步把前置任务加入联合训练，决策准确率从 40.9% 进一步升到 47.9%。这支撑了"训练一次、跨任务复用"的范式。
