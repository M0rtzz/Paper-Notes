...
### 整体框架
RFC（Relational Feature Caching）搭在已有的Taylor展开缓存方法之上，核心是把"何时全量计算"和"缓存步如何预测输出"这两件事都交给一个几乎免费的线索来处理——同一模块的输入特征变化。在全量计算步，它缓存该步的输出特征和输入特征；在随后的缓存步，RFE用当前输入相对缓存的变化来校正Taylor对输出的预测，同时RCS盯着输入特征的预测误差累积，一旦超过阈值就重新触发全量计算。两个组件都只额外读取输入特征（LayerNorm、缩放、平移等轻量操作即可得到），不引入新的训练。

### 关键设计

**1. RFE：用输入变化的幅度补救Taylor外推丢失的"变化量级"**

纯时间外推之所以会预测不准，是因为DiT输出特征的变化幅度在时步间高度不规则，而Taylor展开假设它平滑演化。RFC的破解办法来自一个稳定的观察量：输出变化与输入变化的比值 $s_k(t-k) = \frac{\|\Delta_k O(t-k)\|_2}{\|\Delta_k I(t-k)\|_2}$ 在不同时步间几乎是常数，实测相对标准差只有约2%。于是 RFE 把预测拆成方向和幅度两部分——方向仍由 Taylor 展开给出，幅度则改由输入变化来定标：用最近两次全量计算间得到的比值 $s_N(t)$ 近似 $s_k(t-k)$，从而 $\|\Delta_k O(t-k)\| \approx s_N(t)\,\|\Delta_k I(t-k)\|_2$。最终的缓存步输出预测为

$$O_{\text{RFE}}(t-k) = O(t) + \big(s_N(t)\|\Delta_k I(t-k)\|_2\big) \cdot g\!\left(\sum_{i=1}^{m}\frac{k^i}{i!}\frac{\Delta_N^i O(t)}{N^i}\right),$$

其中 $g(\cdot)$ 是 L2 归一化函数，把 Taylor 项压成单位方向向量，幅度则全部由前面那个标量负责。这样做之所以有效，是因为输入特征本来就要算（而且代价极低），用它来锚定"这一步到底变了多少"，比硬靠时间外推外推幅度可靠得多。

**2. RCS：用输入预测误差作代理，动态决定何时再做一次全量计算**

理想的调度应该在"输出预测误差大"时才花钱做全量计算，但输出误差恰恰要全量计算才能测到，形成一个鸡生蛋的死循环。RCS 绕开它的办法是改测输入侧的预测误差——作者发现输入误差和输出误差的趋势高度一致（Fig. 2b）。具体地，定义归一化的输入预测误差 $\mathcal{E}_I(t-k) = \frac{\|E_I(t-k)\|_1}{\|I(t-k)\|_1}$，其中 $E_I(t-k) = I(t-k) - I_{\text{Taylor}}(t-k)$ 是输入特征真实值与其 Taylor 预测值之差；当累积误差越过阈值 $\sum_{j=1}^{k} \mathcal{E}_I(t-j) > \tau$ 时就触发一次全量计算并刷新缓存。阈值 $\tau$ 直接调节质量-效率的权衡。更省的是，监控只需挂在第一个模块上而非全部模块（Table 6 验证了这一选择足够），调度的额外开销可以忽略。

**3. Proposition 1：为"比值随 $k$ 不变"给出局部线性的理论支撑**

RFE 的整套操作建立在 $s_N(t)$ 能近似 $s_k(t-k)$ 这一前提上，Proposition 1 把它讲清楚：若输入到输出的映射在局部线性 $O(t) = AI(t) + b$，且输入变化方向 $u_k(t-k)$ 在 $1 \leq k \leq N$ 范围内保持不变，则 $s_k(t-k) = \|A\,u_k(t-k)\|_2$ 是与 $k$ 无关的常数。这两条假设在扩散模型里都站得住——相邻时步特征变化很小，支持局部线性；已有研究也表明特征的变化方向在时步间相当稳定——因此用上一区间的比值去定标当前缓存步是合理的，这也解释了为何实测比值的波动只有约2%。
