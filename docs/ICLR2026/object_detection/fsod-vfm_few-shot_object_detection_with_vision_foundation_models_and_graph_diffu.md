...
这是抑制碎片提案的核心。方法把所有提案当作图的节点，边的方向和权重编码"覆盖关系"：若提案 $i$ 的 UPN objectness 分数低于提案 $j$，就建一条从 $j$ 指向 $i$ 的边，权重为两者掩码重叠占 $i$ 自身面积的比例 $\mathrm{Area}(M_i \cap M_j)/\mathrm{Area}(M_i)$。直觉上，一个分数更高、且几乎把 $i$ 盖住的大框，应当把抑制信号推给被它覆盖的碎片 $i$。然后在这张图上跑 PageRank 式迭代传播一个"被抑制度" $\pi$：

$$\pi^{t+1} = \alpha \, P \, \pi^{t} + (1-\alpha)\, w$$

其中 $P$ 是按上述边权归一化的转移矩阵，$w$ 是初始抑制分布，$\alpha$ 控制传播与重启的比例。迭代收敛后，被多个高分大框覆盖的碎片提案会累积到较高的 $\pi$ 值。最终置信度把这个抑制度乘回匹配分数：
