...
整条管线分三段接力。先从源演示的多视角 RGB 把场景重建成带真实物理尺度的 3D 点云；再在点云里把目标物体挪到新位置、用逆运动学重新算出机械臂的接近轨迹，渲染出一串编辑后的深度图；最后把这串深度图连同动作、边缘、光线等条件喂给视频扩散模型，生成视觉逼真的多视角操作视频，每一帧 RGB 配上对应动作标注，就是一条全新的演示数据。

### 关键设计

**1. 度量尺度 3D 重建：让"向右挪 10cm"在物理上真的成立**

纯 2D 图像编辑（如 inpainting）有个根本毛病——在图里被"移动"的物体，在 3D 空间里可能压根找不到一个合理位姿，编辑出来的场景几何上是矛盾的。这篇用 DUSt3R 一类端到端重建模型，从演示的多个相机视角恢复稠密点云和相机参数，关键是**度量尺度（metric-scale）**：重建出的 3D 坐标与真实世界的厘米级坐标一一对应，所以"把杯子向右移动 10cm"这种指令在点云里有确切的物理含义，而不只是像素层面的平移。编辑后渲染深度图时还要做一道几何校正——物体挪到新位置后，机械臂的关节角度要通过逆运动学（IK）重新求解以保证运动学可行，再把校正后的机械臂模型渲染进新场景的深度图。先在 3D 里改、再投影回 2D，空间一致性从源头上就被锁死了。

**2. 多条件视频生成：四路信号各管一摊，把深度图"画"成真实视频**
...
**3. 空间增强与轨迹生成：用算法把"采位置 + 编轨迹"自动化**

空间泛化卡在哪？卡在训练数据里物体位置不够多样，而人工去摆不同位置再演示成本极高。这篇的做法是把这一步彻底自动化：给定源演示中物体初始位置 $p_0$，在它周围定义一个采样空间（如以 $p_0$ 为中心、半径 $r$ 的球体），均匀或随机采出一批新位置 $\{p_1, p_2, \dots, p_M\}$。对每个新位置，先在点云里把物体从 $p_0$ 平移/旋转到 $p_i$，再用 IK 重新规划抓取前的接近轨迹，而抓取之后的抬起、移动、放下这些动作本身直接从源演示复制——因为这些动作相对物体是不变的，变的只是怎么够到它。最后渲染整段过程的深度图序列交给视频生成。这套流程还能扩展到高度编辑（改物体垂直位置）和纹理编辑（改物体外观），用算法替代人工，以极低成本铺满目标空间。
