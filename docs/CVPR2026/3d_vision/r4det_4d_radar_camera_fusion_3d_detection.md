...
flowchart TD
    A["多视角图像 + 4D雷达点云"] --> B["图像 backbone / 雷达编码器<br/>分别提取图像与雷达特征"]
    B --> C["全景深度融合（PDF）<br/>雷达 query 邻域交叉注意力聚合图像语义<br/>+ 三重深度监督生成 BEV 特征"]
    C --> D["与雷达 BEV 拼接融合<br/>初始融合 BEV X_t"]
    D --> E["可变形门控时序融合（DGTF）<br/>DCN 运动对齐 + GRU 门控更新（无需 ego pose）"]
    E --> F["时序一致特征 F_RC"]
    F --> G["实例引导动态精炼（IGDR）<br/>2D 实例原型预测仿射参数校准 + 前景门控"]
    G --> H["最终特征 F_final → 3D 检测头"]
```
