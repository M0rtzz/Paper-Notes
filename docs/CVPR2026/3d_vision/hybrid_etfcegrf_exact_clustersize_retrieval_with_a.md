...
有了精确聚类大小，下一步是怎么不靠重标记就得到 $p$ 值。本文沿用 pTFCE 的 GRF 框架：高度 $h$ 处聚类大小的生存函数为 $P(C > c \mid h) = \exp(-\lambda_h c^{2/3})$，其中 $\lambda_h = (E[c_h]/\Gamma(5/2))^{-2/3}$、期望聚类大小 $E[c_h] = N(1-\Phi(h))/E[\chi_h]$。再用贝叶斯定理把体素高度先验和聚类似然结合成条件概率 $P(Z_v \geq \tau_i \mid c_v^{uf}(\tau_i))$，逐阈值累积成

$$A(v) = \sum_i -\log P\big(Z_v \geq \tau_i \mid c_v^{uf}(\tau_i)\big).$$
