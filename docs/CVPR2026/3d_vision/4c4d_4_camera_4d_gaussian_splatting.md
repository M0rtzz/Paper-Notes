...
### 整体框架
4C4D 想解决的是：只有 4 台相机时，4DGS 会把全部精力花在拟合训练视角的颜色上，几何（深度）几乎学不动，于是新视角一换就崩。它的做法不改 3D 表示本身，而是在每个 4D 高斯的不透明度上挂一个**可学习的衰减因子**，借此改变优化时梯度的流向。整条管线是：4D 高斯原语先按原本的时间高斯算出不透明度，神经衰减函数 $f_\theta$ 读入每个高斯的属性预测一个衰减系数 $\tau$ 去调制它，再叠加一层"只对当前可见高斯生效"的分离策略，最后仍用普通的光度渲染损失把衰减函数和 4D 高斯一起端到端优化。
