...
description: >-
  [CVPR 2026 Findings][3D视觉][3D高斯泼溅] 提出 LTGS 框架，通过构建可复用的物体级高斯模板，从时空稀疏的观测图像中高效更新 3DGS 场景重建，实现长期环境演化的时序建模。 3DGS/NeRF 等新视角合成方法能从常规相机输入重建高质量静态 3D 场景，但日常环境中物体频繁移动（如家具搬动、物…
tags:
  - "CVPR 2026 Findings"
  - "3D视觉"
  - "3D高斯泼溅"
...
整条管线的核心思路是**把"重建一整个场景"降级成"跟踪几个动过的物体"**：先找出哪里变了，再把变化的物体在不同时间步之间关联起来、估出它们各自的 6DoF 位姿，最后让同一个物体模板在各时间步之间复用——一次构建，多步刚性变换。四个阶段依次是变化检测、物体模板构建与 2D 实例匹配、物体模板跟踪与 3D 配准、长期高斯优化。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["初始重建 G₀ + 极少量随机更新图像（如 3 张）"] --> B["变化检测<br/>SAM 语义相似度 + SSIM 光度差融合 → 物体级伪掩码"]
    B --> C["物体模板构建与 2D 实例匹配<br/>MASt3R 密集匹配归实例 + SAM 特征匈牙利匹配跨时间步对应"]
    C --> D["物体模板跟踪与 3D 配准<br/>点云挂 DINO 特征鲁棒配准 → 6DoF 变换 + Chamfer 校验"]
    D --> E["长期高斯优化<br/>模板按位姿搬运 + 6DoF 位姿微调 + 时序不透明度滤波"]
    E --> F["场景演化序列 𝒮 = {G₀, G₁, …, G_M}"]
```
