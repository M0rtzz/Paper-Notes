...
flowchart TD
    A["输入：本体状态 + 3D 点云观测"] --> B["3D 观测编码器 PointNet<br/>→ 全局条件向量"]
    B --> C["无时间步 1D U-Net 生成器<br/>高斯噪声 → 动作轨迹（1 NFE）"]
    C -->|推理时| Z["输出：未来 H=16 步动作轨迹"]
    C -->|训练时| D["训练时漂移场<br/>吸引专家模式 − 排斥其他预测"]
    D --> E["多尺度场聚合<br/>温度 0.02 / 0.05 / 0.2 归一加权"]
    E --> F["Sigmoid 调度的损失过渡<br/>前 70% MSE 搭骨架 → 后段漂移场锐化"]
    F -->|反传更新生成器| C
```
