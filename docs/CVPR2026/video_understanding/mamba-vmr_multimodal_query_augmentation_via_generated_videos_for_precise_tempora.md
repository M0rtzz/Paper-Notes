...
flowchart TD
    Q["文本查询 + 视频字幕"] --> A["LLM 引导的字幕匹配与查询处理<br/>LLaMA-3.1 按动词拆子事件、补隐含动作<br/>+ 字幕 top-k 相关性匹配"]
    A --> B["时序先验生成<br/>融成叙事提示 → CogVideoX 生成约 6 秒短视频"]
    O["目标长视频"] --> M
    B --> M["多模态控制 Mamba 网络<br/>文本 / 生成视频 / 目标视频三路融合<br/>+ 视频引导门控调制状态传播"]
    M --> P["线性头预测起止时间戳"]
    P --> NMS["NMS 精炼候选区间 → 输出时间段"]
```
