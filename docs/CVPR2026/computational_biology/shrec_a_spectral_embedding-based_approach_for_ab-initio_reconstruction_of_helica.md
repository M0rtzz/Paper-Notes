...
description: >-
  [CVPR 2026][计算生物][cryo-EM] 提出 SHREC 算法，通过谱嵌入（spectral embedding）从冷冻电镜 2D 投影图像中直接恢复螺旋分子片段的投影角度，无需预先知道螺旋对称参数（rise/twist），实现了真正的 ab-initio 螺旋结构重建。
tags:
  - "CVPR 2026"
  - "计算生物"
  - "cryo-EM"
...
**2. 密度不变图拉普拉斯谱嵌入：从成对距离把图像摊到圆上、读出相位角。**

知道数据在圆上还不够，得有办法把每张图真正放到圆上的对应位置。SHREC 先算所有片段两两之间的 $L^2$ 距离，用高斯核 $W_{ij} = \exp(-d_{ij}^2 / 2\varepsilon)$ 构造相似度矩阵，再做一次密度归一化得到密度不变图拉普拉斯
...
**4. 基于 PCA 的 Wiener 滤波去噪：在算距离之前先把信噪比抬上来。**

cryo-EM 图像的信噪比极低，如果直接拿原图算成对 $L^2$ 距离，距离矩阵会被噪声主导、流形结构当场被破坏。SHREC 在进入谱嵌入前先做一轮去噪：对所有投影图像做 PCA，低阶主成分主要装信号、高阶主成分主要装噪声，于是从高阶主成分估计噪声功率谱 $\hat{P}_{NN}$（径向平均以满足各向同性假设），再用 $\hat{P}_{SS} = \max(0,\,\hat{P}_{YY} - \hat{P}_{NN})$ 估出信号功率谱，最后构造 Wiener 滤波器 $G(\mathbf{f}) = \hat{P}_{SS} / (\hat{P}_{SS} + \hat{P}_{NN})$ 逐张滤波。去噪后的距离才真正反映片段间的几何差异，圆形嵌入也才稳定。
