...
description: >-
  [CVPR2026][音频/语音][多模态融合] 针对第10届 ABAW 竞赛的矛盾/犹豫 (A/H) 视频识别任务，提出基于散度的多模态融合策略，通过计算视觉（AU）、音频（Wav2Vec 2.0）和文本（BERT）三个模态嵌入的逐对绝对差来显式建模跨模态冲突，在 BAH 数据集上以 Macro F1 0.6808 大幅超越基线 0.2827。
tags:
  - "CVPR2026"
  - "音频/语音"
  - "多模态融合"
...
视觉这一路如果用黑盒深度特征，就回答不了“到底哪种面部行为和 A/H 相关”。作者改用 Py-Feat 从预裁剪人脸逐帧提取 20 个 Action Units（每 3 帧取 1，约 10 fps），并且不取静态强度，而是在窗口大小 $W=16$、步长 $S=8$ 的滑动窗口内对每个 AU 算四种统计量（均值、标准差、斜率、范围），拼成 80 维窗口描述子——因为 A/H 体现为面部动作的**时间波动**而非某个定格表情。这一点有统计支撑：在 1132 个视频上做 Mann-Whitney U 检验，AU 的时间变异性（标准差）才是主要判别因子、而非均值强度，说明 A/H 是面部的“不稳定”。音频以 16 kHz 单声道用 wav2vec2-base-960h 编码成 768 维（约 50 Hz），文本用 BERT-base 取 [CLS] 的 768 维并微调最后两层（学习率降低）。

**2. 时序建模与注意力池化：把变长序列压成可比对的定长向量**

散度融合要逐元素相减，前提是三个模态先落到同一个可比的空间。视觉和音频的时序特征各过 2 层 BiLSTM（隐藏维 64），再用注意力池化把变长序列压成定长向量，最后经线性投影统一映射到 $D=128$ 维共享空间，得到 $\mathbf{h}'_v$、$\mathbf{h}'_a$、$\mathbf{h}'_t$。这一步是后面做差的“对齐前提”——只有维度一致、语义对齐，逐对差值才有意义。

**3. 散度融合：把“模态冲突”直接写成逐对绝对差**

这是全文核心。传统拼接（Fusion A）把三模态首尾相接 $\mathbf{f}_A = [\mathbf{h}'_v;\, \mathbf{h}'_a;\, \mathbf{h}'_t]$，默认模态互补、让分类器自己从 $3\times128=384$ 维里学关系，但在小数据上很难学出微妙的冲突信号。既然 A/H 在定义上就是“共存的冲突信号”，作者干脆让融合模块直接算模态对之间的逐元素绝对差（Fusion B）：

$$\mathbf{f}_B = [|\mathbf{h}'_v - \mathbf{h}'_a|;\, |\mathbf{h}'_v - \mathbf{h}'_t|;\, |\mathbf{h}'_a - \mathbf{h}'_t|]$$

如果视觉表达与音频情感一致，差值趋近零；一旦面部微笑但语调犹豫，差值就在相关维度上炸出显著响应——冲突被显式编码成了特征，维度同样是 $3\times128=384$。Fusion C 则两者都留 $\mathbf{f}_C = [\mathbf{f}_A;\, \mathbf{f}_B]$（768 维，信息最全但参数更多）。实验里纯散度 Fusion B 在测试集反而最好，说明“差值即特征”已经抓住了 A/H 的本质，再堆原始嵌入并不额外加分。

### 损失函数 / 训练策略

融合向量过 3 层 MLP（Dropout $p=0.3$）做二分类。损失用 BCEWithLogitsLoss + 类别权重应对正负样本不均；优化器 AdamW，BERT 部分学习率 $5\times10^{-5}$、其余参数 $5\times10^{-4}$；学习率余弦退火、共 30 个 epoch；梯度裁剪阈值 1.0，早停 patience 为 8。
