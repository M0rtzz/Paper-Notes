...
整个流程围绕三种模态腐蚀展开：先设计好腐蚀策略，再把腐蚀后的数据喂给 GRPO 做训练（幻觉诱导训练），最后把"训练阶段是否腐蚀 × 推理阶段是否腐蚀"交叉成 8 种设定做评估，对照出 RL 训练真正在强化什么。三种腐蚀分别打击不同的模态通道，互为对照，构成诊断的核心。

### 关键设计

**1. 空白图像替换（Blank Image, BI）：把视觉通道直接掐断，看 RL 还能不能涨。**

这一招针对的痛点最直接——既然怀疑模型没在用视觉，那就干脆把视觉拿走。具体做法是把训练/测试中的所有图像换成空白图，GRPO 训练时模型只能从纯文本条件出发生成推理链，碰巧答对就拿正向奖励。它的诊断逻辑很硬：如果在完全没有视觉信息的条件下 RL 训练仍然能提升推理性能，那就直接证明这些提升根本不需要视觉参与，RL 强化的是文本推理而非视觉感知。

**2. 随机图像替换（Random Image, RI）：不止抽走视觉，还塞进干扰，看模型会不会被带偏。**

BI 只是"没有视觉"，RI 更进一步制造"错误视觉"——把每张图换成数据集里随机另一张图，构造文图完全不匹配的训练对。这比 BI 更苛刻，因为模型不仅缺正确视觉信息，还要面对一个与题目无关的图在旁边干扰。如果在这种带干扰的条件下 RL 训练依然有效，说明模型已经学会了主动忽略视觉、退守到文本推理上——这是比 BI 更强的"视觉无用"证据。

**3. 文本信息移除（Textual Removal, TR）：反向对照，逼模型只能靠视觉，看它行不行。**

前两个设计破坏视觉，TR 反过来破坏文本：通过规则匹配把题目里的变量条件和问题描述删掉，只留模板化指令和图像。它的角色是反证——如果 RL 真能利用视觉，那么 TR 训练应该是表现最好的，因为图像里其实还保留着问题条件和标注，视觉通道是唯一的信息来源。但实验结果恰恰相反：TR 训练并没有显著超过完全无视觉的 BI/RI，这从另一侧坐实了"当前 RL 没把视觉用起来"的结论。
