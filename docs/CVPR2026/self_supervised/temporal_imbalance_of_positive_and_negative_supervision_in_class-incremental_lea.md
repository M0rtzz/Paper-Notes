...
description: >-
  [CVPR2026][自监督学习][Class-Incremental Learning] 提出时序不平衡（Temporal Imbalance）这一被忽视的类增量学习偏差来源，并设计 Temporal-Adjusted Loss（TAL）通过时间衰减记忆核动态降低旧类的负监督权重…
tags:
  - "CVPR2026"
  - "自监督学习"
...
给负监督降权会改变损失的整体尺度，若不校正，即便在时序均匀、类别平衡的理想情形下 TAL 也会和 CE 不一致。$\alpha$ 就是为消除这个尺度漂移而设的对齐项，它由类别数 $C$ 和指数 $r$ 唯一确定（⚠️ 闭式表达式以原文为准），不引入新的可调超参。它把 TAL 锚定成 CE 的一个"无偏推广"——只有当时序确实不平衡时才偏离 CE 去保护旧类，否则严丝合缝地退化回原始交叉熵。
