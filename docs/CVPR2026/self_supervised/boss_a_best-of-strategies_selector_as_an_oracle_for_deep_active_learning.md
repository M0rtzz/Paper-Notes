...
flowchart TD
    U["未标注池 U + 已标注集 L"] --> SUB["随机子池采样<br/>每轮随机取 U′ ⊆ U（大小 k_max = 10b）"]
    SUB --> ENS["多策略集成<br/>8 种策略各在 U′ 上提交候选 batch B_1…B_M"]
    ENS --> PROBE["冻结 backbone 的线性探测代理<br/>backbone 冻结，L∪B_m 上只重训线性头，验证集读准确率"]
    PROBE --> SEL["batch 级的最优选择<br/>B* = argmax Acc(θ_L∪B_m; V)"]
    SEL --> ADD["选中 B* 并入 L，标注预算 − b"]
    ADD -->|预算未耗尽| SUB
    ADD -->|预算耗尽| OUT["输出 oracle 上界曲线"]
```
