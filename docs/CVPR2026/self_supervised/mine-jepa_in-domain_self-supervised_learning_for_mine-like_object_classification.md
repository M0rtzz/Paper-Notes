---
title: >-
  [论文解读] MINE-JEPA: In-Domain Self-Supervised Learning for Mineral Exploration
description: >-
  [CVPR 2026][自监督学习][侧扫声纳] 提出 Mine-JEPA，首个面向侧扫声纳（SSS）水雷分类的域内自监督学习流水线——基于 SIGReg 正则化损失、声纳适配增强策略和 ImageNet 初始化，仅用 1,170 张未标注声纳图像预训练即超越了在 17 亿图像上预训练的 DINOv3 基础模型。
tags:
  - "CVPR 2026"
  - "自监督学习"
  - "侧扫声纳"
  - "水雷分类"
  - "域内预训练"
  - "SIGReg"
---

# MINE-JEPA: In-Domain Self-Supervised Learning for Mineral Exploration

**会议**: CVPR 2026  
**arXiv**: [2604.00383](https://arxiv.org/abs/2604.00383)  
**代码**: 无  
**领域**: 自监督学习 / 水下声纳图像  
**关键词**: 自监督学习, 侧扫声纳, 水雷分类, 域内预训练, SIGReg

## 一句话总结

提出 Mine-JEPA，首个面向侧扫声纳（SSS）水雷分类的域内自监督学习流水线——基于 SIGReg 正则化损失、声纳适配增强策略和 ImageNet 初始化，仅用 1,170 张未标注声纳图像预训练即超越了在 17 亿图像上预训练的 DINOv3 基础模型。

## 研究背景与动机

侧扫声纳（SSS）用于海底勘测和水雷检测，是水雷对抗（MCM）的核心技术。该领域面临三大挑战：

**极端数据稀缺**：公开数据集仅 1,170 张声纳图像（含 668 个标注目标），标注获取极其困难

**巨大的域差距**：声纳图像基于声学回波成像，与 RGB 自然图像在成像机理、纹理统计上截然不同——颜色信息几乎无意义，关键线索来自声学亮/暗区域和海床纹理

**大模型并非万能**：直觉上应该用 DINOv3 等大规模预训练模型进行迁移，但声纳域的特殊性使得这一策略未必有效

核心研究问题：在极端小数据声纳场景下，**精心设计的域内 SSL 能否替代大规模通用基础模型？**

## 方法详解

### 整体框架

Mine-JEPA 要回答的问题很具体：当整个声纳域只有 1,170 张图可用时，怎么从这点数据里学出一个比通用大模型还好用的骨干网络。它的答案是一条不碰花哨架构、把每个环节都为声纳重新调过的三阶段流水线。先把 1,170 张 SSS 图像以步长 64 滑窗切成 96×96 的小块，凑出约 153K 张无标注 patch 作为预训练素材；再用一个极简的自监督目标（SIGReg）配上专门为声纳改写的数据增强，从 ImageNet-1K 权重出发训练一个 ViT 骨干；最后冻结或微调这个骨干，接上线性/MLP 头去做水雷分类评估。整条链路没有 teacher-student、没有动量编码器、没有大 batch，刻意把复杂度压到最低，因为在这么小的数据上，越简单的目标越不容易崩。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["1170 张未标注 SSS 图像"] --> B["滑窗切块<br/>96×96，步长 64 → ~153K patch"]
    B --> C["真实+合成混合数据<br/>~153K 真实 + ~256K 合成 ≈ 409K patch"]
    C --> D["SSS 适配增强<br/>去色彩 jitter / 加垂直翻转·±15° 旋转"]
    INIT["ImageNet 初始化<br/>低层先验、保留适配弹性"] --> E
    D --> E["ViT 骨干 + 极低投影维度 d=16<br/>用投影瓶颈做隐式正则"]
    E --> F["SIGReg 自监督损失<br/>不变性项 + 分布正则（防坍缩、O(N)）"]
    F -->|"预训练完成"| G["冻结/微调骨干 + 线性/MLP 头<br/>水雷分类评估"]
```

### 关键设计

**1. SIGReg 自监督损失：在极小数据上找一个不会坍缩又不挑架构的训练目标**

声纳数据少到经不起折腾，常见的对比学习要靠大 batch 凑负样本、蒸馏式方法要维护 EMA teacher，这些机制在 153K patch、小 batch 的设定下都不稳。Mine-JEPA 直接借用 LeJEPA 框架里的 SIGReg，把损失拆成两项：$\mathcal{L} = (1-\lambda)\mathcal{L}_{inv} + \lambda\mathcal{L}_{sig}$。不变性项 $\mathcal{L}_{inv} = \frac{1}{NV}\sum_{i}\sum_{v}\|z_{i,v} - \bar{z}_i\|_2^2$ 逼着同一个 patch 的不同增强视图收敛到相近的表示，这是学语义的部分；SIGReg 项则把整批嵌入的分布正则化成标准正态分布——具体做法是用随机投影把高维嵌入打到若干一维方向上，再用 Epps-Pulley 拟合统计量衡量它和正态的差距。这个分布约束就是防坍缩的关键：不需要负样本对、不需要 EMA teacher，复杂度只有 $O(N)$，天然适配小数据、小 batch 的场景。

**2. 极低投影维度 d=16：把瓶颈当成正则化，而不是省事**

投影头的输出维度在这里被故意压到 16，而 SimCLR 用 128、VICReg 用 2048——这不是图省事的实现细节，而是一个有意的结构瓶颈。数据一旦稀缺，高维投影空间反而给了模型过拟合的余地；把出口收窄到 16 维，等于强迫表示只保留最关键的几个判别方向，相当于一种隐式正则化。在小数据场景下，这种"看似吃亏"的低维设计反而帮助泛化。

**3. SSS 适配增强：把为 RGB 设计的增强里那些对声纳有害的部分换掉**

自监督学习的语义信号全靠增强提供，但声纳图像基于声学回波成像，颜色几乎没有意义，关键线索是声学亮/暗区域和海床纹理。照搬自然图像那套增强会直接误导模型。于是这里做了三类改动：移除色调/饱和度 jitter、solarization、灰度转换这些对声纳无意义甚至有害的操作；保留水平翻转、随机裁剪缩放（0.5–1.0）、高斯模糊这些仍然合理的几何/模糊变换；再新增垂直翻转和 ±15° 随机旋转，因为声纳扫描几何本身具有方向不变性。归一化也改用数据集自身的统计量。这一步的分量很重——直接套用自然图像增强时 F1 只有 0.312，比完全不预训练的 0.557 还差，换成声纳适配增强后回到 0.725，说明增强策略不是锦上添花，而是域内 SSL 能不能成立的前提。

**4. 用 ImageNet 而非 DINOv3 初始化：要的是低层先验，不是被特化锁死的表示**

骨干的初始权重用 ImageNet-1K 预训练得到，投影头随机初始化。一个反直觉的选择是没有用更强的 DINOv3 来起步——实验里从 DINOv3 出发做域内 SSL 反而性能掉了 10–13 个百分点。原因在于 ImageNet 给的是边缘、纹理这类通用低层先验，留有足够的适配弹性；而 DINOv3 的表示已经为自然图像高度特化，继续往声纳域拉反而要先"拆掉"原有结构，得不偿失。换句话说，初始化要的是可塑性，不是越强越好。

**5. 真实+合成混合数据：让正则化式目标吃下异构数据带来的额外样本**

除了约 153K 张真实 patch，训练时还加入约 256K 张灰度的合成声纳 patch，总量约 409K。真实和合成 patch 各自用自己的统计量归一化后再拼到一起训练。值得注意的是，这种异构数据并非对谁都有用：SIGReg、VICReg 这类正则化式目标能从中获益，而对比式（SimCLR）基本持平、蒸馏式（BYOL）反而严重退化——这也从侧面印证了把训练目标选成分布正则化的稳健性。

### 损失函数 / 训练策略

总损失就是 SIGReg：$\mathcal{L} = (1-\lambda)\mathcal{L}_{inv} + \lambda\mathcal{L}_{sig}$，取 $\lambda=0.1$。每张 patch 取 4 个增强视图，batch size 1024，用 AdamW（weight decay 0.05），学习率 $1.4 \times 10^{-3}$，1 个 epoch warmup 后 cosine decay，共 100 epochs。评估阶段提供 4 种探针模式：linear / mlp（冻结骨干）和 finetune / ft_mlp（微调骨干）。

## 实验关键数据

### 主实验

**3 类分类（BG / MILCO / NOMBO）**

| 方法 | 初始化 | SSL 数据 | macro-F1 | NOMBO F1 | Acc |
|------|--------|---------|----------|----------|-----|
| Random Init | Random | — | 0.557 | 0.439 | 58.3% |
| IN1K Only | IN1K | — | 0.739 | 0.647 | 76.3% |
| DINOv3 | DINOv3 | — | 0.810 | 0.700 | 83.4% |
| SimCLR | IN1K | Real+Syn | 0.801 | 0.693 | 82.6% |
| VICReg | IN1K | Real+Syn | 0.800 | 0.676 | 82.8% |
| BYOL | IN1K | Real+Syn | 0.693 | 0.539 | 72.8% |
| **Mine-JEPA** | **IN1K** | **Real+Syn** | **0.820** | **0.734** | **83.8%** |

**二分类（Mine vs Non-mine）+ 模型规模对比**

| 方法 | 初始化 | 参数量 | 3-class F1 | 2-class F1 | MILCO Recall |
|------|--------|--------|-----------|-----------|-------------|
| DINOv3 | DINOv3 | 21.5M | 0.810 | 0.922 | 88.1% |
| Mine-JEPA (ViT-S) | IN1K | 21.6M | **0.820** | **0.935** | 90.9% |
| Mine-JEPA (ViT-Tiny) | IN1K | **5.5M** | 0.814 | **0.935** | **91.4%** |

### 消融实验

**初始化、增强、数据组成的累积效果**

| 配置 | Init | SSL Data | macro-F1 | Δ |
|------|------|---------|----------|---|
| Random Init（无预训练） | Random | — | 0.557 | — |
| 自然图像增强 SSL | Random | Real* | 0.312 | −24.5%p |
| DINOv3 + SIGReg | DINOv3 | Real | 0.706 | −10.4%p vs DINOv3 |
| DINOv3 + SIGReg | DINOv3 | Real+Syn | 0.677 | −13.3%p vs DINOv3 |
| SSS 增强 SIGReg | Random | Real | 0.725 | 基线 |
| + IN1K init | IN1K | Real | 0.756 | +3.1%p |
| + λ 调优 | IN1K | Real | 0.799 | +4.3%p |
| + Real+Syn 数据 | IN1K | Real+Syn | **0.820** | +2.1%p |

**SSL 方法对比与数据扩展性**

| SSL 方法 | 损失类型 | proj dim | Real only | Real+Syn | Δ |
|---------|---------|---------|----------|---------|---|
| SIGReg | 正则化 | 16 | 0.799 | **0.820** | +2.1%p |
| VICReg | 正则化 | 2048 | 0.774 | 0.800 | +2.6%p |
| SimCLR | 对比 | 128 | 0.806 | 0.801 | −0.5%p |
| BYOL | 蒸馏 | 256 | 0.774 | 0.693 | −8.1%p |

### 关键发现

1. **域内 SSL 超越大规模基础模型**：仅用 1,170 张声纳图预训练的 Mine-JEPA（ViT-S）超过了在 17 亿图像上预训练的 DINOv3
2. **强模型域适配反而退化**：对 DINOv3 继续做域内 SSL 导致性能下降 10-13 个百分点——这挑战了"更强底座一定更好"的直觉
3. **增强策略是域内 SSL 的前提条件**：自然图像增强用于声纳会让 F1 从 0.557 降到 0.312（比不预训练还差）
4. **正则化式 SSL 更稳健**：面对异构合成数据时，SIGReg/VICReg 能获益，而 SimCLR 持平、BYOL 严重退化
5. **ViT-Tiny 极具竞争力**：仅 5.5M 参数（DINOv3 的 1/4）就达到了可比性能，适合 AUV 等资源受限平台

## 亮点与洞察

1. **"小而精"vs"大而全"的深刻洞察**：本文最重要的发现不是 Mine-JEPA 的具体设计，而是证明了在极端域偏移场景下，精心适配的小模型优于未适配的大模型
2. **初始化策略的反直觉结论**：中等强度的 ImageNet 初始化比更强的 DINOv3 更适合作为域内 SSL 的起点——因为过度特化的表示缺乏适配弹性
3. **关于异构数据可用性的积极信号**：RGB 真实 patch + 灰度合成 patch 这种模态异构数据在正则化式 SSL 下仍能有效利用
4. **极简设计哲学**：投影维度 16、单超参 λ、无动量编码器——在小数据场景下，最有效的 SSL 不是最复杂的

## 局限与展望

1. **单一数据集**：仅在一个公开声纳数据集（1,170 张图）上实验，结论的普适性需要更多数据验证
2. **测试集较小**：3 类分类仅 110 个测试样本，方差较大
3. **仅 patch 级分类**：未扩展到滑窗检测或语义分割，实际应用中需要端到端检测
4. **合成数据依赖**：Real+Syn 设定中合成数据来源未详细说明
5. **DINOv3 退化原因未深入分析**：仅提供了一些推测（特征变形、分布偏移），缺乏定量分析

## 相关工作与启发

- **与 LeJEPA 的关系**：Mine-JEPA 直接构建在 LeJEPA/SIGReg 之上，主要贡献在于域适配层面（增强策略、初始化策略、数据组成）
- **医学影像类比**：与医学影像领域"域内 SSL > 通用预训练"的发现一致（如 Models Genesis）
- **启发**：在其他极端域偏移场景（如卫星SAR图像、地震数据波形）中，域内 SSL 可能也优于现成的基础模型
- **对基础模型的警示**：不加思考地对基础模型做继续预训练可能适得其反，需要评估初始化的"可塑性"

## 评分

- **新颖性**: ⭐⭐⭐ — 方法层面是已有技术的适配组合，核心贡献在实验洞察
- **实验充分度**: ⭐⭐⭐⭐ — 消融全面系统，涵盖初始化/增强/数据/方法四个维度
- **写作质量**: ⭐⭐⭐⭐ — 逻辑清晰，结论明确，讨论深入
- **实用价值**: ⭐⭐⭐⭐ — 为数据稀缺海洋视觉任务提供了实用方案

<!-- RELATED:START -->

<div class="related-papers" markdown="1">

## 相关论文

- [\[CVPR 2026\] CHEEM: Continual Learning by Reuse, New, Adapt and Skip -- A Hierarchical Exploration-Exploitation Approach](cheem_continual_learning_by_reuse_new_adapt_and_skip_--_a_hierarchical_explorati.md)
- [\[CVPR 2026\] A Stitch in Time: Learning Procedural Workflow via Self-Supervised Plackett-Luce Ranking](a_stitch_in_time_learning_procedural_workflow_via_self_supervised_plackett_luce_r.md)
- [\[CVPR 2026\] Group-DINOmics: Incorporating People Dynamics into DINO for Self-supervised Group Activity Feature Learning](group_dinomics_incorporating_people_dynamics_into_dino_for_self_supervised_group_activity_feature_learning.md)
- [\[CVPR 2026\] Re-Depth Anything: Test-Time Depth Refinement via Self-Supervised Re-lighting](redepth_anything_test-time_depth_refinement_via_self-supervised_re-lighting.md)
- [\[ICML 2026\] Understanding Self-Supervised Learning via Latent Distribution Matching](../../ICML2026/self_supervised/understanding_self-supervised_learning_via_latent_distribution_matching.md)

</div>

<!-- RELATED:END -->
