...
### 整体框架
这篇论文不提新指标，而是回答一个被长期忽略的问题：在一次主观测试给定的条件下，PCC 究竟最高能到多少、MSE 最低能到多少？答案分三步搭起来。先从 MOS 的数学性质推出 PCC 上界和 MSE 下界，二者都只由「投票方差」和「投票人数」决定；再用 BinoVotes 投票模型把投票方差用一个解析公式给出，让没有记录原始投票的主观测试也能套用这套界；最后在 18 项跨域主观测试上验证理论界与实测吻合。
