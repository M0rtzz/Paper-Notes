...
description: >-
  [CVPR 2026][医学图像][去噪] 提出 RelativeFlow，基于 flow matching 的框架，通过将绝对噪声到干净映射分解为相对更噪到噪声映射，结合一致传输约束和基于模拟的速度场，从异质噪声参考中学习统一的去噪流，突破参考偏差限制。 医学图像去噪 (MID) 缺乏绝对干净的图像用于监督——只有来自不同…
tags:
  - "CVPR 2026"
  - "医学图像"
  - "去噪"
...
flowchart TD
    ...
    subgraph SVF["基于模拟的速度场（SVF）"]
        direction TB
        C["模态退化算子 D_Δt<br/>CT 泊松-高斯 / MR Rician → 更噪样本 x_t−Δt"] --> D["速度目标<br/>u = (x_t − D_Δt(x_t)) / (e^Δt − 1)"]
    end
    SVF --> E["网络 N_θ 预测速度，L2 逼近 u"]
    F["渐进式步长课程<br/>逐轮扩大 [Δt_min, Δt_max]"] -.调度 Δt.-> C
    E --> G["采样：多步欧拉积分串接相对去噪步<br/>→ 统一高质量去噪输出"]
```
