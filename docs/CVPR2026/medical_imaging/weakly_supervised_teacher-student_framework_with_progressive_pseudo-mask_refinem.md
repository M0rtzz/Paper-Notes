...
description: >-
  [CVPR 2026][医学图像][弱监督语义分割] 提出弱监督教师-学生框架，利用稀疏病理标注和 EMA 稳定的教师网络生成渐进式精炼伪掩码，结合置信度过滤、自适应融合和课程引导精炼策略，实现结直肠癌病理图像中腺体结构的高效分割。 临床需求 结直肠癌（CRC）是全球第三大常见癌症，其病理分级高度依赖腺体结构的精确分割…
tags:
  - "CVPR 2026"
  - "医学图像"
  - "弱监督语义分割"
...
flowchart TD
    A["稀疏病理标注"] --> B["学生网络<br/>在真值上训练"]
    B -->|EMA 滑动平均更新| C["EMA 稳定的教师网络"]
    C --> D["对未标注区域产出伪掩码"]
    D --> E["置信度过滤<br/>低置信区域标为忽略"]
    E --> F["自适应融合<br/>真值 + 高置信伪掩码"]
    F -->|混合监督| B
    G["课程引导的渐进式精炼<br/>易→难 + 逐阶段重刷伪掩码"] -.调度.-> B
    B --> H["腺体分割输出"]
```
