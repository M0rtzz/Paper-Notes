...
这篇论文要解决的是一个被隐私法规卡死的协作训练问题：多家临床机构各自有一批自闭症儿童的行为视频，合起来训练能得到泛化更好的识别模型，但 HIPAA/GDPR 禁止把原始视频送出机构。作者的思路是给隐私加两道闸门，再在闸门之内做联邦训练。具体流程是：各站点先把本地视频用 ROMP 抽成 3D 骨骼序列 $S \in \mathbb{R}^{T \times 71 \times 3}$（**第一道隐私闸门：骨骼抽象**），这一步当场把面孔、衣着、背景全部抹掉；然后各站点只在本地用 FreqMixFormer 训练动作识别模型，每轮只把模型参数（而非数据）上传，由中心做加权聚合（**第二道隐私闸门：联邦去中心化优化**），再把全局模型回传，各站点用 APFL 自适应混合成个性化模型；如此迭代到收敛。骨骼抽象负责"传出去的东西不含身份信息"，联邦聚合负责"原始数据根本不出站点"，两者叠加才覆盖最严的合规要求，而 APFL 则进一步解决站点间分布异质带来的性能塌陷。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["各临床站点本地视频<br/>（含面孔/衣着/环境）"] --> B["骨骼抽象化层（第一层隐私）<br/>ROMP 抽 71 个 3D 关键点 S∈R^(T×71×3)"]
    B --> C["FreqMixFormer 骨干<br/>DCT 频域注意力识别重复性动作"]
    C -->|"每轮仅上传参数更新 Δw（数据不出站点）"| D["联邦去中心化优化（第二层隐私）<br/>按样本量加权 w^(t+1)=Σ(n_i/n)(w^t+Δw_i)"]
    D -->|"回传全局模型 w"| E["自适应个性化联邦 APFL<br/>v_i=α_i·u_i+(1−α_i)·w，α_i 在线学习"]
    E -->|"迭代 30 轮通信"| C
    E --> F["各站点个性化识别模型"]
```
...
**3. 联邦去中心化优化：把训练范式从"数据汇聚"翻成"模型流动"，作为第二道隐私闸门**

即便骨骼抽象掉了外观信息，运动轨迹本身仍是一种生物特征"指纹"，把大规模骨骼数据汇聚到单一机构依然有再识别风险、也踩合规红线。作者用联邦学习把训练范式从传统的"数据找模型"（data-to-model）翻成"模型找数据"（model-to-data）：采用服务器-客户端架构，每轮通信中心先广播当前全局 FreqMixFormer 参数 $w^t$，各站点 $i$ 在本地私有数据上做 $K$ 步 SGD，只把参数更新 $\Delta w_i^t$ 回传，中心按各站点样本量加权聚合

$$w^{t+1}=\sum_{i=1}^N \frac{n_i}{n}\left(w^t+\Delta w_i^t\right)$$

这样原始数据和骨骼序列始终留在站点的安全基础设施内，离开站点的只有匿名化的模型参数，构成第二道隐私防线。它和第一道骨骼抽象正好分工——骨骼抽象解决"人脸等生物标识"，联邦优化解决"数据驻留"这个行政瓶颈，两者叠加严格对齐 HIPAA 的 "privacy-by-design" 与 GDPR 的"数据最小化"原则。

**4. 自适应个性化联邦学习（APFL）：让每个站点自己决定多信任全局模型**

这是性能上最关键的一环。三个治疗主题（机器人辅助、韵律活动、瑜伽）的行为分布差异极大，属于强非 IID 场景，朴素的 FedAvg 把所有站点拉去拟合同一个全局模型，结果平均只有 70.30%，比各站点单独本地训练（82.61%）还低逾 12 个百分点。APFL 的做法是不强求单一全局模型：每个站点 $i$ 维护一个个性化模型，它是本地模型 $u_i$ 和全局模型 $w$ 的凸组合
...
直觉上，$\alpha_i$ 衡量"这个站点该在多大程度上相信自己的本地数据 vs 全局共识"。实验里 $\alpha$ 初始化为很小的值（0.01）以给一个强全局先验，再随训练逐渐升高，对应模型先借全局知识打底、再慢慢吸收本地特异性——这条演化轨迹本身也给了临床场景一点可解释性。正因为每个站点能自适应地落到不同的全局/本地配比上，APFL 才能在这种高度异质的分布下稳住，而不像 FedAvg 那样被异质性拖垮。作为对照，作者还评估了另外两种结构化个性化方案：FedBN 把 BatchNorm 层留在本地、只聚合卷积/线性权重，FedPer 把分类头留在本地、只聚合骨干——但实验显示二者在这种高维时序骨骼数据上都不如 APFL 的整体自适应混合稳定。

### 损失函数 / 训练策略

分类用标准交叉熵损失。联邦训练跑 30 轮通信，前 5 轮做 warmup 稳定早期收敛；每轮每个站点做 $K=1$ 个本地 epoch，本地优化器用 SGD（学习率 0.1、weight decay $5\times10^{-4}$、momentum 0.9）。聚合用上文设计 3 的样本量加权平均。作为对比基线，FedProx 在本地目标上额外加一个近端正则项 $\frac{\mu}{2}\lVert w - w^t \rVert^2$，把本地更新往全局模型拉近以缓解站点间的异质性，但实验里它只追平、未能超过本地训练基线。
