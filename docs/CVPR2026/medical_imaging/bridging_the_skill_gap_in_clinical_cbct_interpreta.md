...
### 整体框架
这篇工作要解决的不是"再刷一个新的报告生成模型"，而是把口腔颌面CBCT的报告生成从无到有地落到临床里：先攒一份够大的配对数据，再训一个能直接吃三维CBCT体积、吐中英双语结构化报告的系统，最后用一套贴着真实临床的多层级评估去量化它到底帮了哪些医生、帮了多少。整条线是"数据集 → 报告生成系统 → 临床评估"三段。值得注意的是系统并不以"全自动替代医生"为目标，而是嵌进放射科的日常流程：AI先出一版草稿（direct AI draft），医生在草稿上审阅、修改，得到最终的协作报告（collaboration report）——评估也正是围绕"草稿"和"协作报告"两种产物分别展开的。

### 关键设计

**1. 大规模口腔颌面CBCT-报告配对数据集：在一个几乎没有公开数据的细分领域先把地基打出来**

口腔颌面CBCT之所以一直没有像样的报告生成工作，根子在于数据——它是三维体积、解读复杂、标注又贵，现有公开数据集几乎不含CBCT与报告的配对。作者从真实临床环境收集并标注了约7,408例CBCT-报告配对，覆盖55种口腔疾病实体（龋齿、牙周病、根尖周炎、阻生齿、颌骨囊肿等），跨多种采集设备与环境设置，并做成中英双语标注、由专业放射科医生审核与标准化。这个规模在口腔颌面CBCT这个细分领域属于首次达到的量级，而55种疾病的覆盖面直接决定了下游模型能不能扛住临床里五花八门的病例——数据的广度本身就是这篇工作最硬的贡献。

**2. 双语端到端报告生成系统：直接从三维体积出符合临床规范的结构化报告**

有了数据，第二步是把"CBCT体积 → 报告"这条路打通。系统直接接收CBCT三维体积作为输入，经特征提取与解码后生成文本报告，输出按放射科标准格式组织成"发现（findings）+ 结论（impression）"，并支持中英双语。走端到端而非多阶段pipeline，是为了避开分阶段方案里上游错误一路传到下游的问题；双语则是为了让系统在不同语言环境的临床里都能直接用上。结构化的输出格式也不是为了好看——它要能无缝塞进医生现有的写报告工作流，医生才愿意在它上面改。⚠️ 具体的体积编码方式（分片 vs 全局编码）、骨干网络与解码器结构，论文全文不可获取，以原文为准。

**3. 多层级临床评估框架：用贴着临床的分层评估，量化AI到底帮了谁**

如果只报一个BLEU/ROUGE，根本看不出这套系统的临床价值，更看不出它对不同水平医生的差异化作用。作者把评估拆成三层：(a) 自动指标，先量AI草稿的文本质量；(b) 放射科医生中心评估，由同行审报告的准确性、完整性与规范性；(c) 临床医生中心评估，从临床决策角度判报告好不好用。更关键的是评估不报平均值，而是把医生按初级、中级、高级分层统计——正是这种分层，才让"AI把初级医生抬到接近中级、把中级抬到接近高级、给高级医生补漏"这种结论变得可见。这套"自动指标 + 放射科评审 + 临床评审 + 经验分层"的框架本身也能迁移到其他医学AI系统上。

### 损失函数 / 训练策略
基于7,408例配对数据做端到端训练，从CBCT体积到文本报告整体优化，并同时优化中英文输出的生成质量。⚠️ 具体网络结构、损失函数与训练超参受限于论文全文不可获取，以原文为准。
