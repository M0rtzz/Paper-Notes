...
flowchart TD
    A["可用模态输入 {X_j}<br/>部分掩码模拟缺失模态"] --> B["结构编码器 E_str（含 HFIB）<br/>可学习高斯滤波提高频→残差注入，得多尺度结构 S_j^l"]
    A --> STY["风格编码器 E_sty<br/>抽模态特定风格 S_j"]
    B --> MMSF["多模态结构融合 MMSF<br/>Sigmoid 门控按模态加权 → 各尺度 F_l"]
    MMSF --> SEG["共享分割解码器 D_seg<br/>辅助分割任务，逼结构特征模态不变"]
    MMSF --> MSSE["多尺度结构增强 MSSE<br/>低尺度交叉注意力补进最高层 → 统一结构 F_s"]
    STY --> REC["重建解码器 D_rec（F_s, S_j）<br/>风格一致性 + 结构感知损失监督解耦"]
    MSSE --> REC
    MSSE --> LDM["潜在扩散 LDM<br/>以 F_s 为条件去噪生成"]
    LDM --> OUT["合成的缺失模态"]
```
