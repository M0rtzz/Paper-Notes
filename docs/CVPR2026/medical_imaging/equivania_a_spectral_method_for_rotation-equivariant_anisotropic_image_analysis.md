...
EquivAnIA 要做的事是：给一张图，准确估计它的角度功率分布 $S(\theta)$ 和主方向，而且这个估计在图像被旋转后必须随之旋转（旋转等变），不能像传统 binning 那样旋一下结果就乱。整条流程分三步走。先做**稳健 PSD 估计**：对矩形（非圆盘支撑）图像先乘上一个径向对称窗再做 FFT 得到周期图，把"图像角落随旋转进出"的隐患先压住。接着做**定向滤波**：不再把频率点硬分进角度 bin，而是用一族在频域中平滑定义的滤波器，对 PSD 沿每个方向做连续带权平均，得到每个方向的能量响应 $\rho(\theta)$，取 argmax 即主方向。最后是**应用**：单图分析直接输出主方向/角度分布，做角度配准时则比较两图主方向之差。整套方法没有任何可学习参数。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["输入图像<br/>矩形, 非圆盘支撑"] --> B["稳健 PSD 估计<br/>径向窗 w⊙x → FFT 周期图 S=|x̂|²"]
    B --> C["定向滤波器族替代 angular binning<br/>cake wavelet / ridge filter 连续带权平均"]
    C --> D["角度分布 ρ(θ) → argmax 主方向 η"]
    D -->|单图分析| E["输出主方向 / 角度分布"]
    D -->|两图配准| F["角度图像配准<br/>两候选角比 MSE 消 180° 模糊"]
```

### 关键设计

**1. 稳健 PSD 估计：径向对称窗 + 周期图，把"角落随旋转进出"和分辨率损失两个隐患先压住**

整条流程的第一步是从图像估计二维功率谱密度（PSD），这一步藏着两个会破坏旋转一致性的隐患。其一，真实图像是矩形而非圆盘，旋转时四个角落会转进/转出视野，PSD 随之漂移。处理办法是在 FFT 之前先乘一个近似圆盘支撑的径向对称平滑窗，主动丢弃角落信息——代价是损失一圈边缘数据，换来的是无论怎么旋转，参与分析的有效区域始终是那个旋转不变的内接圆盘（CT 这类本就圆盘支撑的图像可跳过加窗）。其二是 PSD 估计器的选择：作者发现直接用单张周期图 $S=|\hat{x}|^2$ 反而优于 Bartlett/Welch 这类分段平均降噪法——后者方差更小但牺牲了角度分辨率，而方向分析里分辨率比降噪更重要。这一步保证了送进后续滤波环节的 PSD 本身就是旋转稳健的。

**2. 定向滤波器族替代 angular binning：用连续频域权重消掉离散 bin 的量化误差**

拿到稳健的 PSD 后，核心一步是怎么从它读出角度分布。传统 binning 的病根在于：笛卡尔网格本身各向异性，0° 方向落进 bin 的频率点远多于 30° 方向，硬边界一刀切，导致同一图旋转后角度分布就变了。EquivAnIA 的做法是把"硬分 bin"换成"软滤波"——从一个基函数 $\phi$ 出发，通过旋转和平移生成一整族滤波器 $\phi_{v,\theta}(u) = \phi(R_\theta^{-1}(u-v))$，对每个位置-方向算出分析系数 $c_{v,\theta}$，再把同一方向上所有位置的能量积掉，得到角度分布 $\rho(\theta) = \int |c_{v,\theta}|^2\, dv$。基函数有两种选择：**cake wavelets** 在频域呈扇形覆盖，适合结构性图像（如 CT 里的清晰取向）；**ridge filters** 呈线形，适合纹理性图像（如树皮）。关键在于滤波器的频域权重是连续平滑的，没有 bin 边界那种量化跳变，而旋转输入与旋转整族滤波器是等价的，于是分析结果天然就随图像一起旋转——旋转等变性是从构造里直接得到的，而非事后近似。
