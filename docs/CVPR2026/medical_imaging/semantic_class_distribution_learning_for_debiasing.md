...
SCDL 是一个挂在现有半监督分割网络上的即插即用模块，整条链路围绕「在特征空间里给每个类建一份代理分布」展开：编码器输出 token 嵌入后，CDBA 为每个语义类维护一个可学习的高斯代理分布，并把嵌入与代理双向对齐；SAC 从标注区域抽取语义锚点，给这些随机初始化的代理一个正确的语义方向；最后从代理分布中采样、构造结构化先验注入解码器各层，让尾类也能拿到稳定的表示。整套机制不改基线架构，只在嵌入空间额外加一组约束。
