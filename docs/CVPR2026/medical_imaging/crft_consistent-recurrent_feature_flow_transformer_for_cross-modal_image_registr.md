...
具体地，一对输入图像 $(I^A, I^B)$ 先过一个共享权重的 ResNet 编码器，抽出 1/2、1/4、1/8 三个尺度的特征。先在最粗的 1/8 尺度上用 self-attention 和 cross-attention 建立全局对应，得到一个稳但糙的初始流场；再把流场带到 1/2、1/4 两个高分辨率尺度，用窗口注意力补上局部细节。最后是整篇的核心——一个跑 $N$ 轮的迭代精化回路：每轮都把当前流场作用到特征上、显式补偿仿射/尺度变形、算出对齐残差、再让残差自己引导注意力去修最差的地方，逐步把流场收敛到亚像素精度。
...
这是 CRFT 的核心创新，针对的是单次前向匹配解决不了的复杂非线性 + 大仿射变形。它把精化做成一个跑 $N$ 轮的回路，每一轮都在上一轮的流场基础上往前修一步。一轮内部依次发生几件事：先是 **Fine-Scale Feature Transformation (FSFT)**，用当前流场把特征 warp 到对齐位置；接着 **Spatial Geometric Transform (SGT)** 显式地把仿射/尺度变换建模成一个可学习模块，专门补偿那些流场难以隐式表达的大变形；warp 后的特征与目标特征相减得到**差异图**，它直观地标出了当前还有哪些区域没对齐；**Discrepancy-Guided Flow Optimization (DGFO)** 拿这张差异图当注意力权重，让网络的注意力自动压到对齐最差的区域上，而不是平摊到全图；据此做一次**残差更新 (RU)** 把流场往前推；最后 **Confidence Estimation Network (CENet)** 预测逐像素置信度，对最终流场做平滑、抑制不可靠区域的乱跳。

之所以有效，关键在"差异引导"这一步：普通迭代每轮都对整张图均匀用力，而 CRFT 让残差自己说话——哪里残差大就把注意力投到哪里，每一轮都花在刀刃上，所以收敛得又快又准（消融显示 $N=3$ 轮已基本收敛，再加收益递减）。SGT 则是另一根支柱，缺了它，遇到大角度、大尺度的仿射变换配准几乎会垮掉。
