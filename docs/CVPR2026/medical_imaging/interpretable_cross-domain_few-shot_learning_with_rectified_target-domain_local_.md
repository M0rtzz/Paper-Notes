...
这篇论文要解决的是 CLIP 跨域微调后局部对齐退化的问题：模型只盯着身体轮廓，看不到真正决定诊断的细粒度病灶。作者的思路是不引入任何 patch 级标注，而是借两条"绕一圈还能回到自己"的循环路径，把局部视觉特征重新锚到正确的文本语义上。一张图进来后，先由语义锚点（SA）模块对 patch 候选池做"先增广、后收缩"的整理，文本侧循环（T-I-T）和图像侧循环（I-T-I）分别在这个池子上施加双向一致性约束，三者产生的正则项叠加到标准交叉熵损失上一起训练。
