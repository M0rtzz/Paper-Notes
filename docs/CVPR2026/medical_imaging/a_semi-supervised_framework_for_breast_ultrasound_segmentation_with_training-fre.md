...
整体分三阶段走。第一阶段 APPG 把 GPT-5 写出的肿瘤外观描述喂给 Grounding DINO 定位、再交给 SAM 分割，为所有未标注图像免训练地生成一批初始伪标签 ⚠️（GPT-5 等模型名以原文为准）。第二阶段用少量真实标注加这批伪标签做 warmup，把训好的模型冻结成静态教师 $T^A$。第三阶段进入双教师半监督训练：冻结的 $T^A$ 和随学生 EMA 更新的动态教师 $T^B$ 同时给出伪标签，经 UEWF 逐像素融合后监督学生，AURCL 再专门盯住边界模糊区域补一刀对比学习。
...
两个教师在不同区域的可靠性并不一样——$T^A$ 可能在整体形状上更稳，$T^B$ 可能在细节上更准，简单平均会把好的和坏的搅在一起。UEWF 的做法是对每个像素分别算两教师预测的信息熵 $H^A$、$H^B$（熵低=该像素预测更笃定），再用对方的熵做逆向权重：

$$\hat{y}^F = w^A \cdot \hat{y}^A + w^B \cdot \hat{y}^B, \quad w^A = \frac{H^B}{H^A + H^B}, \quad w^B = \frac{H^A}{H^A + H^B}$$

某像素上谁更不确定（熵高），分母里它的项就大、自己拿到的权重就小；谁更笃定，权重就更高。这样融合是逐像素自适应的，且整套机制不引入任何额外可学参数，算一次熵就能用。

**4. AURCL：把"模型拿不准"的边界区域反转成可学信号，用对比学习补强**

标准 SSL 把高置信区域吃得很透，但对边界模糊这类模型"拿不准"的区域几乎使不上劲，而这恰恰是 BUS 分割最难的地方。AURCL 专门挖这些难例：先对学生预测跑多次 Monte Carlo Dropout，用逐像素方差当不确定性度量，方差越大越不确定；再以阈值 $\tau$ 筛出高不确定性区域。关键一步是"概率反转"——对这些低置信像素把预测概率取 $1-p$，直觉是"模型既然不确定地把这里判成前景，那反转后的表示反而更可能落在背景一侧"。最后做 patch 级对比学习：把反转后的低置信 patch 特征与高置信的前景/背景 patch 拉近对应类、推远异类。换句话说，它没有简单地把不确定区域丢掉（以往不确定性常被用来过滤伪标签），而是把不确定性当成额外正信号回收利用，在特征空间约束边界表示，补上像素级损失够不到的地方。
...
4. **AURCL 收尾**：MC Dropout 标出这段边界仍是高不确定区，AURCL 把它的概率反转、做对比学习，逼模型在特征空间把这段边界往正确类别那边推。
