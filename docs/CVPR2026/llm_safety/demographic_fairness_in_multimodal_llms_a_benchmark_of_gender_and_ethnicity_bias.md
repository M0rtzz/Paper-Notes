...
**2. 多维度公平性指标：单一数字会骗人，用一组互补指标交叉印证。**
...
**3. 零样本评估：测的是"开箱即用"的公平性。**

所有 9 个 MLLM 都在零样本设置下评测，不做任何微调，也不喂上下文示例，全部用同一套文本提示格式。这样测出的偏见反映的是模型部署时真实会暴露的状态，而非经过特调粉饰后的理想值——对"MLLM 能不能直接顶替传统人脸系统"这个现实问题，零样本才是诚实的答案。
