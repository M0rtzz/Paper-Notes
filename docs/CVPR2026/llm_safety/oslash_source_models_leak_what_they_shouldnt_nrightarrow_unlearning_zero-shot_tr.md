...
整篇方法的思路可以串成一条线：既然真实样本拿不到，就**用源模型自己编码的判别信息反向生成**遗忘类别的代理样本（对抗遗忘样本生成）；有了代理样本，再给它们配上一套**不把概率压成 0、而是重新摊到保留类别上**的软标签（重缩放标签策略），让遗忘训练的梯度稳定可用；最后把"适应目标域"和"遗忘源域类别"这两个互相冲突的目标放进一个**交替优化的对抗框架**里，让二者达到平衡。最终输出的模型既能在目标域上正常分类，又对源域独有类别"失忆"。
...
**2. 重缩放标签策略：别把概率压成 0，而是摊给保留类别。**

拿到遗忘样本后还要给它一个监督信号。最直觉的做法是把遗忘类别的标签直接设为 0，但这会把 softmax 分布压垮，造成梯度消失或训练震荡。重缩放标签换了个思路：设遗忘类别集合为 $\mathcal{F}$、保留类别集合为 $\mathcal{R}$，对每个遗忘样本的伪标签，把原本落在 $\mathcal{F}$ 上的那部分预测概率，按保留类别的相对比例重新均匀摊到 $\mathcal{R}$ 上。模型由此学到的不是"这个类别不存在"，而是"这些特征不该归到遗忘类别，应该分散到保留类别里去"。概率分布的完整性被保住了，遗忘的梯度信号既稳定方向又对——消融里去掉它，保留类别准确率明显下滑。
...
总损失由三部分构成：域自适应损失 $\mathcal{L}_{\text{adapt}}$ 是标准 SFDA 项（信息熵最小化或邻域一致性），负责把模型拉向目标域分布；遗忘损失 $\mathcal{L}_{\text{forget}}$ 在生成的遗忘样本上用重缩放标签算交叉熵，引导模型在遗忘类别上"变笨"；保留损失 $\mathcal{L}_{\text{retain}}$ 守住保留类别的性能不被遗忘操作误伤。训练按"先适应、再遗忘"两阶段交替推进，对应上面对抗框架的交替更新。
