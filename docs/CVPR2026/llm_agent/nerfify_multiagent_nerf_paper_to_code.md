...
### 整体框架
Nerfify 分四阶段把 NeRF 论文转成代码：(1) CFG 形式化与上下文学习，把 Nerfstudio 架构写成 CFG、构建知识库 $\mathcal{K}$；(2) 组合式依赖解析，遍历引用图谱恢复隐含依赖；(3) 语法引导的仓库生成，用 GoT 按拓扑序生成多文件仓库；(4) 视觉驱动反馈，用训练运行的视觉分析迭代改进实现质量。四个阶段对应下面四个关键设计。
