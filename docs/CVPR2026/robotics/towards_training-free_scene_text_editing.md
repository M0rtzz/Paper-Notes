...
TextFlow 建立在 FLUX-Kontext 的流匹配架构之上，输入是源图像加上源/目标两段文本描述，整个编辑只用 50 步去噪、不动任何权重。它的核心判断是：扩散过程不同时间步的信噪比天差地别，早期扰动大、决定全局结构与风格，后期细节渐显、决定字符长什么样——所以与其全程用一套机制硬扛，不如把去噪轨迹切成两半分而治之。前半段交给 FMS，它把源图像编码进潜空间，靠噪声注入与差分几何变换把源的结构约束"焊"进目标的生成轨迹里，守住风格；后半段交给 AttnBoost，它从 DiT 双流 transformer 块里抽出文字到图像的注意力图并放大，给后期的字符渲染补上精确的空间引导。
...
**3. 两阶段解耦：让每个机制只管它擅长的那一半**

前两个设计能各司其职，前提是把去噪过程按时间步切成两段，而不是全程混在一起跑。TextFlow 正是据此把 50 步去噪自然分界：前段噪声重、适合 FMS 建立全局结构与风格，后段细节渐显、适合 AttnBoost 精修字符。把它单列为一个设计点，是因为强行在全程统一处理会顾此失彼——早期就压注意力会破坏结构，后期还在校结构则错过了写字的窗口。步数本身也是个平衡点：实验里 24 步质量不够、70 步收益递减还更慢，50 步在质量和效率间最稳。
