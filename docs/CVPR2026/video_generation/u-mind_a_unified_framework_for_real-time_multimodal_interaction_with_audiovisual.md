...
训练分两阶段：Stage 1 是排练驱动的基础预训练，把多模态对齐任务和纯文本推理数据混在一起练，让模型学会新模态的同时不忘推理；Stage 2 是 CoT 风格的指令微调，明确教它「先想后做」。下面四个设计依次解决：模态怎么统一进 token 空间、怎么学新模态而不遗忘、怎么让推理引导生成、以及怎么把 token 变回视频。
...
直接拿多模态数据去 SFT 一个 LLM，低层的模态适配（学会发声、摆姿势）会和高层的推理规划抢容量，结果往往是模型学会了动嘴动手却忘了怎么想——典型的灾难性遗忘。U-Mind 不是「先训模态再补推理」，而是在预训练时就把两类目标拌在一起持续优化：一边练模态对接任务 T2M（文本→动作）、S2M（语音→动作）、T2S（文本→语音），一边穿插高质量纯文本推理数据（OpenOrca）作为「排练」，让推理这条核心能力在整个训练过程中始终被反复唤醒。配套还有一个分段对齐策略：按语音的韵律和停顿边界把输入切成小段，训练时随机重组这些分段配对，逼模型学到的不是整句死记，而是细粒度的语音-动作时序对应，跨模态同步因此更稳。

**3. 文本优先解码：让符号推理跑在连续模态生成前面，既保住脑子又当生成蓝图。**

如果模型一上来就直接吐语音和动作 token，语言层面的理解和规划就被绕过了——消融里这会让回复的相关性几乎归零。文本优先解码强制每个回复都以一段纯文本的 `<think>...</think>` 内部推理开头，想清楚要表达什么、怎么组织，之后才依次生成文本回复、语音、动作。它和普通 CoT 的区别在于：这里的思维链不只是为了答对，更是后续多模态生成的规划蓝图——动作和语音是顺着这段文本规划长出来的，而不是各自为政，跨模态语义因此天然对齐。

**4. 实时视频渲染：把 token 化的动作和语音落地成逼真的说话人视频。**
