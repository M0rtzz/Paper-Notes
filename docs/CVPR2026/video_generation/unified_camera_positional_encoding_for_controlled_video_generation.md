...
UCPE 想解决的是一个很具体的痛点：让视频生成模型听懂"相机怎么动、镜头是什么样"的指令，而且这个指令要能跨针孔、鱼眼、全景这些差异巨大的相机模型通用。它的做法是先把任意相机统一抽象成一个"像素 token → 观测光线"的映射，再把相机几何拆成两路注入预训练的扩散 Transformer（Wan 模型）——一路是**相对光线编码**，让每个 token 在光线空间里做相对变换，天然兼容任意镜头畸变；另一路是**绝对朝向编码**，用一张重力对齐的方向图锚定 pitch/roll。两路信息都不直接改原网络，而是经一个**空间注意力适配器**旁挂进去，从而保住预训练学到的画面先验。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    IN["相机几何参数<br/>6-DoF 位姿 + 内参 + 镜头畸变"]
    RAY["光线映射 Φ<br/>每像素 token → 光线 (o, d)"]
    IN --> RAY
    RAY --> REL["相对光线编码<br/>局部光线坐标系 → 几何算子在注意力做光线级相对变换"]
    RAY --> ABS["绝对朝向编码（Lat-Up Map）<br/>纬度图 + 上方向图锚定重力 → pitch/roll"]
    REL --> ADP["空间注意力适配器<br/>LoRA 旁路 + 零初始化，<1% 可训练参数"]
    ABS --> ADP
    ADP --> DIT["预训练扩散 Transformer（Wan）<br/>原自注意力不动，旁路条件分支相加"]
    DIT --> OUT["相机可控视频"]
```
