...
**4. 统一设计语言：把四个经验技巧还原成三个几何优化问题的近似解**

前面三个常量 $L_t^*$、$f_t(\lambda)$ 和抑制场 $S_{k,t}$ 有一个共同好处——它们都只依赖噪声调度和patch协方差谱、不用跑一次模型就能算出来，于是这套几何量天然构成了调度与损失设计的统一语言。论文最关键的"实用"落点正在这里：四个原本各自拍脑袋调出来、互不相干的经验技巧，被证明恰好是这套语言里三个显式几何优化问题的近似解，从而第一次有了统一的几何依据。其中收缩阈值的均衡是个绕不过的核心——论文先证明严格均衡 $L_t^*$ 在 $T\geq 3$ 时根本不可能（Thm 28），所以实用做法只能松弛成"最小化 $\mathrm{Var}_t(L_t^*)$、同时不让任何一步成为过弱的环节"。四个技巧对应如下：

- **cosine schedule offset**：直接服务于上面的均衡目标，把最弱环节 $L_1^*$ 从 0.00079 抬到 0.00321（约 4 倍），消除那个几乎没有收缩余量的危险步。
- **Min-SNR 损失加权**：对应让 Kaplan-Yorke 维度的逐步增长在各步之间均衡化（与信息增益均衡等价）。
- **分辨率相关 logSNR shift**：对应保持 Moran 比值不变，使不同分辨率下吸引子的分形几何一致。
- **Align Your Steps（AYS）采样**：对应把有限的采样步集中在 $L_t^*$ 最小（最难收缩）的区间，与本文从收缩 margin 出发的推导一致。

这一节是全文标题"made Practical"的兑现：它把前三条纯分析性的几何量反过来当成设计准则，让一堆经验 trick 不再是玄学。
