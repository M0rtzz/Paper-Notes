...
flowchart TD
    A["输入：类别文本语义 + 骨架序列"] --> B["邻居感知语义校准<br/>Top-k 邻域图聚合给有偏语义纠偏"]
    A --> C["骨架特征提取（脚手架）"]
    B --> D["几何一致性对齐<br/>双 VAE 匹配两模态分布均值 / 方差"]
    C --> D
    D --> E["无噪声开放式流分类器<br/>学语义分布→骨架分布的 token 级速度场"]
    E -->|"逐候选类算速度误差，取最小"| F["输出：未见类动作预测"]
```
