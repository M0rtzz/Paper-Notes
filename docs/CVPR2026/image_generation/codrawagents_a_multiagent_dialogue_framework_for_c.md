...
description: >-
  [CVPR 2026 Findings][图像生成][compositional T2I generation] 提出coDrawAgents交互式多智能体对话框架，Interpreter、Planner、Checker、Painter四个专业智能体闭环协作，以分治策略按语义优先级逐组增量规划布局，基于画布视觉上下文接地推理并显式纠错，在GenEval上以0.94 Overall Score大幅领先GPT Image 1（0.84），在DPG-Bench上达85.17 SOTA。
tags:
  - "CVPR 2026 Findings"
  - "图像生成"
...
flowchart TD
    A["输入文本"] --> B["Interpreter：分治策略<br/>拆对象 + 按语义优先级分组"]
    B -->|"简单场景"| C["layout-free 模式<br/>单次 T2I（Flux）"]
    B -->|"复杂场景：逐组迭代"| D["Planner：VCoT 三步规划<br/>看画布 → 上下文规划 → 物理约束"]
    D --> E["Checker：两阶段检查-修正<br/>当前轮布局 + 历史布局回溯"]
    E --> F["Painter：即插即用渲染<br/>把本组对象画上画布（3DIS）"]
    F -->|"更新后画布作为下一轮视觉输入"| D
    F -->|"所有优先级组画完"| G["最终图像"]
    C --> G
```
