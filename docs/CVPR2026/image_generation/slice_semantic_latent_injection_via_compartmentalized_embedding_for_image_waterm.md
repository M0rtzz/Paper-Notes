...
description: >-
  [CVPR 2026][图像生成][扩散模型水印] 提出 SLICE 语义水印框架，将图像语义分解为主体/环境/动作/细节四个因子并绑定到初始高斯噪声的不同空间分区，实现不仅可检测水印存在还可定位语义篡改的三状态验证机制，对最强 CSI 攻击的攻击成功率仅 19%（SEAL 为 81%）。 领域现状： 扩散模型水印技术从后处…
tags:
  - "CVPR 2026"
  - "图像生成"
  - "扩散模型水印"
...
flowchart TD
    A["输入图像"] --> B["VLM 提取四维语义<br/>主体 / 环境 / 动作 / 细节"]
    B --> C["分区化语义注入<br/>keyed hash 把每个因子绑到独立潜空间分区"]
    C --> D["扩散模型生成水印图像"]
    D -.传播 / 攻击.-> E["可疑图像"]
    E --> F["重抽语义 + 重建参考噪声"]
    F --> G["DDIM 反演取实际噪声<br/>逐分区算匹配率 mk 与全局 mg"]
    G --> H{"三状态检测"}
    H -->|"mg≥τg 且各 mk≥τk"| I["完好"]
    H -->|"mg≥τg 但某 mk<τk"| J["局部篡改<br/>点名被改因子"]
    H -->|"mg<τg"| K["未水印"]
```

### 关键设计

**1. 分区化语义注入：让不同语义因子各守潜空间的一块地盘**

全局语义绑定的死穴在于"整张图共用一个语义指纹"，攻击者只动主体、保持其余不变，整体语义看上去仍连贯，水印就被骗过去了。SLICE 的破局点是把语义拆成四个因子——主体、环境、动作、细节，记为 $\mathcal{K} = \{sub, env, act, det\}$，由 VLM 配一套元提示 $\mathcal{P}_{meta}$ 从图像里抽出四维描述 $\mathcal{S} = \{s_k\}$。关键一步是把潜空间网格切成四个互不重叠的区域 $\mathcal{R}_k$，每个区域只承载一个因子的水印信号：对落在 $\mathcal{R}_k$ 里的每个位置 $p$，先用文本编码器把语义 $s_k$ 编码、再过一个带密钥的哈希得到该位置的噪声值

$$\mathbf{z}_T(p) = H\big(E(s_k),\, p,\, \sigma\big), \quad p \in \mathcal{R}_k.$$

这样一来，改主体只会扰动 $\mathcal{R}_{sub}$ 那一块的噪声匹配，其余三块原封不动——局部篡改第一次有了"局部痕迹"，而不再被全局平均掉。

**2. 三状态检测：把"有没有水印"升级成"哪块被动过"**

既然语义被分区承载，检测就不该只回答"是/否"。SLICE 从可疑图像重抽语义、重建参考噪声，再用 DDIM 反演拿到实际噪声 $\mathbf{z}_{inv}$，逐位置算距离 $d(p) = \|\mathbf{z}_{inv}(p) - \hat{\mathbf{z}}_T(p)\|_2$，进而得到每个区域的匹配率 $m_k$ 和全局匹配率 $m_g$，按两道阈值 $\tau_g, \tau_k$ 落到三种状态：全局达标且四块都达标（$m_g \geq \tau_g$ 且所有 $m_k \geq \tau_k$）判为**完好**；全局达标但某些块掉线（$m_g \geq \tau_g$ 但部分 $m_k < \tau_k$）判为**局部篡改**，而且掉线的那一块直接点名是哪个语义因子被改了；全局都没过（$m_g < \tau_g$）则判为**未水印**。第二种状态正是传统二元判定给不出的信息——它不只说"图被动过"，还指到具体语义维度，这对溯源和版权取证是实打实的增量。

**3. 理论安全保证：定位的鲁棒性和误判率不靠实验背书，而是给出界**

光有实验数字不够，SLICE 还把两件事证成了定理。Theorem 4.3 在"DDIM 反演误差有界 + 语义因子可分离"的假设下给出定位的可靠性：未被篡改的因子匹配率有下界 $m_k \geq 1 - \beta_k - \gamma_k$（$\beta_k$、$\gamma_k$ 是反演误差与编码扰动项），被篡改因子的匹配率则显著下滑，于是三状态判定不会把好的判成坏、也不会漏掉真篡改。Theorem 4.4 管的是另一头——未水印图像被误认成有水印的概率，随潜空间位置数指数衰减，意味着只要分辨率够、参与匹配的位置足够多，误接受率可以压到任意小。

> ⚠️ 上述定理编号、界的具体形式以原文为准。
