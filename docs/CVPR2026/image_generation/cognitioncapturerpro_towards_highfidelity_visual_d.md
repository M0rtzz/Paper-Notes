...
flowchart TD
    IMG["刺激图像"] --> UM["不确定性加权掩蔽（UM）<br/>仿凹视觉模糊，强度随对齐水平动态调节"]
    EEG["EEG/MEG 信号"] --> ENC["四个模态专用编码器<br/>映射到 图像/文本/深度/边缘 嵌入"]
    ENC --> FUS["多模态融合编码器<br/>跨模态 Transformer 整合四路 → z_fus"]
    UM -->|动态对齐目标| STH
    FUS --> STH["共享主干与模态头对齐（STH-Align）<br/>4层MLP主干 + 模态头 → CLIP 图像空间"]
    STH --> REC["SDXL-Turbo + IP-Adapter<br/>推理时还原重建图像"]
```
