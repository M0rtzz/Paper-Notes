...
description: >-
  [CVPR 2026][图像生成][长尾识别] 针对"用扩散模型 + 头→尾特征迁移补长尾数据"会暗中把头类特征泄漏到尾类、模糊决策边界的问题，本文先用三个指标把这种"边界模糊"量化出来，再提出 DBG：用对抗去类化噪声把样本推到决策边界附近、重标成 $k$ 个最易混淆类，并用分类器驱动的双路清洗丢掉有害样本，在 CIFAR-LT 上对所有生成式 baseline 都能降低类间重叠、提升尾类与整体精度。
tags:
  - "CVPR 2026"
  - "图像生成"
  - "长尾识别"
