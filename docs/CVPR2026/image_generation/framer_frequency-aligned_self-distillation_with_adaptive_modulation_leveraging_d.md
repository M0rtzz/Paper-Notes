...
### 损失函数 / 训练策略
最终训练目标在原噪声预测损失上叠加 FRAMER 辅助项：$\mathcal{L}_{total} = \mathcal{L}_{noise} + \sum_i \mathcal{L}_{FRAMER}^{(i)}$，其中每层的 FRAMER 项是经 FAW 和 FAM 门控后的 IntraCL + InterCL 加权和。FAM（频率对齐门控）是另一道闸：它用学生-教师的对齐分数（经 ReLU 截断并 stop-gradient）去缩放蒸馏强度，当某个早期层还不成熟、与教师差得太远时把它的蒸馏信号压下去，避免不成熟的层被强行对齐而崩溃。所有辅助头只在训练期存在，测试时全部移除，因此对推理不增加任何开销。
