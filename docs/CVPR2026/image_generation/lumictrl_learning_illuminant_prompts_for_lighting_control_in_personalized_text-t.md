...
**1. 物理光照增强：用 Planckian locus 凭空造出配对训练数据。**

学习光照 prompt 的第一道坎是数据——你需要同一个概念在 tungsten、daylight、shade 等不同色温下的成对照片，但这种数据现实中几乎拿不到。作者干脆用物理模型合成：沿黑体辐射轨迹（Planckian locus）取 7 个标准色温——tungsten (2850K)、3300K、fluorescent (3800K)、4500K、cloudy (6500K)、7000K、shade (7500K)，每个色温换算成一个 RGB 增益向量，再用 von Kries 模型（一个对角矩阵）对整张图做全局色彩偏移，就得到了该光照下的训练变体。这一步叫 Flat Light Adaptation，它故意做得很糙——只是均匀地给全图染色，不区分前景背景。糙的代价由后面的掩码损失来兜底：模型只被要求精确还原前景光照，背景的不真实之处交给扩散先验去自然修正。

**2. 边缘引导 Prompt 解耦：用冻结的 ControlNet 当结构锚，逼 prompt 只学颜色。**

个性化方法的老毛病是 prompt 会把训练图的具体布局也一起记下来——生成时照搬物体位置、甚至凭空多出或抹掉东西。这里要学的光照 token 同样有这个风险：如果不加约束，它很容易把"这张训练图长什么样"也编码进去，于是光照和结构纠缠在一起。作者的做法是在训练时挂一个预训练、参数冻结的 ControlNet，喂给它 Canny 边缘图。结构信息既然已经由 ControlNet 这条旁路提供，光照 token 的梯度就没有动机再去记结构，只能专注于颜色/光照的变化。关键是这个 ControlNet 只在训练时在场——推理时直接丢掉，所以部署端不需要任何边缘图，干净利落。

**3. 掩码重建损失：把光照学习的注意力压到前景上。**

Flat Light Adaptation 给全图施加的是均匀变换，可真实光照在不同区域因材质、阴影、反射而各异，硬要模型逐像素复刻这种"假"的均匀背景反而会学坏。掩码损失给出的折中是：只对前景严格、对背景放松。形式上

$$\mathcal{L}_{mrl} = (1-\lambda)\cdot\mathcal{L}_{rec}\cdot(1-\mathcal{M}) + \lambda\cdot\mathcal{L}_{rec}\cdot\mathcal{M}$$

其中 $\mathcal{M}$ 是目标概念的前景掩码，$\lambda$ 调节前景与背景的权重配比。把 $\lambda$ 调大，前景像素的损失被放大，模型被迫精确对齐前景物体的光照颜色；而背景因为权重被压低，扩散模型可以放手用先验去填一个看着合理的光照——这正是论文所说的 Contextual Light Adaptation，前景准、背景顺，两头都照顾到。
