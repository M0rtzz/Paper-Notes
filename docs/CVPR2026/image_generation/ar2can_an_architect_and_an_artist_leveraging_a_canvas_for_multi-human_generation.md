...
Ar2Can 想根治的是多人文生图里最顽固的几种失败：身份融合、身份交换、人数出错。它的核心判断是，这些错误源于模型同时在做两件本该分开的事——既要决定"每个人站在哪"，又要决定"每个人长什么样"，空间结构和外观一旦纠缠，身份就会糊到一起。于是 Ar2Can 把生成拆成串行的两阶段，中间用一张 canvas 衔接：先由 Architect 读文本提示 $p$ 和 $N$ 张参考身份图 $\{I_{ref,1}, ..., I_{ref,N}\}$，预测出 $N$ 个 bounding box 组成的空间布局 $\mathcal{L} = \{b_1, ..., b_N\}$；再把每张参考人脸按预测位置贴到一张空画布上拼成 canvas；最后 Artist 以 canvas、参考图和文本为条件渲染成图。Architect 只管"谁在哪"，Artist 只管"画得像不像"，纠缠被从结构上切断。
...
**4. Token Sharing & Dropping：让 canvas 的冗余既省算力又逼模型学遮挡。**

canvas 里大片是非信息背景，Ar2Can 干脆丢掉这些区域的 token，平均把 token 数砍掉一半。更巧的是处理重叠：当两个人脸 box 互相重叠时，它让重叠区共享同一套 RoPE 位置编码——位置编码一旦撞车，模型就没法靠坐标把两张脸硬摆开，只能去学真正的遮挡关系（谁在前、谁在后、怎么重排），而不是简单粘贴。
