...
推理时，模型吃进若干张参考图像和一个正则化的模板体网格：图像被通用视觉编码器切成 token，模板网格切成几何 token，二者一起送进一个大 Transformer 做融合。融合后的特征分两路解码——正则分支输出一套「中性姿态」下的高斯属性，修正分支再根据驱动信号（表情编码 + 身体/手部位姿）输出对这些属性的偏移。两路相加得到驱动后的高斯，经线性蒙皮（LBS）变换到目标位姿，最后用 3D 高斯溅射渲染成图。预训练跑在约 100 万野外视频上，后训练换到数千身份的多视图工作室数据上（⚠️ 具体规模以原文为准）。

### 关键设计

**1. 可扩展双分支架构：让工作室数据和野外数据走同一条管线。**

野外数据和工作室数据的形态天差地别——前者只有随手拍的视频、没有几何或纹理贴图，后者有标定好的多视图。要把两者灌进同一个模型训练，架构就不能依赖任何「高质量条件输入」。LCA 的图像 token 来自一个通用视觉编码器，几何 token 来自正则化的模板体网格，两者都是任何数据源都能现成提供的廉价信号；Transformer 骨干采用混合注意力——全局注意力和逐图像自注意力交替堆叠，这样既能在多张输入图之间交换信息、又能容纳可变数量的参考图。解码端拆成正则分支（输出与姿态无关的中性高斯属性）和修正分支（在驱动信号下输出属性偏移），正是这种「中性 + 偏移」的分解，让同一套权重既能在静态野外帧上学外观先验，又能在动态工作室序列上学表情。
...
表情是头像最关键的控制维度，但传统参数化面部模型（如 blendshape 系数）粒度偏粗，撑不起工作室级的微表情。LCA 改用类似 FACS 的自监督方式直接从数据里学一套面部表情潜在编码，把它作为修正分支的驱动信号；再叠上 SMPL-X 提供的身体与手部位姿，就能从面部一路控制到手指级别。因为这套编码是自监督学出来的、不被预定义的 blendshape 基底框死，修正分支才能表达出参数化模型覆盖不到的细节偏移。

### 损失函数 / 训练策略

训练目标以 3DGS 渲染损失为主（L1 + D-SSIM），再加感知损失和身份保持损失约束外观与身份一致性。两阶段共用这套目标：预训练在约 100 万野外视频上、后训练在多视图工作室数据上（⚠️ 具体损失权重与数据规模以原文为准）。
