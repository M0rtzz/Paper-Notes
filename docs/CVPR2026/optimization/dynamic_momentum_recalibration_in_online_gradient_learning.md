...
### 整体框架
SGDF把固定动量系数换成一个**每步在线计算的时变增益** $K_t$，让优化器自己决定这一步该多信任历史动量、还是多信任当前梯度。整套流程仍然挂在标准 SGD+动量上：先用 EMA 维护一阶矩 $\hat{m}_t$ 和"创新"方差 $\hat{s}_t$，再根据当前梯度 $g_t$ 与动量估计的偏离量 $(g_t-\hat{m}_t)$ 算出最优增益 $K_t$，用它把动量估计和当前梯度做一次线性融合得到 $\hat{g}_t$，最后用 $\hat{g}_t$ 更新参数。关键在于 $K_t$ 不是预设常数，而是由两个不确定源的方差比值实时定出来的——这正对应 Kalman Filter 里"预测+观测"的融合思想。
