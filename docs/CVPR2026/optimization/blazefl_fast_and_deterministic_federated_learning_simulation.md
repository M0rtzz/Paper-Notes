...
description: >-
  [CVPR 2026 (Workshop: FedVision)][优化/理论][联邦学习仿真] 提出 BlazeFL，一个基于 Python free-threading 的轻量级单机联邦学习仿真框架，通过共享内存执行和客户端隔离 RNG 流实现最高 3.1× 加速与比特级可复现。 领域现状：联邦学习（FL）研究普遍依赖单…
tags:
...
BlazeFL 把整个联邦学习仿真塞进单个进程里跑，靠的是 Python 3.14 的 free-threading（移除 GIL 后线程能真并行）。主线程扮演协调者，把一整轮 FL 循环串起来：从客户端池里采样、把全局参数下发、让一批工作线程并行做本地训练、收集回训练结果、在服务器端聚合、最后评估全局模型。关键在于工作线程和主线程共享同一块地址空间，所以服务器和客户端之间交换参数不再需要序列化、管道通信或外部对象存储——下行包（downlink package）直接被线程从内存里读走，训练输出也原路写回。两项核心机制嵌在这条循环上：**共享内存执行**管下行下发与聚合写回这两处参数交换，**确定性随机数管理**则管「每客户端独立 RNG 流」和「按固定采样顺序而非完成顺序收集结果」两个确定性关口。此外框架同时实现了 free-threaded 和 process-based 两种共享内存模式，后者专门留作对照实验，用来把「执行模型」和「随机数控制」两项贡献拆开度量。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["主线程协调者<br/>单进程 free-threading"] --> B["客户端采样<br/>按确定性种子计划"]
    B --> C["共享内存执行：下发全局参数<br/>线程直读内存, 免序列化"]
    C --> D["工作线程并行本地训练<br/>每客户端独立 RNG 流"]
    D --> E["确定性随机数管理：固定顺序收集<br/>按采样列表序, 而非完成序"]
    E --> F["共享内存执行：FedAvg 聚合<br/>结果原路写回内存"]
    F --> G["评估全局模型<br/>记录逐轮 SHA-256 哈希"]
    G -->|未达通信轮数| B
    G -->|完成| H["输出: 比特级可复现的训练轨迹"]
```

> 协议式接口（设计 3）是与这条数据流正交的工程选择——它决定「用户的 PyTorch 组件如何接入框架」，不占用流程里的某个节点，故未画进图中。
