...
description: >-
  [CVPR 2026][目标检测][异常检测] 提出 InvAD，将扩散模型异常检测从"RGB 空间去噪重建"范式转变为"潜空间加噪反演"范式，通过 DDIM 反演直接推断最终潜变量并在先验分布下度量偏差来检测异常，仅需 3 步反演即达 SOTA 性能且推理速度提升约 2 倍。
tags:
  - "CVPR 2026"
  - "目标检测"
  - "异常检测"
...
这篇论文要解决的是扩散模型异常检测里一个绕不开的矛盾：靠"去噪重建"来找异常，既要小心调噪声强度，又要跑很多步去噪，慢且难调。InvAD 换了个方向——既然扩散模型只学过正常数据的分布，那就不重建，直接把图像"加噪反演"到潜空间，看它落在先验分布的什么位置。具体地，输入图像先经骨干网络抽成特征 $\mathbf{z} = g_\phi(\mathbf{x})$，再用 DDIM 沿确定性轨迹只反演 3 步得到终态潜变量 $\mathbf{z}_T$；正常图像会被推到先验分布的高密度区，异常则落到低密度区，于是异常分数就由 $\mathbf{z}_T$ 偏离典型分布的程度算出来。整条管线没有解码器、不做任何重建。

### 关键设计

**1. DDIM 反演加噪：把"检测"从去噪重建挪到加噪反演。**
...
**2. 特征空间扩散建模：在骨干特征而非原始像素上做扩散。**

直接在像素空间扩散，既受低级噪声/光照干扰，分辨率又高、推理慢。InvAD 改用预训练 EfficientNet-B4 抽出的特征 $\mathbf{z} = g_\phi(\mathbf{x}) \in \mathbb{R}^{C \times h \times w}$ 作为扩散模型的输入空间。这样做有两重好处：骨干特征对低级变化天然具有不变性，反演结果更稳定；特征图分辨率更低，反演单步更省算力。扩散主干用的是 DiT-gigant。消融里"像素空间 + 单步"只有 44.9 mAD、换到特征空间立刻跳到 71.0，说明换空间这一步对极少步数下的稳定性是关键。

**3. 混合异常评分：用终态潜变量的范数加 min-max 差异度量偏差。**

有了反演终态 $\mathbf{z}_T$ 还需要把"偏离先验"翻译成分数。像素级异常图取通道维度的欧氏范数 $\mathbf{z}_T^{\text{normed}}[i,j] = \|\mathbf{z}_T[:,i,j]\|_2$，范数越大说明该位置越偏离典型分布。图像级分数则在异常图 $A$ 上用 $s = \max(A) - \min(A) + \sum_{u,v} A[u,v]$。引入 max-min 差异是为了缓解反向评分问题：异常通常局部稀疏分布，单看总和容易被全局偏移淹没，而极值差能把局部尖锐的偏离凸显出来。实验显示这种 NLL + Diff 的混合评分对反演步数 $S$ 鲁棒，单独用任一项都会随步数波动。
