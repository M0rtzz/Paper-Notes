...
description: >-
  [CVPR 2026][可解释性][concept bottleneck model] 提出TextUnlock方法，通过训练轻量MLP将任意冻结视觉分类器的特征投射到文本嵌入空间（同时保持原分类器分布不变），无需CLIP、无需标注、无需训练线性探针，即可将任何legacy分类器转化为可解释的概念瓶颈模型——在40+架构上测试，超越甚至有监督的CLIP基CBM。
tags:
  - "CVPR 2026"
  - "可解释性"
...
这篇要解决的是：能不能不依赖 CLIP、不用任何标注、也不改原分类器的推理，就把一个冻结的视觉分类器变成可解释的概念瓶颈模型（CBM）。做法两步走——先用 **TextUnlock** 训练一个 MLP，把视觉特征映射到文本编码器空间，监督信号是原分类器的 softmax 分布（不是 GT 标签）；再冻结这个 MLP，用文本编码器编码概念集得到概念嵌入矩阵 $C$，靠余弦相似度算概念激活，而概念到类别的权重 $W_{con}$ 直接由 $C \cdot U^T$（概念-类名的文本相似度）无监督推出来。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    A["冻结视觉分类器<br/>→ 视觉特征 f"] --> B["TextUnlock 分布对齐<br/>MLP 映射到文本空间，用原 softmax 蒸馏"]
    B --> D["概念激活<br/>映射特征 f̃ 与概念嵌入 C 算余弦相似度"]
    CC["文本编码器编码概念集<br/>→ 概念嵌入 C"] --> D
    D --> E["无监督概念→类别映射<br/>W_con = C·Uᵀ（文本-文本检索）"]
    E --> F["CBM 预测<br/>S_cn = f̃·CᵀC·Uᵀ"]
    G["推理时即时构建<br/>换概念集无需重训"] -.->|重新编码概念集| CC
```

### 关键设计

**1. TextUnlock 分布对齐：用原分类器的 softmax 而非标签来蒸馏，保住原推理逻辑**

现有 CBM 要么把 CLIP 的 bias 引进来，要么需要标注。TextUnlock 改成一种特殊蒸馏：损失 $\mathcal{L} = -\sum_i^K o_i \log \frac{e^{s_i}}{\sum_j e^{s_j}}$ 里，$o$ 是原分类器的 softmax 输出（不是 GT），$s$ 是 MLP 映射后特征与文本类名嵌入的余弦相似度。它蒸馏的不是"大模型→小模型"，而是把分类器的离散分布蒸馏到它自己的视觉-语言对应分布上。好处是三点叠加：不需要任何标签、原分类器精度几乎不动（平均仅降 ~0.2%）、对任何架构都适用。

**2. 无监督概念到类别映射：在同一文本空间里做文本-文本检索，省掉线性探针**

传统 CBM 还要训一个线性探针 $W_{con}$ 把概念接到类别。本文注意到概念嵌入 $C$ 和类名嵌入 $U$ 本就在同一文本空间，于是直接 $W_{con} = C \cdot U^T$。最终 CBM 输出 $S_{cn} = (\tilde{f} \cdot C^T) \cdot (C \cdot U^T) = \tilde{f} \cdot C^T C \cdot U^T$，相当于原分类器 $\tilde{f} \cdot U^T$ 被概念的 Gram 矩阵 $C^TC$ 缩放——当概念集足够完备使 $C^TC = I$ 时，它干脆退化回原分类器，这也解释了为何可解释化几乎不损精度。

**3. 推理时即时构建 CBM：换概念集不用重训任何组件**

因为概念到类别的映射是即算的文本相似度，概念集可以在推理时任意切换，只需重新编码新概念集即可，无需重训 MLP 或任何模块。这种 plug-and-play 的灵活性正是 CLIP 基方法做不到的。

### 训练策略
- MLP：3 层（$n \to 2n \to 2n \to m$），LayerNorm + GELU + Dropout(0.5)
- 文本编码器：MiniLM (all-MiniLM-L12-v1)，$m=384$
- 概念集：20K 最常用英语单词，经严格过滤（去除类名、同义词、父类等）
- 训练：Adam lr=1e-4，cosine decay，单张 RTX 2080 Ti
