...
flowchart TD
    A1["参考视图"] --> B
    A2["目标视图"] --> B
    B["冻结 ViT 骨干（DINOv3，参数不更新）<br/>各自抽 patch 级特征"]
    B --> C["Transformer 交叉注意力解码器<br/>自注意力精炼单视图 + 交叉注意力对齐两视图 + 2D RoPE"]
    C --> D["MLP 预测头<br/>多输出表示（四元数 / 欧拉角 / 旋转矩阵）"]
    D --> E["后处理<br/>四元数归一化 / 旋转矩阵 SVD 正交化"]
    E --> F["相对位姿 T_rel"]
    F -->|"参考-相对公式化：T_v2 = T_v1 · T_rel"| G["目标视图绝对位姿"]
    C -.->|"仅训练时"| H["双向预测：反向相对位姿<br/>正反互逆 → 一致性监督"]
```
