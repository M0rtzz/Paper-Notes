...
SPAR 想解决的核心矛盾是：滑窗推理精度高但慢 52 倍，单次推理快但精度差，二者似乎不可兼得。它的破局思路是把"贵但好"的滑窗结果当成监督信号，蒸馏给"便宜"的单次模型——既不改架构，也不要像素标注。整条流水线分两步：先让一个冻结的教师（VLM 视觉编码器在滑窗模式下跑）对图像产出高质量的密集特征图，再用一个同架构的学生对整张图单次前向，让它的输出去逼近教师的特征图。训练只是一个特征回归，推理时把教师丢掉、只留学生，于是得到一个"看一眼就给出滑窗级特征"的分辨率无关编码器。
