...
**4. 全量微调 vs LoRA：差距来自表征能力，而非算力取舍**

确定了"该微调"之后，还要回答"用哪种微调"。常规直觉把全量微调（FFT）与 LoRA 的选择当成一笔算力账：LoRA 省参数、省显存，性能接近就该选它。作者把两者放在 0.1%–100% 的同一条数据预算曲线上比，却发现全量微调始终高出 0.04–0.07 mIoU，而且这个差距**在所有预算下几乎恒定**——既不随数据增多而收窄，也不在低数据区被放大。差距恒定恰恰说明它不是数据效率问题（否则应随预算变化），而是表征容量问题：LoRA 只在解码器注意力的几个投影矩阵（$W_q,W_k,W_v,W_o$）里注入低秩更新、骨干冻结，能调整的表征子空间有限；面对薄云、云影这类光谱模糊类别所需的较大表征改造，低秩更新够不着，因此差距在难类上最明显。结论是：FFT 与 LoRA 之争不是"算力换性能"，而是由任务的表征需求（光谱模糊程度）决定的——难类多就得给模型更大的表征调整空间。
