...
具体地，云端用一个 2D ViT（EfficientViT-L1）逐帧提特征，把最近 $K{=}4$ 帧时序拼接后加上一个"延迟嵌入"，送进 3D ViT 编码器，再经自适应空间池化 + 通道瓶颈（DR-AE）压缩后下行传输；本地端用 CNN2D + CoAt2D 处理 704×480 的当前帧，把对齐后的云端特征**逐元素加到** CNN2D 的中间输出上，再做分割。核心关系可以写成 $\hat{y}_t = f_{\text{local}}(x_t,\, z_{t-\tau})$：本地吃当前帧 $x_t$，融合的是 $\tau$ 帧之前发出、为预测当下而生的云端特征 $z_{t-\tau}$。
