...
本文不动架构，只动损失。具体做法：给现有 TAS 模型（MS-TCN、C2F-TCN、FACT 都行）的输出端加**一个**类别无关的边界通道，然后在原有分类损失之外挂两个辅助损失——边界回归损失 $\mathcal{L}_B$ 管「边界对不对齐」，CDF 段形状正则化 $\mathcal{L}_{CDF}$ 管「段内连不连贯」。关键是这两个损失被**按时序区域分开**：边界损失只在转换点附近算，段正则只在段内部算，互不打架。推理时这个边界通道直接弃用，只取原始分类输出，所以零额外开销。
...
这个形式的灵感来自一维 Wasserstein 距离正好等于两个 CDF 之差的积分 ⚠️ 以原文为准，但这里没有真去算最优传输，而是把它当成一个简单的段级形状约束直接监督。效果上，它逼着预测在每个段内呈现平滑、单调递增的累积曲线，相当于告诉模型「这一整段应该是同一个动作、别在中间断开」，从形状层面压住碎片化。
