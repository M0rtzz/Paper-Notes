...
现有无人机数据集既缺细粒度 mask 标注，又没有推理导向的文本监督，撑不起系统性的推理分割研究。DRSeg 用四阶段半自动流程补上这块：先人工筛出场景复杂的高分辨率图像，再由 SAM2 生成粗 mask、人工精修，接着用 GPT-5 结合图像、mask 和类别生成覆盖三个推理维度的 QA 对（含 CoT 推理链），最后人工审核把关。⚠️ GPT-5 等模型名以原文为准。整套数据按空间 / 属性 / 场景三个推理维度均匀分布（各约 33.3%），分别对应位置关系、视觉状态和全局上下文，最终得到 10K 张图像及配套的链式推理标注。
