...
它的闭式解就是交叉协方差 $K_c^\top Q_c$ 做 SVD 后的正交因子 $UV^\top$。把 $R^*$ 作用到去中心化的 key 上，在同一个注意力块里重新算一遍注意力分数和输出。之所以选正交映射（而不是任意线性变换），是因为正交旋转只改方向、不改局部幅值，修的恰好是 patch 特征在 query 子空间里的方向性一致性——这正是余弦相似度看重的东西。代价也很低，每个 head 只多一个 $d \times d$ 的 SVD 和两次 $N \times d$ 矩阵乘；若想完全避开 SVD，还能用 Newton-Schulz 迭代近似出正交因子。
...
**3. 滑动窗口推理与融合：把大图切成 patch 分辨率够用的小块。**

ViT 的 patch 尺寸是固定的（如 16×16），整张高分辨率图一次喂进去，patch 级的空间分辨率就不够，细节会糊。PEARL 用重叠窗口把图盖一遍，每个窗口独立跑完上面的 PA + TLP 得到自己的上采样 logit，再按重叠权重融合回全图坐标系。重叠区的加权融合既保住了每块的 patch 级细节，又让相邻块的拼缝平滑过渡。
