...
要在保证因果的前提下不被串行延迟拖垮，三个阶段被拆进三条线程（TME/TMP/TMC）异步流水，靠 FIFO 共享队列传数据，整条管线的吞吐由最慢的那一级决定。这把原本累加的串行耗时 $t_{est} + t_{prop} + t_{smooth}$ 压成了三者的最大值，理论加速比为 $S = (t_{est} + t_{prop} + t_{smooth}) / \max\{t_{est}, t_{prop}, t_{smooth}\}$，并通过队列的 back-pressure 机制避免内存爆掉。
