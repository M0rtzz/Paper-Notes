...
RHO要解决的是「在恶劣天气和传感器噪声下，仅凭一张地面图和一块OpenStreetMap瓦片，把相机定位到米级位置和度级朝向」。它没有沿用单一图像源，而是同时吃下360°全景和120°针孔两路输入，做成一个双分支的Pin-Pan架构。两路各自走「编码 → BEV投影 → 与OSM特征图匹配」的流程，分别得到一个覆盖位置和朝向的三维概率体 $(u, v, \theta)$；全景分支在编码前多了一道SUM去畸变手续，最后由POF模块把两个概率体融成一个，读出最终的3-DoF相机位姿。之所以要两路并行，是因为全景和针孔在「定位」和「定朝向」上各有所长（见下文POF），单独用哪一路都偏科。
