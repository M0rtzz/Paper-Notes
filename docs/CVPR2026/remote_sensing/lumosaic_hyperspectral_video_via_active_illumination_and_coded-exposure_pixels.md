...
Lumosaic 想在一帧普通的彩色曝光时间里，同时把空间、光谱、时间三个维度都采下来，从而得到运动鲁棒的实时高光谱视频。硬件端用 12 个窄带 LED（20–30nm FWHM，Lumileds Luxeon C）做可编程主动光源，配一台 VGA 的编码曝光像素（CEP）相机（640×480，12500 子帧/秒），由 ESP32 微控制器在微秒级把"哪一刻点哪个 LED"和"哪些像素此刻在曝光"对齐起来。每帧切成 $S=158$ 个子帧（每子帧 170µs），约 27ms 积分加约 6ms 读出/同步，正好凑成 30fps。采到的原始帧先做光谱去马赛克拆出 12 个 LED 子图像，再用 RIFE 光流把各子图像对齐到同一时刻，最后送进 HAN 网络重建出 31 通道（400–700nm）的高光谱视频。
