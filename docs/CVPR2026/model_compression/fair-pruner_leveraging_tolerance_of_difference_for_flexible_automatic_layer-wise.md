...
flowchart TD
    A["预训练模型 + 少量标注样本<br/>(约 640 个即可算稳 U-Score)"] --> B["U-Score<br/>Wasserstein 类条件可分性<br/>→ 低分=冗余单元(该删)"]
    A --> C["R-Score<br/>一阶 Taylor 损失敏感度<br/>→ 高分=关键单元(要护)"]
    B --> D["ToD 逐层扫描候选数 m<br/>移除集 ∩ 保护集 / m<br/>取 ToD ≤ α 的最大 m"]
    C --> D
    D --> E["按各层 m̂ 删除<br/>U-Score 最低的单元"]
    E --> F["微调恢复精度<br/>换压缩率只需调 α 重扫 m"]
```
