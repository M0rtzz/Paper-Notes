...
这篇论文要回答两个问题：基于反向信道编码（RCC）的零样本扩散压缩为什么天然抗比特翻转，以及怎么把这种抗错能力再往上推一截。它不动模型也不重新训练，整条 pipeline 沿用 DDCM / Turbo-DDCM——编码器在反向扩散的每一步从一个固定码本里挑原子来引导去噪轨迹逼近目标图像，被挑中的原子索引序列就是压缩表示；解码器拿同一份码本和同一串索引，重跑一遍确定性的反向扩散就还原出图像。论文先把 RCC 抗错的根因讲清楚，再指出 Turbo-DDCM 在编码协议上留了个漏洞，最后只改这一处编码方式就换来数量级的鲁棒性提升。
