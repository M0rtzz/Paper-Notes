...
整条管线有三个组件：冻结的图像编码器 $F_I$（DINOv2-b）负责出图像特征，掩码边缘编码器 $F_E$ 把"已知的那部分边缘图"编码进来，共享边缘解码器 $D$ 再融合两者预测被遮挡像素的边缘概率。训练分两阶段：先在 40 万张 SAM 合成的边缘图上预训练 $F_E$ 和 $D$，再用 LoRA 适配器在下游数据集（BSDS 等）上微调，只增加约 1.2% 参数量。
...
**3. Classifier-Free Guidance 改造的多粒度预测：一个参数调边缘密度。**
