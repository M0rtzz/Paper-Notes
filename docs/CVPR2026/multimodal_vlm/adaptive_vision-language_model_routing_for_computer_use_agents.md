...
description: >-
  [CVPR 2026][多模态VLM][Computer Use Agent] 提出 AVR 自适应路由框架，通过轻量多模态嵌入模型评估动作难度 + 小模型 logprob 置信度探测 + warm agent 记忆注入，实现三层路由（简单→小模型，困难→大模型，高风险→大模型+guardrail），在推理成本降低 78% 的同时仅损失 2pp 准确率。
tags:
  - "CVPR 2026"
  - "多模态VLM"
  - "Computer Use Agent"
...
flowchart TD
    A["截图 + 指令"] --> B["多模态难度评估器<br/>120M 模型估难度 d"]
    B -->|明显简单| S["小 VLM 直接执行"]
    B -->|其余动作| C["Logprob 置信度探测<br/>小 VLM 生成动作，算置信度 C"]
    C -->|"C > θ_high"| S
    C -->|"C ≤ θ_low"| L["升级大 VLM"]
    C -->|中等置信| M["Warm Agent 记忆注入<br/>检索 top-k 经验重生成，再探 C"]
    M -->|够信| S
    M -->|仍不足| L
    L -->|高风险动作| G["Guardrail 验证"]
    L -->|普通动作| O["执行动作"]
    S --> O
    G --> O
```
