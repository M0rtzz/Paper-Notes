...
整条流水线是：从 26 个不同领域的数据集里筛出约 218K 张图像-问题对，每一对都被设计成只考一种 AVA；接着把待测 VFM 接上一个固定的 LLM，按 LLaVA 式两阶段（先预训练 connector、再 LoRA 微调）在每个 AVA 上分别训练和测试；最后把 14 个分数汇成这个 VFM 的"能力指纹"，谁强在哪、弱在哪一目了然。
