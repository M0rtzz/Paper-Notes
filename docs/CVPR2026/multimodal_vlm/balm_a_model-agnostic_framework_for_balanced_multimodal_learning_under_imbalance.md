...
### 整体框架
BALM 要解决的是「不均衡缺失率（IMR）」下的多模态学习——不同模态以不同概率缺失，这件事同时带来两个麻烦：频繁缺失的模态特征分布被扭曲，融合时拖累整体；而且梯度被高可用模态垄断，低可用模态欠拟合。BALM 的做法是在任意骨干网络上挂两个互补的即插即用模块，从表征层和优化层两头同时再平衡。数据先过**特征校准模块（FCM）**——在进编码器之前，用所有可用模态的全局统计把各模态被缺失扭曲的特征拉回正常分布；loss 反传时再由**梯度再平衡模块（GRM）**介入，调制各模态梯度的量级和方向。整个过程不改骨干架构、也不试图重建缺失模态。
