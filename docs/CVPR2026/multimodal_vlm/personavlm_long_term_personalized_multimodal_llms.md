...
**2. 人格演化机制（PEM）：用余弦衰减的动量更新让人格"先快后稳"地漂移。**

静态人格假设处理不了"用户一开始表现外向、聊久了又流露内向"这种渐变。PEM 维护一个长期人格向量 $\mathbf{p} \in \mathbb{R}^5$，每轮先从当前交互推断一个瞬时人格 $\mathbf{p}'_m$，再用指数移动平均把它融进长期向量：

$$\mathbf{p}_m \leftarrow \lambda \cdot \mathbf{p}_{m-1} + (1-\lambda) \cdot \mathbf{p}'_m$$

关键不在 EMA 本身，而在 $\lambda$ 走余弦衰减调度：对话早期 $\lambda$ 小，新观测权重大、人格快速适应；交互攒多了之后 $\lambda$ 增大，长期画像权重大、不再被单轮波动带偏。这等于把"冷启动时多学、稳定后少动"的直觉写进了调度曲线，省去了手调学习率。更新完的数值向量会被翻译成一段文字描述，再喂给生成阶段当条件。

**3. 两阶段训练（SFT + GRPO）：先学会管记忆，再学会策略性地检索。**

仅靠监督微调，模型学得会记忆的读写格式，却学不会"该不该查、查什么、从哪个时间段查"这类决策。所以训练分两步：SFT 阶段用 78K 合成样本打底，把记忆管理和多轮推理的基本能力灌进去；RL 阶段再用 GRPO 强化推理，强制输出遵循 `<think>` → `<retrieve>/<answer>` 的结构，让模型在思考后显式决定是继续检索还是直接作答。奖励函数

$$r_i = f_{\text{acc}} \cdot f_{\text{cons}} + 0.5 \cdot f_{\text{format}}$$

把准确性 $f_{\text{acc}}$ 和推理一致性 $f_{\text{cons}}$ 相乘（两者都要好才得高分），再加半权的格式合规项 $f_{\text{format}}$。训练数据由 PersonaHub 生成 500 个多样化用户画像、模拟出 30K+ 长期多模态交互，正好补上个性化训练数据稀缺的缺口。
