...
MUPO 是 GRPO 的即插即用替代品，要治的是 GRPO 的"多样性坍缩"——训练几步后所有响应都挤到同一条推理策略上。它的思路一句话概括：把一道题的 $K$ 个响应分成几组，**组内**各自局部估计优势（保证每条策略都被认真优化 = 深度），**组间**加一个多样性奖励把不同组推开（保证维持多种策略 = 广度）。下面两个设计分别对应"为什么分组"和"为什么要显式推开"。

### 关键设计

**1. 多组局部化优势估计：别让一条策略淹没所有信号**

GRPO 在全部 $K$ 个响应上算一个全局优势基线，结果是少数高奖励策略拿到极大优势值，其它策略的更新信号被压没了——这正是坍缩的根源。MUPO 把 $K$ 个响应分成 $G$ 组，**每组独立计算优势**。这样每组都能维护自己的"组内最优"，不被全局主导策略碾压。直觉上，每个组是一种推理策略的独立试验田，互不抢信号。

**2. 多样性奖励：把组与组真正推开**

光分组还不够——若不加约束，各组可能仍收敛到相似策略。于是在准确率 + 格式奖励之外，再加一项**多样性奖励**：衡量不同组的推理嵌入距离，组间距离越大奖励越高，逼着不同组代表真正不同的推理路径。这一项是"分离"的显式激励，把"深度（组内充分优化）+ 广度（组间保持差异）"拼成发散思维的完整定义——不是简单生成不同答案，而是用不同方法思考同一题。

### 一个完整 walkthrough（解一道几何题，K=8、G=4）

1. **采样**：模型对同一题 rollout 出 8 条响应，按 2 条一组分成 4 组。
2. **组内优势**：组 A 两条都走"坐标法"、组 B 走"相似三角形"、组 C 走"面积法"、组 D 走"辅助线"。每组各自算优势——即便"坐标法"全局奖励最高，组 B/C/D 仍按各自组内基线获得正常更新信号，不被坐标法淹没。
3. **组间多样性**：计算 4 组推理嵌入两两距离，距离大 → 多样性奖励高；若组 C 和组 D 思路趋同（嵌入贴近），该奖励变小，逼它们分化。
