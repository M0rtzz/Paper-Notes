...
BiCLIP 极度简单：冻结 CLIP/SigLIP 的双编码器，在图像特征和文本特征的点积之间插入一个可学习的变换矩阵 $W \in \mathbb{R}^{D \times D}$。相似度从 $S = it^\top$ 变为 $S^{bi} = (iW)t^\top$。用标准对比损失/Sigmoid 损失训练 $W$，仅需少量 epoch。整条 pipeline 是一个**双路汇合**结构：图像特征经 $W$ 几何对齐、文本特征原样保留，二者在双线性相似度处汇合，损失只回传更新 $W$ 这一个矩阵——下面三个关键设计都落在这个唯一的 $W$ 节点上（$W$ 本身、它的结构约束、它的初始化）。

```mermaid
%%{init: {'flowchart': {'rankSpacing': 24, 'nodeSpacing': 28, 'padding': 6, 'wrappingWidth': 400}}}%%
flowchart TD
    IMG["输入图像"] --> IENC["冻结图像编码器<br/>图像特征 i"]
    TXT["类别文本"] --> TENC["冻结文本编码器<br/>文本特征 t"]
    IENC --> W["双线性特征变换 W<br/>上三角结构约束 + 单位矩阵初始化"]
    TENC --> SIM["双线性相似度<br/>S = (iW)·tᵀ"]
    W --> SIM
    SIM --> LOSS["对比 / Sigmoid 损失<br/>仅更新 W"]
    LOSS -.梯度.-> W
```
