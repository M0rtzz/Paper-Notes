...
### 整体框架
这篇论文想解决一件听起来很简单、却让 SOTA VLM 集体翻车的事：看一张真实照片里的模拟时钟，报出现在几点几分。作者把问题拆成数据和对齐两条线。数据这条线先造一个覆盖真实场景的 TickTockVQA 数据集，用它做标准监督微调（SFT），把"怎么读时钟"这件事的基本能力灌进模型；对齐这条线则在 SFT 之后再叠一层 Swap-DPO，专门去纠正模型最爱犯的"把时针当分针读"的错误。最后配一套 Swap-Equivalence 评估口径，避免把语义等价的读法误判成错。
