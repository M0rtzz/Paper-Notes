...
框架包住的基础检索器本身也是论文单独训的一块——HIVE-Retriever：基于 Qwen3-Embedding-4B，在合成的困难负样本上做对比训练，取 EOS token 最后一层隐状态当嵌入、用余弦相似度检索；遇到多模态查询时先用 GPT-4o 把图像转成描述 $\delta(q_v)$，再和文本 $q_t$ 拼起来喂进去。它单独就已经把纯文本 SOTA（DiVeR 32.2）顶到 33.2，而四阶段框架在它之上再加到 41.7。
