...
**2. PaddleOCR-VL-0.9B：识别模块只面对裁剪小块，所以能用很小的模型**

既然有效区域已经被 VRFM 框出来并裁好，精细识别这一步就不必再处理整页。PaddleOCR-VL-0.9B 是一个仅 0.9B 参数的紧凑视觉语言模型，每次只吃一个裁剪区域，视觉 token 数因此大幅下降，却仍要覆盖文本、公式、表格、图表等多种元素以及 109 种语言。关键在于解耦带来的"减负"：模型不用在一张大图里既找元素又认内容，只需在已经定位好的小块上专心识别，于是更小的参数量反而能达到更好的效果——这正是论文"聪明地选择在哪里投入计算"思路的落点。
