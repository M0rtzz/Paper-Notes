...
description: >-
  [ICML2025][LLM 其他][过参数化集成] 在过参数化条件下，无限集成模型与单个无穷宽模型逐点等价，集成方差不再反映传统贝叶斯不确定性而是衡量增加模型容量的预期效果，从理论上解释了深度集成相比大模型无本质泛化优势的经验观察。 经典集成学习（bagging、boosting、随机森林等）在欠参数化：模型上表现出色：通…
tags:
  - "ICML2025"
  - "LLM 其他"
  - "过参数化集成"
