...
description: >-
  [ICML 2025][联邦学习] FedTAIL 提出了一个联邦域泛化框架，通过梯度一致性正则化、逐类锐度感知最小化和曲率感知动态加权三个模块，同时解决域偏移和长尾类别不平衡的双重挑战，在多个基准上达到 SOTA。 领域现状：域泛化（DG）旨在训练能泛化到未见目标域的模型。锐度感知最小化（SAM）通过寻找平坦极小值来改善泛…
tags:
  - "ICML 2025"
  - "联邦学习"
