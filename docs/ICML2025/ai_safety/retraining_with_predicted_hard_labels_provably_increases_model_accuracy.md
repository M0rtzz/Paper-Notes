...
description: >-
  [AI安全] 在噪声标签场景下，用模型自身预测的硬标签（0/1）对训练集重新标注并重训练，可以可证明地：提升分类精度；进一步提出共识筛选策略（仅对预测标签与给定标签一致的样本重训练），在标签差分隐私训练中无额外隐私代价即可大幅提升性能。 标签差分隐私（Label DP）通过向标签注入噪声来保护隐私，但随之而来的标签噪声会显著…
tags:
  - "AI安全"
---
