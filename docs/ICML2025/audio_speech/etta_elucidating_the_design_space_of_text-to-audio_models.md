...
description: >-
  [ICML 2025][音频/语音][文本到音频] ETTA 通过大规模系统性实验阐明了文本到音频(TTA)模型的设计空间（数据、架构、训练目标、采样策略），并基于分析结论构建了当前公开数据下最优的 TTA 模型。 领域现状：文本到音频(TTA)合成近年取得显著进展，用户可以用自然语言 prompt 生成音频，丰富创意工作流…
tags:
  - "ICML 2025"
  - "音频/语音"
  - "文本到音频"
