...
description: >-
  [ICML 2025][物理/科学计算][LLM微调] 提出L2D微调方法，将预训练LLM视为单步扩散模型，引入并行扩散路径实现多步推理缩放，不修改原始权重即可随推理步数增加获得单调递增的准确率，在4个LLM上的数学/编码/推理任务上取得一致提升。 领域现状：自回归LLM在语言领域取得巨大成功，但本质上缺乏按需缩放推理计算的…
tags:
  - "ICML 2025"
  - "物理/科学计算"
  - "LLM微调"
