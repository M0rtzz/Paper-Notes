...
description: >-
  [ICML 2025][图像生成][对称流形] 提出一种高效的对称流形（环面、球面、SO(n)、U(n)）扩散模型框架，通过欧几里得布朗运动的投影和Itô引理绕过热核计算，将训练复杂度从指数级降至近线性，并提供多项式级采样精度保证。 许多应用（机器人学、药物发现、量子物理）的数据天然约束在非欧流形上，如环面 $\mathbb…
tags:
  - "ICML 2025"
  - "图像生成"
  - "对称流形"
